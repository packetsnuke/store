...
class WC_PCSVIS_Product_Variation_Import extends WC_PCSVIS_Product_Import {
	...
	public function process_product( $post ) {
		...
		if ( empty( $post['post_parent'] ) ) {
			$this->add_import_result( 'skipped', __( 'No product variation parent set', 'ultimatewoo-pro' ), $processing_product_id, 'Not set', $post['sku'] );
			WC_Product_CSV_Import_Suite::log( __('> Skipping - no post parent set.', 'ultimatewoo-pro') );
			return;
		}

		if ( ! empty( $processing_product_id ) && isset( $this->processed_posts[ $processing_product_id ] ) ) {
			$this->add_import_result( 'skipped', __( 'Product variation already processed', 'ultimatewoo-pro' ), $processing_product_id, get_the_title( $post['post_parent'] ), $post['sku'] );
			WC_Product_CSV_Import_Suite::log( __('> Post ID already processed. Skipping.', 'ultimatewoo-pro') );
			return;
		}

		if ( isset( $post['post_status'] ) && 'auto-draft' === $post['post_status'] ) {
			$this->add_import_result( 'skipped', __( 'Skipping auto-draft', 'ultimatewoo-pro' ), $processing_product_id, get_the_title( $post['post_parent'] ), $post['sku'] );
			WC_Product_CSV_Import_Suite::log( __('> Skipping auto-draft.', 'ultimatewoo-pro') );
			return;
		}
		...
		if ( ! $post_parent_exists ) {
			$this->add_import_result( 'failed', __( 'Variation parent does not exist', 'ultimatewoo-pro' ), $processing_product_id, 'Does not exist', $post['sku'] );
			WC_Product_CSV_Import_Suite::log( sprintf( __('> Variation parent does not exist! (#%d)', 'ultimatewoo-pro'), $post_parent ) );
			return;
		}

		// Check post type to avoid conflicts with IDs
		if ( $merging && get_post_type( $processing_product_id ) !== 'product_variation' ) {
			$this->add_import_result( 'skipped', __( 'Post is not a product variation', 'ultimatewoo-pro' ), $processing_product_id, 'Not a variation', $post['sku'] );
			WC_Product_CSV_Import_Suite::log( sprintf( __('> &#8220;%s&#8221; is not a product variation.', 'ultimatewoo-pro'), $processing_product_id ), true );
			unset( $post );
			return;
		}
		...
		if ( $merging ) {
			...
			WC_Product_CSV_Import_Suite::log( sprintf( __('> Merging post ID %s.', 'ultimatewoo-pro'), $post_id ) );
			...
			if ( sizeof( $postdata ) ) {
				if ( wp_update_post( $postdata ) ) {
					...
					WC_Product_CSV_Import_Suite::log( __( '> Failed to merge post data: ', 'ultimatewoo-pro' ) . print_r( $postdata, true ) );
				}
			}
			...
			if ( $this->variation_exists( $post_parent, $processing_product_id, $processing_product_sku ) ) {
				$this->add_import_result( 'skipped', __( 'Variation already exists', 'ultimatewoo-pro' ), $processing_product_id, get_the_title( $post['post_parent'] ), $processing_product_sku );
				WC_Product_CSV_Import_Suite::log( sprintf( __( '> &#8220;%s&#8221; already exists.', 'ultimatewoo-pro' ), esc_html( $post['post_title'] ) ), true );
				unset( $post );
				return;
			}

			// Insert product
			WC_Product_CSV_Import_Suite::log( __('> Inserting variation.', 'ultimatewoo-pro') );
			...
			if ( is_wp_error( $post_id ) ) {
				...
				$this->add_import_result( 'failed', __( 'Failed to import product variation', 'ultimatewoo-pro' ), $processing_product_id, get_the_title( $post['post_parent'] ), $post['sku'] );

				WC_Product_CSV_Import_Suite::log( sprintf( __( 'Failed to import product &#8220;%s&#8221;', 'ultimatewoo-pro' ), esc_html($post['post_title']) ) );
				return;
		...
		if ( ! empty( $post['images'] ) ) {
			...
			if ( $merging ) {
				...
				WC_Product_CSV_Import_Suite::log( __( '> > Old images processed', 'ultimatewoo-pro' ) );
		...
		if ( $merging ) {
			...
			WC_Product_CSV_Import_Suite::log( sprintf( __('> Finished merging variation ID %s.', 'ultimatewoo-pro'), $post_id ) );
		} else {
			$this->add_import_result( 'imported', 'Import successful', $post_id, get_the_title( $post_parent ), $post['sku'] );
			WC_Product_CSV_Import_Suite::log( sprintf( __('> Finished importing variation ID %s.', 'ultimatewoo-pro'), $post_id ) );
		}
	...
	function import_start( $file, $mapping, $start_pos, $end_pos ) {
		WC_Product_CSV_Import_Suite::log( __( 'Parsing product variations CSV.', 'ultimatewoo-pro' ) );
		...
		WC_Product_CSV_Import_Suite::log( __( 'Finished parsing product variations CSV.', 'ultimatewoo-pro' ) );
	...
	function header() {
		...
		echo '<h2>' . ( empty( $_GET['merge'] ) ? __( 'Import Product Variations', 'ultimatewoo-pro' ) : __( 'Merge Product Variations', 'ultimatewoo-pro' ) ) . '</h2>';
	}
