...
class WC_PCSVIS_Product_Import extends WP_Importer {
		...
		switch ( $step ) {
			...
			case 1 :
				...
				if ( $this->handle_upload() )
					...
					_e( 'Error with handle_upload!', 'ultimatewoo-pro' );
			break;
			case 2 :
				$this->header();
				...
				if ( $file ) {
					...
					<table id="import-progress" class="widefat_importer widefat">
						<thead>
							<tr>
								<th class="status">&nbsp;</th>
								<th class="row"><?php _e( 'Row', 'ultimatewoo-pro' ); ?></th>
								<th><?php _e( 'SKU', 'ultimatewoo-pro' ); ?></th>
								<th><?php _e( 'Product', 'ultimatewoo-pro' ); ?></th>
								<th class="reason"><?php _e( 'Status Msg', 'ultimatewoo-pro' ); ?></th>
							</tr>
						</thead>
						<tfoot>
								...
								return $.ajax({
									...
									success:    function( response ) {
										...
										if ( response ) {
											...
											$('#import-progress tbody').append( '<tr class="error"><td class="status" colspan="5">' + '<?php _e( 'AJAX Error', 'ultimatewoo-pro' ); ?>' + '</td></tr>' );
										}
								...
								$.ajax({
									...
									success: function( response ) {
										if ( response !== Object( response ) || ( typeof response.success === "undefined" && typeof response.error === "undefined" ) ) {
											...
											response.error = "<?php printf( esc_js( __( 'The resize request was abnormally terminated (ID %s). This is likely due to the image exceeding available memory or some other type of fatal error.', 'ultimatewoo-pro' ) ), '" + id + "' ); ?>";
										}

										regen_count ++;

										$('#import-progress tbody .regenerating .progress').css( 'width', ( ( regen_count / attachments.length ) * 100 ) + '%' ).html( regen_count + ' / ' + attachments.length + ' <?php echo esc_js( __( 'thumbnails regenerated', 'ultimatewoo-pro' ) ); ?>' );
					...
					echo '<p class="error">' . __( 'Error finding uploaded file!', 'ultimatewoo-pro' ) . '</p>';
				}
			break;
			case 3 :
			...
			case 4 :
				...
				_e( 'Cleaning up...', 'ultimatewoo-pro' ) . ' ';

				wp_defer_term_counting( true );
				wp_defer_comment_counting( true );

				_e( 'Clearing transients...', 'ultimatewoo-pro' ) . ' ';
				...
				_e( 'Backfilling parents...', 'ultimatewoo-pro' ) . ' ';

				$this->backfill_parents();

				if ( ! empty( $this->upsell_skus ) ) {
					_e( 'Linking upsells...', 'ultimatewoo-pro' ) . ' ';
				...
				if ( ! empty( $this->crosssell_skus ) ) {
					_e( 'Linking crosssells...', 'ultimatewoo-pro' ) . ' ';
				...
				if ( 'woocommerce_variation_csv' === $this->import_page && ! empty( $this->processed_posts ) ) {
					...
					_e( 'Syncing variations...', 'ultimatewoo-pro' ) . ' ';
				...
				// SUCCESS
				_e( 'Finished. Import complete.', 'ultimatewoo-pro' );
	...
	public function import() {
		...
		WC_Product_CSV_Import_Suite::log( __( 'Processing products.', 'ultimatewoo-pro' ) );
		...
		WC_Product_CSV_Import_Suite::log( __( 'Finished processing products.', 'ultimatewoo-pro' ) );
	}
	...
	public function import_start( $file, $mapping, $start_pos, $end_pos ) {
		...
		WC_Product_CSV_Import_Suite::log( __( 'Parsing products CSV.', 'ultimatewoo-pro' ) );
		...
		WC_Product_CSV_Import_Suite::log( __( 'Finished parsing products CSV.', 'ultimatewoo-pro' ) );
	...
	protected function handle_initial_upload_file() {
		...
		if ( isset( $file['error'] ) ) {
			...
			throw new Exception( sprintf( __( 'Sorry, there has been an error: %s.', 'ultimatewoo-pro' ), $file['error'] ) );
		}
	...
	protected function handle_initial_path_file_check() {
		if ( ! $this->is_safe_path( ABSPATH, $_POST['file_url'] ) ) {
			...
			throw new Exception( sprintf( __( 'Sorry, there has been an error: path file must exist inside %s.', 'ultimatewoo-pro' ), ABSPATH ) );
		}

		$filepath = ABSPATH . $_POST['file_url'];
		if ( ! file_exists( $filepath ) ) {
			/* translators: placeholder is file path */
			throw new Exception( sprintf( __( 'Sorry, there has been an error: %s does not exist.', 'ultimatewoo-pro' ), $filepath ) );
		}
		...
		if ( ! $this->is_acceptable_csv_file( $filepath ) ) {
			...
			throw new Exception( sprintf( __( 'File must have .csv extension with acceptable mime types (%s)', 'ultimatewoo-pro' ), $mime_types ) );
		}
	...
	public function process_product( $post ) {
		...
		if ( ! empty( $processing_product_id ) && isset( $this->processed_posts[ $processing_product_id ] ) ) {
			$this->add_import_result( 'skipped', __( 'Product already processed', 'ultimatewoo-pro' ), $processing_product_id, $processing_product_title, $processing_product_sku );
			WC_Product_CSV_Import_Suite::log( __('> Post ID already processed. Skipping.', 'ultimatewoo-pro'), true );
			unset( $post );
			return;
		}

		if ( ! empty ( $post['post_status'] ) && $post['post_status'] == 'auto-draft' ) {
			$this->add_import_result( 'skipped', __( 'Skipping auto-draft', 'ultimatewoo-pro' ), $processing_product_id, $processing_product_title, $processing_product_sku );
			WC_Product_CSV_Import_Suite::log( __('> Skipping auto-draft.', 'ultimatewoo-pro'), true );
			unset( $post );
			return;
		}
		...
		if ( ! $merging ) {
			if ( $this->product_exists( $processing_product_title, $processing_product_sku, $post['post_name'] ) ) {
				$this->add_import_result( 'skipped', __( 'Product already exists', 'ultimatewoo-pro' ), $processing_product_id, $processing_product_title, $processing_product_sku );
				WC_Product_CSV_Import_Suite::log( sprintf( __('> &#8220;%s&#8221; already exists.', 'ultimatewoo-pro'), esc_html($processing_product_title) ), true );
				unset( $post );
				return;
			}
			if ( $processing_product_id && is_string( get_post_status( $processing_product_id ) ) ) {
				$this->add_import_result( 'skipped', __( 'Importing post ID conflicts with an existing post ID', 'ultimatewoo-pro' ), $processing_product_id, get_the_title( $processing_product_id ), '' );
				WC_Product_CSV_Import_Suite::log( sprintf( __('> &#8220;%s&#8221; ID already exists.', 'ultimatewoo-pro'), esc_html( $processing_product_id ) ), true );
				unset( $post );
				return;
			}
		}
		...
		if ( $merging && $processing_product_id && get_post_type( $processing_product_id ) !== 'product' ) {
			$this->add_import_result( 'skipped', __( 'Post is not a product', 'ultimatewoo-pro' ), $processing_product_id, $processing_product_title, $processing_product_sku );
			WC_Product_CSV_Import_Suite::log( sprintf( __('> &#8220;%s&#8221; is not a product.', 'ultimatewoo-pro'), esc_html($processing_product_id) ), true );
			unset( $post );
			return;
		}
		...
		if ( $merging ) {
			...
			WC_Product_CSV_Import_Suite::log( sprintf( __('> Merging post ID %s.', 'ultimatewoo-pro'), $post_id ), true );
			...
			if ( sizeof( $postdata ) > 1 ) {
				...
				if ( is_wp_error( $result ) ) {
					...
					WC_Product_CSV_Import_Suite::log( sprintf( __('> Failed to update product %s', 'ultimatewoo-pro'), $post_id ), true );
					unset( $post );
					return;
				} else {
					...
					WC_Product_CSV_Import_Suite::log( __( '> Merged post data: ', 'ultimatewoo-pro' ) . print_r( $postdata, true ) );
				}
			}
			...
			// Insert product
			WC_Product_CSV_Import_Suite::log( sprintf( __('> Inserting %s', 'ultimatewoo-pro'), esc_html( $processing_product_title ) ), true );
			...
			if ( is_wp_error( $post_id ) ) {
				$this->add_import_result( 'failed', __( 'Failed to import product', 'ultimatewoo-pro' ), $processing_product_id, $processing_product_title, $processing_product_sku );
				WC_Product_CSV_Import_Suite::log( sprintf( __( 'Failed to import product &#8220;%s&#8221;', 'ultimatewoo-pro' ), esc_html($processing_product_title) ) );
				unset( $post );
				return;
			} else {
				if ( version_compare( WC_VERSION, '3.0', '>=' ) ) {
				...
				WC_Product_CSV_Import_Suite::log( sprintf( __('> Inserted - post ID is %s.', 'ultimatewoo-pro'), $post_id ) );
			}
		}
		...
		if ( ! empty( $post['images'] ) && is_array( $post['images'] ) ) {
					...
					if ( in_array( $attachment_url, $post['images'] ) || in_array( $attachment_basename, $image_basenames ) ) {
						foreach( $post['images'] as $key => $image ) {
							if ( $image == $attachment_url || basename( $image ) == $attachment_basename ) {
								...
								WC_Product_CSV_Import_Suite::log( sprintf( __( '> > Image exists - skipping %s', 'ultimatewoo-pro' ), basename( $image ) ) );
			...
			if ( $post['images'] ) foreach ( $post['images'] as $image_key => $image ) {
				...
				WC_Product_CSV_Import_Suite::log( sprintf( __( '> > Importing image "%s"', 'ultimatewoo-pro' ), $image ) );
				...
				if ( ! is_wp_error( $attachment_id ) && $attachment_id ) {
					WC_Product_CSV_Import_Suite::log( sprintf( __( '> > Imported image "%s"', 'ultimatewoo-pro' ), $image ) );
					...
					WC_Product_CSV_Import_Suite::log( sprintf( __( '> > Error importing image "%s"', 'ultimatewoo-pro' ), $image ) );
			...
			WC_Product_CSV_Import_Suite::log( __( '> > Images set', 'ultimatewoo-pro' ) );
		...
		if ( $merging ) {
			...
			WC_Product_CSV_Import_Suite::log( sprintf( __('> Finished merging post ID %s.', 'ultimatewoo-pro'), $post_id ) );
		} else {
			$this->add_import_result( 'imported', 'Import successful', $post_id, $processing_product_title, $processing_product_sku );
			WC_Product_CSV_Import_Suite::log( sprintf( __('> Finished importing post ID %s.', 'ultimatewoo-pro'), $post_id ) );
		}
	...
	public function process_attachment( $post, $url, $post_id ) {
		...
		if ( ! is_wp_error( $attachment_id ) && $attachment_id > 0 ) {
			WC_Product_CSV_Import_Suite::log( sprintf( __( '> > Inserted image attachment "%s"', 'ultimatewoo-pro' ), $url ) );
	...
	public function fetch_remote_file( $url, $post ) {
		...
		if ( 0 == $filesize ) {
			@unlink( $upload['file'] );
			unset( $upload );
			return new WP_Error( 'import_file_error', __('Zero size file downloaded', 'ultimatewoo-pro') );
		}
	...
	public function header() {
		...
		echo '<h2>' . ( empty( $_GET['merge'] ) ? __( 'Import Products', 'ultimatewoo-pro' ) : __( 'Merge Products', 'ultimatewoo-pro' ) ) . '</h2>';
	}
