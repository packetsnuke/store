...
class WC_CSV_Parser {
	...
	public function parse_product( $item, $merge_empty_cells = 0 ) {
		...
		if ( $merging ) {
			...
			WC_Product_CSV_Import_Suite::log( sprintf( __( '> Row %s - preparing for merge.', 'ultimatewoo-pro' ), $this->row ) );

			// Required fields
			if ( ! $post_id && empty( $item['sku'] ) ) {
				WC_Product_CSV_Import_Suite::log( __( '> > Cannot merge without id or sku. Importing instead.', 'ultimatewoo-pro') );
				$merging = false;
			} else {
				...
				if ( ! $post_id ) {
					...
					if (  $is_variation && ! $found_product_id && empty( $item[ 'post_title' ] ) ) {
						...
						WC_Product_CSV_Import_Suite::log( sprintf(__( '> > Skipped. Cannot find product with sku %s.', 'ultimatewoo-pro'), $item['sku']) );
						return new WP_Error( 'parse-error', __( 'Skipped. Cannot find product with sku.', 'ultimatewoo-pro' ) );
					} elseif ( ! $found_product_id ) {
						WC_Product_CSV_Import_Suite::log( sprintf(__( '> > Skipped. Cannot find product with sku %s. Importing instead.', 'ultimatewoo-pro'), $item['sku']) );
						$merging = false;
					} else {
						$post_id = $found_product_id;
						WC_Product_CSV_Import_Suite::log( sprintf(__( '> > Found product with ID %s.', 'ultimatewoo-pro'), $post_id) );
					}
				}
			}
		}
		...
		if ( ! $merging ) {
			...
			WC_Product_CSV_Import_Suite::log( sprintf( __('> Row %s - preparing for import.', 'ultimatewoo-pro'), $this->row ) );

			// Required fields
			if ( $this->post_type == 'product' && empty( $item['post_title'] ) ) {
				WC_Product_CSV_Import_Suite::log( __( '> > Skipped. No post_title set for new product.', 'ultimatewoo-pro') );
				return new WP_Error( 'parse-error', __( 'No post_title set for new product.', 'ultimatewoo-pro' ) );
			}
		...
		foreach ( $item as $key => $value ) {
			...
			elseif ( strstr( $key, 'tax:' ) ) {
				...
				if ( ! taxonomy_exists( $taxonomy ) ) {
					WC_Product_CSV_Import_Suite::log( sprintf( __('> > Skipping taxonomy "%s" - it does not exist.', 'ultimatewoo-pro'), $taxonomy ) );
					continue;
				}
				...
				if ( 'product_type' === $taxonomy ) {
					...
					if ( ! array_key_exists( $term, $this->allowed_product_types ) ) {
						WC_Product_CSV_Import_Suite::log( sprintf( __('> > > Product type "%s" not allowed - using simple.', 'ultimatewoo-pro'), $term ) );
					...
					if ( strstr( $raw_term, '>' ) ) {
						...
						foreach ( $raw_term as $term ) {
							...
							if ( isset( $this->inserted_terms[ $taxonomy ][ $parent ][ $term ] ) ) {
								...
								WC_Product_CSV_Import_Suite::log( sprintf( __( '> > (' . __LINE__ . ') Term %s (%s) exists? %s', 'ultimatewoo-pro' ), sanitize_text_field( $term ), esc_html( $taxonomy ), $term_may_exist ? print_r( $term_may_exist, true ) : '-' ) );
								...
								if ( ! $term_id ) {
									...
									if ( ! is_wp_error( $t ) ) {
										...
										WC_Product_CSV_Import_Suite::log( sprintf( __( '> > (' . __LINE__ . ') Failed to import term %s, parent %s - %s', 'ultimatewoo-pro' ), sanitize_text_field( $term ), sanitize_text_field( $parent ), sanitize_text_field( $taxonomy ) ) );
										break;
									}
								}
						...
						if ( isset( $this->inserted_terms[ $taxonomy ][0][$raw_term] ) ) {
							...
							if ( ! $term_id ) {
								...
								if ( ! is_wp_error( $t ) ) {
									...
									WC_Product_CSV_Import_Suite::log( sprintf( __( '> > Failed to import term %s %s', 'ultimatewoo-pro' ), esc_html($raw_term), esc_html($taxonomy) ) );
									break;
								}
							}
			...
			elseif ( strstr( $key, 'attribute:' ) ) {
				...
				if ( substr( $attribute_key, 0, 3 ) == 'pa_' ) {
					...
					if ( ! taxonomy_exists( $taxonomy ) ) {
						...
						WC_Product_CSV_Import_Suite::log( sprintf( __('> > Attribute taxonomy "%s" does not exist. Adding it. Nicename: %s', 'ultimatewoo-pro'), $taxonomy, $nicename ) );
						...
						if ( ! $exists_in_db ) {
							...
							WC_Product_CSV_Import_Suite::log( sprintf( __('> > Attribute taxonomy %s already exists in DB.', 'ultimatewoo-pro'), $taxonomy ) );
						}
						...
						foreach ( $raw_terms as $raw_term ) {
							...
							if ( ! $term_id ) {
								...
								if ( ! is_wp_error( $t ) ) {
									...
									WC_Product_CSV_Import_Suite::log( sprintf( __( '> > Inserted Raw Term %s ID = %s', 'ultimatewoo-pro' ), esc_html( $raw_term ), $term_id ) );
								} else {
									WC_Product_CSV_Import_Suite::log( sprintf( __( '> > Failed to import term %s %s', 'ultimatewoo-pro' ), esc_html($raw_term), esc_html($taxonomy) ) );
									break;
								}
							} else {
								WC_Product_CSV_Import_Suite::log( sprintf( __( '> > Raw Term %s ID = %s', 'ultimatewoo-pro' ), esc_html( $raw_term ), $term_id ) );
							}
