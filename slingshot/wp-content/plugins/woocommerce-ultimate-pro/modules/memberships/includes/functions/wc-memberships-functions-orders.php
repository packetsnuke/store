	...
	if ( is_numeric( $order_id ) ) {
		...
		if ( ! empty( $memberships ) ) {
			...
			if ( ! empty( $memberships_with_members_area ) ) {
				...
				$message = '<p>' . __( 'Thanks for purchasing a membership!', 'ultimatewoo-pro' );

				if ( 1 === count( $memberships_with_members_area ) ) {

					/* translators: Placeholders: %1$s - <a> tag, %2$s - </a> tag */
					$message .= ' ' . sprintf( __( 'You can view more details about your membership from %1$syour account%2$s.', 'ultimatewoo-pro' ), '<a href="' . esc_url( wc_memberships_get_members_area_url( key( $memberships_with_members_area ) ) ) . '">', '</a>' );

				} else {

					$message .= ' ' . __( 'You can view details for each membership in your account:', 'ultimatewoo-pro' );
					$message .= '<ul>';
