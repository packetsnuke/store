...
class WC_Memberships_User_Membership {
	...
	public function unschedule_expiration_events() {
		...
		if ( wc_next_scheduled_action( 'wc_memberships_user_membership_expiry', $hook_args, 'ultimatewoo-pro'  ) ) {
			wc_unschedule_action( 'wc_memberships_user_membership_expiry', $hook_args, 'ultimatewoo-pro' );
		}

		// unschedule any previous expiring soon hooks
		if ( wc_next_scheduled_action( 'wc_memberships_user_membership_expiring_soon', $hook_args, 'ultimatewoo-pro' ) ) {
			wc_unschedule_action( 'wc_memberships_user_membership_expiring_soon', $hook_args, 'ultimatewoo-pro' );
		}

		// unschedule any previous renewal reminder hooks
		if ( wc_next_scheduled_action( 'wc_memberships_user_membership_renewal_reminder', $hook_args, 'ultimatewoo-pro' ) ) {
			wc_unschedule_action( 'wc_memberships_user_membership_renewal_reminder', $hook_args, 'ultimatewoo-pro' );
		}
	...
	public function schedule_expiration_events( $end_timestamp = null ) {
		...
		if ( is_numeric( $end_timestamp ) && (int) $end_timestamp > strtotime( 'today', $now ) ) {
			...
			wc_schedule_single_action( $end_timestamp, 'wc_memberships_user_membership_expiry', $hook_args, 'ultimatewoo-pro' );
			...
			if ( $end_timestamp - $days_before_expiry >= DAY_IN_SECONDS ) {
				...
				if ( $days_before_expiry > $now ) {
					...
					wc_schedule_single_action( $days_before_expiry, 'wc_memberships_user_membership_expiring_soon', $hook_args, 'ultimatewoo-pro' );
					...
					wc_schedule_single_action( $median_time, 'wc_memberships_user_membership_expiring_soon', $hook_args, 'ultimatewoo-pro' );
				}
			}
		}
	}
	...
	public function pause_membership( $note = null ) {
		...
		$this->update_status( 'paused', ! empty( $note ) ? $note : __( 'Membership paused.', 'ultimatewoo-pro' ) );
	...
	public function cancel_membership( $note = null ) {
		...
		$this->update_status( 'cancelled', ! empty( $note ) ? $note : __( 'Membership cancelled.', 'ultimatewoo-pro' ) );
	...
	public function expire_membership() {
		...
		if ( true === apply_filters( 'wc_memberships_expire_user_membership', true, $this ) ) {
			...
			$this->update_status( 'expired', __( 'Membership expired.', 'ultimatewoo-pro' ) );
			...
			if ( wc_next_scheduled_action( 'wc_memberships_user_membership_renewal_reminder', $event_args, 'ultimatewoo-pro' ) ) {
				wc_unschedule_action( 'wc_memberships_user_membership_renewal_reminder', $event_args, 'ultimatewoo-pro' );
			}

			// now that the membership expired, set the renewal reminder event
			if ( $this->can_be_renewed() ) {
				wc_schedule_single_action( $this->get_expired_time_after( $current_time ), 'wc_memberships_user_membership_renewal_reminder', $event_args, 'ultimatewoo-pro' );
			}
	...
	public function activate_membership( $note = null ) {
		...
		if ( ! $was_delayed && $this->is_active() ) {
			...
			// reactivation
			$default_note = __( 'Membership resumed.', 'ultimatewoo-pro' );
			$this->set_paused_interval( 'end', current_time( 'timestamp', true ) );
		} else {
			// activation
			$default_note = __( 'Membership activated.', 'ultimatewoo-pro' );
		}
	...
	public function transfer_ownership( $to_user ) {
		...
		$new_owner          = $to_user;
		$error              = array();
		$default_error      = array( 3 => __( 'An error occurred.', 'ultimatewoo-pro' ) );

		if ( ! $new_owner instanceof WP_User ) {
			$error[0] = __( 'Please select a valid user to transfer the membership to.', 'ultimatewoo-pro' );
		} elseif ( $new_owner->ID === $this->id ) {
			$error[1] = __( 'The user you have selected to transfer the membership to is the same user owning the membership to be transferred. Please select a different user.', 'ultimatewoo-pro' );
		} elseif ( ! $this->can_be_transferred( $new_owner->ID ) ) {
			$error[2] = __( 'This membership cannot be transferred to this user.', 'ultimatewoo-pro' );
		...
		$this->add_note(
			...
			sprintf( __( 'Membership transferred from %1$s to %2$s.', 'ultimatewoo-pro' ),
