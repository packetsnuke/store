		...
		<tbody class="rule content-restriction-rule content-restriction-rule-<?php echo esc_attr( $index ); ?> <?php if ( ! $this->rule->current_user_can_edit() || ! $this->rule->current_context_allows_editing() ) : ?>disabled<?php endif; ?>">
			...
			<tr>
				...
				<th class="check-column">
					<p class="form-field">
						<label for="_content_restriction_rules_<?php echo esc_attr( $index ); ?>_checkbox"><?php esc_html_e( 'Select', 'ultimatewoo-pro' ); ?>:</label>
				...
				<?php if ( 'membership_plan' === $context ) : ?>
					...
					<td class="content-restriction-content-type content-type-column">
						<p class="form-field">
							<label for="_content_restriction_rules_<?php echo esc_attr( $index ); ?>_content_type_key"><?php esc_html_e( 'Type', 'ultimatewoo-pro' ); ?>:</label>
							...
							<select
								...
								<optgroup label="<?php esc_attr_e( 'Post types', 'ultimatewoo-pro' ); ?>">
								...
								<optgroup label="<?php esc_attr_e( 'Taxonomies', 'ultimatewoo-pro' ); ?>">
					...
					<td class="content-restriction-objects objects-column">
						<p class="form-field">
							<label for="_content_restriction_rules_<?php echo esc_attr( $index ); ?>_object_ids"><?php esc_html_e( 'Title', 'ultimatewoo-pro' ); ?>:</label>
				...
				<?php else : ?>
					...
					<td class="content-restriction-membership-plan membership-plan-column">
						<p class="form-field">
							<label for="_content_restriction_rules_<?php echo esc_attr( $index ); ?>_membership_plan_id"><?php esc_html_e( 'Plan', 'ultimatewoo-pro' ); ?>:</label>
				...
				<td class="content-restriction-access-schedule access-schedule-column">
					<p class="form-field">
						...
						<span class="rule-control-group rule-control-group-access-schedule-specific js-hide-if-access-schedule-immediate <?php if ( 'immediate' === $this->rule->get_access_schedule() ) : ?>hide<?php endif;?>">
							...
							printf( __( '%1$safter%2$s %3$s %4$s', 'ultimatewoo-pro' ),
				...
				<tr class="disabled-notice">
					...
					<td colspan="<?php echo ( 'wc_membership_plan' === $this->post->post_type ) ? 4 : 3; ?>">
						...
						<?php if ( ! $this->rule->is_new() && ! wc_memberships()->get_rules_instance()->rule_content_type_exists( $this->rule ) ) : ?>
							...
							<span class="description"><?php esc_html_e( 'This rule applies to a content type created by a plugin or theme that has been deactivated or deleted.', 'ultimatewoo-pro' ); ?></span>
						...
						<?php else : ?>
							...
							<span class="description"><?php
								...
								printf( __( 'This rule cannot be edited here because it applies to multiple content objects. You can %1$sedit this rule on the membership plan screen%2$s.', 'ultimatewoo-pro' ),
