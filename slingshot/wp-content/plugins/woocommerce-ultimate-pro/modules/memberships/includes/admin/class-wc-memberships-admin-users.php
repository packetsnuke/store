...
class WC_Memberships_Admin_Users {
	...
	public function add_user_columns( $columns ) {
		...
		if ( current_user_can( 'manage_woocommerce' ) ) {
			...
			$columns['wc_memberships_user_memberships'] = __( 'Active Memberships', 'ultimatewoo-pro' );
	...
	public function show_user_memberships( WP_User $user ) {
		...
		<div class="wc-memberships user-memberships" style="padding-bottom: 15px;">
			<h3><?php esc_html_e( 'Memberships', 'ultimatewoo-pro' ); ?></h3>
			<p>
				<?php if ( ! empty( $user_memberships ) ) : $plan_links = array(); ?>
					...
					<?php if ( ! empty( $plan_links ) ) : ?>
						...
						printf( __( 'This user is a member of %s.', 'ultimatewoo-pro' ),
							wc_memberships_list_items( $plan_links, __( 'and', 'ultimatewoo-pro' ) )
						); ?>
						<?php if ( $can_edit_user_memberships ) : ?>
							<br><br><a class="button" href="<?php echo esc_url( admin_url( 'post-new.php?post_type=wc_user_membership&user=' . $user->ID ) ); ?>"><?php esc_html_e( 'Add another membership.', 'ultimatewoo-pro' ); ?></a>
						<?php endif; ?>

					<?php else : ?>

						<?php esc_html_e( 'This user is already a member of every plan.', 'ultimatewoo-pro' ); ?>

					<?php endif; ?>

				<?php else : ?>

					<?php esc_html_e( 'This user has no memberships yet.', 'ultimatewoo-pro' ); ?>
					<?php if ( $can_edit_user_memberships ) : ?>
						<br><br><a class="button" href="<?php echo esc_url( admin_url( 'post-new.php?post_type=wc_user_membership&user=' . $user->ID ) ); ?>"><?php esc_html_e( 'Add a membership manually.', 'ultimatewoo-pro' ); ?></a>
					<?php endif; ?>
