...
class WC_Memberships_Meta_Box_Membership_Plan_Data extends WC_Memberships_Meta_Box {
	...
	public function get_title() {
		return __( 'Membership Plan Data', 'ultimatewoo-pro' );
	}
		...
		<div id="membership-plan-data-general" class="panel woocommerce_options_panel">
			...
			<div class="options_group">
				...
				<p class="form-field plan-access-method-field">
					<label for="_access_method"><?php esc_html_e( 'Grant access upon', 'ultimatewoo-pro' ); ?></label>
					...
					<span class="plan-access-method-selectors">
						...
						echo wc_help_tip( __( 'Choose how customers will gain access to this membership plan. Memberships can always be manually assigned.', 'ultimatewoo-pro' ) );
				...
				<p class="form-field js-show-if-access-method-purchase <?php if ( 'purchase' !== $current_access_type ) : ?>hide<?php endif; ?>">
					<label for="_product_ids"><?php esc_html_e( 'Products', 'ultimatewoo-pro' ); ?></label>
					...
					<?php echo wc_help_tip( __( 'Leave empty to only allow members you manually assign.', 'ultimatewoo-pro' ) ); ?>
				</p>
			...
			<div class="options_group">
				...
				<p class="form-field plan-access-length-field">
					<label for="_access_length"><?php esc_html_e( 'Membership length', 'ultimatewoo-pro' ); ?></label>
					...
					<span class="plan-access-length-selectors">
						...
						echo wc_help_tip( __( 'When does the membership expire?', 'ultimatewoo-pro' ) )
					...
					<span class="plan-access-length-fixed js-show-if-access-length-type-fixed <?php if ( 'fixed' !== $current_access_length ) : ?>hide<?php endif;?>">
						...
						?>
						<span>
							<label for="_access_start_date"><?php esc_html_e( 'Start date', 'ultimatewoo-pro' ); ?></label>
							<input
								type="text"
						...
						<span>
							<label for="_access_end_date"><?php esc_html_e( 'End date', 'ultimatewoo-pro' ); ?></label>
							<input
								type="text"
								id="_access_end_date"
	...
	private function output_restrict_content_panel( $membership_plan, $post ) {
		...
		<div id="membership-plan-data-restrict-content" class="panel woocommerce_options_panel">
			...
			if ( ! empty( $public_posts ) ) {
				printf( '<p>' . __( 'These posts are public, and will be excluded from all restriction rules: %s', 'ultimatewoo-pro' ) . '<p>', $this->list_post_links( $public_posts ) );
			}
	...
	private function output_restrict_products_panel( $membership_plan, $post ) {
		...
		<div id="membership-plan-data-restrict-products" class="panel woocommerce_options_panel">
			...
			if ( ! empty( $public_products ) ) {
				printf( '<p>' . __( 'These products are public, and will be excluded from all restriction rules: %s', 'ultimatewoo-pro' ) . '</p>', $this->list_post_links( $public_products ) );
			}
	...
	private function output_members_area_panel( $membership_plan, $post ) {
		...
		<div id="membership-plan-members-area" class="panel woocommerce_options_panel">
			...
			<p><?php esc_html_e( 'The Members Area is a portion of the My Accounts page that shows the member lists of available content, products, discounts, and / or membership notes.', 'ultimatewoo-pro' ); ?></p>

			<p class="form-field">
				<label for="_members_area_sections"><?php esc_html_e( 'Include sections for:', 'ultimatewoo-pro' ); ?></label>
				...
				<select
					...
					data-placeholder="<?php esc_html_e( 'Choose sections for this plan&hellip;', 'ultimatewoo-pro' ); ?>"
					style="width: 90%;">
				...
				<?php echo wc_help_tip( __( 'Leave empty to hide members area for this membership.', 'ultimatewoo-pro' ) ); ?>
			...
			<p><!-- // legend -->
				...
				<?php if ( array_key_exists( 'my-membership-content', $members_area_sections ) ) : ?>
					<em><?php esc_html_e( '"Content" will show all pages, posts and other content.', 'ultimatewoo-pro' ); ?></em><br>
				<?php endif; ?>

				<?php if ( array_key_exists( 'my-membership-products', $members_area_sections ) ) : ?>
					<em><?php esc_html_e( '"Products" will show products that are viewable or purchaseable.', 'ultimatewoo-pro' ); ?></em><br>
				<?php endif; ?>

				<?php if ( array_key_exists( 'my-membership-discounts', $members_area_sections ) ) : ?>
					<em><?php esc_html_e( '"Discounts" will list products carrying membership discounts.', 'ultimatewoo-pro' ); ?></em><br>
				<?php endif; ?>

				<?php if ( array_key_exists( 'my-membership-notes', $members_area_sections ) ) : ?>
					<em><?php esc_html_e( '"Notes" will only display notes that have been emailed to the customer (no internal membership notes).', 'ultimatewoo-pro' ); ?></em><br />
				<?php endif; ?>

				<?php if ( array_key_exists( 'my-membership-details', $members_area_sections ) ) : ?>
					<em><?php esc_html_e( '"Manage" will show details about the membership, such as status and action links.', 'ultimatewoo-pro' ); ?></em>
				<?php endif; ?>
		...
		<div id="membership-plan-email-content" class="panel woocommerce_options_panel">
			...
			<?php foreach ( $emails as $id => $email ) :
				...
				<div class="options_group" style="padding: 10px;">
					<h4><?php echo esc_html( $email->get_title() ); ?> <small>(<a href="<?php echo esc_url( $email_setting_link ); ?>"><?php echo strtolower( esc_html__( 'Configure Email', 'ultimatewoo-pro' ) ); ?></a>)</small></h4>
			...
			<?php if ( 0 === $enabled ) : ?>
				...
				<p><em><?php printf( __( 'It looks like you haven\'t enabled any of the Memberships emails. To configure email content for this plan, you need to enable at least one membership email from the %1$sWooCommerce Emails settings%2$s', 'ultimatewoo-pro' ), '<a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=email' ) ) . '">', '</a>' ); ?></em></p>
	...
	private function list_post_links( $posts ) {
		...
		if ( ! empty( $posts ) ) {
			...
			$post_links = wc_memberships_list_items( $items, __( 'and', 'ultimatewoo-pro' ) );
		}
	...
	private function add_admin_notices() {
		...
		wc_memberships()->get_admin_notice_handler()->add_admin_notice(
			...
			sprintf( __( 'When you add a restriction rule for content, it will no longer be public on your site. By adding a rule for a page, post, or taxonomy, it will become restricted, and can only be accessed by members of this plan, or by members of another plan that grants access to the content.%1$sLearn more about %2$srestriction rules in the documentation%3$s.', 'ultimatewoo-pro' ),
				'<br />',
		...
		wc_memberships()->get_admin_notice_handler()->add_admin_notice(
			...
			sprintf( __( 'When you add a %1$sviewing%2$s restriction rule for a product, it will no longer be public on your site, and can only be accessed by members of this plan, or by members of another plan that grants access to the product. By adding a %3$spurchasing%4$s restriction rule, the product can be viewed publicly, but only purchased by members.', 'ultimatewoo-pro' ),
				'<strong>', '</strong>',
				'<strong>',	'</strong>'
			),
				...
				if (    'specific' === $_POST['_access_length']
					...
					if ( $time_start >= $time_end ) {
						...
						wc_memberships()->get_admin_instance()->get_message_handler()->add_error(
							__( 'You cannot set an access start date after the access end date, or on the same day. The two dates have been set one day apart from each other.', 'ultimatewoo-pro' )
						);
					}
					...
					if ( $time_end < strtotime( 'midnight', current_time( 'timestamp', true ) ) ) {
						...
						wc_memberships()->get_admin_instance()->get_message_handler()->add_error(
							__( 'You have chosen an end date that is set in the past. The selected access dates have been saved, but please make sure that this is correct.', 'ultimatewoo-pro' )
						);
					}
				}
			}
		...
		if ( $access_products = $membership_plan->get_product_ids() ) {
			...
			if ( is_array( $restricted_products ) ) {
				...
				if ( ! empty( $conflicting_products ) ) {
					wc_memberships()->get_admin_instance()->get_message_handler()->add_error(
						__( 'It looks like that you have restricted one or more products that could grant access to this membership plan: this could make it impossible for shop customers to become members via purchase. Please double check your plan rules and the chosen products that grant access.', 'ultimatewoo-pro' )
					);
				}
			}
		}
