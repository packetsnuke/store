...
class WC_Memberships_Meta_Box_Post_Memberships_Data extends WC_Memberships_Meta_Box {
	...
	public function output( WP_Post $post ) {
		...
		?>
		<h4><?php esc_html_e( 'Content Restriction', 'ultimatewoo-pro' ); ?></h4>

		<?php woocommerce_wp_checkbox( array(
			'id'          => '_wc_memberships_force_public',
			'class'       => 'js-toggle-rules',
			'label'       => __( 'Disable restrictions', 'ultimatewoo-pro' ),
			'description' => __( 'Check this box if you want to force the content to be public regardless of any restriction rules that may apply now or in the future.', 'ultimatewoo-pro' ),
		) ); ?>
		...
		<div class="js-restrictions <?php if ( 'yes' === wc_memberships_get_content_meta( $post, '_wc_memberships_force_public', true ) ) : ?>hide<?php endif; ?>">
			...
			?>
			<h4><?php esc_html_e( 'Custom Restriction Message', 'ultimatewoo-pro' ); ?></h4>
			<?php
			...
			woocommerce_wp_checkbox( array(
				...
				'description' => __( 'Check this box if you want to customize the content restricted message for this content.', 'ultimatewoo-pro' ),
			) );
			...
			<div
				...
				<p>
					...
					printf( __( '%1$s automatically inserts the product(s) needed to gain access. %2$s inserts the URL to my account page. HTML is allowed.', 'ultimatewoo-pro' ), '<strong><code>{products}</code></strong>', '<strong><code>{login_url}</code></strong>' ); ?>
				</p>
				<?php
