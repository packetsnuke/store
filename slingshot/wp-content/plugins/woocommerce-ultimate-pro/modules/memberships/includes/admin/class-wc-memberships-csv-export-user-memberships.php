...
class WC_Memberships_CSV_Export_User_Memberships extends WC_Memberships_Import_Export {
	...
	public function __construct() {
		...
		$this->action_label = __( 'Export', 'ultimatewoo-pro' );
	...
	public function set_admin_page_title( $admin_title, $title ) {
		...
		if ( isset( $_GET['page'] ) && 'wc_memberships_import_export' === $_GET['page'] ) {
			return __( 'Export Members', 'ultimatewoo-pro' ) . ' ' . $admin_title;
		}
	...
	protected function get_fields() {
		...
		$options = array(
			...
			// section start
			array(
				'title' => __( 'Export Members', 'ultimatewoo-pro' ),
				'type'  => 'title',
			),
			...
			array(
				...
				'title'             => __( 'Plan', 'ultimatewoo-pro' ),
				'desc_tip'          => __( 'Choose which plan(s) to export members from. Leave blank to export members from every plan.', 'ultimatewoo-pro' ),
				'type'              => 'multiselect',
				...
				'custom_attributes' => array(
					'data-placeholder' => __( 'Leave blank to export members of any plan.', 'ultimatewoo-pro' ),
				)
			),
			...
			array(
				...
				'title'             => __( 'Status', 'ultimatewoo-pro' ),
				'desc_tip'          => __( 'Choose to export user memberships with specific status(es) only. Leave blank to export user memberships of any status.', 'ultimatewoo-pro' ),
				'type'              => 'multiselect',
				...
				'css'               => 'min-width: 250px',
				'custom_attributes' => array(
					'data-placeholder' => __( 'Leave blank to export members with any status.', 'ultimatewoo-pro' ),
				)
			),
			...
			array(
				...
				'title' => __( 'Start Date', 'ultimatewoo-pro' ),
				/* translators: Placeholder: %s - date format */
				'desc'  => sprintf(
					__( 'Start date of memberships to include in the exported file, in the format %s.', 'ultimatewoo-pro' ) . '<br>' .
					__( 'You can optionally specify a date range, or leave one of the fields blank for open-ended ranges.', 'ultimatewoo-pro' ),
			...
			array(
				...
				'title' => __( 'End Date', 'ultimatewoo-pro' ),
				/* translators: Placeholder: %s - date format */
				'desc'  => sprintf(
					__( 'Expiration date of memberships to include in the exported file, in the format %s.', 'ultimatewoo-pro' ) . '<br>' .
					__( 'You can optionally specify a date range, or leave one of the fields blank for open-ended ranges.', 'ultimatewoo-pro' ),
					'<code>YYYY-MM-DD</code>'
				),
				'type'  => 'wc-memberships-date-range',
			...
			array(
				...
				'name'     => __( 'Meta data', 'ultimatewoo-pro' ),
				'type'     => 'checkbox',
				'desc'     => __( 'Include additional meta data', 'ultimatewoo-pro' ),
				'desc_tip' => __( 'Add an extra column to the CSV file with all post meta of each membership in JSON format.', 'ultimatewoo-pro' ),
				'default'  => 'no'
			),
			...
			array(
				...
				'name'     => __( 'Separate fields by', 'ultimatewoo-pro' ),
				'type'     => 'select',
				'desc_tip' => __( 'Change the delimiter based on your desired output format.', 'ultimatewoo-pro' ),
				'options'  => array(
			...
			array(
				...
				'name'              => __( 'Limit Records', 'ultimatewoo-pro' ),
				'type'              => 'number',
				'desc'              => __( 'Limit the number of rows to be exported. Use this option when exporting very large files that are unable to complete in a single attempt.', 'ultimatewoo-pro' ),
				'class'             => 'small-text',
				'custom_attributes' => array(
					'min'  => 0,
					'step' => 1,
			...
			array(
				...
				'name'              => __( 'Offset Records', 'ultimatewoo-pro' ),
				'type'              => 'number',
				'desc'              => __( 'Set the number of records to be skipped in this export. Use this option when exporting very large files that are unable to complete in a single attempt.', 'ultimatewoo-pro' ),
				'class'             => 'small-text',
				'custom_attributes' => array(
					'min'  => 0,
					'step' => 1,
	...
	public function process_export() {
		...
		if ( ! empty( $user_memberships_ids ) ) {
			...
			wc_memberships()->get_message_handler()->add_error( __( 'No User Memberships found matching the criteria to export.', 'ultimatewoo-pro' ) );
		}
	}
		...
		if ( ! empty( $columns ) ) {
			...
			foreach ( $columns as $column_name ) {
				...
				switch ( $column_name ) {
					...
					case 'has_access' :
						$value = $user_membership->is_active() ? strtolower( __( 'Yes', 'ultimatewoo-pro' ) ) : strtolower( __( 'No', 'ultimatewoo-pro' ) );
					break;
