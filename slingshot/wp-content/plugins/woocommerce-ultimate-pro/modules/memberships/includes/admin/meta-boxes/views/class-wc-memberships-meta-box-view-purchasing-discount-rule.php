		...
		<tbody
			...
			<tr>
				...
				<th scope="row" class="check-column">
					<p class="form-field">
						<label for="_purchasing_discount_rules<?php echo esc_attr( $index ); ?>_checkbox"><?php esc_html_e( 'Select', 'ultimatewoo-pro' ); ?>:</label>
				...
				<?php if ( 'membership_plan' === $context ) : ?>
					...
					<td class="purchasing-discount-content-type content-type-column">
						<p class="form-field">
							<label for="_purchasing_discount_rules<?php echo esc_attr( $index ); ?>_content_type_key"><?php esc_html_e( 'Discount', 'ultimatewoo-pro' ); ?>:</label>
					...
					<td class="purchasing-discount-objects objects-column">
						<p class="form-field">
							<label for="_purchasing_discount_rules<?php echo esc_attr( $index ); ?>_object_ids"><?php esc_html_e( 'Title', 'ultimatewoo-pro' ); ?>:</label>
							...
							<?php if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ) : ?>
								...
								<select
									...
									style="width: 90%;"
									multiple="multiple"
									data-placeholder="<?php esc_attr_e( 'Search&hellip; or leave blank to apply to all', 'ultimatewoo-pro' ); ?>"
				...
				<?php else : ?>
					...
					<?php if ( $is_variable_product ) : ?>
						...
						<td class="purchasing-discount-applies-to product-variation-column">
							<p class="form-field"><?php esc_html_e( 'Any variation', 'ultimatewoo-pro' ); ?></p>
						</td>
					...
					<td class="purchasing-discount-membership-plan membership-plan-column">
						<p class="form-field">
							<label for="_purchasing_discount_rules_<?php echo esc_attr( $index ); ?>_membership_plan_id"><?php esc_html_e( 'Plan', 'ultimatewoo-pro' ); ?>:</label>
				...
				<td class="purchasing-discount-type discount-type-column">
					<p class="form-field">
						<label for="_purchasing_discount_rules<?php echo esc_attr( $index ); ?>_discount_type"><?php esc_html_e( 'Type', 'ultimatewoo-pro' ); ?>:</label>
				...
				<td class="purchasing-discount-amount amount-column">
					<p class="form-field">
						<label for="_purchasing_discount_rules<?php echo esc_attr( $index ); ?>_discount_amount"><?php esc_html_e( 'Amount', 'ultimatewoo-pro' ); ?>:</label>
				...
				<td class="purchasing-discount-active active-columns">
					<p class="form-field">
						<label for="_purchasing_discount_rules<?php echo esc_attr( $index ); ?>_discount_active"><?php esc_html_e( 'Active', 'ultimatewoo-pro' ); ?>:</label>
						<input
							type="checkbox"
				...
				<tr class="disabled-notice" <?php echo $this->rule->is_trashed() ? 'style="display: none; height: 0;"' : ''; ?>>
					...
					<td colspan="<?php echo ( 'wc_membership_plan' === $this->post->post_type ) ? 4 : 3; ?>">
						...
						<?php if ( ! $this->rule->is_new() && ! wc_memberships()->get_rules_instance()->rule_content_type_exists( $this->rule ) ) : ?>
							...
							<span class="description"><?php esc_html_e( 'This rule applies to content generated by a plugin or theme that has been deactivated or deleted.', 'ultimatewoo-pro' ); ?></span>

						<?php elseif ( ! $this->rule->current_user_can_edit() ) : ?>

							<span class="description"><?php esc_html_e( 'You are not allowed to edit this rule', 'ultimatewoo-pro' ); ?></span>

						<?php else : ?>

							<span class="description"><?php
								/* translators: Placeholders: %1$s - opening HTML <a> tag, %2$s - closing </a> HTML tag */
								printf( esc_html__( 'This rule cannot be edited here because it applies to multiple products. You can %1$sedit this rule on the membership plan screen%2$s.', 'ultimatewoo-pro' ),
