...
class WC_Memberships_Admin {
	...
	public function add_import_export_admin_page() {
		...
		add_submenu_page(
			'',
			__( 'Import / Export', 'ultimatewoo-pro' ),
			__( 'Import / Export', 'ultimatewoo-pro' ),
			$capability,
			'wc_memberships_import_export',
	...
	private function enqueue_scripts() {
		...
		wp_localize_script( 'wc-memberships-admin', 'wc_memberships_admin', array(
			...
			'i18n' => array(
				...
				// 'log_in' => __( 'Log In', 'ultimatewoo-pro' )

				'delete_membership_confirm'     => __( 'Are you sure that you want to permanently delete this membership?', 'ultimatewoo-pro' ),
				'delete_memberships_confirm'    => __( 'Are you sure that you want to permanently delete these memberships?', 'ultimatewoo-pro' ),
				'please_select_user'            => __( 'Please select a user.', 'ultimatewoo-pro' ),
			),
		) );
	}
	...
	public function render_tabs() {
		...
		if ( $screen && in_array( $screen->id, $this->get_screen_ids( 'tabs' ), true ) ) :
			...
			$tabs = apply_filters( 'wc_memberships_admin_tabs', array(
				'members'       => array(
					'title' => __( 'Members', 'ultimatewoo-pro' ),
					'url'   => admin_url( 'edit.php?post_type=wc_user_membership' ),
				),
				'memberships'   => array(
					'title' => wp_is_mobile() ? __( 'Plans', 'ultimatewoo-pro' ) : __( 'Membership Plans', 'ultimatewoo-pro' ),
					'url'   => admin_url( 'edit.php?post_type=wc_membership_plan' ),
				),
				'import-export' => array(
					'title' => __( 'Import / Export', 'ultimatewoo-pro' ),
					...
					if ( ! empty( $has_atts ) ) {
						...
						<div class="notice notice-warning">
							<p><?php
								...
								printf( __( 'It looks like you might be using the %1$s shortcode with deprecated attributes, such as %2$s. These attributes have been deprecated since WooCommerce 2.6 and may no longer have any effect on the shortcode output. Furthermore, they might cause a server error when visiting the Members Area while WooCommerce Memberships is active.', 'ultimatewoo-pro' ), '<code>woocommerce_my_account</code>', '<code>order_count</code>' );
								?></p>
						</div>
						<?php
					}
	...
	public function process_import_export_form() {
		...
		if ( ! current_user_can( 'manage_woocommerce' ) || ! check_admin_referer( 'wc_memberships_' . $action, '_wpnonce' ) ) {
			wp_die( __( 'You are not allowed to perform this action.', 'ultimatewoo-pro' ) );
		}
