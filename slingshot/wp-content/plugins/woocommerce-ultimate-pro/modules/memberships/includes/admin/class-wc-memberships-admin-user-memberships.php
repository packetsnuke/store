...
class WC_Memberships_Admin_User_Memberships {
	...
	public function customize_columns( $columns ) {
		...
		$columns['title']        = __( 'Name', 'ultimatewoo-pro' );         // member name column
		$columns['email']        = __( 'Email', 'ultimatewoo-pro' );        // member email
		$columns['plan']         = __( 'Plan', 'ultimatewoo-pro' );         // associated membership plan
		$columns['status']       = __( 'Status', 'ultimatewoo-pro' );       // user membership status
		$columns['member_since'] = __( 'Member since', 'ultimatewoo-pro' ); // membership created
		$columns['expires']      = __( 'Expires', 'ultimatewoo-pro' );      // expiration date-time
	...
	public function customize_row_actions( $actions, WP_Post $post ) {
		...
		if ( 'wc_user_membership' === $post->post_type ) {
			...
			if ( $user_membership->is_paused() ) {
				...
				$actions['resume'] = '<a href="' . esc_url( $resume_link ) . '">' . esc_html__( 'Resume', 'ultimatewoo-pro' ) . '</a>';
			} else {
				$pause_link = add_query_arg( 'action', 'pause', wp_nonce_url( $post_link, 'wc-memberships-pause-membership-' . $post->ID ) );
				$actions['pause']  = '<a href="' . esc_url( $pause_link ) . '">'  . esc_html__( 'Pause', 'ultimatewoo-pro' )  . '</a>';
			}
			...
			if ( ! $user_membership->is_cancelled() ) {
				...
				$actions['cancel'] = '<a href="' . esc_url( $cancel_link ) . '">' . esc_html__( 'Cancel', 'ultimatewoo-pro' ) . '</a>';
			}

			if ( current_user_can( 'delete_post', $post->ID ) ) {
				$actions['delete'] = "<a class='submitdelete delete-membership' title='" . esc_attr__( 'Delete this membership permanently', 'ultimatewoo-pro' ) . "' href='" . esc_url( get_delete_post_link( $post->ID, '', true ) ) . "'>" . esc_html__( 'Delete', 'ultimatewoo-pro' ) . "</a>";
			}
		}
		...
		if ( $post_type === 'wc_user_membership' && current_user_can( 'manage_woocommerce_user_memberships' ) ) :
			...
			<script type="text/javascript">
				jQuery( document ).ready( function() {
					jQuery( '<option>' ).val( 'delete' ).text( '<?php esc_html_e( 'Delete', 'ultimatewoo-pro' ); ?>').appendTo( "select[name='action']" );
					jQuery( '<option>' ).val( 'delete' ).text( '<?php esc_html_e( 'Delete', 'ultimatewoo-pro' ); ?>').appendTo( "select[name='action2']" );
				} );
			</script>
			<?php
			...
			<div class="inline-edit-col">
				<div class="inline-edit-group">
					<label class="inline-edit-status alignleft">
						<span class="title"><?php esc_html_e( 'Status', 'ultimatewoo-pro' ); ?></span>
						<select name="_status">
							<option value="-1"><?php echo '&mdash; ' . esc_html__( 'No Change', 'ultimatewoo-pro' ) . ' &mdash;'; ?></option>
							<?php
		...
		switch ( $column ) {
			...
			case 'expires':
				...
				if ( ! empty( $end_time ) && is_numeric( $end_time ) ) {
					...
					$date = esc_html__( 'Never', 'ultimatewoo-pro' );
					$time = '';
				}
	...
	public function restrict_user_memberships() {
		...
		$expires = apply_filters( 'wc_memberships_expiry_terms_dropdown_options', array(
			'today'      => __( 'Today', 'ultimatewoo-pro' ),
			'this_week'  => __( 'This week', 'ultimatewoo-pro' ),
			'this_month' => __( 'This month', 'ultimatewoo-pro' ),
		) );
		...
		<select name="post_parent">
			<option value=""><?php esc_html_e( 'All plans', 'ultimatewoo-pro' ); ?></option>
			<?php
		...
		<select name="post_status">
			<option value=""><?php esc_html_e( 'All statuses', 'ultimatewoo-pro' ); ?></option>
			<?php
				if ( ! empty( $statuses ) ) {
		...
		<select name="expires">
			<option value=""><?php esc_html_e( 'Expires', 'ultimatewoo-pro' ); ?></option>
			<?php
				if ( ! empty( $expires ) ) {
	...
	public function validate_user_membership( $maybe_empty, $postarr ) {
		...
		if ( ! $postarr['post_parent'] && isset( $_POST['post_ID'] ) ) {
			...
			wc_memberships()->get_admin_instance()->get_message_handler()->add_error( __( 'Please select a membership plan.', 'ultimatewoo-pro' ) );
	...
	public function maybe_prevent_adding_user_membership() {
		...
		if ( 'post-new.php' === $pagenow ) {
			...
			if ( ! $user_id || ! $user ) {
				...
				wc_memberships()->get_admin_instance()->get_message_handler()->add_error( __( 'Please select a user to add as a member.', 'ultimatewoo-pro' ) );
			...
			if ( count( $user_memberships ) === count( $membership_plans ) ) {
				...
				wc_memberships()->get_admin_instance()->get_message_handler()->add_message( __( 'This user is already a member of every plan.', 'ultimatewoo-pro' ) );
				wp_redirect( wp_get_referer() ); exit;
			}
		}
	}
