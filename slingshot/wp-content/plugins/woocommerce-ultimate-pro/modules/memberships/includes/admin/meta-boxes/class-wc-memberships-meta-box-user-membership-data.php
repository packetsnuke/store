...
class WC_Memberships_Meta_Box_User_Membership_Data extends WC_Memberships_Meta_Box {
	...
	public function get_title() {
		return __( 'User Membership Data', 'ultimatewoo-pro' );
	}
			...
			<ul class="sections">
				...
				<?php if ( count( $user_memberships ) !== count( $membership_plans ) ) : ?>
					...
					<li <?php if ( ! $current_membership ) : ?>class="active"<?php endif; ?>>
						<a href="<?php echo esc_url( admin_url( 'post-new.php?post_type=wc_user_membership&user=' . $user->ID ) ); ?>"><?php esc_html_e( 'Add a plan...', 'ultimatewoo-pro' ); ?></a>
					</li>
			...
			<li class="wide">
				...
				if ( current_user_can( 'delete_post', $post->ID ) ) {
					...
					$user_membership_actions['delete-action'] = array(
						...
						'text'  => __( 'Delete User Membership', 'ultimatewoo-pro' ),
					);
					...
					if ( 'auto-draft' !== $post->post_status ) {
						...
						$user_membership_actions['transfer-action'] = array(
							...
							'link'              => '#',
							'text'              => __( 'Transfer', 'ultimatewoo-pro' ),
				...
				<input
					...
					value="<?php esc_attr_e( 'Save', 'ultimatewoo-pro' ); ?>"
					data-tip="<?php esc_attr_e( 'Save/update the membership', 'ultimatewoo-pro' ); ?>"
				/>
			</li>
			<?php
	...
	private function output_plan_details_panel( $user_membership, $status_options ) {
		...
		<div class="plan-details">
			<h4><?php esc_html_e( 'Membership Details', 'ultimatewoo-pro' ); ?></h4>
			<div class="woocommerce_options_panel">
				...
				woocommerce_wp_select( array(
					'id'      => 'post_status',
					'label'   => __( 'Status:', 'ultimatewoo-pro' ),
					'options' => $status_options,
					'value'   => 'wcm-' . $user_membership->get_status(),
					'class'   => 'wide',
				) );

				$yy_mm_dd_hint = __( 'YYYY-MM-DD', 'ultimatewoo-pro' );
				...
				$calc_start_date = '<a href="#" class="js-calc-plan-date js-calc-plan-start-date">' . esc_html__( 'Update start date to plan start access date', 'ultimatewoo-pro' ) . '</a>';

				// start date
				woocommerce_wp_text_input( array(
					'id'          => '_start_date',
					'label'       => __( 'Member since:', 'ultimatewoo-pro' ),
				...
				$calc_end_date = '<a href="#" class="js-calc-plan-date js-calc-plan-end-date">' . esc_html__( 'Update expiration date to plan length', 'ultimatewoo-pro' ) . '</a>';

				// end date
				woocommerce_wp_text_input( array(
					'id'          => '_end_date',
					'label'       => __( 'Expires:', 'ultimatewoo-pro' ),
					'class'       => 'js-user-membership-date',
				...
				if ( $paused_date = $user_membership->get_local_paused_date( 'timestamp' ) ) :
					...
					<p class="form-field">
						<span class="description"><?php
							...
							printf( __( 'Paused since %s', 'ultimatewoo-pro' ),
	...
	private function output_billing_details_panel( $user_membership, $order ) {
		...
		<div class="billing-details">
			...
			<h4><?php esc_html_e( 'Billing Details', 'ultimatewoo-pro' ); ?></h4>
			...
			<div class="woocommerce_options_panel">
				...
				if ( $order ) {
					...
					$order_ref       = '<a href="' . esc_url( get_edit_post_link( SV_WC_Order_Compatibility::get_prop( $order, 'id' ) ) ) . '">' . sprintf(  __( 'Order %s', 'ultimatewoo-pro' ), $order->get_order_number() ) . '</a>';
					$billing_fields  = array(
						__( 'Purchased in:', 'ultimatewoo-pro' ) => $order_ref,
						__( 'Order Date:', 'ultimatewoo-pro' )   => date_i18n( wc_date_format(), SV_WC_Order_Compatibility::get_date_created( $order )->getTimestamp() ),
						__( 'Order Total:', 'ultimatewoo-pro' )  => $order->get_formatted_order_total(),
					);

				} else {

					$billing_fields = array(
						__( 'No billing details:', 'ultimatewoo-pro' ) => esc_html__( 'This membership was created manually.', 'ultimatewoo-pro' ),
					);
				}
