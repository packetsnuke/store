			...
			<thead>
				<tr>
					...
					<td class="check-column" style="width: 5%;">
						<label class="screen-reader-text" for="product-restriction-rules-select-all"> <?php esc_html_e( 'Select all', 'ultimatewoo-pro' ); ?></label>
						<input type="checkbox"
					...
					<?php else : ?>
						...
						<?php if ( $is_variable_product ) : ?>
							<th scope="col" class="product-restriction-applies-to product-variation-column" style="width: 25%;">
								<?php esc_html_e( 'Applies to', 'ultimatewoo-pro' ); ?>
							</th>
						<?php endif; ?>
					...
					<th scope="col" class="product-restriction-access-type access-type-column" style="width: 15%;">
						<?php esc_html_e( 'Only Members Can', 'ultimatewoo-pro' ); ?>
					</th>
					...
					<th scope="col" class="product-restriction-access-schedule access-schedule-column" style="<?php echo 'membership_plan' !== $context && ! $is_variable_product ? 'width: 40%;' : 'width: 25%;'; ?>">
						...
						<?php echo wc_help_tip( __( 'When will members gain access to products?', 'ultimatewoo-pro' ) ); ?>
					</th>
					...
					<td colspan="<?php echo $colspan; ?>">
						...
						if ( 'membership_plan' === $context ) {
							...
							if ( empty( $membership_plans ) ) {
								...
								esc_html_e( 'This product can be viewed & purchased by all customers. Add a rule to restrict viewing and/or purchasing to members.', 'ultimatewoo-pro' );
							}
						}
				...
				<tfoot>
										...
										<?php if ( in_array( $object_id, $variation_ids, false ) && ( $variation = wc_get_product( $object_id ) ) ) : ?>
											...
											<tr>
												...
												<td class="product-restriction-membership-plan membership-plan-column">
													<p class="form-field">
														<label for="_variation_product_restriction_rules_<?php echo esc_attr( $object_id ); ?>_membership_plan_id"><?php esc_html_e( 'Plan', 'ultimatewoo-pro' ); ?>:</label>
												...
												<td class="product-restriction-access-type access-type-column">
													<p class="form-field">
														<label for="_variation_product_restriction_rules_<?php echo esc_attr( $object_id ); ?>_access_type"><?php esc_html_e( 'Only Members Can', 'ultimatewoo-pro' ); ?>:</label>
					...
					<tr>
						<th colspan="<?php echo $colspan; ?>">
							<button
								type="button"
								class="button button-primary add-rule js-add-rule"><?php esc_html_e( 'Add New Rule', 'ultimatewoo-pro' ); ?></button>
							<button
								type="button"
								class="button button-secondary remove-rules js-remove-rules
						        <?php if ( count( $product_restriction_rules ) < 2 ) : ?>hide<?php endif; ?>"><?php esc_html_e( 'Delete Selected', 'ultimatewoo-pro' ); ?></button>
						</th>
					</tr>
