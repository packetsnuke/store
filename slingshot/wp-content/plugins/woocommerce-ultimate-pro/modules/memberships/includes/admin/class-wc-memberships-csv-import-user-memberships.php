...
class WC_Memberships_CSV_Import_User_Memberships extends WC_Memberships_Import_Export {
	...
	public function __construct() {
		...
		$this->action_label = __( 'Upload File and Import', 'ultimatewoo-pro' );
		...
		$docs_button = '<p><a class="button" href="https://docs.woocommerce.com/document/woocommerce-memberships-import-and-export/">' . esc_html__( 'See Documentation', 'ultimatewoo-pro' ). '</a>';

		wc_memberships()->get_admin_notice_handler()->add_admin_notice(
			'<p>' . __( '<strong>Members CSV Import</strong> - Importing members will create or update automatically User Memberships in bulk. Importing members <strong>does not</strong> create any associated billing, subscription or order records.', 'ultimatewoo-pro' ) . '</p>' . $docs_button,
	...
	public function set_admin_page_title( $admin_title, $title ) {
		...
		if ( isset( $_GET['page'] ) && 'wc_memberships_import_export' === $_GET['page'] ) {
			return __( 'Import Members', 'ultimatewoo-pro' ) . ' ' . $admin_title;
		}
	...
	protected function get_fields() {
		...
		$options = array(
			...
			// section start
			array(
				'title' => __( 'Import Members', 'ultimatewoo-pro' ),
				/* translators: Placeholders: %1$s - opening <a> link HTML tag, $2$s - closing </a> link HTML tag */
				'desc'  => sprintf(
					__( 'Your CSV file must be formatted with the correct column names and cell data. Please %1$ssee the documentation%2$s for more information and a sample CSV file.', 'ultimatewoo-pro' ),
			...
			array(
				...
				'title'    => __( 'Choose a file from your computer', 'ultimatewoo-pro' ),
				/* translators: Placeholder: %s - maximum uploadable file size (e.g. 8M, 20M, 100M...)  */
				'desc_tip' => sprintf(
					__( 'Acceptable file types: CSV or tab-delimited text files. Maximum file size: %s', 'ultimatewoo-pro' ),
					empty( $max_upload_size ) ? '<em>' . __( 'Undetermined', 'ultimatewoo-pro' ) . '</em>' : $max_upload_size
				),
			...
			array(
				...
				'title'         => __( 'Import Options', 'ultimatewoo-pro' ),
				'desc'          => __( 'Update existing records if a matching user membership is found (by User Membership ID)', 'ultimatewoo-pro' ),
				'default'       => 'yes',
				'type'          => 'checkbox',
				'checkboxgroup' => 'start',
			),
			...
			array(
				...
				'desc'          => __( 'Allow membership transfer between users if the imported user differs from the existing user for the membership (skips conflicting rows when disabled)', 'ultimatewoo-pro' ),
				'default'       => 'no',
				'type'          => 'checkbox',
				'checkboxgroup' => '',
			),
			...
			array(
				...
				'desc'          => __( 'Create new user memberships if a matching User Membership ID is not found (skips rows when disabled)', 'ultimatewoo-pro' ),
				'default'       => 'yes',
				'type'          => 'checkbox',
				'checkboxgroup' => '',
			),
			...
			array(
				...
				'desc'          => __( 'Create a new user if no matching user is found (skips rows when disabled)', 'ultimatewoo-pro' ),
				'default'       => 'no',
				'type'          => 'checkbox',
				'checkboxgroup' => 'end',
			),
			...
			array(
				...
				'title'       => __( 'Default Start Date', 'ultimatewoo-pro' ),
				'desc'        => __( "When creating new memberships, you can specify a default date to set a membership start date if not defined in the import data. Leave this blank to use today's date otherwise.", 'ultimatewoo-pro' ),
				'default'     => '',
			...
			array(
				...
				'title'    => __( 'Dates timezone', 'ultimatewoo-pro' ),
				'type'     => 'select',
				'desc_tip' => __( 'Choose the timezone the dates in the import are from.', 'ultimatewoo-pro' ),
				'options'  => array(
					 $site_timezone => __( 'Site timezone', 'ultimatewoo-pro' ),
					'UTC'           => __( 'UTC', 'ultimatewoo-pro' ),
				),
			),
			...
			array(
				...
				'title'    => __( 'Fields are separated by', 'ultimatewoo-pro' ),
				'type'     => 'select',
				'desc_tip' => __( 'Change the delimiter based on your input file format.', 'ultimatewoo-pro' ),
				'options'  => array(
					'comma' => __( 'Comma', 'ultimatewoo-pro' ),
					'tab'   => __( 'Tab space', 'ultimatewoo-pro' ),
				),
			),
	...
	public function process_import() {
		...
		if ( empty( $_FILES['wc_memberships_members_csv_import_file'] ) || empty( $_FILES['wc_memberships_members_csv_import_file']['name'] ) ) {
			...
			wc_memberships()->get_admin_instance()->get_message_handler()->add_error(
				__( 'You must upload a file to import User Memberships from.', 'ultimatewoo-pro' )
			);
		...
		// bail out if an upload error occurred (most likely a server issue)
			...
			wc_memberships()->get_admin_instance()->get_message_handler()->add_error(
				...
				sprintf( __( 'There was a problem uploading the file: %s', 'ultimatewoo-pro' ), '<em>' . $this->get_file_upload_error( $_FILES['wc_memberships_members_csv_import_file']['error'] ) . '</em>' )
			);
		...
		// process the file once uploaded
			...
			if ( empty( $csv_data ) || count( $csv_data ) <= 1 ) {
				...
				wc_memberships()->get_admin_instance()->get_message_handler()->add_error(
					__( 'Could not find User Memberships to import from uploaded file.', 'ultimatewoo-pro' )
				);
	...
	private function import_user_membership( $action = '', array $import_data ) {
		...
		if ( ! $user_membership instanceof WC_Memberships_User_Membership ) {
			...
			$import_note = sprintf( __( "Membership created from %s's import.", 'ultimatewoo-pro' ), wp_get_current_user()->display_name );
	...
	private function show_results_notice( $total_rows = 0, $created = 0, $merged = 0 ) {
		...
		if ( 0 === $total_rows ) {
			...
			$notice_type = 'error';
			$message     = __( 'Could not find User Memberships to import from uploaded file.', 'ultimatewoo-pro' );

		} else {

			/* translators: Placeholder: %s - User Memberships to import found in uploaded file */
			$message = sprintf( _n( '%s record found in file.', '%s records found in file.', $total_rows, 'ultimatewoo-pro' ), $total_rows ) . '<br>';
			...
			if ( $rows_processed > 0 ) {
				...
				$message .= ' ' . sprintf( _n( '%s row processed for import.', '%s rows processed for import.', $rows_processed, 'ultimatewoo-pro' ), $rows_processed );

				if ( $created > 0 ) {
					/* translators: Placeholder: %s - User Memberships created in import */
					$message .= ' ' . sprintf( _n( '%s new User Membership created.', '%s new User Memberships created.', $created, 'ultimatewoo-pro' ), $created );
				}

				if ( $merged > 0 ) {
					/* translators: Placeholder: %s - User Memberships updated during import */
					$message .= ' ' . sprintf( _n( '%s existing User Membership updated.', '%s existing User Memberships updated.', $merged, 'ultimatewoo-pro' ), $merged );
				}

				if ( $skipped_rows > 0 ) {
					/* translators: Placeholder: %s - skipped User Memberships to import from file */
					$message .= ' ' . sprintf( _n( '%s row skipped.', '%s rows skipped.', $skipped_rows, 'ultimatewoo-pro' ), $skipped_rows );
				}

			} else {

				$notice_type  = 'error';
				$message     .=  __( 'However, no User Memberships were created or updated with the given options.', 'ultimatewoo-pro' );
			}
		}
	...
	private function get_file_upload_error( $error_code ) {
		...
		switch ( $error_code ) {
			case 1 :
			case 2 :
				return __( 'The file uploaded exceeds the maximum file size allowed.', 'ultimatewoo-pro' );
			case 3 :
				return __( 'The file was only partially uploaded. Please try again.', 'ultimatewoo-pro' );
			case 4 :
				return __( 'No file was uploaded.', 'ultimatewoo-pro' );
			case 6 :
				return __( 'Missing a temporary folder to store the file. Please contact your host.', 'ultimatewoo-pro' );
			case 7 :
				return __( 'Failed to write file to disk. Perhaps a permissions error, please contact your host.', 'ultimatewoo-pro' );
			case 8 :
				return __( 'A PHP Extension stopped the file upload. Please contact your host.', 'ultimatewoo-pro' );
			default :
				return __( 'Unknown error.', 'ultimatewoo-pro' );
		}
	}
