...
class WC_Memberships_Meta_Box_User_Membership_Member_Details extends WC_Memberships_Meta_Box {
	...
	public function get_title() {
		return __( 'Member Details', 'ultimatewoo-pro' );
	}
	...
	public function output( WP_Post $post ) {
		...
		?>
		<h2 class="member-name"><?php echo esc_html( $user->display_name ); ?> <a class="edit-member" href="<?php echo esc_url( get_edit_user_link( $user->ID ) ); ?>">(<?php echo strtolower( esc_html__( 'Edit User', 'ultimatewoo-pro' ) ); ?>)</a></h2>
		...
		<p>
			...
			<?php if ( $member_since = wc_memberships()->get_user_memberships_instance()->get_user_member_since_local_date( $user->ID, 'timestamp' ) ) : ?>
				...
				<span class="member-since"><?php
					...
					printf( __( 'Member since %s', 'ultimatewoo-pro' ),
		...
		<address>
			...
			if ( ! $formatted_address ) {
				esc_html_e( 'User has not set up their billing address yet.', 'ultimatewoo-pro' );
			} else {
