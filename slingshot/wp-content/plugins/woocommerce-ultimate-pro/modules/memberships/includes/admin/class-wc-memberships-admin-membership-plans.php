...
class WC_Memberships_Admin_Membership_Plans {
	...
	public function customize_columns( $columns ) {
		...
		if ( defined( 'WP_DEBUG' ) && true === WP_DEBUG ) {
			$columns['rules'] = __( 'Rules', 'ultimatewoo-pro' );
		}

		$columns['length']  = __( 'Access length', 'ultimatewoo-pro' );
		$columns['access']  = __( 'Access from', 'ultimatewoo-pro' );
		$columns['members'] = __( 'Members', 'ultimatewoo-pro' );
			...
			switch ( $column ) {
				...
				case 'rules':
					...
					if ( defined( 'WP_DEBUG' ) && true === WP_DEBUG ) {
						...
						$rules = array(
							__( 'Restrict content', 'ultimatewoo-pro' )     => ! empty( $content_restriction_rules ),
							__( 'Restrict products', 'ultimatewoo-pro' )    => ! empty( $product_restriction_rules ),
							__( 'Purchasing discounts', 'ultimatewoo-pro' ) => ! empty( $purchasing_discount_rules ),
						);
				...
				case 'access':
					...
					if ( 'manual-only' === $access_method ) {
						esc_html_e( 'Assigned manually', 'ultimatewoo-pro' );
					} elseif ( 'signup' === $access_method ) {
						esc_html_e( 'Account registration', 'ultimatewoo-pro' );
					} elseif ( 'purchase' === $access_method ) {
						esc_html_e( 'Purchase', 'ultimatewoo-pro' );
				...
				case 'members':
					...
					echo '<a href="' . esc_url( $view_members ) . '" title="' . esc_html__( 'View Members', 'ultimatewoo-pro' ) . '">';
			...
			foreach ( $product_ids as $product_id ) {
				...
				if ( $product = wc_get_product( $product_id ) )  {
					...
					printf(
						...
						$is_subscription ? ' <small>(' . strtolower( __( 'Subscription', 'ultimatewoo-pro' ) ) . ')</small> ' : ''
					);
				}
			}
		...
		if ( 'wc_membership_plan' === $post->post_type ) {
			...
			if ( $plan && isset( $actions['trash'] ) && $plan->has_active_memberships() ) {
				...
				if ( 'trash' === $post->post_status ) {
					$tip = esc_attr__( 'This item cannot be restored because it has active members.', 'ultimatewoo-pro' );
				} elseif ( EMPTY_TRASH_DAYS ) {
					$tip = esc_attr__( 'This item cannot be moved to trash because it has active members.', 'ultimatewoo-pro' );
				}

				if ( 'trash' === $post->post_status || ! EMPTY_TRASH_DAYS ) {
					$tip = esc_attr__( 'This item cannot be permanently deleted because it has active members.', 'ultimatewoo-pro' );
				}

				$actions['trash'] = '<span title="' . $tip . '" style="cursor: help;">' . strip_tags( $actions['trash'] ) . '</span>';
			}

			$duplicate_link_open  = '<a href="' . wp_nonce_url( admin_url( 'edit.php?post_type=wc_membership_plan&action=duplicate_plan&amp;post=' . $post->ID ), 'wc-memberships-duplicate-plan_' . $post->ID ) . '" title="' . __( 'Make a duplicate from this membership plan', 'ultimatewoo-pro' ) . '" rel="permalink">';
			$duplicate_link_close = '</a>';

			// add duplicate plan action
			$actions['duplicate'] = $duplicate_link_open . _x( 'Duplicate', 'Duplicate a Membership Plan', 'ultimatewoo-pro' ) . $duplicate_link_close;
		}
		...
		if ( 'post.php' === $pagenow ) :
			...
			<div class="misc-pub-section misc-pub-grant-access" <?php echo $plan->is_access_method( 'purchase' ) ? '' : 'style="display: none;"'; ?>>
				<span class="grant-access">
					<?php esc_html_e( 'Existing purchases:', 'ultimatewoo-pro' ); ?>
					<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'action', 'grant_access', get_edit_post_link( $post->ID ) ), 'wc-memberships-grant-access-plan_' . $post->ID ) ); ?>" class="button" id="grant-access"><?php esc_html_e( 'Grant Access', 'ultimatewoo-pro' ); ?></a>
				</span>
			</div>
			<?php
	...
	public function grant_access_to_membership() {
		...
		if ( $grant_count > 0 ) {
			$message = sprintf( _n( '%d order found that granted or extended access from existing purchases.', '%d orders found that granted or extended access from existing purchases.', $grant_count, 'ultimatewoo-pro' ), $grant_count );
			wc_memberships()->get_admin_instance()->get_message_handler()->add_message( $message );
		} else {
			$message = __( 'No orders found to grant or extend access from existing purchases.', 'ultimatewoo-pro' );
	...
	public function duplicate_button() {
		...
		if ( isset( $_GET['post'] ) ) {
			...
			<div id="duplicate-action">
				<a class="submitduplicate duplication" href="<?php echo esc_url( $url ); ?>"><?php esc_html_e( 'Make a copy', 'ultimatewoo-pro' ); ?></a>
			</div>
			<?php
		}
	}
	...
	public function duplicate_membership_plan() {
		...
		if ( is_object( $post ) ) {
			...
			if ( $new_id > 0 ) {
				...
				wc_memberships()->get_admin_instance()->get_message_handler()->add_message( __( 'Membership plan copied.', 'ultimatewoo-pro' ) );
		...
		wp_die( sprintf( __( 'Membership plan creation failed, could not find original plan to copy: %d', 'ultimatewoo-pro' ), (int) $id ) );
	}
	...
	public function duplicate_plan( $post, $parent = 0, $post_status = 'publish' ) {
		...
		if ( is_object( $post ) ) {
			...
			if ( $parent > 0 ) {
				...
				$suffix      = ' ' . __( '(Copy)', 'ultimatewoo-pro' );
			}
