...
class WC_Memberships_Meta_Box_Product_Memberships_Data extends WC_Memberships_Meta_Box {
	...
	public function get_title() {
		return __( 'Memberships', 'ultimatewoo-pro' );
	}
	...
	public function output( WP_Post $post ) {
		...
		<p class="grouped-notice <?php if ( ! $product->is_type( 'grouped' ) ) : ?>hide<?php endif; ?>">
			<?php esc_html_e( 'Memberships do not support grouped products.', 'ultimatewoo-pro' ); ?>
		</p>
		...
		<div class="panel-wrap wc-memberships-data <?php if ( $product->is_type( 'grouped' ) ) : ?>hide<?php endif; ?>">
			...
			<ul class="memberships_data_tabs wc-tabs">
				...
				$memberships_data_tabs = apply_filters( 'wc_memberships_product_data_tabs', array(
					...
					'grant_access' => array(
						'label'  => __( 'Grant Access', 'ultimatewoo-pro' ),
						'target' => 'memberships-data-grant-access',
					),
					'purchasing_discounts' => array(
						'label'  => __( 'Discounts', 'ultimatewoo-pro' ),
	...
	private function output_restrict_product_panel( $product, $post ) {
		...
		<div id="memberships-data-restrict-product" class="panel woocommerce_options_panel">
			...
			woocommerce_wp_checkbox( array(
				...
				'label'       => __( 'Disable restrictions', 'ultimatewoo-pro' ),
				'description' => __( 'Check this box if you want to force this product to be public regardless of any restriction rules that may apply now or in the future.', 'ultimatewoo-pro' ),
			) );
			...
			<div class="js-restrictions <?php if ( 'yes' === wc_memberships_get_content_meta( $post->ID, '_wc_memberships_force_public', true ) ) : ?>hide<?php endif; ?>">
				...
				if ( ! empty( $membership_plans ) ) :
					...
					?><p><em><?php esc_html_e( 'Need to add or edit a plan?', 'ultimatewoo-pro' ); ?></em> <a target="_blank" href="<?php echo esc_url( admin_url( 'edit.php?post_type=wc_membership_plan' ) ); ?>"><?php esc_html_e( 'Manage Membership Plans', 'ultimatewoo-pro' ); ?></a></p><?php
				...
				<div class="options_group">
					...
					woocommerce_wp_checkbox( array(
						...
						'label'       => __( 'Use custom message', 'ultimatewoo-pro' ),
						'description' => __( 'Check this box if you want to customize the <strong>viewing restricted message</strong> for this product.', 'ultimatewoo-pro' )
					) );
					...
					<div
						...
						printf( '<p>' . __( '%1$s automatically inserts the product(s) needed to gain access. %2$s inserts the URL to my account page. HTML is allowed.', 'ultimatewoo-pro' ) . '</p>',
				...
				<div class="options_group">
					...
					woocommerce_wp_checkbox( array(
						...
						'class'       => 'js-toggle-custom-message',
						'label'       => __( 'Use custom message', 'ultimatewoo-pro' ),
						'description' => __( 'Check this box if you want to customize the <strong>purchasing restricted message</strong> for this product.', 'ultimatewoo-pro' )
					) );
	...
	private function output_grant_access_panel( $product, $post ) {
		...
		<div id="memberships-data-grant-access" class="panel woocommerce_options_panel">
			...
			<p class="variable-notice <?php if ( ! $product->is_type( 'variable' ) ) : ?>hide<?php endif; ?>">
				...
				printf( __( 'These settings affect all variations. For variation-level control use the %1$smembership plan screen%2$s.', 'ultimatewoo-pro' ), '<a href="' . admin_url( 'edit.php?post_type=wc_membership_plan' ) . '">', '</a>' ); ?>
			</p>
			...
			<div class="options_group">
				...
				<?php if ( empty( $membership_plans ) ) : ?>
					...
					<p>
						...
						printf( __( 'To grant membership access, please %1$sAdd a Membership Plan%2$s', 'ultimatewoo-pro' ), '<a target="_blank" href="' . esc_url( admin_url( 'post-new.php?post_type=wc_membership_plan' ) ) . '">', '</a>' ); ?>
					</p>

				<?php else : ?>

					<p class="form-field">
						<label for="_wc_memberships_membership_plan_ids"><?php esc_html_e( 'Purchasing product grants access to', 'ultimatewoo-pro' ); ?></label>
						...
						<?php if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ) : ?>
							...
							<select
								...
								style="width: 90%;"
								multiple="multiple"
								data-placeholder="<?php esc_attr_e( 'Search for a membership plan&hellip;', 'ultimatewoo-pro' ); ?>"
						...
						<?php echo wc_help_tip( __( 'Select which membership plans does purchasing this product grant access tp.', 'ultimatewoo-pro' ) ); ?>
					</p>
					...
					if ( $this->product->is_type( 'variable' ) && ( $variations = $this->product->get_children() ) ) {
						...
						<?php if ( ! empty( $variation_grants_access_to_plans ) ) : ?>
							...
							<p class="form-field">
								<label><?php esc_html_e( 'Purchasing individual variations grants access to', 'ultimatewoo-pro' ) ?></label>
								<br />
								<span>
									<?php foreach ( $variation_grants_access_to_plans as $variation_id => $access_plan ) : ?>
										...
										<?php if ( $variation_product = wc_get_product( $variation_id ) ) : ?>
											<strong><?php echo esc_html( $variation_product->get_name() ); ?></strong>: <?php echo wc_memberships_list_items( $variation_grants_access_to_plans, __( 'and', 'ultimatewoo-pro' ) ); ?>.<br />
										<?php endif; ?>
					...
					<p><em><?php esc_html_e( 'Need to add or edit a plan?', 'ultimatewoo-pro' ); ?></em> <a target="_blank" href="<?php echo esc_url( admin_url( 'edit.php?post_type=wc_membership_plan' ) ); ?>"><?php esc_html_e( 'Manage Membership Plans', 'ultimatewoo-pro' ); ?></a>.</p>
	...
	private function output_purchasing_discounts_panel( $product, $post ) {
		...
		<div id="memberships-data-purchasing-discounts" class="panel woocommerce_options_panel">
			...
			woocommerce_wp_checkbox( array(
				...
				'label'       => __( 'Disable discounts', 'ultimatewoo-pro' ),
				'description' => __( 'Check this box if you want to exclude this product from member discounts of any membership plan, regardless of discount rules that may apply now or in the future.', 'ultimatewoo-pro' ),
			) );
