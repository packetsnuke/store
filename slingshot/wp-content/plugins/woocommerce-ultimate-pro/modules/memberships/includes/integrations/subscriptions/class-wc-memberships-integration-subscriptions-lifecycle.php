...
class WC_Memberships_Integration_Subscriptions_Lifecycle {
	...
	public function pause_free_trial_subscription_memberships() {
		...
		foreach ( $posts as $post ) {
			...
			$user_membership->pause_membership( __( 'Membership paused because WooCommerce Subscriptions was deactivated.', 'ultimatewoo-pro' ) );
		}
	}
			...
			if ( ! $integration->has_subscription_same_status( $subscription, $user_membership ) ) {
				...
				if ( 'active' === $subscription_status && 'paused' === $user_membership->get_status() ) {
					...
					if ( ! $trial_end || current_time( 'timestamp', true ) >= $trial_end ) {
						$user_membership->activate_membership( __( 'Membership activated because WooCommerce Subscriptions was activated.', 'ultimatewoo-pro' ) );
					// ...otherwise, put the membership on free trial
					} else {
						$user_membership->update_status( 'free_trial', __( 'Membership free trial activated because WooCommerce Subscriptions was activated.', 'ultimatewoo-pro' ) );
