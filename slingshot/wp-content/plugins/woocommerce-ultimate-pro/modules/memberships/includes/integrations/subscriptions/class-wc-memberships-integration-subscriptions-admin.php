...
class WC_Memberships_Integration_Subscriptions_Admin {
	...
	private function get_edit_subscription_input( $user_membership, $subscription = null ) {
		...
		if ( $subscription && $subscription instanceof WC_Subscription ) {
			...
			$selected        = array();
			$subscription_id = '';
			$subscription_link = esc_html__( 'Membership not linked to a Subscription', 'ultimatewoo-pro' );
		}

		/* translators: Placeholders: %1$s - link to a Subscription, %2$s - opening <a> HTML tag, %3%s - closing </a> HTML tag */
		$input = sprintf( __( '%1$s - %2$sEdit Link%3$s', 'ultimatewoo-pro' ),
			$subscription_link,
		...
		<span class="wc-memberships-edit-subscription-link-field">
			...
			<?php if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ) : ?>
				...
				<select
					...
					data-placeholder="<?php esc_attr_e( 'Link to a Subscription or keep empty to leave unlinked', 'ultimatewoo-pro' ); ?>"
	...
	public function add_subscription_details( $fields, $user_membership ) {
		...
		$fields[ __( 'Subscription:', 'ultimatewoo-pro' ) ] = $edit_subscription_input;
		$fields[ __( 'Next Bill On:', 'ultimatewoo-pro' ) ] = $next_payment ? date_i18n( wc_date_format(), $next_payment ) : esc_html__( 'N/A', 'ultimatewoo-pro' );
	...
	public function output_subscription_options() {
		...
		<p class="form-field plan-subscription-access-length-field js-show-if-has-subscription <?php if ( ! $has_subscription ) : ?>hide<?php endif; ?>">
			<label for="_subscription_access_length"><?php esc_html_e( 'Subscription-tied Membership length', 'ultimatewoo-pro' ); ?></label>
			...
			<span class="plan-subscription-access-length-selectors">
				...
				$subscription_access_length_period_toggler_options = array(
					'unlimited'    => __( 'Unlimited', 'ultimatewoo-pro' ),
					/* translators: Membership of an unlimited length */
					'subscription' => __( 'Subscription length', 'ultimatewoo-pro' ),
					/* translators: Specify the length of a membership */
					'specific'     => __( 'Specific length', 'ultimatewoo-pro' ),
					/* translators: Membership set to expire in a specified date */
					'fixed'        => __( 'Fixed dates', 'ultimatewoo-pro' )
				);
				...
				echo wc_help_tip( __( 'When does the membership tied to a subscription expire?', 'ultimatewoo-pro' ) );
			...
			<span class="subscription-access-notice description js-show-if-subscription-access-length-unlimited" <?php if ( 'unlimited' !== $current_subscription_access_length_type ) : ?>style="display: none;"<?php endif; ?>>
				<?php esc_html_e( 'The membership will be active indefinitely, even after the subscription billing cycle is complete, as long as it has been fully paid.', 'ultimatewoo-pro' ); ?>
					<?php echo wc_help_tip( __( 'When unlimited access is granted via the purchase of a subscription, the membership will be active for the period of the subscription length, and will stay active beyond that as long as the customer successfully completed the subscription billing cycle.', 'ultimatewoo-pro' ) ); ?>
			</span>
			...
			<span class="subscription-access-notice description js-show-if-subscription-access-length-subscription" <?php if ( 'subscription' !== $current_subscription_access_length_type ) : ?>style="display: none;"<?php endif; ?>>
				<?php esc_html_e( 'The membership will be active as long as the purchased subscription stays active.', 'ultimatewoo-pro' ); ?>
				<?php echo wc_help_tip( __( 'When access is granted via the purchase of a subscription, the membership length becomes tied to the length of the subscription.', 'ultimatewoo-pro' ) ); ?>
			</span>
			...
			<span class="plan-access-length-specific plan-subscription-access-length-specific js-show-if-subscription-access-length-specific <?php if ( 'specific' !== $current_subscription_access_length_type ) : ?>hide<?php endif;?>">
				...
				<span class="subscription-access-notice description">
					<?php esc_html_e( 'The membership will be active for the length specified above, regardless of billing dates, so long as the subscription has been fully paid.', 'ultimatewoo-pro' ); ?>
					<?php echo wc_help_tip( __( 'When membership access is granted via the purchase of a subscription, then membership length will last for the specified period, regardless of the subscription length, as long as the customer pays for the subscription costs.', 'ultimatewoo-pro' ) ); ?>
				</span>
			...
			<span class="plan-access-length-fixed plan-subscription-access-length-fixed js-show-if-access-length-fixed <?php if ( 'fixed' !== $current_subscription_access_length_type ) : ?>hide<?php endif;?>">
				...
				?>
				<span>
					<label for="_subscription_access_start_date"><?php esc_html_e( 'Start date', 'ultimatewoo-pro' ); ?></label>
					<input
						type="text"
				...
				<span>
					<label for="_subscription_access_end_date"><?php esc_html_e( 'End date', 'ultimatewoo-pro' ); ?></label>
					<input
						type="text"
				...
				<span class="subscription-access-notice description">
					<?php esc_html_e( 'The membership will be active between the selected dates, regardless of billing dates, so long as the subscription has been fully paid.', 'ultimatewoo-pro' ); ?>
					<?php echo wc_help_tip( __( 'When membership access is granted via the purchase of a subscription, the membership will last until the specified date, regardless of the subscription sign up date and the subscription length, as long as the customer pays for the subscription costs.', 'ultimatewoo-pro' ) ); ?>
				</span>
	...
	public function output_exclude_trial_option( $rule, $index ) {
		...
		<span class="rule-control-group rule-control-group-access-schedule-trial <?php if ( ! $has_subscription ) : ?>hide<?php endif; ?> js-show-if-has-subscription">
			...
			<label for="_<?php echo esc_attr( $type ); ?>_rules_<?php echo $index; ?>_access_schedule_exclude_trial" class="label-checkbox">
				<?php esc_html_e( 'Start after trial', 'ultimatewoo-pro' ); ?>
			</label>
		...
		if (    'length' === $column
			...
			if ( ( $human_length !== $sub_human_length ) || $subscription_tied ) {
				...
				if ( $subscription_tied ) {
					$sub_human_length = __( 'Subscription length', 'ultimatewoo-pro' );
				}

				/* translators: Placeholder: %s - length of a Subscription-tied membership plan */
				echo $membership_plan->get_products( true ) ? '<br><small>' . sprintf( __( 'Subscription based: %s', 'ultimatewoo-pro' ), $sub_human_length ) . '</small>' : $sub_human_length;
			}
		}
	}
			...
			if ( $subscription instanceof WC_Subscription ) {
				...
				$actions = array_merge( array(
					'delete-with-subscription' => array(
						...
						'link'              => '#',
						'text'              => __( 'Delete User Membership with Subscription', 'ultimatewoo-pro' ),
						'custom_attributes' => array(
							'data-user-membership-id' => $user_membership_id,
							'data-subscription-id'    => SV_WC_Order_Compatibility::get_prop( $subscription, 'id' ),
							'data-tip'                => __( 'Delete this membership permanently and the subscription associated with it', 'ultimatewoo-pro' ),
						),
					),
				), $actions );
			}
				...
				if (    'specific' === $posted_data['_subscription_access_length']
					...
					if ( $subscription_access_start_time >= $subscription_access_end_time ) {
						...
						wc_memberships()->get_admin_instance()->get_message_handler()->add_error(
							...
							sprintf( __( 'Subscription-tied membership plan: %s', 'ultimatewoo-pro' ),
