...
class WC_Memberships_Integration_User_Switching {
	...
	public function customize_membership_plan_row_actions( $actions, WP_Post $post ) {
		...
		$actions['view_as_member'] = '<a href="' . wp_nonce_url( admin_url( 'edit.php?post_type=wc_membership_plan&action=view_as_member&amp;post=' . $post->ID ), 'wc-memberships-view-as-member-of_' . $post->ID ) . '" title="' . __( 'View site as a member of this plan', 'ultimatewoo-pro' ) . '" rel="permalink">' .
		                             __( 'View site as member', 'ultimatewoo-pro' ) .
		                             '</a>';
	...
	public function view_as_member() {
		...
		$temp_user_data = apply_filters( 'wc_memberships_temporary_user_data', array(
			...
			'first_name'  => sprintf( __( '%s Plan', 'ultimatewoo-pro' ), $plan->get_name() ),
			'last_name'   => __( 'Test User', 'ultimatewoo-pro' ),
			/* translators: %s - Membership Plan name */
			'description' => sprintf( __( "A temporary user created for testing the %s membership plan. If you don't use it, feel free to delete this user.", 'ultimatewoo-pro' ), $plan->get_name() ),
			'role'        => 'customer',
		), $plan );
