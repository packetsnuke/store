...
class WC_Memberships_Integration_Groups {
	...
	public function groups_integration_settings( $settings ) {
		...
		$settings[] = array(
			'name' => __( 'Groups Integration', 'ultimatewoo-pro' ),
			'type' => 'title',
		...
		$settings[] = array(
			...
			'title'             => __( 'Groups Import', 'ultimatewoo-pro' ),
			'class'             => 'button import-groups',
			'desc_tip'          => __( 'Click this button to import memberships from WordPress Groups plugin.', 'ultimatewoo-pro' ),
			'default'           => __( 'Import Members from Groups', 'ultimatewoo-pro' ),
		);
	...
	public function admin_title( $title ) {
		...
		if ( isset( $_GET['page'] ) && 'wc-memberships-import-groups' === $_GET['page'] ) {
			$title = _x( 'Import members from Groups', 'Page title', 'ultimatewoo-pro' ) . $title;
		}
	...
	public function render_import_page() {
		...
		if ( ! count( $this->get_groups() ) ) {
			...
			esc_html_e( 'There are no groups to import.', 'ultimatewoo-pro' );
			return;
		}
		...
		<form method="post" id="import_groups">
			...
			<div class="wrap">
				...
				<h2><?php esc_html_e( 'Import Members from Groups', 'ultimatewoo-pro' ); ?></h2>
				<p><?php esc_html_e( 'For each group, specify which plan should we import members to.', 'ultimatewoo-pro' ); ?></p>

				<?php foreach ( $this->get_groups() as $group ) : ?>

					<fieldset>
						<h4><?php printf( /* translators: %s - Group name, %d - Group ID */
								__( 'Group: %s (ID #%d) ', 'ultimatewoo-pro' ), $group->name, $group->group_id ); ?></h4>

						<p>
							<legend><?php esc_html_e( 'What should be done with members of this group?', 'ultimatewoo-pro' ) ?></legend>
						</p>
						...
						<ul style="list-style:none;">
							<li>
								<label>
									...
									<?php esc_html_e( 'Skip', 'ultimatewoo-pro' ); ?>
								</label>
							</li>
							...
							<li>
								<label>
									<input
									    ...
									    class="js-import-action"
									/>
									<?php esc_html_e( 'Import all members to:', 'ultimatewoo-pro' ); ?>
								</label>
				...
				<p class="submit">
					...
					<input
						...
						id="submit"
						class="button"
						value="<?php esc_attr_e( 'Confirm Import', 'ultimatewoo-pro' ); ?>"
						disabled=""
					/>
	...
	public function import_groups() {
		...
		// add admin message
		if ( $import_count ) {
			$message = sprintf( _n( '%d member imported from Groups.', '%d members imported from Groups', $import_count, 'ultimatewoo-pro' ), $import_count );
		} else {
			$message = __( 'No members were imported from Groups.', 'ultimatewoo-pro' );
		}
