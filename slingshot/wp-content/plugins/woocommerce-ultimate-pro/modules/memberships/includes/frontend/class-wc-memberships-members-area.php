...
class WC_Memberships_Members_Area {
	...
	private function get_members_area_memberships_endpoint_title( $plan = null ) {
		...
		$endpoint_title = __( 'Memberships', 'ultimatewoo-pro' );
	...
	public function get_members_area_navigation_items( $membership_plan ) {
		...
		$menu_items         = array( 'back-to-memberships' => array(
			...
			'label' => sprintf( __( 'Back to %s', 'ultimatewoo-pro' ), 1 === $memberships_count ? __( 'Dashboard', 'ultimatewoo-pro' ) : $this->get_members_area_memberships_endpoint_title() ),
			'class' => '',
		) );
	...
	private function get_members_area_user_membership_details( $user_membership ) {
		...
		$details = array(
			...
			'start-date' => array(
				...
				'content' => $user_membership->has_start_date() ? date_i18n( wc_date_format(), $user_membership->get_local_start_date( 'timestamp' ) ) : esc_html__( 'N/A', 'ultimatewoo-pro' ),
				'class'   => 'my-membership-detail-user-membership-start-date',
			),
			'expires' => array(
				'label'   => __( 'Expires', 'ultimatewoo-pro' ),
				'content' => $user_membership->has_end_date() ? date_i18n( wc_date_format(), $user_membership->get_local_end_date( 'timestamp' ) ) : esc_html__( 'N/A', 'ultimatewoo-pro' ),
				'class'   => 'my-membership-detail-user-membership-expires',
			),
			'actions' => array(
				'label'   => __( 'Actions', 'ultimatewoo-pro' ),
