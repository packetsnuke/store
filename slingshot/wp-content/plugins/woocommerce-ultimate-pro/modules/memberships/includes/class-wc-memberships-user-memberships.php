...
class WC_Memberships_User_Memberships {
	...
	public function create_user_membership( $args = array(), $action = 'create' ) {
		...
		if ( ! wc_memberships_get_membership_plan( $args['plan_id'] ) ) {
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Cannot create User Membership: Membership Plan with ID %d does not exist', 'ultimatewoo-pro' ), (int) $args['plan_id'] ) );
		}
		...
		if ( 0 === $user_membership_id || is_wp_error( $user_membership_id ) ) {
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Cannot create User Membership: %s', 'ultimatewoo-pro' ), implode( ', ', $user_membership_id->get_error_messages() ) ) );
		}
	...
	public function get_user_membership_statuses() {
		...
		$statuses = array(
			...
			'wcm-active'        => array(
				'label'       => _x( 'Active', 'Membership Status', 'ultimatewoo-pro' ),
			...
			'wcm-delayed'       => array(
				'label'       => _x( 'Delayed', 'Membership status', 'ultimatewoo-pro' ),
				/* translators: Delayed Membership(s) */
				'label_count' => _n_noop( 'Delayed <span class="count">(%s)</span>', 'Delayed <span class="count">(%s)</span>', 'ultimatewoo-pro' ),
			),

			'wcm-complimentary' => array(
				'label'       => _x( 'Complimentary', 'Membership Status', 'ultimatewoo-pro' ),
				/* translators: Complimentary Membership(s) */
				'label_count' => _n_noop( 'Complimentary <span class="count">(%s)</span>', 'Complimentary <span class="count">(%s)</span>', 'ultimatewoo-pro' ),
			),

			'wcm-pending'       => array(
				'label'       => _x( 'Pending Cancellation', 'Membership Status', 'ultimatewoo-pro' ),
				/* translators: Membership(s) Pending Cancellation */
				'label_count' => _n_noop( 'Pending Cancellation <span class="count">(%s)</span>', 'Pending Cancellation <span class="count">(%s)</span>', 'ultimatewoo-pro' ),
			),

			'wcm-paused'        => array(
				'label'       => _x( 'Paused', 'Membership Status', 'ultimatewoo-pro' ),
				/* translators: Paused Membership(s) */
				'label_count' => _n_noop( 'Paused <span class="count">(%s)</span>', 'Paused <span class="count">(%s)</span>', 'ultimatewoo-pro' ),
			),

			'wcm-expired'       => array(
				'label'       => _x( 'Expired', 'Membership Status', 'ultimatewoo-pro' ),
			...
			'wcm-cancelled'     => array(
				'label'       => _x( 'Cancelled', 'Membership Status', 'ultimatewoo-pro' ),
	...
	public function transition_post_status( $new_status, $old_status, WP_Post $post ) {
		...
		$status_note   = sprintf( __( 'Membership status changed from %1$s to %2$s.', 'ultimatewoo-pro' ), wc_memberships_get_user_membership_status_name( $old_status ), wc_memberships_get_user_membership_status_name( $new_status ) );
	...
	public function handle_order_trashed( $order_id ) {
		...
		$this->handle_order_cancellation( $order_id, __( 'Membership cancelled because the associated order was trashed.', 'ultimatewoo-pro' ) );
	}
	...
	public function handle_order_refunded( $order_id ) {
		...
		$this->handle_order_cancellation( $order_id, __( 'Membership cancelled because the associated order was refunded.', 'ultimatewoo-pro' ) );
	}
