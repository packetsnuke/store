...
class WC_Memberships_User_Membership_Renewal_Reminder_Email extends WC_Memberships_User_Membership_Email {
	...
	public function __construct() {
		...
		$this->id             = __CLASS__;
		$this->title          = __( 'Membership renewal reminder', 'ultimatewoo-pro' );

		/* translators: Placeholders: %1$s - <strong> tag, %2$s - </strong> tag */
		$description  =  sprintf( __( 'Membership renewal reminder emails are sent to former plan members inviting them to renew their membership. %1$sRenewal reminders are not sent for non-renewable memberships, such as registration-based or admin-assign only memberships%2$s.', 'ultimatewoo-pro' ), '<strong>', '</strong>' );
		/* translators: Placeholders: %1$s - Opening <a> HTML tag, %2$s - Closing </a> HTML tag */
		$description .= '<br>' . sprintf( __( 'You can edit the content of this email for %1$seach one of your plans%2$s individually.', 'ultimatewoo-pro' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=wc_membership_plan' )  ) . '">', '</a>' );

		$this->description    = $description;
		$this->subject        = __( 'Renew your {site_title} membership!', 'ultimatewoo-pro');
		$this->heading        = __( 'You can renew your {membership_plan}', 'ultimatewoo-pro');
	...
	public function init_form_fields() {
		...
		if ( isset( $form_fields['enabled'] ) ) {
			...
			$form_fields = SV_WC_Helper::array_insert_after( $form_fields, 'enabled', array(
				'send_days_after' => array(
					'title'             => __( 'Send Email Days After', 'ultimatewoo-pro' ),
					'type'              => 'number',
					'css'               => 'width: 50px;',
					/* translators: Days after a membership has expired */
					'description'       => __( 'day(s) after', 'ultimatewoo-pro' ),
					'desc_tip'          => __( "Number of days after the membership has expired the email will be sent to users who haven't renewed their membership yet.", 'ultimatewoo-pro' ),
					'default'           => 1,
		...
		if ( isset( $form_fields['subject'] ) ) {
			...
			$form_fields['subject']['description'] = sprintf( __( '%s inserts your site name.', 'ultimatewoo-pro' ), '<strong><code>{site_name}</code></strong>' );
		}
		...
		if ( isset( $form_fields['heading'] ) ) {
			...
			$form_fields['heading']['description'] = sprintf( __( '%s inserts the membership plan name.', 'ultimatewoo-pro' ), '<strong><code>{membership_plan}</code></strong>' );
		}
