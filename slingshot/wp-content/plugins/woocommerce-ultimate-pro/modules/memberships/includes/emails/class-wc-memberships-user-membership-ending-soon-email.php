...
class WC_Memberships_User_Membership_Ending_Soon_Email extends WC_Memberships_User_Membership_Email {
	...
	public function __construct() {
		...
		$this->title          = __( 'Membership ending soon', 'ultimatewoo-pro' );

		$description  = __( 'Membership ending soon emails are sent to plan members when their membership is about to expire.', 'ultimatewoo-pro' );
		...
		$this->subject        = __( 'Your {site_title} membership ends soon!', 'ultimatewoo-pro');
		$this->heading        = __( 'An update about your {membership_plan}', 'ultimatewoo-pro');
		...
		if ( isset( $form_fields['enabled'] ) ) {
			...
			$form_fields = SV_WC_Helper::array_insert_after( $form_fields, 'enabled', array(
				'send_days_before' => array(
					'title'             => __( 'Send Email Days Before', 'ultimatewoo-pro' ),
					'type'              => 'number',
					'css'               => 'width: 50px;',
					/* translators: Days before a membership expires */
					'description'       => __( 'day(s) before', 'ultimatewoo-pro' ),
					'desc_tip'          => __( "Number of days before the membership expires the email will be sent. Note: this shouldn't exceed the length of the Membership plan itself.", 'ultimatewoo-pro' ),
					'default'           => 3,
