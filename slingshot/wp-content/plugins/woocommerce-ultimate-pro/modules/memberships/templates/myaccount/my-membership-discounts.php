...
<?php if ( empty ( $discounted_products->posts ) ) : ?>
	...
	<p><?php esc_html_e( 'There are no discounts available for this membership.', 'ultimatewoo-pro' ); ?></p>
		...
		<thead>
			<tr>
				...
				$my_membership_discounts_columns = (array) apply_filters( 'wc_memberships_members_area_my_membership_discounts_column_names', array(
					...
					'membership-discount-title'   => esc_html__( 'Title', 'ultimatewoo-pro' ),
					'membership-discount-amount'  => esc_html__( 'Discount', 'ultimatewoo-pro' ),
					'membership-discount-price'   => esc_html__( 'My Price', 'ultimatewoo-pro' ),
					'membership-discount-excerpt' => esc_html__( 'Description', 'ultimatewoo-pro' ),
							...
							<td class="membership-discount-amount" data-title="<?php echo esc_attr( $column_header ); ?>">
								...
								<?php else : ?>
									<time datetime="<?php echo date( 'Y-m-d', $purchase_start_time ); ?>" title="<?php echo esc_attr( $purchase_start_time ); ?>">
										...
										<?php echo esc_html( sprintf( __( 'Available from %s', 'ultimatewoo-pro' ), date_i18n( get_option( 'date_format' ), $purchase_start_time ) ) ); ?>
									</time>
								<?php endif; ?>
							</td>
			...
			<?php if ( $available_discounts < 1 ) : ?>
				<tr>
					<td colspan="<?php echo count( $my_membership_discounts_columns ); ?>">
						<span class="membership-discounts-no-active-discounts"><?php esc_html_e( 'There are no member discounts currently active.', 'ultimatewoo-pro' ); ?></span>
					</td>
				</tr>
			<?php endif; ?>
