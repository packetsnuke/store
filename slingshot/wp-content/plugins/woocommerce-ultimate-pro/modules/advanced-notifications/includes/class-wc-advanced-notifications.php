...
class WC_Advanced_Notifications {
	...
	public function new_order( $order_id ) {
		...
		if ( $notifications ) {
			...
			$email_heading = __( 'New Customer Order', 'ultimatewoo-pro' );

			$subject = apply_filters( 'woocommerce_email_subject_new_order', sprintf( __( '[%s] New Order (%s)', 'ultimatewoo-pro' ), $blogname, $order->get_order_number() ), $order );
	...
	public function low_stock( $product ) {
		...
		if ( $notifications ) {
			...
			$subject = apply_filters( 'woocommerce_email_subject_low_stock', sprintf( '[%s] %s', $blogname, __( 'Product low in stock', 'ultimatewoo-pro' ) ), $product );
			...
			if ( ! empty( $variation_id ) )
				$title = sprintf(__('Variation #%s of %s', 'ultimatewoo-pro' ), $variation_id, get_the_title( $product->get_id() ) ) . ' ' . $sku;
			else
				$title = sprintf(__('Product #%s - %s', 'ultimatewoo-pro' ), $product->get_id(), get_the_title( $product->get_id() ) ) . ' ' . $sku;

			foreach ( $notifications as $notification ) {

				$message = sprintf( __( 'Hi %s,', 'ultimatewoo-pro' ), $notification->recipient_name ) . "\n\n";

				$message .= $title . __('is low in stock.', 'ultimatewoo-pro' ) . "\n\n";
	...
	public function out_of_stock( $product ) {
		...
		if ( $notifications ) {
			...
			$subject = apply_filters( 'woocommerce_email_subject_no_stock', sprintf( '[%s] %s', $blogname, __( 'Product out of stock', 'ultimatewoo-pro' ) ), $product );
			...
			if ( ! empty( $variation_id ) ) {
				...
				$title = sprintf( __('Product #%1$s - %2$s', 'ultimatewoo-pro' ), $product->get_id(), get_the_title( $product->get_id() ) ) . ' ' . $sku;
			}
			...
			foreach ( $notifications as $notification ) {
				...
				$message .= $title . __('is out of stock.', 'ultimatewoo-pro' ) . "\n\n";
	...
	public function backorder( $args ) {
		...
		if ( $notifications ) {
			...
			$subject = apply_filters( 'woocommerce_email_subject_backorder', sprintf( '[%s] %s', $blogname, __( 'Product Backorder', 'ultimatewoo-pro' ) ), $product );
			...
			if ( ! empty( $variation_id ) ) {
				$title = sprintf( __( 'Variation #%1$s of %2$s', 'ultimatewoo-pro' ), $variation_id, get_the_title( $product->get_id() ) ) . $sku;
			} else {
				$title = sprintf(__('Product #%s - %s', 'ultimatewoo-pro' ), $product->get_id(), get_the_title( $product->get_id() ) ) . $sku;
			}
			...
			foreach ( $notifications as $notification ) {
				...
				$message .= sprintf(__('%s units of %s have been backordered in order #%s.', 'ultimatewoo-pro' ), $quantity, $title, $order_id ) . "\n\n";
	...
	public function refund( $order_id ) {
		...
		if ( $notifications ) {
			...
			// Prepare email.
			$blogname = wp_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
			$email_heading = __( 'Customer Refund', 'ultimatewoo-pro' );

			$subject = apply_filters( 'woocommerce_email_subject_refund', sprintf( __( '[%s] Order Refund (%s)', 'ultimatewoo-pro' ), $blogname, $order->get_order_number() ), $order );
