...
class WC_Advanced_Notifications_Admin {
	...
	public function screen_ids( $ids ) {
		$wc_screen_id = strtolower( __( 'WooCommerce', 'ultimatewoo-pro' ) );
	 ...
	 */
	function admin_menu() {
		$page = add_submenu_page( 'woocommerce', __( 'Advanced Notifications', 'ultimatewoo-pro' ), __( 'Notifications', 'ultimatewoo-pro' ), 'manage_woocommerce', 'advanced-notifications', array( $this, 'admin_screen' ) );
	...
	function write_panel() {
		...
		?>
		<p class="form-field">
			<label><?php _e( 'Notifications', 'ultimatewoo-pro' ); ?></label>
			<select id="notification_recipients" name="notification_recipients[]" multiple="multiple" style="width:300px;" data-placeholder="<?php _e('Choose recipients for this product&hellip;', 'ultimatewoo-pro'); ?>" class="wc-enhanced-select chosen_select">
				<?php
	...
	function admin_screen() {
		...
		if ( ! empty( $_GET['delete'] ) ) {
			...
			echo '<div class="updated fade"><p>' . __( 'Notification deleted successfully', 'ultimatewoo-pro' ) . '</p></div>';
			...
			if ( ! empty( $_POST['save_recipient'] ) ) {
				...
				if ( is_wp_error( $result ) ) {
					...
					echo '<div class="updated fade"><p>' . __( 'Notification saved successfully', 'ultimatewoo-pro' ) . '</p></div>';
	...
	function add_recipient() {
		...
		if ( empty( $recipient_name ) ) {
			return new WP_Error( 'input', __( 'Recipient name is a required field', 'ultimatewoo-pro' ) );
		}

		if ( empty( $recipient_email ) ) {
			return new WP_Error( 'input', __( 'Recipient email is a required field', 'ultimatewoo-pro' ) );
		}
		...
		foreach ( $recipient_emails as $email ) {
			if ( ! is_email( $email ) ) {
				return new WP_Error( 'input', __( 'A recipient email is invalid:', 'ultimatewoo-pro' ) . ' ' . $email );
			}
		}
