...
<p><?php printf( __( 'Hi %s,', 'ultimatewoo-pro' ), esc_html( $recipient_name ) ); ?></p>

<p><?php printf( __( 'Order #%s has been refunded.', 'ultimatewoo-pro' ), esc_html( $order->get_order_number() ) ); ?></p>

<h2><a class="link" href="<?php echo admin_url( 'post.php?post=' . ( version_compare( WC_VERSION, '3.0.0', '<' ) ? $order->id : $order->get_id() ) . '&action=edit' ); ?>"><?php printf( __( 'Order #%s', 'ultimatewoo-pro' ), esc_html( $order->get_order_number() ) ); ?></a> (<?php printf( '<time datetime="%s">%s</time>', esc_attr( date_i18n( 'c', strtotime( version_compare( WC_VERSION, '3.0.0', '<' ) ? $order->order_date : $order->get_date_created() ) ) ), esc_html( date_i18n( wc_date_format(), strtotime( version_compare( WC_VERSION, '3.0.0', '<' ) ? $order->order_date : $order->get_date_created() ) ) ) ); ?>)</h2>
...
<table class="td" cellspacing="0" cellpadding="6" style="width: 100%; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif;" border="1">
	<thead>
		<tr>
			<th class="td" scope="col" style="text-align:left;"><?php esc_html_e( 'Product', 'ultimatewoo-pro' ); ?></th>
			<th class="td" scope="col" style="text-align:left;" <?php if ( ! $show_prices ) : ?>colspan="2"<?php endif; ?>><?php esc_html_e( 'Quantity', 'ultimatewoo-pro' ); ?></th>
			<?php if ( $show_prices ) : ?>
				<th class="td" scope="col" style="text-align:left;"><?php _e( 'Price', 'ultimatewoo-pro' ); ?></th>
			<?php endif; ?>
		</tr>
	</thead>
	...
	<tfoot>
		<?php
			if ( $triggers['all'] && ( $totals = $order->get_order_item_totals() ) ) {
				...
				?>
				<tr>
					<th class="td" scope="col" colspan="2" style="font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif; text-align:left;"><?php _e( 'Total', 'ultimatewoo-pro' ); ?></th>
