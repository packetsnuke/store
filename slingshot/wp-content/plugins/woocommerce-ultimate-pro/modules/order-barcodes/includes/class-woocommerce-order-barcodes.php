...
class WooCommerce_Order_Barcodes {
	...
	public function update_order_meta ( $order_id = 0 ) {
	    ...
	    $order_note = sprintf( __( 'Barcode generated successfully: %s', 'ultimatewoo-pro' ), '<a href="' . esc_url( $barcode_url ) . '" target="_blank">' . $barcode_url . '</a>' );
...
<?php
	...
	public function add_order_metabox () {
		...
		if( 'yes' == $this->barcode_enable || $barcode_text ) {
			add_meta_box( 'woocommerce-order-barcode', __( 'Order Barcode', 'ultimatewoo-pro' ), array( $this, 'get_metabox_barcode' ), 'shop_order', 'side', 'default' );
		}
	}
	...
	public function display_barcode ( $order_id = 0, $before = '', $after = '', $echo = true ) {
		...
		$barcode = $before . '<img src="' . esc_url( $barcode_url ) . '" title="' . __( 'Barcode', 'ultimatewoo-pro' ) . '" alt="' . __( 'Barcode', 'ultimatewoo-pro' ) . '" style="display:inline;border:0;" /><br/><span style="color:' . $this->barcode_colours['foreground'] . ';font-family:monospace;">' . $barcode_text . '</span>' . $after;
			...
			$html .= '<div id="barcode-scan-form">
						<form name="barcode-scan" action="" method="post">
							<select name="scan-action" id="scan-action" class="scan_action" required>
								<option value="" ' . selected( $action, '', false ) . '>' . __( 'Select action', 'ultimatewoo-pro' ) . '</option>
								<option value="lookup" ' . selected( $action, 'lookup', false ) . '>' . __( 'Look up', 'ultimatewoo-pro' ) . '</option>
								<option value="complete" ' . selected( $action, 'complete', false ) . '>' . __( 'Complete order', 'ultimatewoo-pro' ) . '</option>
								<option value="checkin" ' . selected( $action, 'checkin', false ) . '>' . __( 'Check in', 'ultimatewoo-pro' ) . '</option>
								<option value="checkout" ' . selected( $action, 'checkout', false ) . '>' . __( 'Check out', 'ultimatewoo-pro' ) . '</option>
							</select>

							<input type="text" name="scan-code" id="scan-code" value="" placeholder="' . __( 'Scan or enter barcode', 'ultimatewoo-pro' ) . '" required />

							<input type="submit" value="' . __( 'Go', 'ultimatewoo-pro' ) . '" />
						</form>
					  </div>';

			// Add loading text
			$html .= '<div id="barcode-scan-loader">' . __( 'Processing barcode...', 'ultimatewoo-pro' ) . '</div>';
	...
	public function scan_barcode () {
		...
		if( $do_nonce_check && ! wp_verify_nonce( $_POST[ $this->_token . '_scan_nonce' ], 'scan-barcode' ) ) {
			$this->display_notice( __( 'Permission denied: Security check failed', 'ultimatewoo-pro' ), 'error' );
			exit;
		}
		...
		if( ! $order_id ) {
			$this->display_notice( __( 'Invalid barcode', 'ultimatewoo-pro' ), 'error' );
			exit;
		}
		...
		if( ! $can_scan ) {
			$this->display_notice( __( 'Permission denied: You do not have sufficient permissions to scan barcodes', 'ultimatewoo-pro' ), 'error' );
			exit;
		}
		...
		if( ! is_a( $order, 'WC_Order' ) || is_wp_error( $order ) ) {
			$this->display_notice( __( 'Invalid order ID', 'ultimatewoo-pro' ), 'error' );
			exit;
		}
		...
		switch( $action ) {
			case 'complete':
				if ( apply_filters( $this->_token . '_complete_order', true, $order_id ) ) {
					if ( 'completed' === $order->get_status() ) {
						$response      = __( 'Order already completed', 'ultimatewoo-pro' );
						$response_type = 'notice';
					} else {
						$order->update_status( 'completed' );
						$response = __( 'Order marked as complete', 'ultimatewoo-pro' );
						$order = new WC_Order( $order_id );
					}
				} else {
					$response = __( 'Not able to complete order', 'ultimatewoo-pro' );
					$response_type = 'error';
				}
			break;
			...
			case 'checkin':
				if ( 'yes' === get_post_meta( $order_id, '_checked_in', true ) ) {
					$response      = __( 'Customer already checked in', 'ultimatewoo-pro' );
					$response_type = 'notice';
				} else {
					update_post_meta( $order_id, '_checked_in', 'yes' );
					$response = __( 'Customer has checked in', 'ultimatewoo-pro' );
				}
			break;

			case 'checkout':
				if ( 'no' === get_post_meta( $order_id, '_checked_in', true ) ) {
					$response      = __( 'Customer already checked out', 'ultimatewoo-pro' );
					$response_type = 'notice';
				} else {
					update_post_meta( $order_id, '_checked_in', 'no' );
					$response = __( 'Customer has checked out', 'ultimatewoo-pro' );
				}
			break;

			case 'lookup':
				$response = sprintf( __( 'Found matched order: #%s', 'ultimatewoo-pro' ), $order_id );
			break;

			default:
				$response      = __( 'Please select an action to perform', 'ultimatewoo-pro' );
				$response_type = 'error';
			break;
		}
		...
		if ( $checked_in ) {
			$checkin_status = ( 'yes' === $checked_in ) ? __( 'Checked in', 'ultimatewoo-pro' ) : __( 'Checked out', 'ultimatewoo-pro' );
	...
	public function checkin_status_edit_field ( $order ) {
		...
		if( $checked_in ) {
			?>
			<p class="form-field form-field-wide"><label for="checkin_status"><?php _e( 'Check in status:', 'ultimatewoo-pro' ) ?></label>
			<select id="checkin_status" name="checkin_status">
				<option value="<?php esc_attr_e( 'yes' ); ?>" <?php selected( 'yes', $checked_in, true ); ?>><?php _e( 'Checked in', 'ultimatewoo-pro' ); ?></option>
				<option value="<?php esc_attr_e( 'no' ); ?>" <?php selected( 'no', $checked_in, true ); ?>><?php _e( 'Checked out', 'ultimatewoo-pro' ); ?></option>
			</select></p>
			<?php
		}
