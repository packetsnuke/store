	...
	public static function points_rewards_composite_message( $message, $points_n_rewards ) {
		...
		if ( $product->is_type( 'composite' ) && self::$single_product_message_filter_active ) {
			...
			if ( false === self::has_fixed_points( $product ) && $product->contains( 'priced_individually' ) ) {
				...
				add_filter( WC_CP_Core_Compatibility::is_wc_version_gte_2_7() ? 'woocommerce_product_get_price' : 'woocommerce_get_price', array( __CLASS__, 'replace_price' ), 9999, 2 );
				...
				remove_filter( WC_CP_Core_Compatibility::is_wc_version_gte_2_7() ? 'woocommerce_product_get_price' : 'woocommerce_get_price', array( __CLASS__, 'replace_price' ), 9999, 2 );
