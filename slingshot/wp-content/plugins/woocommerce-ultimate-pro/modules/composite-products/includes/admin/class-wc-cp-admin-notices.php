...
class WC_CP_Admin_Notices {
	...
	public static function updating_notice() {
		...
		if ( WC_CP_Install::is_update_pending() ) {
			...
			$notice = '<strong>' . __( 'WooCommerce Composite Products Data Update', 'ultimatewoo-pro' ) . '</strong> &#8211; ' .  __( 'Your database is being updated in the background.', 'ultimatewoo-pro' ) . $fallback;
			self::add_notice( $notice, 'info' );

		// Show persistent notice to indicate that the updating process is complete.
		} else {
			$notice         = __( 'WooCommerce Composite Products data update complete.', 'ultimatewoo-pro' );
