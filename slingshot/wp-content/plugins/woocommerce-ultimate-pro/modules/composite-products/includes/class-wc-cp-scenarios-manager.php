...
class WC_CP_Scenarios_Manager {
	...
	public function validate_configuration( $configuration ) {
		...
		$matching_scenarios = $this->find_matching( $configuration );
	 ...
	 * @return boolean
	 */
	public function find_matching( $configuration ) {
		...
		foreach ( $configuration as $component_id => $component_configuration ) {
			...
			if ( $component_is_hidden ) {
				...
				if ( -1 === $product_id && empty( $scenarios_matching_component_cg ) ) {
				...
				if ( 'variable' === $product->get_type() ) {
					...
					if ( ! empty( $child_ids ) ) {
						...
						foreach ( $child_ids as $child_id ) {
							...
							$options_map[ $component_id ][ $variation_id ] = $variation_in_scenarios;
					...
					$options_map[ $component_id ][ $product_id ] = $product_in_scenarios;
				}
			}
		}
