...
class WCOPC_Admin_Editor {
	...
	public static function one_page_checkout_shortcode_iframe() {
		...
		if ( PP_One_Page_Checkout::is_woocommerce_pre( '2.3' ) ) {
			...
			wp_localize_script( 'wcopc_tinymce_dialog', 'ultimatewoo-pro', $params );
		} else {
...
<form id="wcopc_settings" style="float: left; width: 100%;">
	...
	<fieldset id="wcopc_product_ids_fields" style="margin: 1em 0;">
		<label for="wcopc_product_ids" style="width: 70px; display: inline-block;"><strong><?php _e( 'Products:', 'ultimatewoo-pro' ); ?></strong></label>
		<?php if ( PP_One_Page_Checkout::is_woocommerce_pre( '3.0' ) ) { ?>
		<input type="hidden" id="wcopc_product_ids" name="wcopc_product_ids[]" data-multiple="true" class="wc-product-search" data-placeholder="<?php _e( 'Search for a product&hellip;', 'ultimatewoo-pro' ); ?>" style="width: 75%;"/>
		<?php } else { ?>
		<select id="wcopc_product_ids" name="wcopc_product_ids[]" class="wc-product-search" multiple="multiple" data-placeholder="<?php _e( 'Search for a product&hellip;', 'ultimatewoo-pro' ); ?>" style="width: 75%;"></select>
		<?php } ?>
	</fieldset>
	<fieldset id="wcopc_template_fields" style="margin: 1em 0;">
		<div style="font-weight: bold;"><?php _e( 'Template:', 'ultimatewoo-pro' ); ?></div>
		<?php $first = true; ?>
	...
	<fieldset style="margin: 1em 0;">
		<input id="wcopc_submit" type="submit" class="button-primary" value="<?php _e( 'Create Shortcode', 'ultimatewoo-pro' ); ?>" />
		<input id="wcopc_cancel" type="button" class="button" value="<?php _e( 'Cancel', 'ultimatewoo-pro' ); ?>" />
	</fieldset>
</form>
</div>
<?php
