...
class PP_One_Page_Checkout {
	...
	public static function init() {
		...
		self::$plugin_url     = untrailingslashit( ULTIMATEWOO_MODULES_URL . '/one-page-checkout' );
		self::$plugin_path    = untrailingslashit( ULTIMATEWOO_MODULES_DIR . '/one-page-checkout' );
		self::$template_path  = self::$plugin_path . '/templates/';

		self::$templates   = apply_filters( 'wcopc_templates', array(
			'product-table' => array(
				'label'               => __( 'Product Table', 'ultimatewoo-pro' ),
				'description'         => __( 'Display a row for each product containing its thumbnail, title and price. Best for a few simple products where the thumbnails are helpful, e.g. a set of halloween masks.', 'ultimatewoo-pro' ),
				'supports_containers' => false,
			),
			'product-list' => array(
				'label'               => __( 'Product List', 'ultimatewoo-pro' ),
				'description'         => __( 'Display a list of products with a radio button for selection. Useful when the customer does not need a description or photograph to choose, e.g. versions of an eBook.', 'ultimatewoo-pro' ),
				'supports_containers' => false,
			),
			'product-single'  => array(
				'label'               => __( 'Single Product', 'ultimatewoo-pro' ),
				'description'         => __( "Display the single product template for each product. Useful when the description, images, gallery and other meta data will help the customer choose, e.g. evening gowns.", 'ultimatewoo-pro' ),
				'supports_containers' => false,
			),
			'pricing-table'  => array(
				'label'               => __( 'Pricing Table', 'ultimatewoo-pro' ),
				'description'         => __( "Display a simple pricing table with each product's attributes, weight and dimensions. Useful to allow customers to compare different, but related products, e.g. membership subscriptions.", 'ultimatewoo-pro' ),
				'supports_containers' => false,
			),
		) );
	...
	public static function maybe_filter_add_to_cart_message( $message, $product_id ) {
		...
		if ( is_wcopc_checkout() ) {
			...
			if ( is_array( $product_id ) ) {
				...
				$product_titles = join( __( '&quot; and &quot;', 'ultimatewoo-pro' ), array_filter( array_merge( array( join( '&quot;, &quot;', array_slice( $titles, 0, -1 ) ) ), array_slice( $titles, -1 ) ) ) );
	...
	public static function maybe_filter_error_message( $message ) {
		...
		if ( is_wcopc_checkout() ) {
			...
			$message = preg_replace('/<a[^>]*>(' . __( 'View Cart', 'ultimatewoo-pro' ) .')<\/a>/iU','',$message);
	...
	protected static function get_add_to_cart_message( $quantity, $product_title ) {
		...
		return sprintf( __( '%s&quot; added to your order. Complete your order below.', 'ultimatewoo-pro' ), $product_title );
	}
	...
	public static function modify_single_add_to_cart_text( $product ) {
		return __( 'Add to order', 'ultimatewoo-pro' );
	}
	...
	public static function ajax_remove_from_cart() {
		...
		foreach ( $cart as $cart_item_id => $value ) {
			...
			wc_add_notice( sprintf( __( '&quot;%s&quot; was successfully removed from your order.', 'ultimatewoo-pro' ), get_the_title( $value['product_id'] ) ), 'success' );
		...
		if ( ! $item_removed ) {
			wc_add_notice( sprintf( __( '&quot;%s&quot; could not be removed from your order.', 'ultimatewoo-pro' ), get_the_title( $value['product_id'] ) ), 'error' );
		...
		if ( ! $bypass ) {
			...
			if ( 'variation' === $add_to_cart_handler ) {
				...
				if ( $all_variations_set ) {
					...
					wc_add_notice( __( 'Please choose product options&hellip;', 'ultimatewoo-pro' ), 'error' );
				}
	...
	public static function enqueue_scripts() {
		...
		if ( self::$add_scripts ) {
			...
			$params = apply_filters( 'wcopc_script_data', array(
				...
				'wcopc_complete_order_prompt' => '<a class="wc-south opc-complete-order" href="#customer_details">' . __( 'Modify &amp; complete order below', 'ultimatewoo-pro' ) . '</a>',
				'ajax_error_notice'           => '<div class="woocommerce-error">' . __( 'Error processing your request. Please try refreshing the page. Contact us if you continue to have issues.', 'ultimatewoo-pro' ) . '</div>',
				'ajax_url'                    => WC()->ajax_url(),
			) );

			// ULTIMATEWOO FIX - change object to "wcopc"
	...
	public static function improve_empty_cart_error( $error ) {
		...
		if ( defined( 'WOOCOMMERCE_CHECKOUT' ) && $error == sprintf( __( 'Sorry, your session has expired. <a href="%s">Return to homepage &rarr;</a>', 'ultimatewoo-pro' ), home_url() ) ) {
			$error = __( 'You must select a product.', 'ultimatewoo-pro' );
		}
		...
		if ( current_user_can( 'activate_plugins' ) ) :
			if ( ! is_woocommerce_active() ) : ?>
				<div id="message" class="error">
					<p><?php printf( __( '%sWooCommerce One Page Checkout is inactive.%s The %sWooCommerce plugin%s must be active for WooCommerce One Page Checkout to work. Please %sinstall & activate WooCommerce%s', 'ultimatewoo-pro' ), '<strong>', '</strong>', '<a href="http://wordpress.org/extend/plugins/woocommerce/">', '</a>', '<a href="' . admin_url( 'plugins.php' ) . '">', '&nbsp;&raquo;</a>' ); ?></p>
				</div>
						<?php elseif ( version_compare( get_option( 'woocommerce_db_version' ), '2.1', '<' ) ) : ?>
				<div id="message" class="error">
					<p><?php printf( __( '%sWooCommerce One Page Checkout is inactive.%s This plugin requires WooCommerce 2.1 or newer. Please %supdate WooCommerce to version 2.1 or newer%s', 'ultimatewoo-pro' ), '<strong>', '</strong>', '<a href="' . admin_url( 'plugins.php' ) . '">', '&nbsp;&raquo;</a>' ); ?></p>
				</div>
		<?php endif; ?>
	<?php endif;
	}
	...
	public static function product_type_options( $options ){
		...
		$options['wcopc'] = array(
			'id'            => '_wcopc',
			'wrapper_class' => '',
			'label'         => __( 'One Page Checkout', 'ultimatewoo-pro'),
			'description'   => __( 'Add checkout to product page.', 'ultimatewoo-pro'),
			'default'       => 'no'
		);
	...
	public static function filter_single_product_wcopc() {
		...
		if ( is_product() && is_wcopc_checkout() ) {
			...
			if ( ! empty( $product ) && 'yes' == wcopc_get_products_prop( $product, 'ultimatewoo-pro', '_' ) ) {
