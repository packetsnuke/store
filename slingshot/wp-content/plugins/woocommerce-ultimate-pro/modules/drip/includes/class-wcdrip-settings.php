...
if ( ! class_exists( 'WC_Drip_Settings' ) && class_exists( 'WC_Integration' ) ) {
	...
	class WC_Drip_Settings extends WC_Integration {
		...
		public function __construct() {
			...
			$this->id                 = 'wcdrip';
			$this->method_title       = __( 'Drip', 'ultimatewoo-pro' );
			$this->method_description = __( 'Configure the settings below to connect your WooCommerce store to your Drip account and start recording events.', 'ultimatewoo-pro' ) . '<br /><br />' . __( 'After authenticating and selecting an account, the Drip embed code will automatically be added to your site.', 'ultimatewoo-pro' );
		...
		public function init_form_fields() {
			...
			$this->form_fields = array(
				'api-title' => array(
					'title'       => __( 'Authentication & Account', 'ultimatewoo-pro' ),
					'type'        => 'title',
					'description' => __( 'Please authenticate your Drip account here and choose the account to connect it with.', 'ultimatewoo-pro' ),
					'class'       => 'wcdrip-section-title-main',
				),
				'api_key' => array(
					'title'       => __( 'API Token', 'ultimatewoo-pro' ),
					'type'        => 'text',
					'description' => sprintf( __( 'Go the your %s and copy the API Token provided. %s', 'ultimatewoo-pro' ), '<a href="https://www.getdrip.com/user/edit" target="_blank"><strong>' . __( 'Drip User Account Settings', 'ultimatewoo-pro' ) . '</strong></a>', '<a href="' . get_admin_url() . 'admin.php?page=wc-settings&tab=integration&section=wcdrip&wcdrip_clear_api_key=1">' . __( 'Clear API Token?', 'ultimatewoo-pro' ) . '</a>' ),
					'default'     => '',
				),
				'account' => array(
					'title'       => __( 'Account', 'ultimatewoo-pro' ),
					'type'        => 'select',
					'description' => __( 'Which account do you want to link this site up with?', 'ultimatewoo-pro' ),
					'options'     => $this->accounts(),
				),

				'subscribe-title' => array(
					'title'       => __( 'Subscribe Checkbox', 'ultimatewoo-pro' ),
					'type'        => 'title',
					'description' => __( 'Add a subscription box to the checkout page and WooCommerce registration form.', 'ultimatewoo-pro' ),
				...
				'subscribe-enable' => array(
					'title'       => __( 'Subscribe Checkbox', 'ultimatewoo-pro' ),
					'type'        => 'checkbox',
					'label'       => __( 'Enable Subscription Checkbox', 'ultimatewoo-pro' ),
					'default'     => 'no',
					'desc_tip'    => true,
					'description' => __( 'This will add a subscription box on checkout', 'ultimatewoo-pro' ),
				),

				'subscribe-campaign' => array(
					'title'       => __( 'Campaign', 'ultimatewoo-pro' ),
					'type'        => 'select',
					'description' => __( 'Which campaign do you want to the subscribe box to sign a user up for?', 'ultimatewoo-pro' ) . '<br />' . '<strong>' . __( 'Note:', 'ultimatewoo-pro' ) . '</strong> ' . __( 'After saving a new account above, please use the Reload Data option below.', 'ultimatewoo-pro' ),
					'options'     => $this->campaigns(),
					'class'       => 'wcdrip-subscribe-field',
				),

				'subscribe-text' => array(
					'title'       => __( 'Subscribe Text', 'ultimatewoo-pro' ),
					'type'        => 'text',
					'placeholder' => 'Subscribe to {campaign_name}',
					'desc_tip'    => false,
					'description' => sprintf( __( '%s Subscribe to {campaign_name}%sText for the subscribe checkbox - valid HTML allowed. Use the template tag %s to display the campaign name (optional).', 'ultimatewoo-pro' ), '<strong>' . __( 'Default:', 'ultimatewoo-pro' ) . '</strong>', '<br />', '<span class="wcdrip-tag"><strong>{campaign_name}</strong></span>' ),
					'class'       => 'wcdrip-subscribe-field',
				),

				'notif-title' => array(
					'title' => __( 'Other', 'ultimatewoo-pro' ),
					'type'  => 'title',
					'class' => 'wcdrip-section-title',
				),

				'event-sale-name' => array(
					'title'       => __( 'Event Sale Name', 'ultimatewoo-pro' ),
					'type'        => 'text',
					'placeholder' => 'Purchase',
					'desc_tip'    => false,
					'description' => sprintf( __( '%s Purchase%s Name the event that is added to a subscriber after a purchase. It should (optionally) match your Conversion Goal name exactly.', 'ultimatewoo-pro' ), '<strong>' . __( 'Default:', 'ultimatewoo-pro' ) . '</strong>', '<br />' ),
				),
				...
				'logging_enabled' => array(
					...
					'type'        => 'checkbox',
					'description' => __( 'Log Drip debug events, such as API requests, to WooCommerce System Status log.', 'ultimatewoo-pro' ),
				)
			);
		...
		public function scripts() {
			...
			wp_register_script( 'wcdrip-admin-js', ULTIMATEWOO_MODULES_URL . 'drip/assets/js/wcdrip-admin.js', array( 'jquery') );
		...
		public function generate_reload_accounts_html() {
			...
			<tr valign="top" id="service_options">
				<th scope="row" class="titledesc"><?php _e( 'Reload Drip Data', 'ultimatewoo-pro' ); ?></th>
				<td>
					<p><a href="<?php echo get_admin_url(); ?>admin.php?page=wc-settings&tab=integration&section=wcdrip&wcdrip_reload_accounts=1" class="button" id="wcdrip-reload-data-button"><?php _e('Reload Accounts / Campaigns', 'ultimatewoo-pro'); ?></a></p>
					<p><em><?php _e( 'If you have added a new account and/or campaign and it\'s not showing, this will refresh the list.', 'ultimatewoo-pro' ); ?></em></p>
				</td>
			</tr>
			<?php
			return ob_get_clean();
		...
		public function display_errors( ) {
			...
			foreach ( $this->errors as $key => $value ) { ?>
				...
				<div class="error">
					<p><?php _e( 'The API Key is invalid. Please double check it and try again!', 'ultimatewoo-pro' ); ?></p>
				</div>
