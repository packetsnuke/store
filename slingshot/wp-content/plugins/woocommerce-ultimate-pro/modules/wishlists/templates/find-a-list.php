...
<?php do_action( 'woocommerce_wishlists_before_wrapper' ); ?>
    <div id="wl-wrapper" class="woocommerce">
        ...
        <form class="wl-search-form" method="get">
            <label for="f-list"><?php _e( "Find Someone's List", 'ultimatewoo-pro' ); ?></label>
            <input type="text" name="f-list" id="f-list" class="find-input" value="<?php echo( isset( $_GET['f-list'] ) ? esc_attr( $_GET['f-list'] ) : '' ); ?>" placeholder="<?php _e( 'Enter name or email', 'ultimatewoo-pro' ); ?>"/>
            <input type="submit" class="button" value="<?php _e( 'Search', 'ultimatewoo-pro' ); ?>"/>
        </form>
        <hr/>
		...
		<?php if ( have_posts() ) : ?>
            ...
            <table class="shop_table cart wl-table wl-manage wl-find-table" cellspacing="0">
                ...
                <tr>
                    ...
                    <th class="wl-pers-name"><?php _e( 'Name', 'ultimatewoo-pro' ); ?></th>
		...
		<?php elseif ( isset( $_GET['f-list'] ) ): ?>
            ...
            <p><?php _e( "We're sorry, we couldn't find a list for that name. Please double check your entry and try again.", 'ultimatewoo-pro' ); ?></p>
            <h2 class="wl-search-result"><?php _e( 'We found 0 matching lists', 'ultimatewoo-pro' ); ?></h2>
		<?php endif; ?>
    </div>
