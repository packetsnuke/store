...
class WC_Wishlists_Wishlist_Admin {
		...
		if ( in_array( $screen->id, array( 'edit-wishlist', 'wishlist' ) ) ) {
			...
			if ( $post ) {
				...
				$params = array(
					...
					'remove_item_notice'  => __( 'Are you sure you want to remove the selected items?', 'ultimatewoo-pro' ),
	...
	public function add_columns( $columns ) {
		...
		$columns = array(
			...
			'status'   => __( 'Type', 'ultimatewoo-pro' ),
			'sharing'  => __( 'Sharing Status', 'ultimatewoo-pro' ),
			'user'     => __( 'User', 'ultimatewoo-pro' ),
			'email'    => __( 'Email on List', 'ultimatewoo-pro' ),
			'name'     => __( 'Name on List', 'ultimatewoo-pro' ),
	...
	function add_metaboxes() {
		add_meta_box( 'wc_wishlists_viewing', __( 'View Wishlist', 'ultimatewoo-pro' ), array(
			&$this,
			'viewing_metabox'
		), 'wishlist', 'side', 'high' );
		add_meta_box( 'wc_wishlists_items', __( 'Items', 'ultimatewoo-pro' ), array(
			&$this,
			'items_metabox'
		), 'wishlist', 'advanced', 'default' );
		add_meta_box( 'wc_wishlists_nameinfo', __( 'Name and Information', 'ultimatewoo-pro' ), array(
			&$this,
			'nameinfo_metabox'
		), 'wishlist', 'advanced', 'default' );
		add_meta_box( 'wc_wishlists_sharing', __( 'Sharing', 'ultimatewoo-pro' ), array(
			&$this,
			'sharing_metabox'
		), 'wishlist', 'advanced', 'default' );
		add_meta_box( 'wc_wishlists_notifications', __( 'Notifications', 'ultimatewoo-pro' ), array(
			&$this,
            ...
            <ul>
                <li>
                    <a href="<?php echo $wishlist->get_the_url_view( $post->ID ); ?>"
                       target="_blank"><?php _e( 'Preview', 'ultimatewoo-pro' ); ?></a>
                </li>
                <li>
                    <a href="<?php echo $wishlist->get_the_url_edit( $post->ID ); ?>"
                       target="_blank"><?php _e( 'Manage List', 'ultimatewoo-pro' ); ?></a>
                </li>
            </ul>
	...
	public function sharing_metabox( $post ) {
        ...
        <div class="wl-admin-wrapper">
            <p class="form-row">
                <strong><?php _e( 'Privacy Settings', 'ultimatewoo-pro' ); ?></strong>
            <table class="wl-rad-table">
                <tr>
                    <td><input type="radio" name="wishlist_sharing" id="rad_pub"
                               value="Public" <?php checked( 'Public', $sharing ); ?>></td>
                    <td><label for="rad_pub"><?php _e( 'Public', 'ultimatewoo-pro' ); ?> <span
                                    class="wl-small">- <?php _e( 'Anyone can search for and see this list. You can also share using a link.', 'ultimatewoo-pro' ); ?></span></label>
                    </td>
                </tr>
                <tr>
                    <td><input type="radio" name="wishlist_sharing" id="rad_shared"
                               value="Shared" <?php checked( 'Shared', $sharing ); ?>></td>
                    <td><label for="rad_shared"><?php _e( 'Shared', 'ultimatewoo-pro' ); ?> <span
                                    class="wl-small">- <?php _e( 'Only people with the link can see this list. It will not appear in public search results.', 'ultimatewoo-pro' ); ?></span></label>
                    </td>
                </tr>
                <tr>
                    <td><input type="radio" name="wishlist_sharing" id="rad_priv"
                               value="Private" <?php checked( 'Private', $sharing ); ?>></td>
                    <td><label for="rad_priv"><?php _e( 'Private', 'ultimatewoo-pro' ); ?> <span
                                    class="wl-small">- <?php _e( 'Only you can see this list.', 'ultimatewoo-pro' ); ?></span></label>
                    </td>
                </tr>
            </table>
            </p>
	...
	public function nameinfo_metabox( $post ) {
		...
		?>
        <div class="wl-admin-wrapper">
            <p class="no-marg"><?php _e( 'Enter a name you would like associated with this list.  If your list is public, users can find it by searching for this name.', 'ultimatewoo-pro' ); ?></p>
            ...
            <p class="form-row form-row-full">
                <label for="wishlist_owner_email"><?php _e( 'Email Assoicated with the List', 'ultimatewoo-pro' ); ?></label>
			...
			<?php if ( get_post_meta( $post->ID, '_wishlist_status', true ) == 'temporary' ): ?>
                ...
                <hr/>
                <p>
                    <strong><?php _e( 'Attach to a User', 'ultimatewoo-pro' ); ?></strong><br/>
                    Enter an user ID to assign this list to a specific user.
                </p>
			<?php endif; ?>
            <p class="form-row form-row-full">
                <label for="wishlist_owner"><?php _e( 'User ID Assoicated with the List', 'ultimatewoo-pro' ); ?></label>
	...
	public function notifications_metabox( $post ) {
        ...
        <div class="wl-admin-wrapper">
            <p class="form-row">
                <strong><?php _e( 'Notification Settings', 'ultimatewoo-pro' ); ?></strong>
            <table class="wl-rad-table">
                <tr>
                    <td><input type="radio" id="rad_notification_yes" name="wishlist_owner_notifications"
                               value="yes" <?php checked( 'yes', $notifications ); ?>></td>
                    <td><label for="rad_notification_yes"><?php _e( 'Yes', 'ultimatewoo-pro' ); ?> <span
                                    class="wl-small">- <?php _e( 'Send an email if a price reduction occurs.', 'ultimatewoo-pro' ); ?></span></label>
                    </td>
                </tr>
                <tr>
                    <td><input type="radio" id="rad_notification_no" name="wishlist_owner_notifications"
                               value="no" <?php checked( 'no', $notifications ); ?>></td>
                    <td><label for="rad_notification_no"><?php _e( 'No', 'ultimatewoo-pro' ); ?> <span
                                    class="wl-small">- <?php _e( 'Do not send an email if a price reduction occurs.', 'ultimatewoo-pro' ); ?></span></label>
                    </td>
                </tr>
            </table>
            </p>
