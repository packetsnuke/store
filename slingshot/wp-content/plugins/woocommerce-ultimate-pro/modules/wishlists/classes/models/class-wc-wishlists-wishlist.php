...
class WC_Wishlists_Wishlist {
	...
	public static function get_the_wishlist_sharing( $id, $translate = false ) {
		if ( $translate ) {
			...
			switch ( $value ) {
				case 'Public' :
					$result = __( 'Public', 'ultimatewoo-pro' );
					break;
				case 'Shared' :
					$result = __( 'Shared', 'ultimatewoo-pro' );
					break;
				case 'Private' :
					$result = __( 'Private', 'ultimatewoo-pro' );
					break;
				default:
					$result = $value;
			}
	...
	public static function create_list( $title, $raw_args = array() ) {
		...
		$wishlist_data = array(
			'post_type'    => 'wishlist',
			'post_title'   => $args['wishlist_title'] ? $args['wishlist_title'] : sprintf( __( 'New List %s', 'ultimatewoo-pro' ), date( 'Y-m-d h:i:s' ) ),
	...
	public static function update_list( $post_id, $raw_args = array() ) {
		...
		if ( !$wishlist_id || is_wp_error( $wishlist_id ) ) {
			...
			WC_Wishlist_Compatibility::wc_add_notice( __( 'Unable to update list', 'ultimatewoo-pro' ), 'error' );
