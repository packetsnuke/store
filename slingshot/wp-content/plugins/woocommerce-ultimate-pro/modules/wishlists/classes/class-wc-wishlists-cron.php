				...
				if ( $wishlist_items ) {
					...
					foreach ( $wishlist_items as $item ) {
						...
						if ( $wl_price && $wl_price > $price ) {
							...
							$text = sprintf( __( '<a href="%s">%s</a> has been reduced in price! Was %s, now avaiable for %s ', 'ultimatewoo-pro' ), get_permalink( $product->get_id() ), $product->get_title(), $wl_price, $price );
		...
		if ( $receivers && count( $receivers ) ) {
			...
			foreach ( $receivers as $receiver => $changes ) {
				if ( count( $changes ) ) {
					...
					$email_heading = sprintf( __( '%s', 'ultimatewoo-pro' ), get_option( 'blogname' ) );
					...
					$to      = $receiver;
					$subject = apply_filters( 'wc_wishlist_price_change_email_subject', __( 'Price Change Notification', 'ultimatewoo-pro' ) );
					$message = $html;
