	...
	private static function make_text( $key ) {
		...
		switch ( $key ) {
			case 'error_creating_list':
				$message = __( 'Error creating list.  Please try again later', 'ultimatewoo-pro' );
				break;
			case 'label_create_list':
				$message = __( 'Create a List', 'ultimatewoo-pro' );
				break;
			case 'label_create_list_desc' :
				$message = __( 'Create a list to save items for later.', 'ultimatewoo-pro' );
		}
