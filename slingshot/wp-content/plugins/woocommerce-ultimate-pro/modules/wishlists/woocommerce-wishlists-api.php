...
function woocommerce_wishlist_handle_add_to_wishlist_action() {
	...
	if ( !$wishlist_id && ( WC_Wishlists_Settings::get_setting( 'wc_wishlist_autocreate', 'yes' ) == 'yes' ) ) {
		$wishlist_id = WC_Wishlists_Wishlist::create_list( __( 'Wishlist', 'ultimatewoo-pro' ) );
		...
		if ( $wishlist_id ) {
			if ( is_user_logged_in() ) {
				...
				if ( WC_Wishlists_Settings::get_setting( 'woocommerce_wishlist_redirect_after_add', 'yes' ) == 'yes' && WC_Wishlist_Compatibility::wc_error_count() == 0 ) {
					$message = sprintf( __( 'This list has been automatically created for you.', 'ultimatewoo-pro' ) );
					WC_Wishlist_Compatibility::wc_add_notice( apply_filters( 'woocommerce_wishlist_wishlist_created_message', $message ) );
				} else {
					$message = sprintf( __( 'A list has been created for you. <a href="%s">Manage list</a> ', 'ultimatewoo-pro' ), $redirect_url );
					WC_Wishlist_Compatibility::wc_add_notice( apply_filters( 'woocommerce_wishlist_wishlist_created_message', $message ) );
				}
			} else {
				...
				$message      = sprintf( __( 'A temporary list has been created for you. <a href="%s">Login</a> or <a href="%s">register for an account</a> to save this list for future use.  You may access this temporary list for up to 30 days or until you clear your browser history.', 'ultimatewoo-pro' ), $auth_url, $register_url );
				WC_Wishlist_Compatibility::wc_add_notice( apply_filters( 'woocommerce_wishlist_wishlist_created_message', $message ) );
			}
		}
	} else {
	...
	if ( !$wishlist_id ) {
		WC_Wishlist_Compatibility::wc_add_notice( __( 'Unable to locate or create a list for you.  Please try again later', 'ultimatewoo-pro' ), 'error' );
	...
	if ( $added_to_wishlist && $wishlist_id != 'session' ) {
		...
		if ( $added_to_wishlist ) {
			WC_Wishlist_Compatibility::wc_add_notice( sprintf( __( '%s Items ready to move to a new list', 'ultimatewoo-pro' ), 1 ) );
		}
...
function woocommerce_wishlist_add_to_cart_handler_grouped( $product_id, $wishlist_id ) {
	...
	if ( !empty( $_REQUEST['quantity'] ) && is_array( $_REQUEST['quantity'] ) ) {
		...
		if ( !empty( $items_to_add ) && count( $items_to_add ) == count( $_REQUEST['quantity'] ) ) {
		...
		if ( !$was_added_to_cart && !$quantity_set ) {
			wc_add_notice( __( 'Please choose the quantity of items you wish to add to your list&hellip;', 'ultimatewoo-pro' ), 'error' );
		} elseif ( !$was_added_to_cart ) {
			wc_add_notice( __( 'There was a problem adding items to your wishlist.  Please review and try again.&hellip;', 'ultimatewoo-pro' ), 'error' );
		...
		/* Link on product archives */
		wc_add_notice( __( 'Please choose a product to add to your list&hellip;', 'ultimatewoo-pro' ), 'error' );
	}
...
function woocommerce_wishlist_add_to_wishlist_message( $wishlist_id = false ) {
	...
	if ( WC_Wishlists_Settings::get_setting( 'woocommerce_wishlist_redirect_after_add_to_cart', 'yes' ) == 'yes' ) :
		...
		$message   = sprintf( '<a href="%s" class="button">%s</a> %s', $return_to, __( 'Continue Shopping &rarr;', 'woocommerce' ), __( 'Product successfully added to your wishlist.', 'ultimatewoo-pro' ) );
	else :
		...
		$success_message        = sprintf( __( 'Product successfully added to %s.', 'ultimatewoo-pro' ), esc_html( $title ) );
		$message                = sprintf( '<a href="%s" class="button">%s</a> %s', $edit_list_url, __( 'Manage Wishlist &rarr;', 'ultimatewoo-pro' ), $success_message );
	endif;
...
function woocommerce_wishlist_handle_share_via_email_action() {
	...
	$name    = $name ? $name : __( 'Someone', 'ultimatewoo-pro' );
	...
	if ( $to ) {
		...
		if ( count( $clean_addresses ) ) {
			$body .= '<br />' . sprintf( __( '%s has a list to share', 'ultimatewoo-pro' ), $name ) .
			         sprintf( __( ' on <a href="%s">%s</a>. %s You can view this list clicking on the link or copy and pasting it into your browser. <br />View List: <a href="%s">%s</a>', 'ultimatewoo-pro' ), get_site_url(), get_bloginfo( 'name' ), $content, $wishlist->get_the_url_view( $wishlist_id, true ), $wishlist->get_the_url_view( $wishlist_id, true )
			         );
			...
			$sent    = wp_mail( $clean_addresses, sprintf( __( '%s has a list to share', 'ultimatewoo-pro' ), $name ), $body, $headers );
	...
	if ( $sent ) {
		WC_Wishlist_Compatibility::wc_add_notice( __( 'Your email has been sent', 'ultimatewoo-pro' ) );
	} elseif ( $sent === false ) {
		WC_Wishlist_Compatibility::wc_add_notice( __( 'Unable to send mail.  Please check your values and try again.', 'ultimatewoo-pro' ) . ' ' . $phpmailer->ErrorInfo, 'error' );
	} else {
		WC_Wishlist_Compatibility::wc_add_notice( __( 'Unable to send mail.  Please check your values and try again.', 'ultimatewoo-pro' ), 'error' );
	}
}
