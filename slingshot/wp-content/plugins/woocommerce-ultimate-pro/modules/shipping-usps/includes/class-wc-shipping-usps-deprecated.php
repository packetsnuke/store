...
class WC_Shipping_USPS extends WC_Shipping_Method {
	...
	public function __construct() {
		$this->id                 = 'usps';
		$this->method_title       = __( 'USPS', 'ultimatewoo-pro' );
		$this->method_description = __( 'The <strong>USPS</strong> extension obtains rates dynamically from the USPS API during cart/checkout.', 'ultimatewoo-pro' );
	...
	private function environment_check() {
		...
		if ( get_woocommerce_currency() != "USD" ) {
			echo '<div class="error">
				<p>' . sprintf( __( 'USPS requires that the <a href="%s">currency</a> is set to US Dollars.', 'ultimatewoo-pro' ), admin_url( 'admin.php?page=' . $admin_page . '&tab=general' ) ) . '</p>
			</div>';
		}

		elseif ( ! in_array( WC()->countries->get_base_country(), $this->domestic ) ) {
			echo '<div class="error">
				<p>' . sprintf( __( 'USPS requires that the <a href="%s">base country/region</a> is the United States.', 'ultimatewoo-pro' ), admin_url( 'admin.php?page=' . $admin_page . '&tab=general' ) ) . '</p>
			</div>';
		}

		elseif ( ! $this->origin && $this->enabled == 'yes' ) {
			echo '<div class="error">
				<p>' . __( 'USPS is enabled, but the origin postcode has not been set.', 'ultimatewoo-pro' ) . '</p>
			</div>';
		}
	}
    ...
    public function init_form_fields() {
    	...
    	$this->form_fields  = array(
			...
			'title'            => array(
				...
				'default'     => __( 'USPS', 'ultimatewoo-pro' ),
				'desc_tip'    => true
			),
			'origin'           => array(
		    ...
		    'availability'  => array(
				...
				'options'         => array(
					...
					'excluding' => __( 'Exclude Specific Countries', 'ultimatewoo-pro' ),
				),
			),
			'countries'        => array(
		    ...
		    'api'           => array(
				...
				'type'        => 'title',
				'description' => sprintf( __( 'You can obtain a USPS user ID by %s, or just use ours by leaving the field blank. This is optional.', 'ultimatewoo-pro' ), '<a href="https://www.usps.com/business/web-tools-apis/welcome.htm">' . __( 'signing up on the USPS website', 'ultimatewoo-pro' ) . '</a>' )
		    ),
		    'user_id'           => array(
				'title'       => __( 'USPS User ID', 'ultimatewoo-pro' ),
				'type'        => 'text',
				'description' => __( 'Obtained from USPS after getting an account.', 'ultimatewoo-pro' ),
				'default'     => '',
		    ...
		    'rates'           => array(
				'title'           => __( 'Rate Options', 'ultimatewoo-pro' ),
				'type'            => 'title',
				'description'     => '',
		    ),
			'shippingrates'  => array(
				'title'       => __( 'Shipping Rates', 'ultimatewoo-pro' ),
				'type'        => 'select',
				'default'     => 'ALL',
				'options'     => array(
					'ONLINE'  => __( 'Use Commercial Rates', 'ultimatewoo-pro' ),
					'ALL'     => __( 'Use Retail Rates', 'ultimatewoo-pro' ),
				),
				'desc_tip'    => true,
				'description' => __( 'Choose which rates to show your customers: Standard retail or discounted commercial rates.', 'ultimatewoo-pro' ),
			),
			'offer_rates'   => array(
			...
			'fallback' => array(
				...
				'type'        => 'text',
				'desc_tip'    => true,
				'description' => __( 'If USPS returns no matching rates, offer this amount for shipping so that the user can still checkout. Leave blank to disable.', 'ultimatewoo-pro' ),
				'default'     => '',
				'placeholder' => __( 'Disabled', 'ultimatewoo-pro' )
			),
			'flat_rates'           => array(
				'title'           => __( 'Flat Rates', 'ultimatewoo-pro' ),
				'type'            => 'title',
		    ),
		    'enable_flat_rate_boxes'  => array(
				'title'           => __( 'Flat Rate Boxes &amp; envelopes', 'ultimatewoo-pro' ),
				'type'            => 'select',
				'default'         => 'yes',
				'options'         => array(
					'yes'         => __( 'Yes - Enable flat rate services', 'ultimatewoo-pro' ),
					'no'          => __( 'No - Disable flat rate services', 'ultimatewoo-pro' ),
					'priority'    => __( 'Enable Priority flat rate services only', 'ultimatewoo-pro' ),
					'express'     => __( 'Enable Express flat rate services only', 'ultimatewoo-pro' ),
				),
				'description'     => __( 'Enable this option to offer shipping using USPS Flat Rate services. Items will be packed into the boxes/envelopes and the customer will be offered a single rate from these.', 'ultimatewoo-pro' ),
				'desc_tip'    => true
			),
			'flat_rate_express_title'           => array(
				'title'           => __( 'Express Flat Rate Service Name', 'ultimatewoo-pro' ),
				'type'            => 'text',
				'description'     => '',
				'default'         => '',
		    ...
		    'flat_rate_priority_title'           => array(
				'title'           => __( 'Priority Flat Rate Service Name', 'ultimatewoo-pro' ),
				'type'            => 'text',
				'description'     => '',
				'default'         => '',
		    ...
		    'standard_rates'           => array(
				'title'           => __( 'API Rates', 'ultimatewoo-pro' ),
				'type'            => 'title',
		    ),
			'enable_standard_services'  => array(
				'title'       => __( 'Standard Services', 'ultimatewoo-pro' ),
				'label'       => __( 'Enable Standard Services from the API', 'ultimatewoo-pro' ),
				'type'        => 'checkbox',
				'default'     => 'no',
				'desc_tip'    => true,
				'description' => __( 'Enable non-flat rate services.', 'ultimatewoo-pro' )
			),
			'packing_method'  => array(
				...
				'options'         => array(
					...
					'weight_based'    => __( 'Weight based: Regular sized items (< 12 inches) are grouped and quoted for weights only. Large items are quoted individually.', 'ultimatewoo-pro' ),
				),
			),
			'boxes'  => array(
				'type'            => 'box_packing'
			...
			'unpacked_item_handling'   => array(
				'title'           => __( 'Unpacked item handling', 'ultimatewoo-pro' ),
				'type'            => 'select',
				'description'     => '',
				'default'         => 'all',
				'options'         => array(
					''         => __( 'Get a quote for the unpacked item by itself', 'ultimatewoo-pro' ),
					'ingore'   => __( 'Ignore the item - do not quote', 'ultimatewoo-pro' ),
					'fallback' => __( 'Use the fallback price (above)', 'ultimatewoo-pro' ),
					'abort'    => __( 'Abort - do not return any quotes for the standard services', 'ultimatewoo-pro' ),
				),
		    ),
			'services'  => array(
				'type'            => 'services'
			...
			'mediamail_restriction'        => array(
				'title'           => __( 'Restrict Media Mail to...', 'ultimatewoo-pro' ),
				'type'            => 'multiselect',
				'class'           => 'chosen_select',
				'css'             => 'width: 450px;',
				'default'         => '',
				...
				'custom_attributes'      => array(
					'data-placeholder' => __( 'No restrictions', 'ultimatewoo-pro' ),
				)
			),
		);
    }
    ...
    function test_user_id() {
		...
		echo '<div class="error">
			<p>' . __( 'The USPS User ID you entered is invalid. Please make sure you entered a valid ID (<a href="https://www.usps.com/business/web-tools-apis/welcome.htm">which can be obtained here</a>). Our User ID will be used instead.', 'ultimatewoo-pro' ) . '</p>
		</div>';
    ...
    public function calculate_shipping( $package = array() ) {
    	...
    	$this->debug( __( 'USPS debug mode is on - to hide these messages, turn debug mode off in the settings.', 'ultimatewoo-pro' ) );
			...
			if ( $this->found_rates ) {
				foreach ( $this->found_rates as $key => $value ) {
					...
					if ( $this->unpacked_item_costs && ! empty( $this->found_rates[ $key ] ) ) {
						$this->debug( sprintf( __( 'Adding unpacked item costs to rate %s', 'ultimatewoo-pro' ), $key ) );
    ...
    private function per_item_shipping( $package ) {
    	...
    	foreach ( $package['contents'] as $item_id => $values ) {
    		...
    		if ( ! $values['data']->needs_shipping() ) {
    			$this->debug( sprintf( __( 'Product # is virtual. Skipping.', 'ultimatewoo-pro' ), $item_id ) );
    			continue;
    		}

    		if ( ! $values['data']->get_weight() ) {
	    		$this->debug( sprintf( __( 'Product # is missing weight. Using 1lb.', 'ultimatewoo-pro' ), $item_id ) );
    ...
    private function weight_based_shipping( $package ) {
    	...
    	foreach ( $package['contents'] as $item_id => $values ) {
    		...
    		if ( ! $values['data']->needs_shipping() ) {
    			$this->debug( sprintf( __( 'Product #%d is virtual. Skipping.', 'ultimatewoo-pro' ), $item_id ) );
    			continue;
    		}

    		if ( ! $values['data']->get_weight() ) {
	    		$this->debug( sprintf( __( 'Product #%d is missing weight. Using 1lb.', 'ultimatewoo-pro' ), $item_id ) );
    ...
    private function box_shipping( $package ) {
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			if ( $values['data']->length && $values['data']->height && $values['data']->width ) {
				...
				$this->debug( sprintf( __( 'Product #%d is missing dimensions! Using 1x1x1.', 'ultimatewoo-pro' ), $item_id ) );
