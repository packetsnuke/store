...
class WC_Product_Booking extends WC_Product_Booking_Compatibility {
	...
	public function single_add_to_cart_text() {
		return $this->get_requires_confirmation() ? apply_filters( 'woocommerce_booking_single_check_availability_text', __( 'Check Availability', 'ultimatewoo-pro' ), $this ) : apply_filters( 'woocommerce_booking_single_add_to_cart_text', __( 'Book now', 'ultimatewoo-pro' ), $this );
	}
	...
	public function get_price_html( $price = '' ) {
		...
		if ( $original_price_suffix !== $display_price_suffix ) {
			...
			if ( $this->has_additional_costs() || $this->get_display_cost() ) {
				$price_html = sprintf( __( 'From: %s', 'ultimatewoo-pro' ), wc_price( $display_price ) ) . $this->get_price_suffix();
			} else {
				$price_html = wc_price( $display_price ) . $this->get_price_suffix();
			}
		} elseif ( ! $this->has_additional_costs() ) {
			$price_html = __( 'Free', 'ultimatewoo-pro' );
		} else {
			$price_html = '';
		}
	...
	*/
			...
			if ( $available_qty < $qty ) {
				...
				if ( in_array( $this->get_duration_unit(), array( 'hour', 'minute' ) ) ) {
					return new WP_Error( 'Error', sprintf(
						_n( 'There is a maximum of %d place remaining', 'There are a maximum of %d places remaining', $display_available_qty , 'ultimatewoo-pro' ),
						$display_available_qty
					) );
				} elseif ( ! $available_qty ) {
					return new WP_Error( 'Error', sprintf(
						_n( 'There is a maximum of %1$d place remaining on %2$s', 'There are a maximum of %1$d places remaining on %2$s', $display_available_qty , 'ultimatewoo-pro' ),
						$display_available_qty,
	...
	public function get_all_resources_availability( $start_date, $end_date, $qty ) {
		...
		if ( empty( $available_resources ) ) {
			return new WP_Error( 'Error', __( 'This block cannot be booked.', 'ultimatewoo-pro' ) );
		}
