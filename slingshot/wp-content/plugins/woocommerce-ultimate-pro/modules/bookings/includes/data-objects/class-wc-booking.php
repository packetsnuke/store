...
class WC_Booking extends WC_Bookings_Data {
	...
	protected function status_transition() {
		if ( $this->status_transitioned ) {
			$allowed_statuses = array(
				'was-in-cart' => __( 'Was In Cart', 'ultimatewoo-pro' ),
			);
	...
	protected function status_transitioned_handler( $from, $to ) {
		...
		if ( $order ) {
			$order->add_order_note( sprintf( __( 'Booking #%1$d status changed from "%2$s" to "%3$s"', 'ultimatewoo-pro' ), $this->get_id(), $from, $to ) );
		}
	...
	*/
	...
	public function get_customer() {
		...
		if ( $order = $this->get_order() ) {
			...
			$name 		= 0 !== absint( $user_id ) ? $name : sprintf( _x( '%s (Guest)', 'Guest string with name from booking order in brackets', 'ultimatewoo-pro' ), $name );
	...
	*/
	...
	public function update_status( $status ) {
		...
		$allowed_statuses['was-in-cart'] = __( 'Was In Cart', 'ultimatewoo-pro' );
