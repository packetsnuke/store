...
class WC_Product_Booking_Person_Type_Data_Store_CPT extends WC_Data_Store_WP {
	...
	public function create( &$person_type ) {
		if ( ! $person_type->get_name() ) {
			$person_type->set_name( sprintf( __( 'Person Type #%d', 'ultimatewoo-pro' ), $person_type->get_sort_order() ) );
		}
	...
	public function read( &$person_type ) {
		...
		if ( ! $person_type_post ) {
			throw new Exception( __( 'Invalid booking person type.', 'ultimatewoo-pro' ) );
		}
