...
class WC_Bookings_Google_Calendar_Integration extends WC_Integration {
	...
	public function __construct() {
		...
		$this->method_title       = __( 'Google Calendar', 'ultimatewoo-pro' );
		$this->method_description = __( 'WooCommerce Bookings - Google Calendar integration.', 'ultimatewoo-pro' );
	...
	public function init_form_fields() {
		$this->form_fields = array(
			'client_id' => array(
				'title'       => __( 'Client ID', 'ultimatewoo-pro' ),
				'type'        => 'text',
				'description' => __( 'Enter with your Google Client ID.', 'ultimatewoo-pro' ),
				'desc_tip'    => true,
				'default'     => '',
			),
			'client_secret' => array(
				'title'       => __( 'Client Secret', 'ultimatewoo-pro' ),
				'type'        => 'text',
				'description' => __( 'Enter with your Google Client Secret.', 'ultimatewoo-pro' ),
				'desc_tip'    => true,
				'default'     => '',
			),
			'calendar_id' => array(
				'title'       => __( 'Calendar ID', 'ultimatewoo-pro' ),
				'type'        => 'text',
				'description' => __( 'Enter with your Calendar ID.', 'ultimatewoo-pro' ),
				'desc_tip'    => true,
				'default'     => '',
			),
			'authorization' => array(
				'title'       => __( 'Authorization', 'ultimatewoo-pro' ),
				'type'        => 'google_calendar_authorization',
			),
			'testing' => array(
				'title'       => __( 'Testing', 'ultimatewoo-pro' ),
				'type'        => 'title',
				'description' => '',
			),
			'debug' => array(
				...
				'default'     => 'no',
				'description' => sprintf( __( 'Log Google Calendar events, such as API requests, inside %s', 'ultimatewoo-pro' ), '<code>woocommerce/logs/' . $this->id . '-' . sanitize_file_name( wp_hash( $this->id ) ) . '.txt</code>' ),
			),
		);
	}
		...
		<tr valign="top">
			...
			<td class="forminp">
				...
				<?php if ( ! $access_token && ( $client_id && $client_secret && $calendar_id ) ) : ?>
					<p class="submit"><a class="button button-primary" onclick="jQuery('#wc_bookings_google_calendar_redirect').val('1'); jQuery('#mainform').submit();"><?php _e( 'Connect with Google', 'ultimatewoo-pro' ); ?></a></p>
				<?php elseif ( $access_token ) : ?>
					<p><?php _e( 'Successfully authenticated.', 'ultimatewoo-pro' ); ?></p>
					<p class="submit"><a class="button button-primary" href="<?php echo esc_url( add_query_arg( array( 'logout' => 'true' ), $this->redirect_uri ) ); ?>"><?php _e( 'Disconnect', 'ultimatewoo-pro' ); ?></a></p>
				<?php else : ?>
					<p><?php _e( 'Unable to authenticate, you must enter with your <strong>Client ID</strong>, <strong>Client Secret</strong> and <strong>Calendar ID</strong>.', 'ultimatewoo-pro' ); ?></p>
				<?php endif; ?>
			</td>
		</tr>
		<?php
	...
	public function oauth_redirect() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( __( 'Permission denied!', 'ultimatewoo-pro' ) );
		}
		...
		wp_die( __( 'Invalid request!', 'ultimatewoo-pro' ) );
	}
	...
	public function admin_notices() {
		...
		if ( 'woocommerce_page_wc-settings' == $screen->id && isset( $_GET['wc_gcalendar_oauth'] ) ) {
			if ( 'success' == $_GET['wc_gcalendar_oauth'] ) {
				echo '<div class="updated fade"><p><strong>' . __( 'Google Calendar', 'ultimatewoo-pro' ) . '</strong> ' . __( 'Account connected successfully!', 'ultimatewoo-pro' ) . '</p></div>';
			} else {
				echo '<div class="error fade"><p><strong>' . __( 'Google Calendar', 'ultimatewoo-pro' ) . '</strong> ' . __( 'Failed to connect to your account, please try again, if the problem persists, turn on Debug Log option and see what is happening.', 'ultimatewoo-pro' ) . '</p></div>';
			}
		}

		if ( 'woocommerce_page_wc-settings' == $screen->id && isset( $_GET['wc_gcalendar_logout'] ) ) {
			if ( 'success' == $_GET['wc_gcalendar_logout'] ) {
				echo '<div class="updated fade"><p><strong>' . __( 'Google Calendar', 'ultimatewoo-pro' ) . '</strong> ' . __( 'Account disconnected successfully!', 'ultimatewoo-pro' ) . '</p></div>';
			} else {
				echo '<div class="error fade"><p><strong>' . __( 'Google Calendar', 'ultimatewoo-pro' ) . '</strong> ' . __( 'Failed to disconnect to your account, please try again, if the problem persists, turn on Debug Log option and see what is happening.', 'ultimatewoo-pro' ) . '</p></div>';
			}
		}
	}
	...
	public function sync_booking( $booking_id ) {
		...
		$booking_data = array(
			__( 'Booking ID', 'ultimatewoo-pro' )   => $booking_id,
			__( 'Booking Type', 'ultimatewoo-pro' ) => is_object( $resource ) ? $resource->get_title() : '',
			__( 'Persons', 'ultimatewoo-pro' )      => $booking->has_persons() ? array_sum( $booking->get_persons() ) : 0,
		);
		...
		// Set the event data
		$data = array(
			'summary'     => wp_kses_post( '#' . $booking->get_id() . ' - ' . ( $product ? $product->get_title() : __( 'Booking', 'ultimatewoo-pro' ) ) ),
