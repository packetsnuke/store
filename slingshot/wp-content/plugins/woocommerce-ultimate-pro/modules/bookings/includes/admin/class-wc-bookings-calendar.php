...
class WC_Bookings_Calendar {
				...
				echo '<li><a href="' . admin_url( 'post.php?post=' . $booking->get_id() . '&action=edit' ) . '">';
					...
					echo '<ul>';
						if ( ( $customer = $booking->get_customer() ) && ! empty( $customer->name ) ) {
							echo '<li>' . __( 'Booked by', 'ultimatewoo-pro' ) . ' ' . $customer->name . '</li>';
						}
						echo '<li>';
						if ( $booking->is_all_day() ) {
							echo __( 'All Day', 'ultimatewoo-pro' );
						} else {
							echo $booking->get_start_date() . '&mdash;' . $booking->get_end_date();
						}
						echo '</li>';
						if ( $resource = $booking->get_resource() )
							echo '<li>' . __( 'Resource #', 'ultimatewoo-pro' ) . $resource->ID . ' - ' . $resource->post_title . '</li>';
					echo '</ul></a>';
				echo '</li>';
			}
		}
	...
	public function get_tip( $booking ) {
		...
		if ( ( $customer = $booking->get_customer() ) && ! empty( $customer->name ) ) {
			$return .= '<br/>' . __( 'Booked by', 'ultimatewoo-pro' ) . ' ' . $customer->name;
		}
		if ( $resource = $booking->get_resource() )
			$return .= '<br/>' . __( 'Resource #', 'ultimatewoo-pro' ) . $resource->ID . ' - ' . $resource->post_title;
