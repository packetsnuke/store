...
class WC_Bookings_CPT {
		...
		if ( $this->type === $post_type ) {
			...
			<script type="text/javascript">
				jQuery( document ).ready( function ( $ ) {
					$( '<option value="confirm_bookings"><?php _e( 'Confirm bookings', 'ultimatewoo-pro' )?></option>' ).appendTo( 'select[name="action"], select[name="action2"]' );
					$( '<option value="unconfirm_bookings"><?php _e( 'Unconfirm bookings', 'ultimatewoo-pro' )?></option>' ).appendTo( 'select[name="action"], select[name="action2"]' );
					$( '<option value="cancel_bookings"><?php _e( 'Cancel bookings', 'ultimatewoo-pro' )?></option>' ).appendTo( 'select[name="action"], select[name="action2"]' );
					$( '<option value="mark_paid_bookings"><?php _e( 'Mark bookings as paid', 'ultimatewoo-pro' )?></option>' ).appendTo( 'select[name="action"], select[name="action2"]' );
					$( '<option value="mark_unpaid_bookings"><?php _e( 'Mark bookings as unpaid', 'ultimatewoo-pro' )?></option>' ).appendTo( 'select[name="action"], select[name="action2"]' );
				});
			</script>
			<?php
		}
	...
	public function bulk_admin_notices() {
		...
		if ( isset( $_REQUEST['bookings_confirmed'] ) || isset( $_REQUEST['bookings_marked_paid'] ) || isset( $_REQUEST['bookings_marked_unpaid'] ) || isset( $_REQUEST['bookings_unconfirmed'] ) || isset( $_REQUEST['bookings_cancelled'] ) ) {
			...
			if ( 'edit.php' == $pagenow && $this->type == $post_type ) {
				$message = sprintf( _n( 'Booking status changed.', '%s booking statuses changed.', $number, 'ultimatewoo-pro' ), number_format_i18n( $number ) );
	...
	public function enter_title_here( $text, $post ) {
		if ( 'wc_booking' === $post->post_type ) {
			return __( 'Booking Title', 'ultimatewoo-pro' );
		}
		return $text;
	}
	...
	public function edit_columns( $existing_columns ) {
		...
		$columns                    = array();
		$columns["booking_status"]  = '<span class="status_head tips" data-tip="' . esc_attr__( 'Status', 'ultimatewoo-pro' ) . '">' . esc_attr__( 'Status', 'ultimatewoo-pro' ) . '</span>';
		$columns["booking_id"]      = __( 'ID', 'ultimatewoo-pro' );
		$columns["booked_product"]  = __( 'Booked Product', 'ultimatewoo-pro' );
		$columns["num_of_persons"]  = __( '# of Persons', 'ultimatewoo-pro' );
		$columns["customer"]        = __( 'Booked By', 'ultimatewoo-pro' );
		$columns["order"]           = __( 'Order', 'ultimatewoo-pro' );
		$columns["start_date"]      = __( 'Start Date', 'ultimatewoo-pro' );
		$columns["end_date"]        = __( 'End Date', 'ultimatewoo-pro' );
		$columns["booking_actions"] = __( 'Actions', 'ultimatewoo-pro' );
	...
	public function custom_columns( $column ) {
		...
		switch ( $column ) {
			case 'booking_status' :
				...
				break;
			case 'booking_id' :
				printf( '<a href="%s">' . __( 'Booking #%d', 'ultimatewoo-pro' ) . '</a>', admin_url( 'post.php?post=' . $post->ID . '&action=edit' ), $post->ID );
				break;
			case 'num_of_persons' :
				if ( ! is_object( $product ) || ! $product->has_persons() ) {
					esc_html_e( 'N/A', 'ultimatewoo-pro' );
					break;
				}
			...
			case 'booking_actions' :
				...
				if ( in_array( $booking->get_status(), array( 'pending-confirmation' ) ) ) {
					$actions['confirm'] = array(
						...
						'name'   => __( 'Confirm', 'ultimatewoo-pro' ),
						'action' => 'confirm',
					);
				}
	...
	public function booking_filters() {
		...
		if ( $filters ) {
			...
			$output .= '<option value="">' . __( 'All Bookable Products', 'ultimatewoo-pro' ) . '</option>';
