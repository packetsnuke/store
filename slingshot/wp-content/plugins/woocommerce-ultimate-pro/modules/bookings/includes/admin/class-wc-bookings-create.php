	...
	public function output() {
		...
		try {
			if ( ! empty( $_POST ) && ! check_admin_referer( 'create_booking_notification' ) ) {
				throw new Exception( __( 'Error - please try again', 'ultimatewoo-pro' ) );
			}
			...
			if ( ! empty( $_POST['create_booking'] ) ) {
				...
				if ( ! $bookable_product_id ) {
					throw new Exception( __( 'Please choose a bookable product', 'ultimatewoo-pro' ) );
				}
				...
				if ( 'existing' === $booking_order ) {
					...
					if ( ! $booking_order || get_post_type( $booking_order ) !== 'shop_order' ) {
						throw new Exception( __( 'Invalid order ID provided', 'ultimatewoo-pro' ) );
					}
				}
				...
				if ( 'new' === $booking_order ) {
					...
					if ( ! $order_id ) {
						throw new Exception( __( 'Error: Could not create order', 'ultimatewoo-pro' ) );
					}
				...
				if ( $order_id ) {
					...
					if ( ! $item_id ) {
						throw new Exception( __( 'Error: Could not create item', 'ultimatewoo-pro' ) );
					}
