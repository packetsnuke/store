...
class WC_Bookings_Ajax {
	...
	public function add_bookable_resource() {
		...
		if ( $add_resource_id ) {
			...
			if ( in_array( $add_resource_name, $resource_ids ) ) {
				wp_send_json( array( 'error' => __( 'The resource has already been linked to this product', 'ultimatewoo-pro' ) ) );
			}
		...
		wp_send_json( array( 'error' => __( 'Unable to add resource', 'ultimatewoo-pro' ) ) );
	}
	...
	public function mark_booking_confirmed() {
		...
		if ( ! check_admin_referer( 'wc-booking-confirm' ) ) {
			wp_die( __( 'You have taken too long. Please go back and retry.', 'ultimatewoo-pro' ) );
		}
	...
	public function calculate_costs() {
		...
		if ( ! $product ) {
			wp_send_json( array(
				'result' => 'ERROR',
				'html'   => '<span class="booking-error">' . __( 'This booking is unavailable.', 'ultimatewoo-pro' ) . '</span>',
			) );
		}
		...
		wp_send_json( array(
			'result' => 'SUCCESS',
			'html'   => apply_filters( 'woocommerce_bookings_booking_cost_string', __( 'Booking cost', 'ultimatewoo-pro' ), $product ) . ': <strong>' . wc_price( $display_price ) . $price_suffix . '</strong>',
		) );
	}
	...
	public function get_time_blocks_for_date() {
		...
		if ( empty( $timestamp ) ) {
			die( '<li>' . esc_html__( 'Please enter a valid date.', 'ultimatewoo-pro' ) . '</li>' );
		}
		...
		if ( empty( $block_html ) ) {
			$block_html .= '<li>' . __( 'No blocks available.', 'ultimatewoo-pro' ) . '</li>';
		}
