...
class WC_Bookings_Details_Meta_Box {
	...
	public function __construct() {
		...
		$this->title      = __( 'Booking Details', 'ultimatewoo-pro' );
		$this->context    = 'normal';
		$this->priority   = 'high';
	...
	private function sanity_check_notices( $booking, $product ) {
		if ( $booking->get_start() && $booking->get_start() > strtotime( '+ 2 year', current_time( 'timestamp' ) ) ) {
			echo '<div class="updated highlight"><p>' . __( 'This booking is scheduled over 2 years into the future. Please ensure this is correct.', 'ultimatewoo-pro' ) . '</p></div>';
		}
		...
		if ( $product && is_callable( array( $product, 'get_max_date' ) ) ) {
			...
			if ( $booking->get_start() > $max_date || $booking->get_end() > $max_date ) {
				echo '<div class="updated highlight"><p>' . sprintf( __( 'This booking is scheduled over the products allowed max booking date (%s). Please ensure this is correct.', 'ultimatewoo-pro' ), date_i18n( wc_date_format(), $max_date ) ) . '</p></div>';
			}
		}

		if ( $booking->get_start() && $booking->get_end() && $booking->get_start() > $booking->get_end() ) {
			echo '<div class="error"><p>' . __( 'This booking has an end date set before the start date.', 'ultimatewoo-pro' ) . '</p></div>';
		}

		if ( $booking->get_product_id() && ! wc_get_product( $booking->get_product_id() ) ) {
			echo '<div class="error"><p>' . __( 'It appears the booking product associated with this booking has been removed.', 'ultimatewoo-pro' ) . '</p></div>';
			return;
		}

		if ( $product && is_callable( array( $product, 'is_skeleton' ) ) && $product->is_skeleton() ) {
			echo '<div class="error"><p>' . sprintf( __( 'This booking is missing a required add-on (product type: %s). Some information is shown below but might be incomplete. Please install the missing add-on through the plugins screen.', 'ultimatewoo-pro' ), $product_check->get_type() ) . '</p></div>';
		}
	}
	...
	public function meta_box_inner( $post ) {
		...
		$bookable_products = array( '' => __( 'N/A', 'ultimatewoo-pro' ) );
		...
		<div class="panel-wrap woocommerce">
			<div id="booking_data" class="panel">
				<h2><?php printf( __( 'Booking #%s details', 'ultimatewoo-pro' ), esc_html( $post->ID ) ) ?></h2>
				<p class="booking_number"><?php
					if ( $order ) {
						printf( ' ' . __( 'Linked to order %s.', 'ultimatewoo-pro' ), '<a href="' . admin_url( 'post.php?post=' . absint( ( is_callable( array( $order, 'get_id' ) ) ? $order->get_id() : $order->id ) ) . '&action=edit' ) . '">#' . esc_html( $order->get_order_number() ) . '</a>' );
					}

					if ( $product && is_callable( array( $product, 'is_bookings_addon' ) ) && $product->is_bookings_addon() ) {
						printf( ' ' . __( 'Booking type: %s', 'ultimatewoo-pro' ), $product->bookings_addon_title() );
					}
				?></p>

				<div class="booking_data_column_container">
					<div class="booking_data_column">
						<h4><?php _e( 'General details', 'ultimatewoo-pro' ); ?></h4>

						<p class="form-field form-field-wide">
							<label for="_booking_order_id"><?php _e( 'Order ID:', 'ultimatewoo-pro' ); ?></label>
							<?php if ( version_compare( WC_VERSION, '3.0', '<' ) ) : ?>
								<input type="hidden" name="_booking_order_id" id="_booking_order_id" value="<?php echo esc_attr( $booking->get_order_id() ); ?>" data-selected="<?php echo esc_attr( $order ? $order->get_order_number() : '' ); ?>" data-placeholder="<?php _e( 'N/A', 'ultimatewoo-pro' ); ?>" data-allow_clear="true" />
							<?php else : ?>
								<select name="_booking_order_id" id="_booking_order_id" data-placeholder="<?php _e( 'N/A', 'ultimatewoo-pro' ); ?>" data-allow_clear="true">
						...
						<p class="form-field form-field-wide"><label for="booking_date"><?php _e( 'Date created:', 'ultimatewoo-pro' ); ?></label>
							<input type="text" class="date-picker-field" name="booking_date" id="booking_date" maxlength="10" value="<?php echo date_i18n( 'Y-m-d', $booking->get_date_created() ); ?>" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" /> @ <input type="number" class="hour" placeholder="<?php _e( 'h', 'ultimatewoo-pro' ); ?>" name="booking_date_hour" id="booking_date_hour" maxlength="2" size="2" value="<?php echo date_i18n( 'H', $booking->get_date_created() ); ?>" pattern="\-?\d+(\.\d{0,})?" />:<input type="number" class="minute" placeholder="<?php _e( 'm', 'ultimatewoo-pro' ); ?>" name="booking_date_minute" id="booking_date_minute" maxlength="2" size="2" value="<?php echo date_i18n( 'i', $booking->get_date_created() ); ?>" pattern="\-?\d+(\.\d{0,})?" />
						</p>

						<p class="form-field form-field-wide">
							<label for="_booking_status"><?php _e( 'Booking status:', 'ultimatewoo-pro' ); ?></label>
							<select id="_booking_status" name="_booking_status" class="wc-enhanced-select"><?php
								foreach ( $statuses as $key => $value ) {
									echo '<option value="' . esc_attr( $key ) . '" ' . selected( $key, $booking->get_status(), false ) . '>' . esc_html__( $value, 'ultimatewoo-pro' ) . '</option>';
								}
							?></select>
						...
						<p class="form-field form-field-wide">
							<label for="_booking_customer_id"><?php _e( 'Customer:', 'ultimatewoo-pro' ); ?></label>
							<?php
								$name = ! empty( $customer->name ) ? ' &ndash; ' . $customer->name : '';
								$guest_placeholder = __( 'Guest', 'ultimatewoo-pro' );
								if ( 'Guest' === $name ) {
									$guest_placeholder = sprintf( _x( 'Guest (%s)', 'ultimatewoo-pro', 'Admin booking guest placeholder' ), $name );
								}
					...
					<div class="booking_data_column">
						<h4><?php _e( 'Booking specification', 'ultimatewoo-pro' ); ?></h4>
						...
						<?php
							woocommerce_wp_select( array(
								...
								'label'         => __( 'Booked product:', 'ultimatewoo-pro' ),
							...
							woocommerce_wp_text_input( array(
								...
								'label'         => __( 'Parent booking ID:', 'ultimatewoo-pro' ),
								'wrapper_class' => 'form-field form-field-wide',
								'placeholder'   => 'N/A',
								'class'         => '',
							...
							echo '<br class="clear" />';
							echo '<h4>' . esc_html__( 'Person(s)', 'ultimatewoo-pro' ) . '</h4>';
					...
					<div class="booking_data_column">
						<h4><?php _e( 'Booking date &amp; time', 'ultimatewoo-pro' ); ?></h4>
						<?php
							woocommerce_wp_text_input( array(
								'id'          => 'booking_start_date',
								'label'       => __( 'Start date:', 'ultimatewoo-pro' ),
							...
							woocommerce_wp_text_input( array(
								'id'          => 'booking_end_date',
								'label'       => __( 'End date:', 'ultimatewoo-pro' ),
								'placeholder' => 'yyyy-mm-dd',
							...
							woocommerce_wp_checkbox( array(
								'id'          => '_booking_all_day',
								'label'       => __( 'All day booking:', 'ultimatewoo-pro' ),
								'description' => __( 'Check this box if the booking is for all day.', 'ultimatewoo-pro' ),
								'value'       => $booking->get_all_day( 'edit' ) ? 'yes' : 'no',
							) );

							woocommerce_wp_text_input( array(
								'id'          => 'booking_start_time',
								'label'       => __( 'Start time:', 'ultimatewoo-pro' ),
								'placeholder' => 'hh:mm',
							...
							woocommerce_wp_text_input( array(
								'id'          => 'booking_end_time',
								'label'       => __( 'End time:', 'ultimatewoo-pro' ),
								'placeholder' => 'hh:mm',
	...
	public function get_labels_for_statuses( $statuses = array() ) {
		...
		foreach ( $statuses as $status ) {
			$labels[ $status ] = __( $status, 'ultimatewoo-pro' );
		}
		return $labels;
	}
