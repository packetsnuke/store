...
class WC_Bookings_Menus {
	...
	public function admin_menu() {
		$create_booking_page = add_submenu_page( 'edit.php?post_type=wc_booking', __( 'Create Booking', 'ultimatewoo-pro' ), __( 'Create Booking', 'ultimatewoo-pro' ), 'manage_bookings', 'create_booking', array( $this, 'create_booking_page' ) );
		$calendar_page       = add_submenu_page( 'edit.php?post_type=wc_booking', __( 'Calendar', 'ultimatewoo-pro' ), __( 'Calendar', 'ultimatewoo-pro' ), 'manage_bookings', 'booking_calendar', array( $this, 'calendar_page' ) );
		$notification_page   = add_submenu_page( 'edit.php?post_type=wc_booking', __( 'Send Notification', 'ultimatewoo-pro' ), __( 'Send Notification', 'ultimatewoo-pro' ), 'manage_bookings', 'booking_notification', array( $this, 'notifications_page' ) );
		$settings_page       = add_submenu_page( 'edit.php?post_type=wc_booking', __( 'Global Availability', 'ultimatewoo-pro' ), __( 'Global Availability', 'ultimatewoo-pro' ), 'manage_bookings', 'wc_bookings_global_availability', array( $this, 'global_availability_page' ) );
		...
		if ( ! empty( $_POST ) && check_admin_referer( 'send_booking_notification' ) ) {
			...
			try {
				if ( ! $notification_product_id )
					throw new Exception( __( 'Please choose a product', 'ultimatewoo-pro' ) );

				if ( ! $notification_message )
					throw new Exception( __( 'Please enter a message', 'ultimatewoo-pro' ) );

				if ( ! $notification_subject )
					throw new Exception( __( 'Please enter a subject', 'ultimatewoo-pro' ) );
				...
				echo '<div class="updated fade"><p>' . __( 'Notification sent successfully', 'ultimatewoo-pro' ) . '</p></div>';
