	...
	public static function cancel_booking() {
		if ( isset( $_GET['cancel_booking'] ) && isset( $_GET['booking_id'] ) ) {
			...
			if ( $booking->has_status( 'cancelled' ) ) {
				...
				// Message
				wc_add_notice( apply_filters( 'woocommerce_booking_cancelled_notice', __( 'Your booking was cancelled.', 'ultimatewoo-pro' ) ), apply_filters( 'woocommerce_booking_cancelled_notice_type', 'notice' ) );

				do_action( 'woocommerce_bookings_cancelled_booking', $booking->get_id() );
			} elseif ( ! $booking_can_cancel ) {
				wc_add_notice( __( 'Your booking can no longer be cancelled. Please contact us if you need assistance.', 'ultimatewoo-pro' ), 'error' );
			} else {
				wc_add_notice( __( 'Invalid booking.', 'ultimatewoo-pro' ), 'error' );
			}
