...
class WC_Booking_Form {
	...
	public function scripts() {
		...
		$wc_bookings_booking_form_args = array(
			'closeText'                  => __( 'Close', 'ultimatewoo-pro' ),
			'currentText'                => __( 'Today', 'ultimatewoo-pro' ),
			'prevText'                   => __( 'Previous', 'ultimatewoo-pro' ),
			'nextText'                   => __( 'Next', 'ultimatewoo-pro' ),
		...
		$booking_form_params = array(
			...
			'i18n_date_unavailable'      => __( 'This date is unavailable', 'ultimatewoo-pro' ),
			'i18n_date_fully_booked'     => __( 'This date is fully booked and unavailable', 'ultimatewoo-pro' ),
			'i18n_date_partially_booked' => __( 'This date is partially booked - but bookings still remain', 'ultimatewoo-pro' ),
			'i18n_date_available'        => __( 'This date is available', 'ultimatewoo-pro' ),
			'i18n_start_date'            => __( 'Choose a Start Date', 'ultimatewoo-pro' ),
			'i18n_end_date'              => __( 'Choose an End Date', 'ultimatewoo-pro' ),
			'i18n_dates'                 => __( 'Dates', 'ultimatewoo-pro' ),
			'i18n_choose_options'        => __( 'Please select the options for your booking above first', 'ultimatewoo-pro' ),
		);
	...
	private function duration_field() {
		...
		if ( 'customer' === $this->product->get_duration_type() ) {
			...
			switch ( $this->product->get_duration_unit() ) {
				case 'month' :
					if ( $this->product->get_duration() > 1 ) {
						$after = sprintf( __( '&times; %s Months', 'ultimatewoo-pro' ), $this->product->get_duration() );
					} else {
						$after = __( 'Month(s)', 'ultimatewoo-pro' );
					}
					break;
				case 'week' :
					if ( $this->product->get_duration() > 1 ) {
						$after = sprintf( __( '&times; %s weeks', 'ultimatewoo-pro' ), $this->product->get_duration() );
					} else {
						$after = __( 'Week(s)', 'ultimatewoo-pro' );
					}
					break;
				case 'day' :
					if ( $this->product->get_duration() % 7 ) {
						if ( $this->product->get_duration() > 1 ) {
							$after = sprintf( __( '&times; %s days', 'ultimatewoo-pro' ), $this->product->get_duration() );
						} else {
							$after = __( 'Day(s)', 'ultimatewoo-pro' );
						}
					} else {
						if ( 1 == ( $this->product->get_duration() / 7 ) ) {
							$after = __( 'Week(s)', 'ultimatewoo-pro' );
						} else {
							$after = sprintf( __( '&times; %s weeks', 'ultimatewoo-pro' ), $this->product->get_duration() / 7 );
						}
					}
					break;
				case 'night' :
					if ( $this->product->get_duration() > 1 ) {
								$after = sprintf( __( '&times; %s nights', 'ultimatewoo-pro' ), $this->product->get_duration() );
					} else {
							$after = __( 'Night(s)', 'ultimatewoo-pro' );
					}
					break;
				case 'hour' :
					if ( $this->product->get_duration() > 1 ) {
						$after = sprintf( __( '&times; %s hours', 'ultimatewoo-pro' ), $this->product->get_duration() );
					} else {
						$after = __( 'Hour(s)', 'ultimatewoo-pro' );
					}
					break;
				case 'minute' :
					if ( $this->product->get_duration() > 1 ) {
						$after = sprintf( __( '&times; %s minutes', 'ultimatewoo-pro' ), $this->product->get_duration() );
					} else {
						$after = __( 'Minute(s)', 'ultimatewoo-pro' );
					}
					break;
			}
			...
			$this->add_field( array(
				'type'  => 'number',
				'name'  => 'duration',
				'label' => __( 'Duration', 'ultimatewoo-pro' ),
				'after' => $after,
		...
		if ( $this->product->has_persons() ) {
			if ( $this->product->has_person_types() ) {
				...
				$this->add_field( array(
					...
					'name'  => 'persons',
					'label' => __( 'Persons', 'ultimatewoo-pro' ),
				) );
			}
		}
	}
	...
	private function resources_field() {
		...
		foreach ( $resources as $resource ) {
			...
			if ( $resource->get_block_cost() && ! $this->product->get_display_cost() ) {
				...
				if ( in_array( $duration_unit, array( 'minute', 'hour' ) ) ) {
					$duration_unit = __( 'block', 'ultimatewoo-pro' );
				}
				$additional_cost[] = sprintf( __( '+%1$1s per %2$2s', 'ultimatewoo-pro' ), wc_price( $resource->get_block_cost() ), $duration_unit );
			}
		...
		$label = $this->product->get_resource_label() ? $this->product->get_resource_label() : __( 'Type', 'ultimatewoo-pro' );
		$this->add_field( array(
			'type'    => 'select',
			'name'    => 'resource',
			'label'   => $label,
	...
	public function output() {
		...
		foreach ( $this->fields as $key => $field ) {
			wc_get_template( 'booking-form/' . $field['type'] . '.php', array( 'field' => $field ), 'ultimatewoo-pro', WC_BOOKINGS_TEMPLATE_PATH );
		}
	}
	...
	public function get_posted_data( $posted = array() ) {
		...
		if ( $this->product->has_persons() ) {
			if ( $this->product->has_person_types() ) {
				...
				$data[ __( 'Persons', 'ultimatewoo-pro' ) ] = absint( $posted['wc_bookings_field_persons'] );
		...
		if ( 'customer' == $this->product->get_duration_type() ) {
			...
			switch ( $booking_duration_unit ) {
				case 'month' :
					$data['duration'] = $total_duration . ' ' . _n( 'month', 'months', $total_duration, 'ultimatewoo-pro' );
					break;
				case 'day' :
					if ( $total_duration % 7 ) {
						$data['duration'] = $total_duration . ' ' . _n( 'day', 'days', $total_duration, 'ultimatewoo-pro' );
					} else {
						$duration_in_weeks 	= ( $total_duration / 7 );
						$data['duration'] 	= $duration_in_weeks . ' ' . _n( 'week', 'weeks', $duration_in_weeks, 'ultimatewoo-pro' );
					}
					break;
				case 'hour' :
					$data['duration'] = $total_duration . ' ' . _n( 'hour', 'hours', $total_duration, 'ultimatewoo-pro' );
					break;
				case 'minute' :
					$data['duration'] = $total_duration . ' ' . _n( 'minute', 'minutes', $total_duration, 'ultimatewoo-pro' );
					break;
				case 'night' :
					$data['duration'] = $total_duration . ' ' . _n( 'night', 'nights', $total_duration, 'ultimatewoo-pro' );
					break;
				default :
	...
	public function is_bookable( $data ) {
		...
		if ( $this->product->has_resources() && $this->product->is_resource_assignment_type( 'customer' ) ) {
			if ( empty( $data['_resource_id'] ) ) {
				return new WP_Error( 'Error', __( 'Please choose a resource type', 'ultimatewoo-pro' ) );
			}
		...
		if ( $this->product->is_duration_type( 'customer' ) ) {
			if ( empty( $data['_duration'] ) ) {
				return new WP_Error( 'Error', __( 'Duration is required - please enter a duration greater than zero above', 'ultimatewoo-pro' ) );
			}
			if ( $data['_duration'] > $this->product->get_max_duration() ) {
				return new WP_Error( 'Error', sprintf( __( 'The maximum duration is %d', 'ultimatewoo-pro' ), $this->product->get_max_duration() ) );
			}
			if ( $data['_duration'] < $this->product->get_min_duration() ) {
				return new WP_Error( 'Error', sprintf( __( 'The minimum duration is %d', 'ultimatewoo-pro' ), $this->product->get_min_duration() ) );
			}
		}

		// Validate date and time
		if ( empty( $data['date'] ) ) {
			return new WP_Error( 'Error', __( 'Date is required - please choose one above', 'ultimatewoo-pro' ) );
		}
		if ( in_array( $this->product->get_duration_unit(), array( 'minute', 'hour' ) ) && empty( $data['time'] ) ) {
			return new WP_Error( 'Error', __( 'Time is required - please choose one above', 'ultimatewoo-pro' ) );
		}
		if ( $data['_date'] && date( 'Ymd', strtotime( $data['_date'] ) ) < date( 'Ymd', current_time( 'timestamp' ) ) ) {
			return new WP_Error( 'Error', __( 'You must choose a future date and time.', 'ultimatewoo-pro' ) );
		}
		...
		if ( $min = $this->product->get_min_date() ) {
			...
			if ( strtotime( $data['_date'] . ' ' . $data['_time'] ) < $min_date ) {
				return new WP_Error( 'Error', sprintf( __( 'The earliest booking possible is currently %s.', 'ultimatewoo-pro' ), date_i18n( wc_date_format() . ' ' . get_option( 'time_format' ), $min_date ) ) );
			}
		}
		if ( $max = $this->product->get_max_date() ) {
			$max_date = strtotime( "+{$max['value']} {$max['unit']}", $now );
			if ( strtotime( $data['_date'] . ' ' . $data['_time'] ) > $max_date ) {
				return new WP_Error( 'Error', sprintf( __( 'The latest booking possible is currently %s.', 'ultimatewoo-pro' ), date_i18n( wc_date_format() . ' ' . get_option( 'time_format' ), $max_date ) ) );
			}
		}
		...
		if ( $this->product->has_restricted_days() ) {
			...
			if ( ! in_array( date( 'w', strtotime( $data['date'] ) ) , $restricted_days ) ) {
				return new WP_Error( 'Error', __( 'Sorry, bookings cannot start on this day.', 'ultimatewoo-pro' ) );
			}
		}
		...
		if ( $this->product->has_persons() ) {
			...
			if ( empty( $persons ) ) {
				return new WP_Error( 'Error', __( 'Persons are required - please enter the number of persons above', 'ultimatewoo-pro' ) );
			}
			if ( $this->product->get_max_persons() && $persons > $this->product->get_max_persons() ) {
				return new WP_Error( 'Error', sprintf( __( 'The maximum persons per group is %d', 'ultimatewoo-pro' ), $this->product->get_max_persons() ) );
			}
			if ( $persons < $this->product->get_min_persons() ) {
				return new WP_Error( 'Error', sprintf( __( 'The minimum persons per group is %d', 'ultimatewoo-pro' ), $this->product->get_min_persons() ) );
			}
			...
			if ( $this->product->has_person_types() ) {
				...
				foreach ( $person_types as $person ) {
					...
					if ( is_numeric( $person_max ) && isset( $data['_persons'][ $person->get_id() ] ) && $data['_persons'][ $person->get_id() ] > $person_max ) {
						return new WP_Error( 'Error', sprintf( __( 'The maximum %1$s per group is %2$d', 'ultimatewoo-pro' ), $person->post_title, $person_max ) );
					}
					...
					if ( is_numeric( $person_min ) && isset( $data['_persons'][ $person->get_id() ] ) && $data['_persons'][ $person->get_id() ] < $person_min ) {
						return new WP_Error( 'Error', sprintf( __( 'The minimum %1$s per group is %2$d', 'ultimatewoo-pro' ), $person->post_title, $person_min ) );
					}
				}
			}
		}
		...
		if ( is_wp_error( $available_bookings ) ) {
			...
			return new WP_Error( 'Error', __( 'Sorry, the selected block is not available', 'ultimatewoo-pro' ) );
		}
		...
		if ( ! empty( $buffer_period ) ) {
			...
			if ( ! in_array( $this->product->get_duration_unit(), array( 'minute', 'hour' ) ) ) {
				...
				if ( $contains_buffer_days ) {
					...
					return new WP_Error( 'Error', sprintf( __( 'The duration of this booking must be at least %s days.', 'ultimatewoo-pro' ), $block_duration_string ) );
				}
			}
		}
