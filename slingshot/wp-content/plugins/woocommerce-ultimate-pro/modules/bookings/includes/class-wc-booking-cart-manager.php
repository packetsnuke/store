...
class WC_Booking_Cart_Manager {
	...
	public function add_to_cart() {
		...
		// Get template
		wc_get_template( 'single-product/add-to-cart/booking.php', array( 'booking_form' => $booking_form ), 'ultimatewoo-pro', WC_BOOKINGS_TEMPLATE_PATH );
	}
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			if ( isset( $cart_item['booking'] ) ) {
				...
				if ( ! $booking || ! $booking->has_status( array( 'was-in-cart', 'in-cart', 'unpaid', 'paid' ) ) ) {
					...
					wc_add_notice( sprintf( __( 'A booking for %s has been removed from your cart due to inactivity.', 'ultimatewoo-pro' ), '<a href="' . get_permalink( $cart_item['product_id'] ) . '">' . get_the_title( $cart_item['product_id'] ) . '</a>' ), 'notice' );
				}
			}
		}
	}
	...
	public function validate_booking_requires_confirmation( $passed, $product_id ) {
		if ( wc_booking_requires_confirmation( $product_id ) ) {
			...
			wc_add_notice( __( 'A booking that requires confirmation has been removed from your cart. It is not possible to complete the purchased along with a booking that doesn\'t require confirmation.', 'ultimatewoo-pro' ), 'notice' );
		}
