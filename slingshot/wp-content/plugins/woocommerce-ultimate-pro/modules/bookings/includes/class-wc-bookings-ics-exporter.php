...
class WC_Bookings_ICS_Exporter {
	...
	protected function generate() {
		...
		$ics .= 'X-WR-CALDESC:' . $this->sanitize_string( sprintf( __( 'Bookings from %s', 'ultimatewoo-pro' ), $sitename ) ) . $this->eol;
		...
		foreach ( $this->bookings as $booking ) {
			...
			if ( $resource = $booking->get_resource() ) {
				$description .= __( 'Resource #', 'ultimatewoo-pro' ) . $resource->ID . ' - ' . $resource->post_title . '\n\n';
			}
			...
			if ( $booking->has_persons() ) {
				foreach ( $booking->get_persons() as $id => $qty ) {
					...
					$description .= sprintf( __( '%1$s: %2$d', 'ultimatewoo-pro' ), $person_type, $qty ) . '\n';
				}
			...
			if ( '' !== $post_excerpt ) {
				$description .= __( 'Booking description:', 'ultimatewoo-pro' ) . '\n';
