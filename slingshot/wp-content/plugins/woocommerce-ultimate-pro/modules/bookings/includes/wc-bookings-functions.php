...
function get_wc_booking_data_label( $key, $product ) {
	$labels = apply_filters( 'woocommerce_bookings_data_labels', array(
			'type'     => ( $product->get_resource_label() ? $product->get_resource_label() : __( 'Booking Type', 'ultimatewoo-pro' ) ),
			'date'     => __( 'Booking Date', 'ultimatewoo-pro' ),
			'time'     => __( 'Booking Time', 'ultimatewoo-pro' ),
			'duration' => __( 'Duration', 'ultimatewoo-pro' ),
			'persons'  => __( 'Person(s)', 'ultimatewoo-pro' ),
	) );
...
function get_wc_booking_rules_explanation() {
	return __( 'Rules with lower priority numbers will override rules with a higher priority (e.g. 9 overrides 10 ). Ordering is only applied within the same priority and higher order overrides lower order.', 'ultimatewoo-pro' );
}
...
function get_wc_booking_priority_explanation() {
	return __( 'Rules with lower priority numbers will override rules with a higher priority (e.g. 9 overrides 10 ). Global rules take priority over product rules which take priority over resource rules. By using priority numbers you can execute rules in different orders.', 'ultimatewoo-pro' );
}
...
function wc_bookings_get_time_slots_html( $bookable_product, $blocks, $intervals = array(), $resource_id = 0, $from = 0, $to = 0 ) {
	...
	foreach ( $available_blocks as $block => $quantity ) {
		if ( $quantity['available'] > 0 ) {
			if ( $quantity['booked'] ) {
				$block_html .= '<li class="block" data-block="' . esc_attr( date( 'Hi', $block ) ) . '"><a href="#" data-value="' . date( 'G:i', $block ) . '">' . date_i18n( get_option( 'time_format' ), $block ) . ' <small class="booking-spaces-left">(' . sprintf( _n( '%d left', '%d left', $quantity['available'], 'ultimatewoo-pro' ), absint( $quantity['available'] ) ) . ')</small></a></li>';
			} else {
...
function wc_bookings_get_summary_list( $booking, $is_admin = false ) {
	...
	$label    = $product && is_callable( array( $product, 'get_resource_label' ) ) && $product->get_resource_label() ? $product->get_resource_label() : __( 'Type', 'ultimatewoo-pro' );
	...
	wc_get_template( 'order/booking-summary-list.php', $template_args, 'ultimatewoo-pro', WC_BOOKINGS_TEMPLATE_PATH );
}
