...
?>
	<ul class="wc-booking-summary-list">
		...
		<?php if ( $resource ) : ?>
			<li><?php echo esc_html( sprintf( __( '%s: %s', 'ultimatewoo-pro' ), $label, $resource->get_name() ) ); ?></li>
		<?php endif; ?>
		...
		<?php
			if ( $product->has_persons() ) {
				if ( $product->has_person_types() ) {
					...
					?>
					<li><?php echo esc_html( sprintf( __( '%d Persons', 'ultimatewoo-pro' ), array_sum( $booking->get_person_counts() ) ) ); ?></li>
					<?php
				}
			}
		?>
