...
enericSiteIdent=',
	...
	public function shop_order_columns( $columns ) {
		$columns['shipment_tracking'] = __( 'Shipment Tracking', 'ultimatewoo-pro' );
		return $columns;
	}
	...
	public function add_meta_box() {
		add_meta_box( 'woocommerce-shipment-tracking', __( 'Shipment Tracking', 'ultimatewoo-pro' ), array( $this, 'meta_box' ), 'shop_order', 'side', 'high' );
	}
			...
			<div class="tracking-item" id="tracking-item-<?php echo esc_attr( $item['tracking_id'] ); ?>">
				<p class="tracking-content">
					...
					<?php if ( strlen( $formatted['formatted_tracking_link'] ) > 0 ) : ?>
						- <?php echo sprintf( '<a href="%s" target="_blank" title="' . esc_attr( __( 'Click here to track your shipment', 'ultimatewoo-pro' ) ) . '">' . __( 'Track', 'ultimatewoo-pro' ) . '</a>', esc_url( $formatted['formatted_tracking_link'] ) ); ?>
					<?php endif; ?>
					<br/>
				...
				<p class="meta">
					...
					<?php echo esc_html( sprintf( __( 'Shipped on %s', 'ultimatewoo-pro' ), date_i18n( 'Y-m-d', $item['date_shipped'] ) ) ); ?>
					<a href="#" class="delete-tracking" rel="<?php echo esc_attr( $item['tracking_id'] ); ?>"><?php _e( 'Delete', 'ultimatewoo-pro' ); ?></a>
				</p>
			</div>
			<?php
	}
	...
	public function meta_box() {
		...
		echo '<button class="button button-show-form" type="button">' . __( 'Add Tracking Number', 'ultimatewoo-pro' ) . '</button>';

		echo '<div id="shipment-tracking-form">';
		// Providers
		echo '<p class="form-field tracking_provider_field"><label for="tracking_provider">' . __( 'Provider:', 'ultimatewoo-pro' ) . '</label><br/><select id="tracking_provider" name="tracking_provider" class="chosen_select" style="width:100%;">';

		echo '<option value="">' . __( 'Custom Provider', 'ultimatewoo-pro' ) . '</option>';
		...
		woocommerce_wp_text_input( array(
			...
			'label'       => __( 'Provider Name:', 'ultimatewoo-pro' ),
			'placeholder' => '',
			'description' => '',
			'value'       => '',
		) );
		...
		woocommerce_wp_text_input( array(
			'id'          => 'tracking_number',
			'label'       => __( 'Tracking number:', 'ultimatewoo-pro' ),
			'placeholder' => '',
			'description' => '',
			'value'       => '',
		) );
		...
		woocommerce_wp_text_input( array(
			...
			'label'       => __( 'Tracking link:', 'ultimatewoo-pro' ),
			'placeholder' => 'http://',
			'description' => '',
			'value'       => '',
		) );
		...
		woocommerce_wp_text_input( array(
			'id'          => 'date_shipped',
			'label'       => __( 'Date shipped:', 'ultimatewoo-pro' ),
			'placeholder' => date_i18n( __( 'Y-m-d', 'ultimatewoo-pro' ), time() ),
			'description' => '',
			'class'       => 'date-picker-field',
			'value'       => date_i18n( __( 'Y-m-d', 'ultimatewoo-pro' ), current_time( 'timestamp' ) ),
		) );

		echo '<button class="button button-primary button-save-form">' . __( 'Save Tracking', 'ultimatewoo-pro' ) . '</button>';

		// Live preview
		echo '<p class="preview_tracking_link">' . __( 'Preview:', 'ultimatewoo-pro' ) . ' <a href="" target="_blank">' . __( 'Click here to track your shipment', 'ultimatewoo-pro' ) . '</a></p>';
