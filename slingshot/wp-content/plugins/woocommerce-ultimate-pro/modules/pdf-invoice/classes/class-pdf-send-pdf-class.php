<?php
        ...
        class WC_send_pdf {
			...
			function get_pdf_order_note( $order_id ) {
				...
				if( $customer_note ) {
					$output = '<h3>' . __('Note:', 'ultimatewoo-pro') . '</h3>' . wpautop( wptexturize( $customer_note ) );
			...
			function get_pdf_order_subtotal( $order_id ) {
				...
				$output = 	'<tr>' .
							'<td align="right">' .
							'<strong>' . __('Subtotal', 'ultimatewoo-pro') . '</strong></td>' .
			...
			function get_pdf_order_shipping( $order_id ) {
				...
				$output = 	'<tr>' .
							'<td align="right">' .
							'<strong>' . __('Shipping', 'ultimatewoo-pro') . '</strong></td>' .
			...
			function pdf_coupons_used( $order_id ) {
				...
				if( $order->get_used_coupons() ) {
					...
					$return_coupon .= '<br /><strong>' . __('Coupons used', 'ultimatewoo-pro') . ' (' . $coupons_count . ') :</strong>' . $coupons_list;
			...
			function get_pdf_order_discount( $order_id ) {
				...
				if ( $order_discount > 0 ) {
					$output .=  '<tr>' .
								...
								'<strong>' . esc_html__('Discount', 'ultimatewoo-pro') . '</strong>' . $this->pdf_coupons_used( $order_id ) . '</td>' .
			...
			function get_pdf_order_tax( $order_id ) {
				...
				if ( $order->get_total_tax()>0 ) {
					...
					if ( count( $tax_items ) > 1 ) {
						...
						$output .=  '<tr>' .
									'<td align="right">' . __('Total Tax', 'ultimatewoo-pro') . '</td>' .
			...
			function get_pdf_order_total( $order_id ) {
				...
				$output =  	'<tr>' .
							'<td align="right">' .
							'<strong>' . __('Grand Total', 'ultimatewoo-pro') . '</strong></td>' .
			...
			function get_woocommerce_invoice_content( $order_id ) {
				...
				if ( !isset( $pdfcompanyname ) || $pdfcompanyname == '' ) {
					$pdfcompanyname    = __( $woocommerce_pdf_invoice_options['pdf_company_name'], 'ultimatewoo-pro' );
				}
				...
				$headers =  '<table class="shop_table orderdetails" width="100%">' . 
							'<thead>' .
							'<tr><th colspan="7" align="left"><h2>' . esc_html__('Order Details', 'ultimatewoo-pro') . '</h2></th></tr>' .
							'<tr>' .
							'<th width="5%" valign="top" align="right">'  . esc_html__( 'Qty', 'ultimatewoo-pro' ) 		. '</th>' .						
							'<th width="50%" valign="top" align="left">'  . esc_html__( 'Product', 'ultimatewoo-pro' ) 	. '</th>' .
							'<th width="9%" valign="top" align="right">'  . esc_html__( 'Price Ex', 'ultimatewoo-pro' ) 	. '</th>' .
							'<th width="9%" valign="top" align="right">'  . esc_html__( 'Total Ex.', 'ultimatewoo-pro' ) 	. '</th>' .
							'<th width="7%" valign="top" align="right">'  . esc_html__( 'Tax', 'ultimatewoo-pro' ) 		. '</th>' .
							'<th width="10%" valign="top" align="right">' . esc_html__( 'Price Inc', 'ultimatewoo-pro' ) 	. '</th>' .
							'<th width="10%" valign="top" align="right">' . esc_html__( 'Total Inc', 'ultimatewoo-pro' ) 	. '</th>' .
							'</tr>' .
							'</thead>' .
							'</table>';
				...
				if ( get_post_meta( $order_id,'VAT Number',TRUE ) ) {
					$content = str_replace(	'[[PDFBILLINGVATNUMBER]]', '<br />' . __( 'VAT Number : ', 'ultimatewoo-pro' ) . get_post_meta( $order_id,'VAT Number',TRUE ), $content );
				} elseif ( get_post_meta( $order_id,'vat_number',TRUE ) ) {	
					$content = str_replace(	'[[PDFBILLINGVATNUMBER]]', '<br />' . __( 'VAT Number : ', 'ultimatewoo-pro' ) . get_post_meta( $order_id,'vat_number',TRUE ), $content );
				} else {
			...
			public static function send_test_pdf() {
				...
				$emailsubject 	= __( 'Test Email with PDF Attachment', 'ultimatewoo-pro' );
				$emailbody 		= __( 'A PDF should be attached to this email to confirm that the PDF is being created and attached correctly', 'ultimatewoo-pro' );
