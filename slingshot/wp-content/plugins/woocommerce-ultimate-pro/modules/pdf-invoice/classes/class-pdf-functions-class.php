<?php
        ...
        class WC_pdf_functions {
			...
			function woocommerce_pdf_admin_css() {
				wp_register_style('woocommerce-pdf-admin-css', ULTIMATEWOO_MODULES_URL . 'pdf-invoice/assets/css/woocommerce-pdf-admin-css.css' );
			...
			function invoice_details_admin_init($post_type,$post) {
				if ( get_post_meta( $post->ID, '_invoice_number_display', TRUE ) ) {
  					add_meta_box( 'woocommerce-invoice-details', __('Invoice Details', 'ultimatewoo-pro'), array($this,'woocommerce_invoice_details_meta_box'), 'shop_order', 'side', 'high');
				}
			}
			...
			function woocommerce_invoice_details_meta_box( $post ) {
				...
				<div class="invoice_details_group">
					<ul class="totals">
                        ...
                        <li class="left">
							<a href="<?php echo $_SERVER['REQUEST_URI'] ?>&pdfid=<?php echo $post->ID ?>"><?php _e( 'Download Invoice', 'ultimatewoo-pro' ); ?></a>
						</li>
			...
			function send_invoice_icon_admin_init( $actions, $order ) {
				...
				if ( get_post_meta( $post->ID, '_invoice_number', TRUE ) ) {
					...
					$actions['sendpdf'] = array(
						...
						'name' 		=> __( 'Send PDF', 'ultimatewoo-pro' ),
						'action' 	=> "icon-sendpdf"
					);
			...
			function download_invoice_icon_admin_init( $actions, $order ) {
				...
				$actions['downloadpdf'] = array(
					...
					'name' 		=> __( 'Download PDF', 'ultimatewoo-pro' ),
			...
			function pdfinvoice_admin_send_pdf() {
				...
				if ( !is_admin() ) die;
				if ( !current_user_can('edit_posts') ) wp_die( __('You do not have sufficient permissions to access this page.', 'ultimatewoo-pro') );
				if ( !check_admin_referer('pdfinvoice-admin-send-pdf')) wp_die( __('You have taken too long. Please go back and retry.', 'ultimatewoo-pro') );
			 ...
			 function my_account_pdf( $actions = NULL, $order = NULL ) {
				...
				if ( get_post_meta( $order_id, '_invoice_number', TRUE ) ) {
				 	...
				 	$actions['pdf'] = array(
						...
						'name' => __( apply_filters('woocommerce_pdf_my_account_button_label', __( 'PDF Invoice', 'ultimatewoo-pro' ) ) )
					);
