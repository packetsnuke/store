...
class WC_Brands_Admin {
	...
	public function __construct() {
		...
		$this->settings_tabs = array(
			'brands' => __( 'Brands', 'ultimatewoo-pro' )
		);
	...
	public function add_coupon_brands_fields () {
		...
		// Brands
		?>
		<p class="form-field"><label for="product_brands"><?php _e( 'Product brands', 'ultimatewoo-pro' ); ?></label>
		<select id="product_brands" name="product_brands[]" style="width: 50%;"  class="wc-enhanced-select" multiple="multiple" data-placeholder="<?php _e( 'Any brand', 'ultimatewoo-pro' ); ?>">
			<?php
			...
			?>
		</select> <img class="help_tip" data-tip='<?php _e( 'A product must be associated with this brand for the coupon to remain valid or, for "Product Discounts", products with these brands will be discounted.', 'ultimatewoo-pro' ); ?>' src="<?php echo WC()->plugin_url(); ?>/assets/images/help.png" height="16" width="16" /></p>
		<?php

		// Exclude Brands
		?>
		<p class="form-field"><label for="exclude_product_brands"><?php _e( 'Exclude brands', 'ultimatewoo-pro' ); ?></label>
		<select id="exclude_product_brands" name="exclude_product_brands[]" style="width: 50%;"  class="wc-enhanced-select" multiple="multiple" data-placeholder="<?php _e( 'No brands', 'ultimatewoo-pro' ); ?>">
			<?php
			...
			?>
		</select> <img class="help_tip" data-tip='<?php _e( 'Product must not be associated with these brands for the coupon to remain valid or, for "Product Discounts", products associated with these brands will not be discounted.', 'ultimatewoo-pro' ) ?>' src="<?php echo WC()->plugin_url(); ?>/assets/images/help.png" height="16" width="16" /></p>
		<?php
	...
	function init_form_fields() {
		...
		$this->settings = apply_filters( 'woocommerce_brands_settings_fields', array(
			...
			array( 'name' => __( 'Brands Archives', 'ultimatewoo-pro' ), 'type' => 'title','desc' => '', 'id' => 'brands_archives' ),

			array(
				'name' => __( 'Show description', 'ultimatewoo-pro' ),
				'desc' => __( 'Choose to show the brand description on the archive page. Turn this off if you intend to use the description widget instead.', 'ultimatewoo-pro' ),
				'tip'  => '',
	...
	function add_thumbnail_field() {
		...
		<div class="form-field">
			...
			<div style="line-height:60px;">
				...
				<button type="button" class="upload_image_button button"><?php _e('Upload/Add image', 'ultimatewoo-pro'); ?></button>
				<button type="button" class="remove_image_button button"><?php _e('Remove image', 'ultimatewoo-pro'); ?></button>
			</div>
				...
				jQuery(function(){
					...
					jQuery(document).on( 'click', '.upload_image_button', function( event ){
						...
						file_frame = wp.media.frames.downloadable_file = wp.media({
							title: '<?php _e( 'Choose an image', 'ultimatewoo-pro' ); ?>',
							button: {
								text: '<?php _e( 'Use image', 'ultimatewoo-pro' ); ?>',
							},
							multiple: false
						});
	...
	function taxonomy_description() {
		echo wpautop( __( 'Brands be added and managed from this screen. You can optionally upload a brand image to display in brand widgets and on brand archives', 'ultimatewoo-pro' ) );
	}
	...
	function columns( $columns ) {
		...
		$new_columns['thumb'] = __('Image', 'ultimatewoo-pro');
		unset( $columns['cb'] );
	...
	public function product_filter( $filters ) {
		...
		$output .= '<option value="" ' .  selected( $current_product_brand, '', false ) . '>' . __( 'Select a brand', 'ultimatewoo-pro' ) . '</option>';
