...
class WC_COG_Admin_Report_Profit_by_Product extends WC_COG_Admin_Report {
	...
	public function get_chart_legend() {
		...
		$legend = array(
			...
			array(
				...
				'title'            => sprintf( __( '%1$s sales for the selected items', 'ultimatewoo-pro' ), '<strong>' . wc_price( $data->total_sales ) . '</strong>' ),
			...
			array(
				...
				'title'            => sprintf( __( '%1$s cost of goods for the selected items', 'ultimatewoo-pro' ), '<strong>' . wc_price( $data->total_cogs ) . '</strong>' ),
			...
			array(
				...
				'title'            => sprintf( __( '%1$s profit for the selected items', 'ultimatewoo-pro' ), '<strong>' . wc_price( $data->total_profit ) . '</strong>' ),
			...
			array(
				...
				'title'            => sprintf( __( '%1$s purchases for the selected items', 'ultimatewoo-pro' ), '<strong>' . $data->total_items . '</strong>' ),
	...
	public function get_chart_widgets() {
		...
		$widgets = array(
			array(
				'title'    => __( 'Product Search', 'ultimatewoo-pro' ),
		...
		if ( ! empty( $this->product_ids ) ) {
			...
			array_unshift( $widgets, array(
				'title'    => __( 'Showing reports for:', 'ultimatewoo-pro' ),
	...
	public function output_current_filters_widget() {
		...
		printf( '<p><strong>%1$s</strong></p><p><a class="button" href="%2$s">%3$s</a></p>', implode( ', ', $product_titles ), esc_url( remove_query_arg( 'product_ids' ) ), __( 'Reset', 'ultimatewoo-pro' ) );
	}
		...
		<div class="section">
			<form method="GET">
				<div>
					...
					<input type="submit" class="submit button" value="<?php esc_attr_e( 'Show', 'ultimatewoo-pro' ); ?>" />
	...
	public function output_profitable_sellers_widget() {
		...
		if ( isset( $_GET['show_least_profitable'] ) ) {
			$order_by = 'ASC';
			$title = esc_html__( 'Least Profitable Sellers', 'ultimatewoo-pro' );
		} else {
			$order_by = 'DESC';
			$title = esc_html__( 'Most Profitable Sellers', 'ultimatewoo-pro' );
		}
				...
				if ( $sellers ) {
					foreach ( $sellers as $product ) {
						...
						<tr class="<?php echo in_array( $product->product_id, $this->product_ids, false ) ? 'active' : ''; ?>">
							<td class="tips" data-tip="<?php /* translators: Placeholders: %1$s - profit margin as a percentage, e.g. 85.4% */ printf( esc_attr__( '%1$s%% profit margin', 'ultimatewoo-pro' ), $profit_margin ); ?>">
								<?php echo wc_price( $product->total_profit ) . ' <small>' . esc_html__( 'total profit', 'ultimatewoo-pro' ) . '</small>'; ?></td>
					...
					echo '<tr><td colspan="3">' . __( 'No products found in range', 'ultimatewoo-pro' ) . '</td></tr>';
				}
				?><tr><td colspan="3" style="text-align: right"><small><?php
					if ( isset( $_GET['show_least_profitable'] ) ) {
						echo '<a href="' . esc_url( remove_query_arg( 'show_least_profitable' ) ) . '">' . esc_html__( 'show most profitable', 'ultimatewoo-pro' ) . '</a>';
					} else {
						echo '<a href="' . esc_url( add_query_arg( array( 'show_least_profitable' => 1 ) ) ) . '">' . esc_html__( 'show least profitable', 'ultimatewoo-pro' ) . '</a>';
					}
				?>
				</small></td></tr>
			</table>
	...
	public function get_main_chart() {
		...
		if ( empty( $this->product_ids ) ) {
			...
			<div class="chart-container">
				<p class="chart-prompt"><?php _e( '&larr; Choose a product to view stats', 'ultimatewoo-pro' ); ?></p>
			</div>
			<?php
			return;
		}
		...
		<script type="text/javascript">
			...
			jQuery(function(){
				...
				var drawGraph = function( highlight ) {
					...
					var series = [
						{
							label     : "<?php echo esc_js( __( 'Number of items sold', 'ultimatewoo-pro' ) ) ?>",
						...
						{
							label     : "<?php echo esc_js( __( 'Sales amount', 'ultimatewoo-pro' ) ) ?>",
						...
						{
							label     : "<?php echo esc_js( __( 'Cost of Goods Sold', 'ultimatewoo-pro' ) ) ?>",
						...
						{
							label     : "<?php echo esc_js( __( 'Profit amount', 'ultimatewoo-pro' ) ) ?>",
