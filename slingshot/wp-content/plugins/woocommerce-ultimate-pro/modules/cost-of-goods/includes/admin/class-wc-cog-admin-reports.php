...
class WC_COG_Admin_Reports {
	...
	public function add_reports( $core_reports ) {
		...
		$profit_reports = array(
			'profit' => array(
				'title'   => __( 'Profit', 'ultimatewoo-pro' ),
				'reports' => array(
					'profit_by_date'     => array(
						'title'       => __( 'Profit by date', 'ultimatewoo-pro' ),
						'description' => '',
						'hide_title'  => true,
						'callback'    => array( $this, 'load_report' )
					),
					'profit_by_product'  => array(
						'title'       => __( 'Profit by product', 'ultimatewoo-pro' ),
						'description' => '',
						'hide_title'  => true,
						'callback'    => array( $this, 'load_report' )
					),
					'profit_by_category' => array(
						'title'       => __( 'Profit by category', 'ultimatewoo-pro' ),
						'description' => '',
						'hide_title'  => true,
						'callback'    => array( $this, 'load_report' )
					),
		...
		$stock_reports = array(
			'product_valuation' => array(
				'title'       => __( 'Product Valuation', 'ultimatewoo-pro' ),
				'description' => '',
				'hide_title'  => false,
				'function'    => array( $this, 'load_report' ),
			),
			'total_valuation' => array(
				'title'       => __( 'Total Valuation', 'ultimatewoo-pro' ),
				'description' => __( 'Total valuation provides the value of all inventory within your store at both the cost of the good, as well as the total value of inventory at the retail price (regular price, or sale price if set). Stock count must be set to be included in this valuation.', 'ultimatewoo-pro' ),
				'hide_title'  => false,
				'function'    => array( $this, 'load_report' ),
			),
		);
		...
		if ( ! empty( $product_ids->posts ) ) {
			...
			foreach ( $product_ids->posts as $product_id ) {
				...
				if ( $product->is_type( 'variation' ) ) {
					...
					$product_data .= ' ' . wp_kses_post( wc_get_formatted_variation( $product, true ) );
					$product_data .= ',' . get_the_title( SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ? $product->get_parent_id() : $product->parent->id );
				...
				$product_data .= ',"' . $total_cost . '"';

				$product_data .= ',' . $product->get_stock_quantity();

				if ( $product->is_in_stock() ) {
					$stock_html = __( 'In stock', 'ultimatewoo-pro' );
				} else {
					$stock_html = __( 'Out of stock', 'ultimatewoo-pro' );
				}

				$product_data .= ',' . esc_html( apply_filters( 'woocommerce_admin_stock_html', $stock_html, $product ) ) . "\n";
			}
		}

		// Remove last newline character if data is available.
