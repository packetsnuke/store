...
class WC_COG_Admin {
	...
	public static function get_global_settings() {
		...
		return apply_filters( 'wc_cog_global_settings', array(
			...
			// section start
			array(
				'name' => __( 'Cost of Goods Options', 'ultimatewoo-pro' ),
				'type' => 'title',
			...
			// include fees
			array(
				'title'         => __( 'Exclude these item(s) from income when calculating profit. ', 'ultimatewoo-pro' ),
				'desc'          => __( 'Fees charged to customer (e.g. Checkout Add-Ons, Payment Gateway Based Fees)', 'ultimatewoo-pro' ),
			...
			array(
				'desc'          => __( 'Shipping charged to customer', 'ultimatewoo-pro' ),
			...
			// include taxes
			array(
				'desc'          => __( 'Tax charged to customer', 'ultimatewoo-pro' ),
	...
	public function render_apply_costs_section( $field ) {
		...
		<tr valign="top">
			<th scope="row" class="titledesc">
				<label for="<?php echo esc_attr( $field['id'] ); ?>"><?php esc_html_e( 'Apply Costs to Previous Orders', 'ultimatewoo-pro' ); ?></label>
				<?php echo wc_help_tip( __( 'This will apply costs to previous orders based on your selection when "Apply Costs" is clicked and cannot be reversed.', 'ultimatewoo-pro' ) ); ?>
			</th>
			<td class="forminp forminp-<?php echo sanitize_html_class( $field['type'] ) ?>">
				<fieldset>
					<ul>
						<li><label><input name="wc_cog_apply_costs_option" id="wc_cog_apply_cost_option" type="radio" checked="checked" /><?php esc_html_e( 'Apply costs to orders that do not have costs set', 'ultimatewoo-pro' );?></label></li>
						<li><label><input name="wc_cog_apply_costs_option" id="wc_cog_apply_costs_option_all" type="radio" /><?php esc_html_e( 'Apply costs to all orders, overriding previous costs.', 'ultimatewoo-pro' );?></label></li>
					</ul>
					<span class="description"><?php esc_html_e( 'Choose carefully, there is no reversing either action!', 'ultimatewoo-pro' ); ?></span>
				</fieldset>
				<fieldset>
					<a href="<?php echo esc_url( add_query_arg( array( 'action' => 'apply_costs' ) ) ); ?>" class="button" id="<?php echo esc_attr( $field['id'] ); ?>"><?php esc_html_e( 'Apply Costs', 'ultimatewoo-pro' ); ?></a>
				</fieldset>
			</td>
		</tr>
		<?php
	...
	public function render_apply_costs_javascript() {
		...
		if ( wc_cog()->is_plugin_settings() ) {
			...
			$confirm_message = __( 'Are you sure you want to apply costs to all previous orders that have not already had costs generated? This cannot be reversed! Note that this can take some time in shops with a large number of orders, if an error occurs, simply Apply Costs again to continue the process.', 'ultimatewoo-pro' );
			$confirm_message_all = __( 'Are you sure you want to apply costs to all previous orders, overriding those with existing costs? This cannot be reversed! Note that this can take some time in shops with a large number of orders, if an error occurs, simply Apply Costs again to continue the process.', 'ultimatewoo-pro' );
			...
			if ( 'apply_costs' === $current_action || 'apply_costs_all' === $current_action ) {
				...
				do {
					...
					if ( is_wp_error( $order_ids ) ) {
						...
						wc_cog()->get_message_handler()->add_error( __( 'Database error while applying product costs.', 'ultimatewoo-pro' ) );
				...
				wc_cog()->get_message_handler()->add_message( sprintf( _n( '%d order updated.', '%d orders updated.', $success_count, 'ultimatewoo-pro' ), $success_count ) );
