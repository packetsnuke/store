...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.6.5', __( 'WooCommerce Costs of Goods', 'ultimatewoo-pro' ), __FILE__, 'init_woocommerce_cost_of_goods', array(
...
class WC_COG extends SV_WC_Plugin {
	...
	public function get_plugin_name() {
		return __( 'WooCommerce Cost of Goods', 'ultimatewoo-pro' );
	}
		...
		if ( version_compare( $installed_version, '1.1', '<' ) ) {
			...
			do {
				...
				if ( is_wp_error( $product_ids ) ) {
					...
					wp_die( sprintf( __( 'Error upgrading <strong>WooCommerce Cost of Goods</strong>: %s', 'ultimatewoo-pro' ), '<ul><li>' . implode( '</li><li>', $product_ids->get_error_messages() ) . '</li></ul>' ) .
						'<a href="' . admin_url( 'plugins.php' ) . '">' . __( '&laquo; Go Back', 'ultimatewoo-pro' ) . '</a>' );
				}
