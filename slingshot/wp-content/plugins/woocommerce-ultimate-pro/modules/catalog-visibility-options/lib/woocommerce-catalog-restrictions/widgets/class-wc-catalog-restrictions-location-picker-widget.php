...
class WC_Catalog_Restrictions_Location_Picker_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$this->woo_widget_description = __( 'Display a location picker and displays the users current location in the sidebar.', 'ultimatewoo-pro' );
		$this->woo_widget_idbase = 'woocommerce_location_picker';
		$this->woo_widget_name = __( 'WooCommerce Location Picker', 'ultimatewoo-pro' );
	...
	function widget( $args, $instance ) {
		...
		$location_set_title = (!empty( $instance['location_set_title'] )) ? $instance['location_set_title'] : __( 'Current location is %s', 'ultimatewoo-pro' );
		$location_not_set_title = (!empty( $instance['location_not_set_title'] )) ? $instance['location_not_set_title'] : __( 'Choose your location', 'ultimatewoo-pro' );
		...
		<form method="post">
			...
			<p><input type="submit" class="submitbutton" name="wp-submit" id="wp-submit" value="<?php _e( 'Go &rarr;', 'ultimatewoo-pro' ); ?>" /> </p>
			<div>
	...
	function form( $instance ) {
		...
		<p><label for="<?php echo $this->get_field_id( 'location_not_set_title' ); ?>"><?php _e( 'Location not set title:', 'woocommerce' ) ?></label>
			...
			if ( isset( $instance['location_not_set_title'] ) )
				...
				echo __( 'Choose your location', 'ultimatewoo-pro' );
			?>" /></p>
		...
		<p><label for="<?php echo $this->get_field_id( 'location_set_title' ); ?>"><?php _e( 'Logged set title:', 'woocommerce' ) ?></label>
			<input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'location_set_title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'location_set_title' ) ); ?>" value="<?php
			       if ( isset( $instance['location_set_title'] ) )
				       ...
				       echo __( 'Current location is %s', 'ultimatewoo-pro' );
			       ?>" /></p>
