...
class WC_Catalog_Restrictions_Product_Admin {
	...
	function add_post_meta_boxes() {
		...
		add_meta_box(
			...
			esc_html__( 'Product Restrictions', 'ultimatewoo-pro' ), // Title
		...
		<div class="">
			...
			<?php if ( $wc_catalog_restrictions->get_setting( '_wc_restrictions_locations_enabled', 'no' ) == 'yes' ) : ?>
				<div class="options_group">
					<?php
					woocommerce_wp_select( array('id' => '_wc_restrictions_location', 'label' => __( 'What locations is this product enabled for?', 'ultimatewoo-pro' ), 'options' => array(
						'inherit' => __( 'Use Category Settings', 'ultimatewoo-pro' ),
						'public' => __( 'All Locations', 'ultimatewoo-pro' ),
						'restricted' => __( 'Specific Locations', 'ultimatewoo-pro' )
					...
					<div id="wc_catalog_restrictions_locations_container" class="wc_restrictions_options_panel" style="<?php echo (get_post_meta( $object->ID, '_wc_restrictions_location', true ) == 'restricted' ? 'display:block;' : 'display:none;'); ?>">
						...
						?>
						<label for="wc_restrictions_locations"><?php _e( 'Target Locations', 'ultimatewoo-pro' ); ?></label><br />
						<select name="wc_restrictions_locations[]" class="" multiple="multiple" data-placeholder="<?php _e( 'Search for a country&hellip;', 'ultimatewoo-pro' ); ?>">
							<?php woocommerce_catalog_restrictions_country_multiselect_options( $current_locations ); ?>
							?>
						</select> <img style="width:16px;height:16px;" class="help_tip" data-tip='<?php _e( 'Choose locations for this product.  Only users who select a matching location will be able to view and purchase this product.', 'ultimatewoo-pro' ) ?>' src="<?php echo $woocommerce->plugin_url(); ?>/assets/images/help.png" />
					</div>
				</div>
			<?php endif; ?>
			...
			<?php
			woocommerce_wp_select( array('id' => '_wc_restrictions', 'label' => __( 'Who can view this product', 'ultimatewoo-pro' ), 'options' => array(
				'inherit' => __( 'Use Category Settings', 'ultimatewoo-pro' ),
				'public' => __( 'Everyone', 'ultimatewoo-pro' ),
				'restricted' => __( 'Specific Roles', 'ultimatewoo-pro' )
			    ), 'std' => 'inherit', 'desc_tip' => true, 'description' => __( 'If you would like to only show this product to users who are in certian roles select "Specific Roles"', 'ultimatewoo-pro' )
			) );
			?>
			...
			<div id="wc_catalog_restrictions_roles_container" class="wc_restrictions_options_panel" style="<?php echo (get_post_meta( $object->ID, '_wc_restrictions', true ) == 'restricted' ? 'display:block;' : 'display:none;'); ?>">
				<p><?php _e( "Choose the roles that can view this product", 'ultimatewoo-pro' ); ?></p>
				<table style="">
			...
			<?php
			woocommerce_wp_select( array('id' => '_wc_restrictions_purchase', 'label' => __( 'Who can purchase this product', 'ultimatewoo-pro' ), 'options' => array(
				'inherit' => __( 'Use Category Settings', 'ultimatewoo-pro' ),
				'public' => __( 'Everyone', 'ultimatewoo-pro' ),
				'restricted' => __( 'Specific Roles', 'ultimatewoo-pro' )
			...
			<div id="wc_catalog_restrictions_purchase_roles_container" class="wc_restrictions_options_panel" style="<?php echo (get_post_meta( $object->ID, '_wc_restrictions_purchase', true ) == 'restricted' ? 'display:block;' : 'display:none;'); ?>">
				<label><?php _e( "Choose the roles that can purchase this product", 'ultimatewoo-pro' ); ?></label><br />
				<div class="clearfix"></div>
				<?php $chunks = array_chunk( $all_roles, ceil( count( $all_roles ) / 3 ), true ); ?>
				<?php foreach ( $chunks as $chunk ) : ?>
			...
			<?php
			woocommerce_wp_select( array('id' => '_wc_restrictions_price', 'label' => __( 'Who can view prices', 'ultimatewoo-pro' ), 'options' => array(
				'inherit' => __( 'Use Category Settings', 'ultimatewoo-pro' ),
				'public' => __( 'Everyone', 'ultimatewoo-pro' ),
				'restricted' => __( 'Specific Roles', 'ultimatewoo-pro' )
			    ), 'std' => 'inherit', 'desc_tip' => true, 'description' => __( 'If you would like to only specific users who are in certian roles select "Specific Roles".  Select "Everyone" to override category settings.' )
			...
			<div id="wc_catalog_restrictions_prices_roles_container" class="wc_restrictions_options_panel" style="<?php echo (get_post_meta( $object->ID, '_wc_restrictions_price', true ) == 'restricted' ? 'display:block;' : 'display:none;'); ?>">
				<label><?php _e( "Choose the roles that can view this products price", 'ultimatewoo-pro' ); ?></label><br />
				<div class="clearfix"></div>
				<?php $chunks = array_chunk( $all_roles, ceil( count( $all_roles ) / 3 ), true ); ?>
				<?php foreach ( $chunks as $chunk ) : ?>
					<ul class="list-column">        
