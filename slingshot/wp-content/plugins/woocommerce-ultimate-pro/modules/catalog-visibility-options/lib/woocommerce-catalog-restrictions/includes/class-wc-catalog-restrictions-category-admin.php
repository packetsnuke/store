...
class WC_Catalog_Restrictions_Category_Admin {
	...
	function add_category_fields() {
		...
		<div id="wc_catalog_restrictions" class="form-field">
			...
			<label><?php _e( 'Role Restrictions', 'ultimatewoo-pro' ); ?></label>

			<label for="_wc_restrictions"><?php _e( 'Which customer roles can view and purchase products in this category?', 'ultimatewoo-pro' ); ?></label>
			<select name="_wc_restrictions" id="_wc_restrictions">
				<option value="no-restriction-setting" <?php selected( $restricted, 'no-restriction-setting' ); ?>><?php _e( 'Not Configured', 'ultimatewoo-pro' ); ?></option>
				<option value="public" <?php selected( $restricted, 'public' ); ?>><?php _e( 'Everyone', 'ultimatewoo-pro' ); ?></option>
				<option value="restricted" <?php selected( $restricted, 'restricted' ); ?>><?php _e( 'Specific Roles', 'ultimatewoo-pro' ); ?></option>
			</select>

			<div id="wc_catalog_restrictions_roles_container" style="<?php echo ($restricted == 'restricted' ? 'display:block;' : 'display:none;'); ?>">
				<p class="title"><?php _e( "Choose the roles that can view this product", 'ultimatewoo-pro' ); ?></p>
		...
		<div id="wc_catalog_restrictions_location" class="form-field">
			<?php if ( $wc_catalog_restrictions->get_setting( '_wc_restrictions_locations_enabled', 'no' ) == 'yes' ) : ?>
				...
				<label for="_wc_restrictions_location"><?php _e( 'What locations should this category be enabled for?', 'ultimatewoo-pro' ); ?></label>

				<select name="_wc_restrictions_location" id="_wc_restrictions_location">
					<option value="no-restriction-setting" <?php selected( $location_restriction, 'no-restriction-setting' ); ?>><?php _e( 'Not Configured', 'ultimatewoo-pro' ); ?></option>
					<option value="public" <?php selected( $location_restriction, 'public' ); ?>><?php _e( 'All Locations', 'ultimatewoo-pro' ); ?></option>
					<option value="restricted" <?php selected( $location_restriction, 'restricted' ); ?>><?php _e( 'Specific Locations', 'ultimatewoo-pro' ); ?></option>
				</select>
				...
				<div id="wc_catalog_restrictions_locations_container" class="woocommerce_options_panel"  style="<?php echo ($location_restriction == 'restricted' ? 'display:block;' : 'display:none;'); ?>">
					<p class="form-field"><label for="wc_restrictions_locations"><?php _e( 'Target Locations', 'ultimatewoo-pro' ); ?></label>
						...
						</select> <img style="width:16px;height:16px;" class="help_tip" data-tip='<?php _e( 'Choose locations for this category.  Only users who select a matching location will be able to view and purchase products in this category.', 'ultimatewoo-pro' ) ?>' src="<?php echo $woocommerce->plugin_url(); ?>/assets/images/help.png" /></p>
				</div>
			<?php endif; ?>
		</div>
		<?php
	...
	function edit_category_fields( $term, $taxonomy ) {
		...
		?>
		<tr class="form-field">
			<th scope="row" valign="top"><label><?php _e( 'Role Visibility Rules', 'ultimatewoo-pro' ); ?></label></th>
			<td>
				<div id="wc_catalog_restrictions" class="form-field">
					<label for="_wc_restrictions"><?php _e( 'Choose the type of visibility rule to use with products in this category.', 'ultimatewoo-pro' ); ?></label><br />
					<select name="_wc_restrictions" id="_wc_restrictions">
						<option value="no-restriction-setting"><?php _e( 'None', 'ultimatewoo-pro' ); ?></option>
						<option value="restricted" <?php selected( $restricted, 'restricted' ); ?>><?php _e( 'Show to Specific Roles', 'ultimatewoo-pro' ); ?></option>
						<option value="public" <?php selected( $restricted, 'public' ); ?>><?php _e( 'Show to Everyone', 'ultimatewoo-pro' ); ?></option>
					</select>
		...
		<?php if ( $wc_catalog_restrictions->get_setting( '_wc_restrictions_locations_enabled', 'no' ) == 'yes' ) : ?>
			<tr class="form-field">
				<th scope="row" valign="top"><label><?php _e( 'Location Visibility Rules', 'ultimatewoo-pro' ); ?></label></th>
				<td>
					<div id="wc_catalog_restrictions_location" class="form-field">
						...
						<label for="_wc_restrictions_location"><?php _e( 'Choose the type of location rule to use for products in this category.', 'ultimatewoo-pro' ); ?></label>
						<br />
						<select name="_wc_restrictions_location" id="_wc_restrictions_location">
							<option value="no-restriction-setting"><?php _e( 'None', 'ultimatewoo-pro' ); ?></option>
							<option value="restricted" <?php selected( $location_restriction, 'restricted' ); ?>><?php _e( 'Specific Locations', 'ultimatewoo-pro' ); ?></option>
							<option value="public" <?php selected( $location_restriction, 'public' ); ?>><?php _e( 'Any Location', 'ultimatewoo-pro' ); ?></option>
						</select>
	...
	function cat_columns( $columns ) {
		...
		$new_columns['restrictions'] = __( 'Restrictions', 'ultimatewoo-pro' );
			...
			if ( !$restricted ) {
				...
				if ( count( $current_restrictions ) == 1 ) {
					if ( !empty( $current_restrictions[0] ) ) {
						$columns .= sprintf( __( '%s role has access', 'ultimatewoo-pro' ), count( $current_restrictions ) );
					} else {
						$columns .= __( 'No one has access', 'ultimatewoo-pro' );
					}
				} else {
					$columns .= sprintf( __( '%s roles have access', 'ultimatewoo-pro' ), count( $current_restrictions ) );
				}
			}
		}
		return $columns;
