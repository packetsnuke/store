...
class WC_Price_Calculator_Product {
	...
	public static function get_area_measurement( $product ) {
		...
		if ( is_numeric( $length ) && is_numeric( $width ) ) {
			...
			$measurement = new WC_Price_Calculator_Measurement( $unit, $area, 'area', __( 'Area', 'ultimatewoo-pro' ) );
		...
		if ( ! empty( $area ) ) {
			$measurement = new WC_Price_Calculator_Measurement( get_option( 'woocommerce_area_unit' ), $area, 'area', __( 'Area', 'ultimatewoo-pro' ) );
		}

		// if no measurement, just create a default empty one
		if ( ! $measurement ) {
			$measurement = new WC_Price_Calculator_Measurement( get_option( 'woocommerce_area_unit' ), 0, 'area', __( 'Area', 'ultimatewoo-pro' ) );
		}
	...
	public static function get_perimeter_measurement( $product ) {
		...
		if ( is_numeric( $length ) && is_numeric( $width ) ) {
			...
			$measurement = new WC_Price_Calculator_Measurement( get_option( 'woocommerce_dimension_unit' ), $perimeter, 'length', __( 'Perimeter', 'ultimatewoo-pro' ) );
		}

		// if no measurement, just create a default empty one
		if ( ! $measurement ) {
			$measurement = new WC_Price_Calculator_Measurement( get_option( 'woocommerce_dimension_unit' ), 0, 'length', __( 'Perimeter', 'ultimatewoo-pro' ) );
		}
	...
	public static function get_surface_area_measurement( $product ) {
		...
		if ( is_numeric( $length ) && is_numeric( $width ) && is_numeric( $height ) ) {
			...
			$measurement  = new WC_Price_Calculator_Measurement( get_option( 'woocommerce_dimension_unit' ), $surface_area, 'area', __( 'Surface Area', 'ultimatewoo-pro' ) );
		}

		// if no measurement, just create a default empty one
		if ( ! $measurement ) {
			$measurement = new WC_Price_Calculator_Measurement( get_option( 'woocommerce_dimension_unit' ), 0, 'area', __( 'Surface Area', 'ultimatewoo-pro' ) );
		}
	...
	public static function get_volume_measurement( $product ) {
		...
		if ( is_numeric( $length ) && is_numeric( $width ) && is_numeric( $height ) ) {
			...
			$measurement = new WC_Price_Calculator_Measurement( $unit, $volume, 'volume', __( 'Volume', 'ultimatewoo-pro' ) );
		...
		if ( ! empty( $area ) && is_numeric( $height ) ) {
			...
			$measurement = new WC_Price_Calculator_Measurement( $volume_unit, $volume, 'volume', __( 'Volume', 'ultimatewoo-pro' ) );
		...
		if ( ! empty( $volume ) ) {
			$measurement = new WC_Price_Calculator_Measurement( get_option( 'woocommerce_volume_unit' ), $volume, 'volume', __( 'Volume', 'ultimatewoo-pro' ) );
		}

		// if no measurement, just create a default empty one
		if ( ! $measurement ) {
			$measurement = new WC_Price_Calculator_Measurement( get_option( 'woocommerce_volume_unit' ), 0, 'volume', __( 'Volume', 'ultimatewoo-pro' ) );
		}
	...
	public static function get_weight_measurement( $product ) {
		...
		return new WC_Price_Calculator_Measurement( get_option( 'woocommerce_weight_unit' ), $weight, 'weight', __( 'Weight', 'ultimatewoo-pro' ) );
	}
	...
	public static function get_pricing_rules_price_html( $product ) {
		...
		$pricing_label      = $sep . ' ' . __( $settings->get_pricing_label(), 'ultimatewoo-pro' );
