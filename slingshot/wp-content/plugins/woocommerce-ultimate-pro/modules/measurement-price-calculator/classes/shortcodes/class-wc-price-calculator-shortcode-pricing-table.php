...
class WC_Price_Calculator_Shortcode_Pricing_Table {
	...
	public static function get_pricing_rules_table( $rules, $settings ) {
		...
		$html .= '<thead><tr><th>' . sprintf( __( 'Range (%s)', 'ultimatewoo-pro' ), '<span class="units">' . __( $settings->get_pricing_label(), 'ultimatewoo-pro' ) . '</span>' ) . '</th>';
		/* translators: Placeholders: %s - currency symbol */
		$html .= '<th>' . sprintf( __( 'Price (%s)', 'ultimatewoo-pro' ),  '<span class="units">' . get_woocommerce_currency_symbol() . '/' . __( $settings->get_pricing_label(), 'ultimatewoo-pro' ) . '</span>' ) . '</th></tr></thead>';
		$html .= '<tbody>';
		foreach ( $rules as $rule ) {
			...
			$range .= ' ' . __( $settings->get_pricing_label(), 'ultimatewoo-pro' );
