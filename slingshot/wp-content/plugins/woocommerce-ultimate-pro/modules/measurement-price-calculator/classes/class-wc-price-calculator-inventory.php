				...
				if ( $total_stock > 0 ) {
					...
					switch ( $format_option ) {
						...
						case 'low_amount' :
							...
							$format = ( $total_stock <= $low_amount ) ? __( 'Only %s %s left in stock', 'ultimatewoo-pro' ) : __( 'In stock', 'ultimatewoo-pro' );
						break;
						default :
							$format = __( '%s %s in stock', 'ultimatewoo-pro' );
						break;
					}
					...
					if ( $product->backorders_allowed() && $product->backorders_require_notification() ) {
						$availability .= ' ' . __( '(backorders allowed)', 'ultimatewoo-pro' );
					}
		...
		if ( empty( $item ) ) {
			...
			if ( 0 === $regular_items ) {
				...
				if ( count( array_count_values( $pricing_units ) ) > 1 ) {
					...
					$backordered_text = __( 'Backordered measure', 'ultimatewoo-pro' );
				} else {
				...
				$backordered_text = __( 'Backordered quantity or measure', 'ultimatewoo-pro' );
			}
