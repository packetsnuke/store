	...
	public function get_pricing_rule_price_html( $rule ) {
		...
		if ( $rule['price'] > 0 ) {
			...
			if ( '' !== $rule['sale_price'] && '' !== ( $rule['regular_price'] ) ) {
				$price_html .= WC_Price_Calculator_Product::get_price_html_from_to( $rule['regular_price'], $rule['price'], $sep . ' ' . __( $this->get_pricing_label(), 'ultimatewoo-pro' ) );
			} else {
				$price_html .= wc_price( $rule['price'] ) . ' ' . $sep . ' ' . __( $this->get_pricing_label(), 'ultimatewoo-pro' );
			}
			...
			if ( $rule['price'] === $rule['sale_price'] && '' !== $rule['regular_price'] ) {
				$price_html .= WC_Price_Calculator_Product::get_price_html_from_to( $rule['regular_price'], __( 'Free!', 'ultimatewoo-pro' ), __( $this->get_pricing_label(), 'ultimatewoo-pro' ) );
			} else {
