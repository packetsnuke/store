...
class WC_Price_Calculator_Cart {
		...
		if ( $valid && WC_Price_Calculator_Product::pricing_calculator_enabled( $product ) ) {
			...
			foreach ( $measurements as $measurement ) {
				...
				if ( ! $value || ! is_numeric( $value ) || $value <= 0 ) {
					...
					wc_add_notice( sprintf( __( '%s missing.', 'ultimatewoo-pro' ), $measurement->get_label() ), 'error' );
					$valid = false;
				...
				if ( $input_minimum && $value < $input_attributes['min'] ) {
					...
					wc_add_notice( sprintf( __( '%1$s value must be greater than or equal to %2$s.', 'ultimatewoo-pro' ), $measurement->get_label(), $input_attributes['min'] ), 'error' );
					$valid = false;
				...
				if ( $input_maximum && $value > $input_attributes['max'] ) {
					...
					wc_add_notice( sprintf( __( '%1$s value must be less than or equal to %2$s.', 'ultimatewoo-pro' ), $measurement->get_label(), $input_attributes['max'] ), 'error' );
					$valid = false;
				...
				if ( $input_increment && $input_minimum && $input_maximum ) {
					...
					if ( ! in_array( $value, $values_range ) ) {
						...
						$message = sprintf( __( '%1$s must be between %2$s and %3$s in increments of %4$s.', 'ultimatewoo-pro' ),
			...
			if ( $measurement_needed_value && $measurement_needed_value_unit && $settings->pricing_rules_enabled() ) {
				...
				if ( null === $settings->get_pricing_rules_price( $measurement_needed ) ) {
					...
					$message = apply_filters( 'wc_measurement_price_calculator_no_price_available_notice_text', __( "No price available for a product with this measurement, please contact the store for assistance.", 'ultimatewoo-pro' ), $product );
	...
	private function humanize_cart_item_data( $item, $cart_item_data ) {
		...
		foreach ( $calculator_measurements as $measurement ) {
			...
			if ( isset( $cart_item_data[ $measurement_name ] ) ) {
				...
				$label = $measurement->get_unit_label() ?
					sprintf( '%1$s (%2$s)', $measurement->get_label(), __( $measurement->get_unit_label(), 'ultimatewoo-pro' ) ) :
					__( $measurement->get_label(), 'ultimatewoo-pro' );
		...
		if ( isset( $cart_item_data['_measurement_needed_overage'] ) && 1 === count( $calculator_measurements ) ) {
			...
			$new_cart_item_data[ sprintf( __( 'Overage Estimate (%s)', 'ultimatewoo-pro' ), $measurement->get_unit_label() ) ] = $cart_item_data['_measurement_needed_overage'];
		}
		...
		if (     isset( $cart_item_data['_measurement_needed'] )
			...
			$total_amount_text = apply_filters(
				...
				$product_measurement->get_unit_label() ?
					...
					sprintf( __( 'Total %1$s (%2$s)', 'ultimatewoo-pro' ), $product_measurement->get_label(), __( $product_measurement->get_unit_label(), 'ultimatewoo-pro' ) ) :
					/* translators: Placeholders: %s - measurement label */
					sprintf( __( 'Total %s', 'ultimatewoo-pro' ), $product_measurement->get_label() ),
				$item
			);
			...
			if ( isset( $cart_item_data['_measurement_needed_overage'] ) ) {
				$overage_amount_text = apply_filters(
					...
					$product_measurement->get_unit_label() ?
						/* translators: Placeholders: %1$s - measurement label, %2$s - measurement unit label */
						sprintf( __( 'Overage %1$s (%2$s)', 'ultimatewoo-pro' ), $product_measurement->get_label(), __( $product_measurement->get_unit_label(), 'ultimatewoo-pro' ) ) :
						/* translators: Placeholders: %s - measurement label */
						sprintf( __( 'Overage %s', 'ultimatewoo-pro' ), $product_measurement->get_label() ),
					$item
				);
		...
		if ( isset( $cart_item_data['_price_overage'], $cart_item_data['_overage_percentage'] ) ) {
			$new_cart_item_data[ sprintf( __( 'Overage Cost (%s%%)', 'ultimatewoo-pro' ), $cart_item_data['_overage_percentage'] * 100 ) ] = wc_price( $cart_item_data['_price_overage'] );
		}
