...
function wc_tab_manager_meta_boxes_save( $post_id, $post, $is_update ) {
	...
	if ( isset( $_POST['tab_type'] ) && 'global' === $_POST['tab_type'] ) {
		...
		if ( isset( $_POST['_include_in_search'] ) && 'yes' === $_POST['_include_in_search'] ) {
			...
			$message = sprintf( __( 'Please %1$srun the tab search update%2$s to ensure the new tab content is indexed and searchable.', 'ultimatewoo-pro' ),
...
function wc_tab_manager_meta_boxes() {
	...
	add_meta_box( 'wc-tab-manager-product-tab-actions', __( 'Tab Actions', 'ultimatewoo-pro' ), 'wc_tab_manager_product_tab_actions_meta_box', 'wc_product_tab', 'side', 'high' );
}
