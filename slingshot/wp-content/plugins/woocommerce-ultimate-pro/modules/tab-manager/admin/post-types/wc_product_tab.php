...
function wc_tab_manager_edit_product_tab_columns( $columns ) {
	...
	$is_searchable_text = esc_html__( 'Searchable?', 'ultimatewoo-pro' );
	$is_searchable_html = '<span class="dashicons dashicons-search" title="' . $is_searchable_text . '" aria-label="' . $is_searchable_text . '"></span>';

	$columns = array(
		'cb'             => '<input type="checkbox" />',
		'name'           => esc_html__( 'Name', 'ultimatewoo-pro' ),
		'type'           => esc_html__( 'Tab Type', 'ultimatewoo-pro' ),
		'parent-product' => esc_html__( 'Parent Product', 'ultimatewoo-pro' ),
		'product-cat'    => esc_html__( 'Categories', 'ultimatewoo-pro' ),
...
function wc_tab_manager_custom_product_tab_columns( $column ) {
	...
	switch ( $column ) {
		...
		case 'name':
			...
			echo '<a class="row-title" href="' . esc_url( $edit_link ) . '">' . esc_html__( _draft_or_post_title(), 'ultimatewoo-pro' ) . '</a>';

			// Display post states a little more selectively than `_post_states( $post )`.
			if ( 'draft' === $post->post_status ) {
				echo ' <span class="post-state">(' . esc_html__( 'Draft', 'ultimatewoo-pro' ) . ')</span>';
			}
			...
			if ( current_user_can( $post_type_object->cap->delete_post, $post->ID ) ) {
				if ( 'trash' === $post->post_status ) {
					...
					$actions['untrash'] = '<a title="' . esc_attr__( 'Restore this item from the Trash', 'ultimatewoo-pro' ) . '" href="' . esc_url( $untrash_link ) . '">' . esc_html__( 'Restore', 'ultimatewoo-pro' ) . '</a>';
				} else {
					if ( defined( 'EMPTY_TRASH_DAYS' ) && EMPTY_TRASH_DAYS ) {
						$trash_link       = get_delete_post_link( $post->ID );
						$actions['trash'] = '<a class="submitdelete" title="' . esc_attr__( 'Move this item to the Trash', 'ultimatewoo-pro' ) . '" href="' . esc_url( $trash_link ) . '">' . esc_html__( 'Trash', 'ultimatewoo-pro' ) . '</a>';
					} else {
						$delete_link       = get_delete_post_link( $post->ID, '', true );
						$actions['delete'] = '<a class="submitdelete" title="' . esc_attr__( 'Delete this item permanently', 'ultimatewoo-pro' ) . '" href="' . esc_url( $delete_link ) . '">' . esc_html__( 'Delete Permanently', 'ultimatewoo-pro' ) . '</a>';
					}
				}
			}
		...
		case 'type':
			...
			if ( $post->post_parent ) {
				esc_html_e( 'Product', 'ultimatewoo-pro' );
			} else {
				esc_html_e( 'Global', 'ultimatewoo-pro' );
			}
		...
		case 'parent-product':
			...
			if ( $post->post_parent ) {
				...
				echo '<em>' . esc_html__( 'N/A', 'ultimatewoo-pro' ) . '</em>';
			}
...
function wc_tab_manager_product_tabs_by_type() {
	...
	if ( 'wc_product_tab' === $typenow ) :
		...
		<select name="product_tab_type" id="dropdown_product_tab_type">
			<option value="">
				<?php esc_html_e( 'Show all Tabs', 'ultimatewoo-pro' ); ?>
			</option>
			<option value="product" <?php selected( 'product', $product_tab_type ); ?>>
				<?php esc_html_e( 'Show product tabs', 'ultimatewoo-pro' ); ?>
			</option>
			<option value="global" <?php selected( 'global', $product_tab_type ); ?>>
				<?php esc_html_e( 'Show global tabs', 'ultimatewoo-pro' ); ?>
			</option>
		</select>
		<?php
	endif;
