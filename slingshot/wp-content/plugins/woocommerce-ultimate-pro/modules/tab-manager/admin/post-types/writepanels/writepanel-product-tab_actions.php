...
function wc_tab_manager_product_tab_actions_meta_box( $post ) {
	...
	<ul class="wc_product_tab_actions">
		...
		<li class="wide">
			<input type="submit" class="button button-primary tips" name="publish" value="<?php esc_attr_e( 'Save Tab', 'ultimatewoo-pro' ); ?>" data-tip="<?php esc_attr_e( 'Save/update the tab', 'ultimatewoo-pro' ); ?>" />
		</li>
		...
		<?php if ( current_user_can( 'delete_post', $post->ID ) ) : ?>
			<li class="wide">
				<?php
				if ( ! EMPTY_TRASH_DAYS ) {
					$delete_text = __( 'Delete Permanently', 'ultimatewoo-pro' );
				} else {
					$delete_text = __( 'Move to Trash', 'ultimatewoo-pro' );
				}
				?>
