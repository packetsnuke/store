	...
	<div id="woocommerce_product_tabs" class="panel wc-metaboxes-wrapper">
		<p class="toolbar">
			<?php if ( 'product' === $typenow ) :
				...
				?>
				<label for="_override_tab_layout"><?php esc_html_e( 'Override default tab layout:', 'ultimatewoo-pro' ); ?></label> <input type="checkbox" name="_override_tab_layout" id="_override_tab_layout" <?php checked( $override_tab_layout, "yes" ); ?> />
			<?php endif; ?>
			<a href="#" class="close_all"><?php esc_html_e( 'Close all', 'ultimatewoo-pro' ); ?></a> / <a href="#" class="expand_all"><?php esc_html_e( 'Expand all', 'ultimatewoo-pro' ); ?></a>
		</p>
		...
		<div class="woocommerce_product_tabs wc-metaboxes">
			...
			<?php
				...
				foreach ( $posts as $post_obj ) {
					...
					$tab['content'] .= ' <a href="' . get_edit_post_link( $post_obj->ID ) . '">' . __( 'Edit Global Tab Content', 'ultimatewoo-pro' ) . '</a>';
				...
				foreach ( $combined_tabs as $id => $tab ) {
					...
					<div class="woocommerce_product_tab wc-metabox <?php echo sanitize_html_class( 'product_tab_' . $tab['type'] ); ?> <?php echo sanitize_html_class( $id ); ?>" rel="<?php echo esc_attr( $position ); ?>" <?php if ( ! $active ) echo ' style="display:none;"'; ?>>
						<h3>
							<button type="button" class="remove_row button"><?php esc_html_e( 'Remove', 'ultimatewoo-pro' ); ?></button>
							<div class="handlediv" title="<?php esc_attr_e( 'Click to toggle', 'ultimatewoo-pro' ); ?>"></div>
							...
							<tr>
								<td>
									...
									<?php if ( 'third_party' === $tab['type'] ) : ?>
										<p><em><?php echo esc_html( $tab['description'] ? $tab['description'] : __( 'The title/content for this tab will be provided by a third party plugin', 'ultimatewoo-pro' ) ); ?></em></p>
									<?php endif; ?>
									<div class="options_group">
										<?php if ( 'third_party' !== $tab['type'] ) : ?>
											<p class="form-field product_tab_title_field">
												<label for="product_tab_title_<?php echo $i; ?>"><?php esc_html_e( 'Title', 'ultimatewoo-pro' ); ?></label>
												<?php if ( 'global' === $tab['type'] ) : ?>
													<span><?php echo esc_html( $tab['title'] ); ?></span>
													<input type="hidden" name="product_tab_title[<?php echo $i; ?>]" value="<?php echo esc_attr( $tab['title'] ); ?>" />
												<?php else: ?>
													<input type="text" value="<?php echo esc_attr( $tab['title'] ); ?>" id="product_tab_title_<?php echo $i; ?>" name="product_tab_title[<?php echo $i; ?>]" class="short product_tab_title"> <span class="description"><?php esc_html_e( "The tab title, this appears in the tab", 'ultimatewoo-pro' ); ?></span>
												<?php endif; ?>
											</p>
										<?php endif; ?>
										<?php if ( isset( $core_tabs[ $id ]['heading'] ) && $core_tabs[ $id ]['heading'] ) : ?>
											<p class="form-field product_tab_heading_field">
												<label for="product_tab_heading_<?php echo $i; ?>"><?php esc_html_e( 'Heading', 'ultimatewoo-pro' ); ?></label>
												<input type="text" value="<?php echo esc_attr( $tab['heading'] ); ?>" id="product_tab_heading_<?php echo $i; ?>" name="product_tab_heading[<?php echo $i; ?>]" class="short"> <span class="description"><?php esc_html_e( "The tab heading, this appears just before the tab content", 'ultimatewoo-pro' ); ?></span>
											</p>
										<?php endif; ?>
										<?php if ( 'global' === $tab['type'] ) : ?>
											<p class="form-field product_tab_heading_field">
												<label for="product_tab_content_<?php echo $i; ?>"><?php esc_html_e( 'Content', 'ultimatewoo-pro' ); ?></label>
		...
		<p class="toolbar">
			<button type="button" class="button button-primary add_product_tab"><?php esc_html_e( 'Add', 'ultimatewoo-pro' ); ?></button>
			<select name="product_tab" class="product_tab">
				<?php if ( 'product' === $typenow ) : ?>
					<option value=""><?php esc_html_e( 'Custom Tab', 'ultimatewoo-pro' ); ?></option>
				<?php endif; ?>
				<?php
