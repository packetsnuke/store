...
function wc_tab_manager_admin_enqueue_scripts() {
	...
	if ( $is_default_layout_page || $is_custom_layout_page ) {
		...
		wp_localize_script( 'wc_tab_manager_admin', 'wc_tab_manager_admin_params', array(
			'remove_product_tab' => __( 'Remove this product tab?', 'ultimatewoo-pro' ),
			'remove_label'       => __( 'Remove', 'ultimatewoo-pro' ),
			'click_to_toggle'    => __( 'Click to toggle', 'ultimatewoo-pro' ),
			'title_label'        => __( 'Title', 'ultimatewoo-pro' ),
			'title_description'  => __( 'The tab title, this appears in the tab', 'ultimatewoo-pro' ),
			'content_label'      => __( 'Content', 'ultimatewoo-pro' ),
			'ajax_url'           => admin_url( 'admin-ajax.php' ),
...
function wc_tab_manager_product_tab_updated_messages( $messages ) {
	...
	$messages['wc_product_tab'] = array(
		...
		1 => __( 'Tab updated.', 'ultimatewoo-pro' ),
		2 => __( 'Custom field updated.', 'ultimatewoo-pro' ),
		3 => __( 'Custom field deleted.', 'ultimatewoo-pro' ),
		4 => __( 'Tab updated.', 'ultimatewoo-pro' ),
		5 => isset( $_GET['revision'] ) ? sprintf( __( 'Tab restored to revision from %s', 'ultimatewoo-pro' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
		6 => __( 'Tab updated.', 'ultimatewoo-pro' ),
		7 => __( 'Tab saved.', 'ultimatewoo-pro' ),
		8 => __( 'Tab submitted.', 'ultimatewoo-pro' ),
		9 => sprintf(
			__( 'Tab scheduled for: <strong>%1$s</strong>.', 'ultimatewoo-pro' ),
			date_i18n( __( 'M j, Y @ G:i', 'ultimatewoo-pro' ), strtotime( $post->post_date ) )
		),
		10 => __( 'Tab draft updated.', 'ultimatewoo-pro' ),
	);
...
function wc_tab_manager_admin_nav() {
	...
	if ( $is_admin_page ) {
		$tabs_active    = '';
		$edit_tab_label = __( 'Add Global Tab', 'ultimatewoo-pro' );
		$edit_active    = '';
		$search_query   = '';
		$default_active = '';
		...
		if ( $is_edit_page && isset( $_GET['action'] ) && 'edit' === $_GET['action'] ) {
			$edit_tab_label = __( 'Edit Tab', 'ultimatewoo-pro' );
		}
		...
		<h1 class="nav-tab-wrapper woo-nav-tab-wrapper">
			<a class="nav-tab <?php echo esc_attr( $tabs_active ); ?>" href="<?php echo esc_url( admin_url( 'edit.php?post_type=wc_product_tab' ) ); ?>">
				<?php esc_html_e( 'Tabs', 'ultimatewoo-pro' ); ?>
			</a>
			...
			<a class="nav-tab <?php echo esc_attr( $default_active ); ?>" href="<?php echo esc_url( admin_url( 'admin.php?page=' . WC_Tab_Manager::PLUGIN_ID ) ); ?>">
				<?php esc_html_e( 'Default Tab Layout', 'ultimatewoo-pro' ); ?>
			</a>
			<?php if ( ! empty( $search_query ) ) : ?>
				<span class="subtitle">
					<?php /* translators: Placeholders: %s - keyword search query */
					$search_text = __( 'Search results for &#8220;%s&#8221;', 'ultimatewoo-pro' );
...
function wp_tab_manager_register_layout_page() {
	...
	add_submenu_page(
		null,                                                       // parent menu
		__( 'WooCommerce Tab Manager', 'ultimatewoo-pro' ), // page title
