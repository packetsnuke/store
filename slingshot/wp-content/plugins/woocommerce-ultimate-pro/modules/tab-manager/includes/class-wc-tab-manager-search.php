...
class WC_Tab_Manager_Search extends WP_Query {
		...
		if ( ! $post->post_parent ) : ?>
			...
			<li class="wide">
				...
				<label for="wc_product_tab_include_in_search">
					...
					<?php esc_html_e( 'Include tab content in search results?', 'ultimatewoo-pro' ); ?>
				</label>
			</li>
	...
	public function tab_layout_filter_dropdown() {
		...
		<select name="product_tab_layout">
			<option value="">
				<?php esc_html_e( 'Any tab layout', 'ultimatewoo-pro' ); ?>
			</option>
			<option value="default" <?php selected( 'default', $current_type ); ?>>
				<?php esc_html_e( 'Default layout', 'ultimatewoo-pro' ); ?>
			</option>
			<option value="custom" <?php selected( 'custom', $current_type ); ?>>
				<?php esc_html_e( 'Custom layout', 'ultimatewoo-pro' ); ?>
			</option>
		</select>
		<?php
	}
	...
	public function modify_search_clauses( $clauses, $_wp_query ) {
		...
		if ( ! ( $_wp_query instanceof WP_Query ) || ! $_wp_query->is_search || is_admin() ) {
			return $clauses;
		}
		...
		if ( empty( $search_terms ) || empty( $searchable_tabs ) ) {
			return $clauses;
		}
	...
	public function update_products_after_upgrade_nag() {
		...
		if ( ! empty( $show_update_nag ) ) {
			$nag_text  = __( 'Hey there, Tab Manager has a nifty new search feature. If you\'d like to take advantage of this you\'ll need to update your products and tabs (you only need to do this once).', 'ultimatewoo-pro' );
			$url_param = 'wc_tab_manager_show_update_products_nag';
		} else if ( ! empty( $show_relevanssi_nag ) ) {
			$nag_text  = __( 'It looks like you recently installed the Relevanssi plugin. If you\'d like to use the new Tab Manager search feature with Relevanssi, you\'ll need to update your products and tabs so they can be indexed.', 'ultimatewoo-pro' );
		...
		$update_url_text  = __( 'Go to the update page.', 'ultimatewoo-pro' );
		$dismiss_url      = add_query_arg( $url_param, 'false' );
		$dismiss_url_text = __( 'OK, don\'t remind me again.', 'ultimatewoo-pro' );
