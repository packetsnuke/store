...
class Product_Addon_Display {
	...
	public function styles() {
		...
		if ( is_singular( 'product' ) || class_exists( 'WC_Quick_View' ) ) {
			wp_enqueue_style( 'woocommerce-addons-css', ULTIMATEWOO_MODULES_URL . '/product-addons/assets/css/frontend.css' );
		}
	}
	...
	public function addon_scripts() {
		...
		wp_register_script( 'accounting', ULTIMATEWOO_MODULES_URL . '/product-addons/assets/js/addons' . $suffix . '.js', array( 'jquery' ), '0.4.2' );
		wp_enqueue_script( 'woocommerce-addons', ULTIMATEWOO_MODULES_URL . '/product-addons/assets/js/addons' . $suffix . '.js', array( 'jquery', 'accounting' ), '1.0', true );

		$params = array(
			'price_display_suffix'         => esc_attr( get_option( 'woocommerce_price_display_suffix' ) ),
			'ajax_url'                     => WC()->ajax_url(),
			'i18n_addon_total'             => __( 'Options total:', 'ultimatewoo-pro' ),
			'i18n_sub_total'               => __( 'Sub total:', 'ultimatewoo-pro' ),
			'i18n_remaining'               => __( 'characters remaining', 'ultimatewoo-pro' ),
	...
	public function plugin_path() {
		return $this->plugin_path = ULTIMATEWOO_MODULES_DIR . '/product-addons';
	}
		...
		if ( is_array( $product_addons ) && count( $product_addons ) > 0 ) {
			...
			foreach ( $product_addons as $addon ) {
				...
				wc_get_template( 'addons/addon-start.php', array(
					...
					'type'        => $addon['type'],
				), 'ultimatewoo-pro', $this->plugin_path() . '/templates/' );
	...
	public function get_custom_letters_only_html( $addon ) {
		wc_get_template( 'addons/custom_pattern.php', array(
			'addon'   => $addon,
			'pattern' => '[A-Za-z ]*',
			'title'   => __( 'Please enter letters only', 'ultimatewoo-pro' ),
		), 'ultimatewoo-pro', $this->plugin_path() . '/templates/' );
	}
	...
	public function get_custom_digits_only_html( $addon ) {
		wc_get_template( 'addons/custom_pattern.php', array(
			'addon'   => $addon,
			'pattern' => '[0-9]*',
			'title'   => __( 'Please enter digits only', 'ultimatewoo-pro' ),
		), 'ultimatewoo-pro', $this->plugin_path() . '/templates/' );
	}
	...
	public function get_custom_letters_or_digits_html( $addon ) {
		wc_get_template( 'addons/custom_pattern.php', array(
			'addon' => $addon,
			'pattern' => '[A-Za-z0-9 ]*',
			'title' => __( 'Please enter letters or digits only', 'ultimatewoo-pro' ),
		), 'ultimatewoo-pro', $this->plugin_path() . '/templates/' );
	}
	...
	public function get_custom_email_html( $addon ) {
		wc_get_template( 'addons/custom_email.php', array(
			'addon' => $addon,
			'title' => __( 'Please enter an email address', 'ultimatewoo-pro' ),
		), 'ultimatewoo-pro', $this->plugin_path() . '/templates/' );
	}
	...
	public function add_to_cart_text( $text, $product = null ) {
		...
		if ( ! is_single( $product->get_id() ) ) {
			if ( $this->check_required_addons( $product->get_id() ) ) {
				$text = apply_filters( 'addons_add_to_cart_text', __( 'Select options', 'ultimatewoo-pro' ) );
			}
		}
