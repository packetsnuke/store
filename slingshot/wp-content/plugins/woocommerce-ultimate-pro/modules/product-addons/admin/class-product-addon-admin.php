...
class Product_Addon_Admin {
	...
	public function admin_menu() {
		$page = add_submenu_page( 'edit.php?post_type=product', __( 'Global Add-ons', 'ultimatewoo-pro' ), __( 'Global Add-ons', 'ultimatewoo-pro' ), 'manage_woocommerce', 'global_addons', array( $this, 'global_addons_admin' ) );
	}
	...
	public function styles() {
		wp_enqueue_style( 'woocommerce_product_addons_css', ULTIMATEWOO_MODULES_URL . '/product-addons/assets/css/admin.css' );
	}
		...
		if ( ! empty( $_GET['add'] ) || ! empty( $_GET['edit'] ) ) {
			...
			if ( $_POST ) {
				...
				if ( $edit_id = $this->save_global_addons() ) {
					echo '<div class="updated"><p>' . __( 'Add-on saved successfully', 'ultimatewoo-pro' ) . '</p></div>';
				}
			...
			if ( ! empty( $_GET['edit'] ) ) {
				...
				if ( ! $global_addon ) {
					echo '<div class="error">' . __( 'Error: Global Add-on not found', 'ultimatewoo-pro' ) . '</div>';
					return;
				}
			...
			if ( ! empty( $_GET['delete'] ) && wp_verify_nonce( $_REQUEST['_wpnonce'], 'delete_addon' ) ) {
				...
				echo '<div class="updated"><p>' . __( 'Add-on deleted successfully', 'ultimatewoo-pro' ) . '</p></div>';
			}
	 ...
	 */
	public function tab() {
		?><li class="addons_tab product_addons"><a href="#product_addons_data"><span><?php _e( 'Add-ons', 'ultimatewoo-pro' ); ?></span></a></li><?php
	}
