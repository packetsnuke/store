...
<div id="product_addons_data" class="panel woocommerce_options_panel wc-metaboxes-wrapper">
	...
	<div class="woocommerce-product-add-ons-toolbar woocommerce-product-add-ons-toolbar--add-import-export toolbar">
		<button type="button" class="button add_new_addon"><?php _e( 'New add-on', 'ultimatewoo-pro' ); ?></button>

		<div class="woocommerce-product-add-ons-toolbar__import-export">
			<button type="button" class="button import_addons"><?php _e( 'Import', 'ultimatewoo-pro' ); ?></button>
			<button type="button" class="button export_addons"><?php _e( 'Export', 'ultimatewoo-pro' ); ?></button>
		</div>
		...
		<textarea name="import_product_addon" class="import" cols="20" rows="5" placeholder="<?php _e('Paste exported form data here and then save to import fields. The imported fields will be appended.', 'ultimatewoo-pro'); ?>"></textarea>

	</div>
	<?php if ( $exists ) : ?>
		<div class="options_group">
			<p class="form-field">
			<label for="_product_addons_exclude_global"><?php _e( 'Global Addon Exclusion', 'ultimatewoo-pro' ); ?></label>
			<input id="_product_addons_exclude_global" name="_product_addons_exclude_global" class="checkbox" type="checkbox" value="1" <?php checked( $exclude_global, 1 ); ?>/><span class="description"><?php _e( 'Check this to exclude this product from all Global Addons', 'ultimatewoo-pro' ); ?></span>
			</p>
		</div>
	<?php endif; ?>
</div>
<?php
$empty_name_message = __( 'All addon fields require a name.', 'ultimatewoo-pro' );
?>
<script type="text/javascript">
	jQuery(function( $ ){
		...
		.on( 'change', 'select.product_addon_type', function() {
			...
			switch ( value ) {
				case 'custom_price':
					column_title = '<?php echo esc_js( __( 'Min / max price', 'ultimatewoo-pro' ) ); ?>';
				break;

				case 'input_multiplier':
					column_title = '<?php echo esc_js( __( 'Min / max multiplier', 'ultimatewoo-pro' ) ); ?>';
				break;
				...
				case 'custom_email':
				case 'custom':
					column_title = '<?php echo esc_js( __( 'Min / max characters', 'ultimatewoo-pro' ) ); ?>';
				break;

				default:
					column_title = '<?php echo esc_js( __( 'Min / max', 'ultimatewoo-pro' ) ); ?>';
				break;
			}
		...
		.on( 'click', '.remove_addon', function() {
			...
			var answer = confirm('<?php _e('Are you sure you want remove this add-on?', 'ultimatewoo-pro'); ?>');
		...
		jQuery('button.remove_addon_option').on('click', function(){
			...
			var answer = confirm('<?php _e('Are you sure you want delete this option?', 'ultimatewoo-pro'); ?>');
