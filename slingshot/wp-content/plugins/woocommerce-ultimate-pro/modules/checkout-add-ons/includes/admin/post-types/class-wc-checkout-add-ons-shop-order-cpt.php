				...
				if ( $item_id ) {
					...
					switch ( $add_on->type ) {
						...
						case 'file':
							...
							echo '<a href="#" class="wc-checkout-add-ons-files-toggle">' . sprintf( _n( '%d file', '%d files', $files_count, 'ultimatewoo-pro' ), $files_count ) . '</a>';
							...
							foreach ( $file_ids as $key => $file_id ) {
								if ( $url = get_edit_post_link( $file_id ) ) {
									...
									echo '<li>' . esc_html__( '(File has been removed)', 'ultimatewoo-pro' ) . '</li>';
								}
							}
							echo '</ul>';
			...
			if ( $add_on->is_filterable() ) {
				...
				if ( $add_on->has_options() ) {
					...
					<select
						...
						printf( esc_attr( __( 'Show all %s', 'ultimatewoo-pro' ) ), $add_on->name ); ?>"
						...
						<?php foreach ( $add_on->get_options() as $option ) : ?>
							...
							<option value="<?php echo esc_attr( $value ); ?>" <?php echo $selected; ?>><?php echo esc_html__( $option['label'], 'ultimatewoo-pro' ); ?></option>
						<?php endforeach; ?>
					</select>
					<?php
					...
					<?php if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ) : ?>
						...
						<select
							...
							printf( __( 'Show all %s', 'ultimatewoo-pro' ), $add_on->label ); ?>"
							...
							data-request_data = "<?php echo esc_attr( json_encode( array( 'add_on_id' => $add_on->id, 'default' => addslashes( __( 'Show all ', 'ultimatewoo-pro' ) . $add_on->name ) ) ) ) ?>">
