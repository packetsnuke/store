...
<div class="wc-social-login-profile">
	...
	<h2><?php esc_html_e( 'Social Login Accounts', 'ultimatewoo-pro' ); ?></h2>
	...
	<?php if ( $linked_profiles ) : ?>
		...
		if ( count( $linked_profiles ) < count( $available_providers ) ) {
			...
			$add_more_link = ' ' . sprintf( __( '%1$sAdd more...%2$s', 'ultimatewoo-pro' ), '<a href="#" class="js-show-available-providers">', '</a>' );
		}
		?>

		<p><?php echo esc_html__( 'Your account is connected to the following social login providers.', 'ultimatewoo-pro' ) . $add_more_link; ?></p>

		<table class="shop_table shop_table_responsive wc-social-login-linked-profiles">
			<thead>
				<tr>
					<th><?php esc_html_e( 'Provider', 'ultimatewoo-pro' ); ?></th>
					<th><?php esc_html_e( 'Account', 'ultimatewoo-pro' ); ?></th>
					<th colspan="2"><?php esc_html_e( 'Last login', 'ultimatewoo-pro' ); ?></th>
				</tr>
			</thead>
			...
			?>
			<tr>
				<td data-title="<?php esc_attr_e( 'Provider', 'ultimatewoo-pro' ); ?>">
				...
				<td data-title="<?php esc_attr_e( 'Account', 'ultimatewoo-pro' ); ?>">
					<?php
						/**
				...
				<td data-title="<?php esc_attr_e( 'Last login', 'ultimatewoo-pro' ); ?>">
					<?php if ( $login_timestamp ) : ?>
						<?php
							/* translators: Placeholders: %1$s - date, %2$s - time */
							printf( esc_html__( '%1$s @ %2$s', 'ultimatewoo-pro' ), date_i18n( wc_date_format(), $login_timestamp ), date_i18n( wc_time_format(), $login_timestamp ) );
						?>
					<?php else : ?>
						<?php esc_html_e( 'Never', 'ultimatewoo-pro' ); ?>
					<?php endif; ?>
				</td>
				<td class="profile-actions">
					<a href="<?php echo esc_url( wp_nonce_url( $provider->get_auth_url( $return_url, 'unlink' ), 'unlink' ) ); ?>" class="button unlink-social-login-profile">
						<?php /* translators: This is an action: unlink account from a social profile */ esc_html_e( 'Unlink', 'ultimatewoo-pro' ); ?>
					</a>
				</td>
			</tr>
			<?php endforeach; ?>
	...
	<?php else : ?>
		...
		<?php
			/* translators: Placeholders: %1$s - <a> tag, %2$s - </a> tag */
			printf( esc_html__( 'You have no social login profiles connected. %1$sConnect one now%2$s', 'ultimatewoo-pro' ), '<a href="#" class="js-show-available-providers">', '</a>' );
		?>
		</p>
	...
	<div class="wc-social-login-available-providers" style="display:none;">
		...
		<p><?php esc_html_e( 'You can link your account to the following providers:', 'ultimatewoo-pro' ); ?></p>
