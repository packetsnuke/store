...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.6.4', __( 'WooCommerce Social Login', 'ultimatewoo-pro' ), __FILE__, 'init_woocommerce_social_login', array(
...
class WC_Social_Login extends SV_WC_Plugin {
	...
	public function add_delayed_admin_notices() {
		...
		if ( $this->is_plugin_settings() ) {
			...
			$this->get_admin_notice_handler()->add_admin_notice(
				...
				sprintf( __( 'Thanks for installing Social Login! Before you get started, please take a moment to %1$sread through the documentation%2$s.', 'ultimatewoo-pro' ), '<a href="' . $this->get_documentation_url() . '">', '</a>' ),
				'read-the-docs',
				array(
		...
		foreach ( $this->get_providers() as $provider ) {
			...
			if ( $provider->is_enabled() && $provider->requires_ssl() ) {
				...
				if ( 'no' === get_option( 'woocommerce_force_ssl_checkout' ) ) {
					...
					$message = sprintf( _x( 'WooCommerce Social Login: %s requires SSL for authentication, please force WooCommerce over SSL.', 'Requires SSL', 'ultimatewoo-pro' ), '<strong>' . $provider->get_title() . '</strong>' );
	...
	public function add_admin_notices() {
		...
		if ( class_exists( 'ITSEC_Tweaks' ) ) {
			...
			if ( $this->is_plugin_settings() && isset( $ithemes_security_settings['long_url_strings'] ) && $ithemes_security_settings['long_url_strings'] ) {
				...
				$this->get_admin_notice_handler()->add_admin_notice(
					esc_html__( 'Oops, looks like iThemes Security is set to Filter Long URLs. This is likely to cause a conflict with Social Login -- please disable that setting for optimal functionality.', 'ultimatewoo-pro' ),
		...
		if ( get_option( 'wc_social_login_upgraded_from_opauth' ) && 'legacy' === get_option( 'wc_social_login_callback_url_format' ) ) {
			...
			$this->get_admin_notice_handler()->add_admin_notice(
				...
				sprintf( esc_html__( 'Please update callback URLs for each Social Login provider, then switch callback URLs to the Default format in the %1$sSocial Login settings%2$s. You can %3$slearn more from the plugin documentation%4$s.', 'ultimatewoo-pro' ), '<a href="' . $this->get_settings_url() . '">', '</a>', '<a href="https://docs.woocommerce.com/document/woocommerce-social-login/#upgrading-to-v2">', '</a>' ),
	...
	public function get_plugin_name() {
		...
		return __( 'WooCommerce Social Login', 'ultimatewoo-pro' );
	}
	...
	protected function install() {
		...
		add_option( 'wc_social_login_text', __( 'For faster checkout, login or register using your social account.', 'ultimatewoo-pro' ) );
		add_option( 'wc_social_login_text_non_checkout', __( 'Use a social account for faster login or easy registration.', 'ultimatewoo-pro' ) );
	}
