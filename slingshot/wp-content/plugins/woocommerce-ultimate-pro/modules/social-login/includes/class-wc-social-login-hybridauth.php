...
class WC_Social_Login_HybridAuth {
	...
	public function authenticate( $provider_id, $return_url = null ) {
		...
		try {
			...
			wc_add_notice( __( 'Provider Authentication error', 'ultimatewoo-pro' ), 'error' );
	...
	public function redirect( $user_id = 0 ) {
		...
		if ( isset( $user->user_email ) && '' === $user->user_email && ! SV_WC_Helper::str_starts_with( $return_url, wc_get_checkout_url() ) ) {
			...
			if ( $return_url === wc_get_page_permalink( 'myaccount' ) ) {
				wc_add_notice( __( 'Please enter your email address to complete your registration', 'ultimatewoo-pro' ), 'error' );
			} else {
				/* translators: %s - a URL */
				wc_add_notice( sprintf( __( 'Please enter your email address to complete your registration and continue to %s', 'ultimatewoo-pro' ), $return_url ), 'error' );
			}
	...
	private function process_profile( $profile ) {
		...
		if ( ! $provider ) {
			throw new SV_WC_Plugin_Exception( sprintf( __( 'No provider class found for %s', 'ultimatewoo-pro' ), $profile->get_provider_id() ) );
		}
		...
		if ( ! $profile->get_identifier() ) {
			throw new SV_WC_Plugin_Exception( sprintf( __( '%s returned an invalid user identifier.', 'ultimatewoo-pro' ), $profile->get_provider_id() ) );
		}
		...
		if ( ! is_user_logged_in() && $user && 'email' === $found_via && ! in_array( $user->roles[0], apply_filters( 'wc_social_login_find_by_email_allowed_user_roles', array( 'subscriber', 'customer' ) ) ) ) {
			throw new SV_WC_Plugin_Exception( __( 'Oops, it looks like you may already have an account&hellip; please log in to link your profile.', 'ultimatewoo-pro' ) );
		}
	...
	private function create_new_customer( $profile ) {
		...
		if ( is_wp_error( $customer_id ) ) {
			throw new SV_WC_Plugin_Exception( '<strong>' . __( 'ERROR', 'ultimatewoo-pro' ) . '</strong>: ' . __( 'Couldn&#8217;t register you&hellip; please contact us if you continue to have problems.', 'ultimatewoo-pro' ) );
		}
