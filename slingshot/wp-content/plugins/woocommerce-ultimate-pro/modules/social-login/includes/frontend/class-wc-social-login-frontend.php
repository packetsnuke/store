...
class WC_Social_Login_Frontend {
	...
	public function maybe_render_social_buttons( $template_name ) {
		...
		// notice on thank you page
			...
			$message = '<p>' . esc_html__( 'Save time next time you checkout by linking your account to your favorite social network. No need to remember another username and password.', 'ultimatewoo-pro' ) . '</p>';
	...
	public function checkout_social_login_message( $message ) {
		...
		if ( is_checkout() && $this->is_displayed_on( 'checkout' ) && strpos( $message, '<a href="#" class="showlogin">' ) !== false && count( wc_social_login()->get_available_providers() ) > 0 ) {
			$message .= '. <br/>' . get_option( 'wc_social_login_text' ) . ' <a href="#" class="js-show-social-login">' . esc_html__( 'Click here to login', 'ultimatewoo-pro' ) .'</a>';
		}
