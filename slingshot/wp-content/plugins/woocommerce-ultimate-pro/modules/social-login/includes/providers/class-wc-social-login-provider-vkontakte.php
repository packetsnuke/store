...
class WC_Social_Login_Provider_VKontakte extends WC_Social_Login_Provider {
	...
	public function __construct( $base_auth_path ) {
		...
		$this->id                = 'vkontakte';
		$this->title             = __( 'VK', 'ultimatewoo-pro' );
		$this->color             = '#4972a4';
	...
	public function get_description() {
		...
		return sprintf( __( 'Need help setting up and configuring VK? %1$sRead the docs%2$s', 'ultimatewoo-pro' ), '<a href="http://docs.woocommerce.com/document/woocommerce-social-login-create-social-apps#vk">', '</a>' );
	}
	...
	public function init_form_fields() {
		...
		$this->form_fields['id']['title']     = __( 'Application ID', 'ultimatewoo-pro' );
		$this->form_fields['secret']['title'] = __( 'Secure Key', 'ultimatewoo-pro' );
	}
	...
	public function get_default_login_button_text() {
		return __( 'Log in with VK', 'ultimatewoo-pro' );
	}
	...
	public function get_default_link_button_text() {
		return __( 'Link your account to VK', 'ultimatewoo-pro' );
	}
	...
	public function get_notices() {
		return array(
			'account_linked'         => __( 'Your VK account is now linked to your account.', 'ultimatewoo-pro' ),
			'account_unlinked'       => __( 'VK was successfully unlinked from your account.', 'ultimatewoo-pro' ),
			'account_already_linked' => __( 'This VK account is already linked to another user account.', 'ultimatewoo-pro' ),
			'account_already_exists' => __( 'A user account using the same email address as this VK account already exists.', 'ultimatewoo-pro' ),
		);
	}
