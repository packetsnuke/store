...
class WC_Social_Login_Provider_Twitter extends WC_Social_Login_Provider {
	...
	public function __construct( $base_auth_path ) {
		...
		$this->id                = 'twitter';
		$this->title             = __( 'Twitter', 'ultimatewoo-pro' );
		$this->color             = '#55acee';
	...
	public function get_description() {
		...
		$description = sprintf( __( 'Need help setting up and configuring Twitter? %1$sRead the docs%2$s', 'ultimatewoo-pro' ), '<a href="http://docs.woocommerce.com/document/woocommerce-social-login-create-social-apps#twitter">', '</a>');

		$callback_url_format = get_option( 'wc_social_login_callback_url_format' );

		/* translators: Placeholder: %s - a url */
		$description .= '<br/><br/>' . sprintf( __( 'The callback URL is %s', 'ultimatewoo-pro' ), '<code>' . $this->get_callback_url() . '</code>' );

		if ( 'legacy' === $callback_url_format ) {

			$description .= ' <strong>' . __( '(Please update your Twitter app to use this URL)', 'ultimatewoo-pro' ) . '</strong>';

			/* translators: Placeholder: %s - a url */
			$description .= '<br/><br/>' . sprintf( __( 'The legacy callback URL is %s', 'ultimatewoo-pro' ), '<code>' . $this->get_callback_url( $callback_url_format ) . '</code>' );
		}
	...
	public function init_form_fields() {
		...
		$this->form_fields['id']['title']     = __( 'Consumer Key', 'ultimatewoo-pro' );
		$this->form_fields['secret']['title'] = __( 'Consumer Secret', 'ultimatewoo-pro' );
	}
	...
	public function get_default_login_button_text() {
		return __( 'Log in with Twitter', 'ultimatewoo-pro' );
	}
	...
	public function get_default_link_button_text() {
		return __( 'Link your account to Twitter', 'ultimatewoo-pro' );
	}
	...
	public function get_notices() {
		return array(
			'account_linked'         => __( 'Your Twitter account is now linked to your account.', 'ultimatewoo-pro' ),
			'account_unlinked'       => __( 'Twitter was successfully unlinked from your account.', 'ultimatewoo-pro' ),
			'account_already_linked' => __( 'This Twitter account is already linked to another user account.', 'ultimatewoo-pro' ),
			'account_already_exists' => __( 'A user account using the same email address as this Twitter account already exists.', 'ultimatewoo-pro' ),
		);
	}
