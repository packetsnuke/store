...
class WC_Social_Login_Provider_Facebook extends WC_Social_Login_Provider {
	...
	public function __construct( $base_auth_path ) {
		...
		$this->id          = 'facebook';
		$this->title       = __( 'Facebook', 'ultimatewoo-pro' );
		$this->color       = '#3b5998';
		$this->require_ssl = false;

		/* translators: Placeholders: %1$s - <a> tag, %2$s - </a> tag */
		$this->description = sprintf( __( 'Need help setting up and configuring Facebook? %1$sRead the docs%2$s', 'ultimatewoo-pro' ), '<a href="http://docs.woocommerce.com/document/woocommerce-social-login-create-social-apps#facebook">', '</a>' );
	...
	public function get_description() {
		...
		return sprintf( __( 'Need help setting up and configuring Facebook? %1$sRead the docs%2$s', 'ultimatewoo-pro' ), '<a href="http://docs.woocommerce.com/document/woocommerce-social-login-create-social-apps#facebook">', '</a>' );
	}
	...
	public function init_form_fields() {
		...
		$this->form_fields['id']['title']     = __( 'App ID', 'ultimatewoo-pro' );
		$this->form_fields['secret']['title'] = __( 'App Secret', 'ultimatewoo-pro' );
	}
	...
	public function get_default_login_button_text() {
		return __( 'Log in with Facebook', 'ultimatewoo-pro' );
	}
	...
	public function get_default_link_button_text() {
		return __( 'Link your account to Facebook', 'ultimatewoo-pro' );
	}
	...
	public function get_notices() {
		return array(
			'account_linked'         => __( 'Your Facebook account is now linked to your account.', 'ultimatewoo-pro' ),
			'account_unlinked'       => __( 'Facebook was successfully unlinked from your account.', 'ultimatewoo-pro' ),
			'account_already_linked' => __( 'This Facebook account is already linked to another user account.', 'ultimatewoo-pro' ),
			'account_already_exists' => __( 'A user account using the same email address as this Facebook account already exists.', 'ultimatewoo-pro' ),
		);
	}
