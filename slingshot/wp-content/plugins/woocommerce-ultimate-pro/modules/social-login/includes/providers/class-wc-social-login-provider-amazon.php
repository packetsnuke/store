...
class WC_Social_Login_Provider_Amazon extends WC_Social_Login_Provider {
	...
	public function __construct( $base_auth_path ) {
		...
		$this->id                = 'amazon';
		$this->title             = __( 'Amazon', 'ultimatewoo-pro' );
		$this->color             = '#ff9900';
	...
	public function get_description() {
		...
		$description = sprintf( __( 'Need help setting up and configuring Amazon? %1$sRead the docs%2$s', 'ultimatewoo-pro' ), '<a href="http://docs.woocommerce.com/document/woocommerce-social-login-create-social-apps#amazon">', '</a>');

		$callback_url_format = get_option( 'wc_social_login_callback_url_format' );

		/* translators: Placeholder: %s - a url */
		$description .= '<br/><br/>' . sprintf( __( 'The allowed return URL is %s', 'ultimatewoo-pro' ), '<code>' . $this->get_callback_url() . '</code>' );

		if ( 'legacy' === $callback_url_format ) {

			$description .= ' <strong>' . __( '(Please update your Amazon app to use this URL)', 'ultimatewoo-pro' ) . '</strong>';

			/* translators: Placeholder: %s - a url */
			$description .= '<br/><br/>' . sprintf( __( 'The legacy allowed return URL is %s', 'ultimatewoo-pro' ), '<code>' . $this->get_callback_url( $callback_url_format ) . '</code>' );
		}
	...
	public function get_default_login_button_text() {
		return __( 'Log in with Amazon', 'ultimatewoo-pro' );
	}
	...
	public function get_default_link_button_text() {
		return __( 'Link your account to Amazon', 'ultimatewoo-pro' );
	}
	...
	public function get_notices() {
		return array(
			'account_linked'         => __( 'Your Amazon account is now linked to your account.', 'ultimatewoo-pro' ),
			'account_unlinked'       => __( 'Amazon was successfully unlinked from your account.', 'ultimatewoo-pro' ),
			'account_already_linked' => __( 'This Amazon account is already linked to another user account.', 'ultimatewoo-pro' ),
			'account_already_exists' => __( 'A user account using the same email address as this Amazon account already exists.', 'ultimatewoo-pro' ),
		);
	}
