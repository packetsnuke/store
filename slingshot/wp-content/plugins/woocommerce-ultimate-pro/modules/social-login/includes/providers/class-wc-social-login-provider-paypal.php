...
class WC_Social_Login_Provider_PayPal extends WC_Social_Login_Provider {
	...
	public function __construct( $base_auth_path ) {
		...
		$this->id                = 'paypal';
		$this->title             = __( 'PayPal', 'ultimatewoo-pro' );
		$this->color             = '#00457c';
	...
	public function get_description() {
		...
		$description = sprintf( __( 'Need help setting up and configuring PayPal? %1$sRead the docs%2$s', 'ultimatewoo-pro' ), '<a href="http://docs.woocommerce.com/document/woocommerce-social-login-create-social-apps#paypal">', '</a>');

		$callback_url_format = get_option( 'wc_social_login_callback_url_format' );

		/* translators: Placeholder: %s - a url */
		$description .= '<br/><br/>' . sprintf( __( 'The App redirect URL is %s', 'ultimatewoo-pro' ), '<code>' . $this->get_callback_url() . '</code>' );

		if ( 'legacy' === $callback_url_format ) {

			$description .= ' <strong>' . __( '(Please update your PayPal App to use this URL)', 'ultimatewoo-pro' ) . '</strong>';

			/* translators: Placeholder: %s - a url */
			$description .= '<br/><br/>' . sprintf( __( 'The legacy App redirect URL is %s', 'ultimatewoo-pro' ), '<code>' . $this->get_callback_url( $callback_url_format ) . '</code>' );
		}
	...
	public function init_form_fields() {
		...
		$this->form_fields['environment'] = array(
			...
			'title'    => __( 'Environment', 'ultimatewoo-pro' ),
			'type'     => 'select',
			'desc_tip' => __( 'Select which environment to process logins under.', 'ultimatewoo-pro' ),
			'options'  => array(
				/* translators: Live (Production) environment - https:www.skyverge.com/for-translators-environments/ */
				'live'    => __( 'Live', 'ultimatewoo-pro' ),
				/* translators: Placeholders: Sandbox (Test) environment - https:www.skyverge.com/for-translators-environments/ */
				'sandbox' => __( 'Sandbox', 'ultimatewoo-pro' ),
			),
			'default'  => 'live',
		);

		$this->form_fields['secret']['title'] = __( 'Secret', 'ultimatewoo-pro' );
	}
	...
	public function get_default_login_button_text() {
		return __( 'Log in with PayPal', 'ultimatewoo-pro' );
	}
	...
	public function get_default_link_button_text() {
		return __( 'Link your account to PayPal', 'ultimatewoo-pro' );
	}
	...
	public function get_notices() {
		return array(
			'account_linked'         => __( 'Your PayPal account is now linked to your account.', 'ultimatewoo-pro' ),
			'account_unlinked'       => __( 'PayPal was successfully unlinked from your account.', 'ultimatewoo-pro' ),
			'account_already_linked' => __( 'This PayPal account is already linked to another user account.', 'ultimatewoo-pro' ),
			'account_already_exists' => __( 'A user account using the same email address as this PayPal account already exists.', 'ultimatewoo-pro' ),
		);
	}
