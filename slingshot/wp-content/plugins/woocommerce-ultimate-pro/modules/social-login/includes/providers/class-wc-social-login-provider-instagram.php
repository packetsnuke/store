...
class WC_Social_Login_Provider_Instagram extends WC_Social_Login_Provider {
	...
	public function __construct( $base_auth_path ) {
		...
		$this->id                = 'instagram';
		$this->title             = __( 'Instagram', 'ultimatewoo-pro' );
		$this->color             = '#e4405f';
	...
	public function get_description() {
		...
		$description = sprintf( __( 'Need help setting up and configuring Instagram? %1$sRead the docs%2$s', 'ultimatewoo-pro' ), '<a href="http://docs.woocommerce.com/document/woocommerce-social-login-create-social-apps#instagram">', '</a>');

		$callback_url_format = get_option( 'wc_social_login_callback_url_format' );

		/* translators: Placeholder: %s - a url */
		$description .= '<br/><br/>' . sprintf( __( 'The OAuth redirect_uri is %s', 'ultimatewoo-pro' ), '<code>' . $this->get_callback_url() . '</code>' );

		if ( 'legacy' === $callback_url_format ) {

			$description .= ' <strong>' . __( '(Please update your Instagram app to use this URL)', 'ultimatewoo-pro' ) . '</strong>';

			/* translators: Placeholder: %s - a url */
			$description .= '<br/><br/>' . sprintf( __( 'The legacy OAuth redirect_uri is %s', 'ultimatewoo-pro' ), '<code>' . $this->get_callback_url( $callback_url_format ) . '</code>' );
		}
	...
	public function get_default_login_button_text() {
		return __( 'Log in with Instagram', 'ultimatewoo-pro' );
	}
	...
	public function get_default_link_button_text() {
		return __( 'Link your account to Instagram', 'ultimatewoo-pro' );
	}
	...
	public function get_notices() {
		return array(
			'account_linked'         => __( 'Your Instagram account is now linked to your account.', 'ultimatewoo-pro' ),
			'account_unlinked'       => __( 'Instagram was successfully unlinked from your account.', 'ultimatewoo-pro' ),
			'account_already_linked' => __( 'This Instagram account is already linked to another user account.', 'ultimatewoo-pro' ),
			'account_already_exists' => __( 'A user account using the same email address as this Instagram account already exists.', 'ultimatewoo-pro' ),
		);
	}
