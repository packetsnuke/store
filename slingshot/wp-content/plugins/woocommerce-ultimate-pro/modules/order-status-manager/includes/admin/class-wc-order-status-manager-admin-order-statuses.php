...
class WC_Order_Status_Manager_Admin_Order_Statuses {
	...
	public function order_status_columns( $columns ) {
		...
		$columns['slug']        = __( 'Slug', 'ultimatewoo-pro' );
		$columns['description'] = __( 'Description', 'ultimatewoo-pro' );
		$columns['paid']        = __( 'Paid', 'ultimatewoo-pro' );
		$columns['reports']     = __( 'Reports', 'ultimatewoo-pro' );
		$columns['type']        = __( 'Type', 'ultimatewoo-pro' );

		$first_column = array( 'icon' => __( 'Icon', 'ultimatewoo-pro' ) );
	...
	public function add_meta_boxes() {
		...
		add_meta_box(
			...
			__( 'Order Status Data', 'ultimatewoo-pro' ),
		...
		add_meta_box(
			...
			__( 'Order Status Actions', 'ultimatewoo-pro' ),
	...
	public function order_status_data_meta_box() {
		...
		<div id="order_status_options" class="panel woocommerce_options_panel">
			<div class="options_group">
				...
				woocommerce_wp_text_input( array(
					'id'                => 'post_name',
					'label'             => __( 'Slug', 'ultimatewoo-pro' ),
					'value'             => $post->post_name,
					'custom_attributes' => $custom_attributes,
					'desc_tip'          => true,
					'description'       => __( 'Optional. If left blank, the slug will be automatically generated from the name. Maximum: 17 characters, cannot contain leading numbers.', 'ultimatewoo-pro' ),
				) );
				...
				woocommerce_wp_textarea_input( array(
					...
					'desc_tip'    => true,
					'description' => __( 'Optional status description. If set, this will be shown to customers while viewing an order.', 'ultimatewoo-pro' ),
			...
			<div class="options_group">
				...
				woocommerce_wp_text_input( array(
					'id'          => '_color',
					'label'       => __( 'Color', 'ultimatewoo-pro' ),
					'type'        => 'text',
					'class'       => 'colorpick',
					'default'     => '#000000',
					'description' => __( 'Color displayed behind the order status image or name', 'ultimatewoo-pro' ),
				) );
				...
				<p class="form-field _icon_field">
					<label for="_icon"><?php esc_html_e( 'Icon', 'ultimatewoo-pro' ); ?></label>
					...
					<a href="#_icon" class="button upload-icon upload-icon-image" data-uploader-button-text="<?php _e( 'Set as status icon', 'ultimatewoo-pro' ); ?>"><?php _e( "Select File", 'ultimatewoo-pro' ); ?></a>
					<?php echo wc_help_tip( __( 'Optional status icon. If not supplied, then Name will be displayed to represent the status', 'ultimatewoo-pro' ) ); ?>
				</p>
				...
				<p class="form-field _action_icon_field">
					<label for="_action_icon"><?php esc_html_e( 'Action Icon', 'ultimatewoo-pro' ); ?></label>
					...
					<a href="#_action_icon" class="button upload-icon upload-icon-image" data-uploader-button-text="<?php esc_attr_e( 'Set as status icon', 'ultimatewoo-pro' ); ?>"><?php esc_html_e( "Select File", 'ultimatewoo-pro' ); ?></a>
					<?php echo wc_help_tip( __( 'Optional action icon displayed in the action buttons for the next statuses.', 'ultimatewoo-pro' ) ); ?>
				</p>
			</div><!-- // .options_group -->
			...
			<div class="options_group">
				...
				<p class="form-field _next_statuses_field">
					<label for="_next_statuses"><?php esc_html_e( 'Next Statuses', 'ultimatewoo-pro' ); ?></label>
					<select id="_next_statuses"
					        name="_next_statuses[]"
					        class="select short"
					        data-placeholder="<?php esc_html_e( 'Choose statuses that follow this one in your workflow.', 'ultimatewoo-pro' ); ?>"
					        multiple >
					...
					<?php echo wc_help_tip( __( 'Zero or more statuses that would be considered next during normal order status flow. Action buttons will be available to move an order with this custom status to these next statuses.', 'ultimatewoo-pro' ) ); ?>
				</p>
				<?php
				...
				woocommerce_wp_checkbox( array(
					'id'          => '_bulk_action',
					'label'       => __( 'Bulk action', 'ultimatewoo-pro' ),
					'description' => __( 'Check this to add this order status to the Orders list table Bulk Actions list.', 'ultimatewoo-pro' ),
				...
				woocommerce_wp_checkbox( array(
					...
					'label'       => __( 'Include in reports', 'ultimatewoo-pro' ),
					'description' => __( 'Check this to include orders with this order status in the order reports.', 'ultimatewoo-pro' ),
				...
				woocommerce_wp_select( array(
					'id'          => '_is_paid',
					'label'       => _x( 'Paid', 'Order status has had payment received', 'ultimatewoo-pro' ),
					'description' => __( 'Choose whether this status implies a payment that has been received or not.', 'ultimatewoo-pro' ),
					'desc_tip'    => true,
					'options'     => array(
						'yes'           => __( 'Orders with this status have been paid.', 'ultimatewoo-pro' ),
						'needs_payment' => __( 'Orders with this status require payment (similar to "pending").', 'ultimatewoo-pro' ),
						'no'            => __( 'Orders are neither paid nor require payment (similar to "on-hold" or "refunded").', 'ultimatewoo-pro' ),
					),
			...
			<li class="wide">
				<div id="delete-action">
					<?php if ( ! $status->is_core_status() && current_user_can( "delete_post", $post->ID ) ) : ?>
						<a class="submitdelete deletion" href="<?php echo esc_url( get_delete_post_link( $post->ID, '', true ) ); ?>"><?php esc_html_e( 'Delete', 'ultimatewoo-pro' ); ?></a>
					<?php endif; ?>
				</div>
				...
				<input
					...
					value="<?php esc_attr_e( 'Save Order Status', 'ultimatewoo-pro' ); ?>"
					data-tip="<?php esc_attr_e( 'Save/update the order status', 'ultimatewoo-pro' ); ?>"
				/>
			</li>
			<?php
	...
	public function order_status_table_actions( $actions ) {
		...
		return array(
			'import-custom-statuses' => ' <button id="import-custom-statuses" class="button help_tip" data-tip="' . esc_attr__( 'Add statuses that may have been added by other plugins or custom code.', 'ultimatewoo-pro' ) . '" >' .
			                            esc_html__( 'Import custom statuses', 'ultimatewoo-pro' ) .
			                            '</button> ',
		);
	}
	...
	public function custom_column_content( $column, $post_id ) {
		...
		switch ( $column ) {
			...
			case 'reports' :
				echo esc_html( $status->include_in_reports() ? __( 'Yes', 'ultimatewoo-pro' ) : __( 'No', 'ultimatewoo-pro' ) );
			break;

			case 'paid':
				echo esc_html ( $status->is_paid() ? __( 'Yes', 'ultimatewoo-pro' ) : __( 'No', 'ultimatewoo-pro' ) );
			break;
	...
	public function reassign_order_status_popup() {
		...
		if ( 'wc_order_status' === $typenow ) :
			...
			<div id="reassign-order-status-popup" style="display: none;">
				...
				<h3><?php esc_html_e( 'Are you sure that you want to delete this order status?', 'ultimatewoo-pro' ); ?></h3>
				...
				<p class="singular" style="display: none">
					<a class="order-status-link"  href=""><?php
						...
						printf( __( 'There is currently %1$s order marked as %2$s.', 'ultimatewoo-pro' ), '<strong class="order-status-count"></strong>', '<strong class="order-status-name"></strong>' ); ?></a>
				</p>
				<p class="plural" style="display: none">
					<a class="order-status-link" href=""><?php
						...
						printf( __( 'There are currently %1$s orders marked as %2$s.', 'ultimatewoo-pro' ), '<strong class="order-status-count"></strong>', '<strong class="order-status-name"></strong>' ); ?></a>
				</p>

				<p><?php esc_html_e( 'You can choose to reassign the status of existing orders with another before deleting or have it reassigned automatically.', 'ultimatewoo-pro' ) ?></p>
				...
				<label for="wc-order-status-manager-reassign-status">
					...
					<button class="button reassign"><?php esc_html_e( 'Reassign and delete', 'ultimatewoo-pro' ); ?></button>
				</label>

				<button class="button delete"><?php esc_html_e( 'Delete', 'ultimatewoo-pro' ); ?></button>

				<p>
					<em><?php esc_html_e( 'Either operation cannot be undone automatically.', 'ultimatewoo-pro' ); ?></em><br>
					<em><?php esc_html_e( 'Emails and other actions will not be triggered automatically.', 'ultimatewoo-pro' ); ?></em>
				</p>
