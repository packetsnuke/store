...
class WC_Order_Status_Manager_Admin {
	...
	public function print_settings_tabs() {
		...
		?>
		<a href="<?php echo esc_url( admin_url( 'edit.php?post_type=wc_order_status' ) ); ?>" class="nav-tab <?php echo ( 'wc_order_status' === $typenow ? 'nav-tab-active' : '' ); ?>"><?php esc_html_e( 'Order Statuses', 'ultimatewoo-pro' ); ?></a>
		<?php
	}
	...
	public function output_sections() {
		...
		?>
		<ul class="subsubsub">
			<li><a href="<?php echo esc_url( admin_url( 'edit.php?post_type=wc_order_status' ) ); ?>" class="<?php echo ( 'wc_order_status' === $typenow ? 'current' : '' ); ?>"><?php esc_html_e( 'Statuses', 'ultimatewoo-pro' ); ?></a> | </li>
			<li><a href="<?php echo esc_url( admin_url( 'edit.php?post_type=wc_order_email' ) ); ?>" class="<?php echo ( 'wc_order_email' === $typenow ? 'current' : '' ); ?>"><?php esc_html_e( 'Emails', 'ultimatewoo-pro' ); ?></a></li>
		</ul>
		<br class="clear" />
		<?php
	}
	...
	public function load_styles_scripts() {
		...
		if ( $this->is_order_status_manager_screen() ) {
			...
			$script_data = array(
				...
				'i18n' => array(
					'remove_this_condition' => __( 'Remove this condition?', 'ultimatewoo-pro' ),
					'from_status'           => __( 'From Status', 'ultimatewoo-pro' ),
					'to_status'             => __( 'To Status', 'ultimatewoo-pro' ),
					'remove'                => __( 'Remove', 'ultimatewoo-pro' ),
					'any'                   => __( 'Any', 'ultimatewoo-pro' ),
					'select_icon'           => __( 'Select Icon', 'ultimatewoo-pro' ),
					'all_icon_packages'     => __( 'All icon packages', 'ultimatewoo-pro' ),
					'search_icons'          => __( 'Search Icons', 'ultimatewoo-pro' ),
					'choose_file'           => __( 'Choose a file', 'ultimatewoo-pro' ),
	...
	public function normalize_columns( $columns ) {
		...
		$columns['title'] = __( 'Name', 'ultimatewoo-pro' );
		...
		if ( $this->is_order_status_manager_post_type( get_post_type( $post->ID ) ) ) {
			...
			if ( current_user_can( 'delete_post', $post->ID ) && ! $order_status->is_core_status() ) {
				...
				$actions['delete'] = sprintf(
					...
					esc_attr__( 'Delete this item permanently', 'ultimatewoo-pro' ),
					get_delete_post_link( $post->ID, '', true ),
					__( 'Delete Permanently', 'ultimatewoo-pro' )
				);
			}
		}
