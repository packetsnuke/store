	...
	public function handle_order_status_delete( $post_id, $replacement_status = 'on-hold' ) {
		...
		if ( ! empty( $order_rows ) ) {
			...
			foreach ( $order_rows as $order_row ) {
				...
				$order->update_status( $replacement_status, sprintf( __( 'Order status updated from %1$s to %2$s because the former status was deleted.', 'ultimatewoo-pro' ), $old_status_name, $new_status_name ) );
