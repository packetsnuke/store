...
class WC_Pre_Orders_List_Table extends WP_List_Table {
	...
	public function __construct() {
		...
		parent::__construct(
			array(
				'singular' => __( 'Pre-Order', 'ultimatewoo-pro' ),
				'plural'   => __( 'Pre-Orders', 'ultimatewoo-pro' ),
				'ajax'     => false
			)
		);
	}
	...
	public function get_columns() {
		...
		$columns = array(
			...
			'status'            => '<span class="status_head tips" data-tip="' . __( 'Status', 'ultimatewoo-pro' ) . '">' . __( 'Status', 'ultimatewoo-pro' ) . '</span>',
			'customer'          => __( 'Customer', 'ultimatewoo-pro' ),
			'product'           => __( 'Product', 'ultimatewoo-pro' ),
			'order'             => __( 'Order', 'ultimatewoo-pro' ),
			'order_date'        => __( 'Order Date', 'ultimatewoo-pro' ),
			'availability_date' => __( 'Availability Date', 'ultimatewoo-pro' ),
		);
		...
		switch ( $column_name ) {
			...
			case 'status':
				...
				if ( WC_Pre_Orders_Manager::can_pre_order_be_changed_to( 'cancelled', $order ) ) {
					...
					$actions['cancel'] = sprintf( '<a href="%s">%s</a>', esc_url( $cancel_url ), esc_html__( 'Cancel', 'ultimatewoo-pro' ) );
				}
			...
			case 'order':
				$column_content = sprintf( '<a href="%s">%s</a>', get_edit_post_link( $order_id ), sprintf( __( 'Order %s', 'ultimatewoo-pro' ), $order->get_order_number() ) );
			break;
	...
	public function no_items() {
		...
		if ( isset( $_REQUEST['s'] ) ) : ?>
			<p><?php _e( 'No pre-orders found', 'ultimatewoo-pro' ); ?></p>
		<?php else : ?>
			<p><?php _e( 'Pre-Orders will appear here for you to view and manage once purchased by a customer.', 'ultimatewoo-pro' ); ?></p>
			<p><?php printf( __( '%sLearn more about managing pre-orders%s', 'ultimatewoo-pro' ), '<a href="http://docs.woothemes.com/document/pre-orders/#section-6" target="_blank">', ' &raquo;</a>' ); ?></p>
			<p><?php printf( __( '%sSetup a product to allow pre-orders%s', 'ultimatewoo-pro' ), '<a href="' . admin_url( 'post-new.php?post_type=product' ) . '">', ' &raquo;</a>' ); ?></p>
		<?php endif;
	}
	...
	public function extra_tablenav( $which ) {
		...
		if ( 'top' == $which ) {
			...
			<select id="dropdown_customers" name="_customer_user">
				<option value=""><?php _e( 'Show all customers', 'ultimatewoo-pro' ) ?></option>
				<?php
			...
			<select id="dropdown_products" name="_product">
				<option value=""><?php _e( 'Show all Products', 'ultimatewoo-pro' ) ?></option>
				<?php
			...
			?>
					<select id="dropdown_customers" style="width: 250px;" class="wc-customer-search" name="_customer_user" data-placeholder="<?php esc_attr_e( 'Search for a customer&hellip;', 'ultimatewoo-pro' ); ?>">
						<?php
							if ( ! empty( $_GET['_customer_user'] ) ) {
				...
				<?php } else { ?>
					<input type="hidden" id="dropdown_customers" class="wc-customer-search" name="_customer_user" data-placeholder="<?php _e( 'Search for a customer&hellip;', 'ultimatewoo-pro' ); ?>" data-selected="<?php echo esc_attr( $user_string ); ?>" value="<?php echo $user_id; ?>" data-allow_clear="true" style="width: 250px;" />
				<?php } ?>

				<?php if ( version_compare( WC_VERSION, '3.0.0', '>=' ) ) { ?>
					<select id="dropdown_products" class="wc-product-search" style="width: 250px;" name="_product" data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'ultimatewoo-pro' ); ?>" data-action="woocommerce_json_search_products_and_variations">
						<?php
							if ( ! empty( $_GET['_product'] ) ) {
				...
				<?php } else { ?>
					<input type="hidden" id="dropdown_products" class="wc-product-search" name="_product" data-placeholder="<?php _e( 'Search for a product&hellip;', 'ultimatewoo-pro' ); ?>" data-selected="<?php echo esc_attr( $product_name ); ?>" value="<?php echo $product_id; ?>" data-allow_clear="true" style="width: 250px;" />
			...
			echo '<textarea cols="62" rows="3" name="customer_message" placeholder="' . __( 'Optional message to include in the email to the customer', 'ultimatewoo-pro' ) . '"></textarea>';
			echo '</div>';
			...
			$javascript = "
				...
				$( 'span.cancel' ).click( function( e ) {
					if ( ! window.confirm( '" . __( 'Are you sure you want to cancel this pre-order?', 'ultimatewoo-pro' ) . "' ) ) {
						e.preventDefault();
					}
				});
			";
				...
				$chosen = "
					...
					$('select#dropdown_customers').css('width', '250px').ajaxChosen({
						...
						data: {
							...
							default:  '" . __( 'Show all customers', 'ultimatewoo-pro' ) . "'
						}
					}, function (data) {
			...
			echo '<textarea cols="62" rows="3" name="customer_message2" placeholder="' . __( 'Optional message to include in the email to the customer', 'ultimatewoo-pro' ) . '"></textarea>';
			echo '</div>';
		}
	}
	...
	private function render_availability_dates_dropdown() {
		...
		<select id="dropdown_availability_dates" name="availability_date" class="wc-enhanced-select" style="width: 250px;">
			<option<?php selected( $availability_date, 0 ); ?> value='0'><?php _e( 'Show all Availability Dates', 'ultimatewoo-pro' ); ?></option>
			<?php
