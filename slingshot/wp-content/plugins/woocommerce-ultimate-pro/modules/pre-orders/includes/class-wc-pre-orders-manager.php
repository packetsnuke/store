...
class WC_Pre_Orders_Manager {
	...
	public static function change_release_date_for_all_pre_orders( $product, $new_availability_date, $message = '' ) {
		...
		foreach ( $orders as $order ) {
			...
			$order->add_order_note( sprintf( __( 'Pre-Order Release Date Changed to %s', 'ultimatewoo-pro' ), WC_Pre_Orders_Product::get_localized_availability_date( $product, __( 'N/A', 'ultimatewoo-pro' ) ) ) );
	...
	public static function cancel_pre_order( $order, $message = '' ) {
		...
		$order->add_order_note( __( 'Pre-Order Cancelled', 'ultimatewoo-pro' ) );
	...
	public function check_cancel_pre_order() {
		...
		$string = __( 'Your pre-order has been cancelled.', 'ultimatewoo-pro' );
