	...
	public function validate_cart( $valid, $product_id ) {
		...
		if ( WC_Pre_Orders_Product::product_can_be_pre_ordered( $product_id ) ) {
			...
			if( $woocommerce->cart->get_cart_contents_count() >= 1 ) {
				...
				$string = __( 'Your previous cart was emptied because pre-orders must be purchased separately.', 'ultimatewoo-pro' );
			...
			if ( $this->cart_contains_pre_order() ) {
				...
				// Backwards compatible (pre 2.1) for outputting notice
				if ( function_exists( 'wc_add_notice' ) ) {
					wc_add_notice( __( 'This product cannot be added to your cart because it already contains a pre-order, which must be purchased separately.', 'ultimatewoo-pro' ) );
				} else {
					$woocommerce->add_error( __( 'This product cannot be added to your cart because it already contains a pre-order, which must be purchased separately.', 'ultimatewoo-pro' ) );
				}
