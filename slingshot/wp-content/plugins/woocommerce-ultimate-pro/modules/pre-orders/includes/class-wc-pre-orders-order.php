...
class WC_Pre_Orders_Order  {
	...
	public function register_order_status() {
		register_post_status( 'wc-pre-ordered', array(
			'label'                     => _x( 'Pre ordered', 'Order status', 'ultimatewoo-pro' ),
			'public'                    => true,
			'exclude_from_search'       => false,
			'show_in_admin_all_list'    => true,
			'show_in_admin_status_list' => true,
			'label_count'               => _n_noop( 'Pre ordered <span class="count">(%s)</span>', 'Pre ordered <span class="count">(%s)</span>', 'ultimatewoo-pro' )
		) );
	}
	...
	public function order_statuses( $order_statuses ) {
		$order_statuses['wc-pre-ordered'] = _x( 'Pre ordered', 'Order status', 'ultimatewoo-pro' );
	...
	public static function get_pre_order_status_to_display( $order ) {
		...
		switch ( $status ) {
			case 'active' :
				$status_string = __( 'Active', 'ultimatewoo-pro' );
				break;
			case 'completed' :
				$status_string = __( 'Completed', 'ultimatewoo-pro' );
				break;
			case 'cancelled' :
				$status_string = __( 'Cancelled', 'ultimatewoo-pro' );
				break;
			default :
	...
	public static function update_pre_order_status( $order, $new_status, $message = '' ) {
		...
		// add order note
		$order->add_order_note( $message . sprintf( __( 'Pre-Order status changed from %s to %s.', 'ultimatewoo-pro' ), $old_status, $new_status ) );
	}
