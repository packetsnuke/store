...
class WC_Pre_Orders_Admin_Pre_Orders {
	...
	protected function get_tabs() {
		return array(
			'manage'  => __( 'Manage', 'ultimatewoo-pro' ),
			'actions' => __( 'Actions', 'ultimatewoo-pro' ),
		);
	}
	...
	public function show_sub_menu_page() {
		...
		if ( ! empty( $_GET['success'] ) ) {
			...
			switch ( $_GET['success'] )  {
				...
				case 'email':
					$message = __( 'Pre-Order customers emailed successfully', 'ultimatewoo-pro' );
					break;

				case 'change-date':
					$message = __( 'Pre-Order Date Changed', 'ultimatewoo-pro' );
					break;

				case 'complete':
					$message = __( 'Pre-Orders completed', 'ultimatewoo-pro' );
					break;

				case 'cancel':
					$message = __( 'Pre-Orders cancelled', 'ultimatewoo-pro' );
					break;
	...
	public function add_pre_orders_list_options() {
		$args = array(
			'label'   => __( 'Pre-Orders', 'ultimatewoo-pro' ),
			'default' => 20,
	...
	public function process_cancel_pre_order_action() {
		...
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			wp_die( __( 'You do not have the correct permissions to do this.', 'ultimatewoo-pro' ) );
		}
		...
		$this->_redirect_with_notice( sprintf( __( 'Pre-order #%s cancelled.', 'ultimatewoo-pro' ), $order_id ) );
	}
	...
	public function process_manage_tab() {
		...
		if ( isset( $_GET['page'] ) && 'wc_pre_orders' == $_GET['page'] ) {
			...
			switch ( $action ) {
				case 'cancel':
					if ( $success_count > 0 ) {
						$messages[] = sprintf( _n( '%d pre-order cancelled.', '%s pre-orders cancelled.', $success_count, 'ultimatewoo-pro' ), $success_count );
					}
					if ( $error_count > 0 ) {
						$messages[] = sprintf( _n( '%d pre-order could not be cancelled.', '%s pre-orders could not be cancelled.', $error_count, 'ultimatewoo-pro' ), $error_count );
					}
				break;

				case 'complete':
					if ( $success_count > 0 ) {
						$messages[] = sprintf( _n( '%d pre-order completed.', '%s pre-orders completed.', $success_count, 'ultimatewoo-pro' ), $success_count );
					}
					if ( $error_count > 0 ) {
						$messages[] = sprintf( _n( '%d pre-order could not be completed.', '%s pre-orders could not be completed.', $error_count, 'ultimatewoo-pro' ), $error_count );
					}
				break;

				case 'message':
					$messages[] = sprintf( _n( '%d email dispatched.', '%s emails dispatched.', count( $order_ids ), 'ultimatewoo-pro' ), count( $order_ids ) );
				break;
			}
	...
	public function get_bulk_actions() {
		$actions = array(
			...
			'complete' => __( 'Complete', 'ultimatewoo-pro' ),
			'message'  => __( 'Customer Message', 'ultimatewoo-pro' ),
		);
	...
	private function show_actions_tab() {
		...
		if ( empty( $pre_order_products ) ) {
			...
			<div class="notice notice-warning">
				<p><?php _e( 'There is no pre-order product currently. List of pre-order products will appear in the drop-down Product below.', 'ultimatewoo-pro' ); ?></p>
			</div>
			<?php
		}
		...
		$actions = array(
			...
			'change-date' => __( 'Change Release Date', 'ultimatewoo-pro' ),
			'complete'    => __( 'Complete', 'ultimatewoo-pro' ),
			'cancel'      => __( 'Cancel', 'ultimatewoo-pro' ),
		);
	...
	private function show_manage_tab() {
		...
		echo '<h2>' . __( 'Manage Pre-Orders', 'ultimatewoo-pro' );
		if ( isset( $_GET['s'] ) && $_GET['s'] ) {
			echo '<span class="subtitle">' . sprintf( __( 'Search results for "%s"', 'ultimatewoo-pro' ), $_GET['s'] ) . '</span>';
		}
		echo '</h2>';
		...
		// Display the views
		$manage_table->views();
		$manage_table->search_box( __( 'Search Pre-Orders', 'ultimatewoo-pro' ), 'pre_order' );
	...
	private function get_action_fields( $section ) {
		...
		$products = array( '' => __( 'Select a Product', 'ultimatewoo-pro' ) );
		...
		$fields = array(
			...
			'email' => array(
				...
				array(
					'name' => __( 'Email Pre-Order Customers', 'ultimatewoo-pro' ),
					'desc' => sprintf( __( 'You may send an email message to all customers who have pre-ordered a specific product. This will use the default template specified for the %sCustomer Note%s Email.', 'ultimatewoo-pro' ), '<a href="' . admin_url( 'admin.php?page=wc-settings&tab=email&section=wc_email_customer_note' ) . '">', '</a>' ),
					'type' => 'title'
				),

				array(
					'id'                => 'wc_pre_orders_action_product',
					'name'              => __( 'Product', 'ultimatewoo-pro' ),
					'desc_tip'          => __( 'Select which product to email all pre-ordered customers.', 'ultimatewoo-pro' ),
					'default'           => ' ',
					'options'           => $products,
					'type'              => 'select',
				...
				array(
					...
					'desc_tip'          => __( 'Enter a message to include in the email notification to customer. Limited HTML allowed.', 'ultimatewoo-pro' ),
					'css'               => 'min-width: 300px;',
					'default'           => '',
					'type'              => 'textarea',
					'custom_attributes' => array(
				...
				array(
					'name' => __( 'Send Emails', 'ultimatewoo-pro' ),
					'type' => 'submit_button',
				),
			),
			...
			'change-date' => array(
				...
				array(
					'name' => __( 'Change the Pre-Order Release Date', 'ultimatewoo-pro' ),
					'desc' => __( 'You may change the release date for all pre-orders of a specific product. This will send an email notification to each customer informing them that the pre-order release date was changed, along with the new release date.', 'ultimatewoo-pro' ),
					'type' => 'title'
				),
				...
				array(
					'id'                => 'wc_pre_orders_action_product',
					'name'              => __( 'Product', 'ultimatewoo-pro' ),
					'desc_tip'          => __( 'Select which product to change the release date for.', 'ultimatewoo-pro' ),
					'default'           => ' ',
					'options'           => $products,
					'type'              => 'select',
					'custom_attributes' => array(
				...
				array(
					...
					'name'              => __( 'New Availability Date', 'ultimatewoo-pro' ),
					'desc_tip'          => __( 'The new availability date for the product. This must be later than the current availability date.', 'ultimatewoo-pro' ),
					'default'           => '',
					'type'              => 'text',
					'custom_attributes' => array(
						'required' => 'required',
				...
				array(
					...
					'name'    => __( 'Send Email Notification', 'ultimatewoo-pro' ),
					'desc'    => __( 'Uncheck this to prevent email notifications from being sent to customers.', 'ultimatewoo-pro' ),
					'default' => 'yes',
					'type'    => 'checkbox'
				),
				...
				array(
					'id'       => 'wc_pre_orders_action_email_message',
					'name'     => __( 'Message', 'ultimatewoo-pro' ),
					'desc_tip' => __( 'Enter a message to include in the email notification to customer.', 'ultimatewoo-pro' ),
					'default'  => '',
					'css'      => 'min-width: 300px;',
					'type'     => 'textarea',
				),
				...
				array(
					'name' => __( 'Change Release Date', 'ultimatewoo-pro' ),
					'type' => 'submit_button',
				),
			),
			...
			'complete' => array(
				...
				array(
					'name' => __( 'Complete Pre-Orders', 'ultimatewoo-pro' ),
					'desc' => __( 'You may complete all pre-orders for a specific product. This will charge the customer\'s card the pre-ordered amount, change their order status to completed, and send them an email notification.', 'ultimatewoo-pro' ),
					'type' => 'title'
				),
				...
				array(
					'id'                => 'wc_pre_orders_action_product',
					'name'              => __( 'Product', 'ultimatewoo-pro' ),
					'desc_tip'          => __( 'Select which product to complete all pre-orders for.', 'ultimatewoo-pro' ),
					'default'           => ' ',
					'options'           => $products,
					'type'              => 'select',
					'custom_attributes' => array(
			...
			'cancel' => array(
				array(
					'name' => __( 'Cancel Pre-Orders', 'ultimatewoo-pro' ),
					'desc' => __( 'You may cancel all pre-orders for a specific product. This will mark the order as cancelled and send the customer an email notification. If pre-orders were charged upfront, you must manually refund the orders.', 'ultimatewoo-pro' ),
					'type' => 'title'
				),
				...
				array(
					'id'                => 'wc_pre_orders_action_product',
					'name'              => __( 'Product', 'ultimatewoo-pro' ),
					'desc_tip'          => __( 'Select which product to cancel all pre-orders for.', 'ultimatewoo-pro' ),
					'default'           => ' ',
					'options'           => $products,
					'type'              => 'select',
					'custom_attributes' => array(
