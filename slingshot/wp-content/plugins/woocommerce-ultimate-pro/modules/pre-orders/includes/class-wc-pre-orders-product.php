...
class WC_Pre_Orders_Product {
			...
			if ( $product->managing_stock() ) {
				...
				if ( $product->is_in_stock() && $product_total_stock > get_option( 'woocommerce_notify_no_stock_amount' ) ) {
					switch ( get_option( 'woocommerce_stock_format' ) ) {
						case 'no_amount' :
							$availability = __( 'Available for pre-ordering', 'ultimatewoo-pro' );
						break;
						case 'low_amount' :
							if ( $product_total_stock <= get_option( 'woocommerce_notify_low_stock_amount' ) ) {
								/* translators: 1: product total stock */
								$availability = sprintf( __( 'Only %s left available for pre-ordering', 'ultimatewoo-pro' ), $product_total_stock );

								if ( $product->backorders_allowed() && $product->backorders_require_notification() ) {
									$availability .= ' ' . __( '(can be backordered)', 'ultimatewoo-pro' );
								}
							} else {
								$availability = __( 'Available for pre-ordering', 'ultimatewoo-pro' );
							}
						...
						default :
							/* translators: 1: product total stock */
							$availability = sprintf( __( '%s available for pre-ordering', 'ultimatewoo-pro' ), $product_total_stock );
					...
					$availability = __( 'No longer available for pre-ordering', 'ultimatewoo-pro' );
					$class        = 'out-of-stock';
				}
	...
	public static function get_localized_availability_date( $product, $none_text = '' ) {
		if ( '' === $none_text ) {
			$none_text = __( 'at a future date', 'ultimatewoo-pro' );
		}
