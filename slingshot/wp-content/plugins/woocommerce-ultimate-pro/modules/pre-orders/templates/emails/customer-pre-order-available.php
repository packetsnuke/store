...
printf( __( "Your pre-order is now available, but requires payment. %sPlease pay for your pre-order now.%s", 'ultimatewoo-pro' ), '<a href="' . $order->get_checkout_payment_url() . '">', '</a>' ); ?></p>

<?php elseif ( 'failed' === $order->get_status() || 'on-hold' === $order->get_status() ) : ?>

	<p><?php
/* translators: 1: href link for checkout payment url 2: closing href link */
printf( __( "Your pre-order is now available, but automatic payment failed. %sPlease update your payment information now.%s", 'ultimatewoo-pro' ), '<a href="' . $order->get_checkout_payment_url() . '">', '</a>' ); ?></p>

<?php else : ?>

<p><?php _e( "Your pre-order is now available. Your order details are shown below for your reference.", 'ultimatewoo-pro' ); ?></p>
