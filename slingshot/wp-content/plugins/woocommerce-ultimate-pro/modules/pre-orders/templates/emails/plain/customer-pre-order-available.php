...
if ( 'pending' === $order->get_status() && ! WC_Pre_Orders_Manager::is_zero_cost_order( $order ) ) :
	...
	echo __( 'Your pre-order is now available, but requires payment. Please pay for your pre-order now: ', 'ultimatewoo-pro' ) . esc_url( $order->get_checkout_payment_url() ) . "\n\n";

elseif ( 'failed' === $order->get_status() || 'on-hold' === $order->get_status() ) :

	echo __( "Your pre-order is now available, but automatic payment failed. Please update your payment information now : ", 'ultimatewoo-pro' ) . esc_url( $order->get_checkout_payment_url() ) . "\n\n";

else :

	echo __( "Your pre-order is now available. Your order details are shown below for your reference.", 'ultimatewoo-pro' ) . "\n\n";
