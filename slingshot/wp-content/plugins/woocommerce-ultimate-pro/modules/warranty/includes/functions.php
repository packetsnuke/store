    ...
    <div class="warranty-tracking-container">
        ...
        if ( $request['request_tracking_code'] == 'y' ) {
            if (! empty($request['tracking_code']) ) {
                ...
                <div class="customer-tracking-code-container">
                    <b><?php _e('Track Your Package', 'ultimatewoo-pro'); ?></b>
                    <?php
                    if ( !empty($request['tracking_provider']) ) {
                        ...
                        echo '<li>'. __('Shipped via', 'ultimatewoo-pro') .' '. $providers_name[$provider] .'</li>';
                        echo '<li>'. __('Tracking Code:', 'ultimatewoo-pro') .' '. $request['tracking_code'] .' <br/> (<a href="'. $link .'" target="_blank">'. __('Track Shipment', 'ultimatewoo-pro') .'</a>)</li>';
                        echo '</ul>';
                    } else {
                        echo '<p>'. __('Tracking Number:', 'ultimatewoo-pro') .' '. $request['tracking_code'] .'</p>';
                    }
                    ?>
                </div>
                <?php
        ...
        if (! empty($request['return_tracking_code']) ) {
            ...
            <div class="store-tracking-code-container">
                <b><?php _e('Shipment Tracking Code', 'ultimatewoo-pro'); ?></b>
                <?php
                $all_providers  = array();
                $providers_name = array();
                ...
                <ul class="warranty-data">
                    <li><?php printf( __('Shipped via %s', 'ultimatewoo-pro'), $providers_name[$provider] ); ?></li>
                    <li><?php printf( __('Tracking Code: %s &mdash; %s', 'ultimatewoo-pro'), $request['return_tracking_code'], '<a href="'. $link .'" target="_blank">'. __('Track Return Shipment', 'ultimatewoo-pro') .'</a>'); ?></li>
                </ul>
            </div>
                <?php
        } else {
            echo '<p>'. __('Your Tracking Number is:', 'ultimatewoo-pro') .' '. $request['return_tracking_code'] .'</p>';
        }
    }
    ?>
    </div>
...
function warranty_request_shipping_tracking_code_form( $request ) {
    ...
    <div class="warranty-tracking-code-container">
        <strong><?php _e('Shipping Tracking Code', 'ultimatewoo-pro'); ?></strong>

        <p class="form-field">
            <label class="form-label" for="shipping_provider_<?php echo $request['ID']; ?>">
                <?php _e('Shipping Provider', 'ultimatewoo-pro'); ?>
            </label>
        ...
        <p class="form-field">
            <label class="form-label" for="tracking_code_<?php echo $request['ID']; ?>">
                <?php _e('Tracking Code', 'ultimatewoo-pro'); ?>
            </label>
            <br/>
        ...
        <input type="submit" name="send" value="<?php _e('Add Tracking Code', 'ultimatewoo-pro'); ?>" class="button" />
    </div>
    <?php
    return ob_get_clean();
}
    ...
    if ( version_compare(WC()->version, '2.0', '>=') ) {
        foreach ( $meta as $key => $value ) {
            ...
            if ( $key == '_item_warranty' ) {
                $warranty = array_merge( $warranty, maybe_unserialize( stripslashes($value) ) );
...
function warranty_get_warranty_duration_string( $warranty, $order ) {
    ...
    if ( $warranty['type'] == 'no_warranty' ) {
        $warranty_string = __('Product has no warranty', 'ultimatewoo-pro');
    } elseif ( $warranty['type'] == 'included_warranty' ) {
        if ( $warranty['length'] == 'lifetime' ) {
            $warranty_string = __('Lifetime', 'ultimatewoo-pro');
        } else {
            $order_date         = ($completed) ? $completed : WC_Warranty_Compatibility::get_order_prop( $order, 'order_date' );
            $warranty_string    = __('Expiry Date: ', 'ultimatewoo-pro') . warranty_get_date($order_date, $warranty['value'], $warranty['duration']);
        }
	    ...
	    if ( isset( $warranty['addons'][ $idx ] ) ) {
		    ...
		    $order_date         = ($completed) ? $completed : WC_Warranty_Compatibility::get_order_prop( $order, 'order_date' );
		    $warranty_string    = __('Expiry Date: ', 'ultimatewoo-pro') . warranty_get_date($order_date, $addon['value'], $addon['duration']);
	    }
    }
...
function warranty_get_warranty_string( $product_id = 0, $warranty = null ) {
    ...
    if ( empty( $warranty ) || $warranty['type'] == 'no_warranty' ) {
        $string = __('No warranty', 'ultimatewoo-pro');
    } elseif ( $warranty['type'] == 'included_warranty' ) {
        if ( $warranty['length'] == 'lifetime' ) {
            $string = __('Lifetime Warranty Included', 'ultimatewoo-pro');
        } else {
            $duration = warranty_duration_i18n( $warranty['duration'], $warranty['value'] );

            $string = sprintf( __('Warranty Included (%d %s)', 'ultimatewoo-pro'), $warranty['value'], $duration );
        }
    } else {
        ...
        foreach ( (array)$warranty['addons'] as $addon ) {
            ...
            $string     .= sprintf( __('%d %s for %s; ', 'ultimatewoo-pro'), $addon['value'], $duration, $amount );
        }
    }
...
function warranty_create_request( $args = array() ) {
    ...
    $warranty = apply_filters( 'wc_warranty_post_data', array(
        'post_content'  => '',
        'post_name'     => __('RMA Request for Order #', 'ultimatewoo-pro') . $args['order_id'],
        'post_status'   => 'publish',
        'post_author'   => 1,
        'post_type'     => 'warranty_request'
    ), $args );
...
function warranty_refund_item( $request_id, $amount = null ) {
    ...
    $product        = wc_get_product( $product_id );
    $order          = wc_get_order( $order_id );
    $refund_reason  = __('Item Returned', 'ultimatewoo-pro');
    $refunded       = false;
    ...
    if ( isset( $payment_gateways[ $payment_method ] ) && $payment_gateways[ $payment_method ]->supports( 'refunds' ) ) {
        ...
        if ( is_wp_error( $result ) ) {
            ...
            $error = new WP_Error( 'wc_refund_error', __('Refund failed', 'ultimatewoo-pro') );
        }
    }
...
function warranty_add_order_note( $warranty_id ) {
    ...
    $note = sprintf( __('<a href="%s">RMA #%d</a> status changed to %s', 'ultimatewoo-pro'), $rma_url, $warranty_id, $status->name );
...
function warranty_get_tracking_data( $request_id ) {
    ...
    if ( ! empty( $request['tracking_code'] ) && ! empty( $request['tracking_provider'] ) ) {
        ...
        $link     = str_replace( '%1$s', $request['tracking_code'], $link );
        $link     = str_replace( '%2$s', '', $link );
        $tracking['customer'] = '<a href="' . $link . '" target="_blank">' . __( 'Track Shipment', 'ultimatewoo-pro' ) . '</a>';
    ...
    if (! empty($request['return_tracking_code']) && ! empty($request['return_tracking_provider']) ) {
        ...
        $link       = str_replace('%1$s', $request['return_tracking_code'], $link);
        $link       = str_replace('%2$s', '', $link);
        $tracking['store'] = '<a href="'. $link .'" target="_blank">'. __('Track Shipment', 'ultimatewoo-pro') .'</a>';
