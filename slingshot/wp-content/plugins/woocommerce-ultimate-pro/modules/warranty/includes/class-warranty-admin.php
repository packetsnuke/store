...
class Warranty_Admin {
    ...
    public function admin_menu() {
        add_menu_page( __('Warranties', 'ultimatewoo-pro'), __('Warranties', 'ultimatewoo-pro'), 'manage_warranties', 'warranties', 'Warranty_Admin::admin_controller', 'dashicons-update', '54.52' );
        add_submenu_page( 'warranties', __('RMA Requests', 'ultimatewoo-pro'), __('RMA Requests', 'ultimatewoo-pro'), 'manage_warranties', 'warranties', 'Warranty_Admin::admin_controller' );
        add_submenu_page( 'warranties', __('New Request', 'ultimatewoo-pro'), __('New Request', 'ultimatewoo-pro'), 'manage_warranties', 'warranties-new', 'Warranty_Admin::admin_controller' );
        add_submenu_page( 'warranties', __('Manage Warranties', 'ultimatewoo-pro'), __('Manage Warranties', 'ultimatewoo-pro'), 'manage_woocommerce', 'warranties-bulk-update', 'Warranty_Admin::admin_controller' );
        add_submenu_page( 'warranties', __('Reports', 'ultimatewoo-pro'), __('Reports', 'ultimatewoo-pro'), 'manage_woocommerce', 'warranties-reports', 'Warranty_Admin::admin_controller' );
        add_submenu_page( 'warranties', __('Settings', 'ultimatewoo-pro'), __('Settings', 'ultimatewoo-pro'), 'manage_woocommerce', 'warranties-settings', 'Warranty_Admin::admin_controller' );
    }
    ...
    public function add_notices() {
        ...
        if (
            ...
            echo '<div class="updated"><p>'. __('Shipping label attached', 'ultimatewoo-pro') .'</p></div>';
        }
        ...
        if ( isset($_GET['page']) && in_array( $_GET['page'], $pages ) ) {
            ...
            $js = '
                    jQuery(".warranty-delete").click(function(e) {
                        return confirm("'. __('Do you really want to delete this request?', 'ultimatewoo-pro') .'");
                    });
                    var tiptip_args = {
                        "attribute" : "data-tip",
                        "fadeIn" : 50,
                    ...
                    var variable_tmpl = "<tr>\
                            ...
                            <td valign=\"middle\">\
                                ...
                                <select name=\"variable_addon_warranty_length_duration[_loop_][]\" style=\"width: auto !important;\">\
                                    <option value=\"days\"><?php _e('Days', 'ultimatewoo-pro'); ?></option>\
                                    <option value=\"weeks\"><?php _e('Weeks', 'ultimatewoo-pro'); ?></option>\
                                    <option value=\"months\"><?php _e('Months', 'ultimatewoo-pro'); ?></option>\
                                    <option value=\"years\"><?php _e('Years', 'ultimatewoo-pro'); ?></option>\
                                </select>\
                            </td>\
    ...
    function panel_add_custom_box() {
            ...
            var tmpl = "<tr>\
                            ...
                            <td valign=\"middle\">\
                                ...
                                <select name=\"addon_warranty_length_duration[]\">\
                                    <option value=\"days\">'. __('Days', 'ultimatewoo-pro') .'</option>\
                                    <option value=\"weeks\">'. __('Weeks', 'ultimatewoo-pro') .'</option>\
                                    <option value=\"months\">'. __('Months', 'ultimatewoo-pro') .'</option>\
                                    <option value=\"years\">'. __('Years', 'ultimatewoo-pro') .'</option>\
                                </select>\
                            </td>\
        ...
        if ( empty($warranty_label) ) {
            $warranty_label = __('Warranty', 'ultimatewoo-pro');
        }
    ...
    public function add_order_action( $actions ) {
        $actions['generate_rma'] = get_option( 'warranty_button_text', __('Create Warranty Request', 'ultimatewoo-pro') );
    ...
    public function create_warranty() {
        ...
        if ( is_wp_error( $request_id ) ) {
            ...
            if ( $order ) {
                ...
                $message = sprintf(
                    __('<a href="admin.php?page=warranties&s=%s">RMA (%s)</a> has been created for %s', 'ultimatewoo-pro'),
                    $rma,
                    $rma,
        ...
        $back   = add_query_arg( 'updated', urlencode(__('Warranty request created', 'ultimatewoo-pro')), $back );
    ...
    public function attach_shipping_label() {
        ...
        if ( isset( $_POST['redirect' ] ) ) {
            ...
            $path = 'admin.php?page=warranties&updated='. urlencode( __('Shipping label uploaded', 'ultimatewoo-pro') );
        }
        wp_redirect( $path );
        exit;
    }
    ...
    public function return_inventory() {
        ...
        wp_redirect( 'admin.php?page=warranties&updated='. urlencode(__('Product stock returned', 'ultimatewoo-pro')) );
        exit;
    }
    ...
    public function refund_item() {
        ...
        if ( $add_notice ) {
            if ( is_wp_error( $refund ) ) {
                ...
                $message = __('Item marked as Refunded', 'ultimatewoo-pro');
            }
        ...
        if ( $request['status'] == $returned && $manage_stock == 'yes' ) {
            if ( get_post_meta( $request['ID'], '_returned', true ) == 'yes' ) {
                $actions['inventory-return'] = array(
                    'text'      => __('Stock returned', 'ultimatewoo-pro'),
                    'disabled'  => true,
                    'class'     => ''
                );
            } else {
                $actions['inventory-return'] = array(
                    'text'      => __('Return Stock', 'ultimatewoo-pro'),
                    'disabled'  => false,
        ...
        if ( is_array( $refunds_status ) && in_array( $request['status'], $refunds_status ) ) {
            ...
            if ( $request_type == 'refund' ) {
                ...
                $actions['item-refund'] = array(
                    'text'      => __( 'Refund Item', 'ultimatewoo-pro' ),
                    'disabled'  => false,
                ...
                $actions['item-coupon'] = array(
                    'text'      => __( 'Send Coupon', 'ultimatewoo-pro' ),
    ...
    public function warranty_delete() {
        ...
        wp_redirect( 'admin.php?page=warranties&updated='. urlencode(__('Warranty request deleted', 'ultimatewoo-pro') ) );
        exit;
    }
    ...
    public function warranty_print() {
        ...
        if (! $product_name ) {
            ...
            if ( $product->is_type( 'variation' ) && version_compare( WC_VERSION, '3.0', '<' ) ) {
        ...
        if ( empty( $tracking ) ) {
            ...
            if ( !empty( $tracking['store'] ) ) {
                $tracking_html .= '<p><strong>'. __('Store:', 'ultimatewoo-pro') .'</strong> '. $warranty['return_tracking_code'];
            ...
            if ( !empty( $tracking['customer'] ) ) {
                $tracking_html .= '<p><strong>'. __('Customer:', 'ultimatewoo-pro') .'</strong> '. $warranty['tracking_code'];
    ...
    public function order_inline_edit_template() {
        ...
        $requests_str = array(
            'replacement'   => __('Replacement item', 'ultimatewoo-pro'),
            'refund'        => __('Refund', 'ultimatewoo-pro'),
            'coupon'        => __('Refund as store credit', 'ultimatewoo-pro')
        );
                ...
                <td colspan="<?php echo self::$shop_order_columns; ?>">
                    ...
                    ?>
                        <div class="warranty-request" id="warranty_request_<?php echo $request_id; ?>">
                            <h2><?php printf(__('RMA %s', 'ultimatewoo-pro'), $request['code']); ?></h2>
                            ...
                            <fieldset class="inline-edit-col-right">
                                <div class="inline-edit-col">
                                    ...
                                    <div class="field">
                                        <span class="label"><?php printf( __('%s &times; %d', 'ultimatewoo-pro'), get_the_title( $request_product['product_id'] ), $request_product['quantity'] ); ?></span>
                                    </div>
                                    <?php endforeach; ?>

                                    <h3><?php _e('RMA Data', 'ultimatewoo-pro'); ?></h3>

                                    <div class="field">
                                        <span class="label"><?php _e('Request Type:', 'ultimatewoo-pro'); ?></span>
                                    ...
                                    ?>
                                        <div class="field">
                                            <span class="label"><?php _e('Refunded:', 'ultimatewoo-pro'); ?></span>
                            ...
                            <fieldset class="inline-edit-col-left">
                                <div class="inline-edit-col">
                                    ...
                                    if ( $readonly ) {
                                        ...
                                        $status_content .= '</select>';
                                                    //<button class="button-primary warranty-update-status" type="button" title="Update" data-id="'. $request['ID'] .'" data-security="'. $nonces['status'] .'"><span>'. __('Update', 'ultimatewoo-pro') .'</span></button>';
                                    }
                                    ?>

                                    <h3><?php _e('RMA Status', 'ultimatewoo-pro'); ?></h3>
                                    <?php echo $status_content; ?>

                                    <div class="codes_form closeable">
                                        <h4><?php _e('Return shipping details', 'ultimatewoo-pro'); ?></h4>
                                        <?php
                                        ...
                                        if ( $shipping_label_id ) {
                                            ...
                                            echo '<a href="'. $lnk .'"><strong>'. __('Download the Shipping Label', 'ultimatewoo-pro') .'</strong></a>';
                                        } else {
                                            ?>
                                            ...
                                            <input class="rma-upload-button button" type="button" data-id="<?php echo $request_id; ?>" data-uploader_title="<?php _e('Set Shipping Label', 'ultimatewoo-pro'); ?>" data-uploader_button_text="<?php _e('Set Shipping Label', 'ultimatewoo-pro'); ?>" value="<?php _e('Select Shipping Label', 'ultimatewoo-pro'); ?>" />
                                        <?php
                                ...
                                <div class="inline-edit-col">
                                    <h3><?php _e('Return Tracking Details', 'ultimatewoo-pro'); ?></h3>
                                    ...
                                    ?>
                                    <div class="codes_form closeable">
                                        <div class="wc-tracking-requested warranty-updated <?php echo $class; ?>"><p><?php _e('Tracking information requested from customer', 'ultimatewoo-pro'); ?></p></div>
                                        ...
                                        if ($request['request_tracking_code'] != 'y'):
                                            ...
                                            <div class="request-tracking-div">
                                                <label>
                                                    ...
                                                    <strong><?php _e('Request tracking code from the Customer', 'ultimatewoo-pro'); ?></strong>
                                                </label>
                                            </div>
                                        <?php
                                            ...
                                            if (! empty($request['tracking_code']) ) {
                                                echo '<strong>'. __('Customer Provided Tracking', 'ultimatewoo-pro') .':</strong>&nbsp;';
                                                ...
                                                if ( !empty( $request['tracking_provider'] ) ) {
                                                    ...
                                                    printf( __('%s via %s (<a href="'. $link .'" target="_blank">Track Shipment</a>)', 'ultimatewoo-pro'), $request['tracking_code'], $provider, $link );
                                                } else {
                                    ...
                                    <div class="codes_form closeable">
                                        ...
                                        if ( !empty( $request['return_tracking_provider'] ) ): ?>
                                            <p>
                                                <label for="return_tracking_provider_<?php echo $request['ID']; ?>"><strong><?php _e('Shipping Provider', 'ultimatewoo-pro'); ?></strong></label>
                                            ...
                                            <p>
                                                <label for="return_tracking_code_<?php echo $request['ID']; ?>"><strong><?php _e('Tracking details', 'ultimatewoo-pro'); ?></strong></label>
                                                <input type="text" class="tracking_code regular-text" name="return_tracking_code" id="return_tracking_code_<?php echo $request['ID']; ?>" value="<?php echo $request['return_tracking_code']; ?>" placeholder="<?php _e('Enter the shipment tracking number', 'ultimatewoo-pro'); ?>" />
                                                <span class="description"><?php _e('Shipping Details/Tracking', 'ultimatewoo-pro'); ?></span>
                                            </p>
                                        <?php else: ?>
                                            <p>
                                                <label for="return_tracking_code_<?php echo $request['ID']; ?>"><strong><?php _e('Tracking details', 'ultimatewoo-pro'); ?></strong></label>
                            ...
                            <fieldset class="inline-edit-col-center">
                                <div class="inline-edit-col">
                                    <h3><?php _e('Actions', 'ultimatewoo-pro'); ?></h3>
                            ...
                            <div class="submit inline-edit-save">
                                <input type="button" class="button close_tr" value="<?php _e('Close', 'ultimatewoo-pro'); ?>" />

                                <div class="alignright">
                                    <a class="button-primary" target="_blank" href="<?php echo wp_nonce_url( 'admin-post.php?action=warranty_print&request='. $request['ID'], 'warranty_print' ); ?>"><?php _e('Print', 'ultimatewoo-pro'); ?></a>
                                    <input type="button" class="button-primary rma-update" data-id="<?php echo $request_id; ?>" data-security="<?php echo $update_nonce; ?>" value="<?php _e('Update', 'ultimatewoo-pro'); ?>" />
                                    <input type="button" class="button-secondary warranty-trash" data-id="<?php echo $request_id; ?>" data-security="<?php echo wp_create_nonce( 'warranty_delete' ); ?>" value="<?php _e('Delete', 'ultimatewoo-pro'); ?>" />
                                </div>
                            </div>
                            ...
                            ?>
                                <div id="warranty-refund-modal-<?php echo $request_id; ?>" style="display:none;">
                                    <table class="form-table">
                                        <tr>
                                            <th><span class="label"><?php _e('Amount refunded:', 'ultimatewoo-pro'); ?></span></th>
                                            <td><span class="value"><?php echo wc_price( $refunded ); ?></span></td>
                                        </tr>
                                        <tr>
                                            <th><span class="label"><?php _e('Item cost:', 'ultimatewoo-pro'); ?></span></th>
                                            <td><span class="value"><?php echo wc_price( $item_amount ); ?></span></td>
                                        </tr>
                                        <tr>
                                            <th><span class="label"><?php _e('Refund amount:', 'ultimatewoo-pro'); ?></span></th>
                                            <td>
                                    ...
                                    <p class="submit alignright">
                                        <input
                                            ...
                                            value="<?php _e('Process Refund', 'ultimatewoo-pro'); ?>"
                            ...
                            <?php elseif ( $request['request_type'] == 'coupon' ):
                                ...
                                <div id="warranty-coupon-modal-<?php echo $request_id; ?>" style="display:none;">
                                    <table class="form-table">
                                        ...
                                        <tr>
                                            <th><span class="label"><?php _e('Coupon amount:', 'ultimatewoo-pro'); ?></span></th>
                                            <td>
                                                <?php echo get_woocommerce_currency_symbol(); ?>
                                    ...
                                    <p class="submit alignright">
                                        <input
                                            ...
                                            value="<?php _e('Send Coupon', 'ultimatewoo-pro'); ?>"
                                            data-id="<?php echo $request_id; ?>"
    ...
    public function add_line_item_warranty_meta( $item_id, $item ) {
        ...
        if ( $warranty['type'] == 'addon_warranty' ) {
            ...
            if ( $warranty['length'] == 'lifetime' ) {
                wc_add_order_item_meta( $item_id, $warranty_label, __('Lifetime', 'ultimatewoo-pro') );
