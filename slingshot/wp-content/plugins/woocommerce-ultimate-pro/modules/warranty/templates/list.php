...
SQL;
    ...
    function get_columns(){
        $columns = array(
            'rma'           => __('Return Details', 'ultimatewoo-pro'),
            'products'      => __('Products', 'ultimatewoo-pro'),
            'request_type'  => __('Request Type', 'ultimatewoo-pro'),
            'date'          => __('Last Updated', 'ultimatewoo-pro'),
    ...
    function extra_tablenav( $which ) {
        if ( $which == 'top' ) {
            ...
            echo '          <option value="">'. __('All Statuses', 'ultimatewoo-pro') .'</option>';
            ...
            submit_button( __( 'Filter', 'ultimatewoo-pro' ), 'secondary', false, false, array( 'id' => 'post-query-submit' ) );
            echo '  </div>';
            echo '</form>';
        }
    }
    ...
    function column_status($item) {
        ...
        if ( $readonly ) {
            ...
            $content .= '</select>
            <button class="button update-status" type="button" title="Update" data-request_id="'. $item['ID'] .'"><span>'. __('Update', 'ultimatewoo-pro') .'</span></button>
            ';
        }
        return $content;
    }
    ...
    function column_rma($item) {
        ...
        $order_str = sprintf( __('Order %s', 'ultimatewoo-pro'), $item_order );

        $actions = array(
            'inline-edit' => '<a href="#" class="inline-edit" data-request_id="'. $item['ID'] .'">'. __('Manage', 'ultimatewoo-pro') .'</a>'
        );
        ...
        if ( $status == $returned && $manage_stock == 'yes' ) {
            if ( get_post_meta( $item['ID'], '_returned', true ) == 'yes' ) {
                $actions['inventory-return'] = __('Stock Returned', 'ultimatewoo-pro');
            } else {
                $actions['inventory-return'] = '<a href="'. wp_nonce_url( 'admin-post.php?action=warranty_return_inventory&id='. $item['ID'], 'warranty_return_inventory' ) .'">'. __('Return Stock', 'ultimatewoo-pro') .'</a>';
            }
        }
        ...
        if ( is_array( $refunds_status ) && in_array( $status, $refunds_status ) ) {
            ...
            if ( $request_type == 'refund' ) {
                $actions['item-refund'] = '<a class="thickbox" title="'. __('Refund', 'ultimatewoo-pro') .'" href="#TB_inline?width=400&height=250&inlineId=warranty-refund-modal-'. $item['ID'] .'">'. __('Refund Item', 'ultimatewoo-pro') .'</a>';
            } elseif ( $request_type == 'coupon' ) {
                $actions['item-coupon'] = '<a class="thickbox" title="'. __('Send Coupon', 'ultimatewoo-pro') .'" href="#TB_inline?width=400&height=250&inlineId=warranty-coupon-modal-'. $item['ID'] .'">'. __('Send Coupon', 'ultimatewoo-pro') .'</a>';
            }
        }

        $actions['trash'] = '<a href="'. wp_nonce_url( 'admin-post.php?action=warranty_delete&id='. $item['ID'], 'warranty_delete') .'" class="submitdelete warranty-delete">'. __('Delete', 'ultimatewoo-pro') .'</a>';

        $content = sprintf(__('<strong>%s</strong> by %s on %s', 'ultimatewoo-pro'), $item['code'], $customer, $order_str );
    ...
    function no_items() {
        _e( 'No requests found.', 'ultimatewoo-pro' );
    }
...
<div class="wrap woocommerce">
    ...
    <h2><?php _e('RMA Requests', 'ultimatewoo-pro'); ?></h2>
<?php
...
?>
    <form action="admin.php" method="get" style="margin-top: 20px;">
        ...
        <p class="search-box">
            <label class="screen-reader-text" for="search"><?php _e('Search', 'ultimatewoo-pro') ?>:</label>
            <input type="search" id="search" name="s" value="<?php _admin_search_query(); ?>" placeholder="RMA #" />
            <?php submit_button( __('Search', 'ultimatewoo-pro'), 'button', false, false, array('id' => 'search-submit') ); ?>
        </p>
    </form>
</div>
