<p class="order-info">
    ...
    printf(
        __('Order <mark class="order-number">%s</mark> made on <mark class="order-date">%s</mark>.', 'ultimatewoo-pro'),
...
<form method="get">
    <table width="100%" cellpadding="5" cellspacing="0" border="1px" class="warranty-table">
        <tr>
            ...
            <th><?php _e('Item', 'ultimatewoo-pro'); ?></th>
            <th><?php _e('Price', 'ultimatewoo-pro'); ?></th>
            <th><?php _e('Details', 'ultimatewoo-pro'); ?></th>
        </tr>
        <?php
        ...
        foreach ( $items as $item_idx => $item ) {
            ...
            if ( $item_warranty->is_expired() ) {
                $warranty_string .= '<br/><strong>'. __('Expired Warranty', 'ultimatewoo-pro') .'</strong>';
            }
        ?>
        <tr>
    ...
    <input type="submit" class="warranty-button button"  value="<?php echo get_option( 'warranty_button_text', __('Request Warranty', 'ultimatewoo-pro') ); ?>" />
    <?php endif; ?>
</form>

<h2><?php _e('Existing Requests', 'ultimatewoo-pro'); ?></h2>
<form method="get">
<table width="100%" cellpadding="5" cellspacing="0" border="1px" class="warranty-table">
    <tr>
        <th><?php _e('RMA', 'ultimatewoo-pro'); ?></th>
        <th><?php _e('Products', 'ultimatewoo-pro'); ?></th>
        <th><?php _e('Details', 'ultimatewoo-pro'); ?></th>
        <th><?php _e('Status', 'ultimatewoo-pro'); ?></th>
    </tr>
    <?php
    ...
    if ( $results ) {
        foreach ( $results as $result ) {
            ...
            $warranty_details[] = __( 'Updated ', 'ultimatewoo-pro' ) . date_i18n( 'F d, Y h:i A', strtotime( $result['post_modified'] ) );
            ...
            if ( $shipping_label_id ) {
                ob_start();
                ?>
                <strong><?php _e('Shipping Label', 'ultimatewoo-pro'); ?></strong><br/>
                <?php
                ...
                if ($post_mime_type == 'application/pdf'){
                    echo '<a href="'. $shipping_label_file_url .'"><img src="'.ULTIMATEWOO_MODULES_URL.'/warranty/assets/images/pdf-icon.png'.'" />&nbsp; '. __('Download' ,'wc_warranty');
                } else {
                    echo '<a href="'. $shipping_label_file_url .'">'. __('Download return label', 'ultimatewoo-pro') .'</a>';
                }
            ...
            if ( !empty( $result['refunded'] ) && $result['refunded'] == 'yes' ) {
                ...
                if ( $refund_amount && $refund_date ) {
                    ...
                    $warranty_details[] = sprintf(__('Item has been refunded the amount of %s on %s', 'ultimatewoo-pro'), wc_price( $refund_amount ), $pretty_date );
                } else {
                    $warranty_details[] = __('Item has been refunded', 'ultimatewoo-pro');
                }
    ...
    <tr>
        <td>
            ...
            <br/>
            <small>
                <?php echo __('created on ', 'ultimatewoo-pro') . date_i18n( wc_date_format() .' '. wc_time_format(), strtotime( $result['post_date'] ) ); ?>
            </small>
        </td>
        <td>
            <?php
