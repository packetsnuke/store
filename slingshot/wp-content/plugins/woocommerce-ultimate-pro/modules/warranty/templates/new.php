...
<div class="wrap woocommerce">
    ...
    <h2><?php _e('New Warranty Request', 'ultimatewoo-pro'); ?></h2>

    <div id="search_form" <?php if ($searched || $form_view) echo 'style="display:none;"'; ?>>
        <form action="admin.php" id="search_form" method="get">
            <h4><?php _e('Search for an Order', 'ultimatewoo-pro'); ?></h4>

            <input type="hidden" name="page" value="warranties-new" />

            <p>
                <select name="search_key" id="search_key">
                    <option value="order_id"><?php _e('Order Number', 'ultimatewoo-pro'); ?></option>
                    <option value="customer"><?php _e('Customer Name or Email', 'ultimatewoo-pro'); ?></option>
                </select>
                ...
                <?php if ( version_compare( WC_VERSION, '3.0', '<' ) ): ?>
                    <input
                        ...
                        data-placeholder="<?php _e('Search for a customer...', 'ultimatewoo-pro'); ?>"
                ...
                <?php else: ?>
                    <select
                        ...
                        name="search_term"
                        multiple="multiple"
                        placeholder="<?php _e('Search for a customer...', 'ultimatewoo-pro'); ?>"
                        style="width: 400px;">
                    </select>
                <?php endif; ?>

                <input type="submit" id="order_search_button" class="button-primary" value="<?php _e('Search', 'ultimatewoo-pro'); ?>" />
            </p>
        </form>
    </div>
...
<?php if ( $searched && empty($orders) ): ?>
    <div class="error"><p><?php _e('No orders found', 'ultimatewoo-pro'); ?></p></div>
<?php endif; ?>  
...
<?php if ( !empty($orders) ): ?>
    <table class="wp-list-table widefat fixed warranty" cellspacing="0">
        <thead>
            <tr>
                <th scope="col" id="order_id" class="manage-column column-order_id"><?php _e('Order ID', 'ultimatewoo-pro'); ?></th>
                <th scope="col" id="order_customer" class="manage-column column-order_customer"><?php _e('Customer', 'ultimatewoo-pro'); ?></th>
                <th scope="col" id="order_status" class="manage-column column-status"><?php _e('Order Status', 'ultimatewoo-pro'); ?></th>
                <th scope="col" id="order_items" class="manage-column column-order_items"><?php _e('Order Items', 'ultimatewoo-pro'); ?></th>
                <th scope="col" id="order_date" class="manage-column column-order_items"><?php _e('Date', 'ultimatewoo-pro'); ?></th>
            </tr>
        </thead>
        <tbody>
            <?php
            ...
            <tr class="alternate">
                ...
                <td class="order_items column-order_items">
                    <form method="get" action="admin.php">
                        ...
                        <input type="submit" class="button" value="<?php _e('Create Request', 'ultimatewoo-pro'); ?>" />
                    </form>
                </td>
