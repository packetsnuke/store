...
<div id="matrix_form">
						...
						<?php foreach ( $columns as $key => $field_data ): ?>
							...
							<td>
								<?php if ( $field_data ) : ?>
									...
									$vmsg = $variation->get_stock_quantity() ? sprintf(__('Only %s available', 'ultimatewoo-pro'), $variation->get_stock_quantity()) : sprintf(__('Currently unavailable', 'ultimatewoo-pro'));
									?>
	...
	<?php if ( ! $wc_bulk_variations->is_only_bulk_variation_form() ) : ?>
        <input class="button btn-back-to-single" type="button" value="<?php _e( '<-- Singular Order Form', 'ultimatewoo-pro' ); ?>" />
	<?php else : ?>
        <input class="button btn-back-to-product" type="button" value="<?php _e( '<-- Product Page', 'ultimatewoo-pro' ); ?>" />
	<?php endif; ?>
