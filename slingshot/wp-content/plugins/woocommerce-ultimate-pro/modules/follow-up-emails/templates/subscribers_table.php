...
<div class="wrap">
	<h2>
		<?php _e('Manage Lists & Subscribers', 'ultimatewoo-pro'); ?>
	</h2>

	<?php if (isset($_GET['deleted']) && $_GET['deleted'] > 0): ?>
		<div id="message" class="updated"><p><?php printf( _n('1 email has been deleted', '%d emails have been deleted', intval($_GET['deleted']), 'ultimatewoo-pro'), intval($_GET['deleted'])); ?></p></div>
	<?php endif; ?>

	<?php if (isset($_GET['added'])): ?>
		<div id="message" class="updated"><p><?php printf(_n('1 subscriber has been added', '%d subscribers have been added', intval($_GET['added']), 'ultimatewoo-pro'), intval($_GET['added'])); ?></p></div>
	<?php endif; ?>

	<?php if (isset($_GET['added_subscriber'])): ?>
		<div id="message" class="updated"><p><?php printf( __('%s has been added', 'ultimatewoo-pro'), wp_kses_post( sanitize_email( $_GET['added_subscriber'] ) ) ); ?></p></div>
	<?php endif; ?>

	<?php if (isset($_GET['imported'])): ?>
		<div id="message" class="updated"><p><?php printf( _n('1 has been added', '%d emails have been added', intval($_GET['imported']), 'ultimatewoo-pro'), intval($_GET['imported'])); ?></p></div>
	<?php endif; ?>
	...
	<form action="admin-post.php" method="post" enctype="multipart/form-data">
		...
		<div class="tablenav top">
			<div class="tablenav-pages">
				<span class="displaying-num"><?php printf( _n( '%d item', '%d items', $newsletter->found_subscribers, 'ultimatewoo-pro' ), $newsletter->found_subscribers ); ?></span>
			...
			<div class="alignleft actions bulkactions">
				...
				<input type="submit" name="button_add" id="post-query-submit" class="button button-secondary" value="<?php _e('Add single subscriber', 'ultimatewoo-pro'); ?>">
			</div>
			...
			<div class="alignright actions">
				<select id="filter_list">
					...
					<option value="-1" <?php selected( $selected_filter, -1 ); ?>><?php _e('Filter by list', 'ultimatewoo-pro'); ?></option>
				...
				<button type="button" class="button run-filter"><?php _e('Filter', 'ultimatewoo-pro'); ?></button>
			</div>
			<br class="clear">
		</div>
			...
			<tr>
				...
				<th scope="col" class="manage-column column-email_address <?php echo $email_order_class; ?> <?php echo $email_order; ?>" style="">
					<a href="<?php echo add_query_arg( array('orderby' => 'email', 'order' => $new_email_order), $base_url ); ?>">
						<span><?php _e('Email Address', 'ultimatewoo-pro'); ?></span> <span class="sorting-indicator"></span>
					</a>
				</th>
				<th scope="col" class="manage-column column-email_list" style=""><?php _e('Subscriptions', 'ultimatewoo-pro'); ?></th>
				<th scope="col" class="manage-column column-date <?php echo $date_order_class; ?> <?php echo $date_order; ?>" style="width:200px;">
					<a href="<?php echo add_query_arg( array('orderby' => 'date_added', 'order' => $new_date_order), $base_url ); ?>">
						<span><?php _e('Signup Date/Time', 'ultimatewoo-pro'); ?></span> <span class="sorting-indicator"></span></th>
					</a>
		...
		<div class="tablenav bottom">
			...
			<div class=" actions bulkactions">
				...
				<select name="action2" id="action2" style="max-width: 300px;">
					...
					<option value="move"><?php _e('Move subscriber to existing list', 'ultimatewoo-pro'); ?></option>
					<option value="new"><?php _e('Create new list and move subscriber', 'ultimatewoo-pro'); ?></option>
					<option value="delete"><?php _e('Remove subscriber and email address', 'ultimatewoo-pro'); ?></option>
				</select>

				<div id="lists" style="display:none;">
					<select name="list[]" id="list" class="select2" multiple style="width: 300px; clear: both;" data-placeholder="<?php _e('Select lists', 'ultimatewoo-pro'); ?>">
						<option></option>
						<?php foreach ( $lists as $list ): ?>
				...
				<div id="new_list" style="display: none; clear: both;">
					<input type="text" name="new_list_name" id="new_list_name" placeholder="<?php _e('List name', 'ultimatewoo-pro'); ?>" style="float: left;" />
				</div>
			...
			<h3><?php _e('Bulk add subscribers', 'ultimatewoo-pro'); ?></h3>

			<p><?php _e('Import your existing mailing lists and email addresses.', 'ultimatewoo-pro'); ?></p>    

			<p class="form-field">
				<input type="file" name="csv" />
			</p>
			<p class="submit">
				<input type="submit" class="button-primary" name="upload" value="<?php _e('Upload list', 'ultimatewoo-pro'); ?>" />
			</p>
			</div>
...
<script>
	jQuery(document).ready(function($) {
		$(".btn-new-list").click(function() {
			var name = prompt("<?php _e('List Name:', 'ultimatewoo-pro'); ?>")
