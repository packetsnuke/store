...
<h2 class="nav-tab-wrapper woo-nav-tab-wrapper">
	<a href="<?php echo get_edit_user_link( $user_id ); ?>" class="nav-tab"><?php _e('Personal Options', 'ultimatewoo-pro'); ?></a>
	<a href="#" class="nav-tab nav-tab-active"><?php _e('Customer Data', 'ultimatewoo-pro'); ?></a>
</h2>
<?php endif; ?>
...
<div id="fue_user_report">
	<div class="col-left">
		...
		<table class="widefat fixed striped posts">
			<thead>
			<tr>
				<th scope="col" id="email_name" class="manage-column column-email_name" style=""><?php _e('Email', 'ultimatewoo-pro'); ?></th>
				<th scope="col" id="trigger" class="manage-column column-trigger" style=""><?php _e('Trigger', 'ultimatewoo-pro'); ?></th>
				<th scope="col" id="opened" class="manage-column column-opened"><?php _e('Opened', 'ultimatewoo-pro'); ?></th>
				<th scope="col" id="clicked" class="manage-column column-clicked"><?php _e('Clicked', 'ultimatewoo-pro'); ?></th>
				<?php do_action('fue_report_customer_emails_header'); ?>
				<th scope="col" id="date_sent" class="manage-column column-date_sent" style=""><?php _e('Date Sent', 'ultimatewoo-pro'); ?></th>
			...
			if ( empty($reports) ):
				?>
				<tr scope="row">
					<th colspan="6"><?php _e('No reports available', 'ultimatewoo-pro'); ?></th>
				</tr>
			<?php
			else:
				foreach ($reports as $report):
					...
					<tr scope="row">
						...
						<td><?php echo ($opens > 0) ? __('<span class="dashicons dashicons-visibility" style="color:#7AD03A;"></span> Yes', 'ultimatewoo-pro') : __('<span class="dashicons dashicons-hidden" style="color:#EEE;"></span> No', 'ultimatewoo-pro'); ?></td>
						<td><?php echo ($clicks > 0) ? __('<span class="dashicons dashicons-carrot" style="color:#7AD03A;"></span> Yes', 'ultimatewoo-pro') : __('<span class="dashicons dashicons-carrot" style="color:#EEE;"></span> No', 'ultimatewoo-pro'); ?></td>
		...
		<h3>
			<?php
			_e('Scheduled Emails', 'ultimatewoo-pro');
			...
			<a class="button button-secondary" href="<?php echo $url; ?>" style="float: right;">
				<?php _e('Clear Cart Emails', 'ultimatewoo-pro'); ?>
			</a>
		</h3>
		...
		<table class="widefat fixed striped posts">
			...
			<tr>
				...
				<th scope="col" id="status" class="manage-column column-status" style=""><?php _e('Status', 'ultimatewoo-pro'); ?></th>
				<th scope="col" id="date_sent" class="manage-column column-date_sent" style="" width="180"><?php _e('Scheduled', 'ultimatewoo-pro'); ?></th>
			...
			<?php if ( empty($queue) ): ?>
				<tr>
					<td colspan="5"><?php _e('No emails scheduled', 'ultimatewoo-pro'); ?></td>
				</tr>
			<?php
			else:
				$email_rows     = array();
				...
				foreach ( $queue as $row ):
					...
					<tr>
						...
						<td>
							<?php
							printf(
								__('<a href="%s">#%d %s</a><br/><small>(%s)</small>', 'ultimatewoo-pro'),
						...
						<td class="status">
							<?php
							if ( $row->status == 1 ) {
								echo __('Queued', 'ultimatewoo-pro');
								echo '<br/><small><a href="#" class="queue-toggle" data-status="queued" data-id="'. $row->id .'">'. __('Do not send', 'ultimatewoo-pro') .'</a></small>';
							} else {
								echo __('Suspended', 'ultimatewoo-pro');
								echo '<br/><small><a href="#" class="queue-toggle" data-status="paused" data-id="'. $row->id .'">'. __('Re-enable', 'ultimatewoo-pro') .'</a></small>';
							}
							?>
						</td>
						<td>
		...
		<h3><?php _e('Conversions', 'ultimatewoo-pro'); ?></h3>

		<table class="wp-list-table widefat fixed striped posts">
			<thead>
			<tr>
				<th><?php _e('Email Received', 'ultimatewoo-pro'); ?></th>
				<th><?php _e('Order #', 'ultimatewoo-pro'); ?></th>
				<th><?php _e('Conversion Value', 'ultimatewoo-pro'); ?></th>
			...
			<?php if ( empty($conversions) ): ?>
				<tr>
					<td colspan="5"><?php _e('No conversions found', 'ultimatewoo-pro'); ?></td>
				</tr>
			<?php
			else:
				/* @var $order WC_Order */
		...
		<h3><?php _e('Abandoned Items', 'ultimatewoo-pro'); ?></h3>

		<?php if ( $cart_updated ): ?>
			<p><?php printf( __('Last updated: %s', 'ultimatewoo-pro'), date( wc_date_format() .' '. wc_time_format(), strtotime( $cart_updated ) ) ); ?></p>
			<p><?php printf( __('Status: %s', 'ultimatewoo-pro'), FUE_Addon_Woocommerce_Cart::get_cart_status( $user_id ) ); ?></p>
		 <?php endif; ?>
		<table class="wp-list-table widefat fixed striped posts">
			<thead>
			<tr>
				<th>&nbsp;</th>
				<th><?php _e('Product', 'ultimatewoo-pro'); ?></th>
				<th><?php _e('Quantity', 'ultimatewoo-pro'); ?></th>
				<th><?php _e('Price', 'ultimatewoo-pro'); ?></th>
			...
			<?php if ( empty($cart) ): ?>
				<tr>
					<td colspan="5"><?php _e('No saved cart items', 'ultimatewoo-pro'); ?></td>
				</tr>
			<?php
			else:
				/* @var $order WC_Order */
		...
		<h3><?php _e('Opt-Outs', 'ultimatewoo-pro'); ?></h3>

		<table class="widefat fixed striped posts">
			<thead>
			<tr>
				<th scope="col" id="exclude_user_email" class="manage-column column-user_email" style=""><?php _e('Email', 'ultimatewoo-pro'); ?></th>
				<th scope="col" id="exclude_order" class="manage-column column-order" style=""><?php _e('Order', 'wc_folloup_emails'); ?></th>
				<th scope="col" id="exclude_date_added" class="manage-column column-date_added" style=""><?php _e('Date Added', 'ultimatewoo-pro'); ?></th>
			...
			<?php if ( empty($excludes) ): ?>
				<tr>
					<td colspan="4"><?php _e('No opt-outs found', 'ultimatewoo-pro'); ?></td>
				</tr>
			<?php
			else:
				$date_format    = get_option('date_format') .' '. get_option('time_format');
				foreach ( $excludes as $row ):
					$order_str = ( empty( $row->order_id ) ) ? __('All emails', 'ultimatewoo-pro') : apply_filters( 'woocommerce_order_number', $row->order_id );
					?>
					<tr>
		...
		<h3><?php _e('Order History', 'ultimatewoo-pro'); ?></h3>

		<table class="widefat fixed striped posts">
			<thead>
			<tr>
				<th scope="col" class="manage-column" style=""><?php _e( 'Order (status)', 'ultimatewoo-pro' ); ?></th>
				<th scope="col" class="manage-column" style=""><?php _e( 'Date', 'ultimatewoo-pro' ); ?></th>
				<th scope="col" class="manage-column" style=""><?php _e( 'Total', 'ultimatewoo-pro' ); ?></th>
				...
				foreach ( $orders as $key => $purchase ) {
					...
					<tr>
						<td>
							...
							printf( __('<a href="%s">%s</a> <br /> (%s)', 'ultimatewoo-pro' ), $url, $purchase_order->get_order_number(), WC_FUE_Compatibility::get_order_prop( $purchase_order, 'status' ) );
							?>
						</td>
						...
						<td>
							...
							if ( isset( $report ) && $report->order_id != 0) {
								printf( __('<a class="button" href="%s">View Order</a>', 'ultimatewoo-pro' ), $url );
							} else {
								echo '-';
							}
							?>
		...
		<p><?php printf( __( '<strong>Lifetime Value:</strong> %s', 'ultimatewoo-pro' ), '<span style="color:#7EB03B; font-size:1.2em; font-weight:bold;">' . wc_price( $lifetime_total ) . '</span>' ); ?></p>

	</div>
	<div class="col-right">
		<div class="postbox" id="fue_customer_followups">
			<h3 class="handle"><?php _e('Schedule Emails', 'ultimatewoo-pro'); ?></h3>
			<div class="inside">
				<p id="schedule_email_error"></p>
				<p id="schedule_email_success"></p>
				<p>
					<label for="email"><?php _e('Select Email', 'ultimatewoo-pro'); ?></label>
					<br/>
				...
				<p>
					<label for="send_schedule"><?php _e('Send', 'ultimatewoo-pro'); ?></label>
					<select id="send_schedule">
						<option value="now"><?php _e('now', 'ultimatewoo-pro'); ?></option>
						<option value="later"><?php _e('later', 'ultimatewoo-pro'); ?></option>
					</select>
				</p>
				<p class="send-later">
					<input type="text" id="send_date" class="datepicker" placeholder="mm/dd/yyyy" />

					<?php _e('at', 'ultimatewoo-pro'); ?>
					...
					<select id="send_time_ampm">
						<option value="am"><?php _e('AM', 'ultimatewoo-pro'); ?></option>
						<option value="pm"><?php _e('PM', 'ultimatewoo-pro'); ?></option>
					</select>
				</p>
				<p class="send-again-p">
					<label>
						<input type="checkbox" id="send_again" />
						<?php _e('Send again', 'ultimatewoo-pro'); ?>
					</label>

					<span class="send-again">
						<?php _e('in', 'ultimatewoo-pro'); ?>
						<input type="number" min="1" id="send_again_value" />
						<select id="send_again_interval">
							<option value="minutes"><?php _e('minutes', 'ultimatewoo-pro'); ?></option>
							<option value="hours"><?php _e('hours', 'ultimatewoo-pro'); ?></option>
				...
				<p class="separated">
					<a class="schedule-email button-primary" href="#"><?php _e('Schedule Email', 'ultimatewoo-pro'); ?></a>
				</p>
			</div>
		</div>
		<?php if ( $customer ): ?>
		<div class="postbox" id="fue_customer_reminders">
			<h3 class="handle"><?php _e('Reminders', 'ultimatewoo-pro'); ?></h3>
			<div class="inside">
				<ul class="customer-reminders">
					<?php if ( empty( $reminders ) ): ?>
						<li><?php _e('There are no reminders yet', 'ultimatewoo-pro'); ?></li>
					<?php
					else:
						...
						foreach( $reminders as $reminder ):
							...
							if ( $assignee->ID == $author->ID ) {
								$meta = sprintf( __('added by %s', 'ultimatewoo-pro'), $author->display_name );
							} else {
								$meta = sprintf( __('assigned to %s by %s', 'ultimatewoo-pro'), $assignee->display_name, $author->display_name );
							}
							?>
							<li class="reminder" rel="<?php echo esc_attr( $reminder->id ); ?>">
								<div class="reminder-content">
									<p>
										<?php printf( __('Reminder set for %s at %s', 'ultimatewoo-pro'), $date, $time ); ?>
									</p>
								...
								<p class="meta">
									<?php echo $meta; ?>
									<a class="delete_reminder" href="#"><?php _e('Delete', 'ultimatewoo-pro'); ?></a>
								</p>
							</li>
						<?php
						endforeach;
				...
				<div class="add-reminder">
					<h4><?php _e('Add Reminder', 'ultimatewoo-pro'); ?></h4>

					<p>
						<textarea id="reminder_note" placeholder="<?php _e('Reminder notes', 'ultimatewoo-pro'); ?>"></textarea>
					</p>

					<p>
						<label>
							<input type="checkbox" id="assign_reminder" />
							<?php _e( 'Assign reminder', 'ultimatewoo-pro' ); ?>
						</label>
					</p>
					...
					<p id="assignee_block">
						<input
							...
							value=""
							data-selected=""
							data-placeholder="<?php _e('Search for a user...', 'ultimatewoo-pro'); ?>"
							name="assignee"
							id="assignee"
					...
					<p class="separated">
						<?php _e('Send in', 'ultimatewoo-pro'); ?>
						<input type="number" min="1" step="1" id="reminder_interval_days" value="1" size="3" />
						<?php _e('day(s)', 'ultimatewoo-pro'); ?>

						<a class="set_interval_reminder button" href="#"><?php _e('Set Reminder', 'ultimatewoo-pro'); ?></a>
					</p>
					
					<p class="separated">
						<?php _e('Send on', 'ultimatewoo-pro'); ?>
						<br/>
						...
						<a class="set_date_reminder button" href="#"><?php _e('Set Reminder', 'ultimatewoo-pro'); ?></a>
					</p>
					<br class="clear"/>
				</div>
		...
		<div class="postbox" id="fue_customer_notes">
			<h3 class="handle"><?php _e('Customer Notes', 'ultimatewoo-pro'); ?></h3>
			<div class="inside">
				<ul class="customer-notes">
					<?php if ( empty( $notes ) ): ?>
					<li><?php _e('There are no notes yet', 'ultimatewoo-pro'); ?></li>
					<?php
					else:
					...
					<li class="note" rel="<?php echo esc_attr( $note->id ); ?>">
						...
						<p class="meta">
							...
							<a class="delete_note" href="#"><?php _e('Delete note', 'ultimatewoo-pro'); ?></a>
						</p>
					</li>
					<?php
						endforeach;
				...
				<div class="add-note">
					<h4><?php _e('Add Note', 'ultimatewoo-pro'); ?></h4>
					...
					<p>
						...
						<a class="add_note button" href="#"><?php _e('Add', 'ultimatewoo-pro'); ?></a>
					</p>
				</div>
			</div>
		</div>
