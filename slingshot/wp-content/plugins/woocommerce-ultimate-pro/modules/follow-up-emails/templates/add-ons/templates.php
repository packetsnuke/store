<div class="wrap fue-addons-wrap">
	<h2>
		<?php _e('Templates', 'ultimatewoo-pro'); ?>
		<a class="add-new-h2" href="admin.php?page=followup-emails-templates&action=new"> <?php _e('Create New Follow-Up', 'ultimatewoo-pro'); ?> </a>
	</h2>
	...
	if ( isset( $_POST['action'] ) ) {
		if ( $_POST['action'] == 'template_create' ) {
			...
			if ( isset($_FILES['template']['tmp_name'] ) && is_uploaded_file( $_FILES['template']['tmp_name'] ) ) {
				...
				show_message( __('No file selected or file is too large', 'ultimatewoo-pro') );
			}
		}
	} else {
