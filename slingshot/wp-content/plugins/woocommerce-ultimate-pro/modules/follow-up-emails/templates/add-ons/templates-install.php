...
if ( !file_exists( $fue_dir ) ) {
	...
	if ( ! $wp_filesystem->mkdir($fue_dir) && ! $wp_filesystem->is_dir( $fue_dir ) ) {
		...
		if ( !file_exists( $fue_dir ) ) {
			show_message( sprintf( __( 'Could not create directory: %s.', 'ultimatewoo-pro' ), $fue_dir ) );
			return;
		}
...
if ( !file_exists( $emails_dir ) ) {
	if ( ! $wp_filesystem->mkdir($emails_dir, FS_CHMOD_DIR) && ! $wp_filesystem->is_dir( $emails_dir ) ) {
		...
		if ( !file_exists( $emails_dir ) ) {
			show_message( sprintf( __( 'Could not create directory.', 'ultimatewoo-pro' ), $emails_dir ) );
			return;
		}
...
show_message( __('Downlading template...', 'ultimatewoo-pro' ) );
...
if ( !isset( $templates->$template_id ) ) {
	show_message( __( 'Could not find the template requested.', 'ultimatewoo-pro' ) );
	return;
}
...
if ( !$wp_filesystem->put_contents( $emails_dir .'/'. $template_file, $contents, FS_CHMOD_FILE ) ) {
	show_message( sprintf( __('Error saving file to %s', 'ultimatewoo-pro' ), $emails_dir .'/'. $template_file ) );
	return;
}

show_message( sprintf( __('Template %s installed!', 'ultimatewoo-pro'), $template_file ) );
show_message( '<a href="admin.php?page=followup-emails-templates">'. __('Go back', 'ultimatewoo-pro') .'</a>');