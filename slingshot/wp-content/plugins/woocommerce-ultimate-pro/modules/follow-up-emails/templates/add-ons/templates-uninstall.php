...
if ( !file_exists( $_dir .'/'. $file ) ) {
	show_message( sprintf( __('The template %s does not exist', 'ultimatewoo-pro'), $_dir .'/'. $file ) );
	return;
}

$tpl = new FUE_Email_Template( $_dir .'/'. $file );

show_message( __('Removing template...', 'ultimatewoo-pro' ) );

if ( !$wp_filesystem->delete( $_dir .'/'. $file ) ) {
	show_message( sprintf( __('Error removing the file: %s', 'ultimatewoo-pro' ), $_dir .'/'. $file ) );
	return;
}

show_message( sprintf( __('Template <b>%s</b> uninstalled!', 'ultimatewoo-pro'), $tpl->name ) );
