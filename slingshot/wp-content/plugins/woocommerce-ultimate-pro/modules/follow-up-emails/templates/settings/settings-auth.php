...
<form action="admin-post.php" method="post" enctype="multipart/form-data">
	...
	<h3><?php _e('DomainKeys Identified Mail (DKIM)', 'ultimatewoo-pro'); ?></h3>
	...
	<table class="form-table">
		<tr>
			<th><label for="dkim_enabled"><?php _e('Enable DKIM', 'ultimatewoo-pro'); ?></label></th>
			<td>
				<input type="checkbox" name="dkim[enabled]" id="dkim_enabled" value="1" <?php checked( 1, $dkim['enabled'] ); ?> />
			</td>
		</tr>
		<tr class="dkim">
			<th><label for="dkim_domain"><?php _e('DKIM Domain', 'ultimatewoo-pro'); ?></label></th>
			<td><input type="text" name="dkim[domain]" id="dkim_domain" value="<?php echo esc_attr( $dkim['domain'] ); ?>" /></td>
		</tr>
		<tr class="dkim">
			<th><label for="dkim_selector"><?php _e('DKIM Selector Prefix', 'ultimatewoo-pro'); ?></label></th>
			<td><input type="text" name="dkim[selector]" id="dkim_selector" value="<?php echo esc_attr( $dkim['selector'] ); ?>" /></td>
		</tr>
		<tr class="dkim">
			<th><label for="dkim_identity"><?php _e('DKIM Identity', 'ultimatewoo-pro'); ?></label></th>
			<td>
				<input type="text" name="dkim[identity]" id="dkim_identity" value="<?php echo esc_attr( $dkim['identity'] ); ?>" />
				<p class="description"><?php _e('Optional. Usually the email address used as the source of the email', 'ultimatewoo-pro'); ?></p>
			</td>
		</tr>
		<tr class="dkim">
			<th><label for="dkim_passphrase"><?php _e('DKIM Passphrase', 'ultimatewoo-pro'); ?></label></th>
			<td>
				<input type="text" name="dkim[passphrase]" id="dkim_passphrase" value="<?php echo esc_attr( $dkim['passphrase'] ); ?>" />
				<p class="description"><?php _e('Optional. Used if your key is encrypted.', 'ultimatewoo-pro'); ?></p>
			</td>
		</tr>
		...
		<tr class="dkim">
		  <?php if ( !function_exists( 'openssl_pkey_new' ) ): ?>
			  <th>&nbsp;</th>
			  <td>
				<?php _e('To generate new keys, please enable the OpenSSL extension in your PHP installation.', 'ultimatewoo-pro'); ?>
			  </td>
		  <?php else: ?>
			<th><label for="dkim_key_size"><?php _e('Key Size', 'ultimatewoo-pro'); ?></label></th>
			<td>
	...
	<table class="form-table dkim">
	  <tr>
		  <th>&nbsp;</th>
		  <td>
			  <input type="button" class="button-secondary generate-dkim-keys" value="<?php _e('Generate DKIM Keys', 'ultimatewoo-pro'); ?>" />
		...
		<tr>
			<th><label for="dkim_public_key"><?php _e('DKIM Public Key', 'ultimatewoo-pro'); ?></label></th>
			<td><textarea name="dkim[public_key]" id="dkim_public_key" rows="5" cols="80"><?php echo esc_attr( $dkim['public_key'] ); ?></textarea></td>
		</tr>
		<tr>
			<th><label for="dkim_private_key"><?php _e('DKIM Private Key', 'ultimatewoo-pro'); ?></label></th>
		...
		?>
			<tr>
				<th><?php _e('DNS Entry', 'ultimatewoo-pro'); ?></th>
				<td>
					<p>
						<strong><?php _e('Host/Name', 'ultimatewoo-pro'); ?>:</strong>
						<br/>
						<pre><code><?php echo $dkim['selector'] .'._domainkey.'. $dkim['domain']; ?></code></pre>
					</p>
					<p>
						<strong><?php _e('TXT Value', 'ultimatewoo-pro'); ?>:</strong>
						<br/>
	...
	<h3><?php _e('Sender Policy Framework (SPF)', 'ultimatewoo-pro'); ?></h3>
	...
	<table class="form-table">
		<tr>
			<th><label for="spf_enabled"><?php _e('Enable SPF', 'ultimatewoo-pro'); ?></label></th>
			<td>
		...
		<tr class="spf">
			...
			<td>
				...
				<p class="description"><?php _e('Optional', 'ultimatewoo-pro'); ?></p>
			</td>
		</tr>
		<tr class="spf">
			<th><label for="spf_domain"><?php _e('Your Domain', 'ultimatewoo-pro'); ?></label></th>
			<td>
				<input type="text" name="spf[domain]" id="spf_domain" size="50" placeholder="example.com" value="<?php echo esc_attr( $spf['domain'] ); ?>" />
				<input type="button" class="button validate-spf" value="<?php _e('Check SPF Record', 'ultimatewoo-pro'); ?>" />
		...
		<tr class="spf-dns-result" style="display: none;">
			...
			<td>
				<div class="success">
					<p>
						<span class="dashicons dashicons-yes"></span> <?php echo __('SPF record found for <code id="spf_result_domain"></code> - <code id="spf_result_data"></code>', 'ultimatewoo-pro' ); ?>
					</p>
				</div>
			</td>
		</tr>
		...
		<tr class="spf-result">
			<th><label for="spf_dns"><?php _e('DNS Entry', 'ultimatewoo-pro'); ?></label></th>
			<td>
				<div id="spf_dns"><?php echo $spf['spf']; ?></div>
				<input type="hidden" name="spf[spf]" id="spf_spf" value="<?php echo esc_attr( $spf['spf'] ); ?>" />
				<p class="description">
					<?php _e('You should add this DNS record to your domain\'s DNS configuration.', 'ultimatewoo-pro'); ?>
				</p>
			</td>
		</tr>
	</table>
