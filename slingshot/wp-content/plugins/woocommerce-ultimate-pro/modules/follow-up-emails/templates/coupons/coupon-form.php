...
<div class="icon32"><img src="<?php echo FUE_TEMPLATES_URL .'/images/send_mail.png'; ?>" /></div>
	<h2>
		<?php
		if ( $action == 'create' ) {
			_e('Create a New Coupon', 'ultimatewoo-pro');
		} else {
			_e('Update Coupon', 'ultimatewoo-pro');
		}
		?>
	</h2>
	<form action="admin-post.php" method="post">
				...
				<div class="postbox-container" id="postbox-container-2" style="float:none;">
					<div id="normal-sortables">
						<div id="woocommerce-coupon-data" class="postbox">
							...
							<h3 class="hndle"><span><?php _e('Coupon Data', 'ultimatewoo-pro'); ?></span></h3>
							<div class="inside">
								<div id="coupon_options" class="panel-wrap coupon_data" style="padding-top: 0px;">
									...
									<div id="general_coupon_data" class="panel woocommerce_options_panel">
										<div class="options_group">
											<p class="form-field">
												<label for="name"><?php _e('Name', 'ultimatewoo-pro'); ?></label>
												<input type="text" name="name" id="name" value="<?php echo esc_attr($data['name']); ?>" class="short" />
												<span class="description"><?php _e('For internal use only', 'ultimatewoo-pro'); ?></span>
											</p>
											<p class="form-field">
												<label for="prefix"><?php _e('Coupon Prefix', 'ultimatewoo-pro'); ?></label>
												<input type="text" name="prefix" id="prefix" value="<?php echo esc_attr($data['prefix']); ?>" class="input-text sized" size="25" />
												<select id="prefixes">
													<option value=""><?php _e('Choose a Variable', 'ultimatewoo-pro'); ?></option>
													<option value="{customer_first_name}"><?php _e('Customer\'s First Name', 'ultimatewoo-pro'); ?></option>
													<option value="{customer_last_name}"><?php _e('Customer\'s Last Name', 'ultimatewoo-pro'); ?></option>
												</select>
												<span class="description"><?php _e('Add a prefix to the generated coupon code', 'ultimatewoo-pro'); ?></span>
											</p>
											<p class="form-field">
												<label for="type"><?php _e('Discount type', 'ultimatewoo-pro'); ?></label>
											...
											<p class="form-field">
												<label for="amount"><?php _e('Coupon Amount', 'ultimatewoo-pro'); ?></label>
												<input type="text" name="amount" id="amount" class="short" value="<?php echo esc_attr($data['amount']); ?>" placeholder="0.0" />
												<span class="description"><?php _e('e.g. 5.99 (do not include the percent symbol)', 'ultimatewoo-pro'); ?></span>
											</p>
											<p class="form-field">
												<label for="free_shipping"><?php _e('Allow free shipping', 'ultimatewoo-pro'); ?></label>
										...
										<div class="options_group">
											<p class="form-field">
												...
												<select name="expiry_type">
													...
													<option value="days" <?php if ($data['expiry_type'] == 'days') echo 'selected'; ?>><?php _e('days', 'ultimatewoo-pro'); ?></option>
													<option value="weeks" <?php if ($data['expiry_type'] == 'weeks') echo 'selected'; ?>><?php _e('weeks', 'ultimatewoo-pro'); ?></option>
													<option value="months" <?php if ($data['expiry_type'] == 'months') echo 'selected'; ?>><?php _e('months', 'ultimatewoo-pro'); ?></option>
												</select>
												<span class="description"><?php _e('after the discount has been sent to the user', 'ultimatewoo-pro'); ?></span>
											</p>
										</div>
									</div>
									<div id="usage_restriction_coupon_data" class="panel woocommerce_options_panel">
										...
										<div class="options_group">
											<p class="form-field">
												<label for="individual"><?php _e('Individual use', 'ultimatewoo-pro'); ?></label>
												<input type="checkbox" class="checkbox" name="individual_use" id="individual" value="yes" <?php if ($data['individual'] != 0) echo 'checked'; ?> />
												<span class="description"><?php _e('Check this box if the coupon cannot be used in conjunction with other coupons', 'ultimatewoo-pro'); ?></span>
											</p>

											<p class="form-field">
												<label for="exclude_sale_items"><?php _e('Exclude sale items', 'ultimatewoo-pro'); ?></label>
												...
												<span class="description"><?php _e('Check this box if the coupon should not apply to items on sale. Per-item coupons will only work if the item is not on sale. Per-cart coupons will only work if there are no sale items in the cart.', 'ultimatewoo-pro'); ?></span>
											</p>
										</div>
										...
										<div class="options_group">
											<p class="form-field">
												...
												<span class="description"><?php _e('Products which need to be in the cart to use this coupon or, for &quot;Product Discounts&quot;, which products are discounted.', 'ultimatewoo-pro'); ?></span>
											</p>
											<p class="form-field">
												...
												<span class="description"><?php _e('Products which must not be in the cart to use this coupon or, for &quot;Product Discounts&quot;, which products are not discounted.', 'ultimatewoo-pro'); ?></span>
											</p>
										</div>
										<div class="options_group">
											<p class="form-field">
												...
												<span class="description"><?php _e('A product must be in this category for the coupon to remain valid or, for &quot;Product Discounts&quot;, products in these categories will be discounted.', 'ultimatewoo-pro'); ?></span>
											</p>
											<p class="form-field">
												...
												<span class="description"><?php _e('Product must not be in this category for the coupon to remain valid or, for &quot;Product Discounts&quot;, products in these categories will not be discounted.', 'ultimatewoo-pro'); ?></span>
											</p>
										</div>
									</div>
									<div id="usage_limit_coupon_data" class="panel woocommerce_options_panel">
										<p class="form-field usage_limit_field">
											...
											<span class="description"><?php _e('How many times this coupon can be used before it is void', 'ultimatewoo-pro'); ?></span>
										</p>
										<p class="form-field usage_limit_per_user_field">
											...
											<span class="description"><?php _e('How many times this coupon can be used by an individual user. Uses billing email for guests, and user ID for logged in users.', 'ultimatewoo-pro'); ?></span>
										</p>
									</div>
								</div>
								<div class="clear"></div>
		...
		<p class="submit">
			...
			<?php if ( $action == 'create' ): ?>
				<input type="submit" name="save" value="<?php _e('Create Coupon', 'ultimatewoo-pro'); ?>" class="button-primary" />
			<?php else: ?>
				<input type="hidden" name="id" value="<?php echo $data['id']; ?>" />
				<input type="submit" name="save" value="<?php _e('Update Coupon', 'ultimatewoo-pro'); ?>" class="button-primary" />
			<?php endif; ?>
		</p>
	</form>
</div>
