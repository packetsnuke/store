...
class FUE_Email {
		...
		if ( 'manual' === $this->type ) {
			...
			if ( 'date' === $this->duration ) {
				...
				if ( $type && $this->trigger ) {
					...
					$trigger = sprintf( __( 'Error getting the trigger. %s is not a valid type', 'ultimatewoo-pro' ), $this->get_type() );
				}
			}
		}
	...
	public function update_status( $new_status ) {
		...
		FUE_Followup_Logger::log( $this->id, sprintf( __( '<p>Email attributes updated:</p><p><strong>status</strong> from <em>%1$s</em> to <em>%2$s</em></p>', 'ultimatewoo-pro' ), $old_status, $new_status ) );
	}
