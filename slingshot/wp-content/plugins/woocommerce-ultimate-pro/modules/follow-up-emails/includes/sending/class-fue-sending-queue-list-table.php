...
class FUE_Sending_Queue_List_Table extends WP_List_Table {
	...
	public function get_user_value( $item ) {
		...
		if ( $item['user_id'] ) {
			return sprintf(
				__('<a href="%s">#%d - %s &lt;%s&gt;', 'ultimatewoo-pro'),
	...
	public function get_order_value( $item ) {
		...
		if ( $order instanceof WC_Order ) {
			return sprintf(
				__('<a href="%s">%s</a>', 'ultimatewoo-pro'),
	...
	public function get_email_value( $item ) {
		...
		if ( $email->status != FUE_Email::STATUS_ACTIVE ) {
			return sprintf(
				__('<a href="%s">#%d %s</a> - Inactive<br/><small>(%s)</small>', 'ultimatewoo-pro'),
	...
	public function get_status_value( $item ) {
		...
		$delete_text    = __('Delete', 'ultimatewoo-pro');

		if ( $item['is_sent'] == 1 ) {
			$status     = __('Sent', 'ultimatewoo-pro');

			return sprintf(
				__( '%s<br/><small><a href="%s">%s</a></small>', 'ultimatewoo-pro'),
				$status,
				$delete_link,
				$delete_text
			);
			...
			if ( $item['status'] == 1 ) {
				$status         = __('Queued', 'ultimatewoo-pro');
				$action_link    = wp_nonce_url( 'admin-post.php?action=fue_update_queue_status&status=0&id='. $item['id'], 'update_queue_status' );
				$actions        = '<a href="'. $action_link .'" onclick="return confirm(\''. __('Really suspend this item?', 'ultimatewoo-pro') .'\');">'. __('Suspend', 'ultimatewoo-pro') .'</a>';

				//if ( defined('FUE_DEBUG') ) {
					$url = wp_nonce_url( 'admin-post.php?action=fue_send_queue_item&id='. $item['id'], 'send_queue_item' );
					$actions .= ' | <a href="'. $url .'" onclick="return confirm(\''. __('Really send this email now?', 'ultimatewoo-pro') .'\');">'. __('Send Now', 'ultimatewoo-pro') .'</a>';
				//}


			} elseif ( $item['status'] == 0 ) {
				$status = __('Suspended', 'ultimatewoo-pro');

				$action = __('Activate', 'ultimatewoo-pro');
				$action_link = wp_nonce_url( 'admin-post.php?action=fue_update_queue_status&status=1&id='. $item['id'], 'update_queue_status' );

				$actions .= '<a href="'. $action_link .'" onclick="return confirm(\''. __('Really activate this item?', 'ultimatewoo-pro') .'\');">'. $action .'</a>';
			}

			$actions .= ' | <span class="trash"><a href="'. $delete_link .'" onclick="return confirm(\''. __('Really delete this item?', 'ultimatewoo-pro') .'\');">'. $delete_text .'</a></span>';

			return sprintf(
				__( '%s<br/><small>%s</small>', 'ultimatewoo-pro'),
				$status,
				$actions
			);
	...
	public function get_columns(){
		...
		$columns = array(
			...
			'user_id'               => __( 'Recipient', 'ultimatewoo-pro' ),
			'order_id'              => __( 'Order', 'ultimatewoo-pro' ),
			'email_id'              => __( 'Email', 'ultimatewoo-pro' ),
			'status'                => __( 'Status', 'ultimatewoo-pro' ),
			'date'                  => __( 'Scheduled', 'ultimatewoo-pro' )
		);
	...
	public function get_bulk_actions() {
		...
		$actions = array(
			'send'      => __( 'Send', 'ultimatewoo-pro' ),
			'activate'  => __( 'Activate', 'ultimatewoo-pro' ),
			'suspend'   => __( 'Suspend', 'ultimatewoo-pro' ),
		...
		if ( 'top' == $which ) { ?>
			<div class="alignleft actions">
				<select id="dropdown_per_page" name="_items_per_page" class="select2">
					<option value="20" <?php selected( 20, $per_page ); ?>><?php _e('Show 20 per page', 'ultimatewoo-pro'); ?></option>
					<option value="50" <?php selected( 50, $per_page ); ?>><?php _e('Show 50 per page', 'ultimatewoo-pro'); ?></option>
					<option value="100" <?php selected( 100, $per_page ); ?>><?php _e('Show 100 per page', 'ultimatewoo-pro'); ?></option>
					<option value="200" <?php selected( 200, $per_page ); ?>><?php _e('Show 200 per page', 'ultimatewoo-pro'); ?></option>
				</select>
				...
				submit_button( __( 'Delete ALL', 'ultimatewoo-pro' ), 'button', 'fue_delete_all', false, array( 'id' => 'delete-all-submit' ) );
	...
	public function messages() {
		...
		if ( isset( $_GET['message'] ) ) {
			...
			printf( __( 'Showing only emails containing "%s"', 'ultimatewoo-pro' ), esc_html( $_REQUEST['s'] ) );
			echo '</p></div>';
		...
		if ( !empty( $_GET['_customer_user'] ) || !empty( $_GET['_product_id'] ) ) {
			...
			if ( ! empty( $_GET['_customer_user'] ) ) {
				...
				if ( false === $user ) {
					_e( 'Invalid user. ', 'ultimatewoo-pro' );
				} else {
					printf( __( "Showing %s's emails", 'ultimatewoo-pro' ), $user->display_name );
				}
			}
			...
			if ( ! empty( $_GET['_product_id'] ) ) {
				...
				if ( false === $product ) {
					printf( __( 'Invalid product.', 'ultimatewoo-pro' ), $user->display_name );
				} elseif ( ! empty( $_GET['_customer_user'] ) ) {
					printf( __( ' for product #%s &ndash; %s%s%s', 'ultimatewoo-pro' ), $product_id, '<em>', $product->get_title(), '</em>' );
				} else {
					printf( __( 'Showing emails for product #%s &ndash; %s%s%s', 'ultimatewoo-pro' ), $product_id, '<em>', $product->get_title(), '</em>' );
				}
			}
