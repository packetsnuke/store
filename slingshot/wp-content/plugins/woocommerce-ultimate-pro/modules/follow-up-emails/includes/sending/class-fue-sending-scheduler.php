...
class FUE_Sending_Scheduler {
	...
	private function insert_email_order( $data, $schedule_event = true ) {
		...
		if ( empty( $item->email_id ) ) {
			...
			if (
				...
				$message = sprintf(
					__('Email has been added to the queue (Queue ID #%d)', 'ultimatewoo-pro'),
					$queue_id
				);
		...
		if ( empty( $email->message ) && empty( $data['message'] ) ) {
			return new WP_Error( 'fue_insert_email_order', __('Cannot schedule an email with a missing message', 'ultimatewoo-pro') );
		}
		...
		if ( !empty($email_meta) ) {
			...
			if ( $email->type != 'manual' && isset($email_meta['one_time']) && $email_meta['one_time'] == 'yes' ) {
				...
				if ( $count_sent > 0 ) {
					...
					return new WP_Error( 'fue_insert_email_order', sprintf( __('One-time email has already been sent to %s', 'ultimatewoo-pro'), $item->user_email ) );
				}
			}
		}
		...
		if ( $adjust_date && $item->user_id ) {
			...
			if ( count( $similar_emails ) > 0 ) {
				...
				$message = sprintf(
					__('Similar queue item found (#%d). Adjusting schedule to send on %s', 'ultimatewoo-pro'),
					$similar_item->id,
		...
		$message = sprintf(
			__('The email "%s" has been added to the queue (Queue ID #%d)', 'ultimatewoo-pro'),
			$email->name,
			$queue_id
		);
		$item->add_note( $message );
	...
	public function schedule_email( $queue_id, $timestamp, $convert_to_gmt = true ) {
		...
		$message = sprintf(
			__('Email is scheduled to be sent on %s', 'ultimatewoo-pro'),
	...
	public function unschedule_email( $queue_id ) {
		...
		if ( $item->exists() ) {
			$item->add_note( __('Email has been removed from the scheduler', 'ultimatewoo-pro') );
		}
	}
