...
class FUE_Sending_Mailer {
	...
	public function send_queue_item( $queue_item, $force_send = false ) {
		...
		if ( empty( $queue_item->email_id ) ) {
			...
			if ( isset( $queue_item->meta['daily_summary'] ) ) {
				if ( 'no' == get_option( 'fue_enable_daily_summary' ) ) {
					return new WP_Error( 'fue_queue_error', __('Queue item deleted because daily summary is not enabled', 'ultimatewoo-pro') );
				}
				...
				if ( ($now - $last_send) < 86000 ) {
					return new WP_Error( 'fue_queue_error', __('Queue item deleted because the daily summary is attempting to send too early', 'ultimatewoo-pro') );
				}
				...
				return new WP_Error( 'fue_queue_error', __('Queue item deleted because no email was assigned to it', 'ultimatewoo-pro') );
			}
		...
		if ( $queue_item->is_sent != 0 ) {
			return new WP_Error( 'fue_queue_error', __('Queue item has already been sent', 'ultimatewoo-pro') );
		}
		...
		if (
			...
			return new WP_Error( 'fue_queue_error', __('Could not send email because it is not active', 'ultimatewoo-pro') );
		}
		...
		if ( !$force_send ) {
			...
			if ( $time_difference > 60 ) {
				...
				return new WP_Error( 'fue_queue_error', sprintf( __('Email tried to send too early. Scheduled on %s but attempted to send on %s', 'ultimatewoo-pro'), $date2, $date1 ) );
			}
		}
		...
		if ( empty($email->message) && empty($queue_item->meta['message']) ) {
			...
			return new WP_Error( 'fue_queue_error', __('Cannot send emails without a message. Email has been deactivated', 'ultimatewoo-pro') );
		}
		...
		if ( FUE_Sending_Scheduler::is_queue_item_locked( $queue_item ) ) {
			...
			return new WP_Error( 'fue_queue_notice', __('This email is locked due to a previous attempt in sending the email. Sending has been rescheduled to avoid duplicate emails.', 'ultimatewoo-pro') );
		}
		...
		if ( $queue_item->user_id > 0 && $queue_item->order_id == 0 && fue_user_opted_out( $queue_item->user_id ) ) {
			...
			Follow_Up_Emails::instance()->scheduler->delete_item( $queue_item->id );
			return new WP_Error( 'fue_queue_notice', __('The customer opted out of recieving non-order related emails. Email has been deleted.', 'ultimatewoo-pro'));
		...
		if (
			...
			Follow_Up_Emails::instance()->scheduler->delete_item( $queue_item->id );
			return new WP_Error( 'fue_queue_notice', __('The customer is on the "Exclude Emails" list. Email deleted.', 'ultimatewoo-pro') );
		...
		if ( $skip ) {
			...
			return new WP_Error( 'fue_queue_notice', __('An add-on has marked this email to be skipped.', 'ultimatewoo-pro') );
		}
	...
	public function send_adhoc_email( $queue_item ) {
		...
		if ( empty( $message ) ) {
			...
			return new WP_Error( 'fue_queue_error', __('Skipped sending because there was no message to be sent', 'ultimatewoo-pro' ) );
		}
	...
	private function process_variable_replacements( $email_data, $queue_item = null, $email = null ) {
		...
		$replacements['webversion_link'] = '<a class="webversion" href="'. $replacements['webversion_url'] .'">'. __('View in browser', 'ultimatewoo-pro') .'</a>';
