...
class FUE_Admin_Actions {
	...
	public static function save_list() {
		if ( ! empty( $_POST['update_priorities'] ) ) {
			...
			$message = __( 'Follow-up emails updated', 'ultimatewoo-pro' );
		} else {
			...
			if ( empty( $count ) ) {
				$message = __( 'No emails selected', 'ultimatewoo-pro' );
	...
	public static function manage_optout() {
		...
		if ( ! empty( $post['button_add'] ) ) {
			...
			if ( ! is_email( $email ) ) {
				$error = urlencode( __( 'The email address is invalid', 'ultimatewoo-pro' ) );
				wp_redirect(
			...
			if ( is_wp_error( $status ) ) {
				...
				if ( 'fue_email_excluded' === $status->get_error_code() ) {
					$message = __( 'This email has already been added', 'ultimatewoo-pro' );
				}
	...
	public static function optout_delete_email() {
		...
		if ( ! is_email( $email ) ) {
			$error = urlencode( __( 'Invalid email address passed.', 'ultimatewoo-pro' ) );
	...
	public static function process_subscribers_lists_bulk_action() {
		...
		if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'bulk-lists' ) ) {
			wp_die( __( 'Bulk edit failed. Invalid Nonce.', 'ultimatewoo-pro' ) );
		}
		...
		if ( in_array( $current_action, array( 'delete_list' ) ) ) {
			...
			foreach ( $lists as $idx => $list_id ) {
				switch ( $current_action ) {
					...
					default :
						$error_messages[] = __( 'Error: Unknown action.', 'ultimatewoo-pro' );
						break;
				}
			}
	...
	public static function process_subscribers_lists_delete_all() {
		...
		if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'bulk-lists' ) ) {
			wp_die( __( 'Are you sure you want to do this?', 'ultimatewoo-pro' ) );
		}
		...
		if ( in_array( $current_action, array( 'send', 'activate', 'suspend', 'delete' ) ) ) {
					...
					case 'send':
						...
						if ( is_wp_error( $sent ) ) {
							$error_messages[] = sprintf(
								__( 'Queue #%1$d: %2$s', 'ultimatewoo-pro' ),
								$item->id,
								$sent->get_error_message()
							);
						} else {
							$messages[] = sprintf(
								__( 'Queue #%d: Scheduled email sent manually', 'ultimatewoo-pro' ),
								$item->id
							);
						}
						break;
			...
			if ( isset( $_GET['_customer_user'] ) || isset( $_GET['_product_id'] ) ) {
				...
				if ( ! empty( $_GET['_customer_user'] ) ) {
					...
					if ( false === $user ) {
						wp_die( __( 'Action failed. Invalid user ID.', 'ultimatewoo-pro' ) );
					}
				...
				if ( ! empty( $_GET['_product_id'] ) ) {
					...
					if ( false === $product ) {
						wp_die( __( 'Action failed. Invalid product ID.', 'ultimatewoo-pro' ) );
					}
	...
	public static function update_queue_item_status() {
		...
		$messages       = array( __( 'Scheduled email updated successfully', 'ultimatewoo-pro' ) );
		$message_nonce  = wp_create_nonce( __FILE__ );
	...
	public static function delete_queue_item() {
		...
		$messages       = array( __( 'Scheduled email deleted', 'ultimatewoo-pro' ) );
		$message_nonce  = wp_create_nonce( __FILE__ );
		set_transient( '_fue_messages_' . $message_nonce, array( 'messages' => $messages ), 60 * 60 );
	...
	public static function send_queue_item() {
		...
		if ( is_wp_error( $sent ) ) {
			...
			$message = __( 'Scheduled email sent manually', 'ultimatewoo-pro' );
			$queue->add_note( $message );
