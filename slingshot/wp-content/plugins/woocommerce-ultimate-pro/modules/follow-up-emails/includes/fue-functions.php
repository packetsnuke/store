...
function fue_save_email( $args ) {
	...
	if ( !empty( $args['type'] ) ) {
		...
		if ( $email_type === false ) {
			return new WP_Error(
				'fue_save_email',
				sprintf( __('Invalid email type passed (%s)', 'ultimatewoo-pro'), $args['type'] )
			);
		}
	}
	...
	if ( $updating ) {
		...
		FUE_Followup_Logger::log( $email_id, __('Created email', 'ultimatewoo-pro') );
	}
...
function fue_clone_email($email_id, $new_name) {
	...
	if ( $email_row ) {
		...
		return new WP_Error( sprintf(__('Email (%d) could not be found', 'ultimatewoo-pro'), $email_id) );
	}
...
function fue_exclude_email_address( $email_address, $email_id = 0, $order_id = 0 ) {
	...
	if ( $count > 0 ) {
		return new WP_Error( 'fue_email_excluded', __('This email has already been removed', 'ultimatewoo-pro') );
	}
