...
class FUE_Meta_Boxes {
	...
	public function form_title_placeholder( $title, $post ) {
		if ( $post->post_type == 'follow_up_email' ) {
			$title = __('Follow-up name', 'ultimatewoo-pro');
		}
	...
	public function add_subject_field() {
		...
		<div id="subjectdiv">
			<div id="subjectwrap">
				<label for="post_excerpt" class="fue-label"><?php _e('Email subject line', 'ultimatewoo-pro'); ?></label>
		...
		<div class="misc-pub-section misc-pub-email-status"><label for="post_status"><?php _e('Status:') ?></label>
			...
			switch ( $email->status ) {
				case FUE_Email::STATUS_ACTIVE:
					_e('Active', 'ultimatewoo-pro');
					break;
				case FUE_Email::STATUS_INACTIVE:
				case 'draft':
				case 'auto-draft':
					_e('Inactive', 'ultimatewoo-pro');
					break;
			...
			<div id="post-status-select" class="hide-if-js">
				...
				<select name='post_status' id='post_status'>
					<option<?php selected( $email->status, FUE_Email::STATUS_ACTIVE ); ?> value='<?php esc_attr_e(FUE_Email::STATUS_ACTIVE); ?>'><?php _e('Active', 'ultimatewoo-pro') ?></option>
					<option<?php selected( $email->status, FUE_Email::STATUS_INACTIVE ); ?> value='<?php esc_attr_e(FUE_Email::STATUS_INACTIVE); ?>'><?php _e('Inactive', 'ultimatewoo-pro') ?></option>
					<option<?php selected( $email->status, FUE_Email::STATUS_ARCHIVED ); ?> value='<?php esc_attr_e(FUE_Email::STATUS_ARCHIVED); ?>'><?php _e('Archived', 'ultimatewoo-pro') ?></option>
	...
	public static function email_test_view( $post ) {
		...
		<p>
			...
			&nbsp; <a href="<?php echo $email->get_preview_url(); ?>" target="_blank"><?php _e('Preview in browser', 'ultimatewoo-pro'); ?></a>
		</p>
	...
	public function add_template_sections_to_variables( $email ) {
		...
		foreach ( $tpl->get_sections() as $section ) {
			?>
			<li class="var hideable var_template_section"><strong>{section:<?php echo $section; ?>}...{/section}</strong> <img class="help_tip" title="<?php _e('Email template section', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
			<?php
		}
	}
