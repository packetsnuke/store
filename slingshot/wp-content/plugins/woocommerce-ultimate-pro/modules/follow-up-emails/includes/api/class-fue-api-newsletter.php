...
class FUE_API_Newsletter extends FUE_API_Resource {
	...
	public function add_subscriber( $data = array() ) {
		...
		if ( empty( $emails ) ) {
			return new WP_Error('fue_api_missing_parameter', __('Cannot add a subscriber without an email address', 'ultimatewoo-pro') );
		}
	...
	public function edit_subscriber( $id, $data ) {
		...
		if ( !empty( $data['email'] ) && $data['email'] != $subscriber['email'] ) {
			if ( !is_email( $data['email'] ) ) {
				return new WP_Error( 'fue_api_cannot_edit_subscriber', __('Invalid email address', 'ultimatewoo-pro') );
			}
			...
			if ( $existing_subscriber && $existing_subscriber['id'] != $subscriber['id'] ) {
				return new WP_Error( 'fue_api_cannot_edit_subscriber', __('The email address is being used by another customer', 'ultimatewoo-pro') );
			}
	...
	public function delete_subscriber( $id ) {
		...
		return new WP_Error( 'fue_api_cannot_delete_subscriber', __('Subscriber could not be found', 'ultimatewoo-pro') );
	}
