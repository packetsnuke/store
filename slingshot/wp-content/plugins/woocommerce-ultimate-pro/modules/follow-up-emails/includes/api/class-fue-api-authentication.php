...
class FUE_API_Authentication {
	...
	private function perform_ssl_authentication() {
		...
		if ( ! empty( $_SERVER['PHP_AUTH_USER'] ) ) {
			...
			throw new Exception( __( 'Consumer Key is missing', 'ultimatewoo-pro' ), 404 );
		}
		...
		if ( ! empty( $_SERVER['PHP_AUTH_PW'] ) ) {
			...
			throw new Exception( __( 'Consumer Secret is missing', 'ultimatewoo-pro' ), 404 );
		}

		$user = $this->get_user_by_consumer_key( $consumer_key );

		if ( ! $this->is_consumer_secret_valid( $user, $consumer_secret ) ) {
			throw new Exception( __( 'Consumer Secret is invalid', 'ultimatewoo-pro' ), 401 );
		}
	...
	private function perform_oauth_authentication() {
		...
		foreach ( $param_names as $param_name ) {
			...
			if ( empty( $params[ $param_name ] ) )
				throw new Exception( sprintf( __( '%s parameter is missing', 'ultimatewoo-pro' ), $param_name ), 404 );
		}
	...
	private function get_user_by_consumer_key( $consumer_key ) {
		...
		if ( empty( $users[0] ) ) {
			...
			if ( empty( $users[0] ) ) {
				throw new Exception( __( 'Consumer Key is invalid', 'ultimatewoo-pro' ), 401 );
			}
		}
	...
	private function check_oauth_signature( $user, $params ) {
		...
		if ( ! uksort( $params, 'strcmp' ) ) {
			throw new Exception( __( 'Invalid Signature - failed to sort parameters', 'ultimatewoo-pro' ), 401 );
		}
		...
		if ( $params['oauth_signature_method'] !== 'HMAC-SHA1' && $params['oauth_signature_method'] !== 'HMAC-SHA256' ) {
			throw new Exception( __( 'Invalid Signature - signature method is invalid', 'ultimatewoo-pro' ), 401 );
		}
		...
		if ( $signature !== $consumer_signature ) {
			throw new Exception( __( 'Invalid Signature - provided signature does not match', 'ultimatewoo-pro' ), 401 );
		}
	}
	...
	private function check_oauth_timestamp_and_nonce( $user, $timestamp, $nonce ) {
		...
		if ( ( $timestamp < time() - $valid_window ) ||  ( $timestamp > time() + $valid_window ) )
			throw new Exception( __( 'Invalid timestamp', 'ultimatewoo-pro' ) );
		...
		if ( in_array( $nonce, $used_nonces ) )
			throw new Exception( __( 'Invalid nonce - nonce has already been used', 'ultimatewoo-pro' ), 401 );
		...
		switch ( Follow_Up_Emails::instance()->api->server->method ) {
			...
			case 'GET':
				if ( 'read' !== $key_permissions && 'read_write' !== $key_permissions ) {
					throw new Exception( __( 'The API key provided does not have read permissions', 'ultimatewoo-pro' ), 401 );
				}
				break;
			...
			case 'DELETE':
				if ( 'write' !== $key_permissions && 'read_write' !== $key_permissions ) {
					throw new Exception( __( 'The API key provided does not have write permissions', 'ultimatewoo-pro' ), 401 );
				}
				break;
		}
	}
