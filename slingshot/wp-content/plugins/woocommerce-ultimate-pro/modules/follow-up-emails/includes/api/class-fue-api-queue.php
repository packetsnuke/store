...
class FUE_API_Queue extends FUE_API_Resource {
	...
	public function create_queue_item( $data ) {
		...
		if ( !current_user_can( 'manage_follow_up_emails' ) ) {
			return new WP_Error( 'fue_api_user_cannot_access_queue', __( 'You do not have permission to access this resource', 'ultimatewoo-pro' ), array( 'status' => 400 ) );
		}
		...
		if ( empty( $data['user_email'] ) ) {
			return new WP_Error( 'fue_api_missing_callback_param', sprintf( __( 'Missing parameter %s', 'ultimatewoo-pro' ), 'user_email' ), array( 'status' => 400 ) );
		}

		if ( empty( $data['email_id'] ) ) {
			return new WP_Error( 'fue_api_missing_callback_param', sprintf( __( 'Missing parameter %s', 'ultimatewoo-pro' ), 'email_id' ), array( 'status' => 400 ) );
		} else {
			$email = new FUE_Email( $data['email_id'] );

			if ( !$email->exists() ) {
				return new WP_Error( 'fue_api_invalid_email_id', __('Invalid email ID', 'ultimatewoo-pro'), array('status' => 400 ) );
			}
		}
	...
	public function delete_queue_item( $id ) {
		...
		return array( 'message' => __( 'Deleted queue item', 'ultimatewoo-pro' ) );
	}
	...
	protected function validate_request( $id, $type = null, $context = null ) {
		...
		// validate ID
		if ( empty( $id ) )
			return new WP_Error( "fue_api_invalid_queue_id", __( 'Invalid Queue ID', 'ultimatewoo-pro' ), array( 'status' => 404 ) );
		...
		if ( !current_user_can('manage_follow_up_emails') ) {
			return new WP_Error( "fue_api_user_cannot_access_queue", __( 'You do not have permission to access this resource', 'ultimatewoo-pro' ), array( 'status' => 401 ) );
		}
