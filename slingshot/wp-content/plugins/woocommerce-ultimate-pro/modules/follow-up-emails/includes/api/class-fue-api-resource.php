...
class FUE_API_Resource {
	...
	protected function validate_request( $id, $type, $context ) {
		...
		// validate ID
		if ( empty( $id ) )
			return new WP_Error( "fue_api_invalid_{$resource_name}_id", sprintf( __( 'Invalid %s ID', 'ultimatewoo-pro' ), $type ), array( 'status' => 404 ) );
		...
		// validate post type
		if ( $type !== $post_type )
			return new WP_Error( "fue_api_invalid_{$resource_name}", sprintf( __( 'Invalid %s', 'ultimatewoo-pro' ), $resource_name ), array( 'status' => 404 ) );
		...
		switch ( $context ) {
			...
			case 'read':
				if ( ! $this->is_readable( $post ) )
					return new WP_Error( "fue_api_user_cannot_read_{$resource_name}", sprintf( __( 'You do not have permission to read this %s', 'ultimatewoo-pro' ), $resource_name ), array( 'status' => 401 ) );
				break;

			case 'edit':
				if ( ! $this->is_editable( $post ) )
					return new WP_Error( "fue_api_user_cannot_edit_{$resource_name}", sprintf( __( 'You do not have permission to edit this %s', 'ultimatewoo-pro' ), $resource_name ), array( 'status' => 401 ) );
				break;

			case 'delete':
				if ( ! $this->is_deletable( $post ) )
					return new WP_Error( "fue_api_user_cannot_delete_{$resource_name}", sprintf( __( 'You do not have permission to delete this %s', 'ultimatewoo-pro' ), $resource_name ), array( 'status' => 401 ) );
				break;
		}
	...
	protected function delete( $id, $type, $force = false ) {
		...
		if ( ! $result )
			return new WP_Error( "fue_api_cannot_delete_{$resource_name}", sprintf( __( 'This %s cannot be deleted', 'ultimatewoo-pro' ), $resource_name ), array( 'status' => 500 ) );

		if ( $force ) {
			return array( 'message' => sprintf( __( 'Permanently deleted %s', 'ultimatewoo-pro' ), $resource_name ) );

		} else {

			$this->server->send_status( '202' );

			return array( 'message' => sprintf( __( 'Deleted %s', 'ultimatewoo-pro' ), $resource_name ) );
		}
