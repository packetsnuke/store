...
class FUE_API_Server {
	...
	public function check_authentication() {
		...
		elseif ( ! is_wp_error( $user ) )
			$user = new WP_Error( 'fue_api_authentication_error', __( 'Invalid authentication method', 'ultimatewoo-pro' ), array( 'code' => 500 ) );
	...
	public function dispatch() {
		...
		switch ( $this->method ) {
			...
			default:
				return new WP_Error( 'fue_api_unsupported_method', __( 'Unsupported request method', 'ultimatewoo-pro' ), array( 'status' => 400 ) );
		}
		...
		foreach ( $this->get_routes() as $route => $handlers ) {
			foreach ( $handlers as $handler ) {
				...
				if ( ! is_callable( $callback ) )
					return new WP_Error( 'fue_api_invalid_handler', __( 'The handler for the route is invalid', 'ultimatewoo-pro' ), array( 'status' => 500 ) );
		...
		return new WP_Error( 'fue_api_no_route', __( 'No route was found matching the URL and request method', 'ultimatewoo-pro' ), array( 'status' => 404 ) );
	}
	...
	protected function sort_callback_params( $callback, $provided ) {
		...
		foreach ( $wanted as $param ) {
			...
			elseif ( $param->isDefaultValueAvailable() ) {
				...
				return new WP_Error( 'fue_api_missing_callback_param', sprintf( __( 'Missing parameter %s', 'ultimatewoo-pro' ), $param->getName() ), array( 'status' => 400 ) );
			}
		}
