...
class FUE_API_Emails extends FUE_API_Resource {
	...
	public function send_manual_email( $id, $data ) {
		...
		if ( empty( $data['send_type'] ) ) {
			return new WP_Error( 'fue_api_missing_callback_param', __('send_type is a required field', 'ultimatewoo-pro') );
		}
		...
		if (! empty($recipients) ) {
			...
			if ( $schedule_email ) {
				if ( empty( $data['sending_schedule_date'] ) ) {
					return new WP_Error( 'send_manual_error', __('Cannot schedule an email without passing a valid `sending_schedule_date`', 'ultimatewoo-pro') );
				}
