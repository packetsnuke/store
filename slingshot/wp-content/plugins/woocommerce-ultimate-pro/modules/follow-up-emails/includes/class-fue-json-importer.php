...
class FUE_JSON_Importer {
	...
	public function import() {
		...
		if ( !$this->json ) {
			return new WP_Error( 'fue_json_importer', __('JSON is not set', 'ultimatewoo-pro') );
		}

		$follow_ups = json_decode( $this->json, true );

		if ( is_null( $follow_ups ) ) {
			return new WP_Error( 'fue_json_importer', __('Error parsing the JSON string', 'ultimatewoo-pro') );
		}
