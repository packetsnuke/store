...
class FUE_Front_Handler {
	...
	public static function process_unsubscribe_request() {
		...
		if ( isset( $_POST['fue_action'] ) &&  'fue_unsubscribe' === $_POST['fue_action'] ) {
			...
			if ( empty( $email ) || ! is_email( $email ) ) {
				$error = urlencode( __( 'Please enter a valid email address', 'ultimatewoo-pro' ) );
			}
			...
			if ( fue_is_email_excluded( $email, 0, $order_id ) ) {
				if ( $order_id > 0 ) {
					$error = sprintf( __( 'The email (%1$s) is already unsubscribed from receiving emails regarding Order %2$d', 'ultimatewoo-pro' ), $email, $order_id );
				} else {
					$error = sprintf( __( 'The email (%s) is already unsubscribed from receiving emails', 'ultimatewoo-pro' ), $email );
				}
			}
			...
			Follow_Up_Emails::show_message( __( 'Thank you. Your email settings have been saved.', 'ultimatewoo-pro' ) );
		}
	}
	...
	public static function process_optout_request() {
		if ( isset( $_POST['fue_action'] ) && 'fue_save_myaccount' === $_POST['fue_action'] ) {
			...
			Follow_Up_Emails::show_message( __( 'Account updated', 'ultimatewoo-pro' ) );
		}
	}
	...
	public static function web_version() {
		...
		if ( ! $item->exists() || 1 != $item->is_sent ) {
			wp_die( __( 'Email could not be found', 'ultimatewoo-pro' ) );
		}

		$item_key = md5( $item->user_email . '.' . $item->email_id . '.' . $item->send_on );

		if ( $item_key != $key ) {
			wp_die( __( 'Invalid request. Please try again.', 'ultimatewoo-pro' ) );
		}
