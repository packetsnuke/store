...
class FUE_Admin_Profile {
	...
	public function add_api_key_field( $user ) {
		...
		$permissions = array(
			'read'       => __( 'Read', 'ultimatewoo-pro' ),
			'write'      => __( 'Write', 'ultimatewoo-pro' ),
			'read_write' => __( 'Read/Write', 'ultimatewoo-pro' ),
		);
		...
		if ( current_user_can( 'edit_user', $user->ID ) ) {
			...
			<table class="form-table">
				<tbody>
				<tr>
					<th><label for="fue_generate_api_key"><?php _e( 'Follow-Up Emails API Keys', 'ultimatewoo-pro' ); ?></label></th>
					<td>
						<?php if ( empty( $user->fue_api_consumer_key ) ) : ?>
							<input name="fue_generate_api_key" type="checkbox" id="fue_generate_api_key" value="0" />
							<span class="description"><?php _e( 'Generate API Key', 'ultimatewoo-pro' ); ?></span>
						<?php else : ?>
							<strong><?php _e( 'Consumer Key:', 'ultimatewoo-pro' ); ?>&nbsp;</strong><code id="fue_api_consumer_key"><?php echo $user->fue_api_consumer_key ?></code><br/>
							<strong><?php _e( 'Consumer Secret:', 'ultimatewoo-pro' ); ?>&nbsp;</strong><code id="fue_api_consumer_secret"><?php echo $user->fue_api_consumer_secret; ?></code><br/>
							<strong><?php _e( 'Permissions:', 'ultimatewoo-pro' ); ?>&nbsp;</strong><span id="fue_api_key_permissions"><select name="fue_api_key_permissions" id="fue_api_key_permissions">
							<?php
							...
							<input name="fue_generate_api_key" type="checkbox" id="fue_generate_api_key" value="0" />
							<span class="description"><?php _e( 'Revoke API Key', 'ultimatewoo-pro' ); ?></span>
						<?php endif; ?>
					</td>
				</tr>
				</tbody>
