...
class FUE_Addon_Woocommerce_Cart {
	...
	public function handle_redirect_to_cart() {
		...
		if ( !is_user_logged_in() && $item->user_id > 0 ) {
			...
			wc_add_notice( __('Please log in to view your saved cart.', 'ultimatewoo-pro'), 'notice' );
	...
	public function process_clear_scheduled_cart_emails() {
		...
		$message = __( 'Cart emails have been cleared for this user', 'ultimatewoo-pro' );
	...
	public static function get_cart_status( $user ) {
		$status     = __('Active', 'ultimatewoo-pro');
		$email      = '';
		$user_id    = 0;
		...
		if ( $time_diff > $time ) {
			$status = __('Abandoned', 'ultimatewoo-pro');
		}
