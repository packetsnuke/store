...
class FUE_Addon_Woocommerce {
	...
	public function register_email_types( $types ) {
		...
		foreach ( $this->get_order_statuses() as $i => $status ) {
			$status_triggers[ $status ] = sprintf( __('after Order Status: %s', 'ultimatewoo-pro'), $status );
		}

		$storewide_triggers = apply_filters( 'fue_wc_storewide_triggers', array_unique( array_merge(
			array(
				'first_purchase'                => __('after first purchase', 'ultimatewoo-pro'),
				'cart'                          => __('after added to cart', 'ultimatewoo-pro'),
				'product_purchase_above_one'    => __('after customer purchased more than one time', 'ultimatewoo-pro'),
				'downloadable_file_added'       => __('after downloadable file added', 'ultimatewoo-pro'),
				'downloaded'                    => __('after file downloaded', 'ultimatewoo-pro'),
				'not_downloaded'                => __('file not yet downloaded', 'ultimatewoo-pro'),
				'coupon'                        => __('after Coupon used', 'ultimatewoo-pro')
			),
			$status_triggers
		) ) );

		$customer_triggers = apply_filters( 'fue_wc_customer_triggers', array(
			'after_last_purchase'   => __('after last purchase', 'ultimatewoo-pro'),
			'order_total_above'     => __('after order total is above', 'ultimatewoo-pro'),
			'order_total_below'     => __('after order total is below', 'ultimatewoo-pro'),
			'purchase_above_one'    => __('after customer purchased more than one time', 'ultimatewoo-pro'),
			'total_orders'          => __('after total orders by customer', 'ultimatewoo-pro'),
			'total_purchases'       => __('after total purchase amount by customer', 'ultimatewoo-pro')
		) );

		if ( WC_FUE_Compatibility::is_wc_version_gte_2_2() ) {
			// add support for refunds
			$storewide_triggers['refund_manual']      = __('after refunded manually', 'ultimatewoo-pro');
			$storewide_triggers['refund_successful']  = __('after refunded successfully', 'ultimatewoo-pro');
			$storewide_triggers['refund_failed']      = __('after refund failed', 'ultimatewoo-pro');
		}

		$props = array(
			'priority'              => 9,
			'label'                 => __('Storewide Emails', 'ultimatewoo-pro'),
			'singular_label'        => __('Storewide Email', 'ultimatewoo-pro'),
			'triggers'              => $storewide_triggers,
			'supports'              => array('conditions'),
			'durations'             => Follow_Up_Emails::$durations,
			'long_description'      => __('Create your emails for all, or specific, products or categories as well as abandoned carts.', 'ultimatewoo-pro'),
			'short_description'     => __('Create your emails for all, or specific, products or categories as well as abandoned carts.', 'ultimatewoo-pro'),
			'list_template'         => FUE_TEMPLATES_DIR .'/email-list/storewide-list.php'
		);
		$types[] = new FUE_Email_Type( 'storewide', $props );

		$props = array(
			'label'                 => __('Customer Emails', 'ultimatewoo-pro'),
			'singular_label'        => __('Customer Email', 'ultimatewoo-pro'),
			'triggers'              => $customer_triggers,
			'durations'             => Follow_Up_Emails::$durations,
			'long_description'      => __('Engage your customers by following up with emails specifically related to customer lifetime value metrics.', 'ultimatewoo-pro'),
			'short_description'     => __('Engage your customers by following up with emails specifically related to customer lifetime value metrics.', 'ultimatewoo-pro'),
			'list_template'         => FUE_TEMPLATES_DIR .'/email-list/storewide-list.php'
		);
		...
		if ( !$this->exclude_quantity_email_type() ) {
			$props   = array(
				...
				'triggers'          => array(
					'processing' => __( 'after Order Status: processing', 'ultimatewoo-pro' ),
					'completed'  => __( 'after Order Status: completed', 'ultimatewoo-pro' )
				),
				'durations'         => Follow_Up_Emails::$durations,
				'long_description'  => __( 'Quantity emails are deprecated. Please switch to Storewide.', 'ultimatewoo-pro' ),
				'short_description' => __( 'Quantity emails are deprecated. Please switch to Storewide.', 'ultimatewoo-pro' ),
				'list_template'     => FUE_TEMPLATES_DIR . '/email-list/storewide-list.php'
			);
		...
		foreach ( $orders as $idx => $order_id ) {
			...
			if ( !$order_id ) {
				$imported[] = array(
					'id'        => $order_id,
					'status'    => 'failed',
					'reason'    => sprintf( __('Importing failed. Invalid order (#%d)', 'ultimatewoo-pro'), $order_id )
				);
				continue;
			}
			...
			if ( $insert ) {
				...
				if ( is_wp_error( $item_id ) ) {
					$imported[] = array(
						'id'        => $order_id,
						'status'    => 'failed',
						'reason'    => sprintf( __('Importing failed. %s', 'ultimatewoo-pro'), $item_id->get_error_message() )
					);
					continue;
				}
				...
				if ( $_order ) {
					...
					$imported[] = array(
						'id'        => $order_id,
						'status'    => 'failed',
						'reason'    => sprintf( __('Importing failed. Could not load order (#%d)', 'ultimatewoo-pro'), $order_id )
					);
				}
			}
