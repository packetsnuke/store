...
class FUE_Addon_Sensei_Conditions {
	...
	public function get_conditions() {
		...
		$conditions['have_not_started_first_lesson'] = __('Have not started the first lesson', 'ultimatewoo-pro');
		$conditions['have_not_completed_a_lesson'] = __('Have not completed a lesson', 'ultimatewoo-pro');
		$conditions['have_not_completed_a_course'] = __('Have not completed a course', 'ultimatewoo-pro');
		$conditions['have_not_taken_quiz'] = __('Have not yet taken a quiz', 'ultimatewoo-pro');
		$conditions['have_failed_quiz'] = __('Have failed a quiz', 'ultimatewoo-pro');
		$conditions['have_passed_quiz'] = __('Have passed a quiz', 'ultimatewoo-pro');
	...
	public function test_condition( $condition, $item ) {
		...
		switch ( $condition['condition'] ) {
			...
			default:
				$result = new WP_Error( 'fue_email_conditions', sprintf( __('Unknown condition: %s', 'ultimatewoo-pro'), $condition['condition'] ) );
				break;
			...
			if ( $course_lessons && is_array( $course_lessons ) ) {
				...
				if ( WooThemes_Sensei_Utils::user_started_lesson( $lesson->ID, $item->user_id ) ) {
					return new WP_Error(
						...
						sprintf( __('have_not_started_first_lesson condition failed for queue #%d (lesson: %d)', 'ultimatewoo-pro'), $item->id, $lesson->ID )
					);
				}
			}
		}
			...
			if ( $course_lessons && is_array( $course_lessons ) ) {
				foreach ( $course_lessons as $lesson ) {
					if ( WooThemes_Sensei_Utils::user_completed_lesson( $lesson->ID, $item->user_id ) ) {
						return new WP_Error(
							'fue_email_conditions',
							sprintf( __('have_not_completed_a_lesson condition failed for queue #%d (lesson: %d)', 'ultimatewoo-pro'), $item->id, $lesson->ID )
						);
					}
				}
			}
	...
	public function test_have_not_completed_a_course( $item, $condition ) {
		...
		foreach ( $course_ids as $course_id ) {
			if ( WooThemes_Sensei_Utils::user_completed_course( $course_id, $item->user_id ) ) {
				return new WP_Error(
					'fue_email_conditions',
					sprintf( __('have_not_completed_a_course condition failed for queue #%d (course: %d)', 'ultimatewoo-pro'), $item->id, $course_id )
				);
			}
		}
	...
	public function test_have_not_taken_quiz( $item, $condition ) {
		...
		foreach ( $lesson_ids as $lesson_id ) {
			...
			if ( $status['status'] != 'not_started') {
				return new WP_Error(
					'fue_email_conditions',
					sprintf( __('have_not_taken_quiz condition failed for queue #%d (lesson: %d)', 'ultimatewoo-pro'), $item->id, $lesson_id )
				);
			}
		}
	...
	public function test_have_failed_quiz( $item, $condition ) {
		...
		foreach ( $lesson_ids as $lesson_id ) {
			...
			if ( $status['status'] != 'failed') {
				return new WP_Error(
					'fue_email_conditions',
					sprintf( __('have_failed_quiz condition failed for queue #%d (lesson: %d)', 'ultimatewoo-pro'), $item->id, $lesson_id )
				);
			}
		}
	...
	public function test_have_passed_quiz( $item, $condition ) {
		...
		foreach ( $lesson_ids as $lesson_id ) {
			...
			if ( !WooThemes_Sensei_Utils::user_passed_quiz( $quiz_id, $item->user_id ) ) {
				return new WP_Error(
					'fue_email_conditions',
					sprintf( __('have_passed_quiz condition failed for queue #%d (lesson: %d)', 'ultimatewoo-pro'), $item->id, $lesson_id )
				);
			}
