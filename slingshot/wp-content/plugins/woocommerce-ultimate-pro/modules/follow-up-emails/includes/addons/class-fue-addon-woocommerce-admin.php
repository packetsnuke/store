...
class FUE_Addon_Woocommerce_Admin {
	...
	public function order_import_check() {
		if (
			...
			<div id="message" class="updated">
				<p><strong><?php _e( 'Initial order scanning is required to accurately send conditional emails', 'ultimatewoo-pro' ); ?></strong></p>
				<p class="submit">
					<a href="<?php echo add_query_arg( 'tab', 'order_import', admin_url( 'admin.php?page=followup-emails' ) ); ?>" class="fue-update-now button-primary"><?php _e( 'Scan Orders', 'ultimatewoo-pro' ); ?></a>
					<?php _e('or', 'ultimatewoo-pro'); ?> <a href="#" class="fue-disable-scan"><?php _e("don't show this again", 'ultimatewoo-pro'); ?></a>
				</p>
			</div>
			<script type="text/javascript">
				jQuery('.fue-update-now').click(function(){
					var answer = confirm( '<?php _e( 'It is strongly recommended that you backup your database before proceeding. Are you sure you wish to run the scanner now?', 'ultimatewoo-pro' ); ?>' );
					return answer;
				});

				jQuery('.fue-disable-scan').click(function() {
					var container   = jQuery(this).parents("div#message");
					var message     = '<?php _e('Not scanning your existing orders may cause customer and conditional emails to not work accurately. Do you wish to continue?', 'ultimatewoo-pro'); ?>';
	...
	public function data_update_page( $tab ) {
		if ( $tab == 'data_update' ) {
			...
			if ( empty( $act ) ) {
				wp_die( __('No action specified', 'ultimatewoo-pro' ) );
			}
	...
	public function add_import_action_link( $email ) {
		if ( $email->import_order_flag == 1 ) {
			echo '| <small><a href="'. esc_url( $this->get_email_import_url( $email->id )) .'" class="email-import-orders" data-id="'. $email->id .'">'. __('Import Orders', 'ultimatewoo-pro') .'</a></small>';
		}
	}
	...
	public function add_bulk_import_option( $type ) {
		...
		?>
		<option value="import"><?php _e('Import Orders', 'ultimatewoo-pro'); ?></option>
	<?php
	}
	...
	public function inject_email_details_tabs( $tabs, $email ) {
		$insert = array (
			'custom_fields' => array(
				'label'  => __( 'Custom Fields', 'ultimatewoo-pro' ),
				'icon'   => 'dashicons-admin-appearance',
				'target' => 'custom_fields_details',
				'class'  => array('custom-fields'),
			),
			'coupons' => array(
				'label'  => __( 'Coupons', 'ultimatewoo-pro' ),
		...
		if ( $email->type == 'storewide' || $email->type == 'wc_bookings' ) {
			$insert['exclusions'] = array(
				'label'  => __( 'Exclusions', 'ultimatewoo-pro' ),
	...
	public function link_to_addon_settings() {
		...
		<a href="admin.php?page=followup-emails-settings&tab=integration"><?php _e('Click here to manage your WooCommerce email styles', 'ultimatewoo-pro'); ?></a>
		<?php
	}
	...
	public function storewide_variables( $email ) {
		...
		?>
		<li class=""><strong>{item_names}</strong> <img class="help_tip" title="<?php _e('Displays a list of purchased items.', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<li class=""><strong>{item_names_list}</strong> <img class="help_tip" title="<?php _e('Displays a comma-separated list of purchased items.', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<li class=""><strong>{item_prices}</strong> <img class="help_tip" title="<?php _e('Displays a list of purchased items, quantities, and prices.', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<li class=""><strong>{item_prices_image}</strong> <img class="help_tip" title="<?php _e('Displays a list of purchased items, quantities, and prices with thumbnails.', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<li class=""><strong>{item_codes_prices}</strong> <img class="help_tip" title="<?php _e('Displays a list of the purchased items with their quantities, codes, and prices.', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<li class=""><strong>{item_prices_categories}</strong> <img class="help_tip" title="<?php _e('Displays a list of the purchased items, with their quantities, prices, and categories.', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<li class=""><strong>{item_categories}</strong> <img class="help_tip" title="<?php _e('The list of categories where the purchased items are under.', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<li class=""><strong>{dollars_spent_order}</strong> <img class="help_tip" title="<?php _e('The the amount spent on an order', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<?php

		if ( in_array( $email->trigger, array('refund_manual', 'refund_successful', 'refund_failed') ) ):
			?>
			<li class=""><strong>{refund_amount}</strong> <img class="help_tip" title="<?php _e('The amount of the refund', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
			<li class=""><strong>{refund_reason}</strong> <img class="help_tip" title="<?php _e('The reason for the refund', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<?php
		endif;
	}
	...
	public function product_variables( $email ) {
		...
		?>
		<li class="var hideable"><strong>{download_url}</strong>  <img class="help_tip" title="<?php _e('The URL of the downloadable file.', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<li class="var hideable"><strong>{download_filename}</strong>  <img class="help_tip" title="<?php _e('The name of the downloadable file.', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<li class=""><strong>{item_name}</strong> <img class="help_tip" title="<?php _e('The name of the purchased item.', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<li class=""><strong>{item_code}</strong> <img class="help_tip" title="<?php _e('The code number of the purchased item.', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<li class=""><strong>{item_url}</strong> <img class="help_tip" title="<?php _e('The URL of the purchased item. Only works when the product is specified in the trigger setting.', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<li class=""><strong>{item_category}</strong> <img class="help_tip" title="<?php _e('The list of categories where the purchased item is under.', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<li class=""><strong>{item_price}</strong> <img class="help_tip" title="<?php _e('The price of the purchased item.', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<li class=""><strong>{item_quantity}</strong> <img class="help_tip" title="<?php _e('The quantity of the purchased item.', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<li class=""><strong>{dollars_spent_order}</strong> <img class="help_tip" title="<?php _e('The the amount spent on an order', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
	<?php
	}
	...
	public function customer_variables( $email ) {
		...
		?>
		<li class=""><strong>{amount_spent_order}</strong> <img class="help_tip" title="<?php _e('The the amount spent on an order', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<li class=""><strong>{amount_spent_total}</strong> <img class="help_tip" title="<?php _e('Total amount spent by the customer', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<li class=""><strong>{number_orders}</strong> <img class="help_tip" title="<?php _e('Total amount spent by the customer', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<li class=""><strong>{last_purchase_date}</strong> <img class="help_tip" title="<?php _e('The date the customer last ordered', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
	<?php
	}
	...
	public function manual_types() {
		$options = array(
			'users'     => __('All Users', 'ultimatewoo-pro'),
			'storewide' => __('All Customers', 'ultimatewoo-pro'),
			'customer'  => __('This Customer', 'ultimatewoo-pro'),
			'product'   => __('Customers who bought these products', 'ultimatewoo-pro'),
			'category'  => __('Customers who bought from these categories', 'ultimatewoo-pro'),
			'timeframe' => __('Customers who bought between these dates', 'ultimatewoo-pro'),
			'not_product'   => __('Customers who never bought these products', 'ultimatewoo-pro')
		);
	...
	public function add_import_link_to_post_message( $messages ) {
		...
		if ( $import ) {
			...
			$messages[1] = __( 'Your email has been saved. You chose to import existing orders. Please click this button to initiate a scan of your orders to schedule emails. <a href="'. esc_url( $url ) .'" class="button">Import existing orders</a>', 'ultimatewoo-pro' );
		}
	...
	public function filter_orders_by_email() {
		...
		if ( ! empty( $emails ) ) {
			...
			<select name="_email_id" id="dropdown_emails_used">
				<option value=""><?php _e( 'Filter by emails sent/queued', 'ultimatewoo-pro' ); ?></option>
