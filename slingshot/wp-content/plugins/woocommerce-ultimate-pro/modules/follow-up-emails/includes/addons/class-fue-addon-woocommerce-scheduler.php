...
class FUE_Addon_Woocommerce_Scheduler {
	...
	public function unqueue_status_emails( $order_id, $old_status, $new_status ) {
		...
		foreach ( $queue as $item ) {
			...
			$order->add_order_note( sprintf( __( 'The email &quot;%s&quot; has been removed due to an order status change', 'ultimatewoo-pro' ), $email_name ) );
	...
	public function add_order_notes_to_queued_emails( $queued ) {
		...
		foreach ( $queued as $row ) {
			if ( isset( $row['order_id'] ) && $row['order_id'] > 0 ) {
				...
				$note = sprintf(
					__( 'Email queued: %1$s scheduled on %1$s<br/>Trigger: %1$s', 'ultimatewoo-pro' ),
					$email->name,
					$send_date,
					$email_trigger
				);
