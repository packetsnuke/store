...
class FUE_Addon_Twitter_Admin {
	...
	public function reset_data() {
		...
		wp_redirect( 'admin.php?page=followup-emails-settings&tab=integration&message='. urlencode(__('Twitter data removed', 'ultimatewoo-pro')) );
		exit;
	}
	...
	public function oauth_callback() {
		...
		if ( isset($_REQUEST['oauth_token']) && $request_token['oauth_token'] !== $_REQUEST['oauth_token']) {
			...
			wp_die( _e('The tokens do not match!', 'ultimatewoo-pro') );
		}
		...
		$message = urlencode( __('Successfully connected your Twitter account!', 'ultimatewoo-pro') );
	...
	public function twitter_variables( $email ) {
		...
		?>
		<li class="var var_twitter var_twitter_handle"><strong>{twitter_handle}</strong> <img class="help_tip" title="<?php _e('The customer\'s Twitter handle (e.g. @johndoe)', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<li class="var var_twitter var_order_items"><strong>{order_items}</strong> <img class="help_tip" title="<?php _e('Displays a list of purchased items.', 'ultimatewoo-pro'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
		<?php
	}
	...
	public static function add_twitter_content() {
		...
		<div id="fue-twitter-content" style="display: none; margin-top: 20px;">
			<label for="twitter_content" class="fue-label"><?php _e('Twitter Message', 'ultimatewoo-pro'); ?></label>
			...
			<div id="fue-twitter-content-character-count-container">
				<div id="fue-twitter-content-character-count">
					<?php _e('Character Count:', 'ultimatewoo-pro'); ?> <span id="fue-twitter-count">0</span> <?php _e('of 140', 'ultimatewoo-pro'); ?>
				</div>
			</div>
		</div>
		<?php
