...
class FUE_Addon_Woocommerce_Conditions {
	...
	public function get_store_conditions() {
		...
		$conditions['bought_products']      = __('customer bought these products', 'ultimatewoo-pro');
		$conditions['bought_categories']    = __('customer bought from these categories', 'ultimatewoo-pro');

		$conditions['first_purchase']       = __('on first purchase', 'ultimatewoo-pro');
		$conditions['order_total_above']    = __('order total above:', 'ultimatewoo-pro');
		$conditions['order_total_below']    = __('order total below:', 'ultimatewoo-pro');

		$conditions['total_orders_above']   = __('total orders by customer above:', 'ultimatewoo-pro');
		$conditions['total_orders_below']   = __('total orders by customer below:', 'ultimatewoo-pro');
		$conditions['total_purchases_above']= __('total purchase amount by customer above:', 'ultimatewoo-pro');
		$conditions['total_purchases_below']= __('total purchase amount by customer below:', 'ultimatewoo-pro');

		$conditions['payment_method']       = __('payment method used:', 'ultimatewoo-pro');
		$conditions['shipping_method']      = __('shipping method used:', 'ultimatewoo-pro');
	...
	public function get_signup_conditions() {
		...
		$conditions['has_purchased']     = __('has made a purchase', 'ultimatewoo-pro');
		$conditions['has_not_purchased'] = __('has not made a purchase', 'ultimatewoo-pro');
	...
	public function test_store_condition( $condition, $item ) {
		...
		switch ( $condition['condition'] ) {
			...
			default:
				return new WP_Error( 'fue_email_conditions', sprintf( __('Unknown condition: %s', 'ultimatewoo-pro'), $condition['condition'] ) );
				break;
	...
	public function test_first_purchase_condition( $item ) {
		...
		if ( !$customer ) {
			return new WP_Error(
				...
				sprintf( __('Customer data could not be found (Order #%d)', 'ultimatewoo-pro'), $item->order_id )
			);
		}
		...
		if ( $count != 1 ) {
			return new WP_Error(
				'fue_email_conditions',
				sprintf( __('first_purchase condition failed for queue #%d (purchases: %d)', 'ultimatewoo-pro'), $item->id, $count )
			);
		}
			...
			foreach ( $products as $product ) {
				...
				if ( !$this->fue_wc->customer_purchased_product( $customer, $product ) ) {
					...
					return new WP_Error(
						'fue_email_conditions',
						sprintf(
							__('Customer has not purchased a required product (#%d - %s)', 'ultimatewoo-pro'),
							$product,
			...
			foreach ( $categories as $category ) {
				...
				if ( !$this->fue_wc->customer_purchased_from_category( $customer, $category ) ) {
					...
					return new WP_Error(
						'fue_email_conditions',
						sprintf(
							__('Customer has not purchased from a required category (%s)', 'ultimatewoo-pro'),
							$wc_category->name
						)
					);
				}
	...
	public function test_order_total_condition( $item, $condition ) {
		...
		if ( !$result ) {
			return new WP_Error(
				'fue_email_conditions',
				sprintf(
					__('Condition "%s" failed. Order total: %s / Condition value: %s', 'ultimatewoo-pro'),
					$condition['condition'],
					$total,
					$value
				)
	...
	public function test_total_orders_count_condition( $item, $condition ) {
		...
		if ( !$result ) {
			return new WP_Error(
				'fue_email_conditions',
				sprintf(
					__('Condition "%s" failed. Orders: %s / Condition value: %s', 'ultimatewoo-pro'),
					$condition['condition'],
					$total,
					$value
				)
	...
	public function test_total_purchases_condition( $item, $condition ) {
		...
		if ( !$result ) {
			return new WP_Error(
				'fue_email_conditions',
				sprintf(
					__('Condition "%s" failed. Total Purchases: %s / Condition value: %s', 'ultimatewoo-pro'),
					$condition['condition'],
					$total,
					$value
				)
	...
	public function test_has_purchased_condition( $item, $condition ) {
		if ( empty( $item->user_id ) ) {
			return new WP_Error(
				'fue_email_conditions',
				sprintf(
					__('Condition "%s" failed. Queue item has no User ID', 'ultimatewoo-pro'),
					$condition['condition']
				)
			);
		}
		...
		if ( $orders == 0 ) {
			return new WP_Error(
				'fue_email_conditions',
				sprintf(
					__('Condition "%s" failed. Customer has 0 orders', 'ultimatewoo-pro'),
					$condition['condition']
				)
			);
		}
	...
	public function test_has_not_purchased_condition( $item, $condition ) {
		...
		if ( $orders > 0 ) {
			return new WP_Error(
				'fue_email_conditions',
				sprintf(
					__('Condition "%s" failed. Customer has %d orders', 'ultimatewoo-pro'),
					$condition['condition'],
					$orders
				)
			);
	...
	public function test_payment_method( $item, $condition ) {
		...
		if ( !$result ) {
			return new WP_Error(
				'fue_email_conditions',
				sprintf(
					__('Condition "%s" failed. Payment method: %s / Condition value: %s', 'ultimatewoo-pro'),
					$condition['condition'],
					$method,
					$value
				)
	...
	public function test_shipping_method( $item, $condition ) {
		...
		if ( !$result ) {
			return new WP_Error(
				'fue_email_conditions',
				sprintf(
					__('Condition "%s" failed. Shipping method mismatch: %s / Condition value: %s', 'ultimatewoo-pro'),
					$condition['condition'],
