...
class FUE_Addon_WC_Memberships {
	...
	public function register_email_type( $types ) {
		$triggers = array(
			'membership_before_expire'  => __('before membership expires', 'ultimatewoo-pro')
		);
		foreach ( wc_memberships_get_user_membership_statuses() as $status => $labels ) {
			$triggers[ $status ] = sprintf( __('after membership status: %s', 'ultimatewoo-pro'), $labels['label'] );
		}
		$props = array(
			'label'                 => __('WC Memberships', 'ultimatewoo-pro'),
			'singular_label'        => __('WC Memberships', 'ultimatewoo-pro'),
			'triggers'              => $triggers,
			'durations'             => Follow_Up_Emails::$durations,
			'long_description'      => __('', 'ultimatewoo-pro'),
			'short_description'     => __('', 'ultimatewoo-pro')
		);
	...
	public static function discount_rule_string( $rule ) {
		...
		if ( $content_type == 'post_type' ) {
			...
			if ( empty( $object_ids ) ) {
				$str = sprintf( __('%s discount on all products', 'ultimatewoo-pro'), $amount );
			} else {
				$object_names = array();
				...
				$str = ( count( $object_ids ) == 1 )
					? sprintf( __('%s discount on %s', 'ultimatewoo-pro'), $amount, $object_names[0] )
					: sprintf(
						__('%s discount on the following products: %s', 'ultimatewoo-pro'),
						$amount,
			...
			if ( empty( $object_ids ) ) {
				$str = sprintf( __('%s discount on all categories', 'ultimatewoo-pro'), $amount );
			} else {
				$object_names = array();
				foreach ( $object_ids as $object_id ) {
				...
				$str = ( count( $object_names ) == 1 )
					? sprintf( __('%s discount on the %s category', 'ultimatewoo-pro'), $amount, $object_names[0] )
					: sprintf(
						__('%s discount on the following categories: %s', 'ultimatewoo-pro'),
						$amount,
						implode( ', ', $object_names )
					);
