...
class FUE_Admin_Controller {
	...
	public static function add_menu() {
		add_menu_page( __( 'Follow-Up Emails', 'ultimatewoo-pro' ), __( 'Follow-Up Emails', 'ultimatewoo-pro' ), 'manage_follow_up_emails', 'followup-emails', 'FUE_Admin_Controller::admin_controller', 'dashicons-email-alt', '54.51' );
		add_submenu_page( 'followup-emails', __( 'Follow-Up Emails', 'ultimatewoo-pro' ), __( 'Emails', 'ultimatewoo-pro' ), 'manage_follow_up_emails', 'followup-emails', 'FUE_Admin_Controller::admin_controller' );
		add_submenu_page( 'followup-emails', __( 'Campaigns', 'ultimatewoo-pro' ), __( 'Campaigns', 'ultimatewoo-pro' ), 'manage_follow_up_emails', 'fue_campaigns', 'FUE_Admin_Controller::admin_controller' );

		add_submenu_page( 'followup-emails', __( 'New Email', 'ultimatewoo-pro' ), __( 'New Email', 'ultimatewoo-pro' ), 'manage_follow_up_emails', 'fue_post_email', 'FUE_Admin_Controller::admin_controller' );
		add_submenu_page( 'followup-emails', __( 'New Tweet', 'ultimatewoo-pro' ), __( 'New Tweet', 'ultimatewoo-pro' ), 'manage_follow_up_emails', 'fue_post_tweet', 'FUE_Admin_Controller::admin_controller' );

		do_action( 'fue_menu' );

		add_submenu_page( 'followup-emails', __( 'Scheduled Emails', 'ultimatewoo-pro' ), __( 'Scheduled Emails', 'ultimatewoo-pro' ), 'manage_follow_up_emails', 'followup-emails-queue', 'FUE_Admin_Controller::queue_table' );
		add_submenu_page( 'followup-emails', __( 'Subscribers', 'ultimatewoo-pro' ), __( 'Mailing Lists', 'ultimatewoo-pro' ), 'manage_follow_up_emails', 'followup-emails-subscribers', 'FUE_Admin_Controller::subscribers_table' );
		add_submenu_page( 'followup-emails', __( 'Follow-Up Emails Templates', 'ultimatewoo-pro' ), __( 'Templates', 'ultimatewoo-pro' ), 'manage_follow_up_emails', 'followup-emails-templates', 'FUE_Admin_Controller::templates' );
		add_submenu_page( 'followup-emails', __( 'Follow-Up Emails Settings', 'ultimatewoo-pro' ), __( 'Settings', 'ultimatewoo-pro' ), 'manage_follow_up_emails', 'followup-emails-settings', 'FUE_Admin_Controller::settings' );
	}
	...
	public static function dashboard_widget() {
		wp_add_dashboard_widget( 'fue-dashboard', __( 'Follow-Up Emails', 'ultimatewoo-pro' ), array( 'FUE_Report_Dashboard_Widget', 'display' ) );
	}
	...
	public static function send_manual_email_batches() {
		$args = array(
			'page_title'            => __( 'Processing Emails', 'ultimatewoo-pro' ),
			'return_url'            => admin_url( 'admin.php?page=followup-emails' ),
			'ajax_endpoint'         => 'fue_send_manual_email_batches',
			'entity_label_singular' => __( 'batch', 'ultimatewoo-pro' ),
			'entity_label_plural'   => __( 'batches', 'ultimatewoo-pro' ),
			'action_label'          => _x( 'processed', 'Data updater action label', 'ultimatewoo-pro' ),
		);
	...
	public static function clear_scheduler_updater() {
		...
		$args = array(
			'page_title'            => __( 'Data Update', 'ultimatewoo-pro' ),
			'return_url'            => $return,
	...
	public static function queue_table() {
		...
		<script>
			jQuery(document).ready(function($) {
				$("#delete-all-submit").click(function(e) {
					if ( confirm( "<?php _e( 'This will delete ALL scheduled emails! Continue?', 'ultimatewoo-pro' ); ?>" ) ) {
						return true;
					}
					return false;
				});
		...
		<div class="wrap">
			<h2><?php _e( 'Scheduled Emails', 'ultimatewoo-pro' ); ?></h2>
	...
	public static function settings_scripts() {
		...
		if ( in_array( $page, $fue_pages ) || 'follow_up_email' === $screen->post_type ) {
			...
			$translate = apply_filters( 'fue_script_locale', array(
				'email_name'            => __( 'Email Name', 'ultimatewoo-pro' ),
				'processing_request'    => __( 'Processing request...', 'ultimatewoo-pro' ),
				'dupe'                  => __( 'A follow-up email with the same settings already exists. Do you want to create it anyway?', 'ultimatewoo-pro' ),
				'similar'               => __( 'A similar follow-up email already exists. Do you wish to continue?', 'ultimatewoo-pro' ),
				'save'                  => isset( $_GET['mode'] ) ? __( 'Save', 'ultimatewoo-pro' ) : __( 'Build your email', 'ultimatewoo-pro' ),
