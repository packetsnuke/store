...
class Follow_Up_Emails {
	...
	public function __construct( wpdb $wpdb ) {
		...
		self::$durations = apply_filters( 'fue_default_durations', array(
			'minutes' => array( __( 'minute', 'ultimatewoo-pro' ), __( 'minutes', 'ultimatewoo-pro' ) ),
			'hours'   => array( __( 'hour', 'ultimatewoo-pro' ),   __( 'hours', 'ultimatewoo-pro' ) ),
			'days'    => array( __( 'day', 'ultimatewoo-pro' ),    __( 'days', 'ultimatewoo-pro' ) ),
			'weeks'   => array( __( 'week', 'ultimatewoo-pro' ),   __( 'weeks', 'ultimatewoo-pro' ) ),
			'months'  => array( __( 'month', 'ultimatewoo-pro' ),  __( 'months', 'ultimatewoo-pro' ) ),
			'years'   => array( __( 'year', 'ultimatewoo-pro' ),   __( 'years', 'ultimatewoo-pro' ) ),
		) );
	...
	public static function get_email_types() {
		...
		$signup_props = apply_filters( 'fue_signup_email_properties', array(
			'label'             => __( 'Signup Emails', 'ultimatewoo-pro' ),
			'singular_label'    => __( 'Signup Email', 'ultimatewoo-pro' ),
			'triggers'          => array(
				'signup'      => __( 'after user signs up', 'ultimatewoo-pro' ),
				'list_signup' => __( 'after user signs up to a list', 'ultimatewoo-pro' ),
			),
			'supports'          => array(),
			'durations'         => self::$durations,
			'short_description' => __( 'Create emails to send when a user creates an account in your store, or signs up for a list.', 'ultimatewoo-pro' ),
			'long_description'  => __( 'Create emails to send when a user creates an account in your store, or signs up for a list.', 'ultimatewoo-pro' ),
		) );
		$signup = new FUE_Email_Type( 'signup', $signup_props );

		$manual_props = apply_filters( 'fue_manual_email_properties', array(
			'label'                 => __( 'Manual Emails', 'ultimatewoo-pro' ),
			'singular_label'        => __( 'Manual Email', 'ultimatewoo-pro' ),
			'list_template'         => FUE_TEMPLATES_DIR . '/email-list/manual-list.php',
			'triggers'              => array(),
			'supports'              => array(),
			'durations'             => array(),
			'short_description'     => __( 'Create manual emails to have templates, and create newsletters, for ad-hoc emails to customers and prospects.', 'ultimatewoo-pro' ),
			'long_description'      => __( 'Create manual emails to have templates, and create newsletters, for ad-hoc emails to customers and prospects.', 'ultimatewoo-pro' ),
		) );
	...
	public static function get_duration( $duration, $value = 0 ) {
		...
		if ( isset( $durations[ $duration ] ) ) {
			...
			if ( count( $item ) == 2 ) {
				...
				return _n( $item[0], $item[1], $value, 'ultimatewoo-pro' );
