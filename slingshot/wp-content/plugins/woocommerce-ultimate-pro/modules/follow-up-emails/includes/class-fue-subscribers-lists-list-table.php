...
class FUE_Subscribers_Lists_List_Table extends WP_List_Table {
	...
	public function get_columns() {
		return array(
			...
			'list_name'         => __('Name', 'ultimatewoo-pro'),
			'subscriber_count'  => __('Subscribers in List', 'ultimatewoo-pro'),
			'access'            => __('Access', 'ultimatewoo-pro')
		);
	}
	...
	public function single_row( $item ) {
		...
		echo '<td>
				<select class="list-access">
					<option value="0" '. selected( $item->access, 0, false ) .'>'. __('Private', 'ultimatewoo-pro') .'</option>
					<option value="1" '. selected( $item->access, 1, false ) .'>'. __('Public', 'ultimatewoo-pro') .'</option>
				</select>
				<input type="button" class="button button-primary btn-save" value="'. __('Save', 'ultimatewoo-pro') .'" />
				<input type="button" class="button button-secondary btn-cancel" value="'. __('Cancel', 'ultimatewoo-pro') .'" />
			</td>';
		echo '</tr>';
	}
	...
	public function column_list_name( $list ) {
		...
		// Get actions
		$actions = array(
			'id' => sprintf( __( 'ID: %d', 'ultimatewoo-pro' ), $list->id ),
			'edit' => '<a href="#" class="inline-edit" data-id="'. $list->id .'" data-name="'. esc_attr( $list->list_name ) .'" data-access="'. esc_attr( $list->access ) .'">' . __( 'Edit', 'ultimatewoo-pro' ) . '</a>',
			'trash' => '<a class="submitdelete" title="' . esc_attr( __( 'Delete', 'ultimatewoo-pro' ) ) . '" href="#">' . __( 'Delete', 'ultimatewoo-pro' ) . '</a>'
		);
	...
	public function column_access( $list ) {
		return $list->access == 0 ? __('Private', 'ultimatewoo-pro') : __('Public', 'ultimatewoo-pro');
	}
	...
	public function get_bulk_actions() {
		...
		$actions = array(
			'delete_list'    => __( 'Delete', 'ultimatewoo-pro' ),
		);
	...
	protected function extra_tablenav( $which ) {
		...
		if ( 'top' == $which ) { ?>
			...
			<?php submit_button( __( 'Delete ALL', 'ultimatewoo-pro' ), 'button', 'fue_delete_all_lists', false, array( 'id' => 'delete-all-submit' ) ); ?>
			</div><?php
		}
	}
