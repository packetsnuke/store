...
class FUE_AJAX {
	...
	public static function toggle_email_status() {
		...
		if ($status == FUE_Email::STATUS_INACTIVE || $status == FUE_Email::STATUS_ARCHIVED) {
			...
			$resp['new_status'] = __('Active', 'ultimatewoo-pro');
			$resp['new_action'] = __('Deactivate', 'ultimatewoo-pro');
		} else {
			// deactivate
			$email->update_status( FUE_Email::STATUS_INACTIVE );
			$resp['new_status'] = __('Inactive', 'ultimatewoo-pro');
			$resp['new_action'] = __('Activate', 'ultimatewoo-pro');
		}
	...
	public static function toggle_queue_status() {
		...
		if ($status == 0) {
			...
			$resp['new_status'] = __('Queued', 'ultimatewoo-pro');
			$resp['new_action'] = __('Do not send', 'ultimatewoo-pro');
		} else {
			// deactivate
			...
			$resp['new_status'] = __('Suspended', 'ultimatewoo-pro');
			$resp['new_action'] = __('Re-enable', 'ultimatewoo-pro');
		}
	...
	public static function load_customer_notes() {
		...
		foreach ( $notes as $note ) {
			...
			<li class="note" rel="'. $note->id .'">
				...
				<p class="meta">
					'. sprintf( __('added by %s on <abbr title="%s" class="exact-date">%s</abbr>', 'ultimatewoo-pro'), $author->display_name, $note->date_added, $pretty_date ) .'
					<a class="delete_note" href="#">'. __('Delete note', 'ultimatewoo-pro') .'</a>
				</p>
			</li>';
		}
	...
	public static function add_customer_note() {
		...
		$out = '
			<li class="note" rel="'. $note_id .'">
				...
				<p class="meta">
					'. sprintf( __('added by %s on <abbr title="%s" class="exact-date">%s</abbr>', 'ultimatewoo-pro'), $author->display_name, $now, $pretty_date ) .'
					<a class="delete_note" href="#">'. __('Delete note', 'ultimatewoo-pro') .'</a>
				</p>
			</li>';
	...
	public static function add_customer_reminder() {
		...
		if ( !$date ) {
			die('<li>'. __('Cannot create a reminder without a date', 'ultimatewoo-pro') .'</li>');
		}
		...
		$subject    = sprintf( __('Reminder for Customer "%s"', 'ultimatewoo-pro'), $user->display_name );
		$message    = sprintf( __('<p>Hello %s,</p><p>This is your reminder about customer "%s".</p>', 'ultimatewoo-pro'), $assignee->display_name, $user->display_name );

		if ( !empty( $note ) ) {
			$message .= '<p>'. $note .'</p>';
		}

		$message .= sprintf(__('<p>To view your customer details for "%s", <a href="%s">click here</a>', 'ultimatewoo-pro'), $user->display_name, $url);
		...
		$out = '
			<li class="reminder" rel="'. $item->id .'">
				<div class="reminder-content">
					<p>
						'. sprintf( __('Reminder set for %s at %s', 'ultimatewoo-pro'), $date, $time ) .'
					</p>
					'. $pre .'
				</div>
				<p class="meta">
					'. $meta .'
					<a class="delete_reminder" href="#">'. __('Delete', 'ultimatewoo-pro') .'</a>
				</p>
			</li>';
	...
	public static function schedule_manual_email() {
		...
		if ( empty( $data['email'] ) ) {
			wp_send_json(array(
				'status'    => 'error',
				'message'   => __('Email is empty', 'ultimatewoo-pro')
			));
		}
		...
		if ( empty( $data['user'] ) ) {
			wp_send_json(array(
				'status'    => 'error',
				'message'   => __('Recipient is missing', 'ultimatewoo-pro')
			));
		}
		...
		if ( $data['sending_schedule'] == 'later' && empty($data['send_date']) ) {
			wp_send_json(array(
				'status'    => 'error',
				'message'   => __('Cannot schedule email without a send date', 'ultimatewoo-pro')
			));
		}
		...
		if ( $data['send_again'] && empty( $data['send_again_value'] ) ) {
			wp_send_json(array(
				'status'    => 'error',
				'message'   => __('Invalid resend schedule', 'ultimatewoo-pro')
			));
		}
		...
		if ( is_array( $items ) ) {
			...
			foreach ( $items as $item_id ) {
				if ( is_wp_error( $item_id ) ) {
					...
					$note       = sprintf(
						__('Email queued: %s scheduled on %s', 'ultimatewoo-pro'),
						$email->name,
						$send_on
					);
				}
		...
		if ( empty( $errors ) ) {
			wp_send_json(array(
				'status'    => 'ok',
				'message'   => __('Email added to the queue', 'ultimatewoo-pro')
			));
		} else {
			...
			wp_send_json(array(
			   'status'     => 'error',
			   'message'    => sprintf(
				   __('An error occurred while trying to schedule the email "%s". %s.', 'ultimatewoo-pro'),
				   $email->name,
				   $error
			   )
			));
	...
	public static function archive_email() {
		...
		$resp['status_html'] = __('Archived', 'ultimatewoo-pro') .'<br/><small><a href="#" class="unarchive" data-id="'. $id .'" data-key="'. $type .'">'. __('Activate', 'ultimatewoo-pro') .'</a></small>';
	...
	public static function unarchive_email() {
		...
		$resp['status_html'] = __('Active', 'ultimatewoo-pro') .'<br/><small><a href="#" class="toggle-activation" data-id="'. $id .'">'. __('Deactivate', 'ultimatewoo-pro') .'</a></small>
		|
		<small><a href="#" class="archive-email" data-id="'. $id .'" data-key="'. $email->get_type() .'">'. __('Archive', 'ultimatewoo-pro') .'</a></small>';
	...
	public static function load_template_source() {
		...
		if ( !wp_verify_nonce( $_GET['security'], 'get_template_html' ) ) {
			die(__('Error: Invalid request. Please try again.', 'ultimatewoo-pro'));
		}
	...
	public static function save_template_source() {
		if ( !wp_verify_nonce( $_POST['security'], 'save_template_html' ) ) {
			self::send_response(array(
				'status'    => 'ERR',
				'error'     => __('Invalid request. Please try again.', 'ultimatewoo-pro')
			));
		}
	...
	public static function bounce_emails_test_check() {
		...
		$return['success'] = true;
		$count = $pop3->COUNT;
		$return['msg'] = __('checking for new messages', 'ultimatewoo-pro').str_repeat('.', $passes);

		if ( $passes > 20 ) {
			$return['complete'] = true;
			$return['msg'] = __('<span class="dashicons dashicons-no"></span> Unable to receive test message! Please check your settings.', 'ultimatewoo-pro');
		}
		...
		for ( $i = 1; $i <= $count; $i++ ) {
			...
			if ( strpos( $message, $identifier ) !== false ) {
				...
				$pop3->quit();
				$return['complete'] = true;
				$return['msg'] = __('<span class="dashicons dashicons-yes"></span> Your bounce server is configured!', 'ultimatewoo-pro');
	...
	public static function generate_dkim_keys() {
		if ( !function_exists('openssl_pkey_new') ) {
			self::send_response( array(
				'status' => false,
				'error'  => __('Please enable the OpenSSL extension in your PHP installation', 'ultimatewoo-pro')
			) );
		}
			...
			foreach ( $recipients_part as $recipient_key => $recipient ) {
				...
				if ( $data['recipient_number'] == $emails_per_batch ) {
					...
					$send_data[] = array(
						'status'        => 'success',
						'status_text'   => sprintf(
							__('Emails in batch %d have been processed', 'ultimatewoo-pro'),
							$data['batch_number']
						)
					);
