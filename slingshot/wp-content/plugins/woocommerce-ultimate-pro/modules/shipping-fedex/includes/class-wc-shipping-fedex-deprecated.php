...
class WC_Shipping_Fedex extends WC_Shipping_Method {
	...
	public function __construct() {
		$this->id                               = 'fedex';
		$this->method_title                     = __( 'FedEx', 'ultimatewoo-pro' );
		$this->method_description               = __( 'The <strong>FedEx</strong> extension obtains rates dynamically from the FedEx API during cart/checkout.', 'ultimatewoo-pro' );
	...
	private function environment_check() {
		if ( ! in_array( get_woocommerce_currency(), array( 'USD', 'CAD' ) ) || ! in_array( WC()->countries->get_base_country(), array( 'US', 'CA' ) ) ) {
			echo '<div class="error">
				<p>' . __( 'FedEx requires that the WooCommerce currency is set to US Dollars and that the base country/region is set to United States.', 'ultimatewoo-pro' ) . '</p>
			</div>';
		} elseif ( ! $this->origin && $this->enabled == 'yes' ) {
			echo '<div class="error">
				<p>' . __( 'FedEx is enabled, but the origin postcode has not been set.', 'ultimatewoo-pro' ) . '</p>
			</div>';
		}
	}
	...
	private function per_item_shipping( $package ) {
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			if ( ! $values['data']->needs_shipping() ) {
				$this->debug( sprintf( __( 'Product # is virtual. Skipping.', 'ultimatewoo-pro' ), $item_id ), 'notice' );
				continue;
			}

			if ( ! $values['data']->get_weight() ) {
				$this->debug( sprintf( __( 'Product # is missing weight. Aborting.', 'ultimatewoo-pro' ), $item_id ), 'error' );
				return;
			}
	...
	private function box_shipping( $package ) {
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			if ( $values['data']->length && $values['data']->height && $values['data']->width && $values['data']->weight ) {
				...
				$this->debug( sprintf( __( 'Product #%s is missing dimensions. Aborting.', 'ultimatewoo-pro' ), $item_id ), 'error' );
				return;
			}
		}
	...
	public function residential_address_validation( $package ) {
		...
		if ( $this->residential == false ) {
			$this->debug( __( 'Business Address', 'ultimatewoo-pro' ) );
		}
	}
	...
	public function calculate_shipping( $package = array() ) {
		...
		// Debugging
		$this->debug( __( 'FEDEX debug mode is on - to hide these messages, turn debug mode off in the settings.', 'ultimatewoo-pro' ) );
