...
class WC_Shipping_Fedex_Init {
	...
	public function environment_check() {
		...
		if ( ! in_array( get_woocommerce_currency(), array( 'USD', 'CAD' ) ) || ! in_array( WC()->countries->get_base_country(), array( 'US', 'CA' ) ) ) {
			echo '<div class="error">
				<p>' . __( 'FedEx requires that the WooCommerce currency is set to US Dollars or CA Dollars and that the base country/region is set to United States or Canada.', 'ultimatewoo-pro' ) . '</p>
			</div>';
		}
	}
	...
	public function plugin_links( $links ) {
		$plugin_links = array(
			'<a href="' . admin_url( 'admin.php?page=wc-settings&tab=shipping&section=fedex' ) . '">' . __( 'Settings', 'ultimatewoo-pro' ) . '</a>',
			'<a href="https://support.woocommerce.com/">' . __( 'Support', 'ultimatewoo-pro' ) . '</a>',
			'<a href="https://docs.woocommerce.com/document/fedex/">' . __( 'Docs', 'ultimatewoo-pro' ) . '</a>',
		);
	...
	public function wc_deactivated() {
		echo '<div class="error"><p>' . sprintf( __( 'WooCommerce FedEx Shipping requires %s to be installed and active.', 'ultimatewoo-pro' ), '<a href="https://woocommerce.com" target="_blank">WooCommerce</a>' ) . '</p></div>';
	}
	...
	public function upgrade_notice() {
		...
		<div class="notice notice-success is-dismissible wc-fedex-notice">
			<p><?php echo sprintf( __( 'FedEx now supports shipping zones. The zone settings were added to a new FedEx method on the "Rest of the World" Zone. See the zones %1$shere%2$s ', 'ultimatewoo-pro' ),'<a href="' . $zones_admin_url . '">','</a>' ); ?></p>
		</div>
