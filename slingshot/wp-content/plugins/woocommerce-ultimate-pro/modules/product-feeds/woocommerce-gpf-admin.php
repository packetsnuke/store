...
class WoocommerceGpfAdmin {
	...
	function admin_init() {
		...
		if ( isset( $this->settings['product_fields'] ) && count( $this->settings['product_fields'] ) ) {
			add_meta_box(
				...
				__( 'Product Feed Information', 'ultimatewoo-pro' ),
	...
	function enqueue_styles() {
		wp_enqueue_style( 'woocommerce_gpf_admin', ULTIMATEWOO_MODULES_URL . 'product-feeds/css/woocommerce-gpf.css' );
		wp_enqueue_style( 'wooautocomplete', ULTIMATEWOO_MODULES_URL . 'product-feeds/js/jquery.autocomplete.css' );
	...
	function enqueue_scripts() {
		wp_enqueue_script( 'wooautocomplete', ULTIMATEWOO_MODULES_URL . 'product-feeds/js/jquery.autocomplete.js', array( 'jquery', 'jquery-ui-core' ) );
		wp_enqueue_script( 'woocommerce_gpf', ULTIMATEWOO_MODULES_URL . 'product-feeds/js/woocommerce-gpf.js', array( 'jquery' ) );
		wp_enqueue_script( 'jquery-ui-datepicker' );
	}
	...
	public function variation_input_fields( $loop_idx, $variation_data, $variation ) {
		...
		echo '<h2><strong>';
		echo __( 'Product Feed Information', 'ultimatewoo-pro' );
		echo '</strong><div class="handlediv" aria-label="Click to toggle"></div>';
		echo '</h2>';
		echo '<div class="wc_gpf_metabox_content" style="display:none;">';
		echo '<p>' . __( 'Set values here if you want to override the information for this specific variation. If information should apply to all variations, then set it against the main product.', 'ultimatewoo-pro' ) . '</p>';
		...
		foreach ( $this->product_fields as $key => $fieldinfo ) {
			...
			if ( isset( $fieldinfo['can_default'] ) && ! empty( $product_defaults[ $key ] ) ) {
				$variables['field_defaults'] .= $this->template_loader->get_template_with_variables(
					...
					array(
						'default' => '(' . __( 'Default: ', 'ultimatewoo-pro' ) . esc_html( $product_defaults[ $key ] ) . ')'
					)
				);
				$placeholder = __( 'Use default', 'woo_gpf' );
			}
	...
	private function default_field_variables($key, $loop_idx = null) {
		...
		if ( isset( $_REQUEST['post'] ) || isset( $_REQUEST['taxonomy'] ) || $loop_idx !== null) {
			$variables['emptytext'] = __( 'Use default', 'ultimatewoo-pro' );
		} else {
			$variables['emptytext'] = __( 'No default', 'ultimatewoo-pro' );
		}
		return $variables;
	}
	...
	private function render_exclude_product( $key, $current_data = false, $placeholder = null, $loop_idx = null ) {
		...
		$variables['hide_product_text'] = __( 'Hide this product from the feed', 'ultimatewoo-pro' );
		if ( $loop_idx !== null ) {
	...
	function add_woocommerce_settings_tab( $tabs ) {
		$tabs['gpf'] = __( 'Product Feeds', 'ultimatewoo-pro' );
		return $tabs;
	}
	...
	function config_page() {
		...
		if ( $this->cache->is_enabled() ) {
			...
			$cache_status_variables = array(
				'google_cache_status' => sprintf( __( '<strong>%d</strong> / <strong>%d</strong> generated', 'ultimatewoo-pro' ), $status['google'], $total_cache ),
				'google_inventory_cache_status' => sprintf( __( '<strong>%d</strong> / <strong>%d</strong> generated', 'ultimatewoo-pro' ), $status['googleinventory'], $total_cache ),
				'bing_cache_status' => sprintf( __( '<strong>%d</strong> / <strong>%d</strong> generated', 'ultimatewoo-pro' ), $status['bing'], $total_cache ),
		...
		foreach ( $this->product_fields as $key => $info ) {
			...
			if ( isset( $this->product_fields[ $key ]['can_default'] ) ) {
				$def_vars['defaultinput'] = __( 'Store default: <br>', 'ultimatewoo-pro' ) . $this->render_field_default_input( $key );
			} else {
