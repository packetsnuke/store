...
class WC_Gateway_Stripe_Sepa extends WC_Stripe_Payment_Gateway {
	...
	public function __construct() {
		$this->id                   = 'stripe_sepa';
		$this->method_title         = __( 'Stripe SEPA Direct Debit', 'ultimatewoo-pro' );
		/* translators: link */
		$this->method_description   = sprintf( __( 'All other general Stripe settings can be adjusted <a href="%s">here</a>.', 'ultimatewoo-pro' ), admin_url( 'admin.php?page=wc-settings&tab=checkout&section=stripe' ) );
		$this->supports             = array(
			'products',
			'refunds',
			'tokenization',
	...
	public function get_environment_warning() {
		if ( 'yes' === $this->enabled && ! in_array( get_woocommerce_currency(), $this->get_supported_currency() ) ) {
			$message = __( 'SEPA is enabled - it requires store currency to be set to Euros.', 'ultimatewoo-pro' );
	...
	public function mandate_display() {
		...
		printf( __( 'By providing your IBAN and confirming this payment, you are authorizing %s and Stripe, our payment service provider, to send instructions to your bank to debit your account and your bank to debit your account in accordance with those instructions. You are entitled to a refund from your bank under the terms and conditions of your agreement with your bank. A refund must be claimed within 8 weeks starting from the date on which your account was debited.', 'ultimatewoo-pro' ), WC_Stripe_Helper::clean_statement_descriptor( $this->statement_descriptor ) );
	}
		...
		<fieldset id="wc-<?php echo esc_attr( $this->id ); ?>-form" class="wc-payment-form">
			...
			<p class="form-row form-row-wide validate-required">
				<label for="stripe-sepa-owner">
					<?php esc_html_e( 'IBAN Account Name.', 'ultimatewoo-pro' ); ?>
				</label>
				<input id="stripe-sepa-owner" name="stripe_sepa_owner" value="" />
			</p>
			<p class="form-row form-row-wide validate-required">
				<label for="stripe-sepa-iban">
					<?php esc_html_e( 'IBAN Account Number.', 'ultimatewoo-pro' ); ?>
				</label>
	...
	public function payment_fields() {
		...
		if ( is_add_payment_method_page() ) {
			$pay_button_text = __( 'Add Payment', 'ultimatewoo-pro' );
			$total        = '';
		} else {
			$pay_button_text = '';
		}
		...
		if ( $this->description ) {
			if ( $this->testmode ) {
				$this->description .= ' ' . __( 'TEST MODE ENABLED. In test mode, you can use IBAN number DE89370400440532013000.', 'ultimatewoo-pro' );
