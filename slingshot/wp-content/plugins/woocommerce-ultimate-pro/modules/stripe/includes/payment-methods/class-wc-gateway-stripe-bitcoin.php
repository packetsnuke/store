...
class WC_Gateway_Stripe_Bitcoin extends WC_Stripe_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_title         = __( 'Stripe Bitcoin', 'ultimatewoo-pro' );
		/* translators: link */
	...
	public function get_environment_warning() {
		if ( 'yes' === $this->enabled && ! in_array( get_woocommerce_currency(), $this->get_supported_currency() ) ) {
			$message = __( 'Bitcoin is enabled - it requires store currency to be set to USD.', 'ultimatewoo-pro' );
	...
	public function get_instructions( $order_id, $plain_text = false ) {
		...
		if ( $plain_text ) {
			esc_html_e( 'Please pay the following:', 'ultimatewoo-pro' ) . "\n\n";
			echo "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=\n\n";
			esc_html_e( 'Bitcoin Amount:', 'ultimatewoo-pro' ) . "\n\n";
			echo $data['amount'] . "\n\n";
			echo "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=\n\n";
			esc_html_e( 'Receiver:', 'ultimatewoo-pro' ) . "\n\n";
			echo $data['address'] . "\n\n";
			echo "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=\n\n";
			esc_html_e( 'URI:', 'ultimatewoo-pro' ) . "\n\n";
			echo $data['uri'] . "\n\n";
		} else {
			?>
			<h3><?php esc_html_e( 'Please pay the following:', 'ultimatewoo-pro' ); ?></h3>
			<ul class="woocommerce-order-overview woocommerce-thankyou-order-details order_details">
			<li class="woocommerce-order-overview__order order">
				<?php esc_html_e( 'Bitcoin Amount:', 'ultimatewoo-pro' ); ?>
				<strong><?php echo $data['amount']; ?></strong>
			</li>
			<li class="woocommerce-order-overview__order order">
				<?php esc_html_e( 'Receiver:', 'ultimatewoo-pro' ); ?>
				<strong><?php echo $data['address']; ?></strong>
			</li>
			<li class="woocommerce-order-overview__order order">
				<?php esc_html_e( 'URI:', 'ultimatewoo-pro' ); ?>
				<strong>
				<?php
				/* translators: link */
				printf( __( '<a href="%s">Pay Bitcoin</a>', 'ultimatewoo-pro' ), $data['uri'] );
				?>
				</strong>
			</li>
			</ul>
	...
	public function process_payment( $order_id, $retry = true, $force_save_source = false ) {
		try {
			...
			$order->update_status( 'on-hold', __( 'Awaiting Bitcoin payment', 'ultimatewoo-pro' ) );
