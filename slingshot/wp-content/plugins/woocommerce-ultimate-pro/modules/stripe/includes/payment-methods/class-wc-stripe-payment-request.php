...
class WC_Stripe_Payment_Request {
	...
	public function get_product_data() {
		...
		if ( wc_shipping_enabled() && $product->needs_shipping() ) {
			$items[] = array(
				'label'   => __( 'Shipping', 'ultimatewoo-pro' ),
				'amount'  => 0,
				'pending' => true,
			);
			...
			$data['shippingOptions']  = array(
				'id'     => 'pending',
				'label'  => __( 'Pending', 'ultimatewoo-pro' ),
				'detail' => '',
				'amount' => 0,
			);
		}
	...
	public function scripts() {
		...
		wp_localize_script(
			...
			array(
				...
				'i18n'     => array(
					'no_prepaid_card'  => __( 'Sorry, we\'re not accepting prepaid cards at this time.', 'ultimatewoo-pro' ),
					/* translators: Do not translate the [option] placeholder */
					'unknown_shipping' => __( 'Unknown shipping option "[option]".', 'ultimatewoo-pro' ),
				),
				'checkout' => array(
	...
	public function display_payment_request_button_separator_html() {
		...
		?>
		<p id="wc-stripe-payment-request-button-separator" style="margin-top:1.5em;text-align:center;display:none;">- <?php esc_html_e( 'OR', 'ultimatewoo-pro' ); ?> -</p>
		<?php
	}
			...
			if ( ! empty( $packages ) && WC()->customer->has_calculated_shipping() ) {
				foreach ( $packages as $package_key => $package ) {
					if ( empty( $package['rates'] ) ) {
						throw new Exception( __( 'Unable to find shipping method for address.', 'ultimatewoo-pro' ) );
					}
	...
	public function ajax_create_order() {
		if ( WC()->cart->is_empty() ) {
			wp_send_json_error( __( 'Empty cart', 'ultimatewoo-pro' ) );
		}
	...
	protected function build_display_items() {
		...
		if ( wc_tax_enabled() ) {
			$items[] = array(
				'label'  => esc_html( __( 'Tax', 'ultimatewoo-pro' ) ),
		...
		if ( WC()->cart->needs_shipping() ) {
			$items[] = array(
				'label'  => esc_html( __( 'Shipping', 'ultimatewoo-pro' ) ),
