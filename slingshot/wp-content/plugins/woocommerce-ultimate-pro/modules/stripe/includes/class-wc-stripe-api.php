...
class WC_Stripe_API {
	...
	public static function request( $request, $api = 'charges', $method = 'POST' ) {
		...
		if ( is_wp_error( $response ) || empty( $response['body'] ) ) {
			...
			throw new Exception( __( 'There was a problem connecting to the Stripe API endpoint.', 'ultimatewoo-pro' ) );
		}
	...
	public static function retrieve( $api ) {
		...
		if ( is_wp_error( $response ) || empty( $response['body'] ) ) {
			WC_Stripe_Logger::log( 'Error Response: ' . print_r( $response, true ) );
			return new WP_Error( 'stripe_error', __( 'There was a problem connecting to the Stripe API endpoint.', 'ultimatewoo-pro' ) );
		}
