...
class WC_Stripe_Order_Handler extends WC_Stripe_Payment_Gateway {
	...
	public function process_redirect_payment( $order_id, $retry = true ) {
		try {
			...
			if ( 'failed' === $source_info->status || 'canceled' === $source_info->status ) {
				throw new Exception( __( 'Unable to process this payment, please try again or use alternative method.', 'ultimatewoo-pro' ) );
			}
			...
			$order->update_status( 'failed', sprintf( __( 'Stripe payment failed: %s', 'ultimatewoo-pro' ), $e->getMessage() ) );
		...
		if ( 'stripe' === ( WC_Stripe_Helper::is_pre_30() ? $order->payment_method : $order->get_payment_method() ) ) {
			...
			if ( $charge && 'no' === $captured ) {
				...
				if ( ! empty( $result->error ) ) {
					/* translators: error message */
					$order->update_status( 'failed', sprintf( __( 'Unable to capture charge! %s', 'ultimatewoo-pro' ), $result->error->message ) );
				} else {
					/* translators: transaction id */
					$order->add_order_note( sprintf( __( 'Stripe charge complete (Charge ID: %s)', 'ultimatewoo-pro' ), $result->id ) );
	...
	public function cancel_payment( $order_id ) {
		...
		if ( 'stripe' === ( WC_Stripe_Helper::is_pre_30() ? $order->payment_method : $order->get_payment_method() ) ) {
			...
			if ( $charge_id ) {
				...
				if ( ! empty( $result->error ) ) {
					$order->add_order_note( __( 'Unable to refund charge!', 'ultimatewoo-pro' ) . ' ' . $result->error->message );
				} else {
					/* translators: transaction id */
					$order->add_order_note( sprintf( __( 'Stripe charge refunded (Charge ID: %s)', 'ultimatewoo-pro' ), $result->id ) );
	...
	public function normalize_field( $field ) {
		...
		if ( array_key_exists( $field, $checkout_fields['billing'] ) ) {
			...
			$org_str[]     = 'owner';
			$replace_str[] = __( 'Owner', 'ultimatewoo-pro' );
	...
	public function validate_checkout() {
		...
		foreach ( $required_fields as $field => $field_value ) {
			...
			if ( empty( $field_value ) || '-1' === $field_value ) {
				...
				$errors->add( 'validation', sprintf( __( '%s cannot be empty', 'ultimatewoo-pro' ), $error_field ) );
			}
		}

		// Check if email is valid format.
		if ( ! empty( $required_fields['billing_email'] ) && ! is_email( $required_fields['billing_email'] ) ) {
			$errors->add( 'validation', __( 'Email is not valid', 'ultimatewoo-pro' ) );
		}
		...
		if ( ! empty( $required_fields['billing_phone'] ) ) {
			...
			if ( '' !== $phone && ! WC_Validation::is_phone( $phone ) ) {
				...
				$errors->add( 'validation', __( 'Please enter a valid phone number.', 'ultimatewoo-pro' ) );
			}
		}
		...
		if ( ! empty( $required_fields['billing_postcode'] ) ) {
			...
			if ( '' !== $required_fields['billing_postcode'] && ! WC_Validation::is_postcode( $postcode, $country ) ) {
				$errors->add( 'validation', __( 'Please enter a valid billing postcode / ZIP.', 'ultimatewoo-pro' ) );
			}
		}
		...
		if ( ( isset( $_POST['is_add_payment_page'] ) && 'no' === $_POST['is_add_payment_page'] ) ) {
			if ( empty( $all_fields['woocommerce_checkout_update_totals'] ) && empty( $all_fields['terms'] ) && apply_filters( 'woocommerce_checkout_show_terms', wc_get_page_id( 'terms' ) > 0 ) ) {
				$errors->add( 'terms', __( 'You must accept our Terms &amp; Conditions.', 'ultimatewoo-pro' ) );
			}
		}
		...
		if ( WC()->cart->needs_shipping() && $validate_shipping_fields ) {
			...
			if ( ! empty( $required_fields['shipping_postcode'] ) ) {
				...
				if ( '' !== $required_fields['shipping_postcode'] && ! WC_Validation::is_postcode( $postcode, $country ) ) {
					$errors->add( 'validation', __( 'Please enter a valid shipping postcode / ZIP.', 'ultimatewoo-pro' ) );
				}
			}
		}
		...
		if ( WC()->cart->needs_shipping() ) {
			...
			if ( empty( $shipping_country ) ) {
				$errors->add( 'shipping', __( 'Please enter an address to continue.', 'ultimatewoo-pro' ) );
			} elseif ( ! in_array( WC()->customer->get_shipping_country(), array_keys( WC()->countries->get_shipping_countries() ) ) ) {
				/* translators: country name */
				$errors->add( 'shipping', sprintf( __( 'Unfortunately <strong>we do not ship %s</strong>. Please enter an alternative shipping address.', 'ultimatewoo-pro' ), WC()->countries->shipping_to_prefix() . ' ' . WC()->customer->get_shipping_country() ) );
			} else {
				$chosen_shipping_methods = WC()->session->get( 'chosen_shipping_methods' );

				foreach ( WC()->shipping->get_packages() as $i => $package ) {
					if ( ! isset( $chosen_shipping_methods[ $i ], $package['rates'][ $chosen_shipping_methods[ $i ] ] ) ) {
						$errors->add( 'shipping', __( 'No shipping method has been selected. Please double check your address, or contact us if you need any help.', 'ultimatewoo-pro' ) );
					}
				}
			}
		}
		...
		if ( WC()->cart->needs_payment() ) {
			...
			if ( ! isset( $available_gateways[ $all_fields['payment_method'] ] ) ) {
				$errors->add( 'payment', __( 'Invalid payment method.', 'ultimatewoo-pro' ) );
			} else {
