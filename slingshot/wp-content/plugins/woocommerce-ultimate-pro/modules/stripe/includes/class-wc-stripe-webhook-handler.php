...
class WC_Stripe_Webhook_Handler extends WC_Stripe_Payment_Gateway {
	...
	public function process_webhook_dispute( $notification ) {
		...
		$order->update_status( 'on-hold', __( 'A dispute was created for this order. Response is needed. Please go to your Stripe Dashboard to review this dispute.', 'ultimatewoo-pro' ) );
	...
	public function process_webhook_capture( $notification ) {
		...
		if ( 'stripe' === ( WC_Stripe_Helper::is_pre_30() ? $order->payment_method : $order->get_payment_method() ) ) {
			...
			if ( $charge && 'no' === $captured ) {
				...
				$order->update_status( $order->needs_processing() ? 'processing' : 'completed', sprintf( __( 'Stripe charge complete (Charge ID: %s)', 'ultimatewoo-pro' ), $notification->data->object->id ) );

				// Check and see if capture is partial.
				if ( $this->is_partial_capture( $notification ) ) {
					$order->set_total( $this->get_partial_amount_to_charge( $notification ) );
					$order->add_note( __( 'This charge was partially captured via Stripe Dashboard', 'ultimatewoo-pro' ) );
					$order->save();
				}
			}
		}
	...
	public function process_webhook_charge_failed( $notification ) {
		...
		$order->update_status( 'failed', __( 'This payment failed to clear.', 'ultimatewoo-pro' ) );
	...
	public function process_webhook_source_canceled( $notification ) {
		...
		$order->update_status( 'cancelled', __( 'This payment has cancelled.', 'ultimatewoo-pro' ) );
		...
		if ( 'stripe' === ( WC_Stripe_Helper::is_pre_30() ? $order->payment_method : $order->get_payment_method() ) ) {
			...
			if ( $charge && 'yes' === $captured ) {
				...
				$refund = wc_create_refund( array(
					...
					'reason'         => __( 'Refunded via Stripe Dashboard', 'ultimatewoo-pro' ),
				) );
