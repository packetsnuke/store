...
abstract class WC_Stripe_Payment_Gateway extends WC_Payment_Gateway_CC {
	...
	public function validate_minimum_order_amount( $order ) {
		if ( $order->get_total() * 100 < WC_Stripe_Helper::get_minimum_amount() ) {
			...
			throw new Exception( sprintf( __( 'Sorry, the minimum allowed order total is %1$s to use this payment method.', 'ultimatewoo-pro' ), wc_price( WC_Stripe_Helper::get_minimum_amount() / 100 ) ) );
		}
	}
	...
	public function generate_payment_request( $order, $source ) {
		...
		$post_data['description']          = sprintf( __( '%1$s - Order %2$s', 'ultimatewoo-pro' ), wp_specialchars_decode( get_bloginfo( 'name' ), ENT_QUOTES ), $order->get_order_number() );
		...
		$metadata = array(
			__( 'customer_name', 'ultimatewoo-pro' ) => sanitize_text_field( $billing_first_name ) . ' ' . sanitize_text_field( $billing_last_name ),
			__( 'customer_email', 'ultimatewoo-pro' ) => sanitize_email( $billing_email ),
			'order_id' => WC_Stripe_Helper::is_pre_30() ? $order->id : $order->get_id(),
		);
	...
	public function process_response( $response, $order ) {
		...
		if ( $response->captured ) {
			...
			if ( 'pending' === $response->status ) {
				...
				$order->update_status( 'on-hold', sprintf( __( 'Stripe charge awaiting payment: %s.', 'ultimatewoo-pro' ), $response->id ) );
			}
			...
			if ( 'succeeded' === $response->status ) {
				...
				/* translators: transaction id */
				$message = sprintf( __( 'Stripe charge complete (Charge ID: %s)', 'ultimatewoo-pro' ), $response->id );
			...
			/* translators: transaction id */
			$order->update_status( 'on-hold', sprintf( __( 'Stripe charge authorized (Charge ID: %s). Process order to take payment, or cancel to remove the pre-authorization.', 'ultimatewoo-pro' ), $response->id ) );
		}
	...
	public function prepare_source( $user_id, $force_save_source = false ) {
		...
		if ( ! empty( $_POST['stripe_source'] ) ) {
			...
			if ( ! $wc_token || $wc_token->get_user_id() !== get_current_user_id() ) {
				...
				throw new Exception( __( 'Invalid payment method. Please input a new card number.', 'ultimatewoo-pro' ) );
			}
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if ( ! empty( $response->error ) ) {
			...
			$refund_message = sprintf( __( 'Refunded %1$s - Refund ID: %2$s - Reason: %3$s', 'ultimatewoo-pro' ), $amount, $response->id, $reason );
	...
	public function add_payment_method() {
		$error     = false;
		$error_msg = __( 'There was a problem adding the card.', 'ultimatewoo-pro' );
		$source_id = '';
