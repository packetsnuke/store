...
class WC_Stripe_Compat extends WC_Gateway_Stripe {
	...
	public function process_subscription_payment( $order = '', $amount = 0 ) {
		if ( $amount * 100 < WC_Stripe_Helper::get_minimum_amount() ) {
			...
			return new WP_Error( 'stripe_error', sprintf( __( 'Sorry, the minimum allowed order total is %1$s to use this payment method.', 'ultimatewoo-pro' ), wc_price( WC_Stripe_Helper::get_minimum_amount() / 100 ) ) );
		}
		...
		if ( ! $prepared_source->customer ) {
			return new WP_Error( 'stripe_error', __( 'Customer not found', 'ultimatewoo-pro' ) );
		}
	...
	public function scheduled_subscription_payment( $amount_to_charge, $renewal_order ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			$renewal_order->update_status( 'failed', sprintf( __( 'Stripe Transaction Failed (%s)', 'ultimatewoo-pro' ), $response->get_error_message() ) );
		}

		if ( ! empty( $response->error ) ) {
			/* translators: error message */
			$renewal_order->update_status( 'failed', sprintf( __( 'Stripe Transaction Failed (%s)', 'ultimatewoo-pro' ), $response->error->message ) );
		}
	}
		...
		if ( $sources ) {
			...
			foreach ( $sources as $source ) {
				...
				if ( $source->id === $stripe_source_id ) {
					...
					$payment_method_to_display = sprintf( __( 'Via %1$s card ending in %2$s', 'ultimatewoo-pro' ), ( isset( $card->brand ) ? $card->brand : __( 'N/A', 'ultimatewoo-pro' ) ), $card->last4 );
					break;
				}
			}
		...
		if ( WC_Pre_Orders_Order::order_requires_payment_tokenization( $order_id ) ) {
			try {
				...
				if ( empty( $source->customer ) || empty( $source->source ) ) {
					throw new Exception( __( 'Unable to store payment details. Please try again.', 'ultimatewoo-pro' ) );
				}
	...
	public function process_pre_order_release_payment( $order ) {
		try {
			...
			/* translators: error message */
			$order_note = sprintf( __( 'Stripe Transaction Failed (%s)', 'ultimatewoo-pro' ), $e->getMessage() );
