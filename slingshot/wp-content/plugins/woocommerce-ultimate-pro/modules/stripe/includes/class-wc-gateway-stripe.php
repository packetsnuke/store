...
class WC_Gateway_Stripe extends WC_Stripe_Payment_Gateway {
	...
	public function __construct() {
		$this->id                   = 'stripe';
		$this->method_title         = __( 'Stripe', 'ultimatewoo-pro' );
		/* translators: 1) link to Stripe register page 2) link to Stripe api keys page */
		$this->method_description   = sprintf( __( 'Stripe works by adding payment fields on the checkout and then sending the details to Stripe for verification. <a href="%1$s" target="_blank">Sign up</a> for a Stripe account, and <a href="%2$s" target="_blank">get your Stripe account keys</a>.', 'ultimatewoo-pro' ), 'https://dashboard.stripe.com/register', 'https://dashboard.stripe.com/account/apikeys' );
		...
		if ( $this->stripe_checkout ) {
			$this->order_button_text = __( 'Continue to payment', 'ultimatewoo-pro' );
		}
	...
	private function register_apple_pay_domain( $secret_key = '' ) {
		if ( empty( $secret_key ) ) {
			throw new Exception( __( 'Unable to verify domain - missing secret key.', 'ultimatewoo-pro' ) );
		}
		...
		if ( is_wp_error( $response ) ) {
			...
			throw new Exception( sprintf( __( 'Unable to verify domain - %s', 'ultimatewoo-pro' ), $response->get_error_message() ) );
		}
		...
		if ( 200 !== $response['response']['code'] ) {
			...
			/* translators: error message */
			throw new Exception( sprintf( __( 'Unable to verify domain - %s', 'ultimatewoo-pro' ), $parsed_response->error->message ) );
		}
	}
		...
		try {
			...
			if ( ! file_exists( $path . '/' . $dir ) ) {
				if ( ! @mkdir( $path . '/' . $dir, 0755 ) ) {
					throw new Exception( __( 'Unable to create domain association folder to domain root.', 'ultimatewoo-pro' ) );
				}
			}

			if ( ! file_exists( $fullpath ) ) {
				if ( ! @copy( WC_STRIPE_PLUGIN_PATH . '/' . $file, $fullpath ) ) {
					throw new Exception( __( 'Unable to copy domain association file to domain root.', 'ultimatewoo-pro' ) );
				}
			}
	...
	public function admin_notices() {
		...
		if ( ! empty( $this->secret_key ) && $this->payment_request && ! $this->apple_pay_domain_set ) {
			...
			echo '<div class="error stripe-apple-pay-message"><p>' . sprintf( __( 'Apple Pay domain verification failed. Please check the %1$slog%2$s to see the issue. (Logging must be enabled to see recorded logs)', 'ultimatewoo-pro' ), '<a href="' . admin_url( 'admin.php?page=wc-status&tab=logs' ) . '">', '</a>' ) . '</p></div>';
		}
	}
	...
	public function payment_fields() {
		...
		if ( is_add_payment_method_page() ) {
			$pay_button_text = __( 'Add Card', 'ultimatewoo-pro' );
			$total        = '';
		} else {
			$pay_button_text = '';
		}
		...
		if ( $this->description ) {
			if ( $this->testmode ) {
				...
				$this->description .= ' ' . sprintf( __( 'TEST MODE ENABLED. In test mode, you can use the card number 4242424242424242 with any CVC and a valid expiration date or check the documentation "<a href="%s" target="_blank">Testing Stripe</a>" for more card numbers.', 'ultimatewoo-pro' ), 'https://stripe.com/docs/testing' );
	...
	public function elements_form() {
		...
		<fieldset id="wc-<?php echo esc_attr( $this->id ); ?>-cc-form" class="wc-credit-card-form wc-payment-form" style="background:transparent;">
			...
			<label for="card-element">
				<?php esc_html_e( 'Credit or debit card', 'ultimatewoo-pro' ); ?>
			</label>
	...
	public function payment_scripts() {
		...
		$stripe_params = array(
			...
			'i18n_terms'           => __( 'Please accept the terms and conditions first', 'ultimatewoo-pro' ),
			'i18n_required_fields' => __( 'Please fill in required checkout fields first', 'ultimatewoo-pro' ),
		);
		...
		$stripe_params['no_prepaid_card_msg']                     = __( 'Sorry, we\'re not accepting prepaid cards at this time. Your credit card has not been charge. Please try with alternative payment method.', 'ultimatewoo-pro' );
		$stripe_params['no_bank_country_msg']                     = __( 'Please select a country for your bank.', 'ultimatewoo-pro' );
		$stripe_params['no_sepa_owner_msg']                       = __( 'Please enter your IBAN account name.', 'ultimatewoo-pro' );
		$stripe_params['no_sepa_iban_msg']                        = __( 'Please enter your IBAN account number.', 'ultimatewoo-pro' );
		...
		try {
			...
			if ( ! apply_filters( 'wc_stripe_allow_prepaid_card', true ) ) {
				...
				if ( $stripe_checkout_object && 'token' === $stripe_checkout_object->object && 'prepaid' === $stripe_checkout_object->card->funding ) {
					$error_msg = __( 'Sorry, we\'re not accepting prepaid cards at this time. Your credit card has not been charge. Please try with alternative payment method.', 'ultimatewoo-pro' );
					throw new Exception( $error_msg );
				}
			}

			if ( empty( $prepared_source->source ) ) {
				$error_msg = __( 'Payment processing failed. Please retry.', 'ultimatewoo-pro' );
				throw new Exception( $error_msg );
			}
			...
			if ( $order->get_total() > 0 ) {
				...
				if ( ! empty( $response->error ) ) {
					...
					if ( preg_match( '/No such customer/i', $response->error->message ) && $retry ) {
						...
						$wc_token->delete();
						$message = __( 'This card is no longer available and has been removed.', 'ultimatewoo-pro' );
						$order->add_order_note( $message );
						throw new Exception( $message );
					}
