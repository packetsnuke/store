...
class WC_Shipping_UPS extends WC_Shipping_Method {
	...
	public function __construct() {
		...
		$this->method_description = __( 'The <strong>UPS</strong> extension obtains rates dynamically from the UPS API during cart/checkout.', 'ultimatewoo-pro' );
		$this->init();
	}
	...
	public function init_form_fields() {
		...
		$this->form_fields  = array(
			...
			'api'           => array(
				...
				'type'            => 'title',
				'description'     => sprintf( __( 'You need to obtain UPS account credentials by registering on %svia their website%s.', 'ultimatewoo-pro' ), '<a href="https://www.ups.com/upsdeveloperkit">', '</a>' ),
			...
			'services_packaging'  => array(
				...
				'type'            => 'title',
				'description'     => __( 'Please enable all of the different services you\'d like to offer customers.', 'ultimatewoo-pro' ) . ' <em>' . __( 'By enabling a service, it doesn\'t gaurantee that it will be offered, as the plugin will only offer the available rates based on the package, the origin and the destination.', 'ultimatewoo-pro' ) . '</em>',
				'class'			  => 'ups-section-title',
			),
			'services'  => array(
				'type'            => 'services'
			...
			'negotiated'  => array(
				...
				'type'            => 'checkbox',
				'default'         => 'no',
				'description'     => sprintf( __( 'Enable this %sonly%s if this shipping account has %snegotiated rates%s available.', 'ultimatewoo-pro' ), '<strong>', '</strong>', '<a href="http://www.ups.com/content/au/en/resources/techsupport/worldship/negotiated_rates.html">', '</a>' ),
			),
			'signature'  => array(
				...
				'options'		  => array(
					...
					'regular'	=> sprintf( __( 'Signature Required: $%s per package', 'ultimatewoo-pro' ), self::SIGNATURE_FEE ),
					'adult'		=> sprintf( __( 'Adult Signature Required: $%s per package', 'ultimatewoo-pro' ), self::ADULT_SIGNATURE_FEE ),
				),
				'default'         => 'none',
	...
	public function calculate_shipping( $package = array() ) {
		...
		if ( ! $this->origin_postcode ) {
			$this->debug( sprintf( __( 'UPS: No Origin Postcode has been set. Please %sadd one%s so rates can be calculated!', 'ultimatewoo-pro' ), '<a href="' . admin_url( 'admin.php?page=wc-settings&tab=shipping&section=wc_shipping_ups' ) . '">', '</a>' ), 'error' );
			return;
		}
		...
		if ( $package_requests ) {
			...
			foreach ( $ups_responses as $code => $response ) {
				...
				if ( $xml->Response->ResponseStatusCode == 1 ) {
					...
					$this->debug( sprintf( __( '[UPS] No rate returned for service code %1$s, %2$s (UPS code: %3$s)', 'ultimatewoo-pro' ),
							$code,
