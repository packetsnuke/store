...
class WC_Shipping_UPS extends WC_Shipping_Method {
	...
	public function __construct( $instance_id = 0 ) {
		...
		$this->method_title       = __( 'UPS', 'ultimatewoo-pro' );
		$this->method_description = __( 'The UPS extension obtains rates dynamically from the UPS API during cart/checkout.', 'ultimatewoo-pro' );
		$this->supports           = array(
			'shipping-zones',
			'instance-settings',
			'settings',
	...
	private function environment_check() {
		...
		if ( ( ! $this->user_id || ! $this->password || ! $this->access_key || ! $this->shipper_number ) ) {
			$error_message .= '<p>' . __( 'UPS is enabled, but you have not entered all of your UPS details!', 'ultimatewoo-pro' ) . '</p>';
		}
		...
		if ( 0 < $this->instance_id ) {
			...
			if ( 'box_packing' === $this->packing_method ) {
				if ( empty( $this->ups_packaging )  && empty( $this->boxes ) ) {
					$error_message .= '<p>' . __( 'UPS is enabled, and Parcel Packing Method is set to \'Pack into boxes\', but no UPS Packaging is selected and there are no custom boxes defined. Items will be packed individually.', 'ultimatewoo-pro' ) . '</p>';
				}
			}
			...
			if ( 0 == $ctr ) {
				$error_message .= '<p>' . __( 'UPS is enabled, but there are no services enabled.', 'ultimatewoo-pro' ) . '</p>';
			}
		}
	...
	public function generate_single_select_country_html() {
		...
		<tr valign="top">
			<th scope="row" class="titledesc">
				<label for="origin_country"><?php _e( 'Origin Country', 'ultimatewoo-pro' ); ?></label>
			</th>
	...
	public function generate_services_html() {
		...
		<tr valign="top" id="service_options">
			<th scope="row" class="titledesc"><?php _e( 'Services', 'ultimatewoo-pro' ); ?></th>
			<td class="forminp">
				<table class="ups_services widefat">
					<thead>
						<th class="sort">&nbsp;</th>
						<th><?php _e( 'Service Code', 'ultimatewoo-pro' ); ?></th>
						<th><?php _e( 'Name', 'ultimatewoo-pro' ); ?></th>
						<th><?php _e( 'Enabled', 'ultimatewoo-pro' ); ?></th>
						<th><?php echo sprintf( __( 'Price Adjustment (%s)', 'ultimatewoo-pro' ), get_woocommerce_currency_symbol() ); ?></th>
						<th><?php _e( 'Price Adjustment (%)', 'ultimatewoo-pro' ); ?></th>
					</thead>
					<tfoot>
					<?php if ( 'PL' !== $this->origin_country && ! in_array( $this->origin_country, $this->eu_array ) ) : ?>
						<tr>
							<th colspan="6">
								<small class="description"><?php _e( '<strong>Domestic Rates</strong>: Next Day Air, 2nd Day Air, Ground, 3 Day Select, Next Day Air Saver, Next Day Air Early AM, 2nd Day Air AM', 'ultimatewoo-pro' ); ?></small><br/>
								<small class="description"><?php _e( '<strong>International Rates</strong>: Worldwide Express, Worldwide Expedited, Standard, Worldwide Express Plus, UPS Saver', 'ultimatewoo-pro' ); ?></small>
							</th>
						</tr>
					<?php endif ?>
					</tfoot>
	...
	public function generate_box_packing_html() {
		...
		<tr valign="top" id="packing_options">
			<th scope="row" class="titledesc"><?php _e( 'Custom Boxes', 'ultimatewoo-pro' ); ?></th>
			<td class="forminp">
				<table class="ups_boxes widefat">
					<thead>
						<tr>
							<th class="check-column"><input type="checkbox" /></th>
							<th><?php _e( 'Outer Length', 'ultimatewoo-pro' ); ?></th>
							<th><?php _e( 'Outer Width', 'ultimatewoo-pro' ); ?></th>
							<th><?php _e( 'Outer Height', 'ultimatewoo-pro' ); ?></th>
							<th><?php _e( 'Inner Length', 'ultimatewoo-pro' ); ?></th>
							<th><?php _e( 'Inner Width', 'ultimatewoo-pro' ); ?></th>
							<th><?php _e( 'Inner Height', 'ultimatewoo-pro' ); ?></th>
							<th><?php _e( 'Weight of Box', 'ultimatewoo-pro' ); ?></th>
							<th><?php _e( 'Max Weight', 'ultimatewoo-pro' ); ?></th>
						</tr>
					</thead>
					<tfoot>
						<tr>
							<th colspan="3">
								<a href="#" class="button plus insert"><?php _e( 'Add Box', 'ultimatewoo-pro' ); ?></a>
								<a href="#" class="button minus remove"><?php _e( 'Remove selected box(es)', 'ultimatewoo-pro' ); ?></a>
							</th>
							<th colspan="6">
								<small class="description"><?php _e( 'Items will be packed into these boxes depending based on item dimensions and volume. Outer dimensions will be passed to UPS, whereas inner dimensions will be used for packing. Items not fitting into boxes will be packed individually.', 'ultimatewoo-pro' ); ?></small>
							</th>
						</tr>
					</tfoot>
					<tbody id="rates">
	...
	public function init_form_fields() {
		$this->instance_form_fields = array(
			'core' => array(
				'title'       => __( 'Method & Origin Settings', 'ultimatewoo-pro' ),
				'type'        => 'title',
				'description' => '',
			...
			'title' => array(
				...
				'default'     => __( 'UPS', 'ultimatewoo-pro' ),
				'desc_tip'    => true,
			),
			'origin_city' => array(
				'title'       => __( 'Origin City', 'ultimatewoo-pro' ),
				'type'        => 'text',
				'description' => __( 'Enter the city for the <strong>sender</strong>.', 'ultimatewoo-pro' ),
				'default'     => '',
				'desc_tip'    => true,
			),
			'origin_postcode' => array(
				...
				'type'        => 'text',
				'description' => __( 'Enter the zip/postcode for the <strong>sender</strong>.', 'ultimatewoo-pro' ),
				'default'     => '',
				'desc_tip'    => true,
			),
			...
			'services_packaging' => array(
				'title'       => __( 'Services and Packaging', 'ultimatewoo-pro' ),
				'type'        => 'title',
				'description' => __( 'Please enable all of the different services you\'d like to offer customers.', 'ultimatewoo-pro' ) . ' <em>' . __( 'By enabling a service, it doesn\'t guarantee that it will be offered, as the plugin will only offer the available rates based on the package, the origin and the destination.', 'ultimatewoo-pro' ) . '</em>',
				'class'       => 'ups-section-title',
			),
			'services' => array(
				'type' => 'services',
			...
			'offer_rates' => array(
				...
				'options'     => array(
				    ...
				    'cheapest' => __( 'Offer the customer the cheapest rate only', 'ultimatewoo-pro' ),
				),
			),
			'negotiated' => array(
				'title'       => __( 'Negotiated Rates', 'ultimatewoo-pro' ),
				'label'       => __( 'Enable negotiated rates', 'ultimatewoo-pro' ),
				'type'        => 'checkbox',
				'default'     => 'no',
				'description' => sprintf(
					__( 'Enable this %1$sonly%2$s if this shipping account has %3$snegotiated rates%4$s available.', 'ultimatewoo-pro' ),
					'<strong>', '</strong>',
					'<a href="https://www.ups.com/au/en/help-center/technology-support/worldship/negotiated-rates.page">', '</a>'
				),
			),
			'signature' => array(
				'title'       => __( 'Delivery Confirmation', 'ultimatewoo-pro' ),
				'type'        => 'select',
				'default'     => 'none',
				'description' => __( 'Optionally you may charge customers for signature on delivery. This will just add the specified amount above to the returned rates.', 'ultimatewoo-pro' ),
				'desc_tip'    => true,
				'options'     => array(
					'none'    => __( 'No Signature Required', 'ultimatewoo-pro' ),
					'regular' => __( 'Signature Required', 'ultimatewoo-pro' ),
					'adult'   => __( 'Adult Signature Required', 'ultimatewoo-pro' ),
				),
			),
			...
			'ups_packaging'  => array(
				'title'       => __( 'UPS Packaging', 'ultimatewoo-pro' ),
				'type'        => 'multiselect',
				'description' => __( 'Select UPS standard packaging options to enable', 'ultimatewoo-pro' ),
				'default'     => array(),
				'css'         => 'width: 450px;',
			...
			'advanced_title'  => array(
				...
				'type'        => 'title',
				'description' => __( 'Only modify the following options if needed. They will most likely alter the regularly offered rate(s).', 'ultimatewoo-pro' ),
				'class'       => 'ups-section-title',
			),
			'origin_addressline'  => array(
				'title'       => __( 'Origin Address', 'ultimatewoo-pro' ),
				'type'        => 'text',
				'description' => __( 'Sometimes you may need to enter the address for the <strong>sender / origin</strong>.', 'ultimatewoo-pro' ),
				'default'     => '',
				'desc_tip'    => true,
			),
			'residential'  => array(
				'title'       => __( 'Residential', 'ultimatewoo-pro' ),
				'label'       => __( 'Enable residential address flag', 'ultimatewoo-pro' ),
				'type'        => 'checkbox',
				'default'     => 'yes',
				'description' => __( 'Enable this to indicate to UPS that the receiver / customer is a residential address.', 'ultimatewoo-pro' ),
				'desc_tip'    => true,
			),
			'insuredvalue'  => array(
				'title'       => __( 'Insured Value', 'ultimatewoo-pro' ),
				'label'       => __( 'Request Insurance to be included in UPS rates', 'ultimatewoo-pro' ),
				'type'        => 'checkbox',
				'default'     => 'no',
				'description' => __( 'Enable insured value option to include insurance in UPS rates', 'ultimatewoo-pro' ),
				'desc_tip'    => true,
			),
			'pickup'  => array(
				'title'       => __( 'Pickup Type', 'ultimatewoo-pro' ),
				'type'        => 'select',
				'css'         => 'width: 250px;',
				'description' => __( 'This will adjust the rate you get, so only change it if needed.', 'ultimatewoo-pro' ),
				'class'       => 'chosen_select',
				'default'     => '03',
			...
			'fallback' => array(
				'title'       => __( 'Fallback', 'ultimatewoo-pro' ),
				'type'        => 'price',
				'description' => __( 'If UPS returns no matching rates, offer this amount for shipping so that the user can still checkout. Leave blank to disable. Enter a numeric value with no currency symbols.', 'ultimatewoo-pro' ),
				'default'     => '',
				'desc_tip'    => true,
			),
			'units' => array(
				'title'       => __( 'Weight/Dimension Units', 'ultimatewoo-pro' ),
				'type'        => 'select',
				'description' => __( 'If you see "This measurement system is not valid for the selected country" errors, switch this to metric units.', 'ultimatewoo-pro' ),
				'desc_tip'    => true,
				'default'     => 'imperial',
				'options'     => array(
					'imperial' => __( 'LB / IN', 'ultimatewoo-pro' ),
					'metric'   => __( 'KG / CM', 'ultimatewoo-pro' ),
				),
			),
		);
		...
		$this->form_fields = array(
			'api' => array(
				...
				'type'        => 'title',
				'description' => sprintf( __( 'You need to obtain UPS account credentials by registering on %1$svia their website%2$s.', 'ultimatewoo-pro' ), '<a href="https://www.ups.com/upsdeveloperkit">', '</a>' ),
				'class'       => 'ups-section-title ups-api-title',
			),
			'user_id' => array(
				'title'       => __( 'UPS User ID', 'ultimatewoo-pro' ),
				'type'        => 'text',
				'description' => __( 'Obtained from UPS after getting an account.', 'ultimatewoo-pro' ),
				'default'     => '',
				'class'       => 'ups-api-setting',
				'desc_tip'    => true,
			),
			'password' => array(
				'title'       => __( 'UPS Password', 'ultimatewoo-pro' ),
				'type'        => 'password',
				'description' => __( 'Obtained from UPS after getting an account.', 'ultimatewoo-pro' ),
				'default'     => '',
				'class'       => 'ups-api-setting',
			...
			'access_key' => array(
				'title'       => __( 'UPS Access Key', 'ultimatewoo-pro' ),
				'type'        => 'text',
				'description' => __( 'Obtained from UPS after getting an account.', 'ultimatewoo-pro' ),
				'default'     => '',
				'class'       => 'ups-api-setting',
			...
			'shipper_number' => array(
				'title'       => __( 'UPS Account Number', 'ultimatewoo-pro' ),
				'type'        => 'text',
				'description' => __( 'Obtained from UPS after getting an account.', 'ultimatewoo-pro' ),
				'default'     => '',
				'class'       => 'ups-api-setting',
			...
			'customer_classification_code' => array(
				'title'       => __( 'Customer Classification', 'ultimatewoo-pro' ),
				'type'        => 'select',
				'css'         => 'width: 250px;',
				'description' => __( 'This option only valid if origin country is US.', 'ultimatewoo-pro' ),
				'desc_tip'    => true,
				'class'       => 'chosen_select',
				'default'     => '03',
				'options'     => array(
					''   => __( 'Rate chart from shipper\'s country.', 'ultimatewoo-pro' ),
					'00' => __( 'Rates associated with shipper number', 'ultimatewoo-pro' ),
					'01' => __( 'Daily rates', 'ultimatewoo-pro' ),
					'04' => __( 'Retail rates', 'ultimatewoo-pro' ),
					'05' => __( 'Regional rates', 'ultimatewoo-pro' ),
					'06' => __( 'General list rates', 'ultimatewoo-pro' ),
					'53' => __( 'Standard list rates', 'ultimatewoo-pro' ),
				),
			),
			'debug'  => array(
				...
				'type'        => 'checkbox',
				'default'     => 'no',
				'description' => __( 'Enable debug mode to show debugging information on your cart/checkout.', 'ultimatewoo-pro' ),
				'desc_tip'    => true,
			),
		);
	}
	...
	public function calculate_shipping( $package = array() ) {
		...
		if ( '' === $package['destination']['country'] ) {
			$this->debug( __( 'UPS: Country not supplied. Rates not requested.', 'ultimatewoo-pro' ) );
			return;
		}

		// If no origin postcode set, throw an error and stop the calculation.
		if ( ! $this->origin_postcode ) {
			$this->debug( sprintf( __( 'UPS: No Origin Postcode has been set. Please %1$sadd one%2$s so rates can be calculated!', 'ultimatewoo-pro' ), '<a href="' . admin_url( 'admin.php?page=wc-settings&tab=shipping&section=wc_shipping_ups' ) . '">', '</a>' ), 'error' );
			return;
		}
		...
		if ( $package_requests ) {
			...
			if ( ! $rate_requests ) {
				$this->debug( __( 'UPS: No Services are enabled in admin panel.', 'ultimatewoo-pro' ) );
			}
			...
			foreach ( $rate_requests as $code => $request ) {
				...
				if ( false === $cached_response ) {
					...
					$this->debug( __( 'UPS: Using cached response.', 'ultimatewoo-pro' ) );
				}
			...
			foreach ( $ups_responses as $code => $response ) {
				...
				if ( $this->debug && ! $xml ) {
					$this->debug( __( 'Failed loading XML', 'ultimatewoo-pro' ), 'error' );
				}
				...
				if ( 1 == $xml->Response->ResponseStatusCode ) {
					...
					if ( get_woocommerce_currency() !== $currency ) {
						...
						$this->debug( sprintf( __( '[UPS] Rate for %1$s is in %2$s but store currency is %3$s.', 'ultimatewoo-pro' ), $rate_name, $currency, get_woocommerce_currency() ) );
						continue;
					}
					...
					$this->debug(
						sprintf(
							__( '[UPS] No rate returned for service code %1$s, %2$s (UPS code: %3$s)', 'ultimatewoo-pro' ),
							$code,
		...
		if ( $rates ) {
			...
			$this->debug( __( 'UPS: Using Fallback setting.', 'ultimatewoo-pro' ) );
		}
	}
	...
	private function per_item_shipping( $package ) {
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			if ( ! $values['data']->needs_shipping() ) {
				$this->debug( sprintf( __( 'Product #%d is virtual. Skipping.', 'ultimatewoo-pro' ), $ctr ) );
				continue;
			}

			if ( ! $values['data']->get_weight() ) {
				$this->debug( sprintf( __( 'Product #%d is missing weight. Aborting.', 'ultimatewoo-pro' ), $ctr ), 'error' );
				return;
			}
	...
	private function box_shipping( $package ) {
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			if ( $values['data']->get_length() && $values['data']->get_height() && $values['data']->get_width() && $values['data']->get_weight() ) {
				...
				$this->debug( sprintf( __( 'UPS Parcel Packing Method is set to Pack into Boxes. Product #%d is missing dimensions. Aborting.', 'ultimatewoo-pro' ), $ctr ), 'error' );
				return;
			}
		}
