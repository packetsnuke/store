...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.6.0', __( 'WooCommerce Sequential Order Numbers Pro', 'ultimatewoo-pro' ), __FILE__, 'init_woocommerce_sequential_order_numbers_pro', array(
...
class WC_Seq_Order_Number_Pro extends SV_WC_Plugin {
	...
	public function get_order_number( $order_number, $order ) {
		...
		if ( 'auto-draft' === $post_status ) {
			...
			return $this->format_order_number( $order_number, $this->get_order_number_prefix(), $this->get_order_number_suffix(), $this->get_order_number_length() ) . ' (' . __( 'Draft', 'ultimatewoo-pro' ) . ')';
		}
	...
	public function add_settings_errors() {
		...
		if ( $this->is_order_number_start_invalid( $newvalue ) ) {
			...
			// bad value
			WC_Admin_Settings::add_error( __( 'Order Number Start must be a number greater than or equal to 0.', 'ultimatewoo-pro' ) );
			return;
		}
		...
		if ( $this->is_order_number_start_in_use( $newvalue ) ) {
			...
			WC_Admin_Settings::add_error( sprintf(
				...
				__( 'There is an existing order (%1$s) with a number greater than or equal to %2$s. To set a new order number start please choose a higher number or permanently delete the relevant order(s).', 'ultimatewoo-pro' ), $highest_order_number, (int) $newvalue
			) );
		}
	}
	...
	public function admin_settings( $settings ) {
		...
		foreach ( $settings as $section ) {
			...
			if ( isset( $section['id'] ) && 'general_options' === $section['id']
				...
				$updated_settings[] = array( 'name' => __( 'Order Numbers', 'ultimatewoo-pro' ), 'type' => 'title', 'desc' => '', 'id' => 'order_number_options' );

				$updated_settings[] = array(
					'name'     => __( 'Order Number Start', 'ultimatewoo-pro' ),
					'desc_tip' => sprintf(
						/* translators: Placeholders: %s - sample order number */
						__( 'The starting number for the incrementing portion of the order numbers, unless there is an existing order with a higher number.  Use leading zeroes to pad your order numbers to a desired minimum length.  Any newly placed orders will be numbered like: %s', 'ultimatewoo-pro' ), $this->format_order_number( get_option( 'woocommerce_order_number_start' ), $this->get_order_number_prefix(), $this->get_order_number_suffix(), $this->get_order_number_length() )
					),
					...
					'desc'     => '<br />' . sprintf( __( 'Sample order number: %s', 'ultimatewoo-pro' ), '<span id="sample_order_number">' . $this->format_order_number( get_option( 'woocommerce_order_number_start' ), $this->get_order_number_prefix(), $this->get_order_number_suffix(), $this->get_order_number_length() ) . '</span>' ),
				);

				$updated_settings[] = array(
					'name'     => __( 'Order Number Prefix', 'ultimatewoo-pro' ),
					'desc_tip' => __( 'Set your custom order number prefix.  You may use {DD}, {MM}, {YYYY} for the current day, month and year respectively.', 'ultimatewoo-pro' ),
					'id'       => 'woocommerce_order_number_prefix',
					'type'     => 'text',
					'css'      => 'min-width:300px;',
					'default'  => '',
					/* translators: Placeholders: %1$s - opening <a> link tag, %2$s - closing </a> link tag */
					'desc'     => '<br />' . sprintf( __( 'See the %1$splugin documentation%2$s for the full set of available patterns.', 'ultimatewoo-pro' ), '<a target="_blank" href="http://docs.woocommerce.com/document/sequential-order-numbers/">', '</a>' ),
				);

				$updated_settings[] = array(
					'name'     => __( 'Order Number Suffix', 'ultimatewoo-pro' ),
					'desc_tip' => __( 'Set your custom order number suffix. You may use {DD}, {MM}, {YYYY} for the current day, month and year respectively.', 'ultimatewoo-pro' ),
				...
				$updated_settings[] = array(
					'name'     => __( 'Skip Free Orders', 'ultimatewoo-pro' ),
					'desc'     => __( 'Skip order numbers for free orders', 'ultimatewoo-pro' ),
					'desc_tip' => __( 'With this enabled an order number will not be assigned to an order consisting solely of free products.', 'ultimatewoo-pro' ),
				...
				$updated_settings[] = array(
					'name'     => __( 'Free Order Identifer', 'ultimatewoo-pro' ),
					/* translators: Placeholders: %s - sample order number */
					'desc'     => sprintf( __( 'Example free order identifier: %s', 'ultimatewoo-pro' ), '<span id="sample_free_order_number">' . $this->format_order_number( $this->get_free_order_number_start(), $this->get_free_order_number_prefix() ) . '</span>' ),
					'desc_tip' => __( 'The text to display in place of the order number for free orders. This will be displayed anywhere an order number would otherwise be shown: to the customer, in emails, and in the admin.', 'ultimatewoo-pro' ),
					'id'       => 'woocommerce_free_order_number_prefix',
					'type'     => 'text',
					'css'      => 'min-width:300px;',
					/* translators: FREE- as in free purchase order */
					'default'  => __( 'FREE-', 'ultimatewoo-pro' ),
				);
	...
	private function get_free_order_number_prefix() {
		/* translators: FREE- as in free purchase order */
		return get_option( 'woocommerce_free_order_number_prefix', __( 'FREE-', 'ultimatewoo-pro' ) );
	}
	...
	public function get_plugin_name() {
		return __( 'WooCommerce Sequential Order Numbers Pro', 'ultimatewoo-pro' );
	}
		...
		if ( false === get_option( 'woocommerce_order_number_start' ) ) {
			...
			do {
				...
				if ( is_wp_error( $order_ids ) ) {
					...
					wp_die( sprintf( __( 'Error activating and installing %1$s: %2$s', 'ultimatewoo-pro' ),
							'<strong>' . $this->get_plugin_name() . '</strong>',
							'<ul><li>' . implode( '</li><li>', $order_ids->get_error_messages() ) . '</li></ul>' )
					        . '<a href="' . admin_url( 'plugins.php' ) . '">' . esc_html__( '&laquo; Go Back', 'ultimatewoo-pro' ) . '</a>' );
				}
