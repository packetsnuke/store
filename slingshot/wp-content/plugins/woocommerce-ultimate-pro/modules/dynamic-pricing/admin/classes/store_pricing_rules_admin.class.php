            ...
            <h2 class="nav-tab-wrapper woo-nav-tab-wrapper">
				...
				$tabs = apply_filters( 'woocommerce_dynamic_pricing_tabs', array(
					'order totals' => array(
						'tabs' => array(
							array(
								'title'       => __( 'Order Totals Pricing', 'ultimatewoo-pro' ),
								'description' => __( 'Order Totals pricing allows you to configure price adjustments for the entire store based on the total order amount.  This is the last discount rule that will be applied.  If other discounts have been applied to cart items, these rules will not be applied to the cart.', 'ultimatewoo-pro' ),
								'function'    => 'totals_tab'
							)
						)
					),
					'roles'        => array(
						'tabs' => array(
							array(
								'title'       => __( 'Role Pricing', 'ultimatewoo-pro' ),
								'description' => __( 'Role pricing allows you to configure price adjustments for the entire store based on a users role.', 'ultimatewoo-pro' ),
					...
					'category'     => array(
						'tabs' => array(
							array(
								'title'       => __( 'Category Pricing', 'ultimatewoo-pro' ),
								'description' => __( 'Use bulk category pricing to configure bulk price adjustments based on a product\'s category. Category pricing rules will apply before Membership (role-based pricing discounts), and will be cumulative with any Membership rules by default.   The cumulative filter can be used to change this behavior.', 'ultimatewoo-pro' ),
								'function'    => 'basic_category_tab'
							),
							array(
								'title'       => __( 'Advanced Category Pricing', 'ultimatewoo-pro' ),
								'description' => __( 'Use advanced category pricing to configure price adjustments on items in a customers cart based on quantities.  Adjustments are calculated when the rule matches the configured quantities and will be applied to all items in the cart matching the selected category / categories.   Advanced category adjustments take precedence over bulk category adjustments.', 'ultimatewoo-pro' ),
				...
				if ( wc_dynamic_pricing_is_groups_active() ) {
					$tabs['groups'] = array(
						'tabs' => array(
							array(
								'title'       => __( 'Group Pricing', 'ultimatewoo-pro' ),
				...
				if ( wc_dynamic_pricing_is_brands_active() ) {
					$tabs['brands'] = array(
						'tabs' => array(
							array(
								'title'       => __( 'Brand Pricing', 'ultimatewoo-pro' ),
								'description' => __( 'Use bulk brand pricing to configure bulk price adjustments based on a product\'s brand. Brand pricing rules will apply before Membership (role-based pricing discounts), and will be cumulative with any Membership rules by default.   The cumulative filter can be used to change this behavior.', 'ultimatewoo-pro' ),
								'function'    => 'basic_brand_tab'
							),
							array(
								'title'       => __( 'Advanced Brand Pricing', 'ultimatewoo-pro' ),
					     ...
					     Advanced category adjustments take precedence over bulk brand adjustments.', 'ultimatewoo-pro' ),
				...
				if ( $rule_set['rules_type'] == 'block' ) {
					...
					foreach ( $rule_set['rules'] as $rule ) {
						if ( isset( $rule['adjust'] ) && !empty( $rule['adjust'] ) && isset( $rule['from'] ) && isset( $rule['amount'] ) && !empty( $rule['from'] ) && !empty( $rule['amount'] ) ) {
							...
							$valid   = $valid & false;
							$message .= __( 'You must enter values for all quantity and amount fields', 'ultimatewoo-pro' );
						}
					}
				...
				foreach ( $rule_set['rules'] as $rule ) {
					if ( isset( $rule['to'] ) && !empty( $rule['to'] ) && isset( $rule['from'] ) && isset( $rule['amount'] ) && ( !empty( $rule['from'] ) || $rule['from'] === '0' ) && !empty( $rule['amount'] ) ) {
						...
						if ( $rule['from'] != '*' && $rule['to'] != '*' && intval( $rule['to'] ) < intval( $rule['from'] ) ) {
							$valid   = $valid & false;
							$message .= __( 'Invalid totals configured', 'ultimatewoo-pro' );
						} else {
							$valid = $valid & true;
						}
					} else {
