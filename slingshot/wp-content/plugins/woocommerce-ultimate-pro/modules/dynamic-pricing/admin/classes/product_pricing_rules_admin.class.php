...
class woocommerce_product_pricing_rules_admin {
	...
	public function on_product_write_panel_tabs() {
		if ( WC_Dynamic_Pricing_Compatibility::is_wc_version_gte_2_3() ) :
            ...
            <li class="pricing_tab dynamic_pricing_options dynamic_pricing_options_23">
                <a href="#dynamic_pricing_data"><span><?php _e( 'Dynamic Pricing', 'ultimatewoo-pro' ); ?></span></a></li>
		...
		<?php elseif ( WC_Dynamic_Pricing_Compatibility::is_wc_version_gte_2_1() ) : ?>
            <li class="pricing_tab dynamic_pricing_options dynamic_pricing_options_21">
                <a href="#dynamic_pricing_data"><?php _e( 'Dynamic Pricing', 'ultimatewoo-pro' ); ?></a></li>
		<?php else : ?>
	...
	public function product_data_panel() {
        ...
        <div id="dynamic_pricing_data" class="panel woocommerce_options_panel">
            ...
            <button title="<?php _e( 'Allows you to configure another Price Adjustment.  Useful if you have different sets of conditions and pricing adjustments which need to be applied to this product.', 'ultimatewoo-pro' ); ?>" id="woocommerce-pricing-add-ruleset" type="button" class="button button-primary"><?php _e( 'Add Pricing Group', 'ultimatewoo-pro' ); ?></button>
            <div class="clear"></div>
        </div>
		<?php
	}
		...
		foreach ( $pricing_rule_sets as $name => $pricing_rule_set ) {
            ...
            <div id="woocommerce-pricing-ruleset-<?php echo $name; ?>" class="woocommerce_pricing_ruleset">
                <div id="woocommerce-pricing-conditions-<?php echo $name; ?>" class="section    ">
                    <h4 class="first">Pricing Group<a href="#" data-name="<?php echo $name; ?>" class="delete_pricing_ruleset"><img src="<?php echo WC_Dynamic_Pricing::plugin_url(); ?>/assets/images/delete.png" title="<?php _e( 'Delete this Price Adjustment', 'ultimatewoo-pro' ); ?>" alt="<?php _e( 'Delete this Price Adjustment', 'ultimatewoo-pro' ); ?>" style="cursor:pointer; margin:0 3px;float:right;"/></a>
                    </h4>
					<?php
					$condition_index = 0;
                ...
                <div id="woocommerce-pricing-mode-<?php echo $name; ?>" class="section">
                    <label for="pricing_ruleset_mode_value_<?php echo $name . '_0'; ?>"><?php _e( 'Rule Processing Mode', 'ultimatewoo-pro' ); ?></label>
                    <select id="pricing_ruleset_mode_value_<?php echo $name . '_0'; ?>" name="pricing_rules[<?php echo $name; ?>][mode]" class="pricing_rule_mode">
                        <option <?php selected( 'continuous', $mode ); ?> value="continuous"><?php _e( 'Bulk', 'ultimatewoo-pro' ); ?></option>
                        <option <?php selected( 'block', $mode ); ?> value="block"><?php _e( 'Special Offer', 'ultimatewoo-pro' ); ?></option>
                    </select>
                </div>

                <div id="woocommerce-pricing-dates-<?php echo $name; ?>" class="section pricing-rule-date-fields">
                    <label for="pricing_ruleset_dates_value_<?php echo $name . '_date_from'; ?>"><?php _e( 'Dates', 'ultimatewoo-pro' ); ?></label>
                    <input value="<?php echo $date_from; ?>" type="text" class="short date_from" title="<?php _e( 'Leave both fields blank to not restrict this pricing group to a date range', 'ultimatewoo-pro' ); ?>" name="pricing_rules[<?php echo $name; ?>][date_from]" id="pricing_ruleset_dates_value_<?php echo $name . '_date_from'; ?>" value="" placeholder="<?php echo _x( 'From&hellip;', 'placeholder', 'woocommerce' ) ?> YYYY-MM-DD" maxlength="10" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])">
                    <input value="<?php echo $date_to; ?>" type="text" class="short date_to" title="<?php _e( 'Leave both fields blank to not restrict this pricing group to a date range', 'ultimatewoo-pro' ); ?>" name="pricing_rules[<?php echo $name; ?>][date_to]" id="pricing_ruleset_dates_value_<?php echo $name . '_date_to'; ?>" value="" placeholder="<?php echo _x( 'To&hellip;', 'placeholder', 'woocommerce' ); ?> YYYY-MM-DD" maxlength="10" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])">
                    <div class="clear"></div>
                </div>
                ...
                <div class="section" style="display:<?php echo( $mode == 'continuous' ? 'block' : 'none;' ); ?>">
                    <table id="woocommerce-pricing-rules-table-<?php echo $name; ?>" class="continuous" data-lastindex="<?php echo ( is_array( $pricing_rules ) && sizeof( $pricing_rules ) > 0 ) ? count( $pricing_rules ) : '1'; ?>">
                        <thead>
                        <th>
							<?php _e( 'Minimum Quantity', 'ultimatewoo-pro' ); ?>
                        </th>
                        <th>
							<?php _e( 'Max Quantity', 'ultimatewoo-pro' ); ?>
                        </th>
                        <th>
							<?php _e( 'Type', 'ultimatewoo-pro' ); ?>
                        </th>
                        <th>
							<?php _e( 'Amount', 'ultimatewoo-pro' ); ?>
                        </th>
                        <th>&nbsp;</th>
                        </thead>
                        <tbody>
                ...
                <div class="section" style="display:<?php echo( $mode == 'block' ? 'block' : 'none;' ); ?>">
                    <table id="woocommerce-pricing-blockrules-table-<?php echo $name; ?>" class="block" data-lastindex="<?php echo ( is_array( $pricing_rules ) && sizeof( $pricing_rules ) > 0 ) ? count( $pricing_rules ) : '1'; ?>">
                        <thead>
                        <th>
							<?php _e( 'Purchase', 'ultimatewoo-pro' ); ?>
                        </th>
                        <th>
							<?php _e( 'Receive', 'ultimatewoo-pro' ); ?>
                        </th>
                        <th>
							<?php _e( 'Type', 'ultimatewoo-pro' ); ?>
                        </th>
                        ...
                        <th>
							<?php _e( 'Repeating', 'ultimatewoo-pro' ); ?>
                        </th>
                        <th>&nbsp;</th>
                        </thead>
                        <tbody>
	...
	private function create_condition_apply_to( $condition, $name, $condition_index ) {
        ...
        <div>
            <label for="pricing_rule_apply_to_<?php echo $name . '_' . $condition_index; ?>"><?php _e( 'Applies To:', 'ultimatewoo-pro' ); ?></label>
            <input type="hidden" name="pricing_rules[<?php echo $name; ?>][conditions][<?php echo $condition_index; ?>][type]" value="apply_to"/>

            <select title="<?php _e( 'Choose if this rule should apply to everyone, or to specific roles.  Useful if you only give discounts to existing customers, or if you have tiered pricing based on the users role.', 'ultimatewoo-pro' ); ?>" class="pricing_rule_apply_to" id="pricing_rule_apply_to_<?php echo $name . '_' . $condition_index; ?>" name="pricing_rules[<?php echo $name; ?>][conditions][<?php echo $condition_index; ?>][args][applies_to]">
                <option <?php selected( 'everyone', $condition['args']['applies_to'] ); ?> value="everyone"><?php _e( 'Everyone', 'ultimatewoo-pro' ); ?></option>
                <option <?php selected( 'unauthenticated', $condition['args']['applies_to'] ); ?> value="unauthenticated"><?php _e( 'Guests', 'ultimatewoo-pro' ); ?></option>
                <option <?php selected( 'roles', $condition['args']['applies_to'] ); ?> value="roles"><?php _e( 'Specific Roles', 'ultimatewoo-pro' ); ?></option>
	...
	private function create_variation_selector( $condition, $name ) {
        ...
        <div>
            <label for="pricing_rule_variations_<?php echo $name; ?>"><?php _e( 'Product / Variations:', 'ultimatewoo-pro' ); ?></label>
            <select title="<?php _e( 'Choose what you would like to apply this pricing rule set to', 'ultimatewoo-pro' ); ?>" class="pricing_rule_variations" id="pricing_rule_variations_<?php echo $name; ?>" name="pricing_rules[<?php echo $name; ?>][variation_rules][args][type]">
                <option <?php selected( 'product', $condition['args']['type'] ); ?> value="product"><?php _e( 'All Variations', 'ultimatewoo-pro' ); ?></option>
                <option <?php selected( 'variations', $condition['args']['type'] ); ?> value="variations"><?php _e( 'Specific Variations', 'ultimatewoo-pro' ); ?></option>
            </select>
						...
						<?php foreach ( $chunk as $variation_id ) : ?>
                            ...
                            <li>
                                <label for="variation_<?php echo $variation_id; ?>_<?php echo $name; ?>" class="selectit">
                                    <input <?php echo $variation_checked; ?> type="checkbox" id="variation_<?php echo $variation_id; ?>_<?php echo $name; ?>" name="pricing_rules[<?php echo $name; ?>][variation_rules][args][variations][]" value="<?php echo $variation_id; ?>"/><?php _e( 'Variation ID:', 'ultimatewoo-pro' ); ?> <?php echo $variation_id; ?>
                                </label>
                            </li>
						<?php endforeach; ?>
                    </ul>
	...
	private function create_collector( $collector, $name ) {
		...
		?>
        <label for="pricing_rule_when_<?php echo $name; ?>"><?php _e( 'Quantities based on:', 'ultimatewoo-pro' ); ?></label>
        <select title="<?php _e( 'Choose how to calculate the quantity.  This tallied amount is used in determining the min and max quantities used below in the Quantity Pricing section.', 'ultimatewoo-pro' ); ?>" class="pricing_rule_when" id="pricing_rule_when_<?php echo $name; ?>" name="pricing_rules[<?php echo $name; ?>][collector][type]">
            <option title="<?php _e( 'Calculate quantity based on the Product ID', 'ultimatewoo-pro' ); ?>" <?php selected( 'product', $collector['type'] ); ?> value="product"><?php _e( 'Product Quantity', 'ultimatewoo-pro' ); ?></option>
            <option title="<?php _e( 'Calculate quantity based on the Variation ID', 'ultimatewoo-pro' ); ?>" <?php selected( 'variation', $collector['type'] ); ?> value="variation"><?php _e( 'Variation Quantity', 'ultimatewoo-pro' ); ?></option>
            <option title="<?php _e( 'Calculate quantity based on the Cart Line Item', 'ultimatewoo-pro' ); ?>" <?php selected( 'cart_item', $collector['type'] ); ?> value="cart_item"><?php _e( 'Cart Line Item Quantity', 'ultimatewoo-pro' ); ?></option>
            <option title="<?php _e( 'Calculate quantity based on total amount of a category in the cart', 'ultimatewoo-pro' ); ?>" <?php selected( 'cat', $collector['type'] ); ?> value="cat"><?php _e( 'Quantity of Category', 'ultimatewoo-pro' ); ?></option>
        </select>
        <br/>
	...
	private function get_continuous_row( $rule, $name, $index ) {
        ...
        <tr id="pricing_rule_row_<?php echo $name . '_' . $index; ?>">
            <td>
                <input title="<?php _e( 'Apply this adjustment when the quantity in the cart starts at this value.  Use * for any.', 'ultimatewoo-pro' ); ?>" class="int_pricing_rule" id="pricing_rule_from_input_<?php echo $name . '_' . $index; ?>" type="text" name="pricing_rules[<?php echo $name; ?>][rules][<?php echo $index ?>][from]" value="<?php echo $rule['from']; ?>"/>
            </td>
            <td>
                <input title="<?php _e( 'Apply this adjustment when the quantity in the cart is less than this value.  Use * for any.', 'ultimatewoo-pro' ); ?>" class="int_pricing_rule" id="pricing_rule_to_input_<?php echo $name . '_' . $index; ?>" type="text" name="pricing_rules[<?php echo $name; ?>][rules][<?php echo $index ?>][to]" value="<?php echo $rule['to']; ?>"/>
            </td>
            <td>
                <select title="<?php _e( 'The type of adjustment to apply', 'ultimatewoo-pro' ); ?>" id="pricing_rule_type_value_<?php echo $name . '_' . $index; ?>" name="pricing_rules[<?php echo $name; ?>][rules][<?php echo $index; ?>][type]">
                    <option <?php selected( 'price_discount', $rule['type'] ); ?> value="price_discount"><?php _e( 'Price Discount', 'ultimatewoo-pro' ); ?></option>
                    <option <?php selected( 'percentage_discount', $rule['type'] ); ?> value="percentage_discount"><?php _e( 'Percentage Discount', 'ultimatewoo-pro' ); ?></option>
                    <option <?php selected( 'fixed_price', $rule['type'] ); ?> value="fixed_price"><?php _e( 'Fixed Price', 'ultimatewoo-pro' ); ?></option>
                </select>
            </td>
            <td>
                <input title="<?php _e( 'The value of the adjustment. Currency and percentage symbols are not required', 'ultimatewoo-pro' ); ?>" class="float_rule_number" id="pricing_rule_amount_input_<?php echo $name . '_' . $index; ?>" type="text" name="pricing_rules[<?php echo $name; ?>][rules][<?php echo $index; ?>][amount]" value="<?php echo $rule['amount']; ?>"/>
            </td>
            <td width="48">
                <a class="add_pricing_rule" data-index="<?php echo $index; ?>" data-name="<?php echo $name; ?>"><img
                            ...
                            title="<?php _e( 'add another rule', 'ultimatewoo-pro' ); ?>" alt="<?php _e( 'add another rule', 'ultimatewoo-pro' ); ?>"
	...
	private function get_block_row( $rule, $name, $index, $row_count ) {
        ...
        <tr id="pricing_blockrule_row_<?php echo $name . '_' . $index; ?>">
            <td>
                <input title="<?php _e( 'Apply this adjustment when the quantity in the cart starts at this value.  Use * for any.', 'ultimatewoo-pro' ); ?>" class="int_pricing_rule" id="pricing_rule_from_input_<?php echo $name . '_' . $index; ?>" type="text" name="pricing_rules[<?php echo $name; ?>][blockrules][<?php echo $index ?>][from]" value="<?php echo $rule['from']; ?>"/>
            </td>
            <td>
                <input title="<?php _e( 'Apply the discount to this many items', 'ultimatewoo-pro' ); ?>" class="int_pricing_rule" id="pricing_blockrule_to_input_<?php echo $name . '_' . $index; ?>" type="text" name="pricing_rules[<?php echo $name; ?>][blockrules][<?php echo $index ?>][adjust]" value="<?php echo $rule['adjust']; ?>"/>
            </td>

            <td>
                <select title="<?php _e( 'The type of adjustment to apply', 'ultimatewoo-pro' ); ?>" name="pricing_rules[<?php echo $name; ?>][blockrules][<?php echo $index; ?>][type]">
                    <option <?php selected( 'fixed_adjustment', $rule['type'] ); ?> value="fixed_adjustment"><?php _e( 'Price Discount', 'ultimatewoo-pro' ); ?></option>
                    <option <?php selected( 'percent_adjustment', $rule['type'] ); ?> value="percent_adjustment"><?php _e( 'Percentage Discount', 'ultimatewoo-pro' ); ?></option>
                    <option <?php selected( 'fixed_price', $rule['type'] ); ?> value="fixed_price"><?php _e( 'Fixed Price', 'ultimatewoo-pro' ); ?></option>
                </select>
            </td>

            <td>
                <input title="<?php _e( 'The value of the adjustment. Currency and percentage symbols are not required', 'ultimatewoo-pro' ); ?>" class="float_rule_number" id="pricing_blockrule_amount_input_<?php echo $name . '_' . $index; ?>" type="text"
                       name="pricing_rules[<?php echo $name; ?>][blockrules][<?php echo $index; ?>][amount]" value="<?php echo $rule['amount']; ?>"/>
            </td>

            <td>
                <select title="<?php _e( 'If the rule is repeating', 'ultimatewoo-pro' ); ?>" id="pricing_blockrule_type_value_<?php echo $name . '_' . $index; ?>" name="pricing_rules[<?php echo $name; ?>][blockrules][<?php echo $index; ?>][repeating]">
                    <option <?php selected( 'no', $rule['repeating'] ); ?> value="no"><?php _e( 'No', 'ultimatewoo-pro' ); ?></option>
                    <option <?php selected( 'yes', $rule['repeating'] ); ?> value="yes"><?php _e( 'Yes', 'ultimatewoo-pro' ); ?></option>
                </select>
            </td>
        ...
        <script type="text/javascript">
            ...
            jQuery(document).ready(function ($) {
                ...
                function InsertContinuousRule(previousRowIndex, name) {
                    ...
                    html += '<option value="percentage_discount"><?php _e('Percentage Discount', 'ultimatewoo-pro'); ?></option>';
                    html += '<option value="fixed_price"><?php _e('Fixed Price', 'ultimatewoo-pro'); ; ?></option>';
                    html += '</select>';
                    html += '</td>';
                    html += '<td>';
                ...
                function InsertBlockRule(previousRowIndex, name) {
                    ...
                    html += '<option value="price_discount"><?php _e('Price Discount', 'ultimatewoo-pro'); ?></option>';
                    html += '<option value="percentage_discount"><?php _e('Percentage Discount', 'ultimatewoo-pro'); ?></option>';
                    html += '<option value="fixed_price"><?php _e('Fixed Price', 'ultimatewoo-pro'); ?></option>';
                    html += '</select>';
                    html += '</td>';
                    ...
                    html += '<option value="no"><?php _e('No', 'ultimatewoo-pro'); ?></option>';
                ...
                function DeleteRule(index, name) {
                    if (confirm("<?php _e('Are you sure you would like to remove this price adjustment?', 'ultimatewoo-pro'); ?>")) {
                ...
                function DeleteRuleSet(name) {
                    if (confirm("<?php _e('Are you sure you would like to remove this price set?', 'ultimatewoo-pro'); ?>")) {
