...
class woocommerce_category_pricing_rules_admin {
	...
	private function get_description( $key ) {
		...
		$tips = array(
		    ...
		    'target' => __('Items from the selected categories will be eligible for the discount', 'ultimatewoo-pro'),
		    'source' => __('Items from the selected categories will contribute to the total required before the discount will be applied', 'ultimatewoo-pro'),
			...
			<form method="post" action="options.php">
				...
				<?php if ( !is_wp_error( $terms ) && !empty( $terms ) ) : ?>
					<table class="widefat">
						...
						<th>
							<?php _e('Category', 'ultimatewoo-pro'); ?>
						</th>
						<th>
				...
				<?php else : ?>
					<?php _e( 'You need to create product categories first', 'ultimatewoo-pro' ); ?>
				<?php endif; ?>
			</form>
		</div>
		<?php
	...
	public function advanced_meta_box() {
		...
		<div id="woocommerce-pricing-category">
			...
			<form method="post" action="options.php">
				...
				<?php if ( !empty( $terms ) && !is_wp_error( $terms ) ): ?>
					...
					<button id="woocommerce-pricing-add-ruleset" type="button" class="button button-secondary">+ <?php _e('Add Category Pricing', 'ultimatewoo-pro'); ?></button>
		...
		public function create_empty_ruleset( $set_index ) {
			...
			$pricing_rule_sets['set_' . $set_index]['title'] = __('Rule Set ', 'ultimatewoo-pro') . $set_index;
		...
		private function create_collector( $collector, $name ) {
			...
			<select title="Choose how to calculate the quantity.  This tallied amount is used in determining the min and max quantities used below in the Quantity Pricing section." class="pricing_rule_when" id="pricing_rule_when_<?php echo $name; ?>" name="pricing_rules[<?php echo $name; ?>][collector][type]">
				...
				<option title="Calculate quantity based on total sum of the categories in the cart" <?php selected( 'cat', $collector['type'] ); ?> value="cat"><?php _e( 'Sum of Category', 'ultimatewoo-pro' ); ?></option>
			</select>
			<div class="cats">   
		...
		private function create_target_selector( $targets, $name ) {
			...
			<br />
			<div class="cats">   
				<label> <?php _e( 'Categories to apply adjustment to:', 'ultimatewoo-pro' ); ?> <?php $this->get_description( 'target' ); ?></label>
		...
		<?php do_action( 'woocommerce_dynamic_pricing_metabox_js', 'advanced_category' ); ?>
					...
					function InsertBlockRule(previousRowIndex, name) {
						...
						html += '<option value="yes"><?php _e('Yes', 'ultimatewoo-pro'); ?></option>';
						html += '</select>';
						html += '</td>';
						html += '<td width="48">';
