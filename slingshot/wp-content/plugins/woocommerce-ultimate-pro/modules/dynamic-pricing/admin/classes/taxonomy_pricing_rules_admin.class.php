...
class woocommerce_taxonomy_pricing_rules_admin {
	...
	private function get_description( $key ) {
		...
		$tips = array(
		    'title' => __( 'A title for this rule set to indentify it in the dashboard', 'ultimatewoo-pro' ),
		    'purchase' => __('The quantity the user must purchase at full price', 'ultimatewoo-pro' ),
		    'receive' => __('The quantity the user will receive at the discount', 'ultimatewoo-pro' ),
		    'type' => __('The type of discount to apply', 'ultimatewoo-pro' ),
		    'amount' => __('The amount of discount to be applied', 'ultimatewoo-pro' ),
		    'repeating' => __('If the discount will apply to multiples of the receive amount or if the discount will only be applied to exactly the number of receive items', 'ultimatewoo-pro' ),
		    'target' => __('Items from the selected ' . $this->the_taxonomy->labels->name . ' will be eligible for the discount', 'ultimatewoo-pro' ),
		    'source' => __('Items from the selected ' . $this->the_taxonomy->labels->name . ' will contribute to the total required before the discount will be applied', 'ultimatewoo-pro' ),
		    'apply_to' => __('(optional) Choose the roles customers must be in to be eligible for the discount', 'ultimatewoo-pro' ),
		    'mode' => __('Bulk rules allow you to configure a bulk discount based on specific quantities the user purchases, such as buy 5 items receive a 10% discount.  Special offers allow you to configure rules such as buy one get one free.', 'ultimatewoo-pro' )
		);
		...
		<div id="poststuff" class="woocommerce-category-wrap">
			...
			<form method="post" action="options.php">
				...
				<?php if ( !is_wp_error( $terms ) && !empty( $terms ) ) : ?>
					...
					<p class="submit">
						<input type="submit" class="button-primary" value="<?php _e( 'Save Changes', 'ultimatewoo-pro' ) ?>" />
					</p>
				<?php else : ?>
					<?php printf(__( 'You need to create %s first', 'ultimatewoo-pro' ), $the_taxonomy->labels->name); ?>
				<?php endif; ?>
			</form>
		</div>
		<?php
			...
			<tr>
				...
				<td>
					<select id="pricing_rule_type_value_<?php echo $name . '_' . $index; ?>" name="pricing_rules[<?php echo $name; ?>][rules][<?php echo $index; ?>][type]">
						...
						<option <?php $this->selected( 'fixed_product', $rule['type'] ); ?> value="fixed_product"><?php _e('Price Discount', 'ultimatewoo-pro' ); ?></option>
						<option <?php $this->selected( 'percent_product', $rule['type'] ); ?> value="percent_product"><?php _e('Percentage Discount', 'ultimatewoo-pro' ); ?></option>
					</select>
				</td>
				<td>
	...
	public function advanced_meta_box() {
		...
		<div id="woocommerce-pricing-category">
			<?php settings_errors(); ?>
			<h3><span><?php _e( 'Advanced Rules', 'ultimatewoo-pro' ); ?></span></h3>
			...
			<form method="post" action="options.php">
				...
				<?php else : ?>
					<?php printf(__( 'You need to create %s first', 'ultimatewoo-pro' ), $this->the_taxonomy->labels->name); ?>
				<?php endif; ?>
			</form>
			<?php
		}
		...
		public function create_rulesets( $pricing_rule_sets ) {
			...
			foreach ( $pricing_rule_sets as $name => $pricing_rule_set ) {
				...
				<div id="woocommerce-pricing-ruleset-<?php echo $name; ?>" class="woocommerce_pricing_ruleset <?php echo $validation_class; ?>">
					<h4 class="first"><?php echo (isset( $pricing_rule_set['admin_title'] ) && !empty( $pricing_rule_set['admin_title'] ) ? esc_attr( $pricing_rule_set['admin_title'] ) : sprintf(__( '%s Pricing', 'ultimatewoo-pro' ), $the_taxonomy->labels->name)); ?><a href="#" data-name="<?php echo $name; ?>" class="delete_pricing_ruleset" ><img  src="<?php echo WC_Dynamic_Pricing::plugin_url(); ?>/assets/images/delete.png" title="delete this set" alt="delete this set" style="cursor:pointer; margin:0 3px;float:right;" /></a></h4>
					<div>
						<p>
							<label for="pricing_rule_admin_title_<?php echo $name; ?>"><?php _e( 'Admin Title', 'ultimatewoo-pro' ); ?>:</label><?php $this->get_description( 'title' ); ?>
					...
					<div id="woocommerce-pricing-mode-<?php echo $name; ?>" class="section">
						<label for="pricing_ruleset_mode_value_<?php echo $name . '_0'; ?>"><?php _e( 'Rule Processing Mode', 'ultimatewoo-pro' ); ?></label><?php $this->get_description( 'mode' ); ?>
					...
					<div class="section" style="display:<?php echo ($mode == 'block' ? 'block' : 'none;'); ?>">
						<table  id="woocommerce-pricing-blockrules-table-<?php echo $name; ?>" class="block" data-lastindex="<?php echo (is_array( $pricing_rules ) && sizeof( $pricing_rules ) > 0) ? count( $pricing_rules ) : '1'; ?>">
							<thead>
							<th>
								<?php _e( 'Purchase', 'ultimatewoo-pro' ); ?><?php $this->get_description( 'purchase' ); ?>
							</th>
							<th>
								<?php _e( 'Receive', 'ultimatewoo-pro' ); ?><?php $this->get_description( 'receive' ); ?>
							</th>
							<th>
								<?php _e( 'Discount Type', 'ultimatewoo-pro' ); ?><?php $this->get_description( 'type' ); ?>
							</th>
							<th>
								<?php _e( 'Discount Amount', 'ultimatewoo-pro' ); ?><?php $this->get_description( 'amount' ); ?>
							</th>
							<th>
								<?php _e( 'Repeating', 'ultimatewoo-pro' ); ?><?php $this->get_description( 'repeating' ); ?>
							</th>
					...
					<div id="woocommerce-pricing-rules-continuous-section-<?php echo $name; ?>" class="section" style="display:<?php echo $mode == 'continuous' ? 'block;' : 'none;'; ?>">
						<label><?php _e('Quantity Pricing', 'ultimatewoo-pro' ); ?></label>
		...
		private function create_condition_apply_to( $condition, $name, $condition_index ) {
			...
			<div>
				<label for="pricing_rule_apply_to_<?php echo $name . '_' . $condition_index; ?>"><?php _e('Applies To:', 'ultimatewoo-pro'); ?></label><?php $this->get_description( 'apply_to' ); ?>
				<input type="hidden" name="pricing_rules[<?php echo $name; ?>][conditions][<?php echo $condition_index; ?>][type]" value="apply_to" />
		...
		private function create_collector( $collector, $name ) {
			...
			?>
			<label for="pricing_rule_when_<?php echo $name; ?>"><?php _e( 'Quantities based on:', 'ultimatewoo-pro' ); ?></label><?php $this->get_description( 'source' ); ?>
			<select title="Choose how to calculate the quantity.  This tallied amount is used in determining the min and max quantities used below in the Quantity Pricing section." class="pricing_rule_when" id="pricing_rule_when_<?php echo $name; ?>" name="pricing_rules[<?php echo $name; ?>][collector][type]">
				<option title="Calculate quantity based on cart item quantity" <?php selected( 'cat_product', $collector['type'] ); ?> value="cat_product"><?php _e( 'Cart Line Item Quantity', 'ultimatewoo-pro' ); ?></option>
				<option title="Calculate quantity based on total sum of the <?php echo $this->the_taxonomy->labels->name; ?> in the cart" <?php selected( 'cat', $collector['type'] ); ?> value="cat"><?php printf(__( 'Sum of %s', 'ultimatewoo-pro' ), $this->the_taxonomy->labels->singular_name); ?></option>
			</select>
			<div class="cats">   
		...
		private function create_target_selector( $targets, $name ) {
			...
			<br />
			<div class="cats">   
				<label> <?php printf(__( '%s to apply adjustment to:', 'ultimatewoo-pro' ), $this->the_taxonomy->labels->name); ?> <?php $this->get_description( 'target' ); ?></label>
