...
class WC_CSP_Condition_Shipping_Country_State extends WC_CSP_Condition {
	...
	public function __construct() {
		...
		$this->title                          = __( 'Shipping Country', 'ultimatewoo-pro' );
		...
		foreach ( $shipping_destinations as $shipping_destination ) {
			...
			if ( $data[ 'modifier' ] === 'in' && in_array( $shipping_country, $data[ 'value' ] ) ) {
				...
				if ( empty( $data[ 'states' ][ $shipping_country ] ) ) {
					return sprintf( __( 'select a different shipping %s', 'ultimatewoo-pro' ), __( 'Country', 'ultimatewoo-pro' ) );
					...
					return sprintf( __( 'select a different shipping %s', 'ultimatewoo-pro' ), $state_type );
				}
		...
		<div class="condition_row_inner">
			<div class="condition_modifier">
				<select name="restriction[<?php echo $index; ?>][conditions][<?php echo $condition_index; ?>][modifier]">
					...
					<option value="not-in" <?php selected( $modifier, 'not-in', true ) ?>><?php echo __( 'is not', 'ultimatewoo-pro' ); ?></option>
				</select>
			</div>
			<div class="condition_value select-field">
				<select name="restriction[<?php echo $index; ?>][conditions][<?php echo $condition_index; ?>][value][]" style="width:80%;" class="multiselect <?php echo WC_CSP_Core_Compatibility::is_wc_version_gte_2_3() ? 'wc-enhanced-select' : 'chosen_select'; ?>" multiple="multiple" data-placeholder="<?php _e( 'Select shipping countries&hellip;', 'ultimatewoo-pro' ); ?>">
					<?php
		...
		<div class="condition_row_inner">
			<div class="condition_modifier exclude_states">
				<select name="restriction[<?php echo $index; ?>][conditions][<?php echo $condition_index; ?>][exclude_states]">
					<?php
						echo '<option value="all" ' . selected( empty( $states ), true, false ) . '>' . __( 'all States', 'ultimatewoo-pro' ) . '</option>';
						echo '<option value="specific" ' . selected( ! empty( $states ), true, false ) . '>' . __( 'specific States', 'ultimatewoo-pro' ) . '</option>';
					?>
				</select>
			</div>
			<div class="condition_value excluded_states select-field" <?php echo empty( $states ) ? 'style="display:none;"': ''; ?>><?php
				if ( ! empty( $countries ) ) {
					?><select name="restriction[<?php echo $index; ?>][conditions][<?php echo $condition_index; ?>][states][]" style="width:80%;" class="multiselect <?php echo WC_CSP_Core_Compatibility::is_wc_version_gte_2_3() ? 'wc-enhanced-select' : 'chosen_select'; ?>" multiple="multiple" data-placeholder="<?php _e( 'Select States / Regions&hellip;', 'ultimatewoo-pro' ); ?>">
						<?php
						if ( ! empty( $countries ) ) {
					...
					?><span class="condition_description"><?php
					 echo __( 'To select specific States/Regions, first add some countries with States/Regions and then save your changes.', 'ultimatewoo-pro' );
					?></span><?php
				}
			?></div>
		</div><?php
