...
class WC_CSP_Condition_Package_Weight extends WC_CSP_Condition {
	...
	public function __construct() {
		...
		$this->title                         = __( 'Package Weight', 'ultimatewoo-pro' );
	...
	public function get_condition_resolution( $data, $args ) {
		...
		if ( $data[ 'modifier' ] === 'min' && $data[ 'value' ] <= $pkg_weight ) {
			return sprintf( __( 'decrease the package weight below %1$s%2$s', 'ultimatewoo-pro' ), $data[ 'value' ], get_option( 'woocommerce_weight_unit' ) );
		}

		if ( $data[ 'modifier' ] === 'max' && $data[ 'value' ] > $pkg_weight ) {
			return sprintf( __( 'increase the package weight above %1$s%2$s', 'ultimatewoo-pro' ), $data[ 'value' ], get_option( 'woocommerce_weight_unit' ) );
		}
	...
	public function get_admin_fields_html( $index, $condition_index, $condition_data ) {
		...
		<div class="condition_modifier">
			<select name="restriction[<?php echo $index; ?>][conditions][<?php echo $condition_index; ?>][modifier]">
				<option value="max" <?php selected( $modifier, 'max', true ) ?>><?php echo __( '<', 'ultimatewoo-pro' ); ?></option>
				<option value="min" <?php selected( $modifier, 'min', true ) ?>><?php echo __( '>=', 'ultimatewoo-pro' ); ?></option>
			</select>
		</div>
