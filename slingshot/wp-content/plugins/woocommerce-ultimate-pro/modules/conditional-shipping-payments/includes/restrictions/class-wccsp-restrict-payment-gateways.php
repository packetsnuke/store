...
class WC_CSP_Restrict_Payment_Gateways extends WC_CSP_Restriction implements WC_CSP_Checkout_Restriction {
	...
	public function __construct() {
		...
		$this->title                    = __( 'Payment Gateways', 'ultimatewoo-pro' );
		$this->description              = __( 'Restrict the available payment gateways based on product-related constraints.', 'ultimatewoo-pro' );
		...
		$this->method_title             = __( 'Payment Gateway Restrictions', 'ultimatewoo-pro' );
	...
	function generate_admin_global_fields_html() {
		?><p>
			<?php echo __( 'Restrict the payment gateways available during checkout when the defined conditions apply. Complex rules can be created by adding multiple restriction instances. Each instance will be evaluated independently.', 'ultimatewoo-pro' ); ?>
		</p><?php
	...
	public function get_admin_fields_html( $index, $options = array(), $field_type = 'global' ) {
		...
		<p class="form-field">
			...
			<textarea class="short_description" name="restriction[<?php echo $index; ?>][description]" id="restriction_<?php echo $index; ?>_short_description" placeholder="<?php _e( 'Optional short description for this rule&hellip;', 'ultimatewoo-pro' ); ?>" rows="1" cols="20"><?php echo $description; ?></textarea>
		</p>
		<p class="form-field">
			<label><?php _e( 'Exclude Gateways', 'ultimatewoo-pro' ); ?></label>
			<select name="restriction[<?php echo $index; ?>][gateways][]" style="width:80%" class="multiselect <?php echo WC_CSP_Core_Compatibility::is_wc_version_gte_2_3() ? 'wc-enhanced-select' : 'chosen_select'; ?>" multiple="multiple" data-placeholder="<?php _e( 'Select Payment Gateways&hellip;', 'ultimatewoo-pro' ); ?>">
				<?php
		...
		<p class="form-field">
			...
			<?php echo WC_CSP_Core_Compatibility::wc_help_tip( __( 'By default, excluded payment gateways are removed from the list of gateways available during checkout. Select this option if you prefer to show excluded gateways in the checkout options and display a restriction notice when customers attempt to complete an order using an excluded gateway.', 'ultimatewoo-pro' ) ); ?>
		</p>
		<p class="form-field">
			<label>
				...
				<?php
					...
					if ( $field_type === 'global' ) {
						$tiptip = __( 'Defaults to:<br/>&quot;Unfortunately, your order cannot be checked out via {excluded_gateway}. To complete your order, please select an alternative payment method.&quot;<br/>When conditions are defined, resolution instructions are added to the default message.', 'ultimatewoo-pro' );
					} else {
						$tiptip = __( 'Defaults to:<br/>&quot;Unfortunately, {product} cannot be checked out via {excluded_gateway}. To complete your order, please select an alternative payment method, or remove {product} from your cart.&quot;<br/>When conditions are defined, resolution instructions are added to the default message.', 'ultimatewoo-pro' );
					}
				?>
			</label>
			...
			<?php
				...
				if ( $field_type === 'global' ) {
					$tip = __( 'Define a custom checkout error message to show when selecting an excluded payment gateway. You may include <code>{excluded_gateway}</code> and have it substituted by the selected payment gateway title.', 'ultimatewoo-pro' );
				} else {
					$tip = __( 'Define a custom checkout error message to show when selecting an excluded payment gateway. You may include <code>{product}</code> and <code>{excluded_gateway}</code> and have them substituted by the actual product title and the selected payment gateway title.', 'ultimatewoo-pro' );
				}
	...
	function get_admin_product_fields_html( $index, $options = array() ) {
		?><div class="description">
			<em><?php echo __( 'Restrict the payment gateways available during checkout when an order contains this product.', 'ultimatewoo-pro' ); ?></em>
		</div><?php
	...
	public function process_admin_product_fields( $posted_data ) {
		...
		if ( ! $processed_data ) {
			...
			WC_Admin_Meta_Boxes::add_error( sprintf( __( 'Restriction #%s was not saved. Before saving a &quot;Payment Gateways&quot; restriction, remember to add at least one payment gateway to the exclusions list.', 'ultimatewoo-pro' ), $posted_data[ 'index' ] ) );
			return false;
		}
	...
	public function process_admin_global_fields( $posted_data ) {
		...
		if ( ! $processed_data ) {
			...
			WC_Admin_Settings::add_error( sprintf( __( 'Restriction #%s was not saved. Before saving a &quot;Payment Gateways&quot; restriction, remember to add at least one payment gateway to the exclusions list.', 'ultimatewoo-pro' ), $posted_data[ 'index' ] ) );
			return false;
		}
						...
						if ( $restriction_exists ) {
							...
							if ( ! empty( $restriction[ 'message' ] ) ) {
								...
								if ( $conditions_resolution ) {
									$resolution = sprintf( __( 'To purchase &quot;%3$s&quot; via &quot;%1$s&quot;, please %2$s. Otherwise, select an alternative payment method, or remove &quot;%3$s&quot; from your cart.', 'ultimatewoo-pro' ), $available_gateways[ $chosen_gateway ]->get_title(), $conditions_resolution, $product->get_title() );
								} else {
									$resolution = sprintf( __( 'To complete your order, please select an alternative payment method, or remove &quot;%1$s&quot; from your cart.', 'ultimatewoo-pro' ), $product->get_title() );
								}

								$message = __( 'Unfortunately, &quot;%1$s&quot; cannot be checked out via &quot;%2$s&quot;. %3$s', 'ultimatewoo-pro' );
							}
					...
					if ( ! empty( $restriction[ 'gateways' ] ) && $this->check_conditions_apply( $restriction ) ) {
						...
						if ( in_array( $chosen_gateway, $restricted_gateways ) ) {
							...
							if ( ! empty( $restriction[ 'message' ] ) ) {
								...
								if ( $conditions_resolution ) {
									$resolution = sprintf( __( 'To complete your order via &quot;%1$s&quot;, please %2$s. Otherwise, choose an alternative payment method.', 'ultimatewoo-pro' ), $available_gateways[ $chosen_gateway ]->get_title(), $conditions_resolution );
								} else {
									$resolution = __( 'To complete your order, please select an alternative payment method.', 'ultimatewoo-pro' );
								}

								$message = __( 'Unfortunately, your order cannot be checked out via &quot;%1$s&quot;. %2$s', 'ultimatewoo-pro' );
							}
