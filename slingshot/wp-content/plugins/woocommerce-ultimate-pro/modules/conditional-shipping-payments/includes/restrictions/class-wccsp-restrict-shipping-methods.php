...
class WC_CSP_Restrict_Shipping_Methods extends WC_CSP_Restriction implements WC_CSP_Checkout_Restriction {
	...
	public function __construct() {
		...
		$this->title                    = __( 'Shipping Methods', 'ultimatewoo-pro' );
		$this->description              = __( 'Restrict the available shipping methods based on product-related constraints.', 'ultimatewoo-pro' );
		...
		$this->method_title             = __( 'Shipping Method Restrictions', 'ultimatewoo-pro' );
	...
	public function shipping_method_option( $method_id, $method, $selected_methods ) {
		...
		if ( WC_CSP_Core_Compatibility::is_wc_version_gte_2_6() ) {
			...
			if ( $method->supports( 'shipping-zones' ) ) {
				...
				echo '<option value="' . esc_attr( $method_id ) . '" ' . selected( in_array( $method_id, $selected_methods ), true, false ) . '>' . sprintf( __( 'All &quot;%s&quot; Method Instances', 'ultimatewoo-pro' ), $method->get_method_title() ) . '</option>';
				...
				foreach ( $zones as $zone ) {
					...
					if ( ! empty( $zone[ 'shipping_methods' ] ) ) {
						...
						foreach ( $zone[ 'shipping_methods' ] as $instance_id => $method_instance ) {
							...
							$method_title = sprintf( __( '&quot;%1$s&quot; (Instance ID: %2$s)', 'ultimatewoo-pro' ), $method_instance->get_title(), $instance_id );
							$option_name  = sprintf( __( '%1$s &ndash; %2$s', 'ultimatewoo-pro' ), $zone_name, $method_title );
				...
				if ( $method_id === 'legacy_flat_rate' ) {
					...
					echo '<optgroup label="' . __( 'Flat Rates (Legacy)', 'ultimatewoo-pro' ) . '">';
					echo '<option value="' . esc_attr( $method_id ) . '" ' . selected( in_array( $method_id, $selected_methods ), true, false ) . '>' . $method->get_title() . __( ' (Legacy)', 'ultimatewoo-pro' ) . '</option>';
					...
					$option    = '<option value="' . esc_attr( $method_id ) . '" ' . selected( in_array( $method_id, $selected_methods ), true, false ) . '>' . $method->get_title() . ( $is_legacy ? __( ' (Legacy)', 'ultimatewoo-pro' ) : '' ) . '</option>';
			...
			if ( $method_id === 'table_rate' && class_exists( 'WC_Shipping_Table_Rate' ) ) {
				echo '<optgroup label="' . __( 'Table Rates', 'ultimatewoo-pro' ) . '">';
				echo '<option value="' . esc_attr( $method_id ) . '" ' . selected( in_array( $method_id, $selected_methods ), true, false ) . '>' . __( 'Table Rate Shipping &ndash; All Rates', 'ultimatewoo-pro' ) . '</option>';
				...
				if ( ! empty( $table_rates ) ) {
					...
					foreach ( $table_rates as $table_rate ) {
						...
						$table_rate_zone_name = $table_rate->zone_name ? $table_rate->zone_name : __( 'Default Zone', 'ultimatewoo-pro' );
						...
						if ( ! array_key_exists( $table_rate->shipping_method_id, $methods ) ) {
							...
							$table_rate_method_title                    = sprintf( __( '%1$s (ID: %2$s)', 'ultimatewoo-pro' ), $table_rate_method->title, $table_rate_option_id );
							$methods[ $table_rate->shipping_method_id ] = $table_rate_method_title;

							$table_rate_option_name                     = sprintf( __( 'Table Rate Shipping &ndash; %1$s: %2$s, All Rates', 'ultimatewoo-pro' ), $table_rate_zone_name, $table_rate_method_title );
						...
						$table_rate_rate_label   = $table_rate->rate_label ? sprintf( __( '%1$s (ID: %2$s)', 'ultimatewoo-pro' ), $table_rate->rate_label, $table_rate->rate_id ) : sprintf( __( 'Unlabelled Rate (ID: %1$s)', 'ultimatewoo-pro' ), $table_rate->rate_id );

						$table_rate_option_name  = sprintf( __( 'Table Rate Shipping &ndash; %1$s: %2$s, %3$s', 'ultimatewoo-pro' ), $table_rate_zone_name, $table_rate_method_title, $table_rate_rate_label );
				...
				echo '<optgroup label="' . __( 'Flat Rate Boxes', 'ultimatewoo-pro' ) . '">';
				echo '<option value="' . esc_attr( $method_id ) . '" ' . selected( in_array( $method_id, $selected_methods ), true, false ) . '>' . __( 'Flat Rate Boxes Shipping &ndash; All Rates', 'ultimatewoo-pro' ) . '</option>';
				...
				if ( ! empty( $box_methods ) ) {
					...
					foreach ( $box_methods as $box_method ) {
						...
						$flat_rate_boxes_zone_name = $box_method->zone_name ? $box_method->zone_name : __( 'Default Zone', 'ultimatewoo-pro' );
						...
						if ( ! array_key_exists( $box_method->shipping_method_id, $methods ) ) {
							...
							$flat_rate_boxes_method_title               = sprintf( __( '%1$s (ID: %2$s)', 'ultimatewoo-pro' ), $flat_rate_boxes_method->title, $flat_rate_boxes_option_id );
							$methods[ $box_method->shipping_method_id ] = $flat_rate_boxes_method_title;

							$flat_rate_boxes_option_name                = sprintf( __( 'Flat Rate Boxes Shipping &ndash; %1$s: %2$s', 'ultimatewoo-pro' ), $flat_rate_boxes_zone_name, $flat_rate_boxes_method_title );
				...
				echo '<optgroup label="' . __( 'Flat Rates', 'ultimatewoo-pro' ) . '">';
	...
	private function additional_legacy_flat_rate_options( $method, $selected_methods ) {
		...
		foreach ( $additional_flat_rate_options as $option ) {
			...
			echo '<option value="' . esc_attr( $option_id ) . '" ' . selected( in_array( $option_id, $selected_methods ), true, false ) . '>' . $this_option[0] . __( ' (Legacy)', 'ultimatewoo-pro' ) . '</option>';
		}
	}
	...
	function generate_admin_global_fields_html() {
		?><p>
			<?php echo __( 'Restrict the shipping methods available during checkout when the defined conditions apply. Complex rules can be created by adding multiple restriction instances. Each instance will be evaluated independently.', 'ultimatewoo-pro' ); ?>
		</p><?php
	...
	public function get_admin_fields_html( $index, $options = array(), $field_type = 'global' ) {
		...
		<p class="form-field">
			<label>
				<?php _e( 'Short Description', 'ultimatewoo-pro' ); ?>:
			</label>
			<textarea class="short_description" name="restriction[<?php echo $index; ?>][description]" id="restriction_<?php echo $index; ?>_message" placeholder="<?php _e( 'Optional short description for this rule&hellip;', 'ultimatewoo-pro' ); ?>" rows="1" cols="20"><?php echo $description; ?></textarea>
		</p>
		<p class="form-field">
			<label><?php _e( 'Exclude Shipping Methods', 'ultimatewoo-pro' ); ?></label>
			<select name="restriction[<?php echo $index; ?>][methods][]" style="width:80%" class="multiselect <?php echo WC_CSP_Core_Compatibility::is_wc_version_gte_2_3() ? 'wc-enhanced-select' : 'chosen_select'; ?>" multiple="multiple" data-placeholder="<?php _e( 'Select Shipping Methods&hellip;', 'ultimatewoo-pro' ); ?>">
				<?php
		...
		<p class="form-field">
			<label><?php _e( 'Exclude Shipping Rate IDs', 'ultimatewoo-pro' ); ?></label>
			<input type="text" name="restriction[<?php echo $index; ?>][custom_rates]" style="width:80%" placeholder="<?php _e( 'Shipping rate IDs to exclude, separated by &quot;|&quot;&hellip;', 'ultimatewoo-pro' ); ?>" value="<?php echo $custom_rates_input; ?>"/>
			<?php echo WC_CSP_Core_Compatibility::wc_help_tip( __( 'Manually enter shipping rate IDs to exclude. Useful if you are working with shipping methods that retrieve real-time rates, or if you simply require more granular control of the shipping options available during checkout. <strong>Important</strong>: Exclusion rules based on rate IDs are not necessarily upgrade-safe &ndash; use at your own risk!', 'ultimatewoo-pro' ) ); ?>
		</p>
		<p class="form-field">
			<label>
				<?php _e( 'Show Excluded', 'ultimatewoo-pro' ); ?>
			</label>
			...
			<?php echo WC_CSP_Core_Compatibility::wc_help_tip( __( 'By default, excluded shipping methods are removed from the list of methods available during checkout. Select this option if you prefer to show excluded shipping methods in the checkout options and display a restriction notice when customers attempt to complete an order using an excluded method.', 'ultimatewoo-pro' ) ); ?>
		</p>
		<p class="form-field">
			<label>
				<?php _e( 'Custom Checkout Notice', 'ultimatewoo-pro' ); ?>:
				<?php

					if ( $field_type === 'global' ) {
						$tiptip = __( 'Defaults to:<br/>&quot;Unfortunately, your order cannot be shipped via {excluded_method}. To complete your order, please select an alternative shipping method.&quot;<br/>When conditions are defined, resolution instructions are added to the default message.', 'ultimatewoo-pro' );
					} else {
						$tiptip = __( 'Defaults to:<br/>&quot;Unfortunately, {product} is not eligible for shipping via {excluded_method}. To complete your order, please select an alternative shipping method, or remove {product} from your cart.&quot;<br/>When conditions are defined, resolution instructions are added to the default message.', 'ultimatewoo-pro' );
					}
				?>
			</label>
			...
			<?php
				...
				if ( $field_type === 'global' ) {
					$tip = __( 'Define a custom checkout error message to show when selecting an excluded shipping method. You may include <code>{excluded_method}</code> and have it substituted by the selected shipping method title.', 'ultimatewoo-pro' );
				} else {
					$tip = __( 'Define a custom checkout error message to show when selecting an excluded shipping method. You may include <code>{product}</code> and <code>{excluded_method}</code> and have them substituted by the actual product title and the selected shipping method title.', 'ultimatewoo-pro' );
				}
	...
	function get_admin_product_fields_html( $index, $options = array() ) {
		?><div class="description">
			<em><?php echo __( 'Restrict the available shipping methods when a shipping package contains this product.', 'ultimatewoo-pro' ); ?></em>
		</div><?php
	...
	public function process_admin_product_fields( $posted_data ) {
		...
		if ( ! $processed_data ) {
			...
			WC_Admin_Meta_Boxes::add_error( sprintf( __( 'Restriction #%s was not saved. Before saving a &quot;Shipping Method&quot; restriction, remember to add at least one shipping method to the exclusions list.', 'ultimatewoo-pro' ), $posted_data[ 'index' ] ) );
			return false;
		}
	...
	public function process_admin_global_fields( $posted_data ) {
		...
		if ( ! $processed_data ) {
			...
			WC_Admin_Settings::add_error( sprintf( __( 'Restriction #%s was not saved. Before saving a &quot;Shipping Method&quot; restriction, remember to add at least one shipping method to the exclusions list.', 'ultimatewoo-pro' ), $posted_data[ 'index' ] ) );
			return false;
		}
							...
							if ( $restriction_exists ) {
								...
								if ( ! empty( $restriction[ 'message' ] ) ) {
									...
									if ( $conditions_resolution ) {
										$resolution = sprintf( __( 'To have &quot;%1$s&quot; shipped via %2$s, please %3$s. Otherwise, select an alternative payment method, or remove &quot;%1$s&quot; from your cart.', 'ultimatewoo-pro' ), $product->get_title(), $package[ 'rates' ][ $chosen_methods[ $i ] ]->label, $conditions_resolution );
									} else {
										$resolution = sprintf( __( 'To complete your order, please select an alternative shipping method, or remove &quot;%1$s&quot; from your cart.', 'ultimatewoo-pro' ), $product->get_title() );
									}

									$message = __( 'Unfortunately, &quot;%1$s&quot; is not eligible for shipping via &quot;%2$s&quot;. %3$s', 'ultimatewoo-pro' );
								}
						...
						if ( ! empty( $restriction[ 'methods' ] ) && $this->check_conditions_apply( $restriction, array( 'package' => $package ) ) ) {
							...
							if ( $this->is_restricted( $chosen_methods[ $i ], $restricted_methods ) ) {
								...
								if ( ! empty( $restriction[ 'message' ] ) ) {
									...
									if ( $conditions_resolution ) {
										$resolution = sprintf( __( 'To have your order shipped via %1$s, please %2$s. Otherwise, choose an alternative shipping method.', 'ultimatewoo-pro' ), $package[ 'rates' ][ $chosen_methods[ $i ] ]->label, $conditions_resolution );
									} else {
										$resolution = __( 'To complete your order, please select an alternative shipping method.', 'ultimatewoo-pro' );
									}

									$message = __( 'Unfortunately, your order cannot be shipped via %1$s. %2$s', 'ultimatewoo-pro' );
								}
