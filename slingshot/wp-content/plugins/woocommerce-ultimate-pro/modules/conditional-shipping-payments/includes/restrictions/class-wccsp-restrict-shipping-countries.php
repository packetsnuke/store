...
class WC_CSP_Restrict_Shipping_Countries extends WC_CSP_Restriction implements WC_CSP_Checkout_Restriction, WC_CSP_Cart_Restriction {
	...
	public function __construct() {
		...
		$this->title                    = __( 'Shipping Countries &amp; States', 'ultimatewoo-pro' );
		$this->description              = __( 'Restrict the allowed shipping countries based on product-related constraints.', 'ultimatewoo-pro' );
		...
		$this->method_title             = __( 'Shipping Country Restrictions', 'ultimatewoo-pro' );
	...
	function generate_admin_global_fields_html() {
		?><p>
			<?php echo __( 'Restrict the allowed shipping countries when the defined conditions apply. Complex rules can be created by adding multiple restriction instances. Each instance will be evaluated independently.', 'ultimatewoo-pro' ); ?>
		</p><?php
	...
	public function get_admin_fields_html( $index, $options = array(), $field_type = 'global' ) {
		...
		<p class="form-field select-field">
			<label><?php _e( 'Exclude Countries', 'ultimatewoo-pro' ); ?></label>
			<select name="restriction[<?php echo $index; ?>][countries][]" style="width:80%" class="multiselect <?php echo WC_CSP_Core_Compatibility::is_wc_version_gte_2_3() ? 'wc-enhanced-select' : 'chosen_select'; ?>" multiple="multiple" data-placeholder="<?php _e( 'Select Countries&hellip;', 'ultimatewoo-pro' ); ?>">
				<?php
		...
		<p class="form-field exclude_states">
			<label><?php _e( 'Exclude States / Regions', 'ultimatewoo-pro' ); ?></label>
			<select name="restriction[<?php echo $index; ?>][exclude_states]" style="width:80%" class="<?php echo WC_CSP_Core_Compatibility::is_wc_version_gte_2_3() ? 'wc-enhanced-select' : ''; ?>">
				<?php
					echo '<option value="all" ' . selected( empty( $states ), true, false ) . '>' . __( 'All States / Regions in the selected Countries', 'ultimatewoo-pro' ) . '</option>';
					echo '<option value="specific" ' . selected( ! empty( $states ), true, false ) . '>' . __( 'Specific States / Regions', 'ultimatewoo-pro' ) . '</option>';
				?>
			</select>
		</p>
		<p class="form-field excluded_states select-field" <?php echo empty( $states ) ? 'style="display:none;"': ''; ?>><?php
			if ( ! empty( $countries ) ) {
				?><select name="restriction[<?php echo $index; ?>][states][]" style="width:80%" class="multiselect <?php echo WC_CSP_Core_Compatibility::is_wc_version_gte_2_3() ? 'wc-enhanced-select' : 'chosen_select'; ?>" multiple="multiple" data-placeholder="<?php _e( 'Select States / Regions&hellip;', 'ultimatewoo-pro' ); ?>">
					<?php
					if ( ! empty( $countries ) ) {
				...
				?><span class="description"><?php
				 echo __( 'To select specific States / Regions, please add some countries with States / Regions to the <strong>Exclude Countries</strong> field and then save your changes.', 'ultimatewoo-pro' );
				?></span><?php
			}
			?>
		</p>
		<p class="form-field">
			<label>
				...
				<?php
					...
					if ( $field_type === 'global' ) {
						$tiptip = __( 'Defaults to:<br/>&quot;Unfortunately your order cannot be shipped {to_excluded_destination}. To complete your order, please select an alternative shipping country / state.&quot;<br/>When conditions are defined, resolution instructions are added to the default message.', 'ultimatewoo-pro' );
					} else {
						$tiptip = __( 'Defaults to:<br/>&quot;Unfortunately your order cannot be shipped {to_excluded_destination}. To complete your order, please select an alternative shipping country / state, or remove {product} from your cart.&quot;<br/>When conditions are defined, resolution instructions are added to the default message.', 'ultimatewoo-pro' );
					}
				?>
			</label>
			...
			<?php
				...
				if ( $field_type === 'global' ) {
					$tip = __( 'Define a custom checkout error message to show when selecting an excluded shipping destination. You may include <code>{to_excluded_destination}</code> and have it substituted by the selected shipping country / state.', 'ultimatewoo-pro' );
				} else {
					$tip = __( 'Define a custom checkout error message to show when selecting an excluded shipping destination. You may include <code>{product}</code> and <code>{to_excluded_destination}</code> and have them substituted by the actual product title and the excluded country / state.', 'ultimatewoo-pro' );
				}
	...
	public function process_admin_product_fields( $posted_data ) {
		...
		if ( ! $processed_data ) {
			...
			WC_Admin_Meta_Boxes::add_error( sprintf( __( 'Restriction #%s was not saved. Before saving a &quot;Shipping Countries&quot; restriction, remember to add at least one shipping country to the exclusions list.', 'ultimatewoo-pro' ), $posted_data[ 'index' ] ) );
			return false;
		}
	...
	public function process_admin_global_fields( $posted_data ) {
		...
		if ( ! $processed_data ) {
			...
			WC_Admin_Settings::add_error( sprintf( __( 'Restriction #%s was not saved. Before saving a &quot;Shipping Countries&quot; restriction, remember to add at least one shipping country to the exclusions list.', 'ultimatewoo-pro' ), $posted_data[ 'index' ] ) );
			return false;
		}
							...
							if ( ! empty( $restriction[ 'countries' ] ) && $this->check_conditions_apply( $restriction, array( 'cart_item_data' => $cart_item_data ) ) ) {
								...
								if ( in_array( $shipping_country, $restricted_countries ) ) {
									...
									if ( empty( $restriction[ 'states' ][ $shipping_country ] ) ) {
										...
										$to_destination     = sprintf( _x( '%1$s %2$s', 'to country destination', 'ultimatewoo-pro' ), WC()->countries->shipping_to_prefix(), $destination );
										$destination_type   = __( 'Country', 'ultimatewoo-pro' );
									} elseif ( in_array( $shipping_state, $restriction[ 'states' ][ $shipping_country ] ) ) {
										$restriction_exists = true;
										$destination        = $states[ $shipping_state ];
										$to_destination     = sprintf( _x( 'to %s', 'to state destination', 'ultimatewoo-pro' ), $destination );
							...
							if ( $restriction_exists ) {
								...
								if ( ! empty( $restriction[ 'message' ] ) ) {
									...
									if ( $conditions_resolution ) {
										$resolution = sprintf( _n( 'To have your order shipped %1$s, please %2$s. Otherwise, select an alternative shipping %3$s, or remove &quot;%4$s&quot; from your cart.', 'To have your order shipped %1$s, please %2$s. Otherwise, select an alternative shipping %3$s, or remove &quot;%4$s&quot; from this shipment.', sizeof( $shipping_packages ), 'ultimatewoo-pro' ), $to_destination, $conditions_resolution, $destination_type, $product->get_title() );
									} else {
										$resolution = sprintf( _n( 'To complete your order, please select an alternative shipping %1$s, or remove &quot;%2$s&quot; from your cart.', 'To complete your order, please select an alternative shipping %1$s, or remove &quot;%2$s&quot; from this shipment.', sizeof( $shipping_packages ), 'ultimatewoo-pro' ), $destination_type, $product->get_title() );
									}

									$message = __( 'Unfortunately your order cannot be shipped %2$s. %3$s', 'ultimatewoo-pro' );
								}
							...
							if ( ! empty( $restriction[ 'countries' ] ) && $this->check_conditions_apply( $restriction, array( 'package' => $shipping_package ) ) ) {
								...
								if ( $restriction_exists ) {
									...
									if ( ! empty( $restriction[ 'message' ] ) ) {
										...
										if ( $conditions_resolution ) {
											$resolution = sprintf( __( 'To have your order shipped %1$s, please %2$s. Otherwise, select an alternative shipping %3$s.', 'ultimatewoo-pro' ), $to_destination, $conditions_resolution, $destination_type );
										} else {
											$resolution = sprintf( __( 'To complete your order, please select an alternative shipping %1$s.', 'ultimatewoo-pro' ), $destination_type );
										}

										$message = __( 'Unfortunately your order cannot be shipped %1$s. %2$s', 'ultimatewoo-pro' );
									}
