...
class WC_CSP_Conditions {
	...
	private function get_admin_condition_fields( $scope, $restriction_id, $index, $options = array() ) {
		...
		?><h4><?php
			echo __( 'Conditions', 'ultimatewoo-pro' );
			echo WC_CSP_Core_Compatibility::wc_help_tip( __( 'All <strong>defined</strong> conditions must match <strong>simultaneously</strong> for the restriction to come into effect. Add multiple restriction instances to create complex rules.', 'ultimatewoo-pro' ) ); ?>
		</h4><?php
				...
				<tr>
					<td colspan="3">
						<div class="condition_add_remove">
							<button type="button" class="button button-secondary add_condition"><?php echo __( 'Add Condition', 'ultimatewoo-pro' ); ?></button>
							<button type="button" class="button button-secondary remove_conditions"><?php echo __( 'Remove Selected', 'ultimatewoo-pro' ); ?></button>
						</div>
					</td>
				</tr>
			</tfoot>
