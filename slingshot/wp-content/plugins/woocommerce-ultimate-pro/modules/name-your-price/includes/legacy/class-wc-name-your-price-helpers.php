...
class WC_Name_Your_Price_Helpers {
	...
	public static function get_minimum_price_html( $product ) {
		...
		if( $minimum > 0 ){
			...
			$minimum_text = stripslashes( get_option( 'woocommerce_nyp_minimum_text', __( 'Minimum Price:', 'ultimatewoo-pro' ) ) );
	...
	public static function get_suggested_price_html( $product ) {
		...
		if ( $suggested > 0 ) {
			...
			$suggested_text = stripslashes( get_option( 'woocommerce_nyp_suggested_text', __( 'Suggested Price:', 'ultimatewoo-pro' ) ) );
		...
		if( $price != '' ){
			...
			if( class_exists( 'WC_Subscriptions_Product' ) && WC_Subscriptions_Product::is_subscription( $product ) && 'woocommerce_get_price_html' != current_filter() ) {
				...
				if( self::is_billing_period_variable( $product ) ) { 
					...
					$html = sprintf( _x( ' %s / %s', 'Variable subscription price, ex: $10 / day', 'ultimatewoo-pro' ), wc_price( $price ), self::get_subscription_period_strings( 1, $period ) );
	...
	public static function get_error_message_template( $id = null ){
		...
		$errors = apply_filters( 'woocommerce_nyp_error_message_templates', 
			array( 
				'invalid' => __( '&quot;%%TITLE%%&quot; could not be added to the cart: Please enter a valid, positive number.', 'ultimatewoo-pro' ), 
				'minimum' => __( '&quot;%%TITLE%%&quot; could not be added to the cart: Please enter at least %%MINIMUM%%.', 'ultimatewoo-pro' ),
				'minimum_js' => __( 'Please enter at least %%MINIMUM%%', 'ultimatewoo-pro' )
			) 
		);
