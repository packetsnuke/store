...
class WC_Name_Your_Price_Admin {
	...
	public static function product_type_options( $options ){
	  ...
	  $options['nyp'] = array(
	      ...
	      'label' => __( 'Name Your Price', 'ultimatewoo-pro'),
	      'description' => __( 'Customers are allowed to determine their own price.', 'ultimatewoo-pro'),
	      'default' => 'no'
	    );
		...
		echo '<div class="options_group show_if_nyp">';
			...
			if( class_exists( 'WC_Subscriptions' ) && $show_billing_period_options ) {
				...
				woocommerce_wp_checkbox( array(
						...
						'label' => __( 'Variable Billing Period', 'ultimatewoo-pro' ),
						'description' => __( 'Allow the customer to set the billing period.', 'ultimatewoo-pro' ) ) );
			}
			...
			woocommerce_wp_text_input( array(
				...
				'label' => __( 'Suggested Price', 'ultimatewoo-pro') . ' ('.get_woocommerce_currency_symbol().')' ,
				'desc_tip' => 'true',
				'description' => __( 'Price to pre-fill for customers.  Leave blank to not suggest a price.', 'ultimatewoo-pro' ),
				'data_type' => 'price'
			) );
			...
			woocommerce_wp_text_input( array(
				'id' => '_min_price',
				'class' => 'wc_input_price short',
				'label' => __( 'Minimum Price', 'ultimatewoo-pro') . ' ('.get_woocommerce_currency_symbol().')',
				'desc_tip' => 'true',
				'description' =>  __( 'Lowest acceptable price for product. Leave blank to not enforce a minimum. Must be less than or equal to the set suggested price.', 'ultimatewoo-pro' ),
				'data_type' => 'price'
			) );
	...
	public static function product_variations_options( $loop, $variation_data, $variation ){ 
		...
		<label><input type="checkbox" class="checkbox variation_is_nyp" name="variation_is_nyp[<?php echo $loop; ?>]" <?php checked( $variation_is_nyp, 'yes' ); ?> /> <?php _e( 'Name Your Price', 'ultimatewoo-pro'); ?> <a class="tips" data-tip="<?php _e( 'Customers are allowed to determine their own price.', 'ultimatewoo-pro'); ?>" href="#">[?]</a></label>
		...
		if( WC_Name_Your_Price_Helpers::wc_is_version( '2.3' ) ){ ?>
			...
			<div class="variable_nyp_pricing">
				<p class="form-row form-row-first">
					<label><?php echo __( 'Suggested Price:', 'ultimatewoo-pro' ) . ' ('.get_woocommerce_currency_symbol().')'; ?></label>
				...
				<p class="form-row form-row-last">
					<label><?php echo __( 'Minimum Price:', 'ultimatewoo-pro' ) . ' ('.get_woocommerce_currency_symbol().')'; ?></label>
	...
	public static function bulk_edit_actions(){ ?>
		<optgroup label="<?php esc_attr_e( 'Name Your Price', 'ultimatewoo-pro' ); ?>">
			<option value="toggle_nyp"><?php _e( 'Toggle &quot;Name Your Price&quot;', 'ultimatewoo-pro' ); ?></option>
			<option value="variation_suggested_price"><?php _e( 'Set suggested prices', 'ultimatewoo-pro' ); ?></option>
			<option value="variation_suggested_price_increase"><?php _e( 'Increase suggested prices (fixed amount or %)', 'ultimatewoo-pro' ); ?></option>
			<option value="variation_suggested_price_decrease"><?php _e( 'Decrease suggested prices (fixed amount or %)', 'ultimatewoo-pro' ); ?></option>
			<option value="variation_min_price"><?php _e( 'Set minimum prices', 'ultimatewoo-pro' ); ?></option>
			<option value="variation_min_price_increase"><?php _e( 'Increase minimum prices (fixed amount or %)', 'ultimatewoo-pro' ); ?></option>
			<option value="variation_min_price_decrease"><?php _e( 'Decrease minimum prices (fixed amount or %)', 'ultimatewoo-pro' ); ?></option>
		</optgroup>
    ...
    public static function meta_box_script( $hook ){
		...
		$strings = array ( 'enter_value' => __( 'Enter a value', 'ultimatewoo-pro' ),
							'price_adjust' => __( 'Enter a value (fixed or %)', 'ultimatewoo-pro' ) );
    ...
    public static function add_help_tab(){
	    ...
	    'content'	=>
	    	...
	    	'<h4>' . __( 'Name Your Price', 'ultimatewoo-pro' ) . '</h4>' .

	    	'<p>' . __( 'In the "Product Meta" metabox, check the Name Your Price checkbox to allow your customers to enter their own price.', 'ultimatewoo-pro' ) . '</p>' .

	    	'<p>' . __( 'As of Name Your Price version 2.0, this ability is available for "Simple", "Subscription", "Bundled", "Variable" and "Variable Subscriptions" Products.', 'ultimatewoo-pro' ) . '</p>' .

	    	'<h4>' . __( 'Suggested Price', 'ultimatewoo-pro' ) . '</h4>' .

	    	'<p>' . __( 'This is the price you\'d like to suggest to your customers.  The Name Your Price input will be prefilled with this value.  To not suggest a price at all, you may leave this field blank.', 'ultimatewoo-pro' ) . '</p>' .

	    	'<p>' . __( 'This value must be a positive number.', 'ultimatewoo-pro' ) . '</p>' .

	    	'<h4>' . __( 'Minimum Price', 'ultimatewoo-pro' ) . '</h4>' .

	    	'<p>' . __( 'This is the lowest price you are willing to accept for this product.  To not enforce a minimum (ie: to accept any price, including zero), you may leave this field blank.', 'ultimatewoo-pro' ) . '</p>' .

	    	'<p>' . __( 'This value must be a positive number that is less than or equal to the set suggested price.', 'ultimatewoo-pro' ) . '</p>' .

	    	'<h4>' . __( 'Subscriptions', 'ultimatewoo-pro' ) . '</h4>' .

	    	'<p>' . __( 'If you have a name your price subscription product, the subscription time period fields are still needed, but the price will be disabled in lieu of the Name Your Price suggested and minimum prices.', 'ultimatewoo-pro' ) . '</p>' .

	    	'<p>' . __( 'As of Name Your Price version 2.0, you can now allow variable billing periods.', 'ultimatewoo-pro' ) . '</p>'
	...
	public static function product_filters( $output ){
		...
		if ( $pos ) {
			...
			$nyp_option = "<option value='name-your-price' ";
				...
				$nyp_option .= "> &#42; " . __( 'Name Your Price', 'ultimatewoo-pro' ) . "</option>";
		...
		<style>
		    ...
		    <div id="nyp-fields" class="inline-edit-col-left">
		    	...
		    	<br class="clear" />
			   	...
			   	<h4><?php _e( 'Name Your Price', 'ultimatewoo-pro' ); ?>  <input type="checkbox" name="_nyp" class="nyp" value="1" /></h4>

			    <div class="nyp_prices">
			    	<label>
			            <span class="title"><?php _e( 'Suggested Price', 'ultimatewoo-pro' ); ?></span>
			            <span class="input-text-wrap">
			            	<input type="text" name="_suggested_price" class="text suggested_price" placeholder="<?php _e( 'Suggested Price', 'ultimatewoo-pro' ); ?>" value="">
			            </span>
			        </label>
			        <label>
			            <span class="title"><?php _e( 'Minimum Price', 'ultimatewoo-pro' ); ?></span>
			            <span class="input-text-wrap">
			            	<input type="text" name="_min_price" class="text min_price" placeholder="<?php _e( 'Minimum price', 'ultimatewoo-pro' ); ?>" value="">
			        	</span>
			        </label>
			    </div>
