...
class WC_Gateway_Braintree_Credit_Card extends WC_Gateway_Braintree {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			wc_braintree(),
			array(
				'method_title'       => __( 'Braintree (Credit Card)', 'ultimatewoo-pro' ),
				'method_description' => __( 'Allow customers to securely pay using their credit card via Braintree.', 'ultimatewoo-pro' ),
				'supports'           => array(
	 ...
	 * @return bool
	 *
	 * ULTIMATEWOO fix: adjust path to card-cc-plain.svg in /assets/css/frontend/wc-braintree.min.css
	 */
	...
	protected function get_method_form_fields() {
		...
		$fields = array(
			...
			'fraud_settings_title' => array(
				'title' => __( 'Fraud Settings', 'ultimatewoo-pro' ),
				'type'  => 'title',
			),
			'fraud_tool'           => array(
				'title'    => __( 'Fraud Tool', 'ultimatewoo-pro' ),
				'type'     => 'select',
				'class'    => 'js-fraud-tool',
				'desc_tip' => __( 'Select the fraud tool you want to use. Basic is enabled by default and requires no additional configuration. Advanced requires you to enable advanced fraud tools in your Braintree control panel. To use Kount Direct you must contact Braintree support.', 'ultimatewoo-pro' ),
				'options'  => array(
					'basic'        => __( 'Basic', 'ultimatewoo-pro' ),
					'advanced'     => __( 'Advanced', 'ultimatewoo-pro' ),
					'kount_direct' => __( 'Kount Direct', 'ultimatewoo-pro' ),
				),
			),
			'kount_merchant_id'    => array(
				'title'    => __( 'Kount merchant ID', 'ultimatewoo-pro' ),
				'type'     => 'text',
				'class'    => 'js-kount-merchant-id',
				'desc_tip' => __( 'Speak with your account management team at Braintree to get this.', 'ultimatewoo-pro' ),
			),

			// 3D Secure
			'threed_secure_title' => array(
				'title'       => __( '3D Secure (Verified by Visa, MasterCard SecureCode)', 'ultimatewoo-pro' ),
				'type'        => 'title',
				'description' => sprintf( __( '3D Secure benefits cardholders and merchants by providing an additional layer of verification. %1$sLearn more about 3D Secure%2$s.', 'ultimatewoo-pro' ), '<a href="' . esc_url( $this->get_plugin()->get_documentation_url() ) . '#3d-secure' . '">', '</a>' ),
			),
			'threed_secure_enabled' => array(
				'title'       => __( '3D Secure', 'ultimatewoo-pro' ),
				'type'        => 'checkbox',
				'label'       => __( 'Enable 3D Secure (Visa and MasterCard cards only)', 'ultimatewoo-pro' ),
				'description' => __( 'You must contact Braintree support to add this feature to your Braintree account before enabling this option.', 'ultimatewoo-pro' ),
				'default'     => 'no',
			),
		);
	...
	protected function add_csc_form_fields( $form_fields ) {
		...
		$form_fields['require_csc'] = array(
			'title'   => __( 'Card Verification (CSC)', 'ultimatewoo-pro' ),
			'label'   => __( 'Display and Require the Card Security Code (CVV/CID) field on checkout', 'ultimatewoo-pro' ),
			'type'    => 'checkbox',
			'default' => 'yes',
		);
		...
		if ( $is_valid && $this->is_3d_secure_enabled() && SV_WC_Helper::get_post( 'wc_braintree_credit_card_payment_nonce' ) ) {
			...
			try {
				...
				if ( $payment_method->has_3d_secure_info() && ! $payment_method->get_3d_secure_liability_shifted() && ( $payment_method->get_3d_secure_liability_shift_possible() || $this->is_3d_secure_liability_shift_always_required() ) ) {
					$error = __( 'We cannot process your order with the payment information that you provided. Please use an alternate payment method.', 'ultimatewoo-pro' );
				}
				...
				$error = __( 'Oops, there was a temporary payment error. Please try another payment method or contact us to complete your transaction.', 'ultimatewoo-pro' );
			}
