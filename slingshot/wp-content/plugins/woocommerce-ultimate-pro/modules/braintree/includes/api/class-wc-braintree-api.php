		...
		if ( ! empty( $order->payment->nonce ) && ! empty( $order->payment->token ) ) {
			...
			if ( ! $result->transaction_approved() ) {
				...
				if ( $result->has_avs_rejection() ) {
					...
					$message = __( 'The billing address for this transaction does not match the cardholders.', 'ultimatewoo-pro' );

				} elseif ( $result->has_cvv_rejection() ) {

					$message = __( 'The CSC for the transaction was invalid or incorrect.', 'ultimatewoo-pro' );
	...
	protected function get_braintree_exception_message( $e ) {
		...
		switch ( get_class( $e ) ) {
			...
			case 'Braintree_Exception_Authentication':
				$message = __( 'Invalid Credentials, please double-check your API credentials (Merchant ID, Public Key, Private Key, and Merchant Account ID) and try again.', 'ultimatewoo-pro' );
			break;

			case 'Braintree_Exception_Authorization':
				$message = __( 'Authorization Failed, please verify the user for the API credentials provided can perform transactions and that the request data is correct.', 'ultimatewoo-pro' );
			break;

			case 'Braintree_Exception_DownForMaintenance':
				$message = __( 'Braintree is currently down for maintenance, please try again later.', 'ultimatewoo-pro' );
			break;

			case 'Braintree_Exception_NotFound':
				$message = __( 'The record cannot be found, please contact support.', 'ultimatewoo-pro' );
			break;

			case 'Braintree_Exception_ServerError':
				$message = __( 'Braintree encountered an error when processing your request, please try again later or contact support.', 'ultimatewoo-pro' );
			break;

			case 'Braintree_Exception_SSLCertificate':
				$message = __( 'Braintree cannot verify your server\'s SSL certificate. Please contact your hosting provider or try again later.', 'ultimatewoo-pro' );
			break;
