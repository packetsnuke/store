...
class WC_Intuit_QBMS_API_Request extends XMLWriter implements SV_WC_Payment_Gateway_API_Request {
	...
	public function customer_check_debit( $order ) {
		...
		$this->writeElement( 'InvoiceID', ltrim( $order->get_order_number(), _x( '#', 'hash before order number', 'ultimatewoo-pro' ) ) );
		...
		$comment = SV_WC_Order_Compatibility::get_prop( $order, 'customer_note' );
	...
	public function wallet_add( $order ) {
		...
		if ( 'credit_card' == $order->payment->type ) {
			...
			$this->writeElement( 'NameOnCard',           substr( SV_WC_Helper::str_to_ascii( $order->get_formatted_billing_full_name() ), 0, 30 ) );

			// Only include address validation information for US addresses
			if ( 'US' === $this->get_order_prop( 'billing_country' ) ) {
				$this->writeElement( 'CreditCardAddress',    substr( SV_WC_Helper::str_to_ascii( ! $this->get_order_prop( 'billing_address_2' ) ? $this->get_order_prop( 'billing_address_1' ) : $this->get_order_prop( 'billing_address_1' ) . ' ' . $this->get_order_prop( 'billing_address_2' ) ), 0, 30 ) );
				$this->writeElement( 'CreditCardCity',       substr( SV_WC_Helper::str_to_ascii( $this->get_order_prop( 'billing_city' ) ), 0, 50 ) );
				$this->writeElement( 'CreditCardState',      substr( SV_WC_Helper::str_to_ascii( $this->get_order_prop( 'billing_state' ) ), 0, 20 ) );
				$this->writeElement( 'CreditCardPostalCode', substr( SV_WC_Helper::str_to_ascii( str_replace( '-', '', $this->get_order_prop( 'billing_postcode' ) ) ), 0, 9 ) );
			}
			...
			$this->writeElement( 'PersonalDebitAccountType', 'savings' == $order->payment->account_type ? 'Savings' : 'Checking' ); // Checking/Savings
			$this->writeElement( 'PayorFirstName',           SV_WC_Helper::str_to_ascii( $this->get_order_prop( 'billing_first_name' ) ) );
			$this->writeElement( 'PayorLastName',            SV_WC_Helper::str_to_ascii( $this->get_order_prop( 'billing_last_name' ) ) );

			// </PersonalPaymentInfo>
			$this->endElement();

			$this->writeElement( 'PayorPhoneNumber', SV_WC_Helper::str_to_ascii( $this->get_order_prop( 'billing_phone' ) ) );
	...
	private function credit_card_charge_auth_request( $request_type, $order ) {
		...
		if ( isset( $order->payment->token ) && $order->payment->token ) {
			...
			$this->writeElement( 'NameOnCard',           substr( isset( $order->payment->test_condition ) ? 'configid=' . $order->payment->test_condition : SV_WC_Helper::str_to_ascii( $order->get_formatted_billing_full_name() ), 0, 30 ) );
		...
		$this->writeElement( 'InvoiceID', substr( SV_WC_Helper::str_to_ascii( ltrim( $order->get_order_number(), _x( '#', 'hash before order number', 'ultimatewoo-pro' ) ) ), 0, 100 ) );
	...
	protected function get_order_prop( $prop ) {
		...
		return SV_WC_Order_Compatibility::get_prop( $this->get_order(), $prop );
	}
