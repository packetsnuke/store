...
class WC_Gateway_Intuit_QBMS_Credit_Card extends WC_Gateway_Intuit_QBMS {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			wc_intuit_payments(),
			array(
				'method_title'       => __( 'Intuit QBMS', 'ultimatewoo-pro' ),
				'method_description' => __( 'Allow customers to securely pay using their credit cards with Intuit QBMS.', 'ultimatewoo-pro' ),
				'supports'           => array(
	...
	protected function add_csc_form_fields( $form_fields ) {
		...
		$form_fields['require_csc'] = array(
			'title'    => __( 'Require Card Verification', 'ultimatewoo-pro' ),
			'label'    => __( 'Require the Card Security Code (CV2) for all transactions', 'ultimatewoo-pro' ),
			'desc_tip' => __( 'Enabling this field will require the CSC even for tokenized transactions, and will disable support for WooCommerce Subscriptions and WooCommerce Pre-Orders.  Enable this if you have configured your merchant settings to Reject Transaction if CSC is not available.', 'ultimatewoo-pro' ),
			'type'     => 'checkbox',
			'default'  => 'no',
		);
	...
	public function render_test_amount_input( $desc ) {
		...
		if ( $this->is_test_environment() ) {
			...
			$test_conditions = array(
				'10200_comm'        => __( 'CC Processing Gateway comm error', 'ultimatewoo-pro' ),
				'10201_login'       => __( 'Processing Gateway login error', 'ultimatewoo-pro' ),
				'10301_ccinvalid'   => __( 'Invalid CC account number', 'ultimatewoo-pro' ),
				'10400_insufffunds' => __( 'Insufficient funds', 'ultimatewoo-pro' ),
				'10401_decline'     => __( 'Transaction declined', 'ultimatewoo-pro' ),
				'10403_acctinvalid' => __( 'Invalid merchant account', 'ultimatewoo-pro' ),
				'10404_referral'    => __( 'Declined pending voice auth', 'ultimatewoo-pro' ),
				'10406_capture'     => __( 'Capture error', 'ultimatewoo-pro' ),
				'10500_general'     => __( 'General error', 'ultimatewoo-pro' ),
				'10000_avscvdfail'  => __( 'AVS Failure', 'ultimatewoo-pro' ),
			);

			ob_start();

			echo '<select name="wc-intuit-qbms-credit-card-test-condition">';
				echo '<option value="">' . __( 'Test an Error Condition:', 'ultimatewoo-pro' ) . '</option>';
	...
	public function get_order( $order_id ) {
		...
		if ( $this->is_environment( 'test' ) && SV_WC_Helper::get_post( 'wc-intuit-qbms-credit-card-test-condition' ) ) {
			$order->payment->test_condition = SV_WC_Helper::get_post( 'wc-intuit-qbms-credit-card-test-condition' );
		}
	...
	protected function get_order_for_capture( $order ) {
		...
		$order->capture->request_id = SV_WC_Order_Compatibility::get_prop( $order, 'id' ) . '-' . rand();
	...
	protected function get_order_for_refund( $order, $amount, $reason ) {
		...
		//  the moment since there's no real way to test.  For further info:
		//  https://developer.intuit.com/docs/030_qbms/0060_documentation/error_handling#QBMS_Error_Recovery
		$order->refund->request_id = SV_WC_Order_Compatibility::get_prop( $order, 'id' ) . '-' . rand();
	...
	public function subscriptions_admin_add_payment_meta( $meta, $subscription ) {
		...
		if ( isset( $meta[ $this->get_id() ] ) ) {
			...
			$meta[ $this->get_id() ]['post_meta'][ $this->get_order_meta_prefix() . 'payment_token' ]['label'] = __( 'Wallet Token ID', 'ultimatewoo-pro' );
		}
	...
	public function subscriptions_admin_validate_payment_meta( $meta ) {
		...
		if ( ! ctype_digit( (string) $meta['post_meta'][ $this->get_order_meta_prefix() . 'payment_token' ]['value'] ) ) {
			throw new Exception( __( 'Wallet Token ID must be numeric.', 'ultimatewoo-pro' ) );
		}
	}
