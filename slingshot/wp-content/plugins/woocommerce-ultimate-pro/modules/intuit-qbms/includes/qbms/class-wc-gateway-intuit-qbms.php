...
class WC_Gateway_Intuit_QBMS extends SV_WC_Payment_Gateway_Direct {
	...
	protected function maybe_add_settings_descriptions() {
		...
		$application_login_description = $this->application_login ? '' :
			sprintf( __( 'You can create your own production Application Login and Application ID in the %1$sIntuit Partner Program%2$s, or use the default.  This is optional.', 'ultimatewoo-pro' ),
				'<a href="https://developer.intuit.com">', '</a>' );

		$connection_ticket_description =
			sprintf( __( 'Generate a new production connection ticket for the configured Application ID by following %1$sthis link%2$s', 'ultimatewoo-pro' ),
		...
		$test_application_login_description = $this->test_application_login ? '' :
			sprintf( __( 'You can create your own test Application Login and Application ID in the %1$sIntuit Partner Program%2$s, or use the default.  This is optional.', 'ultimatewoo-pro' ),
					'<a href="https://developer.intuit.com">', '</a>' );

		$test_connection_ticket_description =
			sprintf( __( 'Generate a new test connection ticket for the configured Application ID by following %1$sthis link%2$s', 'ultimatewoo-pro' ),
	...
	protected function get_method_form_fields() {
		...
		return array(
			...
			'application_login' => array(
				'title'    => __( 'Application Login', 'ultimatewoo-pro' ),
				'type'     => 'text',
				'class'    => 'environment-field production-field',
				'desc_tip' => __( 'Your application login value, this will be something like appname.domainname.com', 'ultimatewoo-pro' ),
				'placeholder' => self::PRODUCTION_APP_LOGIN_DEFAULT,
			),

			'application_id' => array(
				'title'    => __( 'Application ID', 'ultimatewoo-pro' ),
				'type'     => 'text',
				'class'    => 'environment-field production-field',
				'desc_tip' => __( 'Your application ID value.', 'ultimatewoo-pro' ),
				'placeholder' => self::PRODUCTION_APP_ID_DEFAULT,
			),

			'connection_ticket' => array(
				'title'    => __( 'Connection Ticket', 'ultimatewoo-pro' ),
				'type'     => 'password',
				'class'    => 'environment-field production-field',
				'desc_tip' => __( 'Your unique connection ticket value.', 'ultimatewoo-pro' ),
			),
	...
	public function render_view_transaction_form() {
		...
		if ( $this->get_id() !== SV_WC_Order_Compatibility::get_prop( $order, 'payment_method' ) ) {
			return;
		}

		// search form parameters
		$transaction_id = $this->get_order_meta( $order, 'trans_id', true );
		$date           = ( SV_WC_Order_Compatibility::get_date_paid( $order ) ) ? SV_WC_Order_Compatibility::get_date_paid( $order )->date( 'm/d/Y' ) : '';
