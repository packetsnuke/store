...
class WC_Points_Rewards_Points_Log_List_Table extends WP_List_Table {
	...
	public function get_columns() {
		...
		$columns = array(
			...
			'event'    => __( 'Event', 'ultimatewoo-pro' ),
		...
		switch ( $column_name ) {
			...
			case 'customer':
				...
				if ( $customer_email ) {
					...
					$column_content = sprintf( '<a href="%s">%s</a>', get_edit_user_link( $log_entry->user_id ), ( $user ) ? $user->user_login : __( 'Unknown', 'ultimatewoo-pro' ) );
				}
	...
	public function no_items() {
		...
		if ( isset( $_REQUEST['s'] ) ) : ?>
			<p><?php _e( 'No log entries found', 'ultimatewoo-pro' ); ?></p>
		<?php else : ?>
			<p><?php _e( 'Point log entries will appear here for you to view and manage.', 'ultimatewoo-pro' ); ?></p>
		<?php endif;
	}
	...
	public function extra_tablenav( $which ) {
		if ( 'top' == $which ) {
			...
			<select id="dropdown_event_types" name="_event_type" class="wc-enhanced-select" style="width:200px">
				<option value=""><?php _e( 'Show All Event Types', 'ultimatewoo-pro' ) ?></option>
				<?php
	...
	private function render_dates_dropdown() {
		...
		<select id="dropdown_dates" name='date' class="wc-enhanced-select" style="width:200px">
			<option<?php selected( $date, 0 ); ?> value='0'><?php _e( 'Show all Event Dates', 'ultimatewoo-pro' ); ?></option>
			<?php
			foreach ( $months as $arc_row ) {
				...
				printf( "<option %s value='%s'>%s</option>\n",
					...
					sprintf( __( '%1$s %2$d', 'ultimatewoo-pro' ), $wp_locale->get_month( $month ), $year )
				);
			}
			?>
		</select>
