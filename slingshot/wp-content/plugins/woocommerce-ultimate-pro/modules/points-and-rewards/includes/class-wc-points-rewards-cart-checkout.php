...
class WC_Points_Rewards_Cart_Checkout {
	...
	public function coupon_label( $label ) {
		if ( strstr( strtoupper( $label ), 'WC_POINTS_REDEMPTION' ) ) {
			$label = esc_html( __( 'Points redemption', 'ultimatewoo-pro' ) );
		}
	...
	public function render_redeem_points_message() {
		...
		$message .= '<input type="submit" class="button wc_points_rewards_apply_discount" name="wc_points_rewards_apply_discount" value="' . __( 'Apply Discount', 'ultimatewoo-pro' ) . '" /></form>';
		...
		if ( 'yes' === get_option( 'wc_points_rewards_partial_redemption_enabled' ) ) {
			...
			wc_enqueue_js( '
				$( "body" ).on( "click", "input.wc_points_rewards_apply_discount", function( e ) {
					var points = prompt( "' . esc_js( __( 'How many points would you like to apply?', 'ultimatewoo-pro' ) ) . '", "' . $points . '" );
					if ( points != null ) {
