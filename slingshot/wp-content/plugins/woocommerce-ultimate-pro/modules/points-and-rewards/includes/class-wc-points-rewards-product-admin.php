...
class WC_Points_Rewards_Product_Admin {
	...
	public function render_simple_product_fields() {
		...
		woocommerce_wp_text_input( array(
				...
				'class'         => 'short',
				'wrapper_class' => 'show_if_simple show_if_booking',
				'label'         => __( 'Maximum Discount', 'ultimatewoo-pro' ),
				'description'   => __( 'Enter either a fixed maximum discount amount or percentage which restricts the amount of points that can be redeemed for a discount based on the product price. For example, if you want to restrict the discount on this product to a maximum of 50%, enter 50%, or enter 5 to restrict the maximum discount to $5.  This setting overrides the global/category defaults, use 0 to disable point discounts for this product, and blank to use the global/category default.', 'ultimatewoo-pro' ),
				'desc_tip'      => true,
				'type'          => 'text',
			)
		);
		...
		if ( class_exists( 'WC_Subscriptions' ) ) {
			...
			woocommerce_wp_text_input( array(
					...
					'label'         => __( 'Change Renewal Points', 'ultimatewoo-pro' ),
	...
	public function render_variable_product_fields( $loop, $variation_data, $variation ) {
		...
		?>
			<p class="form-row form-row-first">
				<label><?php _e( 'Points Earned', 'ultimatewoo-pro' ); ?><a href="#" class="tips" data-tip="<?php echo esc_attr( $points_earned_description ); ?>">: [?]</a></label>
				<input type="text" size="5" name="variable_points_earned[<?php echo esc_attr( $loop ); ?>]" value="<?php echo esc_attr( $points_earned ); ?>" step="any" min="0" placeholder="<?php _e( 'Variation Points Earned', 'ultimatewoo-pro' ); ?>" />
			</p>

			<p class="form-row form-row-last">
				<label><?php _e( 'Maximum Points Discount', 'ultimatewoo-pro' ); ?><a href="#" class="tips" data-tip="<?php echo esc_attr( $max_discount_description ); ?>">: [?]</a></label>
				<input type="text" size="5" name="variable_max_point_discount[<?php echo esc_attr( $loop ); ?>]" value="<?php echo esc_attr( $max_discount ); ?>" placeholder="<?php _e( 'Variation Max Points Discount', 'ultimatewoo-pro' ); ?>" />
			</p>
				<?php
				if ( class_exists( 'WC_Subscriptions' ) ) {
				?>
			<p class="form-row form-row-full">
				<label><?php _e( 'Change Renewal Points', 'ultimatewoo-pro' ); ?><a href="#" class="tips" data-tip="<?php echo esc_attr( $renewal_points_description ); ?>">: [?]</a></label>
				<input type="text" size="5" name="variable_renewal_points[<?php echo esc_attr( $loop ); ?>]" value="<?php echo esc_attr( $renewal_points ); ?>" placeholder="<?php _e( 'Variation Change Rewards Points', 'ultimatewoo-pro' ); ?>" />
			</p>
				<?php
				}
		} else {
			...
			<tr>
				<td>
					...
					<label><?php _e( 'Points Earned', 'ultimatewoo-pro' ); ?></label>
					<input type="number" size="5" name="variable_points_earned[<?php echo esc_attr( $loop ); ?>]" value="<?php echo esc_attr( $points_earned ); ?>" step="any" min="0" placeholder="<?php _e( 'Variation Points Earned', 'ultimatewoo-pro' ); ?>" />
				</td>
				<td>
					...
					<label><?php _e( 'Maximum Points Discount', 'ultimatewoo-pro' ); ?></label>
					<input type="text" size="5" name="variable_max_point_discount[<?php echo esc_attr( $loop ); ?>]" value="<?php echo esc_attr( $max_discount ); ?>" placeholder="<?php _e( 'Variation Max Points Discount', 'ultimatewoo-pro' ); ?>" />
				</td>
			</tr>
			<?php
			...
			?>
				<tr>
					<td class="show_if_variable-subscription">
						...
						<label><?php _e( 'Change Renewal Points', 'ultimatewoo-pro' ); ?></label>
						<input type="text" size="5" name="variable_renewal_points[<?php echo esc_attr( $loop ); ?>]" value="<?php echo esc_attr( $renewal_points ); ?>" placeholder="<?php _e( 'Variation Change Rewards Points', 'ultimatewoo-pro' ); ?>" />
					</td>
				</tr>
			<?php
	...
	public function add_variable_product_bulk_edit_points_action() {
		echo '<option value="variable_points_earned">' . __( 'Points Earned', 'ultimatewoo-pro' ) . '</option>';
		...
		<script type="text/javascript">
			jQuery( document ).ready( function( $ ) {
				$( 'body' ).on( 'variable_points_earned', function( bulk_edit ) {
					var value = window.prompt( "<?php echo esc_js( __( 'Enter a value', 'ultimatewoo-pro' ) ); ?>" );
		...
		?>
			<div class="inline-edit-group">
				<label class="alignleft">
					<span class="title"><?php _e( 'Points Earned', 'ultimatewoo-pro' ); ?></span>
						<span class="input-text-wrap">
							<select class="change_points_earned change_to" name="change_points_earned">
								<?php
								$options = array(
									''  => __( '— No Change —', 'ultimatewoo-pro' ),
									'1' => __( 'Change to:', 'ultimatewoo-pro' ),
									'2' => __( 'Increase by (fixed amount or %):', 'ultimatewoo-pro' ),
									'3' => __( 'Decrease by (fixed amount or %):', 'ultimatewoo-pro' ),
								);
				...
				<label class="alignright">
					<input type="text" name="_wc_points_earned" class="text points_earned" placeholder="<?php _e( 'Enter Points Earned', 'ultimatewoo-pro' ); ?>" value="" />
				</label>
			</div>
		<?php
	}
	...
	private function get_product_category_fields_html( $points = '', $max_discount = '' ) {
		$points_earned_description   = __( 'This can either be a fixed number of points earned for the purchase of any product that belongs to this category, or a percentage which assigns points based on the price of the product. For example, if you want to award points equal to double the normal rate, enter 200%.  This setting modifies the global Points Conversion Rate, but can be overridden by a product/variation. Use 0 to assign no points for products belonging to this category, and empty to use the global setting.', 'ultimatewoo-pro' );
		$max_discount_description    = __( 'Enter either a fixed maximum discount amount or percentage which restricts the amount of points that can be redeemed for a discount based on the price of the product in this category. For example, if you want to restrict the discount on this category to a maximum of 50%, enter 50%, or enter $5 to restrict the maximum discount to $5.  This setting overrides the global default, but can be overridden by a product/variation. Use 0 to disable point discounts for this product, and blank to use the global/category default.', 'ultimatewoo-pro' );
		$renewal_points_description = __( 'For Subscription renwals with a different point value than the signup, enter either a fixed maximum discount amount or percentage which restricts the amount of points that can be redeemed for a discount based on the product price. For example, if you want to restrict the discount on this product to a maximum of 50%, enter 50%, or enter 5 to restrict the maximum discount to $5.  This setting overrides the global/category defaults, use 0 to disable point discounts for this product, and blank to use the global/category default.', 'ultimatewoo-pro' );
		?>
			<tr class="formfield">
				<th scope="row" valign="top"><label><?php _e( 'Points Earned', 'ultimatewoo-pro' ); ?></label></th>
				<td>
					<input type="text" size="5" name="_wc_points_earned" value="<?php echo esc_attr( $points ); ?>" step="any" min="0" placeholder="<?php _e( 'Category Points Earned', 'ultimatewoo-pro' ); ?>" />
			...
			<tr class="formfield">
				<th scope="row" valign="top"><label><?php _e( 'Maximum Points Discount', 'ultimatewoo-pro' ); ?></label></th>
				<td>
		...
		?>
			<tr class="formfield">
				<th scope="row" valign="top"><label><?php _e( 'Change Renewal Points', 'ultimatewoo-pro' ); ?></label></th>
				<td>
	...
	public function add_product_category_list_table_points_column_header( $columns ) {
		...
		foreach ( $columns as $column_key => $column_title ) {
			...
			if ( 'slug' == $column_key ) {
				$new_columns['points_earned'] = __( 'Points Earned', 'ultimatewoo-pro' );
			}
		}
