...
class WC_Points_Rewards_Admin {
	...
	public function __construct() {
		...
		$this->tabs = array(
			'manage'   => __( 'Manage Points', 'ultimatewoo-pro' ),
			'log'      => __( 'Points Log', 'ultimatewoo-pro' ),
			'settings' => __( 'Settings', 'ultimatewoo-pro' )
		);
	...
	public function verify_coupons_enabled() {
		...
		if ( ! $coupons_enabled ) {
			$message = sprintf(
				__( 'WooCommerce Points and Rewards requires coupons to be %senabled%s in order to function properly and allow customers to redeem points during checkout.', 'ultimatewoo-pro' ),
	...
	public function add_menu_link() {
		...
		$this->page_id = add_submenu_page(
			'woocommerce',
			__( 'Points & Rewards', 'ultimatewoo-pro' ),
			__( 'Points & Rewards', 'ultimatewoo-pro' ),
			'manage_woocommerce',
			'woocommerce-points-and-rewards',
	...
	public function add_list_table_options() {
		...
		if ( isset( $_GET['tab'] ) && 'log' === $_GET['tab'] ) {
			$args = array(
				'label' => __( 'Points Log', 'ultimatewoo-pro' ),
				'default' => 20,
				'option' => 'wc_points_rewards_points_log_per_page',
			);
		} else {
			$args = array(
				'label' => __( 'Manage Points', 'ultimatewoo-pro' ),
				'default' => 20,
	...
	private function show_manage_tab() {
		...
		echo '<h2>' . __( 'Manage Customer Points', 'ultimatewoo-pro' ) . '</h2>';
	...
	private function show_log_tab() {
		...
		// title/search result string
		echo '<h2>' . __( 'Points Log', 'ultimatewoo-pro' ) . '</h2>';
	...
	public function render_settings() {
		...
		$confirm_message = __( 'Are you sure you want to apply points to all previous orders that have not already had points generated? This cannot be reversed! Note that this can take some time in shops with a large number of orders, if an error occurs, simply Apply Points again to continue the process.', 'ultimatewoo-pro' );
	...
	public static function get_settings() {
		...
		$settings = array(
			...
			array(
				'title' => __( 'Points Settings', 'ultimatewoo-pro' ),
				'type'  => 'title',
			...
			array(
				'title'    => __( 'Earn Points Conversion Rate', 'ultimatewoo-pro' ),
				'desc_tip' => __( 'Set the number of points awarded based on the product price.', 'ultimatewoo-pro' ),
			...
			// earn points conversion
			array(
				'title'    => __( 'Earn Points Rounding Mode', 'ultimatewoo-pro' ),
				'desc_tip' => __( 'Set how points should be rounded.', 'ultimatewoo-pro' ),
			...
			array(
				'title'    => __( 'Redemption Conversion Rate', 'ultimatewoo-pro' ),
				'desc_tip' => __( 'Set the value of points redeemed for a discount.', 'ultimatewoo-pro' ),
			...
			// redeem points conversion
			array(
				'title'    => __( 'Partial Redemption', 'ultimatewoo-pro' ),
				'desc'     => __( 'Enable partial redemption', 'ultimatewoo-pro' ),
				'desc_tip' => __( 'Lets users enter how many points they wish to redeem during cart/checkout.', 'ultimatewoo-pro' ),
			...
			array(
				'title'    => __( 'Maximum Points Discount', 'ultimatewoo-pro' ),
				'desc_tip' => __( 'Set the maximum product discount allowed for the cart when redeeming points. Use either a fixed monetary amount or a percentage based on the product price. Leave blank to disable.', 'ultimatewoo-pro' ),
			...
			// maximum points discount available
			array(
				'title'    => __( 'Maximum Product Points Discount', 'ultimatewoo-pro' ),
				'desc_tip' => __( 'Set the maximum product discount allowed when redeeming points per-product. Use either a fixed monetary amount or a percentage based on the product price. Leave blank to disable. This can be overridden at the category and product level.', 'ultimatewoo-pro' ),
			...
			// points label
			array(
				'title'    => __( 'Points Label', 'ultimatewoo-pro' ),
				'desc_tip' => __( 'The label used to refer to points on the frontend, singular and plural.', 'ultimatewoo-pro' ),
				'id'       => 'wc_points_rewards_points_label',
				'default'  => sprintf( '%s:%s', __( 'Point', 'ultimatewoo-pro' ), __( 'Points', 'ultimatewoo-pro' ) ),
				'type'     => 'singular_plural',
			),

			// Expire Points
			array(
				'title'       => __( 'Points Expire After', 'ultimatewoo-pro' ),
				'desc_tip'    => __( 'Set the period after which points expire once granted to a user', 'ultimatewoo-pro' ),
				'type'        => 'points_expiry',
			...
			array(
				'title' => __( 'Product / Cart / Checkout Messages', 'ultimatewoo-pro' ),
				'desc'  => sprintf( __( 'Adjust the message by using %1$s{points}%2$s and %1$s{points_label}%2$s to represent the points earned / available for redemption and the label set for points.', 'ultimatewoo-pro' ), '<code>', '</code>' ),
				'type'  => 'title',
			...
			array(
				...
				'desc_tip' => __( 'Add an optional message to the single product page below the price. Customize the message using {points} and {points_label}. Limited HTML is allowed. Leave blank to disable.', 'ultimatewoo-pro' ),
				'id'       => 'wc_points_rewards_single_product_message',
				'css'      => 'min-width: 400px;',
				'default'  => sprintf( __( 'Purchase this product now and earn %s!', 'ultimatewoo-pro' ), '<strong>{points}</strong> {points_label}' ),
				'type'     => 'textarea',
			),

			// variable product page message
			array(
				'title'    => __( 'Variable Product Page Message', 'ultimatewoo-pro' ),
				'desc_tip' => __( 'Add an optional message to the variable product page below the price. Customize the message using {points} and {points_label}. Limited HTML is allowed. Leave blank to disable.', 'ultimatewoo-pro' ),
				'id'       => 'wc_points_rewards_variable_product_message',
				'css'      => 'min-width: 400px;',
				'default'  => sprintf( __( 'Earn up to %s Points.', 'ultimatewoo-pro' ), '<strong>{points}</strong> {points_label}' ),
				'type'     => 'textarea',
			),

			// earn points cart/checkout page message
			array(
				'title'    => __( 'Earn Points Cart/Checkout Page Message', 'ultimatewoo-pro' ),
				'desc_tip' => __( 'Displayed on the cart and checkout page when points are earned. Customize the message using {points} and {points_label}. Limited HTML is allowed.', 'ultimatewoo-pro' ),
				'id'       => 'wc_points_rewards_earn_points_message',
				'css'      => 'min-width: 400px;',
				'default'  => sprintf( __( 'Complete your order and earn %s for a discount on a future purchase', 'ultimatewoo-pro' ), '<strong>{points}</strong> {points_label}' ),
				'type'     => 'textarea',
			),

			// redeem points cart/checkout page message
			array(
				'title'    => __( 'Redeem Points Cart/Checkout Page Message', 'ultimatewoo-pro' ),
				'desc_tip' => __( 'Displayed on the cart and checkout page when points are available for redemption. Customize the message using {points}, {points_value}, and {points_label}. Limited HTML is allowed.', 'ultimatewoo-pro' ),
				'id'       => 'wc_points_rewards_redeem_points_message',
				'css'      => 'min-width: 400px;',
				'default'  => sprintf( __( 'Use %s for a %s discount on this order!', 'ultimatewoo-pro' ), '<strong>{points}</strong> {points_label}', '<strong>{points_value}</strong>' ),
				'type'     => 'textarea',
			),

			// earned points thank you / order received page message
			array(
				'title'    => __( 'Thank You / Order Received Page Message', 'ultimatewoo-pro' ),
				'desc_tip' => __( 'Displayed on the thank you / order received page when points were earned. Customize the message using {points}, {total_points}, {points_label}, and {total_points_label}. Limited HTML is allowed.', 'ultimatewoo-pro' ),
				'id'       => 'wc_points_rewards_thank_you_message',
				'css'      => 'min-width: 400px;min-height: 75px;',
				'default'  => sprintf( __( 'You have earned %s for this order. You have a total of %s.', 'ultimatewoo-pro' ), '<strong>{points}</strong> {points_label}', '<strong>{total_points}</strong> {total_points_label}' ),
				'type'     => 'textarea',
			),
			...
			array(
				'title' => __( 'Points Earned for Actions', 'ultimatewoo-pro' ),
				'desc'  => __( 'Customers can also earn points for actions like creating an account or writing a product review. You can enter the amount of points the customer will earn for each action in this section.', 'ultimatewoo-pro' ),
				'type'  => 'title',
			...
			array(
				'type'  => 'title',
				'title' => __( 'Actions', 'ultimatewoo-pro' ),
				'id'    => 'wc_points_rewards_points_actions_start',
			),

			array(
				'title'       => __( 'Apply Points to Previous Orders', 'ultimatewoo-pro' ),
				'desc_tip'    => __( 'This will apply points to all previous orders (processing and completed) and cannot be reversed.', 'ultimatewoo-pro' ),
				'button_text' => __( 'Apply Points', 'ultimatewoo-pro' ),
				'type'        => 'apply_points',
			...
			?>
				<tr valign="top">
					...
					<td class="forminp forminp-text">
						<fieldset>
							<input name="<?php echo esc_attr( $field['id'] . '_points' ); ?>" id="<?php echo esc_attr( $field['id'] . '_points' ); ?>" type="text" style="max-width: 50px;" value="<?php echo esc_attr( $points ); ?>" />&nbsp;<?php _e( 'Points', 'ultimatewoo-pro' ); ?>
				...
				<td class="forminp forminp-text" style="width: 50%; float: left;">
					<fieldset id="expire_points">
						...
						<select name="<?php echo esc_attr( $field[ 'id' ] . '_period' ); ?>" id="<?php echo esc_attr( $field[ 'id' ] ); ?>_period">
							...
							?>
								<option value="<?php echo esc_attr( $period_id ); ?>" <?php echo $selected; ?>><?php _e( $period_text, 'ultimatewoo-pro' ); ?></option>
							<?php endforeach; ?>
						</select>

						<fieldset>
							<p class="form-field expire-points-since">
								<label for="expire_points_since"><?php printf( __( '%sOnly apply to points earned since%s - %sOptional%s', 'ultimatewoo-pro' ), '<strong>', '</strong>', '<em>', '</em>' ); ?></label>
								<input type="text" class="date-picker" style="width: 200px;" name="expire_points_since" id="expire_points_since" value="<?php echo esc_attr( $expire_since ); ?>" placeholder="<?php echo _x( 'YYYY-MM-DD', 'placeholder', 'ultimatewoo-pro' ); ?>" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" />
								<p class="description"><?php _e( 'Leave blank to apply to all points', 'ultimatewoo-pro' ); ?></p>
							</p>
						</fieldset>
				...
				<td class="forminp forminp-text" style="width: 50%; float: left;">
					<fieldset>
						<p class="form-field apply_points_until_field">
							<label for="apply_points_until_field"><?php printf( __( '%sUp Until%s - %sOptional%s: Leave blank to apply to all orders', 'ultimatewoo-pro' ), '<strong>', '</strong>', '<em>', '</em>' ); ?></label>
							<input type="text" class="date-picker" name="apply_points_until" id="apply_points_until" value="" placeholder="<?php echo _x( 'YYYY-MM-DD', 'placeholder', 'ultimatewoo-pro' ); ?>" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" />
						</p>
					</fieldset>
				</td>
			</tr>
			...
			if ( 'apply_points' == $current_action ) {
				...
				do {
					...
					if ( is_wp_error( $order_ids ) ) {
						$wc_points_rewards->admin_message_handler->add_error( __( 'Database error while applying user points.', 'ultimatewoo-pro' ) );
				...
				// success message
				$wc_points_rewards->admin_message_handler->add_message( sprintf( _n( '%d order updated.', '%s orders updated.', $success_count, 'ultimatewoo-pro' ), $success_count ) );
	...
	public function render_points_earned_redeemed_info( $order_id ) {
		...
		?>
			<h4><?php _e( 'Points', 'ultimatewoo-pro' ); ?></h4>
			<ul class="totals">
				<li class="left">
					<label><?php _e( 'Earned:', 'ultimatewoo-pro' ); ?></label>
					<input type="number" disabled="disabled" id="_wc_points_earned" name="_wc_points_earned" placeholder="<?php _e( 'None', 'ultimatewoo-pro' ); ?>" value="<?php if ( ! empty( $points_earned ) ) echo esc_attr( $points_earned ); ?>" class="first" />
				</li>
				<li class="right">
					<label><?php _e( 'Redeemed:', 'ultimatewoo-pro' ); ?></label>
					<input type="number" disabled="disabled" id="_wc_points_redeemed" name="_wc_points_redeemed" placeholder="<?php _e( 'None', 'ultimatewoo-pro' ); ?>" value="<?php if ( ! empty( $points_redeemed ) ) echo esc_attr( $points_redeemed ); ?>" class="first" />
				</li>
			</ul>
			<div class="clear"></div>
		<?php
	...
	public function render_coupon_points_modifier_field() {
		...
		woocommerce_wp_text_input(
			array(
				...
				'label'       => __( 'Points Modifier', 'ultimatewoo-pro' ),
				'description' => __( 'Enter a percentage which modifies how points are earned when this coupon is applied. For example, enter 200% to double the amount of points typically earned when the coupon is applied.', 'ultimatewoo-pro' ),
				'desc_tip'    => true,
			)
		);
	}
	...
	public function woocommerce_debug_tools( $tools ) {
		$tools['reset_points_rewards'] = array(
			'name'     => __( 'Reset Points and Rewards', 'ultimatewoo-pro' ),
			'button'   => __( 'Delete all points and rewards and clear the logs.', 'ultimatewoo-pro' ),
			'desc'     => __( '<strong class="red">Note:</strong> This action will remove all customer points and cannot be undone.', 'ultimatewoo-pro' ),
	...
	public function reset_points_rewards() {
		...
		echo '<div class="updated"><p>' . __( 'All points and rewards successfully deleted', 'ultimatewoo-pro' ) . '</p></div>';
	}
