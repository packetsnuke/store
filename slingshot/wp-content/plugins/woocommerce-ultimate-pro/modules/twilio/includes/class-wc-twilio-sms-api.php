...
class WC_Twilio_SMS_API {
	...
	public function send( $to, $body, $country_code = null ) {
		...
		if ( ! $to || ! $body ) {
			throw new Exception( __( 'Send SMS: To number / Body is blank!', 'ultimatewoo-pro' ) );
		}
	...
	private function http_request( $method ) {
		...
		if ( ! $this->endpoint || ! $method ) {
			throw new Exception( __( 'Endpoint and / or HTTP Method is blank.', 'ultimatewoo-pro' ) );
		}

		// Check that method is a valid http method
		if ( ! in_array( $method, array( 'GET', 'POST', 'PUT', 'DELETE' ) ) ) {
			throw new Exception( __( 'Requested HTTP Method is invalid.', 'ultimatewoo-pro' ) );
		}
		...
		if ( ! isset( $this->response['response'] ) ) {
			throw new Exception( __( 'Empty Response', 'ultimatewoo-pro' ) );
		}

		if ( ! isset( $this->response['body'] ) ) {
			throw new Exception( __( 'Empty Body', 'ultimatewoo-pro' ) );
		}
	}
	...
	private function parse_response() {
		...
		if ( isset( $this->response['response']['code'] ) ) {
			...
			if ( $this->response['response']['code'] != 200 && $this->response['response']['code'] != 201 ) {
				...
				throw new Exception(
					sprintf(
						__( '%1$s See: %2$s', 'ultimatewoo-pro' ),
						( isset( $this->response['message'] ) ) ? $this->response['message'] : __( 'No Message Available', 'ultimatewoo-pro' ),
						( isset( $this->response['more_info'] ) ) ? $this->response['more_info'] : __( 'N/A', 'ultimatewoo-pro' )
					)
				);
			}
			...
			throw new Exception( __( 'Response HTTP Code Not Set', 'ultimatewoo-pro' ) );
		}
	}
