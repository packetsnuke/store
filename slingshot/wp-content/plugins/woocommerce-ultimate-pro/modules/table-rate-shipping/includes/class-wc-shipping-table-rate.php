...
class WC_Shipping_Table_Rate extends WC_Shipping_Method {
	...
	public function __construct( $instance_id = 0 ) {
		...
		$this->method_title       = __( 'Table rates', 'ultimatewoo-pro' );
		$this->method_description = __( 'Table rates are dynamic rates based on a number of cart conditions.', 'ultimatewoo-pro' );
		...
		// Get settings
		$this->enabled            = 'yes';
		$this->title              = $this->get_option( 'title', __( 'Table Rate', 'ultimatewoo-pro' ) );
	...
	public function init_form_fields() {
		...
		$this->instance_form_fields = array(
			'title' => array(
				...
				'default'     => __( 'Table Rate', 'ultimatewoo-pro' )
			),
			'tax_status' => array(
				'title'       => __( 'Tax Status', 'ultimatewoo-pro' ),
				'type'        => 'select',
				'description' => '',
				'desc_tip'    => true,
				'default'     => 'taxable',
				'options'     => array(
					'taxable' => __('Taxable', 'ultimatewoo-pro'),
					'none'    => __('None', 'ultimatewoo-pro')
				)
			),
			'order_handling_fee' => array(
				'title'       => __( 'Handling Fee', 'ultimatewoo-pro' ),
				'type'        => 'text',
			...
			'max_shipping_cost' => array(
				'title'       => __( 'Maximum Shipping Cost', 'ultimatewoo-pro' ),
				'type'        => 'text',
			...
			'rates' => array(
				'title'       => __( 'Rates', 'ultimatewoo-pro' ),
				'type'        => 'title',
				'description' => __( 'This is where you define your table rates which are applied to an order.', 'ultimatewoo-pro'),
				'default'     => ''
			),
			'calculation_type' => array(
				'title'       => __( 'Calculation Type', 'ultimatewoo-pro' ),
				'type'        => 'select',
				'description' => __( 'Per order rates will offer the customer all matching rates. Calculated rates will sum all matching rates and provide a single total.', 'ultimatewoo-pro' ),
				'default'     => '',
				'desc_tip'    => true,
				'options'     => array(
					''      => __( 'Per order', 'ultimatewoo-pro' ),
					'item'  => __( 'Calculated rates per item', 'ultimatewoo-pro' ),
					'line'  => __( 'Calculated rates per line item', 'ultimatewoo-pro' ),
					'class' => __( 'Calculated rates per shipping class', 'ultimatewoo-pro' )
				)
			),
			'handling_fee' => array(
				'title'       => __( 'Handling Fee Per [item]', 'ultimatewoo-pro' ),
				'type'        => 'text',
			...
			'min_cost' => array(
				'title'       => __( 'Minimum Cost Per [item]', 'ultimatewoo-pro' ),
				'type'        => 'text',
				'desc_tip'    => true,
				'description' => __('Minimum cost for this shipping method (optional). If the cost is lower, this minimum cost will be enforced.', 'ultimatewoo-pro'),
				'default'     => '',
				'placeholder' => __( 'n/a', 'ultimatewoo-pro' )
			),
			'max_cost' => array(
				'title'       => __( 'Maximum Cost Per [item]', 'ultimatewoo-pro' ),
				'type'        => 'text',
				'desc_tip'    => true,
				'description' => __( 'Maximum cost for this shipping method (optional). If the cost is higher, this maximum cost will be enforced.', 'ultimatewoo-pro'),
				'default'     => '',
				'placeholder' => __( 'n/a', 'ultimatewoo-pro' )
			),
		);
	...
	public function instance_options() {
		...
		<table class="form-table">
			...
			?>
			<tr>
				<th><?php _e( 'Table Rates', 'ultimatewoo-pro' ); ?></th>
				<td>
			...
			<?php if ( sizeof( WC()->shipping->get_shipping_classes() ) ) : ?>
				<tr valign="top" id="shipping_class_priorities">
					<th scope="row" class="titledesc"><?php _e( 'Class Priorities', 'ultimatewoo-pro' ); ?></th>
	...
	public function get_rates( $package ) {
		...
		if ( $this->calculation_type == 'item' ) {
			...
			if ( $matched ) {
				...
				if ( $this->max_shipping_cost && ( $costs['order'] + array_sum( $costs ) ) > $this->max_shipping_cost ) {
					$rates[] = array(
						...
						'label' => __( $this->title, 'ultimatewoo-pro' ),
			...
			if ( $matched ) {
				$rates[] = array(
					'id'      => is_callable( array( $this, 'get_rate_id' ) ) ? $this->get_rate_id() : $this->instance_id,
					'label'   => __( $rate_label, 'ultimatewoo-pro' ),
					'cost'    => $total_cost,
					'package' => $package,
				);
			}
