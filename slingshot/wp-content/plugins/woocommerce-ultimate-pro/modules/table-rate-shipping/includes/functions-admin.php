		...
		<thead>
			<tr>
				...
				<?php if ( sizeof( $shipping_classes ) ) : ?>
					<th><?php _e('Shipping Class', 'ultimatewoo-pro'); ?>&nbsp;<a class="tips" data-tip="<?php _e('Shipping class this rate applies to.', 'ultimatewoo-pro'); ?>">[?]</a></th>
				<?php endif; ?>
				<th><?php _e('Condition', 'ultimatewoo-pro'); ?>&nbsp;<a class="tips" data-tip="<?php _e('Condition vs. destination', 'ultimatewoo-pro'); ?>">[?]</a></th>
				<th><?php _e('Min&ndash;Max', 'ultimatewoo-pro'); ?>&nbsp;<a class="tips" data-tip="<?php _e('Bottom and top range for the selected condition. ', 'ultimatewoo-pro'); ?>">[?]</a></th>
				<th width="1%" class="checkbox"><?php _e('Break', 'ultimatewoo-pro'); ?>&nbsp;<a class="tips" data-tip="<?php _e('Break at this point. For per-order rates, no rates other than this will be offered. For calculated rates, this will stop any further rates being matched.', 'ultimatewoo-pro'); ?>">[?]</a></th>
				<th width="1%" class="checkbox"><?php _e('Abort', 'ultimatewoo-pro'); ?>&nbsp;<a class="tips" data-tip="<?php _e('Enable this option to disable all rates/this shipping method if this row matches any item/line/class being quoted.', 'ultimatewoo-pro'); ?>">[?]</a></th>
				<th class="cost"><?php _e('Row cost', 'ultimatewoo-pro'); ?>&nbsp;<a class="tips" data-tip="<?php _e('Cost for shipping the order, excluding tax.', 'ultimatewoo-pro'); ?>">[?]</a></th>
				<th class="cost cost_per_item"><?php _e('Item cost', 'ultimatewoo-pro'); ?>&nbsp;<a class="tips" data-tip="<?php _e('Cost per item, excluding tax.', 'ultimatewoo-pro'); ?>">[?]</a></th>
				<th class="cost cost_per_weight"><?php echo get_option('woocommerce_weight_unit') . ' ' . __('cost', 'ultimatewoo-pro'); ?>&nbsp;<a class="tips" data-tip="<?php _e('Cost per weight unit, excluding tax.', 'ultimatewoo-pro'); ?>">[?]</a></th>
				<th class="cost cost_percent"><?php echo __('% cost', 'ultimatewoo-pro'); ?>&nbsp;<a class="tips" data-tip="<?php _e('Percentage of total to charge.', 'ultimatewoo-pro'); ?>">[?]</a></th>
				<th class="shipping_label"><?php _e('Label', 'ultimatewoo-pro'); ?>&nbsp;<a class="tips" data-tip="<?php _e('Label for the shipping method which the user will be presented. ', 'ultimatewoo-pro'); ?>">[?]</a></th>
			</tr>
		</thead>
		<tfoot>
			<tr>
				<th colspan="2"><a href="#" class="add-rate button button-primary"><?php _e('Add Shipping Rate', 'ultimatewoo-pro'); ?></a></th>
				<th colspan="9"><span class="description"><?php _e('Define your table rates here in order of priority.', 'ultimatewoo-pro'); ?></span> <a href="#" class="dupe button"><?php _e('Duplicate selected rows', 'ultimatewoo-pro'); ?></a> <a href="#" class="remove button"><?php _e('Delete selected rows', 'ultimatewoo-pro'); ?></a></th>
			</tr>
		</tfoot>
		...
		<tr class="table_rate">
			...
			<?php if ( sizeof( $shipping_classes ) ) : ?>
				<td>
					<select class="select" name="shipping_class[{{{ data.index }}}]" style="min-width:100px;">
						<option value="" <# if ( "" === data.rate.rate_class ) { #>selected="selected"<# } #>><?php _e( 'Any class', 'ultimatewoo-pro' ); ?></option>
						<option value="0" <# if ( "0" === data.rate.rate_class ) { #>selected="selected"<# } #>><?php _e( 'No class', 'ultimatewoo-pro' ); ?></option>
			...
			<td>
				<select class="select" name="shipping_condition[{{{ data.index }}}]" style="min-width:100px;">
					<option value="" <# if ( "" === data.rate.rate_condition ) { #>selected="selected"<# } #>><?php _e( 'None', 'ultimatewoo-pro' ); ?></option>
					<option value="price" <# if ( "price" === data.rate.rate_condition ) { #>selected="selected"<# } #>><?php _e( 'Price', 'ultimatewoo-pro'); ?></option>
					<option value="weight" <# if ( "weight" === data.rate.rate_condition ) { #>selected="selected"<# } #>><?php _e( 'Weight', 'ultimatewoo-pro'); ?></option>
					<option value="items" <# if ( "items" === data.rate.rate_condition ) { #>selected="selected"<# } #>><?php _e( 'Item count', 'ultimatewoo-pro'); ?></option>
					<?php if ( sizeof( $shipping_classes ) ) : ?>
						<option value="items_in_class" <# if ( "items_in_class" === data.rate.rate_condition ) { #>selected="selected"<# } #>><?php _e( 'Item count (same class)', 'ultimatewoo-pro' ); ?></option>
					<?php endif; ?>
				</select>
			</td>
			<td class="minmax">
				<input type="text" class="text" value="{{{ data.rate.rate_min }}}" name="shipping_min[{{{ data.index }}}]" placeholder="<?php _e( 'n/a', 'ultimatewoo-pro' ); ?>" size="4" /><input type="text" class="text" value="{{{ data.rate.rate_max }}}" name="shipping_max[{{{ data.index }}}]" placeholder="<?php _e( 'n/a', 'ultimatewoo-pro' ); ?>" size="4" />
			</td>
			...
			<td colspan="4" class="abort_reason">
				<input type="text" class="text" value="{{{ data.rate.rate_abort_reason }}}" placeholder="<?php _e( 'Optional abort reason text', 'ultimatewoo-pro' ); ?>" name="shipping_abort_reason[{{{ data.index }}}]" />
			</td>
			<td class="cost">
				<input type="text" class="text" value="{{{ data.rate.rate_cost }}}" name="shipping_cost[{{{ data.index }}}]" placeholder="<?php _e( '0', 'ultimatewoo-pro' ); ?>" size="4" />
			</td>
			<td class="cost cost_per_item">
				<input type="text" class="text" value="{{{ data.rate.rate_cost_per_item }}}" name="shipping_per_item[{{{ data.index }}}]" placeholder="<?php _e( '0', 'ultimatewoo-pro' ); ?>" size="4" />
			</td>
			<td class="cost cost_per_weight">
				<input type="text" class="text" value="{{{ data.rate.rate_cost_per_weight_unit }}}" name="shipping_cost_per_weight[{{{ data.index }}}]" placeholder="<?php _e( '0', 'ultimatewoo-pro' ); ?>" size="4" />
			</td>
			<td class="cost cost_percent">
				<input type="text" class="text" value="{{{ data.rate.rate_cost_percent }}}" name="shipping_cost_percent[{{{ data.index }}}]" placeholder="<?php _e( '0', 'ultimatewoo-pro' ); ?>" size="4" />
			</td>
...
function wc_table_rate_admin_shipping_class_priorities( $shipping_method_id ) {
	...
	if (!$classes) :
		echo '<p class="description">' . __( 'No shipping classes exist - you can ignore this option :)', 'ultimatewoo-pro' ) . '</p>';
	else :
		...
		<table class="widefat shippingrows" style="position:relative;">
			<thead>
				<tr>
					<th><?php _e('Class', 'ultimatewoo-pro'); ?></th>
			...
			<tfoot>
				<tr>
					<td colspan="2">
						<span class="description per_order"><?php _e('When calculating shipping, the cart contents will be <strong>searched for all shipping classes</strong>. If all product shipping classes are <strong>identical</strong>, the corresponding class will be used.<br/><strong>If there are a mix of classes</strong> then the class with the <strong>lowest number priority</strong> (defined above) will be used.', 'ultimatewoo-pro'); ?></span>
					</td>
				</tr>
			</tfoot>
			<tbody>
				<tr>
					<th><?php _e('Default', 'ultimatewoo-pro'); ?></th>
...
function wc_table_rate_admin_shipping_rows_process( $shipping_method_id ) {
	...
	for ( $i = 0; $i <= $max_key; $i++ ) {
		...
		$rate_cost                 = isset( $shipping_cost[ $i ] ) ? rtrim( rtrim( number_format( (double) $shipping_cost[ $i ], 4, $decimal_separator, '' ), '0' ), $decimal_separator ) : '';
		$rate_cost_per_item        = isset( $shipping_per_item[ $i ] ) ? rtrim( rtrim( number_format( (double) $shipping_per_item[ $i ], 4, $decimal_separator, '' ), '0' ), $decimal_separator ) : '';
		$rate_cost_per_weight_unit = isset( $shipping_cost_per_weight[ $i ] ) ? rtrim( rtrim( number_format( (double) $shipping_cost_per_weight[ $i ], 4, $decimal_separator, '' ), '0' ), $decimal_separator ) : '';
		$rate_cost_percent         = isset( $cost_percent[ $i ] ) ? rtrim( rtrim( number_format( (double) str_replace( '%', '', $cost_percent[ $i ] ), 2, $decimal_separator, '' ), '0' ), $decimal_separator ) : '';
		...
		switch ( $rate_condition ) {
			case 'weight' :
			case 'price' :
				if ( $rate_min ) $rate_min = number_format( $rate_min, 2, $decimal_separator, '' );
				if ( $rate_max ) $rate_max = number_format( $rate_max, 2, $decimal_separator, '' );
			break;
			case 'items' :
			case 'items_in_class' :
