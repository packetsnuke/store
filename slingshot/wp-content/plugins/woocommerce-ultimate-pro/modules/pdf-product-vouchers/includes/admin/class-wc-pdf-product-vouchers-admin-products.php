...
class WC_PDF_Product_Vouchers_Admin_Products {
		...
		if ( 'yes' === $has_voucher && isset( $_POST['_voucher_template_id'] ) ) {
			...
			if ( $voucher_template ) {
				...
				if ( 'taxable' !== $product->get_tax_status() && 'single' === $voucher_template->get_voucher_type() ) {
					wc_pdf_product_vouchers()->get_message_handler()->add_message( sprintf( '%s %s', __( 'Quick note: This product has a single-purpose voucher, so it may need to be taxable.', 'ultimatewoo-pro' ), '<a href="https://docs.woocommerce.com/document/woocommerce-pdf-product-vouchers/#types-and-taxes">' . __( 'View Documentation', 'ultimatewoo-pro' ) . '</a>' ) );
				} elseif ( 'taxable' === $product->get_tax_status() && 'multi' === $voucher_template->get_voucher_type() ) {
					wc_pdf_product_vouchers()->get_message_handler()->add_message( sprintf( '%s %s', __( 'Quick note: This product has a multi-purpose voucher, so it may need to be non-taxable.', 'ultimatewoo-pro' ), '<a href="https://docs.woocommerce.com/document/woocommerce-pdf-product-vouchers/#types-and-taxes">' . __( 'View Documentation', 'ultimatewoo-pro' ) . '</a>' ) );
				}
	...
	public function add_voucher_option( $options ) {
		...
		$options['has_voucher'] = array(
			...
			'label'         => __( 'Has Voucher', 'ultimatewoo-pro' ),
			'description'   => __( 'Voucher products give access to a PDF voucher upon purchase.', 'ultimatewoo-pro' ),
			'default'       => 'no',
		);
	...
	public function product_voucher_options() {
		...
		woocommerce_wp_select( array( 'id' => '_voucher_template_id', 'label' => __( 'Voucher Template', 'ultimatewoo-pro' ), 'options' => $options, 'desc_tip' => true, 'description' => __( 'Select a voucher template to make this into a voucher product.', 'ultimatewoo-pro' ) )  );
	...
	public function product_variation_voucher_options( $loop, $variation_data, WP_Post $variation ) {
		...
		<div class="show_if_variation_has_voucher" style="display: none;">
			<p class="form-row form-row-first">
				<label>
					<?php
						esc_html_e( 'Voucher Template:', 'ultimatewoo-pro' );
						echo wc_help_tip( __( 'Select a voucher template to make this variation into a voucher product.', 'ultimatewoo-pro' ) );
					?>
				</label>
	...
	public function add_voucher_product_filter( $output ) {
		...
		$voucher_option .= '> &rarr;  ' . esc_html__( 'Has Voucher', 'ultimatewoo-pro' ) . '</option>';
