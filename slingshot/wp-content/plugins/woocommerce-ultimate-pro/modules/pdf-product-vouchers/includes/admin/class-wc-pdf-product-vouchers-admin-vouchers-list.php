...
class WC_PDF_Product_Vouchers_Admin_Vouchers_List {
	...
	public function edit_voucher_columns( $columns ){
		...
		$status_title = esc_attr__( 'Status', 'ultimatewoo-pro' );

		$columns['cb']              = '<input type="checkbox" />';
		$columns['voucher_status']  = '<span class="status_head tips" data-tip="' . $status_title . '">' . $status_title . '</span>';
		$columns['thumb']           = __( 'Image', 'ultimatewoo-pro' );
		$columns['title']           = __( 'Number', 'ultimatewoo-pro' );
		$columns['voucher_date']    = __( 'Date', 'ultimatewoo-pro' );

		if ( ! in_array( $status, array( 'wcpdf-redeemed', 'wcpdf-voided' ) ) ) {
			$columns['expiration_date'] = __( 'Expires', 'ultimatewoo-pro' );
		}

		if ( ! in_array( $status, array( 'wcpdf-redeemed' ) ) ) {
			$columns['remaining_value'] = __( 'Remaining value', 'ultimatewoo-pro' );
		}

		$columns['voucher_actions'] = __( 'Actions', 'ultimatewoo-pro' );
	...
	public function edit_voucher_row_actions( $actions, $post ) {
		...
		if ( 'wc_voucher' === $post->post_type ) {
			...
			$actions = array( 'id' => sprintf( __( 'ID: %s', 'ultimatewoo-pro' ), $post->ID ) ) + $actions;
		}
	...
	public function custom_voucher_columns( $column ) {
		...
		switch ( $column ) {
			...
			case 'remaining_value':
				...
				echo sprintf( esc_html__( '%1$s of %2$s', 'ultimatewoo-pro' ), $display_remaining, $display_value );

				// show how may uses the voucher still has
				if ( 'single' === $voucher->get_voucher_type() && $voucher->get_product_quantity() > 1 ) {
					echo '<br><small class="remaining-quantity">' . sprintf( esc_html__( '%1$s of %2$s uses', 'ultimatewoo-pro' ), $voucher->get_remaining_value() / $voucher->get_product_price(), $voucher->get_product_quantity() ) . '</small>';
				}
			...
			case 'expiration_date':
				...
				echo $expiration_date ? $expiration_date : esc_html__( 'Never', 'ultimatewoo-pro' );
			break;
					...
					<?php
						...
						if ( $voucher->has_status( 'active' ) ) {
							...
							$actions['redeem'] = array(
								...
								'name'   => __( 'Redeem', 'ultimatewoo-pro' ),
								'action' => 'redeem',
							);
							...
							$actions['void'] = array(
								...
								'name'   => _x( 'Void', 'verb', 'ultimatewoo-pro' ),
								'action' => 'void',
							);
						}
						...
						if ( $voucher->file_exists() ) {
							...
							$actions['download'] = array(
								...
								'name'   => _x( 'Download', 'verb', 'ultimatewoo-pro' ),
								'action' => 'download',
							);
						}
						...
						$actions['view'] = array(
							...
							'name'   => _x( 'View', 'verb', 'ultimatewoo-pro' ),
							'action' => 'view',
						);
