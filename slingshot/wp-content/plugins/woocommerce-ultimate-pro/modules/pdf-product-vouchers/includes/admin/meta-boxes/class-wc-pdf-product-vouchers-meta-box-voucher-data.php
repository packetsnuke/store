...
class WC_PDF_Product_Vouchers_Meta_Box_Voucher_Data {
	...
	public function add_meta_box() {
		add_meta_box( 'wc-pdf-product-vouchers-voucher-data', __( 'Voucher Data', 'ultimatewoo-pro' ), array( $this, 'output' ), 'wc_voucher', 'normal', 'high' );
	}
	...
	public function save( $post_id, WP_Post $post ) {
		...
		if ( $customer_changed && $voucher->is_editable() && ! $voucher->has_redemptions() && ! $voucher->get_order_id() ) {
			...
			if ( $new_tax != $voucher->get_product_tax() && 'calculate_product_tax' != $action ) {
				wc_pdf_product_vouchers()->get_message_handler()->add_error( __( 'Voucher customer has changed and is taxed differently from previous customer. You may need to recalculate voucher taxes.', 'ultimatewoo-pro' ) );
