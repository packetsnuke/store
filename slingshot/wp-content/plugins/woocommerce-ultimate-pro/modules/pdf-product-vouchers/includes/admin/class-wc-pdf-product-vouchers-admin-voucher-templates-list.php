...
class WC_PDF_Product_Vouchers_Admin_Voucher_Templates_List {
	...
	public function edit_voucher_template_columns( $columns ){
		...
		$columns['days_to_expiry'] = __( 'Expiry Days', 'ultimatewoo-pro' );
	...
	public function edit_voucher_template_row_actions( $actions, $post ) {
		...
		if ( 'wc_voucher_template' === $post->post_type ) {
			...
			$duplicate_link_open  = '<a href="' . wp_nonce_url( admin_url( 'edit.php?post_type=wc_voucher_template&action=duplicate_voucher_template&amp;post=' . $post->ID ), 'wc-pdf-product-vouchers-duplicate-voucher-template_' . $post->ID ) . '" title="' . __( 'Make a duplicate from this voucher template', 'ultimatewoo-pro' ) . '" rel="permalink">';
			$duplicate_link_close = '</a>';

			// add duplicate plan action
			$actions['duplicate'] = $duplicate_link_open . esc_html_x( 'Duplicate', 'Duplicate a Voucher Template', 'ultimatewoo-pro' ) . $duplicate_link_close;
	...
	public function duplicate_voucher_template() {
		...
		if ( $post instanceof WP_Post ) {
			...
			wc_pdf_product_vouchers()->get_message_handler()->add_message( __( 'Voucher template copied.', 'ultimatewoo-pro' ) );
			...
			wp_die( __( 'Voucher template creation failed, could not find original template:', 'ultimatewoo-pro' ) . ' ' . $id );
		}
	}
