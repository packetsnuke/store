...
function wc_pdf_product_vouchers_create_voucher( $args = array() ) {
	...
	if ( (int) $args['user_id'] > 0 ) {
		...
		update_post_meta( $voucher->get_id(), '_customer_user', $args['user_id'] );
	}
  ...
  if ( ! $voucher_type && ! $updating ) {
    ...
    $voucher_type = is_callable( array( $voucher->get_template(), 'get_voucher_type' ) ) ? $voucher->get_template()->get_voucher_type() : 'single';
  }
