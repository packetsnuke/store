...
class WC_PDF_Product_Vouchers_Customizer {
	...
	public function add_customizer_controls( $wp_customize ) {
		...
		$wp_customize->add_control( 'wc_voucher_template_post_title', array(
			'type'        => 'text',
			'label'       => __( 'Template Name', 'ultimatewoo-pro' ),
			'section'     => 'wc_voucher_template_general',
			'input_attrs' => array(
				'placeholder' => __( 'Voucher Template Name', 'ultimatewoo-pro' ),
			),
		) );
		...
		$wp_customize->add_control( 'wc_voucher_template_voucher_type', array(
			'type'    => 'select',
			'label'   => __( 'Voucher type', 'ultimatewoo-pro' ),
			'section' => 'wc_voucher_template_general',
			'choices' => array(
				'single' => __( 'Single purpose (for a particular product / service)', 'ultimatewoo-pro' ),
				'multi'  => __( 'Multi-purpose (can be redeemed on any product / service)', 'ultimatewoo-pro' ),
			),
		) );
		...
		$wp_customize->add_control( new WC_PDF_Product_Vouchers_Customize_Voucher_Image_Control( $wp_customize, 'wc_pdf_product_vouchers_voucher_image', array(
			'label'         => __( 'Voucher Image', 'ultimatewoo-pro' ),
		...
		$wp_customize->add_control( 'wc_pdf_product_vouchers_voucher_image_dpi', array(
			'type'        => 'number',
			'label'       => __( 'Image DPI', 'ultimatewoo-pro' ),
			'description' => __( 'Enter the DPI or resolution at which the voucher background was created. This affects the resolution at which voucher fields are added, so this value should match the image file.', 'ultimatewoo-pro' ),
			'section'     => 'wc_voucher_template_voucher_images',
		...
		$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'wc_voucher_template_voucher_additional_image', array(
			'label'         => __( 'Additional Image', 'ultimatewoo-pro' ),
			'section'       => 'wc_voucher_template_voucher_images',
			'description'   => __( 'Optional image with the same dimensions as the primary voucher image, that will be added as a second page to the voucher. Useful for terms or redemption instructions.', 'ultimatewoo-pro' ),
			'button_labels' => array(
				...
				'frame_title'  => __( 'Select additional voucher image', 'ultimatewoo-pro' ),
	...
	public function customizer_controls_scripts() {
		...
		wp_localize_script( 'woocommerce-pdf-product-vouchers-customizer-control-scripts', 'woocommerce_vouchers_admin_customizer', array(
			'i18n' => array(
				'default'                => __( '(Default)', 'ultimatewoo-pro' ),
				'untitled_template'      => __( '(Untitled Voucher Template)', 'ultimatewoo-pro' ),
				'customizer_description' => __( 'The Customizer allows you to adjust your PDF voucher template with a live preview. No changes are permanent until you click Save.', 'ultimatewoo-pro' ),
			),
	...
	public function get_currently_previewed_voucher_template_name() {
		...
		if ( ! $title ) {
			$title = __( '(Untitled Voucher Template)', 'ultimatewoo-pro' );
		}
	...
	public function validate_voucher_template_post_title( $validity, $value ) {
		...
		if ( ! $value ) {
			$validity->add( 'required', __( 'Please set a name for the template.', 'ultimatewoo-pro' ) );
		}
