...
class WC_PDF_Product_Vouchers_Upgrade {
					...
					if ( ! empty( $item['voucher_id'] ) && ! isset( $item['wc_pdf_product_vouchers_is_converted'] ) ) {
						...
						if ( is_readable( $legacy_path ) ) {
							...
							if ( ! is_writable( $voucher_pdf_path ) ) {
								...
								wc_pdf_product_vouchers()->log( sprintf( __( 'Voucher path %s is not writable', 'ultimatewoo-pro' ), $voucher_pdf_path ) );
								continue;
							}
	...
	private static function create_files() {
		...
		$default_images = array(
			array(
				...
				'title'  => __( 'Voucher Group 1: Modern', 'ultimatewoo-pro' ),
				'id'     => 'group-1-modern',
				'styles' => $group1_styles,
			),
			array(
				'path'   => $plugin_path . '/assets/images/formal.png',
				'title'  => __( 'Voucher Group 1: Formal', 'ultimatewoo-pro' ),
				'id'     => 'group-1-formal',
				'styles' => $group1_styles,
			),
			array(
				'path'   => $plugin_path . '/assets/images/xmas.png',
				'title'  => __( 'Voucher Group 1: Christmas', 'ultimatewoo-pro' ),
			...
			array(
				...
				'title'  => __( 'Voucher Group 2: Celebration', 'ultimatewoo-pro' ),
				'id'     => 'group-2-celebration',
				'styles' => $group2_styles,
			),
			array(
				'path'   => $plugin_path . '/assets/images/childrens.png',
				'title'  => __( 'Voucher Group 2: Children', 'ultimatewoo-pro' ),
				'id'     => 'group-2-children',
				'styles' => $group2_styles,
			),
			array(
				'path'   => $plugin_path . '/assets/images/feminine.png',
				'title'  => __( 'Voucher Group 2: Feminine', 'ultimatewoo-pro' ),
				'id'     => 'group-2-feminine',
				'styles' => $group2_styles,
			),
		);
	...
	private static function create_voucher_image_attachment( $attrs ) {
		...
		if ( ! $attachment_id || is_wp_error( $attachment_id ) ) {
			...
			$message = sprintf( __( 'Could not install default voucher image %1$s: %2$s', 'ultimatewoo-pro' ), $filename, $attachemnt_id->get_error_message() );
