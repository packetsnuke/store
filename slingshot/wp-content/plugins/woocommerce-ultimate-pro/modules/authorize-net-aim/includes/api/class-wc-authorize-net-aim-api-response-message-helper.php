	...
	public function get_user_message( $message_id ) {
		...
		switch ( $message_id ) {
			...
			case 'authorize_net_error_try_later':
				$message = __( 'Oops, sorry! A temporary error occurred. Please try again in 5 minutes.', 'ultimatewoo-pro' );
				break;

			case 'authorize_net_authorized_but_not_settled':
				$message = __( 'This transaction was authorized successfully, but could not be settled. Please contact us.', 'ultimatewoo-pro' );
				break;

			case 'authorize_net_echeck_mismatch':
				$message = __( 'The name and/or bank account type does not match. Please re-enter and try again.', 'ultimatewoo-pro' );
				break;
