	...
	public function create_refund( WC_Order $order ) {
		...
		$this->request_data = array(
			...
			'transactionRequest' => array(
				...
				'order'           => array(
					'invoiceNumber' => ltrim( $this->order->get_order_number(), _x( '#', 'hash before the order number', 'ultimatewoo-pro' ) ),
	...
	private function get_line_items() {
		...
		foreach ( $this->order->get_fees() as $fee_id => $fee ) {
			...
			if ( $this->order->get_item_total( $fee ) >= 0 ) {
				...
				$line_items[] = array(
					'itemId'      => $fee_id,
					'name'        => ! empty( $fee['name'] ) ? SV_WC_Helper::str_truncate( htmlentities( $fee['name'], ENT_QUOTES, 'UTF-8', false ), 31 ) : __( 'Fee', 'ultimatewoo-pro' ),
					'description' => __( 'Order Fee', 'ultimatewoo-pro' ),
					'quantity'    => 1,
	...
	private function get_taxes() {
		...
		if ( $this->order->get_total_tax() > 0 ) {
			...
			return array(
				...
				'name'        => __( 'Order Taxes', 'ultimatewoo-pro' ),
	...
	private function get_shipping() {
		...
		if ( $this->order->get_total_shipping() > 0 ) {
			...
			return array(
				...
				'name'        => __( 'Order Shipping', 'ultimatewoo-pro' ),
