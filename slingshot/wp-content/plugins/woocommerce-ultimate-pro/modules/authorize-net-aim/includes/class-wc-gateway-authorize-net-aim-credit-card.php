...
class WC_Gateway_Authorize_Net_AIM_Credit_Card extends WC_Gateway_Authorize_Net_AIM {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			array(
				'method_title'       => __( 'Authorize.Net AIM', 'ultimatewoo-pro' ),
				'method_description' => __( 'Allow customers to securely pay using their credit cards with Authorize.Net AIM.', 'ultimatewoo-pro' ),
				'supports'           => array(
				...
				'environments'       => array( 'production' => __( 'Production', 'ultimatewoo-pro' ), 'test' => __( 'Test', 'ultimatewoo-pro' ) ),
	...
	protected function get_method_form_fields() {
		...
		$fields = array_merge( parent::get_method_form_fields(), array(
			...
			'accept_js_enabled' => array(
				'title'       => __( 'Accept.js', 'ultimatewoo-pro' ),
				'type'        => 'checkbox',
				'class'       => 'environment-field production-field accept-js-toggle',
				'label'       => __( 'Enable Accept.js to minimize PCI compliance and send credit card details directly to Authorize.Net', 'ultimatewoo-pro' ),
				/** translators: Placeholders: %1$s - <a> tag, %2$s = </a> tag **/
				'description' => sprintf( __( 'You must obtain a Client Key to use Accept.js at checkout. %1$sLearn more &raquo;%2$s', 'ultimatewoo-pro' ), '<a href="https://docs.woocommerce.com/document/authorize-net-aim/#accept-js-support" target="_blank">', '</a>' ),
				'default'     => 'no',
			),
			'client_key' => array(
				'title' => __( 'Client Key', 'ultimatewoo-pro' ),
	...
	protected function get_gateway_js_localized_script_params() {
		...
		$params = array(
			...
			'general_error'     => __( 'An error occurred, please try again or try an alternate form of payment.', 'ultimatewoo-pro' ),
	...
	protected function mark_order_as_held_for_fraud( WC_Order $order, WC_Authorize_Net_AIM_API_Response $response ) {
		...
		$message = sprintf(
			...
			__( '%1$sPossible fraud detected based on your Authorize.Net Fraud Filter configuration.%2$s Please review the transaction from your merchant account before processing.', 'ultimatewoo-pro' ),
			'<strong>', '</strong>'
		);

		if ( $response->get_avs_result() ) {
			/* translators: Placeholders: %s - an AVS result code, such as "A" */
			$message .= '<br />' . sprintf( __( 'AVS Result: %s', 'ultimatewoo-pro' ), $response->get_avs_result() );
		}

		if ( $response->get_csc_result() ) {
			/* translators: Placeholders: %s - a CSC result code, such as "N" */
			$message .= '<br />' . sprintf( __( 'CSC Result: %s', 'ultimatewoo-pro' ), $response->get_csc_result() );
		}
	...
	protected function get_order_for_refund( $order, $amount, $reason ) {
		...
		if ( ! $order->refund->account_four ) {
			return new WP_Error( 'wc_' . $this->get_id() . '_refund_error', sprintf( __( '%s Refund error - order is missing credit card last four.', 'ultimatewoo-pro' ), $this->get_method_title() ) );
		}
