...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.8.3', __( 'WooCommerce Authorize.Net AIM Gateway', 'ultimatewoo-pro' ), __FILE__, 'init_woocommerce_gateway_authorize_net_aim', array(
...
class WC_Authorize_Net_AIM extends SV_WC_Payment_Gateway_Plugin {
	...
	public function plugin_action_links( $actions ) {
		...
		$title  = $this->is_emulation_enabled()
			? esc_html__( 'Disable Emulation Gateway', 'ultimatewoo-pro' )
			: esc_html__( 'Enable Emulation Gateway', 'ultimatewoo-pro' );
	...
	public function get_settings_link( $gateway_id = null ) {
		...
		switch ( $gateway_id ) {
			...
			case self::EMULATION_GATEWAY_ID:
				$label = __( 'Configure Emulator', 'ultimatewoo-pro' );
			break;
	...
	public function add_admin_notices() {
		...
		if ( $credit_card_gateway->is_enabled() && $credit_card_gateway->is_accept_js_enabled() && isset( $_GET['page'] ) && 'wc-settings' === $_GET['page'] ) {
			...
			if ( ! $credit_card_gateway->get_client_key() ) {
				$message = sprintf( __( "%s: A valid Client Key is required to use Accept.js at checkout.", 'ultimatewoo-pro' ), '<strong>' . $this->get_plugin_name() . '</strong>' );
			} elseif ( ! wc_checkout_is_https() ) {
				$message = sprintf( __( "%s: SSL is required to use Accept.js at checkout.", 'ultimatewoo-pro' ), '<strong>' . $this->get_plugin_name() . '</strong>' );
			}
		...
		if ( ! empty( $_GET['wc_authorize_net_aim_emulation'] ) ) {
			...
			$message = ( 'enable' === $_GET['wc_authorize_net_aim_emulation'] )
				? __( 'Authorize.Net AIM Emulation Gateway is now enabled.', 'ultimatewoo-pro' )
				: __( 'Authorize.Net AIM Emulation Gateway is now disabled.', 'ultimatewoo-pro');
	...
	public function get_plugin_name() {
		return __( 'WooCommerce Authorize.Net AIM Gateway', 'ultimatewoo-pro' );
	}
	...
	/**
	 * Returns __FILE__
	 * ULTIMATEWOO fix: change method to public
	 *
	 * @since 3.0
