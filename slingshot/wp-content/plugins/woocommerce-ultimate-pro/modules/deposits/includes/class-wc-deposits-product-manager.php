	...
	public static function get_formatted_deposit_amount( $product_id ) {
		...
		if ( $amount = self::get_deposit_amount_for_display( $product ) ) {
			...
			if ( $product->is_type( 'booking' ) && 'yes' === WC_Deposits_Product_Meta::get_meta( $product_id, '_wc_deposit_multiple_cost_by_booking_persons' ) ) {
				$item = __( 'person', 'ultimatewoo-pro' );
			} else {
				$item = __( 'item', 'ultimatewoo-pro' );
			}

			if ( 'percent' === $type ) {
				return sprintf( __( 'Pay a %1$s deposit per %2$s', 'ultimatewoo-pro' ), '<span class="wc-deposits-amount">' . $amount . '</span>', $item );
			} else {
				return sprintf( __( 'Pay a deposit of %1$s per %2$s', 'ultimatewoo-pro' ), '<span class="wc-deposits-amount">' . $amount . '</span>', $item );
			}
		}
		return '';
	}
