...
class WC_Deposits_Scheduled_Order_Manager {
	...
	public static function schedule_orders_for_plan( $payment_plan, $original_order_id, $item ) {
		...
		if ( $percent_remaining <= 0 ) {
			...
			$original_order->add_order_note( __( 'Error: Unable to schedule orders for product with payment plan. Reason: Already fully paid.', 'ultimatewoo-pro' ) );
		}
	...
	public static function invoice_scheduled_orders() {
		...
		if ( $due_orders ) {
			...
			foreach ( $due_orders as $due_order ) {
				...
				$order->update_status( 'pending-deposit', __( 'Scheduled order ready for payment.', 'ultimatewoo-pro' ) );
