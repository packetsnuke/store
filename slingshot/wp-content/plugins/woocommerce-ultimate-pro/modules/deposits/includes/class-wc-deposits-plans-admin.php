...
class WC_Deposits_Plans_Admin {
	...
	public function styles_and_scripts() {
		...
		wp_localize_script( 'woocommerce-deposits-payment-plans', 'wc_deposits_payment_plans_params', array(
			'i18n_delete_plan' => __( 'Are you sure you want to delete this plan? This action cannot be undone.', 'ultimatewoo-pro' )
		) );
	...
	public function add_menu_item() {
		$page = add_submenu_page( 'edit.php?post_type=product', __( 'Payment Plans', 'ultimatewoo-pro' ), __( 'Payment Plans', 'ultimatewoo-pro' ) , 'manage_woocommerce', 'deposit_payment_plans', array( $this, 'output' ) );
	}
	/**
	...
	public function output() {
		...
		if ( ! empty( $_POST ) ) {
			...
			if ( is_wp_error( $result ) ) {
				...
				echo '<div class="updated success"><p>' . __( 'Plan saved successfully', 'ultimatewoo-pro' ) . '</p></div>';
			}
		}
		...
		if ( ! empty( $_GET['delete_plan'] ) && ! empty( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'delete_plan' ) ) {
			...
			echo '<div class="updated success"><p>' . __( 'Plan deleted successfully', 'ultimatewoo-pro' ) . '</p></div>';
		}
	...
	public function maybe_save_plan() {
		...
		if ( isset( $_POST['plan_name'] ) ) {
			...
			if ( ! isset( $_POST['woocommerce_save_plan_nonce'] ) || ! wp_verify_nonce( $_POST['woocommerce_save_plan_nonce'], 'woocommerce_save_plan' ) ) {
				return new WP_Error( 'error', __( 'Unable to save payment plan - please try again', 'ultimatewoo-pro' ) );
			}

			$plan_id               = $editing;
			$plan_name             = empty( $_POST['plan_name'] ) ? __( 'Payment Plan', 'ultimatewoo-pro' ) : sanitize_text_field( $_POST['plan_name'] );
			...
			if ( ! $plan_id ) {
				return new WP_Error( 'error', __( 'Unable to save payment plan', 'ultimatewoo-pro' ) );
			}
