...
class WC_Deposits_Plan {
	...
	public function get_formatted_schedule( $amount = '' ) {
		...
		if ( $total_years ) {
			$duration[] = sprintf( _n( '%s year', '%s years', $total_years, 'ultimatewoo-pro' ), $total_years );
		}
		if ( $total_months ) {
			$duration[] = sprintf( _n( '%s month', '%s months', $total_months, 'ultimatewoo-pro' ), $total_months );
		}
		if ( $total_days ) {
			$duration[] = sprintf( _n( '%s day', '%s days', $total_days, 'ultimatewoo-pro' ), $total_days );
		}

		if ( $duration ) {
			return sprintf( __( '%1$s payable over %2$s', 'ultimatewoo-pro' ), $amount, implode( ', ', $duration ) );
		} else {
			return sprintf( __( '%1$s total payable', 'ultimatewoo-pro' ), $amount );
		}
	}
}
