...
class WC_Deposits_Plans_Product_Admin {
	...
	public function add_pay_link( $order ) {
		if ( $order->has_status( 'pending-deposit' ) ) : ?>
			<p><?php printf( __( 'An order has been created for you on %s. To pay for this order please use the following link: %s', 'ultimatewoo-pro' ), get_bloginfo( 'name', 'display' ), '<a href="' . esc_url( $order->get_checkout_payment_url() ) . '">' . __( 'pay', 'ultimatewoo-pro' ) . '</a>' ); ?></p>
		<?php endif;
	}
}
