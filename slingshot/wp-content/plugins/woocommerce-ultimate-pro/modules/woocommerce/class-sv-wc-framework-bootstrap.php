	...
	public function load_framework_plugins() {
		...
		foreach ( $this->registered_plugins as $plugin ) {
			...
			if ( ! class_exists( 'SV_WC_Plugin' ) ) {
				// ULTIMATEWOO fix
				require_once( ULTIMATEWOO_MODULES_DIR . '/woocommerce/class-sv-wc-plugin.php' );
			...
			if ( isset( $plugin['args']['is_payment_gateway'] ) && ! class_exists( 'SV_WC_Payment_Gateway' ) ) {
				// ULTIMATEWOO fix
				require_once( ULTIMATEWOO_MODULES_DIR . 'woocommerce/payment-gateway/class-sv-wc-payment-gateway-plugin.php' );
			}
