		...
		<tbody><?php
			...
			foreach ( $bundled_product_attributes as $attribute_name => $options ) {
				...
				?><tr class="attribute-options" data-attribute_label="<?php echo wc_attribute_label( $attribute_name ); ?>">
					<td class="label">
						<label for="<?php echo sanitize_title( $attribute_name ) . '_' . $bundled_item->item_id; ?>"><?php echo wc_attribute_label( $attribute_name ); ?> <abbr class="required" title="<?php _e( 'Required option', 'ultimatewoo-pro' ); ?>">*</abbr></label>
					</td>
					<td class="value"><?php
