...
class WC_PB_Product_Prices {
	...
	public static function filter_get_price_html( $price_html, $product ) {
		...
		if ( $bundled_item ) {
			 ...
			 * @param  WC_Bundled_Item  $bundled_item
			 */
			$price_html = apply_filters( 'woocommerce_bundled_item_price_html', $quantity > 1 ? sprintf( __( '%1$s <span class="bundled_item_price_quantity">/ pc.</span>', 'ultimatewoo-pro' ), $price_html, $quantity ) : $price_html, $price_html, $bundled_item );
		}
