	...
	public static function admin_stock_html( $stock_status, $product ) {
		...
		if ( 'bundle' === $product->get_type() ) {
			if ( $product->is_parent_in_stock() && $product->contains( 'out_of_stock_strict' ) ) {
				...
				$stock_status             = '<mark class="outofstock insufficient_stock">' . __( 'Insufficient stock', 'ultimatewoo-pro' ) . '</mark>';
				...
				if ( ! empty( $insufficient_stock_items ) ) {
					...
					$stock_status .= ' ' . '<a href="#" class="show_insufficient_stock_items closed" title="' . esc_attr ( __( 'View details', 'ultimatewoo-pro' ) ) . '"></a>' . '<div class="insufficient_stock_items" style="display:none;"><ul>' . implode( '', $item_list ) . '</ul></div>';
				}
			}
		}
