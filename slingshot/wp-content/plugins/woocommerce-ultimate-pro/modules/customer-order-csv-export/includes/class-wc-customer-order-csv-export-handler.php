...
class WC_Customer_Order_CSV_Export_Handler {
	...
	public function test_export_via( $method, $export_type = 'orders' ) {
		...
		try {
			...
			if ( ! is_object( $export ) ) {
				...
				throw new SV_WC_Plugin_Exception( sprintf( __( 'Invalid Export Method: %s', 'ultimatewoo-pro' ), $method ) );
			}
			...
			if ( $export->perform_action( $temp_file ) ) {
				return array( __( 'Test was successful!', 'ultimatewoo-pro' ), 'success' );
			} else {
				return array( __( 'Test failed!', 'ultimatewoo-pro' ), 'error' );
			}
			...
			return array( sprintf( __( 'Test failed: %s', 'ultimatewoo-pro' ), $e->getMessage() ), 'error' );
		}
	}
		...
		foreach ( $ids as $order_id ) {
			...
			if ( apply_filters( 'wc_customer_order_csv_export_add_order_note', $add_order_note ) ) {
				...
				switch ( $method ) {
					...
					case 'download':
						$order_note = esc_html__( 'Order exported to CSV and successfully downloaded.', 'ultimatewoo-pro' );
					break;

					case 'ftp':
						$order_note = esc_html__( 'Order exported to CSV and successfully uploaded to server.', 'ultimatewoo-pro' );
					break;

					case 'http_post':
						$order_note = esc_html__( 'Order exported to CSV and successfully POSTed to remote server.', 'ultimatewoo-pro' );
					break;

					case 'email':
						$order_note = esc_html__( 'Order exported to CSV and successfully emailed.', 'ultimatewoo-pro' );
					break;

					default:
						$order_note = esc_html__( 'Order exported to CSV.', 'ultimatewoo-pro' );
					break;
				}
	...
	public function transfer_export( $export_id, $export_method = null ) {
		...
		if ( ! $export ) {
			...
			throw new SV_WC_Plugin_Exception( sprintf( esc_html__( 'Could not find export: %s', 'ultimatewoo-pro' ), $export_id ) );
		}
		...
		if ( ! is_object( $_export_method ) ) {
			...
			throw new SV_WC_Plugin_Exception( sprintf( esc_html__( 'Invalid Export Method: %s', 'ultimatewoo-pro' ), $export_method ) );
		}
	...
	public function start_export( $ids, $args = array() ) {
		...
		if ( ! $filename ) {
			throw new SV_WC_Plugin_Exception( esc_html__( "No filename given for export file, can't export", 'ultimatewoo-pro' ) );
		}
		...
		if ( ! is_writable( $file_path ) || false === $stream ) {
			...
			throw new SV_WC_Plugin_Exception( sprintf( esc_html__( 'Could not open the export file %s for writing', 'ultimatewoo-pro' ), $file_path ) );
		}
	...
	public function export_item( $item, $export ) {
		...
		if ( empty( $export->file_path ) ) {
			wc_customer_order_csv_export()->log( esc_html__( 'No export file path provided, cannot export item.', 'ultimatewoo-pro' ) );
			return;
		}
		...
		if ( false === $stream ) {
			...
			wc_customer_order_csv_export()->log( sprintf( esc_html__( 'Could not open the export file %s for appending.', 'ultimatewoo-pro' ), $export->file_path ) );
			return;
		}
	...
	public function finish_export( $export ) {
		...
		if ( 'download' === $export->method || 'local' === $export->method ) {
			...
			try {
				...
				wc_customer_order_csv_export()->log( sprintf( __( 'Failed to transfer exported file: %s', 'ultimatewoo-pro' ), $e->getMessage() ) );
			}
		}
