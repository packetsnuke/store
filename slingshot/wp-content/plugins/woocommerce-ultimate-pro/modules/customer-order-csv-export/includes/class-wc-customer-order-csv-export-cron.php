...
class WC_Customer_Order_CSV_Export_Cron {
			...
			if ( $this->scheduled_exports_enabled( $export_type ) ) {
				...
				if ( $export_interval ) {
					...
					$schedules[ 'wc_customer_order_csv_export_' . $export_type . '_auto_export_interval' ] = array(
						...
						'display'  => sprintf( _n(  'Every minute', 'Every %d minutes', (int) $export_interval, 'ultimatewoo-pro' ), (int) $export_interval )
					);
				}
			}
		}
	...
	public function auto_export_orders() {
		...
		if ( ! empty( $order_ids ) ) {
			...
			try {
				...
				wc_customer_order_csv_export()->log( sprintf( esc_html__( 'Scheduled orders export failed: %s', 'ultimatewoo-pro' ), $e->getMessage() ) );
			}
		}
	...
	public function auto_export_customers() {
		...
		if ( ! empty( $customers ) ) {
			...
			try {
				...
				// log errors
				/* translators: Placeholders: %s - error message */
				wc_customer_order_csv_export()->log( sprintf( esc_html__( 'Scheduled customers export failed: %s', 'ultimatewoo-pro' ), $e->getMessage() ) );
	...
	public function auto_export_order( $order_id ) {
		...
		try {
			...
			// log errors
			/* translators: Placeholders: %s - error message */
			wc_customer_order_csv_export()->log( sprintf( esc_html__( 'Automatic order export failed: %s', 'ultimatewoo-pro' ), $e->getMessage() ) );
		}
	}
