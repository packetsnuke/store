...
class WC_Customer_Order_CSV_Export_Admin {
	...
	public function load_styles_scripts( $hook_suffix ) {
		...
		if ( $this->is_export_screen() ) {
			...
			wp_localize_script( 'wc-customer-order-csv-export-admin', 'wc_customer_order_csv_export_admin', array(
				'i18n' => array(
					'export_started'           => __( 'Export Started', 'ultimatewoo-pro' ),
					'export_completed'         => __( 'Export Completed', 'ultimatewoo-pro' ),
					'export_failed'            => __( 'Export Failed', 'ultimatewoo-pro' ),
					'export_not_found'         => __( 'Export Not Found', 'ultimatewoo-pro' ),
					'nothing_to_export'        => __( 'Nothing to Export', 'ultimatewoo-pro' ),
					'unexpected_error'         => __( 'Unexpected Error', 'ultimatewoo-pro' ),
					'unexpected_error_message' => sprintf( esc_html__( 'Something unexpected happened while exporting. Your export may or may have not completed. Please check the %1$sExport List%2$s and your site error log for possible clues as to what may have happened.', 'ultimatewoo-pro' ), '<a href="' . admin_url( 'admin.php?page=wc_customer_order_csv_export&tab=export_list' ) . '">', '</a>' ),
					'load_mapping'             => __( 'Load mapping', 'ultimatewoo-pro' ),
					'done'                     => __( 'Done', 'ultimatewoo-pro' ),
					'load'                     => __( 'Load', 'ultimatewoo-pro' ),
					'close'                    => __( 'Close', 'ultimatewoo-pro' ),
					'confirm_export_delete'    => __( 'Are you sure you want to delete this export?', 'ultimatewoo-pro' ),
					'confirm_export_cancel'    => __( 'Are you sure you want to cancel this export?', 'ultimatewoo-pro' ),
					'confirm_export_transfer'  => __( 'Are you sure you want to send/upload this file?', 'ultimatewoo-pro' ),
	...
	private function get_export_finished_message( $export_id ) {
		...
		if ( 'completed' === $export->status ) {
			...
			if ( 'failed' === $export->transfer_status ) {
				...
				$message      = sprintf( __( 'Exported file %1$s is ready! You can download the exported file from the %2$sExport List%3$s.', 'ultimatewoo-pro' ), $filename, '<a href="' . admin_url( 'admin.php?page=wc_customer_order_csv_export&tab=export_list' ) . '">', '</a>' );
	...
	private function get_failure_message( $failure_type, $export, $multiple_failures = false ) {
		...
		$logs_message = sprintf( __( 'Additional details may be found in the CSV Export %1$slogs%2$s.', 'ultimatewoo-pro' ), '<a href="' . admin_url( 'admin.php?page=wc-status&tab=logs' ) . '">', '</a>' );
		...
		if ( 'export' === $failure_type ) {
			...
			if ( $multiple_failures ) {
				...
				$message = __( 'Looks like automatic exports are failing.', 'ultimatewoo-pro' ) . ' ' . $logs_message;
				...
				if ( 'auto' === $export->invocation ) {
					...
					$message = sprintf( __( 'Automatically exporting file %s failed.', 'ultimatewoo-pro' ), $filename );

				} else {

					/* translators: Placeholders: %s - file name */
					$message = sprintf( __( 'Exporting file %s failed.', 'ultimatewoo-pro' ), $filename );
				}
			...
			if ( $multiple_failures ) {
				...
				$message = sprintf( esc_html__( 'Looks like automatic exports are working, but the transfers %1$s are failing. Exported files are available under %2$sExport List%3$s.', 'ultimatewoo-pro' ), $label, '<a href="' . $export_list_url . '">', '</a>' );
				...
				if ( 'auto' === $export->invocation ) {
					...
					$message = sprintf( __( 'File %1$s was automatically exported, but the transfer %2$s failed.', 'ultimatewoo-pro' ), $filename, $label );

				} else {

					/* translators: Placeholders: %1$s - file name, %2$s - export method, such as "via Email" */
					$message = sprintf( __( 'File %1$s was exported, but the transfer %2$s failed.', 'ultimatewoo-pro' ), $filename, $label );
				}

				/* translators: %1$s - opening <a> tag, %2$s - closing </a> tag */
				$message .= ' ' . sprintf( esc_html__( 'Exported file is available under %1$sExport List%2$s.', 'ultimatewoo-pro' ), '<a href="' . $export_list_url . '">', '</a>' );
	...
	public function render_submenu_pages() {
		...
		$this->tabs = array(
			'export'         => __( 'Export', 'ultimatewoo-pro' ),
			'export_list'    => __( 'Export List', 'ultimatewoo-pro' ),
			'settings'       => __( 'Settings', 'ultimatewoo-pro' ),
			'custom_formats' => __( 'Custom Formats', 'ultimatewoo-pro' ),
		);
	...
	private function render_export_page() {
		...
		submit_button( __( 'Export', 'ultimatewoo-pro' ) );
	}
	...
	public function process_export_bulk_actions() {
		...
		if ( 'delete' === $action ) {
			...
			$this->message_handler->add_message( sprintf( _n( '%d exported file deleted.',  '%d exported files deleted.', 'ultimatewoo-pro', $num_deleted ), $num_deleted ) );
	...
	public function add_order_status_column_content( $column ) {
		...
		if ( 'csv_export_status' === $column ) {
			...
			printf( '<mark class="%1$s">%2$s</mark>', $is_exported ? 'csv_exported' : 'csv_not_exported', $is_exported ? esc_html__( 'Exported', 'ultimatewoo-pro' ) : esc_html__( 'Not Exported', 'ultimatewoo-pro' ) );
		}
	}
	...
	public function add_user_export_status_column_content( $output, $column, $user_id ) {
		...
		if ( 'csv_export_status' === $column ) {
			...
			$output = sprintf( '<mark class="%1$s">%2$s</mark>', $is_exported ? 'csv_exported' : 'csv_not_exported', $is_exported ? esc_html__( 'Exported', 'ultimatewoo-pro' ) : esc_html__( 'Not Exported', 'ultimatewoo-pro' ) );
		}
	...
	public function add_order_action( $order ) {
		...
		if ( ! SV_WC_Order_Compatibility::get_meta( $order, '_wc_customer_order_csv_export_is_exported', true ) ) {
			...
			$name   = __( 'Download to CSV', 'ultimatewoo-pro' );
	...
	public function add_order_meta_box_actions( $actions ) {
		...
		$actions['wc_customer_order_csv_export_download'] = __( 'Download to CSV', 'ultimatewoo-pro' );
		...
		if ( $auto_export_method = $this->get_methods_instance()->get_auto_export_method( 'orders' ) ) {
			...
			$actions['wc_customer_order_csv_export_via_auto_export_method'] = sprintf( __( 'Export to CSV %s', 'ultimatewoo-pro' ), $label );
		}
	...
	public function filter_orders_by_export_status() {
		...
		if ( 'shop_order' === $typenow ) {
			...
			$terms = array(
				0 => (object) array( 'count' => $count['not_exported'], 'term' => __( 'Not Exported to CSV', 'ultimatewoo-pro' ) ),
				1 => (object) array( 'count' => $count['exported'],     'term' => __( 'Exported to CSV', 'ultimatewoo-pro' ) )
			);

			?>
			<select name="_shop_order_csv_export_status" id="dropdown_shop_order_csv_export_status">
				<option value=""><?php _e( 'Show all orders', 'ultimatewoo-pro' ); ?></option>
	...
	private function get_bulk_actions( $export_type ) {
		...
		$bulk_actions = array(
			'mark_exported_to_csv'     => __( 'Mark exported to CSV', 'ultimatewoo-pro' ),
			'mark_not_exported_to_csv' => __( 'Mark not exported to CSV', 'ultimatewoo-pro' ),
			'download_to_csv'          => __( 'Download to CSV', 'ultimatewoo-pro' ),
		);
		...
		if ( $auto_export_method = $this->get_methods_instance()->get_auto_export_method( $export_type ) ) {
			...
			/* translators: Placeholders: %s - via [method], full example: Export to CSV via Email */
			$label = sprintf( __( 'Export to CSV %s', 'ultimatewoo-pro' ), $label );
	...
	private function process_bulk_actions( $action, $object_ids, $export_type ) {
		...
		switch ( $action ) {
			...
			case 'mark_exported_to_csv':
				...
				switch ( $export_type ) {
					case 'customers':
						$message = sprintf( _n( '%d customer marked as exported to CSV', '%d customers marked as exported to CSV', count( $object_ids ), 'ultimatewoo-pro' ), count( $object_ids ) );
					break;

					case 'orders':
						$message = sprintf( _n( '%d order marked as exported to CSV', '%d orders marked as exported to CSV', count( $object_ids ), 'ultimatewoo-pro' ), count( $object_ids ) );
					break;
				}
			...
			case 'mark_not_exported_to_csv':
				...
				switch ( $export_type ) {
					case 'customers':
						$message = sprintf( _n( '%d customer marked as not exported to CSV', '%d customers marked as not exported to CSV', count( $object_ids ), 'ultimatewoo-pro' ), count( $object_ids ) );
					break;

					case 'orders':
						$message = sprintf( _n( '%d order marked as not exported to CSV', '%d orders marked as not exported to CSV', count( $object_ids ), 'ultimatewoo-pro' ), count( $object_ids ) );
					break;
				}
				wc_customer_order_csv_export()->get_message_handler()->add_message( $message );
	...
	public static function get_export_options() {
		...
		$options = array(
			...
			'export_section_title' => array(
				'name' => __( 'Export', 'ultimatewoo-pro' ),
				'type' => 'title',
			),

			'type' => array(
				'id'      => 'type',
				'name'    => __( 'Export Orders or Customers', 'ultimatewoo-pro' ),
				'type'    => 'radio',
				'options' => array(
			...
			'export_options_section_title' => array(
				'name' => __( 'Export Options', 'ultimatewoo-pro' ),
				'type' => 'title',
			),
			...
			'start_date' => array(
				'id'   => 'start_date',
				'name' => __( 'Start Date', 'ultimatewoo-pro' ),
				'desc' => __( 'Start date of customers or orders to include in the exported file, in the format <code>YYYY-MM-DD.</code>', 'ultimatewoo-pro' ),
				'type' => 'text',
			),

			'end_date' => array(
				'id'   => 'end_date',
				'name' => __( 'End Date', 'ultimatewoo-pro' ),
				'desc' => __( 'End date of customers or orders to include in the exported file, in the format <code>YYYY-MM-DD.</code>', 'ultimatewoo-pro' ),
				'type' => 'text',
			),
		...
		if ( wc_customer_order_csv_export()->is_plugin_active( 'woocommerce-subscriptions.php' ) ) {
			...
			$options['subscription_options_section_title'] = array(
				'name' => __( 'Subscriptions Options', 'ultimatewoo-pro' ),
				'type' => 'title',
			);

			$options['subscription_orders'] = array(
				'id'            => 'subscription_orders',
				'title'         => __( 'Export Subscriptions Orders Only', 'ultimatewoo-pro' ),
				'desc'          => __( 'Export subscription orders', 'ultimatewoo-pro' ),
				'type'          => 'checkbox',
				'checkboxgroup' => 'start',
			);
			...
			$options['subscription_renewals'] = array(
				...
				'desc'          => __( 'Export renewal orders', 'ultimatewoo-pro' ),
				'type'          => 'checkbox',
				'checkboxgroup' => 'end',
			);
	...
	public function delete_export() {
		...
		if ( ! in_array( $export->status, array( 'completed', 'failed' ), true ) ) {
			$message = __( 'Export cancelled.', 'ultimatewoo-pro' );
		} else {
			$message = __( 'Exported file deleted.', 'ultimatewoo-pro' );
		}
	...
	public function transfer_export() {
		...
		if ( ! $auto_export_method ) {
			...
			/* translators: Placeholders: %s - file name */
			$this->message_handler->add_message( sprintf( __( 'Could not transfer file %s - no auto export method configured.', 'ultimatewoo-pro' ), $filename ) );
		...
		try {
			...
			$this->message_handler->add_message( sprintf( __( 'File %1$s transferred %2$s.', 'ultimatewoo-pro' ), $filename, $label ) );

		} catch ( SV_WC_Plugin_Exception $e ) {

			/* translators: Placeholders: %1$s - file name, %2$3 - transfer method, such as "via Email", %3$s - error message */
			$this->message_handler->add_error( sprintf( __( 'Could not transfer %1$s %2$s: %3$s', 'ultimatewoo-pro' ), $filename, $label, $e->getMessage() ) );
		}
