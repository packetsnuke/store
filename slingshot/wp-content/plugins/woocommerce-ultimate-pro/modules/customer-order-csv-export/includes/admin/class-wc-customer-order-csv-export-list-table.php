 ...
 class WC_Customer_Order_CSV_Export_List_Table extends WP_List_Table {
	...
	public function get_columns() {
		...
		$columns = array(
			...
			'export_status'   => '<span class="status_head tips" data-tip="' . esc_attr__( 'Export Status', 'ultimatewoo-pro' ) . '">' . esc_attr__( 'Export Status', 'ultimatewoo-pro' ) . '</span>',
			'transfer_status' => '<span class="transfer_status_head tips" data-tip="' . esc_attr__( 'Transfer Status', 'ultimatewoo-pro' ) . '">' . esc_attr__( 'Transfer Status', 'ultimatewoo-pro' ) . '</span>',
			'export_type'     => esc_html__( 'Type', 'ultimatewoo-pro' ),
			'invocation'      => esc_html__( 'Invocation', 'ultimatewoo-pro' ),
			'filename'        => esc_html__( 'File name', 'ultimatewoo-pro' ),
			'export_date'     => esc_html__( 'Date', 'ultimatewoo-pro' ),
	...
	public function column_default( $export, $column_name ) {
		...
		switch ( $column_name ) {
			...
			case 'invocation':
				...
				return 'auto' === $export->invocation ? esc_html__( 'Auto', 'ultimatewoo-pro' ) : esc_html__( 'Manual', 'ultimatewoo-pro' );
			...
			<?php
				...
				if ( 'completed' === $export->status ) {
					...
					$actions['download'] = array(
						'url'    => $download_url,
						'name'   => esc_html__( 'Download', 'ultimatewoo-pro' ),
						'action' => 'download'
					);
					...
					if ( $auto_export_method = wc_customer_order_csv_export()->get_methods_instance()->get_auto_export_method( $export->type ) ) {
						...
						if ( 'local' !== $auto_export_method ) {
							...
							$actions['transfer'] = array(
								...
								'name'   => sprintf( esc_html__( 'Send %s', 'ultimatewoo-pro' ), $label ),
				...
				$actions['delete'] = array(
					'url'    => $delete_url,
					'name'   => $done ? esc_html__( 'Delete', 'ultimatewoo-pro' ) : esc_html__( 'Cancel', 'ultimatewoo-pro' ),
