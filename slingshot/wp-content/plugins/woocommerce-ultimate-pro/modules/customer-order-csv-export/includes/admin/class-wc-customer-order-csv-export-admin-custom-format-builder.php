...
class WC_Customer_Order_CSV_Export_Admin_Custom_Format_Builder {
	...
	public static function get_settings( $section_id = null ) {
		...
		$settings = array(
			...
			'orders' => array(
				...
				array(
					'name' => __( 'Format Options', 'ultimatewoo-pro' ),
					'type' => 'title',
				),

				array(
					'id'       => 'wc_customer_order_csv_export_orders_custom_format_row_type',
					'name'     => __( 'A row represents', 'ultimatewoo-pro' ),
					'desc_tip' => __( 'Choose whether a single row in CSV should represent a full, single order or a single line item for an order.', 'ultimatewoo-pro' ),
					'type'     => 'select',
					'options'  => array(
						'order' => __( 'A single order', 'ultimatewoo-pro' ),
						'item'  => __( 'A single line item', 'ultimatewoo-pro' ),
					),
					'default' => 'order',
				...
				array(
					...
					'name'     => __( 'Items columns should use', 'ultimatewoo-pro' ),
					'desc_tip' => __( 'Choose whether items columns (line items, shipping items, fee items, etc) should be pipe-delimited or JSON-formatted.', 'ultimatewoo-pro' ),
					'type'     => 'select',
					'options'  => array(
						'pipe_delimited' => __( 'Pipe-delimited format', 'ultimatewoo-pro' ),
						'json'           => __( 'JSON', 'ultimatewoo-pro' ),
					),
				...
				array(
					...
					'name'     => __( 'CSV delimiter', 'ultimatewoo-pro' ),
					'type'     => 'select',
					'options'  => array(
						","  => __( 'Comma', 'ultimatewoo-pro' ),
						";"  => __( 'Semicolon', 'ultimatewoo-pro' ),
						"\t" => __( 'Tab', 'ultimatewoo-pro' ),
					),
					'default' => ',',
				...
				array(
					...
					'name'    => __( 'Include all meta', 'ultimatewoo-pro' ),
					'desc'    => __( 'Enable to include all meta in the export', 'ultimatewoo-pro' ),
					'default' => 'no',
					'type'    => 'checkbox',
				...
				array(
					'name' => __( 'Column Mapping', 'ultimatewoo-pro' ),
					'type' => 'title',
				),
						...
						<tr>
							<?php
								...
								$fields = apply_filters( 'wc_customer_order_csv_export_field_mapping_fields', array(
									...
									'name'            => esc_html__( 'Column name', 'ultimatewoo-pro' ),
									'source'          => esc_html__( 'Data source', 'ultimatewoo-pro' ),
								), $options );
					...
					<tbody>
								...
								switch ( $field ) {
									...
									case 'source' :
										...
										<td class="data">
											...
											<select name="<?php echo esc_attr( $options['id'] ); ?>[<?php echo esc_attr( $mapping_key ); ?>][source]" class="js-field-key wc-enhanced-select-nostd <?php echo $enhanced; ?>" data-placeholder="<?php esc_attr_e( 'Select a value', 'ultimatewoo-pro' ); ?>">
												...
												<option value="meta" <?php selected( 'meta', $value ); ?>><?php esc_html_e( 'Meta field...', 'ultimatewoo-pro' ); ?></option>
												<option value="static" <?php selected( 'static', $value ); ?>><?php esc_html_e( 'Static value...', 'ultimatewoo-pro' ); ?></option>
											</select>

											<label class="js-field-meta-key-label <?php echo ( 'meta' !== $value ? 'hide' : '' ); ?>">
												<?php esc_html_e( 'Meta key:', 'ultimatewoo-pro' ); ?>
											...
											<label class="js-field-static-value-label <?php echo ( 'static' !== $value ? 'hide' : '' ); ?>">
												<?php esc_html_e( 'Value:', 'ultimatewoo-pro' ); ?>
						...
						<tr class="no-field-mappings <?php if ( count( $mapping ) > 1 ) { echo 'hide'; } ?>">
							<td colspan="<?php echo count( $fields ); ?>">
								<?php esc_html_e( 'There are no mapped columns. Click the Add Column button below to start mapping columns.', 'ultimatewoo-pro' ); ?>
							</td>
						</tr>
					</tbody>
					<tfoot>
						<tr>
							<td colspan="<?php echo count( $fields ); ?>">
								<a class="button js-add-field-mapping" href="#"><?php esc_html_e( 'Add column', 'ultimatewoo-pro' ); ?></a>
								<a class="button js-remove-field-mapping <?php if ( count( $mapping ) < 2 ) { echo 'hide'; } ?>" href="#"><?php esc_html_e( 'Remove selected column(s)', 'ultimatewoo-pro' ); ?></a>
								<a class="button js-load-mapping button-secondary" href="#"><?php esc_html_e( 'Load column mapping', 'ultimatewoo-pro' ); ?></a>
							</td>
						</tr>
					</tfoot>
				</table>
	...
	public function output() {
		...
		submit_button( __( 'Save', 'ultimatewoo-pro' ) );
	}
	...
	public function save() {
		...
		wc_customer_order_csv_export()->get_message_handler()->add_message( __( 'Your column settings have been saved.', 'ultimatewoo-pro' ) );
