...
class WC_Customer_Order_CSV_Export_Methods {
			...
			case 'email':
				...
				switch ( $export_type ) {
					...
					case 'orders':
						...
						$message = esc_html__( 'Order Export for %s', 'ultimatewoo-pro' );
					break;

					case 'customers':
						/* translators: Placeholders: %s - date */
						$message = esc_html__( 'Customer Export for %s', 'ultimatewoo-pro' );
					break;

					default:
						/* translators: Placeholders: %s - date */
						$message = esc_html__( 'Export for %s', 'ultimatewoo-pro' );
					break;
				}
	...
	public function get_export_method_labels() {
		...
		return apply_filters( 'wc_customer_order_csv_export_methods', array(
			...
			'ftp'       => __( 'via FTP', 'ultimatewoo-pro' ),
			'http_post' => __( 'via HTTP POST', 'ultimatewoo-pro' ),
			'email'     => __( 'via Email', 'ultimatewoo-pro' ),
		) );
	}
	...
	public function get_export_method_label( $method ) {
		...
		return ! empty( $methods[ $method ] ) ? $methods[ $method ] : sprintf( __( 'via %s', 'ultimatewoo-pro' ), $method );
	}
