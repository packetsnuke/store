...
class WC_Customer_Order_CSV_Export_Method_FTP_Implicit_SSL extends WC_Customer_Order_CSV_Export_Method_File_Transfer {
	 ...
	 */
	 public function __construct( $args ) {
		...
		if ( ! $this->curl_handle ) {
			...
			throw new SV_WC_Plugin_Exception( __( 'Could not initialize cURL.', 'ultimatewoo-pro' ) );
		}
		...
		foreach ( $options as $option_name => $option_value ) {
			...
			if ( ! curl_setopt( $this->curl_handle, $option_name, $option_value ) ) {
				...
				throw new SV_WC_Plugin_Exception( sprintf( __( 'Could not set cURL option: %s', 'ultimatewoo-pro' ), $option_name ) );
			}
		}
	}
	...
	public function perform_action( $file_path ) {
		...
		if ( ! curl_setopt( $this->curl_handle, CURLOPT_URL, $this->url . $filename ) ) {
			...
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Could not set cURL file name: %s', 'ultimatewoo-pro' ), $filename ) );
		}
		...
		if ( ! $stream ) {
			...
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Could not open %s for reading.', 'ultimatewoo-pro' ), $file_path ) );
		}
		...
		if ( ! curl_setopt( $this->curl_handle, CURLOPT_INFILE, $stream ) ) {
			...
			/* translators: Placeholders: %s - name of file to be updloaded */
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Could not load file %s', 'ultimatewoo-pro' ), $filename ) );
		}
		...
		if ( ! curl_exec( $this->curl_handle ) ) {
			...
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Could not upload file. cURL Error: [%1$s] - %2$s', 'ultimatewoo-pro' ), curl_errno( $this->curl_handle ), curl_error( $this->curl_handle ) ) );
		}
