...
class WC_Customer_Order_CSV_Export_Method_SFTP extends WC_Customer_Order_CSV_Export_Method_File_Transfer {
	 ...
	 */
	 public function __construct( $args ) {
		...
		if ( ! function_exists( 'ssh2_connect' ) ) {
			...
			throw new SV_WC_Plugin_Exception( __( 'SSH2 Extension is not installed, cannot connect via SFTP.', 'ultimatewoo-pro' ) );
		}
		...
		if ( ! $this->ssh_link ) {
			...
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Could not connect via SSH to %1$s on port %2$s, check server address and port.', 'ultimatewoo-pro' ), $this->server, $this->port ) );
		}
		...
		if ( ! ssh2_auth_password( $this->ssh_link, $this->username, $this->password ) ) {
			...
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Could not authenticate via SSH with username %s and password. Check username and password.', 'ultimatewoo-pro' ), $this->username ) );
		}
		...
		if ( ! $this->sftp_link ) {
			...
			throw new SV_WC_Plugin_Exception( __( 'Could not setup SFTP link', 'ultimatewoo-pro' ) );
		}
	}
	...
	public function perform_action( $file_path ) {
		...
		if ( ! $stream ) {
			...
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Could not open remote file: %s.', 'ultimatewoo-pro' ), $remote_file ) );
		}
		...
		if ( false === $data ) {
			...
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Could not open file %s for reading.', 'ultimatewoo-pro' ), $filename ) );
		}
		...
		if ( false === fwrite( $stream, $data ) ) {
			...
			/* translators: Placeholders: %s - file name */
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Could not write data to remote file: %s.', 'ultimatewoo-pro' ), $filename ) );
		}
	...
	public function handle_errors( $error_no, $error_string, $error_file, $error_line ) {
		...
		throw new SV_WC_Plugin_Exception( sprintf( __( 'SFTP error: %s', 'ultimatewoo-pro' ), $error_string ) );
	}
