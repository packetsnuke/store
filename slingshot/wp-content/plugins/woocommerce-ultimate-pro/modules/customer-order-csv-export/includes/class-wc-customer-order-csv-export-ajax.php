...
class WC_Customer_Order_CSV_Export_AJAX {
	...
	public function create_export() {
		...
		if ( empty( $_POST['export_type'] ) || empty( $_POST['export_method'] ) || empty( $_POST['export_query'] ) ) {
			...
			wp_send_json_error( array(
				'title'   => esc_html__( 'Export Failed', 'ultimatewoo-pro' ),
				'message' => esc_html__( 'Missing export type, method or query.', 'ultimatewoo-pro' ),
			) );
		}
		...
		if ( empty( $export_ids ) ) {
			...
			switch ( $export_type ) {
				...
				case 'orders':
					$message = esc_html__( 'No orders found to export', 'ultimatewoo-pro' );
				break;

				case 'customers':
					$message = esc_html__( 'No customers found to export', 'ultimatewoo-pro' );
				break;
			}

			wp_send_json_error( array(
				'title'   => esc_html__( 'Nothing to Export', 'ultimatewoo-pro' ),
				'message' => $message,
			) );
		}
		...
		if ( 'auto_export' === $export_method ) {
			...
			if ( ! $export_method ) {
				...
				wp_send_json_error( array(
					'title'   => esc_html__( 'Export Failed', 'ultimatewoo-pro' ),
					'message' => esc_html__( 'Auto export method not configured.', 'ultimatewoo-pro' ),
				) );
			}
		}
	...
	public function get_export_status() {
		...
		if ( ! $export ) {
			...
			wp_send_json_error( array(
				'title'   => esc_html__( 'Export Not Found', 'ultimatewoo-pro' ),
				/* translators: Placeholders: %s - export ID */
				'message' => sprintf( esc_html__( 'No export found with id %s. It may have been cancelled during export.', 'ultimatewoo-pro' ), $_GET['export_id'] ),
			) );
		}
		...
		if ( 'failed' === $export->status ) {
			...
			wp_send_json_error( array(
				'title'   => esc_html__( 'Export Failed', 'ultimatewoo-pro' ),
				'message' => esc_html__( 'Unfortunately, your export failed.', 'ultimatewoo-pro' ) . ' ' . $logs_message,
			) );
		}
		...
		if ( 'failed' === $export->transfer_status ) {
			...
			wp_send_json_error( array(
				'title'   => esc_html__( 'Export Transfer Failed', 'ultimatewoo-pro' ),
				/* translators: Placeholders: %1$s - via [method], example: "...but the transfer via Email failed.", %2$s - opening <a> tag, %3$s - closing </a> tag */
				'message' => sprintf( esc_html__( 'Export completed successfully, but the transfer %1$s failed. Exported file is available under %2$sExport List%3$s.', 'ultimatewoo-pro' ), $label, '<a href="' . admin_url( 'admin.php?page=wc_customer_order_csv_export&tab=export_list' ) . '">', '</a>' ) . ' ' . $logs_message,
			) );
		}
