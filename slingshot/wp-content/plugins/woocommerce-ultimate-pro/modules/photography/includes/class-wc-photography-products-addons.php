...
class WC_Photography_Products_Addons {
	...
	public function global_edit_objects( $objects ) {
		echo '<optgroup label="' . __( 'Photography Collections', 'ultimatewoo-pro' ) . '">';

			$collections = get_terms( 'images_collections', array( 'hide_empty' => 0 ) );

			foreach ( $collections as $collection ) {
				echo '<option value="' . $collection->term_id . '" ' . selected( in_array( $collection->term_id, $objects ), true, false ) . '>' . __( 'Collection:', 'ultimatewoo-pro' ) . ' ' . $collection->name . '</option>';
			}
	...
	public function global_display_term_names( $term_names, $objects ) {
		foreach ( $objects as $object_id ) {
			...
			if ( $term ) {
				$term_names[] = __( 'Collection:', 'ultimatewoo-pro' ) . ' ' . $term->name;
			}
		}
