...
class WC_Photography_Admin_Collections {
	...
	public function collections_columns( $columns ) {
		...
		$new_columns['visibility'] = __( 'Visibility', 'ultimatewoo-pro' );
		$new_columns['actions']    = '';
	...
	public function collections_column( $columns, $column, $id ) {
		if ( 'thumb' == $column ) {
			...
			$columns .= '<img src="' . esc_url( $image ) . '" alt="' . __( 'Thumbnail', 'ultimatewoo-pro' ) . '" class="wp-post-image" height="48" width="48" />';
		}
		...
		if ( 'actions' == $column ) {
			...
			$columns .= '<a class="button-secondary" href="edit.php?images_collections=' . esc_attr( $collection->slug ) . '&post_type=product" title="' . __( 'View products on this collection.', 'ultimatewoo-pro' ) . '">' . sprintf( __( 'View Products (%d)', 'ultimatewoo-pro' ), $collection->count ) . '</a>';
			$columns .= ' <a class="button-secondary" href="' . get_term_link( $collection ) . '" title="' . __( 'View collection page.', 'ultimatewoo-pro' ) . '">' . __( 'View Collection', 'ultimatewoo-pro' ) . '</a>';
		}
