...
if ( is_woocommerce_active() && ! class_exists( 'WC_Currency_Converter' ) ) {
	...
	class WC_Currency_Converter {
		...
		public function __construct() {
			$this->settings = array(
				array(
					'name' => __( 'Open Exchange Rate API', 'ultimatewoo-pro' ),
					'type' => 'title',
					'desc' => '',
					'id'   => 'woocommerce-currency-converter-widget'
				),
				array(
					'name' => __( 'App Key', 'ultimatewoo-pro' ),
					'desc' => sprintf( __( '(optional) If you have an <a href="%s">Open Exchange Rate API app ID</a>, enter it here.', 'ultimatewoo-pro' ), 'https://openexchangerates.org/signup' ),
		...
		public function admin_notice_wrong_key() {
			?>
			<div class="error">
				<p><?php _e( 'WooCommerce Currency Converter: Incorrect key entered!', 'ultimatewoo-pro' ); ?></p>
			</div>
		<?php
		}
			...
			if ( $currencies ) {
				...
				if ( ! empty( $instance['currency_display'] ) && 'select' === $instance['currency_display'] ) {
					...
					if ( ! empty( $instance['show_reset'] ) ) {
						$html .= ' <a href="#" class="wc-currency-converter-reset reset">' . __( 'Reset', 'ultimatewoo-pro' ) . '</a>';
					}
					...
					if ( ! empty( $instance['show_reset'] ) ) {
						$html .= '<li><a href="#" class="wc-currency-converter-reset reset">' . __( 'Reset', 'ultimatewoo-pro' ) . '</a></li>';
					}
		...
		public function enqueue_assets() {
			...
			// Styles
			wp_enqueue_style( 'currency_converter_styles', ULTIMATEWOO_MODULES_URL . '/currency-converter/assets/css/converter.css' );

			// Scripts
			wp_register_script( 'moneyjs', ULTIMATEWOO_MODULES_URL . '/currency-converter/assets/js/money.min.js', array( 'jquery' ), '0.1.3', true );
			wp_register_script( 'accountingjs', ULTIMATEWOO_MODULES_URL . '/currency-converter/assets/js/accounting.min.js', array( 'jquery' ), '0.3.2', true );
			wp_enqueue_script( 'jquery-cookie' );
			wp_register_script( 'wc_currency_converter', ULTIMATEWOO_MODULES_URL . '/currency-converter/assets/js/conversion.js', array(
				'jquery',
				'moneyjs',
				'accountingjs',
				'jquery-cookie'
			...
			$wc_currency_converter_params = array(
				...
				'i18n_oprice'            => __( 'Original price:', 'ultimatewoo-pro' ),
