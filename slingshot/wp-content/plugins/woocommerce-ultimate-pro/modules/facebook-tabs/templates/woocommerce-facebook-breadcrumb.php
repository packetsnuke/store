...
//$defaults = apply_filters( 'woocommerce_breadcrumb_defaults', $defaults);
...
//	 if ( ! empty( $home ) ) {
//echo woocommerce_facebook_tab_the_permalink(home_url());
//apply_filters( 'woocommerce_breadcrumb_home_url', home_url() )
    $prepend = $before . '<a class="home" href="' .
    woocommerce_facebook_tab_the_permalink(get_permalink( woocommerce_get_page_id( 'shop' ) )) .
         '">' . $home . '</a>' . $after; // . $delimiter;
//	}
if(!is_woocommerce()){
    // just incase we end up at the home page or a page that is not supported by the facebook tab, they will have a button to get to the shop.
    echo $prepend;
    if(is_404()){
        echo $delimiter;
    }
