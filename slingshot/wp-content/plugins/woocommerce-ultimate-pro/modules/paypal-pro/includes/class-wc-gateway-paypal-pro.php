...
class WC_Gateway_PayPal_Pro extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_title         = __( 'PayPal Pro', 'ultimatewoo-pro' );
		$this->method_description   = __( 'PayPal Pro works by adding credit card fields on the checkout and then sending the details to PayPal for verification.', 'ultimatewoo-pro' );
    ...
    public function init_form_fields() {
    	$this->form_fields = array(
			'enabled' => array(
				...
				'label'       => __( 'Enable PayPal Pro', 'ultimatewoo-pro' ),
				'type'        => 'checkbox',
				'description' => '',
				'default'     => 'no'
			),
			'title' => array(
				...
				'default'     => __( 'Credit card (PayPal)', 'ultimatewoo-pro' ),
				'desc_tip'    => true
			),
			'description' => array(
				...
				'default'     => __( 'Pay with your credit card via PayPal Website Payments Pro.', 'ultimatewoo-pro' ),
				'desc_tip'    => true
			),
			'testmode' => array(
				'title'       => __( 'Test Mode', 'ultimatewoo-pro' ),
				'label'       => __( 'Enable PayPal Sandbox/Test Mode', 'ultimatewoo-pro' ),
				'type'        => 'checkbox',
				'description' => __( 'Place the payment gateway in development mode.', 'ultimatewoo-pro' ),
				'default'     => 'no',
				'desc_tip'    => true
			),
			'api_username' => array(
				...
				'type'        => 'text',
				'description' => __( 'Get your API credentials from PayPal.', 'ultimatewoo-pro' ),
				'default'     => '',
				'desc_tip'    => true
			),
			'api_password' => array(
			...
			'paypal_pro_paymentaction' => array(
				'title'       => __( 'Payment Action', 'ultimatewoo-pro' ),
				'type'        => 'select',
				'description' => __( 'Choose whether you wish to capture funds immediately or authorize payment only.', 'ultimatewoo-pro' ),
				'default'     => 'sale',
				'desc_tip'    => true,
				'options'     => array(
					'sale'          => __( 'Capture', 'ultimatewoo-pro' ),
					'authorization' => __( 'Authorize', 'ultimatewoo-pro' )
				)
			),
			'enable_3dsecure' => array(
				'title'       => __( '3DSecure', 'ultimatewoo-pro' ),
				'label'       => __( 'Enable 3DSecure', 'ultimatewoo-pro' ),
				'type'        => 'checkbox',
				'description' => __( 'Allows UK merchants to pass 3-D Secure authentication data to PayPal for debit and credit cards. Updating your site with 3-D Secure enables your participation in the Verified by Visa and MasterCard SecureCode programs. (Required to accept Maestro)', 'ultimatewoo-pro' ),
				'default'     => 'no',
				'desc_tip'    => true
			),
			'centinel_pid' => array(
				'title'       => __( 'Centinel PID', 'ultimatewoo-pro' ),
				'type'        => 'text',
				'description' => __( 'If enabling 3D Secure, enter your Cardinal Centinel Processor ID.', 'ultimatewoo-pro' ),
				'default'     => '',
				'desc_tip'    => true
			),
			'centinel_mid' => array(
				'title'       => __( 'Centinel MID', 'ultimatewoo-pro' ),
				'type'        => 'text',
				'description' => __( 'If enabling 3D Secure, enter your Cardinal Centinel Merchant ID.', 'ultimatewoo-pro' ),
				'default'     => '',
				'desc_tip'    => true
			),
			'centinel_pwd' => array(
				'title'       => __( 'Transaction Password', 'ultimatewoo-pro' ),
				'type'        => 'password',
				'description' => __( 'If enabling 3D Secure, enter your Cardinal Centinel Transaction Password.', 'ultimatewoo-pro' ),
				'default'     => '',
				'desc_tip'    => true
			),
			'liability_shift' => array(
				'title'       => __( 'Liability Shift', 'ultimatewoo-pro' ),
				'label'       => __( 'Require liability shift', 'ultimatewoo-pro' ),
				'type'        => 'checkbox',
				'description' => __( 'Only accept payments when liability shift has occurred.', 'ultimatewoo-pro' ),
				'default'     => 'no',
				'desc_tip'    => true
			),
			'send_items' => array(
				'title'       => __( 'Send Item Details', 'ultimatewoo-pro' ),
				'label'       => __( 'Send Line Items to PayPal', 'ultimatewoo-pro' ),
				'type'        => 'checkbox',
				'description' => __( 'Sends line items to PayPal. If you experience rounding errors this can be disabled.', 'ultimatewoo-pro' ),
				'default'     => 'no',
				'desc_tip'    => true
			),
			'soft_descriptor' => array(
				'title'             => __( 'Soft Descriptor', 'ultimatewoo-pro' ),
				'type'              => 'text',
				'description'       => __( '(Optional) Information that is usually displayed in the account holder\'s statement, for example your website name. Only 23 alphanumeric characters can be included, including the special characters dash (-) and dot (.) . Asterisks (*) and spaces ( ) are NOT permitted.', 'ultimatewoo-pro' ),
				'default'           => '',
				'desc_tip'          => true,
			...
			'debug' => array(
				'title'       => __( 'Debug Log', 'ultimatewoo-pro' ),
				'type'        => 'checkbox',
				'label'       => __( 'Enable logging', 'ultimatewoo-pro' ),
				'default'     => 'no',
				'desc_tip'    => true,
				'description' => __( 'Log PayPal Pro events inside <code>woocommerce/logs/paypal-pro.txt</code>', 'ultimatewoo-pro' ),
			)
		);
    }
	...
	public function payment_fields() {
		...
		if ( $this->description ) {
			echo '<p>' . wp_kses_post( $this->description ) . ( $this->testmode ? ' ' . __( 'TEST/SANDBOX MODE ENABLED. In test mode, you can use the card number 4007000000027 with any CVC and a valid expiration date.  Note that you will get a faster processing result if you use a card from your developer\'s account.', 'ultimatewoo-pro' ) : '' ) . '</p>';
		}

		?>
		<fieldset>
			<p class="form-row form-row-first">
				<label for="<?php echo esc_attr( $this->id ); ?>-card-number"><?php  esc_html_e( 'Card Number', 'ultimatewoo-pro' ); ?> <span class="required">*</span></label>
			...
			<p class="form-row form-row-last">
				<label for="<?php echo esc_attr( $this->id ); ?>-card-expiry"><?php esc_html_e( 'Expiry (MM/YY)', 'ultimatewoo-pro' ); ?> <span class="required">*</span></label>
				<input id="<?php echo esc_attr( $this->id ); ?>-card-expiry" class="input-text wc-credit-card-form-card-expiry" type="text" autocomplete="off" placeholder="<?php esc_attr_e( 'MM / YY', 'ultimatewoo-pro' ); ?>" name="<?php echo esc_attr( $this->id ); ?>-card-expiry" />
			</p>

			<p class="form-row form-row-first">
				<label for="<?php echo esc_attr( $this->id ); ?>-card-cvc"><?php esc_html_e( 'Card Code', 'ultimatewoo-pro' ); ?> <span class="required">*</span></label>
				<input id="<?php echo esc_attr( $this->id ); ?>-card-cvc" class="input-text wc-credit-card-form-card-cvc" type="text" autocomplete="off" placeholder="<?php esc_attr_e( 'CVC', 'ultimatewoo-pro' ); ?>" name="<?php echo esc_attr( $this->id ); ?>-card-cvc" />
			</p>

			<?php if ( isset( $this->available_card_types[ WC()->countries->get_base_country() ]['Maestro'] ) ) : ?>
				<p class="form-row form-row-last">
					<label for="<?php echo esc_attr( $this->id ); ?>-card-startdate"><?php esc_html_e( 'Start Date (MM/YY)', 'ultimatewoo-pro' ); ?></label>
					<input id="<?php echo esc_attr( $this->id ); ?>-card-startdate" class="input-text wc-credit-card-form-card-expiry" type="text" autocomplete="off" placeholder="<?php  esc_html_e( 'MM / YY', 'ultimatewoo-pro' ); ?>" name="<?php echo esc_attr( $this->id ); ?>-card-startdate" />
				</p>
			<?php endif; ?>
		</fieldset>
		<?php
	...
	public function validate_fields() {
		try {
			...
			if ( empty( $card->exp_month ) || empty( $card->exp_year ) ) {
				throw new Exception( __( 'Card expiration date is invalid', 'ultimatewoo-pro' ) );
			}

			// Validate values
			if ( ! ctype_digit( $card->cvc ) ) {
				throw new Exception( __( 'Card security code is invalid (only digits are allowed)', 'ultimatewoo-pro' ) );
			}
			...
			if ( empty( $card->number ) || ! ctype_digit( $card->number ) ) {
				throw new Exception( __( 'Card number is invalid', 'ultimatewoo-pro' ) );
			}
	...
	public function process_payment( $order_id ) {
		...
		if ( $this->enable_3dsecure ) {
			...
			if ( $this->get_centinel_value( 'ErrorNo' ) ) {
				wc_add_notice( __( 'Error in 3D secure authentication: ', 'ultimatewoo-pro' ) . $this->get_centinel_value( 'ErrorDesc' ), 'error' );
				return;
			}
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if ( $details && strtolower( $details['PENDINGREASON'] ) === 'authorization' ) {
			$order->add_order_note( __( 'This order cannot be refunded due to an authorized only transaction.  Please use cancel instead.', 'ultimatewoo-pro' ) );
			$this->log( 'Refund order # ' . absint( $order_id ) . ': authorized only transactions need to use cancel/void instead.' );
			throw new Exception( __( 'This order cannot be refunded due to an authorized only transaction.  Please use cancel instead.', 'ultimatewoo-pro' ) );
		}
		...
		if ( is_wp_error( $response ) ) {
			...
			throw new Exception( __( 'There was a problem connecting to the payment gateway.', 'ultimatewoo-pro' ) );
		}

		parse_str( $response['body'], $parsed_response );

		if ( ! isset( $parsed_response['ACK'] ) ) {
			throw new Exception( __( 'Unexpected response from PayPal.', 'ultimatewoo-pro' ) );
		}

		switch ( strtolower( $parsed_response['ACK'] ) ) {
			case 'success':
			case 'successwithwarning':
				$order->add_order_note( sprintf( __( 'Refunded %1$s - Refund ID: %2$s', 'ultimatewoo-pro' ), $parsed_response['GROSSREFUNDAMT'], $parsed_response['REFUNDTRANSACTIONID'] ) );
				return true;
			default:
		...
		try {
		    ...
		    if ( ! empty( $pares ) ) {
				...
				if ( $this->liability_shift && ( $this->get_centinel_value( 'EciFlag' ) == '07' || $this->get_centinel_value( "EciFlag" ) == '01' ) ) {
					$order->update_status( 'failed', __( '3D Secure error: No liability shift', 'ultimatewoo-pro' ) );
					throw new Exception( __( 'Authentication unavailable.  Please try a different payment method or card.', 'ultimatewoo-pro' ) );
				}
				...
				if ( ! $this->get_centinel_value( 'ErrorNo' ) && in_array( $this->get_centinel_value( 'PAResStatus' ), array( 'Y', 'A', 'U' ) ) && "Y" === $this->get_centinel_value( 'SignatureVerification' ) ) {
					...
					$order->update_status( 'failed', sprintf( __( '3D Secure error: %s', 'ultimatewoo-pro' ), $this->get_centinel_value( 'ErrorDesc' ) ) );
	...
	public function do_payment( $order, $card, $centinel = false ) {
		...
		try {
			...
			if ( empty( $response['body'] ) ) {
				...
				throw new Exception( __( 'Empty Paypal response.', 'ultimatewoo-pro' ) );
			}
			...
			switch ( strtolower( $parsed_response['ACK'] ) ) {
				...
				case 'successwithwarning':
					...
					if ( $details && strtolower( $details['PAYMENTSTATUS'] ) === 'pending' && strtolower( $details['PENDINGREASON'] ) === 'authorization' ) {
						...
						// Mark as on-hold
						$order->update_status( 'on-hold', sprintf( __( 'PayPal Pro charge authorized (Charge ID: %s). Process order to take payment, or cancel to remove the pre-authorization.', 'ultimatewoo-pro' ), $txn_id ) );
						...
						// Add order note
						$order->add_order_note( sprintf( __( 'PayPal Pro payment completed (Transaction ID: %s, Correlation ID: %s)', 'ultimatewoo-pro' ), $txn_id, $correlation_id ) );
				...
				default:
					...
					// Payment failed :(
					$order->update_status( 'failed', sprintf(__('PayPal Pro payment failed (Correlation ID: %s). Payment was rejected due to an error: ', 'ultimatewoo-pro'), $parsed_response['CORRELATIONID'] ) . '(' . $parsed_response['L_ERRORCODE0'] . ') ' . '"' . $error_message . '"' );
			...
			wc_add_notice( '<strong>' . __( 'Payment error', 'ultimatewoo-pro' ) . '</strong>: ' . $e->getMessage(), 'error' );
			return;
		}
	}
