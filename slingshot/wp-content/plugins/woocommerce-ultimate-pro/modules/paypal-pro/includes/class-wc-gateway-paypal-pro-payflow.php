...
class WC_Gateway_PayPal_Pro_PayFlow extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_title 		= __( 'PayPal Pro PayFlow', 'ultimatewoo-pro' );
		$this->method_description 	= __( 'PayPal Pro PayFlow Edition works by adding credit card fields on the checkout and then sending the details to PayPal for verification.', 'ultimatewoo-pro' );
		...
		if ( $this->transparent_redirect ) {
			$this->order_button_text    = __( 'Enter payment details', 'ultimatewoo-pro' );
		}
	...
	public function init_form_fields() {
    	$this->form_fields = array(
			'enabled'         => array(
				...
				'label'       => __( 'Enable PayPal Pro Payflow Edition', 'ultimatewoo-pro' ),
				'type'        => 'checkbox',
				'description' => '',
				'default'     => 'no'
			),
			...
			'description'     => array(
				...
				'default'     => __( 'Pay with your credit card.', 'ultimatewoo-pro' ),
				'desc_tip'    => true
			),
			...
			'transparent_redirect' => array(
				'title'       => __( 'Transparent Redirect', 'ultimatewoo-pro' ),
				'label'       => __( 'Enable Transparent Redirect', 'ultimatewoo-pro' ),
				'type'        => 'checkbox',
				'description' => __( 'Rather than showing a credit card form on your checkout, this shows the form on it\'s own page and posts straight to PayPal, thus making the process more secure and more PCI friendly. "Enable Secure Token" needs to be enabled on your PayFlow account to work.', 'ultimatewoo-pro' ),
				'default'     => 'no',
				'desc_tip'    => true
			),
			'paypal_vendor'   => array(
				'title'       => __( 'PayPal Vendor', 'ultimatewoo-pro' ),
				'type'        => 'text',
				'description' => __( 'Your merchant login ID that you created when you registered for the account.', 'ultimatewoo-pro' ),
				'default'     => '',
				'desc_tip'    => true
			),
			'paypal_password' => array(
				'title'       => __( 'PayPal Password', 'ultimatewoo-pro' ),
				'type'        => 'password',
				'description' => __( 'The password that you defined while registering for the account.', 'ultimatewoo-pro' ),
				'default'     => '',
				'desc_tip'    => true
			),
			'paypal_user'     => array(
				'title'       => __( 'PayPal User', 'ultimatewoo-pro' ),
				'type'        => 'text',
				'description' => __( 'If you set up one or more additional users on the account, this value is the ID
			of the user authorized to process transactions. Otherwise, leave this field blank.', 'ultimatewoo-pro' ),
				'default'     => '',
				'desc_tip'    => true
			),
			'paypal_partner'  => array(
				'title'       => __( 'PayPal Partner', 'ultimatewoo-pro' ),
				'type'        => 'text',
				'description' => __( 'The ID provided to you by the authorized PayPal Reseller who registered you
			for the Payflow SDK. If you purchased your account directly from PayPal, use PayPal or leave blank.', 'ultimatewoo-pro' ),
				'default'     => 'PayPal',
				'desc_tip'    => true
			),
			'paypal_pro_payflow_paymentaction' => array(
				...
				'desc_tip'    => true,
				'options'     => array(
					'S'          => __( 'Capture', 'ultimatewoo-pro' ),
					'A'          => __( 'Authorize', 'ultimatewoo-pro' )
				)
			),
			'debug' => array(
				...
				'default'     => 'no',
				'desc_tip'    => true,
				'description' => __( 'Log PayPal Pro (Payflow) events inside <code>woocommerce/logs/paypal-pro-payflow.txt</code>', 'ultimatewoo-pro' ),
			)
		);
    }
	...
	public function receipt_page( $order_id ) {
		if ( $this->transparent_redirect ) {
			...
			echo wpautop( __( 'Enter your payment details below and click "Confirm and pay" to securely pay for your order.', 'ultimatewoo-pro' ) );
			?>
			<form method="POST" action="<?php echo $url; ?>">
				<div id="payment">
					...
					<div class="payment_box">
						<p><?php echo $this->description . ( $this->testmode ? ' ' . __( 'TEST/SANDBOX MODE ENABLED. In test mode, you can use the card number 4111111111111111 with any CVC and a valid expiration date.', 'ultimatewoo-pro' ) : '' ); ?></p>

						<fieldset id="paypal_pro_payflow-cc-form">
							<p class="form-row form-row-wide">
								<label for="paypal_pro_payflow-card-number"><?php _e( 'Card Number ', 'ultimatewoo-pro' ); ?><span class="required">*</span></label>
							...
							<p class="form-row form-row-first">
								<label for="paypal_pro_payflow-card-expiry"><?php _e( 'Expiry (MM/YY) ', 'ultimatewoo-pro' ); ?><span class="required">*</span></label>
							...
							<p class="form-row form-row-last">
								<label for="paypal_pro_payflow-card-cvc"><?php _e( 'Card Code ', 'ultimatewoo-pro' ); ?><span class="required">*</span></label>
					...
					<input type="submit" value="<?php _e( 'Confirm and pay', 'ultimatewoo-pro' ); ?>" class="submit buy button" style="float:right;"/>
				</div>
		...
		switch ( $result ) {
			...
			case 127 :
				...
				if ( $details && strtolower( $details['TRANSSTATE'] ) === '3' ) {
					...
					// Mark as on-hold
					$order->update_status( 'on-hold', sprintf( __( 'PayPal Pro (PayFlow) charge authorized (Charge ID: %s). Process order to take payment, or cancel to remove the pre-authorization.', 'ultimatewoo-pro' ), $txn_id ) );
					...
					// Add order note
					$order->add_order_note( sprintf( __( 'PayPal Pro (Payflow) payment completed (PNREF: %s)', 'ultimatewoo-pro' ), $parsed_response['PNREF'] ) );
			...
			case 126 :
				...
				$order->update_status( 'on-hold', __( 'The payment was flagged by a fraud filter. Please check your PayPal Manager account to review and accept or deny the payment and then mark this order "processing" or "cancelled".', 'ultimatewoo-pro' ) );
				WC()->cart->empty_cart();
				$redirect = $order->get_checkout_order_received_url();
			break;
			default :
	...
	public function get_token( $order, $post_data, $force_new_token = false ) {
		...
		if ( is_wp_error( $response ) ) {
			wc_add_notice( __( 'There was a problem connecting to the payment gateway.', 'ultimatewoo-pro' ) );
			return false;
		}

		if ( empty( $response['body'] ) ) {
			wc_add_notice( __( 'Empty Paypal response.', 'ultimatewoo-pro' ) );
			return false;
		}
		...
		if ( isset( $parsed_response['RESULT'] ) && in_array( $parsed_response['RESULT'], array( 160, 161, 162 ) ) ) {
			...
			$order->update_status( 'failed', __( 'PayPal Pro (Payflow) token generation failed: ', 'ultimatewoo-pro' ) . '(' . $parsed_response['RESULT'] . ') ' . '"' . $parsed_response['RESPMSG'] . '"' );

			wc_add_notice( __( 'Payment error:', 'ultimatewoo-pro' ) . ' ' . $parsed_response['RESPMSG'], 'error' );
	...
	public function do_payment( $order, $card_number, $card_exp, $card_cvc ) {
		...
		try {
			...
			if ( isset( $parsed_response['RESULT'] ) && in_array( $parsed_response['RESULT'], array( 0, 126, 127 ) ) ) {
				...
				// Payment failed.
				$order->update_status( 'failed', __( 'PayPal Pro (Payflow) payment failed. Payment was rejected due to an error: ', 'ultimatewoo-pro' ) . '(' . $parsed_response['RESULT'] . ') ' . '"' . $parsed_response['RESPMSG'] . '"' );

				wc_add_notice( __( 'Payment error:', 'ultimatewoo-pro' ) . ' ' . $parsed_response['RESPMSG'], 'error' );
				return;
			}
		} catch ( Exception $e ) {
			wc_add_notice( __( 'Connection error:', 'ultimatewoo-pro' ) . ': "' . $e->getMessage() . '"', 'error' );
			return;
		}
	}
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if ( '0' !== $parsed_response['RESULT'] ) {
			...
			$order->add_order_note( sprintf( __( 'Refunded %1$s - PNREF: %2$s', 'ultimatewoo-pro' ), wc_price( number_format( $amount, 2, '.', '' ) ), $parsed_response['PNREF'] ) );
	...
	public function payment_fields() {
		...
		if ( $this->description ) {
			if ( $this->transparent_redirect ) {
				...
				echo '<p>' . $this->description . ( $this->testmode ? ' ' . __( 'TEST/SANDBOX MODE ENABLED. In test mode, you can use the card number 4111111111111111 with any CVC and a valid expiration date.', 'ultimatewoo-pro' ) : '' ) . '</p>';
			}
		}
