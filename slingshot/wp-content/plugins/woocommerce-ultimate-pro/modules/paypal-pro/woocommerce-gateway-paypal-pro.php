...
if ( ! class_exists( 'WC_PayPal_Pro' ) ) :
	...
	class WC_PayPal_Pro {
		...
		public function load_plugin_textdomain() {
			$locale = apply_filters( 'wc_paypal_pro_plugin_locale', get_locale(), 'ultimatewoo-pro' );
		...
		public function woocommerce_missing_notice() {
			echo '<div class="error"><p>' . sprintf( __( 'WooCommerce PayPal Pro Plugin requires WooCommerce to be installed and active. %s', 'ultimatewoo-pro' ), '<a href="http://www.woocommerce.com/" target="_blank">WooCommerce</a>' ) . '</p></div>';
		...
		protected function _capture_payment_paypal_pro( $order ) {
			...
			if ( is_wp_error( $response ) ) {
				$order->add_order_note( __( 'Unable to capture charge!', 'ultimatewoo-pro' ) . ' ' . $response->get_error_message() );
		...
		protected function _capture_payment_paypal_pro_success( $order, $parsed_response ) {
			...
			$order->add_order_note( sprintf( __( 'PayPal Pro charge complete (Transaction ID: %s)', 'ultimatewoo-pro' ), $parsed_response['TRANSACTIONID'] ) );
		...
		protected function _capture_payment_paypal_pro_failed( $order, $parsed_response ) {
			...
			// Back to on-hold.
			$order->update_status( 'on-hold', sprintf( __( 'PayPal Pro capture failed (Correlation ID: %s). Capture was rejected due to an error: ', 'ultimatewoo-pro' ), $parsed_response['CORRELATIONID'] ) . '(' . $parsed_response['L_ERRORCODE0'] . ') ' . '"' . $error_message . '"' );
		}
		...
		protected function _capture_payment_paypal_pro_payflow_success( $order, $parsed_response ) {
			...
			$order->add_order_note( sprintf( __( 'PayPal Pro (Payflow) delay charge complete (PNREF: %s)', 'ultimatewoo-pro' ), $parsed_response['PNREF'] ) );
		...
		protected function _capture_payment_paypal_pro_payflow_failed( $order, $parsed_response ) {
			...
			// Back to on-hold.
			$order->update_status( 'on-hold', sprintf( __( 'PayPal Pro (Payflow) capture failed (Correlation ID: %s). Capture was rejected due to an error: ', 'ultimatewoo-pro' ), $correlation_id ) . '(' . $parsed_response['RESULT'] . ') ' . '"' . $parsed_response['RESPMSG'] . '"' );
		}
		...
		public function cancel_payment( $order_id ) {
			...
			if ( 'paypal_pro' === $payment_method && $txn_id && 'no' === $captured ) {
				...
				if ( is_wp_error( $response ) ) {
					$order->add_order_note( __( 'Unable to void charge!', 'ultimatewoo-pro' ) . ' ' . $response->get_error_message() );

				} else {
					parse_str( $response['body'], $parsed_response );

					$order->add_order_note( sprintf( __( 'PayPal Pro void complete (Authorization ID: %s)', 'ultimatewoo-pro' ), $parsed_response['AUTHORIZATIONID'] ) );
			...
			if ( 'paypal_pro_payflow' === $payment_method && $txn_id && 'no' === $captured ) {
				...
				if ( is_wp_error( $response ) ) {
					...
					$order->add_order_note( sprintf( __( 'PayPal Pro (Payflow) void complete (PNREF: %s)', 'ultimatewoo-pro' ), $parsed_response['PNREF'] ) );
