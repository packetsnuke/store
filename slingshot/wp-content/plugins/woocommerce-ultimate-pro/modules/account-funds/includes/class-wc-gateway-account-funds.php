...
class WC_Gateway_Account_Funds extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_title       = __( 'Account Funds', 'ultimatewoo-pro' );
		$this->method_description = __( 'This gateway takes full payment using a logged in user\'s account funds.', 'ultimatewoo-pro' );
		$this->supports           = array(
			'products',
			'subscriptions',
		...
		$description = sprintf( __( "Available balance: %s", 'ultimatewoo-pro'), WC_Account_Funds::get_account_funds() );
		...
		if ( 'yes' === get_option( 'account_funds_give_discount' ) ) {
			...
			$description .= '<br/><em>' . sprintf( __( 'Use your account funds and get a %s discount on your order.', 'ultimatewoo-pro' ), $amount ) . '</em>';
		}
	...
	public function init_form_fields() {
		$this->form_fields = array(
			...
			'title' => array(
				...
				'default'     => __( 'Account Funds', 'ultimatewoo-pro' )
			)
		);
	}
	...
	public function process_payment( $order_id ) {
		...
		if ( ! is_user_logged_in() ) {
			wc_add_notice( __( 'Payment error:', 'ultimatewoo-pro' ) . ' ' . __( 'You must be logged in to use this payment method', 'ultimatewoo-pro' ), 'error' );
			return;
		}
		...
		if ( $available_funds < $order->get_total() ) {
			wc_add_notice( __( 'Payment error:', 'ultimatewoo-pro' ) . ' ' . __( 'Insufficient account balance', 'ultimatewoo-pro' ), 'error' );
			return;
		}
	...
	public function scheduled_subscription_payment( $amount, $order ) {
		...
		try {
			...
			if ( ! $user_id ) {
				throw new Exception( __( 'Customer not found', 'ultimatewoo-pro' ) );
			}

			$funds = WC_Account_Funds::get_account_funds( $user_id, false );
			if ( $amount > $funds ) {
				throw new Exception( __( 'Insufficient funds', 'ultimatewoo-pro' ) );
			}
			...
			$order->add_order_note( sprintf( __( 'Account funds payment applied: %s', 'ultimatewoo-pro' ), $amount ) );
	...
	public function set_renewal_order_meta( $renewal_order ) {
		...
		$renewal_order->add_order_note( __( 'Account Funds subscription payment completed', 'ultimatewoo-pro' ) );
	}
	...
	public function subscription_payment_method_name( $payment_method_to_display, $subscription_details, $order ) {
		...
		return sprintf( __( 'Via %s', 'ultimatewoo-pro' ), $this->method_title );
	}
}
