...
class WC_CSV_Import_Suite_Coupon_Import extends WC_CSV_Import_Suite_Importer {
	...
	public function __construct() {
		...
		$this->title = __( 'Import Coupons', 'ultimatewoo-pro' );
	...
	public function get_column_mapping_options() {
		...
		return array(
			...
			__( 'Coupon data', 'ultimatewoo-pro' ) => array(
				'code'                       => __( 'Coupon code', 'ultimatewoo-pro' ),
				'type'                       => __( 'Discount type', 'ultimatewoo-pro' ),
				'amount'                     => __( 'Coupon amount', 'ultimatewoo-pro' ),
				'individual_use'             => __( 'Individual use only', 'ultimatewoo-pro' ),
				'products'                   => __( 'Products', 'ultimatewoo-pro' ),
				'exclude_products'           => __( 'Exclude products', 'ultimatewoo-pro' ),
				'usage_limit'                => __( 'Usage limit per coupon', 'ultimatewoo-pro' ),
				'usage_limit_per_user'       => __( 'Usage limit per user', 'ultimatewoo-pro' ),
				'limit_usage_to_x_items'     => __( 'Limit usage to x items', 'ultimatewoo-pro' ),
				'usage_count'                => __( 'Usage count', 'ultimatewoo-pro' ),
				'expiry_date'                => __( 'Coupon expiry date', 'ultimatewoo-pro' ),
				'enable_free_shipping'       => __( 'Allow free shipping', 'ultimatewoo-pro' ),
				'product_categories'         => __( 'Categories', 'ultimatewoo-pro' ),
				'exclude_product_categories' => __( 'Exclude categories', 'ultimatewoo-pro' ),
				'exclude_sale_items'         => __( 'Exclude sale items', 'ultimatewoo-pro' ),
				'minimum_amount'             => __( 'Minimum spend', 'ultimatewoo-pro' ),
				'maximum_amount'             => __( 'Maximum spend', 'ultimatewoo-pro' ),
				'customer_emails'            => __( 'Email restrictions', 'ultimatewoo-pro' ),
				'description'                => __( 'Description', 'ultimatewoo-pro' ),
			),
	...
	protected function parse_item( $item, $options = array(), $raw_headers = array() ) {
		...
		if ( ! $coupon_code ) {
			throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_missing_coupon_code', __( 'Missing coupon code.', 'ultimatewoo-pro' ) );
		}
		...
		if ( $merging ) {
			...
			if ( ! $found_coupon ) {
				...
				if ( $insert_non_matching ) {
					wc_csv_import_suite()->log( sprintf( __( '> > Skipped. Cannot find coupon with code %s. Importing instead.', 'ultimatewoo-pro' ), esc_html( $coupon_code ) ) );
					$merging = false;
				} else {
					throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_cannot_find_coupon', sprintf( __( 'Cannot find coupon with code %s.', 'ultimatewoo-pro' ), esc_html( $coupon_code ) ) );
				}

			} else {
				/* translators: Placeholders: %s - coupon code */
				wc_csv_import_suite()->log( sprintf( __( "> > Found coupon with code '%s'.", 'ultimatewoo-pro' ), esc_html( $coupon_code ) ) );
		...
		if ( ! $merging ) {
			...
			if ( $found_coupon ) {
				/* translators: Placeholders: %s - coupon code */
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_coupon_code_already_exists', sprintf( __( "Coupon code '%s' already exists.", 'ultimatewoo-pro' ), esc_html( $coupon_code ) ) );
			}

			// check required fields
			if ( ! isset( $item['type'] ) || ! $item['type'] ) {
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_missing_coupon_type', __( "Missing coupon discount type.", 'ultimatewoo-pro' ) );
			}
		}
		...
		if ( isset( $item['type'] ) && $item['type'] ) {
			...
			foreach ( $discount_types as $key => $value ) {
				...
				if ( 0 === strcasecmp( $key, $item['type'] ) || 0 === strcasecmp( $value, __( $item['type'], 'ultimatewoo-pro' ) ) ) {
			...
			if ( ! $discount_type_is_valid ) {
				...
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_unknown_coupon_type', sprintf( __( "Unknown discount type '%s'.", 'ultimatewoo-pro' ), esc_html( $item['type'] ) ) );
			}
		}
			...
			switch ( $column ) {
				...
				case 'exclude_products':
					...
					foreach ( $skus as $sku ) {
						...
						if ( ! $product_id ) {
							...
							throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_unknown_product_sku', sprintf( __( 'Unknown product sku: %s.', 'ultimatewoo-pro' ), esc_html( $sku ) ) );
						}
				...
				case 'exclude_product_categories':
					...
					foreach ( $product_cats as $product_cat ) {
						...
						if ( ! $term ) {
							...
							throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_unknown_product_category', sprintf( __( 'Unknown product category: %s.', 'ultimatewoo-pro' ), esc_html( $product_cat ) ) );
						}
				...
				case 'exclude_sale_items':
					...
					if ( $val && 'yes' != $val && 'no' != $val ) {
						...
						throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_invalid_value', sprintf( __( "Column '%s' must be 'yes' or 'no'.", 'ultimatewoo-pro' ), esc_html( $column ) ) );
					}
				...
				case 'expiry_date':
					...
					if ( $val && false === strtotime( $val ) ) {
						...
						throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_invalid_date_format', sprintf( __( "Invalid date format '%s'", 'ultimatewoo-pro' ), esc_html( $item[ $column ] ) ) );
					}
				break;
				...
				case 'limit_usage_to_x_items':
					...
					if ( ! empty( $val ) && ! is_numeric( $val ) ) {
						...
						throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_invalid_value', sprintf( __( 'Invalid %1$s \'%2$s\'.', 'ultimatewoo-pro' ), esc_html( $column ), esc_html( $val ) ) );
					}
				break;
	...
	protected function process_item( $data, $options = array(), $raw_headers = array() ) {
		...
		wc_csv_import_suite()->log( __( 'Processing coupon.', 'ultimatewoo-pro' ) );
		...
		if ( $merging ) {
			...
			wc_csv_import_suite()->log( sprintf( __( "> Merging coupon '%s'.", 'ultimatewoo-pro' ), esc_html( $data['code'] ) ) );
		...
		// importing
			...
			// insert coupon
			wc_csv_import_suite()->log( sprintf( __( "> Inserting coupon '%s'", 'ultimatewoo-pro' ), esc_html( $data['code'] ) ) );
		...
		$message = $merging
						 ...
						 : __( '> Finished importing coupon %s.', 'ultimatewoo-pro' );
	...
	public function create_coupon( $data, $options = array() ) {
		...
		try {
			...
			if ( ! isset( $data['code'] ) ) {
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_missing_coupon_code', sprintf( __( 'Missing parameter %s', 'ultimatewoo-pro' ), 'code' ) );
			}
			...
			if ( $coupon_found ) {
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_coupon_code_already_exists', __( 'The coupon code already exists', 'ultimatewoo-pro' ) );
			}
			...
			if ( ! in_array( wc_clean( $coupon_data['type'] ), array_keys( wc_get_coupon_types() ) ) ) {
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_invalid_coupon_type', sprintf( __( 'Invalid coupon type - the coupon type must be any of these: %s', 'ultimatewoo-pro' ), implode( ', ', array_keys( wc_get_coupon_types() ) ) ) );
			}
	...
	public function update_coupon( $id, $data, $options = array() ) {
		...
		try {
			...
			if ( empty( $id ) ) {
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_invalid_coupon_id', __( 'Invalid coupon ID', 'ultimatewoo-pro' ) );
			}

			$post = get_post( $id );

			if ( null === $post ) {
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_no_coupon_found', sprintf( __( 'No coupon found with the ID equal to %s', 'ultimatewoo-pro' ), $id ) );
			}

			// validate post type
			if ( 'shop_coupon' !== $post->post_type ) {
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_invalid_coupon', __( 'Invalid coupon', 'ultimatewoo-pro' ) );
			}
