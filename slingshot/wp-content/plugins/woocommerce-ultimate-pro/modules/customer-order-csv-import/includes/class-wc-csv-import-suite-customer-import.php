...
class WC_CSV_Import_Suite_Customer_Import extends WC_CSV_Import_Suite_Importer {
	...
	public function __construct() {
		...
		$this->title = __( 'Import Customers', 'ultimatewoo-pro' );
	...
	public function get_column_mapping_options() {
		...
		$shipping_prefix = __( 'Shipping: %s', 'ultimatewoo-pro' );

		return array(

			__( 'User data', 'ultimatewoo-pro' ) => array(
				'id'              => __( 'User ID', 'ultimatewoo-pro' ),
				'username'        => __( 'Username', 'ultimatewoo-pro' ),
				'email'           => __( 'Email', 'ultimatewoo-pro' ),
				'password'        => __( 'Password', 'ultimatewoo-pro' ),
				'date_registered' => __( 'Registered date', 'ultimatewoo-pro' ),
				'role'            => __( 'Role', 'ultimatewoo-pro' ),
				'url'             => __( 'URL', 'ultimatewoo-pro' ),
			),

			__( 'Customer data', 'ultimatewoo-pro' ) => array(
				'billing_first_name'  => sprintf( $billing_prefix,  __( 'First name', 'ultimatewoo-pro' ) ),
				'billing_last_name'   => sprintf( $billing_prefix,  __( 'Last name', 'ultimatewoo-pro' ) ),
				'billing_company'     => sprintf( $billing_prefix,  __( 'Company', 'ultimatewoo-pro' ) ),
				'billing_address_1'   => sprintf( $billing_prefix,  __( 'Address 1', 'ultimatewoo-pro' ) ),
				'billing_address_2'   => sprintf( $billing_prefix,  __( 'Address 2', 'ultimatewoo-pro' ) ),
				'billing_city'        => sprintf( $billing_prefix,  __( 'City', 'ultimatewoo-pro' ) ),
				'billing_state'       => sprintf( $billing_prefix,  __( 'State', 'ultimatewoo-pro' ) ),
				'billing_postcode'    => sprintf( $billing_prefix,  __( 'Postcode', 'ultimatewoo-pro' ) ),
				'billing_country'     => sprintf( $billing_prefix,  __( 'Country', 'ultimatewoo-pro' ) ),
				'billing_email'       => sprintf( $billing_prefix,  __( 'Email', 'ultimatewoo-pro' ) ),
				'billing_phone'       => sprintf( $billing_prefix,  __( 'Phone', 'ultimatewoo-pro' ) ),
				'shipping_first_name' => sprintf( $shipping_prefix, __( 'First name', 'ultimatewoo-pro' ) ),
				'shipping_last_name'  => sprintf( $shipping_prefix, __( 'Last name', 'ultimatewoo-pro' ) ),
				'shipping_company'    => sprintf( $shipping_prefix, __( 'Company', 'ultimatewoo-pro' ) ),
				'shipping_address_1'  => sprintf( $shipping_prefix, __( 'Address 1', 'ultimatewoo-pro' ) ),
				'shipping_address_2'  => sprintf( $shipping_prefix, __( 'Address 2', 'ultimatewoo-pro' ) ),
				'shipping_city'       => sprintf( $shipping_prefix, __( 'City', 'ultimatewoo-pro' ) ),
				'shipping_state'      => sprintf( $shipping_prefix, __( 'State', 'ultimatewoo-pro' ) ),
				'shipping_postcode'   => sprintf( $shipping_prefix, __( 'Postcode', 'ultimatewoo-pro' ) ),
				'shipping_country'    => sprintf( $shipping_prefix, __( 'Country', 'ultimatewoo-pro' ) ),
				'paying_customer'     => __( 'Paying customer', 'ultimatewoo-pro' ),
			),
	...
	public function advanced_import_options() {
		...
		<tr>
			<th scope="row">
				<?php esc_html_e( 'Shipping Address', 'ultimatewoo-pro' ); ?>
			</th>
			<td>
				<label>
					...
					<?php esc_html_e( 'Use billing address as shipping address if not set', 'ultimatewoo-pro' ); ?>
				</label>
			</td>
		</tr>
		...
		<tr>
			<th scope="row">
				<?php esc_html_e( "Don't hash user passwords", 'ultimatewoo-pro' ); ?>
			</th>
			<td>
				<label>
					<input type="checkbox" value="1" name="options[hashed_passwords]" id="wc-csv-import-suite-customer-hashed-passwords" />
					<?php esc_html_e( 'Enable this if your customer import file contains passwords which are already correctly hashed for WordPress.', 'ultimatewoo-pro' ); ?>
				</label>
			</td>
		</tr>
		...
		<tr>
			<th scope="row">
				<?php esc_html_e( 'Send emails', 'ultimatewoo-pro' ); ?>
			</th>
			<td>
				<label>
					<input type="checkbox" value="1" name="options[send_welcome_emails]" id="wc-csv-import-suite-customer-send-emails" />
					<?php esc_html_e( 'Send welcome emails to new customers and password update emails to updated customers after import.', 'ultimatewoo-pro' ); ?>
				</label>
			</td>
		</tr>
	...
	protected function parse_item( $item, $options = array(), $raw_headers = array() ) {
		...
		$preparing = $merging ? __( '> Row %s - preparing for merge.', 'ultimatewoo-pro' ) : __( '> Row %s - preparing for import.', 'ultimatewoo-pro' );
		...
		if ( ! $merging ) {
			...
			if ( ! $email ) {
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_missing_customer_email', __( 'No email set for new customer.', 'ultimatewoo-pro' ) );
			}
			...
			if ( $user_exists ) {
				...
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_customer_already_exists', sprintf( __( 'Customer %s already exists.', 'ultimatewoo-pro' ), $identifier ) );
			}
		}

		// validate username
		if ( $username && ! validate_username( $username ) ) {
			throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_invalid_username', sprintf( __( 'Invalid username: %s', 'ultimatewoo-pro' ), esc_html( $username) ) );
		}

		// validate email
		if ( $email && ! is_email( $email ) ) {
			throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_invalid_email', sprintf( __( 'Invalid email: %s', 'ultimatewoo-pro' ), esc_html( $email ) ) );
		}
		...
		if ( isset( $item['role'] ) && $item['role'] ) {
			...
			if ( ! isset( $wp_roles->role_names[ $item['role'] ] ) ) {
				...
				if ( ! $found_role_by_name ) {
					throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_invalid_role', sprintf( __( 'Role "%s" not found.', 'ultimatewoo-pro' ), $item['role'] ) );
				}
			}
		}
	...
	protected function find_registered_customer( $customer_id, $username, $email, $insert_non_matching ) {
		...
		if ( ! $customer_id && ! $username && ! $email ) {
			...
			$message = __( '> > Cannot merge without id, email, or username.', 'ultimatewoo-pro' );

			if ( $insert_non_matching ) {
				$message .= ' ' .  __( 'Importing instead.', 'ultimatewoo-pro' );
			}
		...
		if ( $customer_id ) {
			...
			if ( ! $found_customer ) {
				...
				if ( ! $username && ! $email ) {
					...
					if ( $insert_non_matching ) {
						wc_csv_import_suite()->log( sprintf( __( '> > Skipped. Cannot find customer with id %s. Importing instead.', 'ultimatewoo-pro' ), $customer_id ) );
						$merging = false;
					} else {
						throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_cannot_find_customer', sprintf( __( 'Cannot find customer with id %s.', 'ultimatewoo-pro' ), $customer_id ) );
					}

				} else {
					// we can keep trying with username and/or email
					wc_csv_import_suite()->log( sprintf( __( '> > Cannot find customer with id %s.', 'ultimatewoo-pro' ), $customer_id ) );
				}

			} else {
				wc_csv_import_suite()->log( sprintf( __( '> > Found user with id %s.', 'ultimatewoo-pro' ), $customer_id ) );
			}
		}
		...
		if ( ! $found_customer && $username ) {
			...
			if ( ! $found_customer ) {
				...
				if ( ! $email ) {
					...
					if ( $insert_non_matching ) {
						wc_csv_import_suite()->log( sprintf( __( '> > Skipped. Cannot find customer with username %s. Importing instead.', 'ultimatewoo-pro' ), $username ) );
						$merging = false;
					} else {
						throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_cannot_find_customer', sprintf( __( 'Cannot find customer with username %s.', 'ultimatewoo-pro' ), $username ) );
					}

				} else {
					// We can keep trying with email
					wc_csv_import_suite()->log( sprintf( __( '> > Cannot find customer with username %s.', 'ultimatewoo-pro' ), $username ) );
				}

			} else {
				wc_csv_import_suite()->log( sprintf( __( '> > Found user with username %s.', 'ultimatewoo-pro' ), $username ) );
		...
		if ( ! $found_customer && $email ) {
			...
			if ( ! $found_customer ) {
				...
				if ( $insert_non_matching ) {
					wc_csv_import_suite()->log( sprintf( __( '> > Skipped. Cannot find customer with email %s. Importing instead.', 'ultimatewoo-pro' ), $email ) );
					$merging = false;
				} else {
					throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_cannot_find_customer', sprintf( __( 'Cannot find customer with email %s.', 'ultimatewoo-pro' ), $email ) );
				}

			} else {
				wc_csv_import_suite()->log( sprintf( __( '> > Found user with email %s.', 'ultimatewoo-pro' ), $email ) );
				$customer_id = $found_customer;
			}
		}
	...
	protected function process_item( $data, $options = array(), $raw_headers = array() ) {
		...
		wc_csv_import_suite()->log( __( 'Processing customer.', 'ultimatewoo-pro' ) );

		$user_id = null;

		if ( $merging ) {

			wc_csv_import_suite()->log( sprintf( __( '> Merging customer ID %s.', 'ultimatewoo-pro' ), $data['id'] ) );
			...
			// insert customer
			wc_csv_import_suite()->log( sprintf( __( '> Inserting customer %s', 'ultimatewoo-pro' ), esc_html( $data['username'] ) ) );
		...
		$message = $merging
						 ...
						 : __( '> Finished importing customer ID %s.', 'ultimatewoo-pro' );
	...
	public function create_customer( $data, $options ) {
		...
		try {
			...
			if ( ! isset( $data['email'] ) ) {
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_missing_customer_email', sprintf( __( 'Missing parameter %s', 'ultimatewoo-pro' ), 'email' ) );
			}
	...
	public function update_customer( $id, $data, $options ) {
		...
		try {
			...
			if ( empty( $id ) ) {
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_invalid_customer_id', __( 'Invalid customer ID', 'ultimatewoo-pro' ) );
			}
			...
			if ( 0 === $customer->ID ) {
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_invalid_customer', __( 'Invalid customer', 'ultimatewoo-pro' ) );
			}
