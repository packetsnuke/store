...
class WC_CSV_Import_Suite_Order_Import extends WC_CSV_Import_Suite_Importer {
	...
	public function __construct() {
		...
		$this->title = __( 'Import Orders', 'ultimatewoo-pro' );
	...
	public function get_column_mapping_options() {
		...
		return array(
			...
			__( 'Order data', 'ultimatewoo-pro' ) => array(
				'order_id'                     => __( 'Order ID', 'ultimatewoo-pro' ),
				'order_number_formatted'       => __( 'Formatted order number', 'ultimatewoo-pro' ),
				'order_number'                 => __( 'Order number', 'ultimatewoo-pro' ),
				'date'                         => __( 'Date', 'ultimatewoo-pro' ),
				'status'                       => __( 'Status', 'ultimatewoo-pro' ),
				'currency'                     => __( 'Currency', 'ultimatewoo-pro' ),
				'shipping_total'               => __( 'Shipping total', 'ultimatewoo-pro' ),
				'shipping_tax_total'           => __( 'Shipping tax total', 'ultimatewoo-pro' ),
				'fee_total'                    => __( 'Fees total', 'ultimatewoo-pro' ),
				'fee_tax_total'                => __( 'Fees tax total', 'ultimatewoo-pro' ),
				'tax_total'                    => __( 'Tax total', 'ultimatewoo-pro' ),
				'cart_discount'                => __( 'Discount', 'ultimatewoo-pro' ),
				'order_total'                  => __( 'Order total', 'ultimatewoo-pro' ),
				'refunded_total'               => __( 'Total refunded amount', 'ultimatewoo-pro' ),
				'payment_method'               => __( 'Payment method', 'ultimatewoo-pro' ),
				'shipping_method'              => __( 'Shipping method', 'ultimatewoo-pro' ),
				'customer_note'                => __( 'Customer note', 'ultimatewoo-pro' ),
				'order_notes'                  => __( 'Order notes', 'ultimatewoo-pro' ),
				'download_permissions_granted' => __( 'Download Permissions Granted', 'ultimatewoo-pro' ),
			),

			__( 'Order items', 'ultimatewoo-pro' ) => array(
				'line_items'                   => __( 'Line items', 'ultimatewoo-pro' ),
				'shipping_items'               => __( 'Shipping items', 'ultimatewoo-pro' ),
				'tax_items'                    => __( 'Taxes', 'ultimatewoo-pro' ),
				'fee_items'                    => __( 'Fees', 'ultimatewoo-pro' ),
				'coupons'                      => __( 'Coupons', 'ultimatewoo-pro' ),
			),

			__( 'Customer data', 'ultimatewoo-pro' ) => array(
				'customer_user'       => __( 'Customer user (ID, username or email)', 'ultimatewoo-pro' ),
			...
			'refunds' => __( 'Refunds', 'ultimatewoo-pro' ),
		);
	}
	...
	public function advanced_column_mapping_options( $options, $importer, $headers, $raw_headers, $columns ) {
		...
		if ( 'woocommerce_order_csv' == $importer ) {
			...
			$group  = __( 'Order items', 'ultimatewoo-pro' );
			...
			switch ( $format ) {
				...
				case 'csv_import_legacy':
					...
					foreach ( $columns as $column => $value ) {
						...
						if ( SV_WC_Helper::str_starts_with( $column, 'order_item_' ) ) {
							...
							$order_item_options[ $column ] = sprintf( __( 'Order item %d', 'ultimatewoo-pro' ), $number );
						}
						...
						elseif ( SV_WC_Helper::str_starts_with( $column, 'tax_item_' ) ) {
							...
							$parts                       = explode( '_', $column );
							$number                      = array_pop( $parts );
							$tax_item_options[ $column ] = sprintf( __( 'Tax item %d', 'ultimatewoo-pro' ), $number );
						}
						...
						elseif ( SV_WC_Helper::str_starts_with( $column, 'shipping_method_' ) ) {
							...
							$parts                       = explode( '_', $column );
							$number                      = array_pop( $parts );
							$shipping_method_options[ $column ] = sprintf( __( 'Shipping method %d', 'ultimatewoo-pro' ), $number );
						}
						...
						elseif ( SV_WC_Helper::str_starts_with( $column, 'shipping_cost_' ) ) {
							...
							$parts                       = explode( '_', $column );
							$number                      = array_pop( $parts );
							$shipping_cost_options[ $column ] = sprintf( __( 'Shipping cost %d', 'ultimatewoo-pro' ), $number );
						}
				...
				case 'csv_export_default_one_row_per_item':
					...
					$new_options = array(
						'item_name'      => __( 'Item name', 'ultimatewoo-pro' ),
						'item_sku'       => __( 'Item SKU', 'ultimatewoo-pro' ),
						'item_quantity'  => __( 'Item quantity', 'ultimatewoo-pro' ),
						'item_total_tax' => __( 'Item tax', 'ultimatewoo-pro' ),
						'item_total'     => __( 'Item total', 'ultimatewoo-pro' ),
						'item_meta'      => __( 'Item meta', 'ultimatewoo-pro' ),
					);
				...
				case 'csv_export_legacy_one_row_per_item':
					...
					$new_options = array(
						...
						'item_name'      => __( 'Item name', 'ultimatewoo-pro' ),
						'item_meta'      => __( 'Item meta', 'ultimatewoo-pro' ),
						'item_quantity'  => __( 'Item quantity', 'ultimatewoo-pro' ), // Item Amount
						'item_total'     => __( 'Item total', 'ultimatewoo-pro' ), // Row Price
					);
	...
	public function advanced_import_options() {
		...
		<tr>
			<th scope="row">
				<?php esc_html_e( 'Allow unknown products', 'ultimatewoo-pro' ); ?>
			</th>
			<td>
				<label>
					<input type="checkbox" value="1" name="options[allow_unknown_products]" id="wc-csv-import-suite-order-allow-unknown-products" />
					<?php esc_html_e( 'Allow line items with unknown product sku/id. The line item will not be linked to any product, so this is not necessarily recommended.', 'ultimatewoo-pro' ); ?>
				</label>
			</td>
		</tr>
		...
		<tr>
			<th scope="row">
				<?php esc_html_e( 'Re-calculate taxes & totals', 'ultimatewoo-pro' ); ?>
			</th>
			<td>
				<label>
					<input type="checkbox" value="1" name="options[recalculate_totals]" id="wc-csv-import-suite-order-recalculate-totals" />
					<?php esc_html_e( 'Re-calculate taxes and totals after importing the order. This may result in different tax and order totals than in the CSV file.', 'ultimatewoo-pro' ); ?>
				</label>
			</td>
		</tr>
		...
		<tr>
			<th scope="row">
				<?php esc_html_e( 'Use addresses from customer profile', 'ultimatewoo-pro' ); ?>
			</th>
			<td>
				<label>
					...
					<?php esc_html_e( 'Use addresses from the customer profile if no address columns are present in the CSV file.', 'ultimatewoo-pro' ); ?>
				</label>
			</td>
		</tr>
	...
	public function parse_json( $string ) {
		...
		switch ( json_last_error() ) {
			...
			case JSON_ERROR_DEPTH:
				$error = esc_html__( 'The maximum stack depth has been exceeded.', 'ultimatewoo-pro' );
			break;

			case JSON_ERROR_STATE_MISMATCH:
				$error = esc_html__( 'Invalid or malformed JSON.', 'ultimatewoo-pro' );
			break;

			case JSON_ERROR_CTRL_CHAR:
				$error = esc_html__( 'Control character error, possibly incorrectly encoded.', 'ultimatewoo-pro' );
			break;

			case JSON_ERROR_SYNTAX:
				$error = esc_html__( 'Syntax error, malformed JSON.', 'ultimatewoo-pro' );
			break;

			// PHP >= 5.3.3
			case JSON_ERROR_UTF8:
				$error = esc_html__( 'Malformed UTF-8 characters, possibly incorrectly encoded.', 'ultimatewoo-pro' );
			break;

			// PHP >= 5.5.0
			case JSON_ERROR_RECURSION:
				$error = esc_html__( 'One or more recursive references in the value to be encoded.', 'ultimatewoo-pro' );
			break;

			// PHP >= 5.5.0
			case JSON_ERROR_INF_OR_NAN:
				$error = esc_html__( 'One or more NAN or INF values in the value to be encoded.', 'ultimatewoo-pro' );
			break;

			case JSON_ERROR_UNSUPPORTED_TYPE:
				$error = esc_html__( 'A value of a type that cannot be encoded was given.', 'ultimatewoo-pro' );
			break;

			default:
				$error = esc_html__( 'Unknown JSON error occured.', 'ultimatewoo-pro' );
			break;
		}
	...
	public function parse_order_date( $item ) {
		...
		if ( ! empty( $item['date'] ) ) {
			...
			if ( false === ( $item['date'] = strtotime( $item['date'] ) ) ) {
				...
				// invalid date format
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_invalid_date_format', sprintf( __( 'Invalid date format %s.', 'ultimatewoo-pro' ), $item['date'] ) );
			} else {
				$date = $item['date'];
			}
		}
