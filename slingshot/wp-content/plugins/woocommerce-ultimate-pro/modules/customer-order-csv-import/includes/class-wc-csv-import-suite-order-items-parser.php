...
class WC_CSV_Import_Suite_Order_Items_Parser {
			...
			case 'default':
				...
				if ( $this->importer->is_possibly_json_array( $item['line_items'] ) ) {
					...
					try {
						...
						throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_line_items_importer->parse_json_error', sprintf( esc_html__( 'Error while parsing line items for line %d: %s', 'ultimatewoo-pro' ), $this->importer->get_line_num(), $e->getMessage() ) );
					}
				...
				if ( ! empty( $item['order_item_1'] ) ) {
					...
					while ( ! empty( $item[ 'order_item_' . $i ] ) ) {
						...
						if ( ! $product_identifier || ! $qty || ! is_numeric( $total ) ) {
							// invalid item
							throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_missing_sku_quantity_or_email', sprintf( esc_html__( 'Missing SKU, quantity or total for %s on line %s.', 'ultimatewoo-pro' ), 'order_item_' . $i, $this->importer->get_line_num() ) );
						}
						...
						if ( ! $allow_unknown_products && ! $product_id ) {
							throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_unknown_order_item', sprintf( esc_html__( 'Unknown order item: %s.', 'ultimatewoo-pro' ), $product_identifier ) );
						}
	...
	private function parse_line_item( $item, $allow_unknown_products = false, $tax_items ) {
		...
		if ( ! $product_identifier || ! $qty || ! is_numeric( $total ) ) {
			...
			// invalid item
			throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_missing_product_id_sku_quantity_or_total', sprintf( __( 'Missing product ID/SKU, quantity or total for order item on line %s.', 'ultimatewoo-pro' ), $this->importer->get_line_num() ) );
		}
		...
		if ( ! $allow_unknown_products && ! $product_id ) {
			throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_unknown_order_item', sprintf( __( 'Unknown order item: %s.', 'ultimatewoo-pro' ), $product_identifier ) );
		}
	...
	private function parse_csv_export_line_item( $item, $allow_unknown_products = false, $meta_key_value_separator = '=' ) {
		...
		if ( ! $sku || ! $qty || ! is_numeric( $total ) ) {
			...
			// invalid item
			throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_missing_sku_quantity_or_total', sprintf( __( 'Missing SKU, quantity or total for order item on line %s', 'ultimatewoo-pro' ), $this->importer->get_line_num() ) );
		}
		...
		if ( ! $allow_unknown_products && ! $product_id ) {
			...
			throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_unknown_order_item', sprintf( __( 'Unknown order item: %s.', 'ultimatewoo-pro' ), $sku ) );
		}
		...
		if ( ! empty( $item['shipping_items'] ) ) {
			...
			if ( $this->importer->is_possibly_json_array( $item['shipping_items'] ) ) {
				...
				try {
					...
					throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_shipping_items_importer->parse_json_error', sprintf( esc_html__( 'Error while parsing shipping items for line %d: %s', 'ultimatewoo-pro' ), $this->importer->get_line_num(), $e->getMessage() ) );
				}
	...
	public function parse_tax_items( $item, &$tax_total = null, &$shipping_tax_total = null, $merging = false ) {
		...
		// default format
			...
			if ( $this->importer->is_possibly_json_array( $item['tax_items'] ) ) {
				...
				try {
					...
					throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_tax_items_importer->parse_json_error', sprintf( esc_html__( 'Error while parsing taxes for line %d: %s', 'ultimatewoo-pro' ), $this->importer->get_line_num(), $e->getMessage() ) );
				}
		...
		if ( 0 === count( $tax_items ) && ( $tax_total || $shipping_tax_total ) && ! $merging ) {
			...
			$tax_items[] = array(
				'code'                => '',
				'rate_id'             => 0,
				'label'               => esc_html__( 'Tax', 'ultimatewoo-pro' ),
				'compound'            => '',
	...
	private function parse_tax_item( $tax_item_data ) {
		...
		if ( ! isset( $tax_item_data['label'] ) || ! $tax_item_data['label'] ) {
			$tax_item_data['label'] = esc_html__( 'Tax', 'ultimatewoo-pro' );
		}
	...
	public function parse_fee_items( $item, $tax_items, $merging = false ) {
		...
		if ( ! empty( $item['fee_items'] ) ) {
			...
			if ( $this->importer->is_possibly_json_array( $item['fee_items'] ) ) {
				...
				try {
					...
					throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_fee_items_importer->parse_json_error', sprintf( esc_html__( 'Error while parsing fees for line %d: %s', 'ultimatewoo-pro' ), $this->importer->get_line_num(), $e->getMessage() ) );
				}
			...
			foreach ( $raw_fee_items as $raw_fee_item ) {
				...
				$fee_item = array(
					...
					'title'         => $this->get_array_key_value( $raw_fee_item, 'name', esc_html__( 'Fee', 'ultimatewoo-pro' ) ),
		...
		if ( empty( $fee_items ) && $fee_total > 0 && ! $merging ) {
			...
			$fee_items[] = array(
				'title'          => esc_html__( 'Fee', 'ultimatewoo-pro' ),
				'total'          => $fee_total,
		...
		if ( isset( $item['coupons'] ) && ! empty( $item['coupons'] ) ) {
			...
			if ( $this->importer->is_possibly_json_array( $item['coupons'] ) ) {
				...
				try {
					...
					throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_coupons_importer->parse_json_error', sprintf( esc_html__( 'Error while parsing coupons for line %d: %s', 'ultimatewoo-pro' ), $this->importer->get_line_num(), $e->getMessage() ) );
				}
			...
			foreach ( $_coupons as $_coupon_data ) {
				...
				if ( ! $this->get_array_key_value( $_coupon_data, 'code' ) ) {
					throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_missing_coupon_code', sprintf( esc_html__( 'Missing coupon code on line %d', 'ultimatewoo-pro' ), $this->importer->get_line_num() ) );
				}
		...
		if ( ! empty( $item['refunds'] ) ) {
			...
			if ( $this->importer->is_possibly_json_array( $item['refunds'] ) ) {
				...
				try {
					...
					throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_refunds_importer->parse_json_error', sprintf( esc_html__( 'Error while parsing refunds for line %d: %s', 'ultimatewoo-pro' ), $this->importer->get_line_num(), $e->getMessage() ) );
				}
		...
		foreach ( $items as $key => $item ) {
			...
			if ( $order_item_id ) {
				...
				switch ( $type ) {
					...
					case 'line_item':
						$message = sprintf( __( '> > Cannot update product "%s" without order item ID. Inserting instead.', 'ultimatewoo-pro' ), $item['product_id'] );
					break;

					case 'coupon':
						$message = sprintf( __( '> > Cannot update coupon "%s" without order item ID. Inserting instead.', 'ultimatewoo-pro' ), $item['code'] );
					break;

					case 'fee':
						$message = sprintf( __( '> > Cannot update fee "%s" without order item ID. Inserting instead.', 'ultimatewoo-pro' ), $item['title'] );
					break;

					case 'shipping':
						$message = sprintf( __( '> > Cannot update shipping method "%s" without order item ID. Inserting instead.', 'ultimatewoo-pro' ), $item['title'] );
					break;

					case 'tax':
						$message = sprintf( __( '> > Cannot update tax rate "%s" without order item ID. Inserting instead.', 'ultimatewoo-pro' ), $item['code'] );
					break;
				}
			...
			if ( ! empty( $matches ) ) {
				...
				if ( count( $matches ) > 1 ) {
					...
					switch ( $type ) {
						...
						case 'line_item':
							$message = sprintf( __( '> > Cannot update product "%s" without order item ID, as multiple similar products exist for the order and no direct match could be determined. Skipping merging line items.', 'ultimatewoo-pro' ), $item['name'] );
						break;

						case 'coupon':
							$message = sprintf( __( '> > Cannot update coupon "%s" without order item ID, as multiple similar coupons exist for the order and no direct match could be determined. Skipping merging coupons.', 'ultimatewoo-pro' ), $item['code'] );
						break;

						case 'fee':
							$message = sprintf( __( '> > Cannot update fee "%s" without order item ID, as multiple similar fees exist for the order and no direct match could be determined. Skipping merging fees.', 'ultimatewoo-pro' ), $item['title'] );
						break;

						case 'shipping':
							$message = sprintf( __( '> > Cannot update shipping method "%s" without order item ID, as multiple similar shipping methods exist for the order and no direct match could be determined. Skipping merging shipping methods.', 'ultimatewoo-pro' ), $item['title'] );
						break;

						case 'tax':
							$message = sprintf( __( '> > Cannot update tax rate "%s" without order item ID, as multiple similar tax rates exist for the order and no direct match could be determined. Skipping merging tax rates.', 'ultimatewoo-pro' ), $item['code'] );
						break;

						default:
							$message = __( 'Cannot determine a unique match for order item', 'ultimatewoo-pro' );
						break;
					}
