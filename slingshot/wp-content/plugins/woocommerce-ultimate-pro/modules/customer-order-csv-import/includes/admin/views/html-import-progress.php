...
<div class="woocommerce">
	...
	<p><?php printf( $is_complete ? esc_html__( 'Imported file: %s', 'ultimatewoo-pro' ) : esc_html__( 'Importing file: %s', 'ultimatewoo-pro' ), $filename ); ?></p>

	<?php if ( isset( $_GET['block_new_import'] ) && 'yes' === $_GET['block_new_import'] ) : ?>
		<div class="notice notice-error">
			<p><?php /* translators: Placeholders: %1$s - <strong>, %2$s - </strong> */
				printf( esc_html__( '%1$sAn import is currently in progress.%2$s Please wait for the current import to complete before beginning another.', 'ultimatewoo-pro' ), '<strong>', '</strong>' );
			?></p>
		</div>
	<?php endif; ?>
	...
	<?php if ( $is_complete ) : ?>
		<div class="notice notice-<?php echo $some_skipped_or_failed ? 'warning' : 'success'; ?> notice-complete">
			<p><?php esc_html_e( 'Import complete.', 'ultimatewoo-pro' ); ?><?php if ( $some_skipped_or_failed ) : echo ' ' . esc_html__( 'Some lines were skipped or failed to import. See below for details.', 'ultimatewoo-pro' ) ; endif; ?></p>
		</div>
	<?php else: ?>
		<div class="notice notice-success js-notice-safe-to-leave-screen">
			<p><?php esc_html_e( 'Import has started. You can safely leave this page - you will see a notice in the admin area once the import is complete.', 'ultimatewoo-pro' ); ?></p>
		</div>
	<?php endif; ?>
	...
	<?php if ( $options['dry_run'] ) : ?>
		<div class="notice notice-warning notice-dry-run">
			<p>
				...
				<?php else : ?>
					<?php esc_html_e( 'Performing a dry run with the selected file. No database records will be inserted or updated.', 'ultimatewoo-pro' ) ; ?>
				<?php endif; ?>
			</p>
		</div>
	<?php endif; ?>
	...
	<div id="wc-csv-import-suite-progress">
		...
		<div class="progress-count">
			<?php esc_html_e( 'Processed:', 'ultimatewoo-pro' ); ?> <?php printf( $csv_importer->i18n['count'], '<span class="processed-count">' . count( $results ) . '</span>' ); ?>
		</div>
	...
	<div id="wc-csv-import-suite-results">
		...
		<h3><?php esc_html_e( 'Results', 'ultimatewoo-pro' ); ?></h3>
				...
				<div class="chart-sidebar">
					...
					<ul class="chart-widgets">
						<li class="chart-widget details-widget js-wc-csv-import-suite-results-details-link-widget" <?php if ( ! $some_skipped_or_failed ) : echo 'style="display:none;"'; endif; ?>>
							<a class="js-wc-csv-import-suite-toggle-results-details details-toggle" href="#"><span class="dashicons dashicons-list-view"></span> <span class="js-wc-csv-import-suite-toggle-results-details-text"><?php esc_html_e( 'View detailed results', 'ultimatewoo-pro' ); ?></span></a>
						</li>
					</ul>
				</div>
		...
		<table class="widefat results-details">
			<thead>
				<tr>
					<th><?php esc_html_e( 'Line', 'ultimatewoo-pro' ); ?></th>
