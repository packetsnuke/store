...
<h3><?php esc_html_e( 'Import data from CSV file', 'ultimatewoo-pro' ); ?></h3>

<?php if ( ! empty( $upload_dir['error'] ) ) : ?>

	<div class="error">
		<p><?php esc_html_e( 'Before you can start importing, you will need to fix the following error:', 'ultimatewoo-pro' ); ?></p>
...
<?php else : ?>
	...
	<table class="form-table">
		...
		<tr>
			<th>
				<label for="import"><?php esc_html_e( 'Choose a file from your computer:', 'ultimatewoo-pro' ); ?></label>
			</th>
			<td>
				<input type="file" id="import" name="import" />
				<input type="hidden" name="max_file_size" value="<?php echo esc_attr( $bytes ); ?>" />
				<p class="description"><small><?php printf( esc_html__( 'Acceptable file types: CSV or tab-delimited text files. Maximum size: %1$s', 'ultimatewoo-pro' ), $size ); ?></small></p>
			</td>
		</tr>
