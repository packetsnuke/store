 ...
 */
 class WC_CSV_Import_Suite_List_Table extends WP_List_Table {
	...
	public function __construct( $args = array() ) {
		...
		$this->statuses = array(
			'queued'     => esc_html__( 'Queued', 'ultimatewoo-pro' ),
			'processing' => esc_html__( 'Processing', 'ultimatewoo-pro' ),
			'completed'  => esc_html__( 'Completed', 'ultimatewoo-pro' ),
	...
	public function get_columns() {
		...
		$columns = array(
			...
			'import_status'   => '<span class="status_head tips" data-tip="' . esc_attr__( 'Import Status', 'ultimatewoo-pro' ) . '">' . esc_attr__( 'Import Status', 'ultimatewoo-pro' ) . '</span>',
			'import_type'     => esc_html__( 'Type', 'ultimatewoo-pro' ),
			'run_type'        => esc_html__( 'Run Type', 'ultimatewoo-pro' ),
			'filename'        => esc_html__( 'File name', 'ultimatewoo-pro' ),
			'import_date'     => esc_html__( 'Start Date', 'ultimatewoo-pro' ),
			'file_actions'    => esc_html__( 'Actions', 'ultimatewoo-pro' ),
		);
		...
		switch ( $column_name ) {
			...
			case 'import_type':
				...
				if ( 'woocommerce_order_csv' === $import->type ) {
					...
					return esc_html__( 'Orders', 'ultimatewoo-pro' );

				} elseif ( 'woocommerce_customer_csv' === $import->type ) {

					return esc_html__( 'Customers', 'ultimatewoo-pro' );

				} elseif ( 'woocommerce_coupon_csv' === $import->type ) {

					return esc_html__( 'Coupons', 'ultimatewoo-pro' );
				}
			...
			case 'run_type':
				...
				return $import->options['dry_run'] ? esc_html__( 'Dry', 'ultimatewoo-pro' ) : esc_html__( 'Live', 'ultimatewoo-pro' );
	...
	public function column_file_actions( $import ) {
		...
		?><p>
			<?php
				...
				if ( ! in_array( $import->status, array( 'failed', 'queued' ), true ) ) {
					...
					$actions['view'] = array(
						'url'    => $view_url,
						'name'   => esc_html__( 'View', 'ultimatewoo-pro' ),
						'action' => 'view',
					);
				}
				...
				$actions['delete'] = array(
					'url'    => $delete_url,
					'name'   => $done ? esc_html__( 'Delete', 'ultimatewoo-pro' ) : esc_html__( 'Stop', 'ultimatewoo-pro' ),
	 ...
	 public function column_cb( $import ) {
		 ...
		 if ( current_user_can( 'manage_woocommerce' ) ) : ?>
			 ...
			 <label class="screen-reader-text" for="cb-select-<?php echo sanitize_html_class( $import->id ); ?>"><?php esc_html_e( 'Select import', 'ultimatewoo-pro' ); ?></label>
	...
	public function no_items() {
		?>
		<p><?php esc_html_e( 'Imported files will appear here. Files are stored for 14 days after the import.', 'ultimatewoo-pro' ); ?></p>
		<?php
	}
	 ...
	 protected function get_bulk_actions() {
		 ...
		 return array(
			 'delete' => esc_html__( 'Delete', 'ultimatewoo-pro' ),
		 );
	 }
