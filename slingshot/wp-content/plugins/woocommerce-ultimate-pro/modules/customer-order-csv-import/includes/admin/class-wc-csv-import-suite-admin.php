...
class WC_CSV_Import_Suite_Admin {
	...
	public function register_importers() {
		...
		register_importer( 'woocommerce_customer_csv',
							...
							__( 'Import <strong>customers</strong> to your store via a csv file.', 'ultimatewoo-pro' ),
							array( $this, 'load_importer' ) );

		register_importer( 'woocommerce_order_csv',
							'WooCommerce Order (CSV)',
							__( 'Import <strong>orders</strong> to your store via a csv file.', 'ultimatewoo-pro' ),
							array( $this, 'load_importer' ) );

		register_importer( 'woocommerce_coupon_csv',
							'WooCommerce Coupon (CSV)',
							__( 'Import <strong>coupons</strong> to your store via a csv file.', 'ultimatewoo-pro' ),
							array( $this, 'load_importer' ) );
	...
	public function add_menu_link() {
		...
		$menu_title = wc_csv_import_suite()->is_plugin_active( 'woocommerce-product-csv-import-suite.php' ) ? __( 'CSV Order Import Suite', 'ultimatewoo-pro' ) : __( 'CSV Import Suite', 'ultimatewoo-pro' );

		$this->page = add_submenu_page(
			'woocommerce',
			__( 'CSV Import Suite', 'ultimatewoo-pro' ),
			$menu_title,
			'manage_woocommerce',
	...
	public function load_styles_scripts() {
		...
		$unexpected_error_message = esc_html__( 'Something unexpected happened while importing. Your import may or may have not completed.', 'ultimatewoo-pro' );

		if ( $background_import && isset( $background_import->options['debug_mode'] ) && $background_import->options['debug_mode'] ) {
			$unexpected_error_message .= ' ' . __( 'Please check your site error log for possible clues as to what may have happened.', 'ultimatewoo-pro' );
		} else {
			$unexpected_error_message .= ' ' . __( 'Please try again with debug mode enabled to gain insight into what happened.', 'ultimatewoo-pro' );
		}
		...
		wp_localize_script( 'wc-csv-import-suite-admin', 'wc_csv_import_suite', array(
			...
			'i18n' => array(
				'show_details'             => esc_html__( 'View detailed results', 'ultimatewoo-pro' ),
				'hide_details'             => esc_html__( 'Hide detailed results', 'ultimatewoo-pro' ),
				'import_complete'          => esc_html__( 'Import complete.', 'ultimatewoo-pro' ),
				'skipped_or_failed_lines'  => esc_html__( 'Some lines were skipped or failed to import. See below for details.', 'ultimatewoo-pro' ),
	...
	private function get_import_finished_message( $import_id ) {
		...
		if ( 'completed' === $import->status ) {
			...
			if ( $import->options['dry_run'] ) {
				...
				$message = sprintf( __( 'Dry run of file %s is complete!', 'ultimatewoo-pro' ), $filename );
			} else {
				/* translators: Placeholders: %s - file name */
				$message = sprintf( __( 'Importing file %s is complete!', 'ultimatewoo-pro' ), $filename );
			}

			/* translators: Placeholders: %1$s - opening <a> tag, %2$s - closing </a> tag */
			$message  .= ' ' . sprintf( __( 'You can see the import results in the %1$simport progress page%2$s', 'ultimatewoo-pro' ), '<a href="' . $import_progress_url . '">', '</a>' );

			if ( $import->options['dry_run'] ) {
				$message .= __( ', then run your live import', 'ultimatewoo-pro' );
			}
			...
			if ( $import->options['dry_run'] ) {
				/* translators: Placeholders: %s - file name */
				$message = sprintf( __( 'Dry run of file %s failed.', 'ultimatewoo-pro' ), $filename );
			} else {
				/* translators: Placeholders: %s - file name */
				$message = sprintf( __( 'Importing file %s failed.', 'ultimatewoo-pro' ), $filename );
			}

			/* translators: Placeholders: %1$s - opening <a> tag, %2$s - closing </a> tag */
			$message .= ' ' . sprintf( __( 'Additional details may be found in the CSV Import %1$slogs%2$s.', 'ultimatewoo-pro' ), '<a href="' . admin_url( 'admin.php?page=wc-status&tab=logs' ) . '">', '</a>' );
	...
	public function render_submenu_pages() {
		...
		$this->tabs = array(
			'import'         => __( 'Import', 'ultimatewoo-pro' ),
			'import_list'    => __( 'Import List', 'ultimatewoo-pro' ),
		);
	...
	public function process_import_bulk_actions() {
		...
		if ( 'delete' === $action ) {
			...
			$this->message_handler->add_message( sprintf( _n( '%d import job deleted.',  '%d import jobs deleted.', 'ultimatewoo-pro', $num_deleted ), $num_deleted ) );
	...
	public function delete_import() {
		...
		switch( $import->status ) {
			...
			case 'queued':
				$message = __( 'Import removed from queue.', 'ultimatewoo-pro' );
			break;
			...
			case 'processing':
				...
				if ( $import->options['dry_run'] ) {
					...
					$message = sprintf( __( 'Dry run for %s import stopped and deleted.', 'ultimatewoo-pro' ), pathinfo( basename( $import->file_path ), PATHINFO_FILENAME ) );
				} else {
					$message = __( 'Import stopped and deleted. Data already updated or inserted by the import has not been removed.', 'ultimatewoo-pro' );
				}
			...
			default:
				...
				if ( $import->options['dry_run'] ) {
					$message = __( 'Information for this dry run has been deleted.', 'ultimatewoo-pro' );
				} else {
					$message = __( 'Import job deleted. Data imported or updated by this job has not been removed.', 'ultimatewoo-pro' );
				}
