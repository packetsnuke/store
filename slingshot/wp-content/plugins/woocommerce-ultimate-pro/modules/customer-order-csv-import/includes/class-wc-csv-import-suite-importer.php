...
class WC_CSV_Import_Suite_Importer extends WP_Importer {
	...
	protected function render_import_source_options() {
		...
		$source_options = apply_filters( 'wc_csv_import_suite_source_options', array(
			...
			array(
				'value'       => 'upload',
				'title'       => __( 'CSV or tab-delimited text file', 'ultimatewoo-pro' ),
				'description' => __( 'Upload & import data from .csv or .txt files', 'ultimatewoo-pro' ),
				'default'     => true,
			),

			array(
				'value'       => 'url',
				'title'       => __( 'URL or file path', 'ultimatewoo-pro' ),
				'description' => __( 'Import data from an URL or path to a file on the server.', 'ultimatewoo-pro' ),
			),

			array(
				'value'       => 'copypaste',
				'title'       => __( 'Copy/paste from file', 'ultimatewoo-pro' ),
				'description' => __( 'Copy & paste data from .xls, .xlsx or .csv files.', 'ultimatewoo-pro' ),
			),
	...
	protected function render_column_mapper( $data, $options, $raw_headers ) {
		...
		$mapping_options['import_as_meta']     = __( 'Custom Field with column name', 'ultimatewoo-pro' );
		$mapping_options['import_as_taxonomy'] = __( 'Taxonomy with column name', 'ultimatewoo-pro' );
	...
	protected function render_import_progress( $job_id ) {
		...
		if ( empty( $job ) ) {
			echo '<p>' . sprintf( esc_html__( 'Could not find job "%s". It may have been completed a while ago, deleted or never existed.', 'ultimatewoo-pro' ), esc_html( $job_id ) ) . '</p>';
			return;
		}
		...
		$legends = array(
			...
			'inserted' => array(
				...
				'label' => esc_html__( 'Inserted', 'ultimatewoo-pro' ),
				'color' => '#5cc488',
				'highlight_series' => 0,
			),
			...
			'merged' => array(
				...
				'label' => esc_html__( 'Merged', 'ultimatewoo-pro' ),
				'color' => '#3498db',
				'highlight_series' => 1,
			),
			...
			'skipped' => array(
				...
				'label' => esc_html__( 'Skipped', 'ultimatewoo-pro' ),
				'color' => '#f1c40f',
				'highlight_series' => 2,
			),
			...
			'failed' => array(
				...
				'label' => esc_html__( 'Failed', 'ultimatewoo-pro' ),
				'color' => '#e74c3c',
				'highlight_series' => 3,
			),
		);
		...
		if ( ! $is_complete ) {
			...
			$dry_run_complete = sprintf( esc_html__( 'Performed a dry run with the selected file. No database records were inserted or updated. %1$sRun a live import now%2$s or %3$sChange import settings%4$s.', 'ultimatewoo-pro' ), '<a href="' . wp_nonce_url( admin_url( 'admin.php?import=' . esc_attr( $_GET['import'] ) . '&job_id=' . esc_attr( $job->id ) . '&action=run_live' ), 'import-woocommerce' ) . '">', '</a>', '<a href="' . admin_url( 'admin.php?import=' . esc_attr( $_GET['import'] ) . '&step=2&file=' . urlencode( $job->file_path ) ) . '">', '</a>' );
	...
	protected function handle_upload( $type, $source = 'upload' ) {
		...
		switch ( $source ) {
			...
			case 'url':
				...
				if ( empty( $_POST['url'] ) ) {
					$error = __( 'Please provide a file path or URL', 'ultimatewoo-pro' );
				...
				// perhaps it's a path to file?
					...
					if ( ! is_readable( $_POST['url'] ) ) {
						$error = sprintf( __( 'Could not find the file %s', 'ultimatewoo-pro' ), esc_html( $_POST['url'] ) );
						$this->handle_upload_error( $error );
						return false;
					}
			...
			case 'copypaste':
				...
				if ( empty( $data ) ) {
					$error = __( 'Please enter some data to import', 'ultimatewoo-pro' );
					$this->handle_upload_error( $error );
					return false;
				}
	...
	private function ensure_file_is_readable( $file_path ) {
		...
		if ( ! is_readable( $file_path ) ) {
			...
			$this->handle_upload_error( sprintf( __( 'Cannot open file %s for importing. The file may not exist or is not readable by WordPress.', 'ultimatewoo-pro' ), $file_path ) );
		}
	}
	...
	protected function handle_upload_error( $error ) {
		...
		$message = sprintf( esc_html__( 'Sorry, there has been an error: %s', 'ultimatewoo-pro' ), $message );
	...
	protected function parse_multiline_item( $item_identifier, $related_items, $options, $raw_headers ) {
		...
		wc_csv_import_suite()->log( sprintf( __( '> Preparing multi-line item %s (rows %s)', 'ultimatewoo-pro' ), $item_identifier, $related ) );
		...
		if ( ! empty( $skipped_items ) ) {
			...
			wc_csv_import_suite()->log( sprintf( __( '> Skipped importing rows %s due to issues with related rows.', 'ultimatewoo-pro' ), $skipped ) );
	...
	protected function parse_taxonomy_terms( $key, $value ) {
		...
		if ( ! taxonomy_exists( $taxonomy ) ) {
			wc_csv_import_suite()->log( sprintf( __('> > Skipping taxonomy "%s" - it does not exist.', 'ultimatewoo-pro'), $taxonomy ) );
			return null;
		}
					...
					if ( isset( $this->inserted_terms[ $taxonomy ][ $parent ][ $term ] ) ) {
						...
						if ( ! $term_id ) {
							...
							if ( ! is_wp_error( $t ) ) {
								...
								wc_csv_import_suite()->log( sprintf( __( '> > (' . $this->get_line_num() . ') Failed to import term %s, parent %s - %s', 'ultimatewoo-pro' ), sanitize_text_field( $term ), sanitize_text_field( $parent ), sanitize_text_field( $taxonomy ) ) );
								break;
							}
						}
				...
				if ( isset( $this->inserted_terms[ $taxonomy ][0][ $raw_term ] ) ) {
					...
					if ( ! $term_id ) {
						...
						if ( ! is_wp_error( $t ) ) {
							...
							wc_csv_import_suite()->log( sprintf( __( '> > Failed to import term %s %s', 'ultimatewoo-pro' ), esc_html( $raw_term ), esc_html( $taxonomy ) ) );
							break;
						}
					}
	...
	public function add_import_result( $status, $message = '', $log = true ) {
		...
		if ( $log ) {
			...
			$labels = array(
				'inserted' => esc_html__( 'Inserted', 'ultimatewoo-pro' ),
				'merged'   => esc_html__( 'Merged', 'ultimatewoo-pro' ),
				'skipped'  => esc_html__( 'Skipped', 'ultimatewoo-pro' ),
				'failed'   => esc_html__( 'Failed', 'ultimatewoo-pro' ),
			);
	...
	public function get_valid_delimiters() {
		...
		if ( ! isset( $this->valid_delimiters ) ) {
			...
			$this->valid_delimiters = apply_filters( 'wc_csv_import_suite_delimiter_choices', array(
				","  => __( 'Comma', 'ultimatewoo-pro' ),
				";"  => __( 'Semicolon', 'ultimatewoo-pro' ),
				"\t" => __( 'Tab', 'ultimatewoo-pro' ), // double quotes are significant
			) );
		}
