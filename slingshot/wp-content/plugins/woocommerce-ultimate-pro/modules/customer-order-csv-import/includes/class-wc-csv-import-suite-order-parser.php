...
class WC_CSV_Import_Suite_Order_Parser {
	...
	public function parse_order( $item, $options = array(), $raw_headers = array() ) {
		...
		$preparing = $options['merge'] ? __( '> Row %s - preparing for merge.', 'ultimatewoo-pro' ) : __( '> Row %s - preparing for import.', 'ultimatewoo-pro' );
		...
		if ( ! $merging && empty( $line_items ) ) {
			throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_missing_line_items', esc_html__( 'Cannot import order without line items', 'ultimatewoo-pro' ) );
		}
		...
		if ( ! empty( $item['currency'] ) ) {
			...
			if ( ! in_array( strtoupper( $item['currency'] ), $currency_codes ) ) {
				...
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_invalid_currency_code', sprintf( __( 'Skipped. Invalid or unsupported currency %s.', 'ultimatewoo-pro' ),	$item['currency'] ) );
			} else {
	...
	private function parse_order_number( $item, $csv_file_format ) {
		...
		if ( ! $is_csv_export_file && $order_number && ! is_numeric( $order_number ) ) {
			throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_order_number_invalid', sprintf( __( 'Order number field must be an integer: %s.', 'ultimatewoo-pro' ), $order_number ) );
		}

		if ( ! $is_csv_export_file && $order_number_formatted && ! $order_number ) {
			throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_missing_numerical_order_number', __( 'Formatted order number provided but no numerical order number, see the documentation for further details.', 'ultimatewoo-pro' ) );
		}
	...
	private function parse_order_id( $item, $order_number_formatted, $options ) {
		...
		if ( $options['merge'] ) {
			...
			if ( ! $order_id && ! $order_number_formatted ) {
				wc_csv_import_suite()->log( __( '> > Cannot merge without id or order number. Importing instead.', 'ultimatewoo-pro' ) );
				return null;
			}
				...
				if ( ! $found_order ) {
					...
					if ( ! $order_id ) {
						...
						if ( $options['insert_non_matching'] ) {
							wc_csv_import_suite()->log( sprintf( __( '> > Skipped. Cannot find order with formatted number %s. Importing instead.', 'ultimatewoo-pro' ), $order_number_formatted ) );
							return null;
						} else {
							throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_cannot_find_order', sprintf( __( 'Cannot find order with formatted number %s.', 'ultimatewoo-pro' ), $order_number_formatted ) );
						}

					} else {
						// we can keep trying with order ID
						wc_csv_import_suite()->log( sprintf( __( '> > Cannot find order with formatted number %s.', 'ultimatewoo-pro' ), $order_number_formatted ) );
					}

				} else {
					wc_csv_import_suite()->log( sprintf( __( '> > Found order with formatted number %s.', 'ultimatewoo-pro' ), $order_number_formatted ) );
			...
			if ( ! $found_order && $order_id ) {
				...
				if ( 'shop_order' !== get_post_type( $order_id ) ) {
					...
					if ( $options['insert_non_matching'] ) {
						wc_csv_import_suite()->log( sprintf( __( '> > Skipped. Cannot find order with id %s. Importing instead.', 'ultimatewoo-pro' ), $order_id ) );
						return null;
					} else {
						throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_cannot_find_order', sprintf( __( 'Cannot find order with id %s.', 'ultimatewoo-pro' ), $order_id ) );
					}

				} else {
					wc_csv_import_suite()->log( sprintf( __( '> > Found order with ID %s.', 'ultimatewoo-pro' ), $order_id ) );
				}
			}
		}
		...
		elseif ( $order_number_formatted ) {
			...
			if ( $order_id ) {
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_order_already_exists', sprintf( __( 'Order %s already exists.', 'ultimatewoo-pro' ), $order_number_formatted ) );
			}
		}
		...
		if ( isset( $item['customer_user'] ) && $item['customer_user'] ) {
			...
			if ( is_numeric( $item['customer_user'] ) ) {
				...
				if ( ! $user ) {
					...
					$message = sprintf( __( 'Cannot find customer with id %s.', 'ultimatewoo-pro' ), $item['customer_user'] );
	...
	private function parse_order_status( $item ) {
		...
		if ( ! empty( $item['status'] ) ) {
			...
			if ( ! array_key_exists( $item['status'], $order_statuses ) ) {
				...
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_invalid_order_status', sprintf( __( 'Unknown order status %1$s (%2$s).', 'ultimatewoo-pro' ),	$item['status'], implode( array_keys( $order_statuses ) ), ', ' ) );
			} else {
				$status = $item['status'];
			}
