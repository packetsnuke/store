...
class WC_CSV_Import_Suite_Order_Processor {
	...
	public function process_order( $data, $options = array(), $raw_headers = array() ) {
		...
		wc_csv_import_suite()->log( __( '> Processing order', 'ultimatewoo-pro' ) );
		...
		try {
			...
			if ( $merging ) {
				...
				wc_csv_import_suite()->log( sprintf( __( '> Merging order %s.', 'ultimatewoo-pro' ), $order_identifier ) );
				...
				// insert customer
				wc_csv_import_suite()->log( sprintf( __( '> Inserting order %s', 'ultimatewoo-pro' ), esc_html( $order_identifier ) ) );
		...
		if ( $merging ) {
			wc_csv_import_suite()->log( sprintf( __( '> Finished merging order %s.', 'ultimatewoo-pro' ), $order_identifier ) );
			$this->importer->add_import_result( 'merged' );
		} else {
			wc_csv_import_suite()->log( sprintf( __( '> Finished importing order %s.', 'ultimatewoo-pro' ), $order_identifier ) );
	...
	private function create_order( $data, $options ) {
		...
		try {
			...
			if ( is_wp_error( $order ) ) {
				...
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_cannot_create_order', sprintf( __( 'Failed to insert order %1$s: %2$s;', 'ultimatewoo-pro' ), esc_html( $post['order_number_formatted'] ), esc_html( $messages ) ) );
			}
			...
			if ( ! empty( $data['order_number_formatted'] ) ) {
				...
				$order->add_order_note( sprintf( __( 'Original order #%s', 'ultimatewoo-pro' ), $data['order_number_formatted'] ) );
	...
	private function update_order( $id, $data, $options ) {
		...
		try {
			...
			if ( is_wp_error( $order ) ) {
				...
				throw new WC_CSV_Import_Suite_Import_Exception( 'wc_csv_import_suite_cannot_update_order', sprintf( __( 'Failed to update order %1$s: %2$s;', 'ultimatewoo-pro' ), esc_html( $order_identifier ), esc_html( $messages ) ) );
			}
	...
	private function add_order_item( WC_Order $order, $item, $type ) {
		...
		switch ( $type ) {
			...
			case 'line_item':
				...
				if ( ! $result ) {
					wc_csv_import_suite()->log( sprintf( __( '> > Warning: cannot add order item "%s".', 'ultimatewoo-pro' ), esc_html( $identifier ) ) );
				}
			break;
			...
			case 'shipping':
				...
				if ( ! $result ) {
					wc_csv_import_suite()->log( sprintf( __( '> > Warning: cannot add shipping method "%s".', 'ultimatewoo-pro' ), esc_html( $item['title'] ) ) );
				}
			break;
			...
			case 'tax':
				...
				if ( ! $result ) {
					wc_csv_import_suite()->log( sprintf( __( '> > Warning: cannot add tax "%s".', 'ultimatewoo-pro' ), esc_html( $item['label'] ) ) );
				}
			break;
			...
			case 'coupon':
				...
				if ( ! $result ) {
					wc_csv_import_suite()->log( sprintf( __( '> > Warning: cannot add coupon "%s".', 'ultimatewoo-pro' ), esc_html( $item['code'] ) ) );
				}
			break;
			...
			case 'fee':
				...
				if ( ! $result ) {
					wc_csv_import_suite()->log( sprintf( __( '> > Warning: cannot add fee "%s".', 'ultimatewoo-pro' ), esc_html( $item['title'] ) ) );
				}
		...
		switch ( $type ) {
			...
			case 'line_item':
				...
				if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ) {
					...
					if ( ! $order_item ) {
						wc_csv_import_suite()->log( sprintf( __( '> > Warning: order item %d does not exist.', 'ultimatewoo-pro' ), $order_item_id ) );
						break;
					}
				...
				if ( ! $result ) {
					wc_csv_import_suite()->log( sprintf( __( '> > Warning: cannot update order item %d.', 'ultimatewoo-pro' ), $order_item_id ) );
				}
			...
			case 'shipping':
				...
				if ( ! $result ) {
					wc_csv_import_suite()->log( sprintf( __( '> > Warning: cannot update shipping method "%s".', 'ultimatewoo-pro' ), esc_html( $item['title'] ) ) );
				}
			break;
			...
			case 'tax':
				...
				if ( ! $result ) {
					wc_csv_import_suite()->log( sprintf( __( '> > Warning: cannot update tax "%s".', 'ultimatewoo-pro' ), esc_html( $item['label'] ) ) );
				}
			break;
			...
			case 'coupon':
				...
				if ( ! $result ) {
					wc_csv_import_suite()->log( sprintf( __( '> > Warning: cannot merge coupon "%s".', 'ultimatewoo-pro' ), esc_html( $item['code'] ) ) );
				}
			break;
			...
			case 'fee':
				...
				if ( ! $result ) {
					wc_csv_import_suite()->log( sprintf( __( '> > Warning: cannot merge fee "%s".', 'ultimatewoo-pro' ), esc_html( $item['title'] ) ) );
				}
			break;
		}
	...
	private function get_product_for_item( $item ) {
		...
		if ( isset( $item['product_id'] ) && $item['product_id'] ) {
			...
			$product_name      = isset( $item['name'] ) ? $item['name'] : __( 'Unknown product', 'ultimatewoo-pro' );
