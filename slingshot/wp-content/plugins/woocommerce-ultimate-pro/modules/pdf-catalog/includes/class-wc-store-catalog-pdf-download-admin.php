...
class WC_Store_Catalog_PDF_Download_Admin {
	...
	public function load_scripts() {
		...
		if ( $screen->base === 'woocommerce_page_wc-settings' && isset( $_GET['tab'] ) && $_GET['tab'] === 'products' ) {
			wp_enqueue_script( 'wc-store-catalog-pdf-download-admin-js', ULTIMATEWOO_MODULES_URL . 'pdf-catalog/assets/js/admin' . $suffix . '.js' );			

			wp_enqueue_media();

			$localized_vars = array(
				'modalLogoTitle'  => __( 'Add Logo Image', 'ultimatewoo-pro' ),
				'buttonLogoText'  => __( 'Upload Image', 'ultimatewoo-pro' ),
				'modalPDFTitle'   => __( 'Add PDF', 'ultimatewoo-pro' ),
				'buttonPDFText'   => __( 'Upload PDF', 'ultimatewoo-pro' ),
				'removeImage'     => __( 'Are you sure you want to remove this image?', 'ultimatewoo-pro' ),
				'removePDF'       => __( 'Are you sure you want to remove this PDF?', 'ultimatewoo-pro' ),
				'previewLinkText' => __( 'Custom PDF Preview Link', 'ultimatewoo-pro' ),
			);
			
			wp_localize_script( 'wc-store-catalog-pdf-download-admin-js', 'wc_store_catalog_pdf_download_admin_local', $localized_vars );

			wp_enqueue_style( 'wc-store-catalog-pdf-download-admin-css', ULTIMATEWOO_MODULES_URL . 'pdf-catalog/assets/css/admin-styles.css' );
		}
	...
	public function add_settings_section( $sections ) {
		...
		$sections['wc_store_catalog_pdf_download'] = __( 'Store Catalog PDF', 'ultimatewoo-pro' );
	...
	public function add_settings( $settings, $current_section ) {
		if ( 'wc_store_catalog_pdf_download' === $current_section ) {
			$new_settings = array(
				array(
					'title'    => __( 'Store Catalog PDF', 'ultimatewoo-pro' ),
	...
	public function get_settings() { 
		...
		$link_label  = get_option( 'wc_store_catalog_pdf_download_link_label', __( 'Download Catalog', 'ultimatewoo-pro' ) );
		...
		?>
		<h3><?php _e( 'Store Catalog PDF', 'ultimatewoo-pro' ); ?></h3>
		
		<h4><?php _e( 'Ready-made PDF', 'ultimatewoo-pro' ); ?></h4>
		
		<table class="form-table">
			<tbody>
				<tr valign="top">
					<th class="titledesc" scope="row"><?php _e( 'Custom PDF', 'ultimatewoo-pro' ); ?><img class="help_tip" data-tip="<?php esc_attr_e( 'Upload a ready made PDF of your store.', 'ultimatewoo-pro' ); ?>" src="<?php echo $help_icon; ?>" height="16" width="16" /></th>

					<td class="forminp"><a href="#" class="wc-store-catalog-pdf-download-upload-custom-pdf button"><?php _e( 'Upload Custom PDF', 'ultimatewoo-pro' ); ?></a><p class="description"><?php _e( 'Optional: This is to be used with the shortcode to display a ready made PDF', 'ultimatewoo-pro' ); ?></p>
						...
						<?php if ( $custom_pdf ) { ?>
							<a href="<?php echo esc_url( $custom_pdf_url ); ?>" target="_blank" class="custom-pdf-preview"><?php _e( 'Custom PDF Preview Link', 'ultimatewoo-pro' ); ?></a>
						...
						<a href="#" class="remove-pdf dashicons dashicons-no" style="<?php echo esc_attr( $hide_remove_pdf_link ); ?>" title="<?php esc_attr_e( 'Click to remove PDF', 'ultimatewoo-pro' ); ?>"></a>
					</td>
				</tr>
			</tbody>
		</table>

		<h4><?php _e( 'PDF Generator', 'ultimatewoo-pro' ); ?></h4>

		<table class="form-table">
			<tbody>
				<tr valign="top">
					<th class="titledesc" scope="row"><?php _e( 'Company Logo', 'ultimatewoo-pro' ); ?><img class="help_tip" data-tip="<?php esc_attr_e( 'Upload a logo to be shown at the top of the PDF.', 'ultimatewoo-pro' ); ?>" src="<?php echo $help_icon; ?>" height="16" width="16" /></th>

					<td class="forminp"><a href="#" class="wc-store-catalog-pdf-download-upload-logo button"><?php _e( 'Upload Logo', 'ultimatewoo-pro' ); ?></a>
						...
						<a href="#" class="remove-image dashicons dashicons-no" style="<?php echo esc_attr( $hide_remove_image_link ); ?>" title="<?php esc_attr_e( 'Click to remove image', 'ultimatewoo-pro' ); ?>"></a>
					</td>
				</tr>

				<tr valign="top">
					<th class="titledesc" scope="row"><?php _e( 'PDF Header', 'ultimatewoo-pro' ); ?></th>
					<td class="forminp">
						<label for="show-header"><input type="checkbox" id="show-header" name="wc_store_catalog_pdf_download_show_header" <?php checked( $show_header, 'yes' ); ?> /> <?php _e( 'Show Header', 'ultimatewoo-pro' ); ?></label>
						<p class="description"><?php _e( 'Enabling this option will show a header information at the top of the PDF such as any intro verbiage.', 'ultimatewoo-pro' ); ?></p>
					</td>
				</tr>

				<tr valign="top" class="header-text-row" style="<?php echo esc_attr( $hide_header_text ); ?>">
					<th class="titledesc" scope="row"><?php _e( 'PDF Header Text', 'ultimatewoo-pro' ); ?></th>
					<td class="forminp">
				...
				<tr valign="top">
					<th class="titledesc" scope="row"><?php _e( 'PDF Footer', 'ultimatewoo-pro' ); ?></th>
					<td class="forminp">
						<label for="show-footer"><input type="checkbox" id="show-footer" name="wc_store_catalog_pdf_download_show_footer" <?php checked( $show_footer, 'yes' ); ?> /> <?php _e( 'Show Footer', 'ultimatewoo-pro' ); ?></label>
						<p class="description"><?php _e( 'Enabling this option will show a footer information at the bottom of the PDF such as foot notes or any outro verbiage.', 'ultimatewoo-pro' ); ?></p>
					</td>
				</tr>

				<tr valign="top" class="footer-text-row" style="<?php echo esc_attr( $hide_footer_text ); ?>">
					<th class="titledesc" scope="row"><?php _e( 'PDF Footer Text', 'ultimatewoo-pro' ); ?></th>
					<td class="forminp">
				...
				<tr valign="top">
					<th class="titledesc" scope="row"><?php _e( 'PDF Layout Format', 'ultimatewoo-pro' ); ?><img class="help_tip" data-tip="<?php esc_attr_e( 'Set how you want the PDF layout format to display for category products.', 'ultimatewoo-pro' ); ?>" src="<?php echo $help_icon; ?>" height="16" width="16" /></th>
					<td class="forminp">
						<select name="wc_store_catalog_pdf_download_layout" class="wc-enhanced-select">
							<option value="list" <?php selected( $layout, 'list' ); ?>><?php _e( 'List Format', 'ultimatewoo-pro' ); ?></option>
							<option value="grid" <?php selected( $layout, 'grid' ); ?>><?php _e( 'Grid Format', 'ultimatewoo-pro' ); ?></option>
						</select>
					</td>
				</tr>

				<tr valign="top">
					<th class="titledesc" scope="row"><?php _e( 'Download Link Label', 'ultimatewoo-pro' ); ?><img class="help_tip" data-tip="<?php esc_attr_e( 'Set the text you want to display next to the download icon.', 'ultimatewoo-pro' ); ?>" src="<?php echo $help_icon; ?>" height="16" width="16" /></th>
					<td class="forminp">
	...
	?>	
		<table class="wc_status_table widefat" cellspacing="0" id="status">
			<thead>
				...
				<tr>
					<th></th>
					<th><?php _e( 'Required', 'ultimatewoo-pro' ); ?></th>
					<th><?php _e( 'Present', 'ultimatewoo-pro' ); ?></th>
				</tr>
			</thead>
			
			<tbody>
				<tr>
					<td data-export-label="System Temp Directory"><?php _e( 'System Temp Directory', 'ultimatewoo-pro' ); ?></td>
					<td><?php _e( 'Yes', 'ultimatewoo-pro' ); ?></td>
				...
				<tr>
					...
					?>
					<td data-export-label="Upload Directory"><?php _e( 'Upload Directory', 'ultimatewoo-pro' ); ?></td>
					<td><?php _e( 'Yes', 'ultimatewoo-pro' ); ?></td>
					<td><?php echo ( file_exists( $pdf_path ) && is_writable( $pdf_path ) ) ? '<span style="color:green">' . __( 'Yes', 'ultimatewoo-pro' ) . '</span>' : '<strong style="color:red;">' . __( 'No', 'ultimatewoo-pro' ) . '</strong>'; ?><?php echo ' ( ' . $pdf_path . ' )'; ?></td>
				</tr>

				<tr>
					<td data-export-label="DOMDocument extension">DOMDocument extension</td>
					<td><?php _e( 'Yes', 'ultimatewoo-pro' ); ?></td>
					<td><?php echo extension_loaded( 'DOM' ) ? '<span style="color:green;">' . __( 'Yes', 'ultimatewoo-pro' ) . '</span>' : '<strong style="color:red;">' . __( 'No', 'ultimatewoo-pro' ) . '</strong>'; ?> ( <?php echo phpversion( 'DOM' ); ?> )</td>
				</tr>

				<tr>
					<td data-export-label="GD Library">GD Library</td>
					<td><?php _e( 'Yes', 'ultimatewoo-pro' ); ?></td>
					<td><?php echo ( extension_loaded( 'gd' ) && function_exists( 'gd_info' ) ) ? '<span style="color:green;">' . __( 'Yes', 'ultimatewoo-pro' ) . '</span>' : '<strong style="color:red;">' . __( 'No', 'ultimatewoo-pro' ) . '</strong>'; ?></td>
				</tr>

				<tr>
					<td data-export-label="MBString">MB String</td>
					<td><?php _e( 'Yes', 'ultimatewoo-pro' ); ?></td>
					<td><?php echo ( extension_loaded( 'mbstring' ) ) ? '<span style="color:green;">' . __( 'Yes', 'ultimatewoo-pro' ) . '</span>' : '<strong style="color:red;">' . __( 'No', 'ultimatewoo-pro' ) . '</strong>'; ?></td>
				</tr>

				<tr>
					<td data-export-label="URL fopen">Allow URL fopen</td>
					<td><?php _e( 'Yes', 'ultimatewoo-pro' ); ?></td>
					<td><?php echo ( ini_get( 'allow_url_fopen' ) ) ? '<span style="color:green;">' . __( 'Yes', 'ultimatewoo-pro' ) . '</span>' : '<strong style="color:red;">' . __( 'No', 'ultimatewoo-pro' ) . '</strong>'; ?></td>
				</tr>

				<tr>
					<td data-export-label="Template Overrides"><?php _e( 'Template Overrides', 'ultimatewoo-pro' ); ?></td>
					<td colspan="2">
