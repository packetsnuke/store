...
class WC_Store_Catalog_PDF_Download_Ajax {
	...
	public function generate_pdf_ajax() {
		...
		if ( $wc_posts = $posts ) {
			...
			$filename   = apply_filters( 'wc_store_catalog_pdf_download_filename', str_replace( ' ', '-', wp_specialchars_decode( get_bloginfo( 'name' ), ENT_QUOTES ) ) . '-' . __( 'Store-Catalog', 'ultimatewoo-pro' ) . '-' . time() . '.pdf' );
	...
	public static function get_placeholder_image( $image_size ) {
		...
		return '<img src="' . wc_placeholder_img_src() . '" alt="' . __( 'Placeholder', 'ultimatewoo-pro' ) . '" width="' . esc_attr( $image_size[0] ) . '" height="' . esc_attr( $image_size[1] ) . '" class="woocommerce-placeholder wp-post-image" />';
	}
}
