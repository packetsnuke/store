...
?>
	...
	<div class="single">
		...
		foreach ( $wc_posts as $post ) {
			...
			$product_image = version_compare( WC_VERSION, '3.0', '<' ) ? $product->get_image( $single_image_size ) : str_replace( '//', ( is_ssl() ? 'https://' : 'http://' ), $product->get_image( $single_image_size ) );
		?>
			...
			<a href="<?php echo get_permalink( $product->get_id() ); ?>" class="single-image-link"><?php echo apply_filters( 'wc_store_catalog_pdf_download_show_product_image', has_post_thumbnail( $post ) ? $product_image : WC_Store_Catalog_PDF_Download_Ajax::get_placeholder_image( array() ), $product ); ?></a>
