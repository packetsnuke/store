...
function wcs_create_subscription( $args = array() ) {
	...
	if ( ! is_string( $args['start_date'] ) || false === wcs_is_datetime_mysql_format( $args['start_date'] ) ) {
		return new WP_Error( 'woocommerce_subscription_invalid_start_date_format', _x( 'Invalid date. The date must be a string and of the format: "Y-m-d H:i:s".', 'Error message while creating a subscription', 'ultimatewoo-pro' ) );
	} else if ( wcs_date_to_time( $args['start_date'] ) > current_time( 'timestamp', true ) ) {
		return new WP_Error( 'woocommerce_subscription_invalid_start_date', _x( 'Subscription start date must be before current day.', 'Error message while creating a subscription', 'ultimatewoo-pro' ) );
	}

	// check customer id is set
	if ( empty( $args['customer_id'] ) || ! is_numeric( $args['customer_id'] ) || $args['customer_id'] <= 0 ) {
		return new WP_Error( 'woocommerce_subscription_invalid_customer_id', _x( 'Invalid subscription customer_id.', 'Error message while creating a subscription', 'ultimatewoo-pro' ) );
	}
	...
	if ( empty( $args['billing_period'] ) || ! in_array( strtolower( $args['billing_period'] ), array_keys( wcs_get_subscription_period_strings() ) ) ) {
		return new WP_Error( 'woocommerce_subscription_invalid_billing_period', __( 'Invalid subscription billing period given.', 'ultimatewoo-pro' ) );
	}

	// check the billing interval
	if ( empty( $args['billing_interval'] ) || ! is_numeric( $args['billing_interval'] ) || absint( $args['billing_interval'] ) <= 0 ) {
		return new WP_Error( 'woocommerce_subscription_invalid_billing_interval', __( 'Invalid subscription billing interval given. Must be an integer greater than 0.', 'ultimatewoo-pro' ) );
	}
	...
	$post_title_date = strftime( _x( '%b %d, %Y @ %I:%M %p', 'Used in subscription post title. "Subscription renewal order - <this>"', 'ultimatewoo-pro' ) );
	// translators: placeholder is order date parsed by strftime
	$subscription_data['post_title']    = sprintf( _x( 'Subscription &ndash; %s', 'The post title for the new subscription', 'ultimatewoo-pro' ), $post_title_date );
	...
	if ( $args['status'] ) {
		if ( ! in_array( 'wc-' . $args['status'], array_keys( wcs_get_subscription_statuses() ) ) ) {
			return new WP_Error( 'woocommerce_invalid_subscription_status', __( 'Invalid subscription status given.', 'ultimatewoo-pro' ) );
		}
...
function wcs_get_subscription_statuses() {
	...
	$subscription_statuses = array(
		'wc-pending'        => _x( 'Pending', 'Subscription status', 'ultimatewoo-pro' ),
		'wc-active'         => _x( 'Active', 'Subscription status', 'ultimatewoo-pro' ),
		'wc-on-hold'        => _x( 'On hold', 'Subscription status', 'ultimatewoo-pro' ),
		'wc-cancelled'      => _x( 'Cancelled', 'Subscription status', 'ultimatewoo-pro' ),
		'wc-switched'       => _x( 'Switched', 'Subscription status', 'ultimatewoo-pro' ),
		'wc-expired'        => _x( 'Expired', 'Subscription status', 'ultimatewoo-pro' ),
		'wc-pending-cancel' => _x( 'Pending Cancellation', 'Subscription status', 'ultimatewoo-pro' ),
	);
...
function wcs_get_subscription_status_name( $status ) {
	...
	if ( ! is_string( $status ) ) {
		return new WP_Error( 'woocommerce_subscription_wrong_status_format', __( 'Can not get status name. Status is not a string.', 'ultimatewoo-pro' ) );
	}
...
function wcs_get_address_type_to_display( $address_type ) {
	if ( ! is_string( $address_type ) ) {
		return new WP_Error( 'woocommerce_subscription_wrong_address_type_format', __( 'Can not get address type display name. Address type is not a string.', 'ultimatewoo-pro' ) );
	}

	$address_types = apply_filters( 'woocommerce_subscription_address_types', array(
		'shipping' => __( 'Shipping Address', 'ultimatewoo-pro' ),
		'billing' => __( 'Billing Address', 'ultimatewoo-pro' ),
	) );
...
function wcs_get_subscription_date_types() {
	...
	$dates = array(
		'start'        => _x( 'Start Date', 'table heading', 'ultimatewoo-pro' ),
		'trial_end'    => _x( 'Trial End', 'table heading', 'ultimatewoo-pro' ),
		'next_payment' => _x( 'Next Payment', 'table heading', 'ultimatewoo-pro' ),
		'last_payment' => _x( 'Last Order Date', 'table heading', 'ultimatewoo-pro' ),
		'cancelled'    => _x( 'Cancelled Date', 'table heading', 'ultimatewoo-pro' ),
		'end'          => _x( 'End Date', 'table heading', 'ultimatewoo-pro' ),
	);
...
function wcs_get_date_meta_key( $date_type ) {
	if ( ! is_string( $date_type ) ) {
		return new WP_Error( 'woocommerce_subscription_wrong_date_type_format', __( 'Date type is not a string.', 'ultimatewoo-pro' ) );
	} elseif ( empty( $date_type ) ) {
		return new WP_Error( 'woocommerce_subscription_wrong_date_type_format', __( 'Date type can not be an empty string.', 'ultimatewoo-pro' ) );
	}
