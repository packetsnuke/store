...
class WCS_Cart_Renewal {
		...
		if ( isset( $_GET['pay_for_order'] ) && isset( $_GET['key'] ) && isset( $wp->query_vars['order-pay'] ) ) {
			...
			if ( wcs_get_objects_property( $order, 'order_key' ) == $order_key && $order->has_status( array( 'pending', 'failed' ) ) && wcs_order_contains_renewal( $order ) ) {
				...
				if ( ! is_user_logged_in() ) {
					...
					wc_add_notice( __( 'That doesn\'t appear to be your order.', 'ultimatewoo-pro' ), 'error' );
				...
				if ( WC()->cart->cart_contents_count != 0 ) {
					...
					wc_add_notice( __( 'Complete checkout to renew your subscription.', 'ultimatewoo-pro' ), 'success' );
				}
	...
	protected function setup_cart( $subscription, $cart_item_data ) {
		...
		foreach ( $subscription->get_items() as $item_id => $line_item ) {
			...
			$product_deleted_error_message = apply_filters( 'woocommerce_subscriptions_renew_deleted_product_error_message', __( 'The %s product has been deleted and can no longer be renewed. Please choose a new product or contact us for assistance.', 'ultimatewoo-pro' ) );
		...
		if ( ! $success && wcs_is_subscription( $subscription ) ) {
			...
			wc_add_notice( sprintf( esc_html__( 'Subscription #%s has not been added to the cart.', 'ultimatewoo-pro' ), $subscription->get_order_number() ) , 'error' );
			WC()->cart->empty_cart( true );
		}
	...
	public function cart_items_loaded_from_session( $cart ) {
		...
		if ( $removed_count_subscription ) {
			$error_message = esc_html( _n( 'We couldn\'t find the original subscription for an item in your cart. The item was removed.', 'We couldn\'t find the original subscriptions for items in your cart. The items were removed.', $removed_count_subscription, 'ultimatewoo-pro' ) );
		...
		if ( $removed_count_order ) {
			$error_message = esc_html( _n( 'We couldn\'t find the original renewal order for an item in your cart. The item was removed.', 'We couldn\'t find the original renewal orders for items in your cart. The items were removed.', $removed_count_order, 'ultimatewoo-pro' ) );
			if ( ! wc_has_notice( $error_message, 'notice' ) ) {
				wc_add_notice( $error_message, 'notice' );
			}
		}
	...
	public function maybe_remove_items( $cart_item_key ) {
		...
		if ( isset( WC()->cart->cart_contents[ $cart_item_key ][ $this->cart_item_key ]['subscription_id'] ) ) {
			...
			if ( $removed_item_count > 1 && 'woocommerce_before_cart_item_quantity_zero' == current_filter() ) {
				wc_add_notice( esc_html__( 'All linked subscription items have been removed from the cart.', 'ultimatewoo-pro' ), 'notice' );
			}
		}
	}
	...
	public function items_removed_title( $product_title, $cart_item ) {
		...
		if ( isset( $cart_item[ $this->cart_item_key ]['subscription_id'] ) ) {
			$subscription  = $this->get_order( $cart_item );
			$product_title = ( count( $subscription->get_items() ) > 1 ) ? esc_html_x( 'All linked subscription items were', 'Used in WooCommerce by removed item notification: "_All linked subscription items were_ removed. Undo?" Filter for item title.', 'ultimatewoo-pro' ) : $product_title;
		}
