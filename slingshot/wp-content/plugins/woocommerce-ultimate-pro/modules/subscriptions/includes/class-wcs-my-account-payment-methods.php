...
class WCS_My_Account_Payment_Methods {
	...
	public static function maybe_update_subscriptions_payment_meta( $deleted_token_id, $deleted_token ) {
		if ( isset( $_GET['delete_subscription_token'] ) && ! empty( $_GET['wcs_nonce'] ) && wp_verify_nonce( $_GET['wcs_nonce'], 'delete_subscription_token_' . $_GET['delete_subscription_token'] ) ) {
			...
			if ( empty( $new_token ) ) {
				$notice = esc_html__( 'The deleted payment method was used for automatic subscription payments, we couldn\'t find an alternative token payment method token to change your subscriptions to.', 'ultimatewoo-pro' );
			...
			if ( ! empty( $subscriptions ) ) {
				...
				foreach ( $subscriptions as $subscription ) {
					...
					if ( $updated ) {
						$subscription->add_order_note( sprintf( _x( 'Payment method meta updated after customer deleted a token from their My Account page. Payment meta changed from %1$s to %2$s', 'used in subscription note', 'ultimatewoo-pro' ), $deleted_token->get_token(), $new_token->get_token() ) );
	...
	public static function get_token_label( $token ) {
		...
		if ( method_exists( $token, 'get_last4' ) && $token->get_last4() ) {
			$label = sprintf( __( '%s ending in %s', 'ultimatewoo-pro' ), esc_html( wc_get_credit_card_type_label( $token->get_card_type() ) ), esc_html( $token->get_last4() ) );
		} else {
