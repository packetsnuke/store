...
class WC_Subscriptions_Change_Payment_Gateway {
	...
	public static function maybe_replace_pay_shortcode() {
		...
		if ( ! self::$is_request_to_change_payment && isset( $wp->query_vars['order-pay'] ) && wcs_is_subscription( absint( $wp->query_vars['order-pay'] ) ) ) {
			...
			<div class="woocommerce">
				<ul class="order_details">
					<li class="order">
						...
						echo wp_kses( sprintf( esc_html__( 'Subscription Number: %s', 'ultimatewoo-pro' ), '<strong>' . esc_html( $subscription->get_order_number() ) . '</strong>' ), array( 'strong' => true ) );
						?>
					</li>
					<li class="date">
						<?php
						...
						echo wp_kses( sprintf( esc_html__( 'Next Payment Date: %s', 'ultimatewoo-pro' ), '<strong>' . esc_html( $subscription->get_date_to_display( 'next_payment' ) ) . '</strong>' ), array( 'strong' => true ) );
						?>
					</li>
					<li class="total">
						<?php
						// translators: placeholder is the formatted total to be paid for the subscription wrapped in <strong> tags
						echo wp_kses_post( sprintf( esc_html__( 'Total: %s', 'ultimatewoo-pro' ), '<strong>' . $subscription->get_formatted_order_total() . '</strong>' ) );
						?>
					</li>
					<?php if ( $subscription->get_payment_method_title() ) : ?>
						<li class="method">
							...
							echo wp_kses( sprintf( esc_html__( 'Payment Method: %s', 'ultimatewoo-pro' ), '<strong>' . esc_html( $subscription->get_payment_method_to_display() ) . '</strong>' ), array( 'strong' => true ) );
							?>
						</li>
					<?php endif; ?>
				</ul>
				...
				wc_add_notice( __( 'Sorry, this subscription change payment method request is invalid and cannot be processed.', 'ultimatewoo-pro' ), 'error' );
			}
			...
			if ( wp_verify_nonce( $_GET['_wpnonce'] ) === false ) {
				...
				wc_add_notice( __( 'There was an error with your request. Please try again.', 'ultimatewoo-pro' ), 'error' );

			} elseif ( empty( $subscription ) ) {

				wc_add_notice( __( 'Invalid Subscription.', 'ultimatewoo-pro' ), 'error' );

			} elseif ( ! current_user_can( 'edit_shop_subscription_payment_method', $subscription->get_id() ) ) {

				wc_add_notice( __( 'That doesn\'t appear to be one of your subscriptions.', 'ultimatewoo-pro' ), 'error' );

			} elseif ( ! $subscription->can_be_updated_to( 'new-payment-method' ) ) {

				wc_add_notice( __( 'The payment method can not be changed for that subscription.', 'ultimatewoo-pro' ), 'error' );

			} else {

				if ( $subscription->get_time( 'next_payment' ) > 0 ) {
					// translators: placeholder is next payment's date
					$next_payment_string = sprintf( __( ' Next payment is due %s.', 'ultimatewoo-pro' ), $subscription->get_date_to_display( 'next_payment' ) );
				} else {
					$next_payment_string = '';
				}

				// translators: placeholder is either empty or "Next payment is due..."
				wc_add_notice( sprintf( __( 'Choose a new payment method.%s', 'ultimatewoo-pro' ), $next_payment_string ), 'notice' );
				wc_print_notices();
				...
				if ( $subscription->get_order_key() == $_GET['key'] ) {
					...
					wc_add_notice( __( 'Invalid order.', 'ultimatewoo-pro' ), 'error' );
	...
	public static function change_payment_method_button( $actions, $subscription ) {
		...
		if ( $subscription->can_be_updated_to( 'new-payment-method' ) ) {
			...
			$actions['change_payment_method'] = array(
				...
				'name' => _x( 'Change Payment', 'label on button, imperative', 'ultimatewoo-pro' ),
			);
			...
			if ( $subscription->get_order_key() == $_GET['key'] ) {
				...
				if ( wc_notice_count( 'error' ) == 0 ) {
					...
					if ( 'success' == $result['result'] ) {
						wc_add_notice( __( 'Payment method updated.', 'ultimatewoo-pro' ), 'success' );
	...
	public static function update_payment_method( $subscription, $new_payment_method ) {
		...
		// Log change on order
		$subscription->add_order_note( sprintf( _x( 'Payment method changed from "%1$s" to "%2$s" by the subscriber from their account page.', '%1$s: old payment title, %2$s: new payment title', 'ultimatewoo-pro' ), $old_payment_method_title, $new_payment_method_title ) );
	...
	public static function change_payment_method_page_title( $title ) {
		...
		if ( is_main_query() && in_the_loop() && is_page() && is_checkout_pay_page() && self::$is_request_to_change_payment ) {
			$title = _x( 'Change Payment Method', 'the page title of the change payment method form', 'ultimatewoo-pro' );
		}
