...
class WCS_Email_Completed_Renewal_Order extends WC_Email_Customer_Completed_Order {
	...
	function __construct() {
		...
		$this->title          = __( 'Completed Renewal Order', 'ultimatewoo-pro' );
		$this->description    = __( 'Renewal order complete emails are sent to the customer when a subscription renewal order is marked complete and usually indicates that the item for that renewal period has been shipped.', 'ultimatewoo-pro' );
		$this->customer_email = true;

		$this->heading        = _x( 'Your renewal order is complete', 'Default email heading for email to customer on completed renewal order', 'ultimatewoo-pro' );
		// translators: $1: {blogname}, $2: {order_date}, variables that will be substituted when email is sent out
		$this->subject        = sprintf( _x( 'Your %1$s renewal order from %2$s is complete', 'Default email subject for email to customer on completed renewal order', 'ultimatewoo-pro' ), '{blogname}', '{order_date}' );
		...
		// Other settings
		$this->heading_downloadable = $this->get_option( 'heading_downloadable', _x( 'Your subscription renewal order is complete - download your files', 'Default email heading for email with downloadable files in it', 'ultimatewoo-pro' ) );
		// translators: $1: {blogname}, $2: {order_date}, variables will be substituted when email is sent out
		$this->subject_downloadable = $this->get_option( 'subject_downloadable', sprintf( _x( 'Your %1$s subscription renewal order from %2$s is complete - download your files', 'Default email subject for email with downloadable files in it', 'ultimatewoo-pro' ), '{blogname}', '{order_date}' ) );
