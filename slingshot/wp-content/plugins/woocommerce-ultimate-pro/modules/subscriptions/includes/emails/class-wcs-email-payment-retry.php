...
class WCS_Email_Payment_Retry extends WC_Email_Failed_Order {
	...
	public function __construct() {
		...
		$this->title          = __( 'Payment Retry', 'ultimatewoo-pro' );
		$this->description    = __( 'Payment retry emails are sent to chosen recipient(s) when an attempt to automatically process a subscription renewal payment has failed and a retry rule has been applied to retry the payment in the future.', 'ultimatewoo-pro' );

		$this->heading        = __( 'Automatic renewal payment failed', 'ultimatewoo-pro' );
		$this->subject        = __( '[{site_title}] Automatic payment failed for {order_number}, retry scheduled to run {retry_time}', 'ultimatewoo-pro' );
