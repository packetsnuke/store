...
class WCS_Retry_Admin {
	...
	public function add_meta_boxes() {
		...
		if ( 'shop_order' === get_post_type( $post_ID ) && wcs_order_contains_renewal( $post_ID ) && WCS_Retry_Manager::store()->get_retry_count_for_order( $post_ID ) > 0 ) {
			add_meta_box( 'renewal_payment_retries', __( 'Automatic Failed Payment Retries', 'ultimatewoo-pro' ), 'WCS_Meta_Box_Payment_Retries::output', 'shop_order', 'normal', 'low' );
		}
	}
		...
		if ( 'subscription_relationship' == $column && wcs_order_contains_renewal( $post_id ) ) {
			...
			if ( ! empty( $retries ) ) {
				...
				foreach ( $retry_counts as $retry_status => $retry_count ) {
					...
					switch ( $retry_status ) {
						case 'pending' :
							$tool_tip .= sprintf( _n( '%d Pending Payment Retry', '%d Pending Payment Retries', $retry_count, 'ultimatewoo-pro' ), $retry_count );
							break;
						case 'processing' :
							$tool_tip .= sprintf( _n( '%d Processing Payment Retry', '%d Processing Payment Retries', $retry_count, 'ultimatewoo-pro' ), $retry_count );
							break;
						case 'failed' :
							$tool_tip .= sprintf( _n( '%d Failed Payment Retry', '%d Failed Payment Retries', $retry_count, 'ultimatewoo-pro' ), $retry_count );
							break;
						case 'complete' :
							$tool_tip .= sprintf( _n( '%d Successful Payment Retry', '%d Successful Payment Retries', $retry_count, 'ultimatewoo-pro' ), $retry_count );
							break;
						case 'cancelled' :
							$tool_tip .= sprintf( _n( '%d Cancelled Payment Retry', '%d Cancelled Payment Retries', $retry_count, 'ultimatewoo-pro' ), $retry_count );
							break;
					}
	...
	public function add_settings( $settings ) {
		...
		$spliced_array = array_splice( $settings, key( $misc_section_end ), 0, array(
			array(
				'name'     => __( 'Retry Failed Payments', 'ultimatewoo-pro' ),
				'desc'     => __( 'Enable automatic retry of failed recurring payments', 'ultimatewoo-pro' ),
				'id'       => $this->setting_id,
				'default'  => 'no',
				'type'     => 'checkbox',
				'desc_tip' => sprintf( __( 'Attempt to recover recurring revenue that would otherwise be lost due to payment methods being declined only temporarily. %sLearn more%s.', 'ultimatewoo-pro' ), '<a href="https://docs.woocommerce.com/document/subscriptions/failed-payment-retry/">', '</a>' ),
			),
		) );
