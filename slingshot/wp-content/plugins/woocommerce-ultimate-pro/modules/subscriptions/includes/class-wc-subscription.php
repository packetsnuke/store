...
class WC_Subscription extends WC_Order {
	...
	public function update_status( $new_status, $note = '', $manual = false ) {
		...
		if ( $new_status !== $old_status || ! in_array( $old_status_key, array_keys( wcs_get_subscription_statuses() ) ) ) {
			...
			if ( ! $this->can_be_updated_to( $new_status ) ) {
				...
				$message = sprintf( __( 'Unable to change subscription status to "%s".', 'ultimatewoo-pro' ), $new_status );
			...
			try {
				...
				$this->add_order_note( sprintf( __( 'Unable to change subscription status to "%s". Exception: %s', 'ultimatewoo-pro' ), $new_status, $e->getMessage() ) );
	...
	protected function status_transition() {
		...
		if ( $this->status_transition ) {
			...
			if ( ! empty( $this->status_transition['from'] ) ) {
				...
				$transition_note = sprintf( __( 'Status changed from %1$s to %2$s.', 'ultimatewoo-pro' ), wcs_get_subscription_status_name( $this->status_transition['from'] ), wcs_get_subscription_status_name( $this->status_transition['to'] ) );
				...
				$transition_note = sprintf( __( 'Status set to %s.', 'ultimatewoo-pro' ), wcs_get_subscription_status_name( $this->status_transition['to'] ) );
			}
	...
	public function get_date_to_display( $date_type = 'next_payment' ) {
		...
		if ( $timestamp_gmt > 0 ) {
			...
			switch ( $date_type ) {
				case 'end' :
					$date_to_display = __( 'Not yet ended', 'ultimatewoo-pro' );
					break;
				case 'cancelled' :
					$date_to_display = __( 'Not cancelled', 'ultimatewoo-pro' );
					break;
				case 'next_payment' :
				case 'trial_end' :
				default :
					$date_to_display = _x( '-', 'original denotes there is no date to display', 'ultimatewoo-pro' );
					break;
			}
		}
	...
	public function delete_date( $date_type ) {
		...
		switch ( $date_type ) {
			case 'date_created' :
				$message = __( 'The start date of a subscription can not be deleted, only updated.', 'ultimatewoo-pro' );
			break;
			case 'last_order_date_created' :
			case 'last_order_date_modified' :
				$message = sprintf( __( 'The %s date of a subscription can not be deleted. You must delete the order.', 'ultimatewoo-pro' ), $date_type );
			break;
			default :
				$message = '';
			break;
		...
		if ( ! empty( $message ) ) {
			throw new Exception( sprintf( __( 'Subscription #%d: ', 'ultimatewoo-pro' ), $this->get_id() ) . $message );
		}
	...
	public function payment_complete( $transaction_id = '' ) {
		...
		if ( 0 == $this->get_total_initial_payment() && 1 == $this->get_completed_payment_count() && false != $this->get_parent() ) {
			$note = __( 'Sign-up complete.', 'ultimatewoo-pro' );
		} else {
			$note = __( 'Payment received.', 'ultimatewoo-pro' );
		}
	...
	public function payment_failed( $new_status = 'on-hold' ) {
		...
		$this->add_order_note( __( 'Payment failed.', 'ultimatewoo-pro' ) );
		...
		if ( 'cancelled' == $new_status || apply_filters( 'woocommerce_subscription_max_failed_payments_exceeded', false, $this ) ) {
			if ( $this->can_be_updated_to( 'cancelled' ) ) {
				$this->update_status( 'cancelled', __( 'Subscription Cancelled: maximum number of failed payments reached.', 'ultimatewoo-pro' ) );
			}
	...
	public function get_payment_method_to_display() {
		...
		if ( $this->is_manual() ) {
			...
			$payment_method_to_display = __( 'Manual Renewal', 'ultimatewoo-pro' );
	...
	protected function set_payment_method_meta( $payment_method_id, $payment_meta ) {
		...
		if ( ! is_array( $payment_meta ) ) {
			throw new InvalidArgumentException( __( 'Payment method meta must be an array.', 'ultimatewoo-pro' ) );
		}
	...
	public function validate_date_updates( $dates, $timezone = 'gmt' ) {
		...
		if ( ! is_array( $dates ) ) {
			throw new InvalidArgumentException( __( 'Invalid format. First parameter needs to be an array.', 'ultimatewoo-pro' ) );
		}

		if ( empty( $dates ) ) {
			throw new InvalidArgumentException( __( 'Invalid data. First parameter was empty when passed to update_dates().', 'ultimatewoo-pro' ) );
		}
		...
		if ( ! empty( $extra_keys ) ) {
			throw new InvalidArgumentException( __( 'Invalid data. First parameter has a date that is not in the registered date types.', 'ultimatewoo-pro' ) );
		}
		...
		foreach ( $this->get_valid_date_types() as $date_type ) {
			...
			if ( isset( $dates[ $date_type ] ) ) {
				...
				if ( ! empty( $datetime ) && false === wcs_is_datetime_mysql_format( $datetime ) ) {
					...
					throw new InvalidArgumentException( sprintf( _x( 'Invalid %s date. The date must be of the format: "Y-m-d H:i:s".', 'appears in an error message if date is wrong format', 'ultimatewoo-pro' ), $date_type ) );
				}
			...
			switch ( $date_type ) {
				case 'end' :
					if ( array_key_exists( 'cancelled', $timestamps ) && $timestamp < $timestamps['cancelled'] ) {
						$messages[] = sprintf( __( 'The %s date must occur after the cancellation date.', 'ultimatewoo-pro' ), $date_type );
					}

				case 'cancelled' :
					if ( array_key_exists( 'last_order_date_created', $timestamps ) && $timestamp < $timestamps['last_order_date_created'] ) {
						$messages[] = sprintf( __( 'The %s date must occur after the last payment date.', 'ultimatewoo-pro' ), $date_type );
					}

					if ( array_key_exists( 'next_payment', $timestamps ) && $timestamp <= $timestamps['next_payment'] ) {
						$messages[] = sprintf( __( 'The %s date must occur after the next payment date.', 'ultimatewoo-pro' ), $date_type );
					}
				case 'next_payment' :
					...
					if ( array_key_exists( 'trial_end', $timestamps ) && $timestamp < $timestamps['trial_end'] ) {
						$messages[] = sprintf( __( 'The %s date must occur after the trial end date.', 'ultimatewoo-pro' ), $date_type );
					}
				case 'trial_end' :
					if ( $timestamp <= $timestamps['date_created'] ) {
						$messages[] = sprintf( __( 'The %s date must occur after the start date.', 'ultimatewoo-pro' ), $date_type );
					}
			}
		...
		if ( $this->object_read && ! empty( $messages ) ) {
			throw new Exception( sprintf( __( 'Subscription #%d: ', 'ultimatewoo-pro' ), $this->get_id() ) . join( ' ', $messages ) );
		}
	...
	public function add_product( $product, $qty = 1, $args = array() ) {
		...
		if ( $item_id && $product->backorders_require_notification() && $product->is_on_backorder( $qty ) ) {
			wc_delete_order_item_meta( $item_id, apply_filters( 'woocommerce_backordered_item_meta_name', __( 'Backordered', 'ultimatewoo-pro' ) ) );
		}
