	...
	public function maybe_setup_cart() {
		...
		if ( isset( $_GET['resubscribe'] ) && isset( $_GET['_wpnonce'] ) ) {
			...
			if ( wp_verify_nonce( $_GET['_wpnonce'], $subscription->get_id() ) === false ) {
				...
				wc_add_notice( __( 'There was an error with your request to resubscribe. Please try again.', 'ultimatewoo-pro' ), 'error' );

			} elseif ( empty( $subscription ) ) {

				wc_add_notice( __( 'That subscription does not exist. Has it been deleted?', 'ultimatewoo-pro' ), 'error' );
				...
				wc_add_notice( __( 'You can not resubscribe to that subscription. Please contact us if you need assistance.', 'ultimatewoo-pro' ), 'error' );
				...
				if ( WC()->cart->get_cart_contents_count() != 0 ) {
					wc_add_notice( __( 'Complete checkout to resubscribe.', 'ultimatewoo-pro' ), 'success' );
				}
		...
		if ( wcs_order_contains_subscription( $order_id ) && wcs_order_contains_resubscribe( $order_id ) ) {
			...
			foreach ( wcs_get_subscriptions_for_resubscribe_order( $order_id ) as $subscription ) {
				if ( $subscription->has_status( 'pending-cancel' ) ) {
					$cancel_note = sprintf( __( 'Customer resubscribed in order #%s', 'ultimatewoo-pro' ), $order->get_order_number() );
