...
class WC_Subscriptions_Order {
	...
	public static function subscription_thank_you( $order_id ) {
		...
		if ( wcs_order_contains_subscription( $order_id, 'any' ) ) {
			...
			$thank_you_message            = '<p>' . _n( 'Your subscription will be activated when payment clears.', 'Your subscriptions will be activated when payment clears.', $subscription_count, 'ultimatewoo-pro' ) . '</p>';
			...
			$thank_you_message .= '<p>' . sprintf( _n( 'View the status of your subscription in %syour account%s.', 'View the status of your subscriptions in %syour account%s.', $subscription_count, 'ultimatewoo-pro' ), '<a href="' . $my_account_subscriptions_url . '">', '</a>' ) . '</p>';
	...
	*/
	public static function add_contains_subscription_column( $columns ) {
		...
		$column_header = '<span class="subscription_head tips" data-tip="' . esc_attr__( 'Subscription Relationship', 'ultimatewoo-pro' ) . '">' . esc_attr__( 'Subscription Relationship', 'ultimatewoo-pro' ) . '</span>';
	...
	*/
	public static function add_contains_subscription_column_content( $column ) {
		...
		if ( 'subscription_relationship' == $column ) {
			if ( wcs_order_contains_subscription( $post->ID, 'renewal' ) ) {
				echo '<span class="subscription_renewal_order tips" data-tip="' . esc_attr__( 'Renewal Order', 'ultimatewoo-pro' ) . '"></span>';
			} elseif ( wcs_order_contains_subscription( $post->ID, 'resubscribe' ) ) {
				echo '<span class="subscription_resubscribe_order tips" data-tip="' . esc_attr__( 'Resubscribe Order', 'ultimatewoo-pro' ) . '"></span>';
			} elseif ( wcs_order_contains_subscription( $post->ID, 'parent' ) ) {
				echo '<span class="subscription_parent_order tips" data-tip="' . esc_attr__( 'Parent Order', 'ultimatewoo-pro' ) . '"></span>';
			} else {
	...
	public static function restrict_manage_subscriptions() {
		...
		<select name='shop_order_subtype' id='dropdown_shop_order_subtype'>
			<option value=""><?php esc_html_e( 'All orders types', 'ultimatewoo-pro' ); ?></option>
			<?php
			$order_types = apply_filters( 'woocommerce_subscriptions_order_type_dropdown', array(
				'original'    => _x( 'Original', 'An order type', 'ultimatewoo-pro' ),
				'parent'      => _x( 'Subscription Parent', 'An order type', 'ultimatewoo-pro' ),
				'renewal'     => _x( 'Subscription Renewal', 'An order type', 'ultimatewoo-pro' ),
				'resubscribe' => _x( 'Subscription Resubscribe', 'An order type', 'ultimatewoo-pro' ),
				'switch'      => _x( 'Subscription Switch', 'An order type', 'ultimatewoo-pro' ),
				'regular'     => _x( 'Non-subscription', 'An order type', 'ultimatewoo-pro' ),
			) );
		...
		if ( wcs_order_contains_subscription( $order, array( 'parent', 'renewal' ) ) ) {
			...
			foreach ( $subscriptions as $subscription ) {
				...
				if ( wcs_get_objects_property( $order, 'id' ) == $latest_order && $subscription->has_status( 'pending-cancel' ) && $subscription->can_be_updated_to( 'cancelled' ) ) {
					...
					$subscription->update_status( 'cancelled', wp_kses( sprintf( __( 'Subscription cancelled for refunded order %1$s#%2$s%3$s.', 'ultimatewoo-pro' ), sprintf( '<a href="%s">', esc_url( wcs_get_edit_post_link( wcs_get_objects_property( $order, 'id' ) ) ) ), $order->get_order_number(), '</a>' ), array( 'a' => array( 'href' => true ) ) ) );
				}
			}
		}
	}
