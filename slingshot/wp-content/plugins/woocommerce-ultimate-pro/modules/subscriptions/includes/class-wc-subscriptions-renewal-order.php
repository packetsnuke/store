...
class WC_Subscriptions_Renewal_Order {
	...
	public static function add_order_note( $renewal_order, $subscription ) {
		...
		if ( is_a( $renewal_order, 'WC_Order' ) && wcs_is_subscription( $subscription ) ) {
			...
			$order_number = sprintf( _x( '#%s', 'hash before order number', 'ultimatewoo-pro' ), $renewal_order->get_order_number() );

			// translators: placeholder is order ID
			$subscription->add_order_note( sprintf( __( 'Order %s created to record renewal.', 'ultimatewoo-pro' ), sprintf( '<a href="%s">%s</a> ', esc_url( wcs_get_edit_post_link( wcs_get_objects_property( $renewal_order, 'id' ) ) ), $order_number ) ) );
		}
	...
	public static function prevent_cancelling_renewal_orders() {
		if ( isset( $_GET['cancel_order'] ) && isset( $_GET['order'] ) && isset( $_GET['order_id'] ) ) {
			...
			if ( wcs_order_contains_renewal( $order ) ) {
				...
				wc_add_notice( __( 'Subscription renewal orders cannot be cancelled.', 'ultimatewoo-pro' ), 'notice' );
