...
class WC_Subscriptions_Manager {
	...
	public static function prepare_renewal( $subscription_id ) {
		...
		$order_note = _x( 'Subscription renewal payment due:', 'used in order note as reason for why subscription status changed', 'ultimatewoo-pro' );
	...
	public static function process_renewal( $subscription_id, $required_status, $order_note ) {
		...
		if ( ! empty( $subscription ) && $subscription->has_status( $required_status ) && ( 0 == $subscription->get_total() || $subscription->is_manual() || '' == $subscription->get_payment_method() || ! $subscription->payment_method_supports( 'gateway_scheduled_payments' ) ) ) {
			...
			if ( is_wp_error( $renewal_order ) ) {
				...
				if ( is_wp_error( $renewal_order ) ) {
					throw new Exception( sprintf( __( 'Error: Unable to create renewal order with note "%s"', 'ultimatewoo-pro' ), $order_note ) );
				}
			}
	...
	public static function expire_subscription( $subscription_id, $deprecated = null ) {
		...
		if ( false === $subscription ) {
			throw new InvalidArgumentException( sprintf( __( 'Subscription doesn\'t exist in scheduled action: %d', 'ultimatewoo-pro' ), $subscription_id ) );
		}
		...
		if ( ! empty( $subscriptions ) ) {
			...
			foreach ( $subscriptions as $subscription ) {
				...
				try {
					...
					$subscription->add_order_note( sprintf( __( 'Failed to activate subscription status for order #%1$s: %2$s', 'ultimatewoo-pro' ), is_object( $order ) ? $order->get_order_number() : $order, $e->getMessage() ) );
				}
			}
		...
		if ( ! empty( $subscriptions ) ) {
			...
			foreach ( $subscriptions as $subscription ) {
				...
				try {
					...
					// translators: $1: order number, $2: error message
					$subscription->add_order_note( sprintf( __( 'Failed to update subscription status after order #%1$s was put on-hold: %2$s', 'ultimatewoo-pro' ), is_object( $order ) ? $order->get_order_number() : $order, $e->getMessage() ) );
				}
			}
		...
		if ( ! empty( $subscriptions ) ) {
			...
			foreach ( $subscriptions as $subscription ) {
				...
				try {
					...
					// translators: $1: order number, $2: error message
					$subscription->add_order_note( sprintf( __( 'Failed to cancel subscription after order #%1$s was cancelled: %2$s', 'ultimatewoo-pro' ), is_object( $order ) ? $order->get_order_number() : $order, $e->getMessage() ) );
				}
			}
		...
		if ( ! empty( $subscriptions ) ) {
			...
			foreach ( $subscriptions as $subscription ) {
				...
				try {
					...
					// translators: $1: order number, $2: error message
					$subscription->add_order_note( sprintf( __( 'Failed to set subscription as expired for order #%1$s: %2$s', 'ultimatewoo-pro' ), is_object( $order ) ? $order->get_order_number() : $order, $e->getMessage() ) );
				}
			}
	...
	public static function failed_subscription_sign_ups_for_order( $order ) {
		...
		if ( ! empty( $subscriptions ) ) {
			...
			if ( $order->has_status( 'failed' ) ) {
				$order->update_status( 'failed', __( 'Subscription sign up failed.', 'ultimatewoo-pro' ) );
			}
			...
			foreach ( $subscriptions as $subscription ) {
				...
				try {
					...
					// translators: $1: order number, $2: error message
					$subscription->add_order_note( sprintf( __( 'Failed to process failed payment on subscription for order #%1$s: %2$s', 'ultimatewoo-pro' ), is_object( $order ) ? $order->get_order_number() : $order, $e->getMessage() ) );
				}
			}
	...
	public static function create_pending_subscription_for_order( $order, $product_id, $args = array() ) {
		...
		if ( ! empty( $subscriptions ) ) {
			...
			if ( is_wp_error( $subscription ) ) {
				throw new Exception( __( 'Error: Unable to create subscription. Please try again.', 'ultimatewoo-pro' ) );
			}
			...
			if ( ! $item_id ) {
				throw new Exception( __( 'Error: Unable to add product to created subscription. Please try again.', 'ultimatewoo-pro' ) );
			}
		}
		...
		$subscription->add_order_note( __( 'Pending subscription created.', 'ultimatewoo-pro' ) );
	...
	public static function get_status_to_display( $status, $subscription_key = '', $user_id = 0 ) {
		...
		switch ( $status ) {
			case 'active' :
				$status_string = _x( 'Active', 'Subscription status', 'ultimatewoo-pro' );
				break;
			case 'cancelled' :
				$status_string = _x( 'Cancelled', 'Subscription status', 'ultimatewoo-pro' );
				break;
			case 'expired' :
				$status_string = _x( 'Expired', 'Subscription status', 'ultimatewoo-pro' );
				break;
			case 'pending' :
				$status_string = _x( 'Pending', 'Subscription status', 'ultimatewoo-pro' );
				break;
			case 'failed' :
				$status_string = _x( 'Failed', 'Subscription status', 'ultimatewoo-pro' );
				break;
			case 'on-hold' :
			case 'suspend' : // Backward compatibility
				$status_string = _x( 'On-hold', 'Subscription status', 'ultimatewoo-pro' );
				break;
			default :
	...
	public static function touch_time( $args = array() ) {
		...
		for ( $i = 1; $i < 13; $i = $i + 1 ) {
			...
			$month_input .= '>' . sprintf( _x( '%1$s-%2$s', 'used in a select box', 'ultimatewoo-pro' ), $month_numeral, $wp_locale->get_month_abbrev( $wp_locale->get_month( $i ) ) ) . "</option>\n";
		}
		$month_input .= '</select>';
		...
		if ( $args['include_time'] ) {
			...
			$touch_time = sprintf( __( '%1$s%2$s, %3$s @ %4$s : %5$s', 'ultimatewoo-pro' ), $month_input, $day_input, $year_input, $hour_input, $minute_input );

		} else {
			// translators: all fields are full html nodes: 1$: month input, 2$: day input, 3$: year input. Change the order if you'd like
			$touch_time = sprintf( __( '%1$s%2$s, %3$s', 'ultimatewoo-pro' ), $month_input, $day_input, $year_input );
		}

		if ( $args['include_buttons'] ) {
			$touch_time .= '<p>';
			$touch_time .= '<a href="#edit_timestamp" class="save-timestamp hide-if-no-js button">' . __( 'Change', 'ultimatewoo-pro' ) . '</a>';
			$touch_time .= '<a href="#edit_timestamp" class="cancel-timestamp hide-if-no-js">' . _x( 'Cancel', 'an action on a subscription', 'ultimatewoo-pro' ) . '</a>';
			$touch_time .= '</p>';
		}
	...
	public static function maybe_put_subscription_on_hold( $user_id, $subscription_key ) {
		...
		if ( 0 == $subscription->get_total() || $subscription->is_manual() || '' == $subscription->get_payment_method() || ! $subscription->payment_method_supports( 'gateway_scheduled_payments' ) ) {
			$subscription->update_status( 'on-hold', _x( 'Subscription renewal payment due:', 'used in order note as reason for why subscription status changed', 'ultimatewoo-pro' ) );
		}
	}
	...
	public static function failed_subscription_signup( $user_id, $subscription_key ) {
		...
		$subscription->get_parent()->add_order_note( sprintf( __( 'Failed sign-up for subscription %s.', 'ultimatewoo-pro' ), $subscription->get_id() ) );
	...
	public static function ajax_update_next_payment_date() {
		...
		if ( ! wp_verify_nonce( $_POST['wcs_nonce'], 'ultimatewoo-pro' ) ) {

			$response['message'] = '<div class="error">' . __( 'Invalid security token, please reload the page and try again.', 'ultimatewoo-pro' ) . '</div>';

		} elseif ( ! current_user_can( 'manage_woocommerce' ) ) {

			$response['message'] = '<div class="error">' . __( 'Only store managers can edit payment dates.', 'ultimatewoo-pro' ) . '</div>';

		} elseif ( empty( $_POST['wcs_day'] ) || empty( $_POST['wcs_month'] ) || empty( $_POST['wcs_year'] ) ) {

			$response['message'] = '<div class="error">' . __( 'Please enter all date fields.', 'ultimatewoo-pro' ) . '</div>';
			...
			if ( is_wp_error( $new_payment_timestamp ) ) {
				...
				if ( $time_diff > 0 && $time_diff < 7 * 24 * 60 * 60 ) {
					...
					$date_to_display = sprintf( __( 'In %s', 'ultimatewoo-pro' ), human_time_diff( gmdate( 'U' ), $new_payment_timestamp ) );
				} else {
					$date_to_display = date_i18n( wc_date_format(), $new_payment_timestamp_user_time );
				}

				$response['status']        = 'success';
				$response['message']       = '<div class="updated">' . __( 'Date Changed', 'ultimatewoo-pro' ) . '</div>';
