...
class WC_Report_Subscription_By_Customer extends WP_List_Table {
	...
	public function __construct() {
		parent::__construct( array(
			'singular'  => __( 'Customer', 'ultimatewoo-pro' ),
			'plural'    => __( 'Customers', 'ultimatewoo-pro' ),
			'ajax'      => false,
		) );
	}
	...
	public function no_items() {
		esc_html_e( 'No customers found.', 'ultimatewoo-pro' );
	}
	...
	public function output_report() {
		...
		echo '	<h3>' . esc_html__( 'Customer Totals', 'ultimatewoo-pro' ) . '</h3>';
		echo '	<p><strong>' . esc_html__( 'Total Subscribers', 'ultimatewoo-pro' ) . '</strong> : ' . esc_html( $this->totals->total_customers ) . '<br />';
		echo '	<strong>' . esc_html__( 'Active Subscriptions', 'ultimatewoo-pro' ) . '</strong> : ' . esc_html( $this->totals->active_subscriptions ) . '<br />';
		echo '	<strong>' . esc_html__( 'Total Subscriptions', 'ultimatewoo-pro' ) . '</strong> : ' . esc_html( $this->totals->total_subscriptions ) . '<br />';
		echo '	<strong>' . esc_html__( 'Total Subscription Orders', 'ultimatewoo-pro' ) . '</strong> : ' . esc_html( $this->totals->initial_order_count + $this->totals->renewal_switch_count ) . '<br />';
		echo '	<strong>' . esc_html__( 'Average Lifetime Value', 'ultimatewoo-pro' ) . '</strong> : ' . wp_kses_post( wc_price( ( $this->totals->initial_order_total + $this->totals->renewal_switch_total ) / $this->totals->total_customers ) ) . '</p>';
		echo '</div></div>';
		$this->display();
		echo '</div>';
	...
	public function get_columns() {
		$columns = array(
			'customer_name'                  => __( 'Customer', 'ultimatewoo-pro' ),
			'active_subscription_count'      => sprintf( __( 'Active Subscriptions %s', 'ultimatewoo-pro' ), wcs_help_tip( __( 'The number of subscriptions this customer has with a status of active or pending cancellation.', 'ultimatewoo-pro' ) ) ),
			'total_subscription_count'       => sprintf( __( 'Total Subscriptions %s', 'ultimatewoo-pro' ), wcs_help_tip( __( 'The number of subscriptions this customer has with a status other than pending or trashed.', 'ultimatewoo-pro' ) ) ),
			'total_subscription_order_count' => sprintf( __( 'Total Subscription Orders %s', 'ultimatewoo-pro' ), wcs_help_tip( __( 'The number of sign-up, switch and renewal orders this customer has placed with your store with a paid status (i.e. processing or complete).', 'ultimatewoo-pro' ) ) ),
			'customer_lifetime_value'        => sprintf( __( 'Lifetime Value from Subscriptions %s', 'ultimatewoo-pro' ), wcs_help_tip( __( 'The total value of this customer\'s sign-up, switch and renewal orders.', 'ultimatewoo-pro' ) ) ),
		);
