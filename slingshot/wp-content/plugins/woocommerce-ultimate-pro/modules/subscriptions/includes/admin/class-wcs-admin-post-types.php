...
class WCS_Admin_Post_Types {
	...
	public function restrict_by_product() {
		...
		WCS_Select2::render( array(
			...
			'name'        => '_wcs_product',
			'placeholder' => esc_attr__( 'Search for a product&hellip;', 'ultimatewoo-pro' ),
	...
	public function print_bulk_actions_script() {
		...
		$bulk_actions = apply_filters( 'woocommerce_subscription_bulk_actions', array(
			'active'    => _x( 'Activate', 'an action on a subscription', 'ultimatewoo-pro' ),
			'on-hold'   => _x( 'Put on-hold', 'an action on a subscription', 'ultimatewoo-pro' ),
			'cancelled' => _x( 'Cancel', 'an action on a subscription', 'ultimatewoo-pro' ),
		) );
	...
	public function parse_bulk_actions() {
		...
		foreach ( $subscription_ids as $subscription_id ) {
			...
			$order_note   = _x( 'Subscription status changed by bulk edit:', 'Used in order note. Reason why status changed.', 'ultimatewoo-pro' );
	...
	public function bulk_admin_notices() {
		...
		foreach ( $subscription_statuses as $slug => $name ) {
			...
			if ( isset( $_REQUEST[ 'marked_' . str_replace( 'wc-', '', $slug ) ] ) ) {
				...
				$message = sprintf( _n( '%s subscription status changed.', '%s subscription statuses changed.', $number, 'ultimatewoo-pro' ), number_format_i18n( $number ) );
				...
				if ( ! empty( $_REQUEST['error_count'] ) ) {
					...
					$message = sprintf( _n( '%1$s subscription could not be updated: %2$s', '%1$s subscriptions could not be updated: %2$s', $error_count, 'ultimatewoo-pro' ), number_format_i18n( $error_count ), $error_msg );
	...
	public function shop_subscription_columns( $existing_columns ) {
		...
		$columns = array(
			...
			'order_title'       => __( 'Subscription', 'ultimatewoo-pro' ),
			'order_items'       => __( 'Items', 'ultimatewoo-pro' ),
			'recurring_total'   => __( 'Total', 'ultimatewoo-pro' ),
			'start_date'        => __( 'Start Date', 'ultimatewoo-pro' ),
			'trial_end_date'    => __( 'Trial End', 'ultimatewoo-pro' ),
			'next_payment_date' => __( 'Next Payment', 'ultimatewoo-pro' ),
			'last_payment_date' => __( 'Last Order Date', 'ultimatewoo-pro' ), // Keep deprecated 'last_payment_date' key for backward compatibility
			'end_date'          => __( 'End Date', 'ultimatewoo-pro' ),
			'orders'            => _x( 'Orders', 'number of orders linked to a subscription', 'ultimatewoo-pro' ),
		);
	...
	public function render_shop_subscription_columns( $column ) {
		...
		switch ( $column ) {
			case 'status' :
				...
				$all_statuses = array(
					'active'    => __( 'Reactivate', 'ultimatewoo-pro' ),
					'on-hold'   => __( 'Suspend', 'ultimatewoo-pro' ),
					'cancelled' => _x( 'Cancel', 'an action on a subscription', 'ultimatewoo-pro' ),
					'trash'     => __( 'Trash', 'ultimatewoo-pro' ),
					'deleted'   => __( 'Delete Permanently', 'ultimatewoo-pro' ),
				);
						...
						if ( in_array( $status, array( 'trash', 'deleted' ) ) ) {
							...
							if ( current_user_can( $post_type_object->cap->delete_post, $post->ID ) ) {
								...
								if ( 'trash' == $post->post_status ) {
									$actions['untrash'] = '<a title="' . esc_attr( __( 'Restore this item from the Trash', 'ultimatewoo-pro' ) ) . '" href="' . wp_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $post->ID ) ), 'untrash-post_' . $post->ID ) . '">' . __( 'Restore', 'ultimatewoo-pro' ) . '</a>';
								} elseif ( EMPTY_TRASH_DAYS ) {
									$actions['trash'] = '<a class="submitdelete" title="' . esc_attr( __( 'Move this item to the Trash', 'ultimatewoo-pro' ) ) . '" href="' . get_delete_post_link( $post->ID ) . '">' . __( 'Trash', 'ultimatewoo-pro' ) . '</a>';
								}

								if ( 'trash' == $post->post_status || ! EMPTY_TRASH_DAYS ) {
									$actions['delete'] = '<a class="submitdelete" title="' . esc_attr( __( 'Delete this item permanently', 'ultimatewoo-pro' ) ) . '" href="' . get_delete_post_link( $post->ID, '', true ) . '">' . __( 'Delete Permanently', 'ultimatewoo-pro' ) . '</a>';
								}
							}
						} else {

							if ( 'pending-cancel' === $the_subscription->get_status() ) {
								$label = __( 'Cancel Now', 'ultimatewoo-pro' );
							}
			...
			case 'order_title' :
				...
				if ( $address = $the_subscription->get_formatted_billing_address() ) {
					$customer_tip .= _x( 'Billing:', 'meaning billing address', 'ultimatewoo-pro' ) . ' ' . esc_html( $address );
				}

				if ( $the_subscription->get_billing_email() ) {
					// translators: placeholder is customer's billing email
					$customer_tip .= '<br/><br/>' . sprintf( __( 'Email: %s', 'ultimatewoo-pro' ), esc_attr( $the_subscription->get_billing_email() ) );
				}

				if ( $the_subscription->get_billing_phone() ) {
					// translators: placeholder is customer's billing phone number
					$customer_tip .= '<br/><br/>' . sprintf( __( 'Tel: %s', 'ultimatewoo-pro' ), esc_html( $the_subscription->get_billing_phone() ) );
				}
				...
				$column_content = sprintf( _x( '%1$s#%2$s%3$s for %4$s', 'Subscription title on admin table. (e.g.: #211 for John Doe)', 'ultimatewoo-pro' ), '<a href="' . esc_url( admin_url( 'post.php?post=' . absint( $post->ID ) . '&action=edit' ) ) . '">', '<strong>' . esc_attr( $the_subscription->get_order_number() ) . '</strong>', '</a>', $username );

				$column_content .= '</div>';

				$column_content .= '<button type="button" class="toggle-row"><span class="screen-reader-text">' . __( 'Show more details', 'ultimatewoo-pro' ) . '</span></button>';
			...
			case 'order_items' :
				...
				switch ( count( $subscription_items ) ) {
					...
					case 1 :
						...
						break;
					default :
						$column_content .= '<a href="#" class="show_order_items">' . esc_html( apply_filters( 'woocommerce_admin_order_item_count', sprintf( _n( '%d item', '%d items', $the_subscription->get_item_count(), 'ultimatewoo-pro' ), $the_subscription->get_item_count() ), $the_subscription ) ) . '</a>';
			...
			case 'recurring_total' :
				...
				$column_content .= '<small class="meta">' . esc_html( sprintf( __( 'Via %s', 'ultimatewoo-pro' ), $the_subscription->get_payment_method_to_display() ) ) . '</small>';
				break;
			...
			case 'end_date':
				...
				if ( 0 == $the_subscription->get_time( $date_type, 'gmt' ) ) {
					...
					$column_content .= sprintf( '<time class="%s" title="%s">%s</time>', esc_attr( $column ), esc_attr( date( __( 'Y/m/d g:i:s A', 'ultimatewoo-pro' ) , $the_subscription->get_time( $date_type, 'site' ) ) ), esc_html( $the_subscription->get_date_to_display( $date_type ) ) );
					...
					if ( 'next_payment_date' == $column && $the_subscription->payment_method_supports( 'gateway_scheduled_payments' ) && ! $the_subscription->is_manual() && $the_subscription->has_status( 'active' ) ) {
						$column_content .= '<div class="woocommerce-help-tip" data-tip="' . esc_attr__( 'This date should be treated as an estimate only. The payment gateway for this subscription controls when payments are processed.', 'ultimatewoo-pro' ) . '"></div>';
					}
				}
	...
	public function post_updated_messages( $messages ) {
		...
		$messages['shop_subscription'] = array(
			...
			1 => __( 'Subscription updated.', 'ultimatewoo-pro' ),
			2 => __( 'Custom field updated.', 'ultimatewoo-pro' ),
			3 => __( 'Custom field deleted.', 'ultimatewoo-pro' ),
			4 => __( 'Subscription updated.', 'ultimatewoo-pro' ),
			// translators: placeholder is previous post title
			5 => isset( $_GET['revision'] ) ? sprintf( _x( 'Subscription restored to revision from %s', 'used in post updated messages', 'ultimatewoo-pro' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			6 => __( 'Subscription updated.', 'ultimatewoo-pro' ),
			7 => __( 'Subscription saved.', 'ultimatewoo-pro' ),
			8 => __( 'Subscription submitted.', 'ultimatewoo-pro' ),
			// translators: php date string
			9 => sprintf( __( 'Subscription scheduled for: %1$s.', 'ultimatewoo-pro' ), '<strong>' . date_i18n( _x( 'M j, Y @ G:i', 'used in "Subscription scheduled for <date>"', 'ultimatewoo-pro' ), wcs_date_to_time( $post->post_date ) ) . '</strong>' ),
			10 => __( 'Subscription draft updated.', 'ultimatewoo-pro' ),
		);
	...
	public static function restrict_by_payment_method() {
		...
		<select class="wcs_payment_method_selector" name="_payment_method" id="_payment_method" class="first">
			<option value=""><?php esc_html_e( 'Any Payment Method', 'ultimatewoo-pro' ) ?></option>
			<option value="none" <?php echo esc_attr( 'none' == $selected_gateway_id ? 'selected' : '' ) . '>' . esc_html__( 'None', 'ultimatewoo-pro' ) ?></option>
		<?php
	...
	public static function restrict_by_customer() {
		...
		if ( ! empty( $_GET['_customer_user'] ) ) {
			...
			$user_string = sprintf(
				...
				esc_html__( '%1$s (#%2$s &ndash; %3$s)', 'ultimatewoo-pro' ),
				$user->display_name,
				absint( $user->ID ),
				$user->user_email
			);
		...
		?>
		<select class="wc-customer-search" name="_customer_user" data-placeholder="<?php esc_attr_e( 'Search for a customer&hellip;', 'ultimatewoo-pro' ); ?>" data-allow_clear="true">
