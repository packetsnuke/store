...
class WCS_Admin_Meta_Boxes {
	...
	public function add_meta_boxes() {
		...
		add_meta_box( 'woocommerce-subscription-data', _x( 'Subscription Data', 'meta box title', 'ultimatewoo-pro' ), 'WCS_Meta_Box_Subscription_Data::output', 'shop_subscription', 'normal', 'high' );

		add_meta_box( 'woocommerce-subscription-schedule', _x( 'Billing Schedule', 'meta box title', 'ultimatewoo-pro' ), 'WCS_Meta_Box_Schedule::output', 'shop_subscription', 'side', 'default' );

		remove_meta_box( 'woocommerce-order-data', 'shop_subscription', 'normal' );

		add_meta_box( 'subscription_renewal_orders', __( 'Related Orders', 'ultimatewoo-pro' ), 'WCS_Meta_Box_Related_Orders::output', 'shop_subscription', 'normal', 'low' );
		...
		if ( 'shop_order' === get_post_type( $post_ID ) && wcs_order_contains_subscription( $post_ID, 'any' ) ) {
			add_meta_box( 'subscription_renewal_orders', __( 'Related Orders', 'ultimatewoo-pro' ), 'WCS_Meta_Box_Related_Orders::output', 'shop_order', 'normal', 'low' );
		}
	}
	...
	public function enqueue_styles_scripts() {
		...
		if ( 'shop_subscription' == $screen->id ) {
			...
			wp_localize_script( 'wcs-admin-meta-boxes-subscription', 'wcs_admin_meta_boxes', apply_filters( 'woocommerce_subscriptions_admin_meta_boxes_script_parameters', array(
				'i18n_start_date_notice'         => __( 'Please enter a start date in the past.', 'ultimatewoo-pro' ),
				'i18n_past_date_notice'          => __( 'Please enter a date at least one hour into the future.', 'ultimatewoo-pro' ),
				'i18n_next_payment_start_notice' => __( 'Please enter a date after the trial end.', 'ultimatewoo-pro' ),
				'i18n_next_payment_trial_notice' => __( 'Please enter a date after the start date.', 'ultimatewoo-pro' ),
				'i18n_trial_end_start_notice'    => __( 'Please enter a date after the start date.', 'ultimatewoo-pro' ),
				'i18n_trial_end_next_notice'     => __( 'Please enter a date before the next payment.', 'ultimatewoo-pro' ),
				'i18n_end_date_notice'           => __( 'Please enter a date after the next payment.', 'ultimatewoo-pro' ),
				'process_renewal_action_warning' => __( "Are you sure you want to process a renewal?\n\nThis will charge the customer and email them the renewal order (if emails are enabled).", 'ultimatewoo-pro' ),
			...
			wp_localize_script( 'wcs-admin-meta-boxes-order', 'wcs_admin_order_meta_boxes', array(
				'retry_renewal_payment_action_warning' => __( "Are you sure you want to retry payment for this renewal order?\n\nThis will attempt to charge the customer and send renewal order emails (if emails are enabled).", 'ultimatewoo-pro' ),
				)
			);
		}
	}
	...
	public static function add_subscription_actions( $actions ) {
		...
		if ( wcs_is_subscription( $theorder ) && ! $theorder->has_status( wcs_get_subscription_ended_statuses() ) ) {
			...
			if ( $theorder->payment_method_supports( 'subscription_date_changes' ) && $theorder->has_status( 'active' ) ) {
				$actions['wcs_process_renewal'] = esc_html__( 'Process renewal', 'ultimatewoo-pro' );
			}

			$actions['wcs_create_pending_renewal'] = esc_html__( 'Create pending renewal order', 'ultimatewoo-pro' );

		} else if ( self::can_renewal_order_be_retried( $theorder ) ) {
			$actions['wcs_retry_renewal_payment'] = esc_html__( 'Retry Renewal Payment', 'ultimatewoo-pro' );
		}
	...
	public static function process_renewal_action_request( $subscription ) {
		...
		$subscription->add_order_note( __( 'Process renewal order action requested by admin.', 'ultimatewoo-pro' ), false, true );
	}
	...
	public static function create_pending_renewal_action_request( $subscription ) {
		...
		$subscription->add_order_note( __( 'Create pending renewal order requested by admin action.', 'ultimatewoo-pro' ), false, true );
	}
