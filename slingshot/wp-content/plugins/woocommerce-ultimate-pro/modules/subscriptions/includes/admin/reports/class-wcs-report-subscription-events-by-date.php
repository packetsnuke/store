...
class WC_Report_Subscription_Events_By_Date extends WC_Admin_Report {
	...
	public function get_chart_legend() {
		...
		$legend[] = array(
			'title'            => sprintf( __( '%s signup revenue in this period', 'ultimatewoo-pro' ), '<strong>' . wc_price( $data->signup_orders_total_amount ) . '</strong>' ),
			'placeholder'      => __( 'The sum of all subscription parent orders, including other items, fees, tax and shipping.', 'ultimatewoo-pro' ),
			'color'            => $this->chart_colours['signup_total'],
			'highlight_series' => 8,
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s renewal revenue in this period', 'ultimatewoo-pro' ), '<strong>' . wc_price( $data->renewal_orders_total_amount ) . '</strong>' ),
			'placeholder'      => __( 'The sum of all renewal orders including tax and shipping.', 'ultimatewoo-pro' ),
			'color'            => $this->chart_colours['renewal_total'],
			'highlight_series' => 10,
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s resubscribe revenue in this period', 'ultimatewoo-pro' ), '<strong>' . wc_price( $data->resubscribe_orders_total_amount ) . '</strong>' ),
			'placeholder'      => __( 'The sum of all resubscribe orders including tax and shipping.', 'ultimatewoo-pro' ),
			'color'            => $this->chart_colours['resubscribe_total'],
			'highlight_series' => 9,
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s new subscriptions', 'ultimatewoo-pro' ), '<strong>' . $this->report_data->new_subscription_total_count . '</strong>' ),
			'placeholder'      => __( 'The number of subscriptions created during this period, either by being manually created, imported or a customer placing an order.', 'ultimatewoo-pro' ),
			'color'            => $this->chart_colours['new_count'],
			'highlight_series' => 1,
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s subscription signups', 'ultimatewoo-pro' ), '<strong>' . $this->report_data->signup_orders_total_count . '</strong>' ),
			'placeholder'      => __( 'The number of subscription parent orders created during this period. This represents the new subscriptions created by customers placing an order via checkout.', 'ultimatewoo-pro' ),
			'color'            => $this->chart_colours['signup_count'],
			'highlight_series' => 2,
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s subscription resubscribes', 'ultimatewoo-pro' ), '<strong>' . $data->resubscribe_orders_total_count . '</strong>' ),
			'placeholder'      => __( 'The number of resubscribe orders processed during this period.', 'ultimatewoo-pro' ),
			'color'            => $this->chart_colours['resubscribe_count'],
			'highlight_series' => 3,
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s subscription renewals', 'ultimatewoo-pro' ), '<strong>' . $data->renewal_orders_total_count . '</strong>' ),
			'placeholder'      => __( 'The number of renewal orders processed during this period.', 'ultimatewoo-pro' ),
			'color'            => $this->chart_colours['renewal_count'],
			'highlight_series' => 4,
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s subscription switches', 'ultimatewoo-pro' ), '<strong>' . $data->switch_orders_total_count . '</strong>' ),
			'placeholder'      => __( 'The number of subscriptions upgraded, downgraded or cross-graded during this period.', 'ultimatewoo-pro' ),
			'color'            => $this->chart_colours['switch_count'],
			'highlight_series' => 0,
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s subscription cancellations', 'ultimatewoo-pro' ), '<strong>' . $data->total_subscriptions_cancelled . '</strong>' ),
			'placeholder'      => __( 'The number of subscriptions cancelled by the customer or store manager during this period.  The pre-paid term may not yet have ended during this period.', 'ultimatewoo-pro' ),
			'color'            => $this->chart_colours['cancel_count'],
			'highlight_series' => 7,
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s subscriptions ended', 'ultimatewoo-pro' ), '<strong>' . $data->total_subscriptions_ended . '</strong>' ),
			'placeholder'      => __( 'The number of subscriptions which have either expired or reached the end of the prepaid term if it was previously cancelled.', 'ultimatewoo-pro' ),
			'color'            => $this->chart_colours['ended_count'],
			'highlight_series' => 6,
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s current subscriptions', 'ultimatewoo-pro' ), '<strong>' . $data->total_subscriptions_at_period_end . '</strong>' ),
			'placeholder'      => __( 'The number of subscriptions during this period with an end date in the future and a status other than pending.', 'ultimatewoo-pro' ),
		...
		if ( $data->total_subscriptions_at_period_end - $data->total_subscriptions_at_period_start >= 0 ) {
			$legend_title = __( '%s net subscription gain', 'ultimatewoo-pro' );
		} else {
			$legend_title = __( '%s net subscription loss', 'ultimatewoo-pro' );
		}
		...
		$legend[] = array(
			...
			'placeholder'      => __( 'Change in subscriptions between the start and end of the period.', 'ultimatewoo-pro' ),
	...
	public function output_report() {
		...
		$ranges = array(
			...
			'last_month'   => __( 'Last Month', 'ultimatewoo-pro' ),
			'month'        => __( 'This Month', 'ultimatewoo-pro' ),
			'7day'         => __( 'Last 7 Days', 'ultimatewoo-pro' ),
		);
	...
	public function get_export_button() {
		...
		<a
			...
			class="export_csv"
			data-export="chart"
			data-xaxes="<?php esc_attr_e( 'Date', 'ultimatewoo-pro' ); ?>"
			data-exclude_series="2"
			data-groupby="<?php echo esc_attr( $this->chart_groupby ); ?>"
		>
			<?php esc_attr_e( 'Export CSV', 'ultimatewoo-pro' ); ?>
		</a>
		<?php
	}
		...
		<script type="text/javascript">
			...
			jQuery(function(){
				...
				var drawGraph = function( highlight ) {
					var series = [
						{
							label: "<?php echo esc_js( __( 'Switched subscriptions', 'ultimatewoo-pro' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'New Subscriptions', 'ultimatewoo-pro' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Subscriptions signups', 'ultimatewoo-pro' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Number of resubscribes', 'ultimatewoo-pro' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Number of renewals', 'ultimatewoo-pro' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Subscriptions', 'ultimatewoo-pro' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Subscriptions Ended', 'ultimatewoo-pro' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Cancellations', 'ultimatewoo-pro' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Signup Totals', 'ultimatewoo-pro' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Resubscribe Totals', 'ultimatewoo-pro' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Renewal Totals', 'ultimatewoo-pro' ) ) ?>",
