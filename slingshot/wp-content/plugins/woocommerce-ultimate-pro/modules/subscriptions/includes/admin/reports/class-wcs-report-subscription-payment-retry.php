...
class WC_Report_Subscription_Payment_Retry extends WC_Admin_Report {
	...
	public function get_chart_legend() {
		...
		$legend[] = array(
			'title'            => sprintf( __( '%s renewal revenue recovered', 'ultimatewoo-pro' ), '<strong>' . wc_price( $data->renewal_total_amount ) . '</strong>' ),
			'placeholder'      => __( 'The total amount of revenue, including tax and shipping, recovered with the failed payment retry system for renewal orders with a failed payment.', 'ultimatewoo-pro' ),
			'color'            => $this->chart_colours['renewal_total'],
			'highlight_series' => 3,
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s renewal orders', 'ultimatewoo-pro' ), '<strong>' . $data->renewal_total_count . '</strong>' ),
			'placeholder'      => __( 'The number of renewal orders which had a failed payment use the retry system.', 'ultimatewoo-pro' ),
			'color'            => $this->chart_colours['renewal_count'],
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s retry attempts succeeded', 'ultimatewoo-pro' ), '<strong>' . $data->retry_success_count . '</strong>' ),
			'placeholder'      => __( 'The number of renewal payment retries for this period which were able to process the payment which had previously failed one or more times.', 'ultimatewoo-pro' ),
			'color'            => $this->chart_colours['retry_success_count'],
			'highlight_series' => 0,
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s retry attempts failed', 'ultimatewoo-pro' ), '<strong>' . $data->retry_failed_count . '</strong>' ),
			'placeholder'      => __( 'The number of renewal payment retries for this period which did not result in a successful payment.', 'ultimatewoo-pro' ),
			'color'            => $this->chart_colours['retry_failure_count'],
			'highlight_series' => 1,
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s retry attempts pending', 'ultimatewoo-pro' ), '<strong>' . $data->retry_pending_count . '</strong>' ),
			'placeholder'      => __( 'The number of renewal payment retries not yet processed.', 'ultimatewoo-pro' ),
		...
		<script type="text/javascript">
			...
			jQuery(function(){
				...
				var drawGraph = function( highlight ) {
					var series = [
						{
							label: "<?php echo esc_js( __( 'Successful retries', 'ultimatewoo-pro' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Failed retries', 'ultimatewoo-pro' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Pending retries', 'ultimatewoo-pro' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Recovered Renewal Revenue', 'ultimatewoo-pro' ) ) ?>",
