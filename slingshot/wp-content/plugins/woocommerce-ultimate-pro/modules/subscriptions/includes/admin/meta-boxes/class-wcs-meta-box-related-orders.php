...
class WCS_Meta_Box_Related_Orders {
	...
	public static function output_rows( $post ) {
		...
		foreach ( $subscriptions as $subscription ) {
			wcs_set_objects_property( $subscription, 'relationship', __( 'Subscription', 'ultimatewoo-pro' ), 'set_prop_only' );
			$orders[] = $subscription;
		}
		...
		if ( wcs_is_subscription( $post->ID ) ) {
			...
			foreach ( $resubscribed_subscriptions as $subscription ) {
				...
				wcs_set_objects_property( $subscription, 'relationship', _x( 'Resubscribed Subscription', 'relation to order', 'ultimatewoo-pro' ), 'set_prop_only' );
				$orders[] = $subscription;
			}
		...
		foreach ( $initial_subscriptions as $subscription ) {
			wcs_set_objects_property( $subscription, 'relationship', _x( 'Initial Subscription', 'relation to order', 'ultimatewoo-pro' ), 'set_prop_only' );
			$orders[] = $subscription;
		}
		...
		if ( 1 == count( $subscriptions ) ) {
			foreach ( $subscriptions as $subscription ) {
				if ( $subscription->get_parent_id() ) {
					...
					wcs_set_objects_property( $order, 'relationship', _x( 'Parent Order', 'relation to order', 'ultimatewoo-pro' ), 'set_prop_only' );
					$orders[] = $order;
				}
			}
		}
		...
		foreach ( $subscriptions as $subscription ) {
			...
			foreach ( $subscription->get_related_orders( 'all', 'renewal' ) as $order ) {
				wcs_set_objects_property( $order, 'relationship', _x( 'Renewal Order', 'relation to order', 'ultimatewoo-pro' ), 'set_prop_only' );
				$orders[] = $order;
			}
		}
