...
class WC_Report_Upcoming_Recurring_Revenue extends WC_Admin_Report {
	...
	public function get_chart_legend() {
		...
		$legend[] = array(
			'title' => sprintf( __( '%s renewal income in this period', 'ultimatewoo-pro' ), '<strong>' . wc_price( $total_renewal_revenue ) . '</strong>' ),
			'color' => $this->chart_colours['renewals_amount'],
			'highlight_series' => 1,
		);
		$legend[] = array(
			'title' => sprintf( __( '%s renewal orders', 'ultimatewoo-pro' ), '<strong>' . $total_renewal_count . '</strong>' ),
			'color' => $this->chart_colours['renewals_count'],
			'highlight_series' => 0,
		);
		$legend[] = array(
			'title' => sprintf( __( '%s average renewal amount', 'ultimatewoo-pro' ), '<strong>' . wc_price( $this->average_sales ) . '</strong>' ),
	...
	public function output_report() {
		...
		$ranges = array(
			'year'       => __( 'Next 12 Months', 'ultimatewoo-pro' ),
			'month'      => __( 'Next 30 Days', 'ultimatewoo-pro' ),
			'last_month' => __( 'Next Month', 'ultimatewoo-pro' ), // misnomer to match historical reports keys, handy for caching
			'7day'       => __( 'Next 7 Days', 'ultimatewoo-pro' ),
		);
	...
	public function get_export_button() {
		...
		>
			<?php esc_html_e( 'Export CSV', 'ultimatewoo-pro' ); ?>
		</a>
		<?php
	}
		...
		<script type="text/javascript">
			...
			jQuery(function(){
				...
				var drawGraph = function( highlight ) {
					var series = [
						{
							label: "<?php echo esc_js( __( 'Renewals count', 'ultimatewoo-pro' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Renewals amount', 'ultimatewoo-pro' ) ) ?>",
