...
class WCS_Meta_Box_Subscription_Data extends WC_Meta_Box_Order_Data {
	...
	public static function output( $post ) {
		...
		<div class="panel-wrap woocommerce">
			...
			<div id="order_data" class="panel">
				...
				printf( esc_html_x( 'Subscription #%s details', 'edit subscription header', 'ultimatewoo-pro' ), esc_html( $subscription->get_order_number() ) ); ?></h2>

				<div class="order_data_column_container">
					<div class="order_data_column">

						<p class="form-field form-field-wide wc-customer-user">
							<label for="customer_user"><?php esc_html_e( 'Customer:', 'ultimatewoo-pro' ) ?> <?php
							if ( $subscription->get_user_id() ) {
								...
								printf( '<a href="%s">%s &rarr;</a>',
									...
									esc_html__( 'View other subscriptions', 'ultimatewoo-pro' )
								);
							}
							?></label>
							<?php
							...
							WCS_Select2::render( array(
								...
								'name'        => 'customer_user',
								'id'          => 'customer_user',
								'placeholder' => esc_attr__( 'Search for a customer&hellip;', 'ultimatewoo-pro' ),
						...
						<p class="form-field form-field-wide">
							<label for="order_status"><?php esc_html_e( 'Subscription status:', 'ultimatewoo-pro' ); ?></label>
					...
					<div class="order_data_column">
						<h3>
							<?php esc_html_e( 'Billing Details', 'ultimatewoo-pro' ); ?>
							<a href="#" class="edit_address"><?php esc_html_e( 'Edit', 'ultimatewoo-pro' ); ?></a>
							<a href="#" class="tips load_customer_billing" data-tip="<?php esc_attr_e( 'Load billing address', 'ultimatewoo-pro' ); ?>" style="display:none;"><?php esc_html_e( 'Load billing address', 'ultimatewoo-pro' ); ?></a>
						</h3>
						<?php
						// Display values
						...
						if ( $subscription->get_formatted_billing_address() ) {
							echo '<p><strong>' . esc_html__( 'Address', 'ultimatewoo-pro' ) . ':</strong>' . wp_kses( $subscription->get_formatted_billing_address(), array( 'br' => array() ) ) . '</p>';
						} else {
							echo '<p class="none_set"><strong>' . esc_html__( 'Address', 'ultimatewoo-pro' ) . ':</strong> ' . esc_html__( 'No billing address set.', 'ultimatewoo-pro' ) . '</p>';
						}
						...
						echo '<p' . ( ( '' != $subscription->get_payment_method() ) ? ' class="' . esc_attr( $subscription->get_payment_method() ) . '"' : '' ) . '><strong>' . esc_html__( 'Payment Method', 'ultimatewoo-pro' ) . ':</strong>' . wp_kses_post( nl2br( $subscription->get_payment_method_to_display() ) );

						// Display help tip
						if ( '' != $subscription->get_payment_method()  && ! $subscription->is_manual() ) {
							echo wcs_help_tip( sprintf( _x( 'Gateway ID: [%s]', 'The gateway ID displayed on the Edit Subscriptions screen when editing payment method.', 'ultimatewoo-pro' ), $subscription->get_payment_method() ) );
						}
					...
					<div class="order_data_column">
						...
						<h3>
							<?php esc_html_e( 'Shipping Details', 'ultimatewoo-pro' ); ?>
							<a href="#" class="edit_address"><?php esc_html_e( 'Edit', 'ultimatewoo-pro' ); ?></a>
							<a href="#" class="tips billing-same-as-shipping" data-tip="<?php esc_attr_e( 'Copy from billing', 'ultimatewoo-pro' ); ?>" style="display:none;"><?php esc_html_e( 'Copy from billing', 'ultimatewoo-pro' ); ?></a>
							<a href="#" class="tips load_customer_shipping" data-tip="<?php esc_attr_e( 'Load shipping address', 'ultimatewoo-pro' ); ?>" style="display:none;"><?php esc_html_e( 'Load shipping address', 'ultimatewoo-pro' ); ?></a>
						</h3>
						<?php
						// Display values
						echo '<div class="address">';
						...
						if ( $subscription->get_formatted_shipping_address() ) {
							echo '<p><strong>' . esc_html__( 'Address', 'ultimatewoo-pro' ) . ':</strong>' . wp_kses( $subscription->get_formatted_shipping_address(), array( 'br' => array() ) ) . '</p>';
						} else {
							echo '<p class="none_set"><strong>' . esc_html__( 'Address', 'ultimatewoo-pro' ) . ':</strong> ' . esc_html__( 'No shipping address set.', 'ultimatewoo-pro' ) . '</p>';
						}
						...
						if ( apply_filters( 'woocommerce_enable_order_notes_field', 'yes' == get_option( 'woocommerce_enable_order_comments', 'yes' ) ) && $post->post_excerpt ) {
							echo '<p><strong>' . esc_html__( 'Customer Provided Note', 'ultimatewoo-pro' ) . ':</strong> ' . wp_kses_post( nl2br( $post->post_excerpt ) ) . '</p>';
						}
						...
						if ( apply_filters( 'woocommerce_enable_order_notes_field', 'yes' == get_option( 'woocommerce_enable_order_comments', 'yes' ) ) ) {
							?>
							<p class="form-field form-field-wide"><label for="excerpt"><?php esc_html_e( 'Customer Provided Note', 'ultimatewoo-pro' ) ?>:</label>
								<textarea rows="1" cols="40" name="excerpt" tabindex="6" id="excerpt" placeholder="<?php esc_attr_e( 'Customer\'s notes about the order', 'ultimatewoo-pro' ); ?>"><?php echo wp_kses_post( $post->post_excerpt ); ?></textarea>
							</p>
							<?php
						}
	...
	public static function save( $post_id, $post = null ) {
		...
		try {
			...
			wcs_add_admin_notice( sprintf( __( 'Error updating some information: %s', 'ultimatewoo-pro' ), $e->getMessage() ), 'error' );
		}
