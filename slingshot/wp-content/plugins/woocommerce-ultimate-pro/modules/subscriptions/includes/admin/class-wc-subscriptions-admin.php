...
class WC_Subscriptions_Admin {
	...
	public static function add_subscription_products_to_select( $product_types ) {
		...
		$product_types['subscription']          = __( 'Simple subscription', 'ultimatewoo-pro' );
		$product_types['variable-subscription'] = __( 'Variable subscription', 'ultimatewoo-pro' );
	...
	public static function subscription_pricing_fields() {
		...
		$price_tooltip = __( 'Choose the subscription price, billing interval and period.', 'ultimatewoo-pro' );
		...
		$trial_tooltip = sprintf( _x( 'An optional period of time to wait before charging the first recurring payment. Any sign up fee will still be charged at the outset of the subscription. %s', 'Trial period field tooltip on Edit Product administration screen', 'ultimatewoo-pro' ), self::get_trial_period_validation_message() );
		...
		?><p class="form-field _subscription_price_fields _subscription_price_field">
			<label for="_subscription_price"><?php printf( esc_html__( 'Subscription price (%s)', 'ultimatewoo-pro' ), esc_html( get_woocommerce_currency_symbol() ) ); ?></label>
			<span class="wrap">
				<input type="text" id="_subscription_price" name="_subscription_price" class="wc_input_price wc_input_subscription_price" placeholder="<?php echo esc_attr_x( 'e.g. 5.90', 'example price', 'ultimatewoo-pro' ); ?>" step="any" min="0" value="<?php echo esc_attr( wc_format_localized_price( $chosen_price ) ); ?>" />
				<label for="_subscription_period_interval" class="wcs_hidden_label"><?php esc_html_e( 'Subscription interval', 'ultimatewoo-pro' ); ?></label>
				...
				<label for="_subscription_period" class="wcs_hidden_label"><?php esc_html_e( 'Subscription period', 'ultimatewoo-pro' ); ?></label>
		...
		woocommerce_wp_select( array(
			...
			'label'       => __( 'Expire after', 'ultimatewoo-pro' ),
			'options'     => wcs_get_subscription_ranges( $chosen_period ),
			'desc_tip'    => true,
			'description' => __( 'Automatically expire the subscription after this length of time. This length is in addition to any free trial or amount of time provided before a synchronised first renewal date.', 'ultimatewoo-pro' ),
			)
		);
		...
		woocommerce_wp_text_input( array(
			...
			'label'       => sprintf( __( 'Sign-up fee (%s)', 'ultimatewoo-pro' ), get_woocommerce_currency_symbol() ),
			'placeholder' => _x( 'e.g. 9.90', 'example price', 'ultimatewoo-pro' ),
			'description' => __( 'Optionally include an amount to be charged at the outset of the subscription. The sign-up fee will be charged immediately, even if the product has a free trial or the payment dates are synced.', 'ultimatewoo-pro' ),
			'desc_tip'    => true,
			'type'        => 'text',
			'data_type'   => 'price',
		...
		?><p class="form-field _subscription_trial_length_field">
			<label for="_subscription_trial_length"><?php esc_html_e( 'Free trial', 'ultimatewoo-pro' ); ?></label>
			<span class="wrap">
				...
				<label for="_subscription_trial_period" class="wcs_hidden_label"><?php esc_html_e( 'Subscription Trial Period', 'ultimatewoo-pro' ); ?></label>
	...
	public static function subscription_shipping_fields() {
		...
		woocommerce_wp_checkbox( array(
			...
			'label'       => __( 'One time shipping', 'ultimatewoo-pro' ),
			'description' => __( 'Shipping for subscription products is normally charged on the initial order and all renewal orders. Enable this to only charge shipping once on the initial order. Note: for this setting to be enabled the subscription must not have a free trial or a synced renewal date.', 'ultimatewoo-pro' ),
			'desc_tip'    => true,
		) );
	...
	public static function variable_subscription_bulk_edit_actions() {
		...
		if ( WC_Subscriptions_Product::is_subscription( $post->ID ) ) : ?>
			<optgroup label="<?php esc_attr_e( 'Subscription pricing', 'ultimatewoo-pro' ); ?>">
				<option value="variable_subscription_sign_up_fee"><?php esc_html_e( 'Subscription sign-up fee', 'ultimatewoo-pro' ); ?></option>
				<option value="variable_subscription_period_interval"><?php esc_html_e( 'Subscription billing interval', 'ultimatewoo-pro' ); ?></option>
				<option value="variable_subscription_period"><?php esc_html_e( 'Subscription period', 'ultimatewoo-pro' ); ?></option>
				<option value="variable_subscription_length"><?php esc_html_e( 'Expire after', 'ultimatewoo-pro' ); ?></option>
				<option value="variable_subscription_trial_length"><?php esc_html_e( 'Free trial length', 'ultimatewoo-pro' ); ?></option>
				<option value="variable_subscription_trial_period"><?php esc_html_e( 'Free trial period', 'ultimatewoo-pro' ); ?></option>
			</optgroup>
		<?php endif;
	}
	...
	public static function check_customer_is_set( $old_status, $new_status, $subscription ) {
		...
		if ( is_admin() && 'active' == $new_status && isset( $_POST['woocommerce_meta_nonce'] ) && wp_verify_nonce( $_POST['woocommerce_meta_nonce'], 'woocommerce_save_data' ) && isset( $_POST['customer_user'] ) && ! empty( $post ) && 'shop_subscription' === $post->post_type ) {
			...
			if ( 0 === $user->ID ) {
				throw new Exception( sprintf( __( 'Unable to change subscription status to "%s". Please assign a customer to the subscription to activate it.', 'ultimatewoo-pro' ), $new_status ) );
			}
		}
	}
	...
	public static function enqueue_styles_scripts() {
		...
		if ( $is_woocommerce_screen ) {
			...
			$trashing_subscription_order_warning = __( 'Trashing this order will also trash the subscriptions purchased with the order.', 'ultimatewoo-pro' );
			...
			if ( $screen->id == 'product' ) {
				...
				$script_params = array(
					...
					'bulkEditPeriodMessage'     => __( 'Enter the new period, either day, week, month or year:', 'ultimatewoo-pro' ),
					'bulkEditLengthMessage'     => __( 'Enter a new length (e.g. 5):', 'ultimatewoo-pro' ),
					'bulkEditIntervalhMessage'  => __( 'Enter a new interval as a single number (e.g. to charge every 2nd month, enter 2):', 'ultimatewoo-pro' ),
					'oneTimeShippingCheckNonce' => wp_create_nonce( 'one_time_shipping' ),
				);
			} else if ( 'edit-shop_order' == $screen->id ) {
				$script_params = array(
					'bulkTrashWarning' => __( "You are about to trash one or more orders which contain a subscription.\n\nTrashing the orders will also trash the subscriptions purchased with these orders.", 'ultimatewoo-pro' ),
				...
				$script_params = array(
					'trashWarning'      => $trashing_subscription_order_warning,
					'changeMetaWarning' => __( "WARNING: Bad things are about to happen!\n\nThe payment gateway used to purchase this subscription does not support modifying a subscription's details.\n\nChanges to the billing period, recurring discount, recurring tax or recurring total may not be reflected in the amount charged by the payment gateway.", 'ultimatewoo-pro' ),
					'removeItemWarning' => __( 'You are deleting a subscription item. You will also need to manually cancel and trash the subscription on the Manage Subscriptions screen.', 'ultimatewoo-pro' ),
	...
	public static function add_user_columns( $columns ) {
		...
		if ( current_user_can( 'manage_woocommerce' ) ) {
			...
			array_pop( $columns );
			$columns['woocommerce_active_subscriber'] = __( 'Active subscriber?', 'ultimatewoo-pro' );
			$columns += $last_column;
		}
...
<div class="wrap">
	...
	<h2><?php esc_html_e( 'Manage Subscriptions', 'ultimatewoo-pro' ); ?></h2>
	...
	<form id="subscriptions-search" action="" method="get"><?php // Don't send all the subscription meta across ?>
		<?php $subscriptions_table->search_box( __( 'Search Subscriptions', 'ultimatewoo-pro' ), 'subscription' ); ?>
	...
	public static function add_manage_subscriptions_screen_options() {
		add_screen_option( 'per_page', array(
			'label'   => __( 'Subscriptions', 'ultimatewoo-pro' ),
			'default' => 10,
	...
	public static function add_subscription_settings_tab( $settings_tabs ) {
		...
		$settings_tabs[ self::$tab_name ] = __( 'Subscriptions', 'ultimatewoo-pro' );
	...
	public static function get_settings() {
		...
		return apply_filters( 'woocommerce_subscription_settings', array(
			...
			array(
				'name'     => __( 'Button Text', 'ultimatewoo-pro' ),
				'type'     => 'title',
				'desc'     => '',
			...
			array(
				'name'     => __( 'Add to Cart Button Text', 'ultimatewoo-pro' ),
				'desc'     => __( 'A product displays a button with the text "Add to Cart". By default, a subscription changes this to "Sign Up Now". You can customise the button text for subscriptions here.', 'ultimatewoo-pro' ),
				'tip'      => '',
				'id'       => self::$option_prefix . '_add_to_cart_button_text',
				'css'      => 'min-width:150px;',
				'default'  => __( 'Sign Up Now', 'ultimatewoo-pro' ),
				'type'     => 'text',
				'desc_tip' => true,
			),

			array(
				'name'     => __( 'Place Order Button Text', 'ultimatewoo-pro' ),
				'desc'     => __( 'Use this field to customise the text displayed on the checkout button when an order contains a subscription. Normally the checkout submission button displays "Place Order". When the cart contains a subscription, this is changed to "Sign Up Now".', 'ultimatewoo-pro' ),
				'tip'      => '',
			...
			array(
				'name'     => __( 'Inactive Subscriber Role', 'ultimatewoo-pro' ),
				'desc'     => __( 'If a subscriber\'s subscription is manually cancelled or expires, she will be assigned this role.', 'ultimatewoo-pro' ),
				'tip'      => '',
			...
			array(
				'name'          => _x( 'Renewals', 'option section heading', 'ultimatewoo-pro' ),
				'type'          => 'title',
				'desc'          => '',
			...
			array(
				'name'            => __( 'Manual Renewal Payments', 'ultimatewoo-pro' ),
				'desc'            => __( 'Accept Manual Renewals', 'ultimatewoo-pro' ),
				'id'              => self::$option_prefix . '_accept_manual_renewals',
				'default'         => 'no',
				'type'            => 'checkbox',
				// translators: placeholders are opening and closing link tags
				'desc_tip'        => sprintf( __( 'With manual renewals, a customer\'s subscription is put on-hold until they login and pay to renew it. %sLearn more%s.', 'ultimatewoo-pro' ), '<a href="http://docs.woocommerce.com/document/subscriptions/store-manager-guide/#accept-manual-renewals">', '</a>' ),
				'checkboxgroup'   => 'start',
				'show_if_checked' => 'option',
			),

			array(
				'desc'            => __( 'Turn off Automatic Payments', 'ultimatewoo-pro' ),
				'id'              => self::$option_prefix . '_turn_off_automatic_payments',
				'default'         => 'no',
				'type'            => 'checkbox',
				// translators: placeholders are opening and closing link tags
				'desc_tip'        => sprintf( __( 'If you don\'t want new subscription purchases to automatically charge renewal payments, you can turn off automatic payments. Existing automatic subscriptions will continue to charge customers automatically. %sLearn more%s.', 'ultimatewoo-pro' ), '<a href="http://docs.woocommerce.com/document/subscriptions/store-manager-guide/#turn-off-automatic-payments">', '</a>' ),
				'checkboxgroup'   => 'end',
				'show_if_checked' => 'yes',
			),
			...
			array(
				'name'          => _x( 'Miscellaneous', 'options section heading', 'ultimatewoo-pro' ),
				'type'          => 'title',
				'desc'          => '',
			...
			array(
				'name'          => __( 'Customer Suspensions', 'ultimatewoo-pro' ),
				'desc'          => _x( 'suspensions per billing period.', 'there\'s a number immediately in front of this text', 'ultimatewoo-pro' ),
				...
				'desc_tip'      => __( 'Set a maximum number of times a customer can suspend their account for each billing period. For example, for a value of 3 and a subscription billed yearly, if the customer has suspended their account 3 times, they will not be presented with the option to suspend their account until the next year. Store managers will always be able to suspend an active subscription. Set this to 0 to turn off the customer suspension feature completely.', 'ultimatewoo-pro' ),
			),

			array(
				'name'          => __( 'Mixed Checkout', 'ultimatewoo-pro' ),
				'desc'          => __( 'Allow multiple subscriptions and products to be purchased simultaneously.', 'ultimatewoo-pro' ),
				'id'            => self::$option_prefix . '_multiple_purchase',
				'default'       => 'no',
				'type'          => 'checkbox',
				'desc_tip'      => __( 'Allow a subscription product to be purchased with other products and subscriptions in the same transaction.', 'ultimatewoo-pro' ),
			),

			array(
				'name'          => __( 'Drip Downloadable Content', 'ultimatewoo-pro' ),
				'desc'          => __( 'Enable dripping for downloadable content on subscription products.', 'ultimatewoo-pro' ),
				'id'            => self::$option_prefix . '_drip_downloadable_content_on_renewal',
				'default'       => 'no',
				'type'          => 'checkbox',
				'desc_tip'      => sprintf( __( 'Enabling this grants access to new downloadable files added to a product only after the next renewal is processed.%sBy default, access to new downloadable files added to a product is granted immediately to any customer that has an active subscription with that product.', 'ultimatewoo-pro' ), '<br />' ),
			),
		...
		<div id="message" class="updated woocommerce-message wc-connect woocommerce-subscriptions-activated">
			<div class="squeezer">
				<h4>
					...
					echo wp_kses( sprintf( __( '%1$sWooCommerce Subscriptions Installed%2$s &#8211; %3$sYou\'re ready to start selling subscriptions!%4$s', 'ultimatewoo-pro' ), '<strong>', '</strong>', '<em>', '</em>' ), array( 'strong' => true, 'em' => true ) );
					?>
				</h4>

				<p class="submit">
					<a href="<?php echo esc_url( self::add_subscription_url() ); ?>" class="button button-primary"><?php esc_html_e( 'Add a Subscription Product', 'ultimatewoo-pro' ); ?></a>
					<a href="<?php echo esc_url( self::settings_tab_url() ); ?>" class="docs button button-primary"><?php esc_html_e( 'Settings', 'ultimatewoo-pro' ); ?></a>
		...
		if ( is_admin() && 'shop_order' == $typenow ) {
			...
			if ( isset( $_GET['_subscription_related_orders'] ) && $_GET['_subscription_related_orders'] > 0 ) {
				...
				if ( ! wcs_is_subscription( $subscription ) ) {
					...
					wcs_add_admin_notice( sprintf( __( 'We can\'t find a subscription with ID #%d. Perhaps it was deleted?', 'ultimatewoo-pro' ), $subscription_id ), 'error' );
	...
	public static function display_renewal_filter_notice() {
		...
		if ( isset( $_GET[ $query_arg ] ) && $_GET[ $query_arg ] > 0 && true === self::$found_related_orders ) {
			...
			if ( version_compare( $wp_version, '4.2', '<' ) ) {
				...
				printf( esc_html__( 'Showing orders for %sSubscription %s%s', 'ultimatewoo-pro' ), '<a href="' . esc_url( get_edit_post_link( absint( $_GET[ $query_arg ] ) ) ) . '">', esc_html( $initial_order->get_order_number() ), '</a>' );
				echo '</p>';
			} else {
	...
	public static function get_trial_period_validation_message( $form = 'combined' ) {
		...
		if ( 'combined' == $form ) {
			...
			$error_message = sprintf( __( 'The trial period can not exceed: %1s, %2s, %3s or %4s.', 'ultimatewoo-pro' ), array_pop( $subscription_ranges['day'] ), array_pop( $subscription_ranges['week'] ), array_pop( $subscription_ranges['month'] ), array_pop( $subscription_ranges['year'] ) );
		} else {
			$error_message = array();
			foreach ( wcs_get_available_time_periods() as $period => $string ) {
				// translators: placeholder is a time period (e.g. "4 weeks")
				$error_message[ $period ] = sprintf( __( 'The trial period can not exceed %s.', 'ultimatewoo-pro' ), array_pop( $subscription_ranges[ $period ] ) );
			}
		}
	...
	public static function render_system_status_items() {
		...
		$debug_data['wcs_debug'] = array(
			'name'    => _x( 'WCS_DEBUG', 'label that indicates whether debugging is turned on for the plugin', 'ultimatewoo-pro' ),
			'note'    => ( $is_wcs_debug ) ? __( 'Yes', 'ultimatewoo-pro' ) :  __( 'No', 'ultimatewoo-pro' ),
			'success' => $is_wcs_debug ? 0 : 1,
		);

		$debug_data['wcs_staging'] = array(
			'name'    => _x( 'Subscriptions Mode', 'Live or Staging, Label on WooCommerce -> System Status page', 'ultimatewoo-pro' ),
			'note'    => '<strong>' . ( ( WC_Subscriptions::is_duplicate_site() ) ? _x( 'Staging', 'refers to staging site', 'ultimatewoo-pro' ) :  _x( 'Live', 'refers to live site', 'ultimatewoo-pro' ) ) . '</strong>',
		...
		$debug_data['wcs_theme_overrides'] = array(
			'name'      => _x( 'Subscriptions Template Theme Overrides', 'label for the system status page', 'ultimatewoo-pro' ),
			'mark'      => '',
	...
	public static function payment_gateways_rewewal_column( $header ) {
		...
		$header_new = array_slice( $header, 0, count( $header ) - 1, true ) +
			array( 'renewals' => __( 'Automatic Recurring Payments', 'ultimatewoo-pro' ) ) + // Ideally, we could add a link to the docs here, but the title is passed through esc_html()
	...
	public static function payment_gateways_rewewal_support( $gateway ) {
		...
		if ( ( is_array( $gateway->supports ) && in_array( 'subscriptions', $gateway->supports ) ) || $gateway->id == 'paypal' ) {
			$status_html = '<span class="status-enabled tips" data-tip="' . esc_attr__( 'Supports automatic renewal payments with the WooCommerce Subscriptions extension.', 'ultimatewoo-pro' ) . '">' . esc_html__( 'Yes', 'ultimatewoo-pro' ) . '</span>';
		} else {
			$status_html = '-';
		}
	...
	*/
	public static function change_order_item_editable_text( $translated_text, $text, $domain ) {
		...
		switch ( $text ) {
			...
			case 'This order is no longer editable.':
				$translated_text = __( 'Subscription items can no longer be edited.', 'ultimatewoo-pro' );
				break;

			case 'To edit this order change the status back to "Pending"':
				$translated_text = __( 'This subscription is no longer editable because the payment gateway does not allow modification of recurring amounts.', 'ultimatewoo-pro' );
				break;
		}
	...
	public static function add_recurring_payment_gateway_information( $settings ) {
		...
		if ( ! WC_Subscriptions_Payment_Gateways::one_gateway_supports( 'subscriptions' ) ) {
			...
			$available_gateways_description = sprintf( __( 'No payment gateways capable of processing automatic subscription payments are enabled. If you would like to process automatic payments, we recommend the %1$sfree Stripe extension%2$s.', 'ultimatewoo-pro' ), '<strong><a href="https://www.woocommerce.com/products/stripe/">', '</a></strong>' );
		}

		$recurring_payment_settings = array(
			array(
				'name' => __( 'Recurring Payments', 'ultimatewoo-pro' ),
			...
			array(
				// translators: placeholders are opening and closing link tags
				'desc' => sprintf( __( 'Payment gateways which don\'t support automatic recurring payments can be used to process %smanual subscription renewal payments%s.', 'ultimatewoo-pro' ), '<a href="http://docs.woocommerce.com/document/subscriptions/renewal-process/">', '</a>' ),
			...
			array(
				...
				'desc' => sprintf( __( 'Find new gateways that %1$ssupport automatic subscription payments%2$s in the official %3$sWooCommerce Marketplace%4$s.', 'ultimatewoo-pro' ), '<a href="' . esc_url( 'http://docs.woocommerce.com/document/subscriptions/payment-gateways/' ) . '">', '</a>', '<a href="' . esc_url( 'http://www.woocommerce.com/product-category/woocommerce-extensions/' ) . '">', '</a>' ),
				'id'   => WC_Subscriptions_Admin::$option_prefix . '_payment_gateways_additional',
				'type' => 'informational',
			),
		);
