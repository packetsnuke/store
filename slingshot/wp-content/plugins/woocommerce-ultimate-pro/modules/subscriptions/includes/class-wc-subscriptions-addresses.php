	...
	public static function add_edit_address_subscription_action( $actions, $subscription ) {
		...
		if ( $subscription->needs_shipping_address() && $subscription->has_status( array( 'active', 'on-hold' ) ) ) {
			$actions['change_address'] = array(
				...
				'name' => __( 'Change Address', 'ultimatewoo-pro' ),
			);
		}
	...
	public static function maybe_add_edit_address_checkbox() {
		...
		if ( wcs_user_has_subscription() ) {
			...
			if ( isset( $_GET['subscription'] ) ) {
				...
				echo '<p>' . esc_html__( 'Both the shipping address used for the subscription and your default shipping address for future purchases will be updated.', 'ultimatewoo-pro' ) . '</p>';
				...
				$label = sprintf( __( 'Update the %1$s used for %2$sall%3$s of my active subscriptions', 'ultimatewoo-pro' ), wcs_get_address_type_to_display( $address_type ), '<strong>', '</strong>' );
