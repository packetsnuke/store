...
class WC_Subscriptions_Coupon {
	...
	public static function add_discount_types( $discount_types ) {
		...
		return array_merge(
			$discount_types,
			array(
				'sign_up_fee'         => __( 'Sign Up Fee Discount', 'ultimatewoo-pro' ),
				'sign_up_fee_percent' => __( 'Sign Up Fee % Discount', 'ultimatewoo-pro' ),
				'recurring_fee'       => __( 'Recurring Product Discount', 'ultimatewoo-pro' ),
				'recurring_percent'   => __( 'Recurring Product % Discount', 'ultimatewoo-pro' ),
			)
		);
	}
	...
	public static function validate_subscription_coupon_for_cart( $valid, $coupon ) {
		...
		if ( ! in_array( $coupon_type, array( 'recurring_fee', 'sign_up_fee', 'recurring_percent', 'sign_up_fee_percent', 'renewal_fee', 'renewal_percent', 'renewal_cart' ) ) ) {
			...
			if ( ( wcs_cart_contains_renewal() || WC_Subscriptions_Cart::cart_contains_subscription() ) && 0 == WC()->cart->subtotal ) {
				self::$coupon_error = __( 'Sorry, this coupon is only valid for an initial payment and the cart does not require an initial payment.', 'ultimatewoo-pro' );
			}
		} else {
			...
			if ( wcs_cart_contains_renewal() && ! in_array( $coupon_type, array( 'renewal_fee', 'renewal_percent', 'renewal_cart' ) ) ) {
				self::$coupon_error = __( 'Sorry, this coupon is only valid for new subscriptions.', 'ultimatewoo-pro' );
			}
			...
			if ( ! wcs_cart_contains_renewal() && ! WC_Subscriptions_Cart::cart_contains_subscription() ) {
				self::$coupon_error = __( 'Sorry, this coupon is only valid for subscription products.', 'ultimatewoo-pro' );
			}
			...
			if ( ! wcs_cart_contains_renewal() && in_array( $coupon_type, array( 'renewal_fee', 'renewal_percent', 'renewal_cart' ) ) ) {
				...
				self::$coupon_error = sprintf( __( 'Sorry, the "%1$s" coupon is only valid for renewals.', 'ultimatewoo-pro' ), wcs_get_coupon_property( $coupon, 'code' ) );
			}
			...
			if ( 0 == WC_Subscriptions_Cart::get_cart_subscription_sign_up_fee() && in_array( $coupon_type, array( 'sign_up_fee', 'sign_up_fee_percent' ) ) ) {
				self::$coupon_error = __( 'Sorry, this coupon is only valid for subscription products with a sign-up fee.', 'ultimatewoo-pro' );
			}
		}
	...
	public static function validate_subscription_coupon_for_order( $valid, $coupon, $order ) {
		...
		if ( in_array( $coupon_type, array( 'recurring_fee', 'recurring_percent' ) ) && ! ( wcs_is_subscription( $order ) || wcs_order_contains_subscription( $order, 'any' ) ) ) {
			$error_message = __( 'Sorry, recurring coupons can only be applied to subscriptions or subscription orders.', 'ultimatewoo-pro' );
		// Sign-up fee coupons can be applied to parent orders which contain subscription products with at least one sign up fee
			...
			$error_message = sprintf( __( 'Sorry, "%s" can only be applied to subscription parent orders which contain a product with signup fees.', 'ultimatewoo-pro' ), wcs_get_coupon_property( $coupon, 'code' ) );
		// Only recurring coupons can be applied to subscriptions
			...
			$error_message = __( 'Sorry, only recurring coupons can only be applied to subscriptions.', 'ultimatewoo-pro' );
		}
	...
	public static function add_pseudo_coupon_types( $coupon_types ) {
		return array_merge(
			$coupon_types,
			array(
				'renewal_percent' => __( 'Renewal % discount', 'ultimatewoo-pro' ),
				'renewal_fee'     => __( 'Renewal product discount', 'ultimatewoo-pro' ),
				'renewal_cart'    => __( 'Renewal cart discount', 'ultimatewoo-pro' ),
			)
		);
	}
	...
	public static function get_pseudo_coupon_label( $label, $coupon ) {
		...
		if ( 'renewal_cart' === wcs_get_coupon_property( $coupon, 'discount_type' ) ) {
			$label = esc_html( __( 'Renewal Discount', 'ultimatewoo-pro' ) );
		}
