...
class WCS_Retry_Manager {
	...
	public static function add_retry_date_type( $subscription_date_types ) {
		...
		$subscription_date_types = wcs_array_insert_after( 'next_payment', $subscription_date_types, 'payment_retry', _x( 'Renewal Payment Retry', 'table heading', 'ultimatewoo-pro' ) );
		...
		if ( self::rules()->has_rule( $retry_count, wcs_get_objects_property( $last_order, 'id' ) ) ) {
			...
			foreach ( array( 'order' => $last_order, 'subscription' => $subscription ) as $object_key => $object ) {
				...
				if ( '' !== $new_status && ! $object->has_status( $new_status ) ) {
					$object->update_status( $new_status, _x( 'Retry rule applied:', 'used in order note as reason for why status changed', 'ultimatewoo-pro' ) );
				}
			}
		...
		if ( null !== $last_retry && 'pending' === $last_retry->get_status() ) {
			...
			if ( $last_order->needs_payment() ) {
				...
				if ( $valid_order_status && $valid_subscription_status ) {
					...
					$last_order->update_status( 'pending', _x( 'Subscription renewal payment retry:', 'used in order note as reason for why order status changed', 'ultimatewoo-pro' ), true );
					...
					foreach ( $subscriptions as $subscription ) {
						$subscription->update_status( 'on-hold', _x( 'Subscription renewal payment retry:', 'used in order note as reason for why subscription status changed', 'ultimatewoo-pro' ) );
					}
