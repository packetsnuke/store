...
class WCS_Query extends WC_Query {
	...
	public function get_endpoint_title( $endpoint ) {
		...
		switch ( $endpoint ) {
			case 'view-subscription':
				...
				$title        = ( $subscription ) ? sprintf( _x( 'Subscription #%s', 'hash before order number', 'ultimatewoo-pro' ), $subscription->get_order_number() ) : '';
				break;
			case 'subscriptions':
	...
	public function add_menu_items( $menu_items ) {
		if ( 1 == count( wcs_get_users_subscriptions() ) ) {
			$label = __( 'My Subscription', 'ultimatewoo-pro' );
		} else {
