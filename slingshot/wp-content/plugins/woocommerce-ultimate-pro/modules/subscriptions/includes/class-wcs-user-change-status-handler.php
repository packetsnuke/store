...
class WCS_User_Change_Status_Handler {
		...
		switch ( $new_status ) {
			case 'active' :
				if ( ! $subscription->needs_payment() ) {
					...
					$subscription->add_order_note( _x( 'Subscription reactivated by the subscriber from their account page.', 'order note left on subscription after user action', 'ultimatewoo-pro' ) );
					wc_add_notice( _x( 'Your subscription has been reactivated.', 'Notice displayed to user confirming their action.', 'ultimatewoo-pro' ), 'success' );
					$changed = true;
				} else {
					wc_add_notice( __( 'You can not reactivate that subscription until paying to renew it. Please contact us if you need assistance.', 'ultimatewoo-pro' ), 'error' );
				}
				break;
			case 'on-hold' :
				if ( wcs_can_user_put_subscription_on_hold( $subscription ) ) {
					$subscription->update_status( $new_status );
					$subscription->add_order_note( _x( 'Subscription put on hold by the subscriber from their account page.', 'order note left on subscription after user action', 'ultimatewoo-pro' ) );
					wc_add_notice( _x( 'Your subscription has been put on hold.', 'Notice displayed to user confirming their action.', 'ultimatewoo-pro' ), 'success' );
					$changed = true;
				} else {
					wc_add_notice( __( 'You can not suspend that subscription - the suspension limit has been reached. Please contact us if you need assistance.', 'ultimatewoo-pro' ), 'error' );
				}
				break;
			case 'cancelled' :
				$subscription->cancel_order();
				$subscription->add_order_note( _x( 'Subscription cancelled by the subscriber from their account page.', 'order note left on subscription after user action', 'ultimatewoo-pro' ) );
				wc_add_notice( _x( 'Your subscription has been cancelled.', 'Notice displayed to user confirming their action.', 'ultimatewoo-pro' ), 'success' );
				$changed = true;
				break;
		}
	...
	public static function validate_request( $user_id, $subscription, $new_status, $wpnonce = '' ) {
		...
		if ( ! wcs_is_subscription( $subscription ) ) {
			wc_add_notice( __( 'That subscription does not exist. Please contact us if you need assistance.', 'ultimatewoo-pro' ), 'error' );
			return false;

		} elseif ( ! empty( $wpnonce ) && wp_verify_nonce( $wpnonce, $subscription->get_id() . $subscription->get_status() ) === false ) {
			wc_add_notice( __( 'Security error. Please contact us if you need assistance.', 'ultimatewoo-pro' ), 'error' );
			return false;
			...
			wc_add_notice( sprintf( __( 'That subscription can not be changed to %s. Please contact us if you need assistance.', 'ultimatewoo-pro' ), wcs_get_subscription_status_name( $new_status ) ), 'error' );
			return false;
		}
