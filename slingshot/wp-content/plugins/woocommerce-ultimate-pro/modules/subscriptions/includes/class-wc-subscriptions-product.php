...
class WC_Subscriptions_Product {
	...
	public static function add_to_cart_text( $button_text, $product_type = '' ) {
		...
		if ( self::is_subscription( $product ) || in_array( $product_type, array( 'subscription', 'subscription-variation' ) ) ) {
			$button_text = get_option( WC_Subscriptions_Admin::$option_prefix . '_add_to_cart_button_text', __( 'Sign Up Now', 'ultimatewoo-pro' ) );
		}
	...
	public static function get_price_string( $product, $include = array() ) {
		...
		if ( $include['subscription_price'] && $include['subscription_period'] ) { // Allow extensions to not show price or billing period e.g. Name Your Price
				...
				switch ( $billing_period ) {
					case 'week':
						...
						if ( 1 == $billing_interval ) {
							...
							$subscription_string = sprintf( __( '%1$s every %2$s', 'ultimatewoo-pro' ), $price, $payment_day_of_week );
						} else {
							// translators: 1$: recurring amount string, 2$: period, 3$: day of the week (e.g. "$10 every 2nd week on Wednesday")
							$subscription_string = sprintf( __( '%1$s every %2$s on %3$s', 'ultimatewoo-pro' ), $price, wcs_get_subscription_period_strings( $billing_interval, $billing_period ), $payment_day_of_week );
						}
						break;
					case 'month':
						if ( 1 == $billing_interval ) {
							if ( $payment_day > 27 ) {
								...
								$subscription_string = sprintf( __( '%s on the last day of each month', 'ultimatewoo-pro' ), $price );
							} else {
								// translators: 1$: recurring amount, 2$: day of the month (e.g. "23rd") (e.g. "$5 every 23rd of each month")
								$subscription_string = sprintf( __( '%1$s on the %2$s of each month', 'ultimatewoo-pro' ), $price, WC_Subscriptions::append_numeral_suffix( $payment_day ) );
							}
						} else {
							if ( $payment_day > 27 ) {
								// translators: 1$: recurring amount, 2$: interval (e.g. "3rd") (e.g. "$10 on the last day of every 3rd month")
								$subscription_string = sprintf( __( '%1$s on the last day of every %2$s month', 'ultimatewoo-pro' ), $price, WC_Subscriptions::append_numeral_suffix( $billing_interval ) );
							} else {
								// translators: 1$: <price> on the, 2$: <date> day of every, 3$: <interval> month (e.g. "$10 on the 23rd day of every 2nd month")
								$subscription_string = sprintf( __( '%1$s on the %2$s day of every %3$s month', 'ultimatewoo-pro' ), $price, WC_Subscriptions::append_numeral_suffix( $payment_day ), WC_Subscriptions::append_numeral_suffix( $billing_interval ) );
							}
						}
						break;
					case 'year':
						if ( 1 == $billing_interval ) {
							...
							$subscription_string = sprintf( __( '%1$s on %2$s %3$s each year', 'ultimatewoo-pro' ), $price, $wp_locale->month[ $payment_day['month'] ], WC_Subscriptions::append_numeral_suffix( $payment_day['day'] ) );
						} else {
							// translators: 1$: recurring amount, 2$: month (e.g. "March"), 3$: day of the month (e.g. "23rd") (e.g. "$15 on March 15th every 3rd year")
							$subscription_string = sprintf( __( '%1$s on %2$s %3$s every %4$s year', 'ultimatewoo-pro' ), $price, $wp_locale->month[ $payment_day['month'] ], WC_Subscriptions::append_numeral_suffix( $payment_day['day'] ), WC_Subscriptions::append_numeral_suffix( $billing_interval ) );
						}
						break;
				}
			} else {
				// translators: 1$: recurring amount, 2$: subscription period (e.g. "month" or "3 months") (e.g. "$15 / month" or "$15 every 2nd month")
				$subscription_string = sprintf( _n( '%1$s / %2$s', ' %1$s every %2$s', $billing_interval, 'ultimatewoo-pro' ), $price, wcs_get_subscription_period_strings( $billing_interval, $billing_period ) );
			}
			...
			$subscription_string = sprintf( __( 'every %s', 'ultimatewoo-pro' ), wcs_get_subscription_period_strings( $billing_interval, $billing_period ) );
		}
		...
		if ( $include_length ) {
			...
			$subscription_string = sprintf( __( '%1$s for %2$s', 'ultimatewoo-pro' ), $subscription_string, $ranges[ $subscription_length ] );
		}
		...
		if ( $include['trial_length'] && 0 != $trial_length ) {
			...
			$subscription_string = sprintf( __( '%1$s with %2$s free trial', 'ultimatewoo-pro' ), $subscription_string, $trial_string );
		}
		...
		if ( $include['sign_up_fee'] && self::get_sign_up_fee( $product ) > 0 ) {
			...
			$subscription_string = sprintf( __( '%1$s and a %2$s sign-up fee', 'ultimatewoo-pro' ), $subscription_string, $sign_up_fee );
		}
	...
	public static function subscription_row_actions( $actions, $post ) {
		...
		if ( ! empty( $the_product ) && ! isset( $actions['untrash'] ) && $the_product->is_type( array( 'subscription', 'variable-subscription', 'subscription_variation' ) ) ) {
			...
			if ( 'trash' == $post->post_status && current_user_can( $post_type_object->cap->edit_post, $post->ID ) ) {
				$actions['untrash'] = "<a
				title='" . esc_attr__( 'Restore this item from the Trash', 'ultimatewoo-pro' ) . "'
				href='" . wp_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $post->ID ) ), 'untrash-post_' . $post->ID ) . "'>" . __( 'Restore', 'ultimatewoo-pro' ) . '</a>';
			}
		}
