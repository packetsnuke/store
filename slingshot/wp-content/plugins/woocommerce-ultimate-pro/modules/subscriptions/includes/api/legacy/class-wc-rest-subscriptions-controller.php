...
class WC_REST_Subscriptions_Controller extends WC_REST_Orders_Controller {
	...
	public function get_subscription_orders( $request ) {
		...
		if ( empty( $id ) || ! wcs_is_subscription( $id ) ) {
			return new WP_Error( 'woocommerce_rest_invalid_shop_subscription_id', __( 'Invalid subscription id.', 'ultimatewoo-pro' ), array( 'status' => 404 ) );
		}
	...
	protected function create_base_order( $args ) {
		...
		if ( is_wp_error( $subscription ) ) {
			throw new WC_REST_Exception( 'woocommerce_rest_cannot_create_subscription', sprintf( __( 'Cannot create subscription: %s.', 'ultimatewoo-pro' ), implode( ', ', $subscription->get_error_messages() ) ), 400 );
		}
	...
	public function update_schedule( $subscription, $data ) {
		...
		try {
			...
			throw new WC_REST_Exception( 'woocommerce_rest_cannot_update_subscription_dates', sprintf( __( 'Updating subscription dates errored with message: %s', 'ultimatewoo-pro' ), $e->getMessage() ), 400 );
		}
	}
	...
	public function update_payment_method( $subscription, $data, $updating = false ) {
		...
		try {
			if ( $updating && ! array_key_exists( $payment_method, WCS_Change_Payment_Method_Admin::get_valid_payment_methods( $subscription ) ) ) {
				throw new Exception( __( 'Gateway does not support admin changing the payment method on a Subscription.', 'ultimatewoo-pro' ) );
			}
			...
			throw new WC_REST_Exception( 'woocommerce_rest_invalid_payment_data', sprintf( __( 'Subscription payment method could not be set to %1$s with error message: %2$s', 'ultimatewoo-pro' ), $payment_method, $e->getMessage() ), 400 );
		}
	}
	...
	public function get_item_schema() {
		...
		$subscriptions_schema = array(
			'billing_interval' => array(
				'description' => __( 'The number of billing periods between subscription renewals.', 'ultimatewoo-pro' ),
				'type'        => 'integer',
				'context'     => array( 'view', 'edit' ),
			),
			'billing_period' => array(
				'description' => __( 'Billing period for the subscription.', 'ultimatewoo-pro' ),
				'type'        => 'string',
				'enum'        => array_keys( wcs_get_subscription_period_strings() ),
				'context'     => array( 'view', 'edit' ),
			),
			'payment_details' => array(
				'description' => __( 'Subscription payment details.', 'ultimatewoo-pro' ),
				'type'        => 'object',
				'context'     => array( 'edit' ),
				'properties'  => array(
					'method_id' => array(
						'description' => __( 'Payment gateway ID.', 'ultimatewoo-pro' ),
						'type'        => 'string',
						'context'     => array( 'edit' ),
					),
				),
			...
			'start_date' => array(
				'description' => __( "The subscription's start date.", 'ultimatewoo-pro' ),
				'type'        => 'date-time',
				'context'     => array( 'view', 'edit' ),
			),
			'trial_date' => array(
				'description' => __( "The subscription's trial date", 'ultimatewoo-pro' ),
				'type'        => 'date-time',
				'context'     => array( 'view', 'edit' ),
			),
			'next_payment_date' => array(
				'description' => __( "The subscription's next payment date.", 'ultimatewoo-pro' ),
				'type'        => 'date-time',
				'context'     => array( 'view', 'edit' ),
			),
			'end_date' => array(
				'description' => __( "The subscription's end date.", 'ultimatewoo-pro' ),
				'type'        => 'date-time',
				'context'     => array( 'view', 'edit' ),
			),
		);
