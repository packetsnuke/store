...
class WC_API_Subscriptions extends WC_API_Orders {
			...
			foreach ( $statuses as &$status ) {
				if ( 'wc-' != substr( $status, 0, 3 ) ) {
					...
					if ( ! array_key_exists( $status, wcs_get_subscription_statuses() ) ) {
						return new WP_Error( 'wcs_api_invalid_subscription_status', __( 'Invalid subscription status given.', 'ultimatewoo-pro' ) );
					}
				}
			}
		}
	...
	public function get_subscriptions( $fields = null, $filter = array(), $status = null, $page = 1 ) {
		...
		if ( ! current_user_can( 'read_private_shop_orders' ) ) {
			return new WP_Error( 'wcs_api_user_cannot_read_susbcription_count', __( 'You do not have permission to read the subscriptions count', 'ultimatewoo-pro' ), array( 'status' => 401 ) );
		}
	...
	public function create_subscription( $data ) {
		...
		try {
			...
			if ( ! current_user_can( 'publish_shop_orders' ) ) {
				throw new WC_API_Exception( 'wcs_api_user_cannot_create_subscription', __( 'You do not have permission to create subscriptions', 'ultimatewoo-pro' ), 401 );
			}
	...
	public function edit_subscription( $subscription_id, $data, $fields = null ) {
		...
		try {
			...
			if ( is_wp_error( $subscription_id ) ) {
				throw new WC_API_Exception( 'wcs_api_cannot_edit_subscription', __( 'The requested subscription cannot be edited.', 'ultimatewoo-pro' ), 400 );
			}
			...
			if ( is_wp_error( $edited ) ) {
				...
				throw new WC_API_Exception( 'wcs_api_cannot_edit_subscription', sprintf( _x( 'Edit subscription failed with error: %s', 'API error message when editing the order failed', 'ultimatewoo-pro' ), $edited->get_error_message() ), $data['status'] );
			}
	...
	public function update_payment_method( $subscription, $payment_details, $updating ) {
		...
		try {
			...
			if ( $updating && ! array_key_exists( $payment_method, WCS_Change_Payment_Method_Admin::get_valid_payment_methods( $subscription ) ) ) {
				throw new Exception( 'wcs_api_edit_subscription_error', __( 'Gateway does not support admin changing the payment method on a Subscription.', 'ultimatewoo-pro' ) );
			}
			...
			throw new Exception( sprintf( __( 'Subscription payment method could not be set to %1$s and has been set to manual with error message: %2$s', 'ultimatewoo-pro' ), ( ! empty( $payment_gateway->id ) ) ? $payment_gateway->id : 'manual', $e->getMessage() ) );
		}
	}
	...
	protected function update_schedule( $subscription, $data ) {
		...
		if ( isset( $data['billing_interval'] ) ) {
			...
			if ( 0 == $interval ) {
				throw new WC_API_Exception( 'wcs_api_invalid_subscription_meta', __( 'Invalid subscription billing interval given. Must be an integer greater than 0.', 'ultimatewoo-pro' ), 400 );
			}
		...
		if ( ! empty( $data['billing_period'] ) ) {
			...
			if ( ! in_array( $period, array_keys( wcs_get_subscription_period_strings() ) ) ) {
				throw new WC_API_Exception( 'wcs_api_invalid_subscription_meta', __( 'Invalid subscription billing period given.', 'ultimatewoo-pro' ), 400 );
			}
	...
	public function delete_subscription_note( $subscription_id, $id ) {
		...
		return array( 'message' => _x( 'Permanently deleted subscription note', 'API response confirming order note deleted from a subscription', 'ultimatewoo-pro' ) );
