...
class WC_Subscriptions_Switcher {
		...
		if ( isset( $_GET['switch-subscription'] ) && isset( $_GET['item'] ) ) {
			...
			if ( ! is_object( $subscription ) || empty( $_GET['_wcsnonce'] ) || ! wp_verify_nonce( $_GET['_wcsnonce'], 'wcs_switch_request' ) || empty( $line_item ) || ! self::can_item_be_switched_by_user( $line_item, $subscription )  ) {
				...
				if ( isset( $_GET['auto-switch'] ) ) {
					$switch_message = __( 'You have a subscription to this product. Choosing a new subscription will replace your existing subscription.', 'ultimatewoo-pro' );
				} else {
					$switch_message = __( 'Choose a new subscription.', 'ultimatewoo-pro' );
				}
			...
			if ( $removed_item_count > 0 ) {
				wc_add_notice( _n( 'Your cart contained an invalid subscription switch request. It has been removed.', 'Your cart contained invalid subscription switch requests. They have been removed.', 	$removed_item_count, 'ultimatewoo-pro' ), 'error' );
				...
				foreach ( $subscriptions as $subscription ) {
					foreach ( $limited_switchable_products as $product ) {
						...
						if ( 'any' == $limitation || $subscription->has_status( $limitation ) ) {
							...
							$subscribed_notice = __( 'You have already subscribed to this product and it is limited to one per customer. You can not purchase the product again.', 'ultimatewoo-pro' );
							...
							if ( $subscription->needs_payment() ) {
								...
								if ( $last_order->needs_payment() ) {
									...
									$subscribed_notice = sprintf( __( '%1$s Complete payment on %2$sOrder %3$s%4$s to be able to change your subscription.', 'ultimatewoo-pro' ), $subscribed_notice, sprintf( '<a href="%s">', $last_order->get_checkout_payment_url() ), $last_order->get_order_number(), '</a>' );
								}
	...
	public static function add_settings( $settings ) {
		...
		array_splice( $settings, 12, 0, array(
			...
			array(
				'name'     => __( 'Switching', 'ultimatewoo-pro' ),
				'type'     => 'title',
				// translators: placeholders are opening and closing link tags
				'desc'     => sprintf( __( 'Allow subscribers to switch (upgrade or downgrade) between different subscriptions. %sLearn more%s.', 'ultimatewoo-pro' ), '<a href="' . esc_url( 'http://docs.woocommerce.com/document/subscriptions/switching-guide/' ) . '">', '</a>' ),
				'id'       => WC_Subscriptions_Admin::$option_prefix . '_switch_settings',
			),

			array(
				'name'    => __( 'Allow Switching', 'ultimatewoo-pro' ),
				'desc'    => __( 'Allow subscribers to switch between subscriptions combined in a grouped product, different variations of a Variable subscription or don\'t allow switching.', 'ultimatewoo-pro' ),
				'tip'     => '',
				...
				'type'    => 'select',
				'options' => array(
					'no'               => _x( 'Never', 'when to allow a setting', 'ultimatewoo-pro' ),
					'variable'         => _x( 'Between Subscription Variations', 'when to allow switching', 'ultimatewoo-pro' ),
					'grouped'          => _x( 'Between Grouped Subscriptions', 'when to allow switching', 'ultimatewoo-pro' ),
					'variable_grouped' => _x( 'Between Both Variations & Grouped Subscriptions', 'when to allow switching', 'ultimatewoo-pro' ),
				),
				'desc_tip' => true,
			),

			array(
				'name'    => __( 'Prorate Recurring Payment', 'ultimatewoo-pro' ),
				'desc'    => __( 'When switching to a subscription with a different recurring payment or billing period, should the price paid for the existing billing period be prorated when switching to the new subscription?', 'ultimatewoo-pro' ),
				'tip'     => '',
				...
				'options' => array(
					'no'              => _x( 'Never', 'when to allow a setting', 'ultimatewoo-pro' ),
					'virtual-upgrade' => _x( 'For Upgrades of Virtual Subscription Products Only', 'when to prorate recurring fee when switching', 'ultimatewoo-pro' ),
					'yes-upgrade'     => _x( 'For Upgrades of All Subscription Products', 'when to prorate recurring fee when switching', 'ultimatewoo-pro' ),
					'virtual'         => _x( 'For Upgrades & Downgrades of Virtual Subscription Products Only', 'when to prorate recurring fee when switching', 'ultimatewoo-pro' ),
					'yes'             => _x( 'For Upgrades & Downgrades of All Subscription Products', 'when to prorate recurring fee when switching', 'ultimatewoo-pro' ),
				),
				'desc_tip' => true,
			),

			array(
				'name'    => __( 'Prorate Sign up Fee', 'ultimatewoo-pro' ),
				'desc'    => __( 'When switching to a subscription with a sign up fee, you can require the customer pay only the gap between the existing subscription\'s sign up fee and the new subscription\'s sign up fee (if any).', 'ultimatewoo-pro' ),
				'tip'     => '',
				...
				'type'    => 'select',
				'options' => array(
					'no'                 => _x( 'Never (do not charge a sign up fee)', 'when to prorate signup fee when switching', 'ultimatewoo-pro' ),
					'full'               => _x( 'Never (charge the full sign up fee)', 'when to prorate signup fee when switching', 'ultimatewoo-pro' ),
			...
			array(
				'name'    => __( 'Prorate Subscription Length', 'ultimatewoo-pro' ),
				'desc'    => __( 'When switching to a subscription with a length, you can take into account the payments already completed by the customer when determining how many payments the subscriber needs to make for the new subscription.', 'ultimatewoo-pro' ),
				'tip'     => '',
				...
				'options' => array(
					'no'                 => _x( 'Never', 'when to allow a setting', 'ultimatewoo-pro' ),
					'virtual'            => _x( 'For Virtual Subscription Products Only', 'when to prorate first payment / subscription length', 'ultimatewoo-pro' ),
					'yes'                => _x( 'For All Subscription Products', 'when to prorate first payment / subscription length', 'ultimatewoo-pro' ),
				),
				'desc_tip' => true,
			),

			array(
				'name'     => __( 'Switch Button Text', 'ultimatewoo-pro' ),
				'desc'     => __( 'Customise the text displayed on the button next to the subscription on the subscriber\'s account page. The default is "Switch Subscription", but you may wish to change this to "Upgrade" or "Change Subscription".', 'ultimatewoo-pro' ),
				'tip'      => '',
				'id'       => WC_Subscriptions_Admin::$option_prefix . '_switch_button_text',
				'css'      => 'min-width:150px;',
				'default'  => __( 'Upgrade or Downgrade', 'ultimatewoo-pro' ),
				'type'     => 'text',
				'desc_tip' => true,
			),
	...
	public static function print_switch_link( $item_id, $item, $subscription ) {
		...
		$switch_text = get_option( WC_Subscriptions_Admin::$option_prefix . '_switch_button_text', __( 'Upgrade or Downgrade', 'ultimatewoo-pro' ) );
			...
			foreach ( $switch_order_data as $subscription_id => $switch_data ) {
				...
				foreach ( $switch_orders as $switch_order_id => $switch_order ) {
					if ( wcs_get_objects_property( $order, 'id' ) !== $switch_order_id && in_array( $switch_order->get_status(), apply_filters( 'woocommerce_valid_order_statuses_for_payment', array( 'pending', 'failed', 'on-hold' ), $switch_order ) ) ) {
						$switch_order->update_status( 'cancelled', sprintf( __( 'Switch order cancelled due to a new switch order being created #%s.', 'ultimatewoo-pro' ), $order->get_order_number() ) );
					}
				}
			}
	...
	public static function switch_order_meta_box_rows( $post ) {
		...
		if ( wcs_is_subscription( $post->ID ) ) {
			...
			foreach ( $orders as $order_id => $order ) {
				wcs_set_objects_property( $order, 'relationship', __( 'Switch Order', 'ultimatewoo-pro' ), 'set_prop_only' );
			}
		...
		if ( is_array( $switched_subscriptions ) ) {
			foreach ( $switched_subscriptions as $subscription_id => $subscription ) {
				wcs_set_objects_property( $subscription, 'relationship', __( 'Switched Subscription', 'ultimatewoo-pro' ), 'set_prop_only' );
			...
			foreach ( WC()->cart->cart_contents as $cart_item_key => $cart_item ) {
				if ( isset( $cart_item['subscription_switch'] ) ) {
					if ( wcs_is_subscription( $cart_item['subscription_switch']['subscription_id'] ) ) {
						...
						WC()->cart->remove_cart_item( $cart_item_key );
						wc_add_notice( __( 'Your cart contained an invalid subscription switch request. It has been removed.', 'ultimatewoo-pro' ), 'error' );
					}
				}
			}
		}
	...
	public static function validate_switch_request( $is_valid, $product_id, $quantity, $variation_id = '' ) {
		...
		try {
			...
			if ( ! WC_Subscriptions_Product::is_subscription( $product_id ) ) {
				throw new Exception( __( 'You can only switch to a subscription product.', 'ultimatewoo-pro' ) );
			}
			...
			if ( empty( $item ) ) {
				...
				throw new Exception( __( 'We can not find your old subscription item.', 'ultimatewoo-pro' ) );
				...
				if ( $is_identical_product ) {
					throw new Exception( __( 'You can not switch to the same subscription.', 'ultimatewoo-pro' ) );
				}
	...
	public static function set_switch_details_in_cart( $cart_item_data, $product_id, $variation_id ) {
		...
		try {
			...
			if ( ! current_user_can( 'switch_shop_subscription', $subscription->get_id() ) ) {
				wc_add_notice( __( 'You can not switch this subscription. It appears you do not own the subscription.', 'ultimatewoo-pro' ), 'error' );
			...
			wc_add_notice( __( 'There was an error locating the switch details.', 'ultimatewoo-pro' ), 'error' );
			WC()->cart->empty_cart( true );
	...
	public static function add_cart_item_switch_direction( $product_subtotal, $cart_item, $cart_item_key ) {
		...
		if ( ! empty( $cart_item['subscription_switch'] ) ) {
			...
			switch ( $cart_item['subscription_switch']['upgraded_or_downgraded'] ) {
				case 'downgraded' :
					$direction = _x( 'Downgrade', 'a switch order', 'ultimatewoo-pro' );
					break;
				case 'upgraded' :
					$direction = _x( 'Upgrade', 'a switch order', 'ultimatewoo-pro' );
					break;
				default :
					$direction = _x( 'Crossgrade', 'a switch order', 'ultimatewoo-pro' );
				break;
			}

			// translators: %1: product subtotal, %2: HTML span tag, %3: direction (upgrade, downgrade, crossgrade), %4: closing HTML span tag
			$product_subtotal = sprintf( _x( '%1$s %2$s(%3$s)%4$s', 'product subtotal string', 'ultimatewoo-pro' ), $product_subtotal, '<span class="subscription-switch-direction">', $direction, '</span>' );
	...
	*/
				...
				if ( ! array_key_exists( 'remove_line_item', reset( $switch_data['switches'] ) ) ) {
					...
					foreach ( $switch_data['switches'] as $order_item_id => $switched_item_data ) {
						...
						if ( empty( $old_subscription_item ) ) {
							throw new Exception( __( 'The original subscription item being switched cannot be found.', 'ultimatewoo-pro' ) );
						} elseif ( empty( $switch_order_item ) ) {
							throw new Exception( __( 'The item on the switch order cannot be found.', 'ultimatewoo-pro' ) );
						} else {
							...
							$add_note = sprintf( _x( 'Customer switched from: %1$s to %2$s.', 'used in order notes', 'ultimatewoo-pro' ), $old_item_name, $new_item_name );
						}
					}
				}
			}
	...
	protected static function switch_line_items_pre_2_1_2( $switches, $order, $subscription ) {
		...
		foreach ( $switches as $order_item_id => $switch_item_data ) {
			...
			if ( empty( $old_order_item ) ) {
				...
				// translators: 1$: old item, 2$: new item when switching
				$subscription->add_order_note( sprintf( _x( 'Customer switched from: %1$s to %2$s.', 'used in order notes', 'ultimatewoo-pro' ), $old_subscription_item_name, $new_order_item_name ) );
			}
		}
	}
	...
	protected static function switch_shipping_line_items_pre_2_1_2( $subscription, $shipping_methods ) {
		...
		foreach ( $shipping_methods as $shipping_line_item ) {
			...
			if ( ! $item_id || empty( $shipping_line_item['method_id'] ) || empty( $shipping_line_item['cost'] ) || empty( $shipping_line_item['taxes'] ) ) {
				throw new Exception( __( 'Failed to update the subscription shipping method.', 'ultimatewoo-pro' ) );
			}
			...
			if ( WC_Subscriptions_Manager::can_subscription_be_changed_to( 'new-subscription', $subscription_key, $user_id ) ) {
				$all_actions[ $subscription_key ] = array(
					'switch' => array(
						...
						'name' => get_option( WC_Subscriptions_Admin::$option_prefix . '_switch_button_text', __( 'Upgrade or Downgrade', 'ultimatewoo-pro' ) ),
					),
	...
	public static function add_switched_status_string( $status_string ) {
		...
		if ( 'switched' === strtolower( $status_string ) ) {
			$status_string = _x( 'Switched', 'Subscription status', 'ultimatewoo-pro' );
		}
