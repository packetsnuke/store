...
class WCS_PayPal_Admin {
	...
	public static function add_form_fields() {
		...
		foreach ( WC()->payment_gateways->payment_gateways as $key => $gateway ) {
			...
			WC()->payment_gateways->payment_gateways[ $key ]->form_fields['receiver_email']['description'] .= ' </p><p class="description">' . __( 'It is <strong>strongly recommended you do not change the Receiver Email address</strong> if you have active subscriptions with PayPal. Doing so can break existing subscriptions.', 'ultimatewoo-pro' );
		}
	}
		...
		if ( $valid_for_use && 'yes' == WCS_PayPal::get_option( 'enabled' ) && ! has_action( 'admin_notices', 'WC_Subscriptions_Admin::admin_installed_notice' ) && current_user_can( 'manage_options' ) ) {
			...
			if ( ! WCS_PayPal::are_credentials_set() ) {
				...
				$notices[] = array(
					...
					'text'  => sprintf( esc_html__( 'PayPal is inactive for subscription transactions. Please %1$sset up the PayPal IPN%2$s and %3$senter your API credentials%4$s to enable PayPal for Subscriptions.', 'ultimatewoo-pro' ),
			...
			if ( isset( $_GET['wcs_paypal'] ) && 'rt_enabled' === $_GET['wcs_paypal'] ) {
				$notices[] = array(
					...
					'text'  => sprintf( esc_html__( '%1$sPayPal Reference Transactions are enabled on your account%2$s. All subscription management features are now enabled. Happy selling!', 'ultimatewoo-pro' ),
						'<strong>',
						'</strong>'
					),
				);
			...
			if ( false !== get_option( 'wcs_paypal_credentials_error' ) ) {
				$notices[] = array(
					...
					'text'  => sprintf( esc_html__( 'There is a problem with PayPal. Your API credentials may be incorrect. Please update your %1$sAPI credentials%2$s. %3$sLearn more%4$s.', 'ultimatewoo-pro' ),
						'<a href="' . esc_url( $payment_gateway_tab_url ) . '">',
						'</a>',
			...
			if ( 'yes' == get_option( 'wcs_paypal_invalid_profile_id' ) ) {
				$notices[] = array(
					...
					'text'  => sprintf( esc_html__( 'There is a problem with PayPal. Your PayPal account is issuing out-of-date subscription IDs. %1$sLearn more%2$s. %3$sDismiss%4$s.', 'ultimatewoo-pro' ),
			...
			if ( ! empty( $last_ipn_error ) && ( false == get_option( 'wcs_fatal_error_handling_ipn_ignored', false ) || isset( $_GET['wcs_reveal_your_ipn_secrets'] ) ) ) {
				$notices[] = array(
					...
					'text' => sprintf( esc_html__( '%sA fatal error has occurred when processing a recent subscription payment with PayPal. Please %sopen a new ticket at WooCommerce Support%s immediately to get this resolved.%sIn order to get the quickest possible response please attach a %sTemporary Admin Login%s and a copy of your PHP error logs to your support ticket.%sLast recorded error: %sTo see the full error, view the %s log file from the %sWooCommerce logs screen.%s', 'ultimatewoo-pro' ),
						...
						'</a></p><div style="margin: 5px 0;"><a class="button" href="' . esc_url( wp_nonce_url( add_query_arg( 'wcs_ipn_error_notice', 'ignore' ), 'wcs_ipn_error_notice', '_wcsnonce' ) ) . '">' . esc_html__( 'Ignore this error (not recommended!)', 'ultimatewoo-pro' ) . '</a> <a class="button button-primary" href="https://woocommerce.com/my-account/marketplace-ticket-form/">' . esc_html__( 'Open up a ticket now!', 'ultimatewoo-pro' ) . '</a></div>'
					),
				);
			}
		}
	...
	public static function profile_link( $subscription ) {
		if ( wcs_is_subscription( $subscription ) && 'paypal' == $subscription->get_payment_method() ) {
			...
			if ( ! empty( $paypal_profile_id ) ) {
				...
				echo esc_html( __( 'PayPal Subscription ID:', 'ultimatewoo-pro' ) );
				echo '</strong>';
				if ( ! empty( $url ) ) {
