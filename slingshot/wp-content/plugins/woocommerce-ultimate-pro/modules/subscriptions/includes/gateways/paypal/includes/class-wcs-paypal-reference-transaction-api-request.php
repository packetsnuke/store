...
class WCS_PayPal_Reference_Transaction_API_Request {
	...
	public function set_express_checkout( $args ) {
		...
		$default_description = sprintf( _x( 'Orders with %s', 'data sent to paypal', 'ultimatewoo-pro' ), get_bloginfo( 'name' ) );
	...
	public function do_reference_transaction( $reference_id, $order, $args = array() ) {
		...
		$defaults = array(
			...
			'invoice_number'       => WCS_PayPal::get_option( 'invoice_prefix' ) . wcs_str_to_ascii( ltrim( $order->get_order_number(), _x( '#', 'hash before the order number. Used as a character to remove from the actual order number', 'ultimatewoo-pro' ) ) ),
		...
		if ( $this->skip_line_items( $order, $order_items ) ) {
			...
			if ( $use_deprecated_params ) {
				$this->add_parameters( array(
					...
					'INVNUM'           => WCS_PayPal::get_option( 'invoice_prefix' ) . wcs_str_to_ascii( ltrim( $order->get_order_number(), _x( '#', 'hash before the order number. Used as a character to remove from the actual order number', 'ultimatewoo-pro' ) ) ),
					'PAYMENTACTION'    => $type,
	...
	public function manage_recurring_payments_profile_status( $profile_id, $new_status, $order = null ) {
		...
		$this->add_parameters( apply_filters( 'woocommerce_subscriptions_paypal_change_status_data', array_merge( $this->get_parameters(), array(
			...
			'NOTE'      => html_entity_decode( sprintf( _x( '%1$s subscription event triggered at %2$s', 'data sent to paypal', 'ultimatewoo-pro' ), $new_status, get_bloginfo( 'name' ) ), ENT_NOQUOTES, 'UTF-8' ),
