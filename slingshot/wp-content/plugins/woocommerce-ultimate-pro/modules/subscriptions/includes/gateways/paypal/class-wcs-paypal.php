...
class WCS_PayPal {
			...
			case 'create_billing_agreement' :
				...
				try {
					...
					if ( 1 == $express_checkout_details_response->get_billing_agreement_status() ) {
						...
						if ( is_null( $order ) ) {
							throw new Exception( __( 'Unable to find order for PayPal billing agreement.', 'ultimatewoo-pro' ) );
						}
	...
	public static function process_subscription_payment( $amount, $order ) {
		...
		if ( wcs_is_paypal_profile_a( $paypal_profile_id, 'billing_agreement' ) ) {
			...
			$response = self::get_api()->do_reference_transaction( $paypal_profile_id, $order, array(
				'amount'         => $amount,
				'invoice_number' => self::get_option( 'invoice_prefix' ) . wcs_str_to_ascii( ltrim( $order->get_order_number(), _x( '#', 'hash before the order number. Used as a character to remove from the actual order number', 'ultimatewoo-pro' ) ) ),
			) );
	...
	public static function process_subscription_payment_response( $order, $response ) {
		...
		if ( $response->has_api_error() ) {
			...
			$order->update_status( 'failed', sprintf( __( 'PayPal API error: (%d) %s', 'ultimatewoo-pro' ), $response->get_api_error_code(), $error_message ) );

		} elseif ( $response->transaction_held() ) {

			// translators: placeholder is PayPal transaction status message
			$order_note   = sprintf( __( 'PayPal Transaction Held: %s', 'ultimatewoo-pro' ), $response->get_status_message() );
			...
			// translators: placeholder is PayPal transaction status message
			$order->update_status( 'failed', sprintf( __( 'PayPal payment declined: %s', 'ultimatewoo-pro' ), $response->get_status_message() ) );

		} elseif ( $response->transaction_approved() ) {

			$order->add_order_note( sprintf( __( 'PayPal payment approved (ID: %s)', 'ultimatewoo-pro' ), $response->get_transaction_id() ) );
	...
	public function get_plugin_name() {
		return _x( 'WooCommerce Subscriptions PayPal', 'used in User Agent data sent to PayPal to help identify where a payment came from', 'ultimatewoo-pro' );
	}
