...
class WCS_PayPal_Reference_Transaction_API_Response extends WC_Gateway_Paypal_Response {
	...
	public function get_api_error_message() {
		...
		foreach ( range( 0, 9 ) as $index ) {
			...
			if ( $this->has_parameter( "L_SHORTMESSAGE{$index}" ) ) {
				...
				$error_message = sprintf( '%s: %s - %s',
					$this->has_parameter( "L_SEVERITYCODE{$index}" ) ? $this->get_parameter( "L_SEVERITYCODE{$index}" ) : _x( 'Error', 'used in api error message if there is no severity code from PayPal', 'ultimatewoo-pro' ),
					$this->get_parameter( "L_SHORTMESSAGE{$index}" ),
					$this->has_parameter( "L_LONGMESSAGE{$index}" ) ? $this->get_parameter( "L_LONGMESSAGE{$index}" ) : _x( 'Unknown error', 'used in api error message if there is no long message', 'ultimatewoo-pro' )
				);
		...
		return empty( $error_messages ) ? _x( 'N/A', 'no information about something', 'ultimatewoo-pro' ) : trim( implode( ', ', $error_messages ) );
	}
