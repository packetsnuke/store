		...
		switch ( $transaction_details['txn_type'] ) {
			case 'subscr_signup':
				...
				if ( $is_payment_change ) {
					...
					$this->add_order_note( _x( 'IPN subscription payment method changed to PayPal.', 'when it is a payment change, and there is a subscr_signup message, this will be a confirmation message that PayPal accepted it being the new payment method', 'ultimatewoo-pro' ), $subscription, $transaction_details );

				} else {

					$this->add_order_note( __( 'IPN subscription sign up completed.', 'ultimatewoo-pro' ), $subscription, $transaction_details );
			...
			case 'subscr_payment':
				...
				if ( 'completed' == strtolower( $transaction_details['payment_status'] ) ) {
					...
					$this->add_order_note( __( 'IPN subscription payment completed.', 'ultimatewoo-pro' ), $subscription, $transaction_details );
					...
					// Process the payment if the subscription is active
						...
						if ( true === $is_renewal_sign_up_after_failure && is_object( $renewal_order ) ) {
							...
							if ( 'paypal' == get_post_meta( $subscription->get_id(), '_old_payment_method', true ) ) {
								...
								$this->add_order_note( __( 'IPN subscription failing payment method changed.', 'ultimatewoo-pro' ), $subscription, $transaction_details );
							}
						}
						...
						$this->add_order_note( __( 'IPN subscription payment completed.', 'ultimatewoo-pro' ), $renewal_order, $transaction_details );
					...
					$this->add_order_note( sprintf( _x( 'IPN subscription payment %s.', 'used in order note', 'ultimatewoo-pro' ), $transaction_details['payment_status'] ), $subscription, $transaction_details );
					...
					if ( ! $is_first_payment ) {
						...
						if ( 'failed' == strtolower( $transaction_details['payment_status'] ) ) {
							...
							// translators: placeholder is payment status (e.g. "completed")
							$this->add_order_note( sprintf( _x( 'IPN subscription payment %s.', 'used in order note', 'ultimatewoo-pro' ), $transaction_details['payment_status'] ), $renewal_order, $transaction_details );
						} else {
							$renewal_order->update_status( 'on-hold' );
							// translators: placeholder is payment status (e.g. "completed")
							$this->add_order_note( sprintf( _x( 'IPN subscription payment %s for reason: %s.', 'used in order note', 'ultimatewoo-pro' ), $transaction_details['payment_status'], $transaction_details['pending_reason'] ), $renewal_order, $transaction_details );
						}
					}
			...
			case 'recurring_payment_suspended':
				...
				if ( wcs_get_paypal_id( $subscription ) != $ipn_profile_id ) {
					...
					$subscription->update_status( 'on-hold', __( 'IPN subscription suspended.', 'ultimatewoo-pro' ) );
			...
			case 'subscr_cancel':
				...
				if ( wcs_get_paypal_id( $subscription ) != $transaction_details['subscr_id'] ) {
					...
					$subscription->cancel_order( __( 'IPN subscription cancelled.', 'ultimatewoo-pro' ) );
			...
			case 'recurring_payment_suspended_due_to_max_failed_payment': // Recurring payment failed
				...
				$ipn_failure_note = __( 'IPN subscription payment failure.', 'ultimatewoo-pro' );
	...
	public static function get_order_id_and_key( $args, $order_type = 'shop_order', $meta_key = '_paypal_subscription_id' ) {
		...
		if ( empty( $order_id ) && isset( $args['custom'] ) ) {
			...
			if ( is_object( $order_details ) ) { // WC 2.3.11+ converted the custom value to JSON, if we have an object, we've got valid JSON
				...
				WC_Gateway_Paypal::log( __( 'Invalid PayPal IPN Payload: unable to find matching subscription.', 'ultimatewoo-pro' ) );
			}
		}
