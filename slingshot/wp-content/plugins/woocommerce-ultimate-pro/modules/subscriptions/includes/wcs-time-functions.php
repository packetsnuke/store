...
function wcs_get_subscription_period_strings( $number = 1, $period = '' ) {
	...
	$translated_periods = apply_filters( 'woocommerce_subscription_periods',
		array(
			...
			'day'   => sprintf( _nx( 'day',   '%s days',   $number, 'Subscription billing period.', 'ultimatewoo-pro' ), $number ),
			// translators: placeholder is number of weeks. (e.g. "Bill this every week / 4 weeks")
			'week'  => sprintf( _nx( 'week',  '%s weeks',  $number, 'Subscription billing period.', 'ultimatewoo-pro' ), $number ),
			// translators: placeholder is number of months. (e.g. "Bill this every month / 4 months")
			'month' => sprintf( _nx( 'month', '%s months', $number, 'Subscription billing period.', 'ultimatewoo-pro' ), $number ),
			// translators: placeholder is number of years. (e.g. "Bill this every year / 4 years")
			'year'  => sprintf( _nx( 'year',  '%s years',  $number, 'Subscription billing period.', 'ultimatewoo-pro' ), $number ),
		),
		$number
	);
...
function wcs_get_subscription_trial_period_strings( $number = 1, $period = '' ) {
	...
	$translated_periods = apply_filters( 'woocommerce_subscription_trial_periods',
		array(
			'day'   => sprintf( _n( '%s day', 'a %s-day', $number, 'ultimatewoo-pro' ), $number ),
			'week'  => sprintf( _n( '%s week', 'a %s-week', $number, 'ultimatewoo-pro' ), $number ),
			'month' => sprintf( _n( '%s month', 'a %s-month', $number, 'ultimatewoo-pro' ), $number ),
			'year'  => sprintf( _n( '%s year', 'a %s-year', $number, 'ultimatewoo-pro' ), $number ),
		),
		$number
	);
...
function wcs_get_non_cached_subscription_ranges() {
	...
	foreach ( array( 'day', 'week', 'month', 'year' ) as $period ) {
		...
		$subscription_lengths = array(
			_x( 'Never expire', 'Subscription length', 'ultimatewoo-pro' ),
		);

		switch ( $period ) {
			case 'day':
				$subscription_lengths[] = _x( '1 day', 'Subscription lengths. e.g. "For 1 day..."', 'ultimatewoo-pro' );
				$subscription_range = range( 2, 90 );
				break;
			case 'week':
				$subscription_lengths[] = _x( '1 week', 'Subscription lengths. e.g. "For 1 week..."', 'ultimatewoo-pro' );
				$subscription_range = range( 2, 52 );
				break;
			case 'month':
				$subscription_lengths[] = _x( '1 month', 'Subscription lengths. e.g. "For 1 month..."', 'ultimatewoo-pro' );
				$subscription_range = range( 2, 24 );
				break;
			case 'year':
				$subscription_lengths[] = _x( '1 year', 'Subscription lengths. e.g. "For 1 year..."', 'ultimatewoo-pro' );
...
function wcs_get_subscription_period_interval_strings( $interval = '' ) {
	...
	$intervals = array( 1 => _x( 'every', 'period interval (eg "$10 _every_ 2 weeks")', 'ultimatewoo-pro' ) );

	foreach ( range( 2, 6 ) as $i ) {
		// translators: period interval, placeholder is ordinal (eg "$10 every _2nd/3rd/4th_", etc)
		$intervals[ $i ] = sprintf( _x( 'every %s', 'period interval with ordinal number (e.g. "every 2nd"', 'ultimatewoo-pro' ), WC_Subscriptions::append_numeral_suffix( $i ) );
	}
...
function wcs_get_available_time_periods( $form = 'singular' ) {
	...
	$translated_periods = apply_filters( 'woocommerce_subscription_available_time_periods',
		array(
			'day'   => _nx( 'day',   'days',   $number, 'Used in the trial period dropdown. Number is in text field. 0, 2+ will need plural, 1 will need singular.', 'ultimatewoo-pro' ),
			'week'  => _nx( 'week',  'weeks',  $number, 'Used in the trial period dropdown. Number is in text field. 0, 2+ will need plural, 1 will need singular.', 'ultimatewoo-pro' ),
			'month' => _nx( 'month', 'months', $number, 'Used in the trial period dropdown. Number is in text field. 0, 2+ will need plural, 1 will need singular.', 'ultimatewoo-pro' ),
			'year'  => _nx( 'year',  'years',  $number, 'Used in the trial period dropdown. Number is in text field. 0, 2+ will need plural, 1 will need singular.', 'ultimatewoo-pro' ),
		)
	);
...
function wcs_get_subscription_trial_lengths( $subscription_period = '' ) {
	...
	foreach ( $all_trial_periods as $period => $trial_periods ) {
		$all_trial_periods[ $period ][0] = _x( 'no', 'no trial period', 'ultimatewoo-pro' );
	}
