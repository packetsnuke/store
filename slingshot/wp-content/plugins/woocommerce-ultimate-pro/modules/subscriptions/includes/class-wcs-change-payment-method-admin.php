...
class WCS_Change_Payment_Method_Admin {
	...
	public static function display_fields( $subscription ) {
		...
		if ( count( $valid_payment_methods ) > 1 ) {
			...
			$found_method = false;
			echo '<label>' . esc_html__( 'Payment Method', 'ultimatewoo-pro' ) . ':</label>';
			...
			echo '<strong>' . esc_html__( 'Payment Method', 'ultimatewoo-pro' ) . ':</strong><br/>' . esc_html( current( $valid_payment_methods ) );
			echo wcs_help_tip( sprintf( _x( 'Gateway ID: [%s]', 'The gateway ID displayed on the Edit Subscriptions screen when editing payment method.', 'ultimatewoo-pro' ), key( $valid_payment_methods ) ) );
	...
	public static function save_meta( $subscription ) {
		...
		if ( ! isset( $valid_payment_methods[ $payment_method ] ) && ! ( isset( $payment_gateways[ $payment_method ] ) && $subscription->get_payment_method() == $payment_gateways[ $payment_method ]->id ) ) {
			throw new Exception( __( 'Please choose a valid payment gateway to change to.', 'ultimatewoo-pro' ) );
		}
	...
	public static function get_valid_payment_methods( $subscription ) {
		...
		$valid_gateways = array( 'manual' => __( 'Manual Renewal', 'ultimatewoo-pro' ) );
