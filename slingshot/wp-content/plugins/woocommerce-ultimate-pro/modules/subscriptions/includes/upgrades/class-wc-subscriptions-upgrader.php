...
class WC_Subscriptions_Upgrader {
		...
		switch ( $_POST['upgrade_step'] ) {
			...
			case 'really_old_version':
				...
				$results = array(
					...
					'message' => sprintf( __( 'Database updated to version %s', 'ultimatewoo-pro' ), $upgraded_versions ),
				);
				break;
			...
			case 'products':
				...
				$results = array(
					...
					'message' => sprintf( _x( 'Marked %s subscription products as "sold individually".', 'used in the subscriptions upgrader', 'ultimatewoo-pro' ), $upgraded_product_count ),
				);
				break;
			...
			case 'hooks':
				...
				$results = array(
					...
					'message'        => sprintf( __( 'Migrated %1$s subscription related hooks to the new scheduler (in %2$s seconds).', 'ultimatewoo-pro' ), $upgraded_hook_count, '{execution_time}' ),
				);
				break;
			...
			case 'subscriptions':
				...
				try {
					...
					$results = array(
						...
						'message'        => sprintf( __( 'Migrated %1$s subscriptions to the new structure (in %2$s seconds).', 'ultimatewoo-pro' ), $upgraded_subscriptions, '{execution_time}' ),
						'status'         => 'success',
						// translators: placeholder is "{time_left}", will be replaced on front end with actual time
						'time_message'   => sprintf( _x( 'Estimated time left (minutes:seconds): %s', 'Message that gets sent to front end.', 'ultimatewoo-pro' ), '{time_left}' ),
					);
					...
					$results = array(
						...
						'message'        => sprintf( __( 'Unable to upgrade subscriptions.<br/>Error: %1$s<br/>Please refresh the page and try again. If problem persists, %2$scontact support%3$s.', 'ultimatewoo-pro' ), '<code>' . $e->getMessage(). '</code>', '<a href="' . esc_url( 'https://woocommerce.com/my-account/create-a-ticket/' ) . '">', '</a>' ),
						'status'         => 'error',
					);
				}
			...
			case 'subscription_dates_repair':
				...
				try {
					...
					$repair_incorrect = sprintf( _x( 'Repaired %d subscriptions with incorrect dates, line tax data or missing customer notes.', 'Repair message that gets sent to front end.', 'ultimatewoo-pro' ), $subscription_counts['repaired_count'] );
					...
					if ( $subscription_counts['unrepaired_count'] > 0 ) {
						...
						$repair_not_needed = sprintf( _nx( ' %d other subscription was checked and did not need any repairs.', '%d other subscriptions were checked and did not need any repairs.', $subscription_counts['unrepaired_count'], 'Repair message that gets sent to front end.', 'ultimatewoo-pro' ), $subscription_counts['unrepaired_count'] );
					}

					// translators: placeholder is "{execution_time}", which will be replaced on front end with actual time
					$repair_time = sprintf( _x( '(in %s seconds)', 'Repair message that gets sent to front end.', 'ultimatewoo-pro' ), '{execution_time}' );
					...
					$repair_message = sprintf( _x( '%1$s%2$s %3$s', 'The assembled repair message that gets sent to front end.', 'ultimatewoo-pro' ), $repair_incorrect, $repair_not_needed, $repair_time );
	...
	public static function updated_welcome_page() {
		$about_page = add_dashboard_page( __( 'Welcome to WooCommerce Subscriptions 2.1', 'ultimatewoo-pro' ), __( 'About WooCommerce Subscriptions', 'ultimatewoo-pro' ), 'manage_options', 'wcs-about', __CLASS__ . '::about_screen' );
