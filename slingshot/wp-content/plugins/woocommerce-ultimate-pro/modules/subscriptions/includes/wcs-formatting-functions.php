...
function wcs_price_string( $subscription_details ) {
	...
	$subscription_details = wp_parse_args( $subscription_details, array(
			'currency'              => '',
			'initial_amount'        => '',
			'initial_description'   => _x( 'up front', 'initial payment on a subscription', 'ultimatewoo-pro' ),
			'recurring_amount'      => '',
	...
	if ( $subscription_details['subscription_length'] > 0 && $subscription_details['subscription_length'] == $subscription_details['subscription_interval'] ) {
		if ( ! empty( $subscription_details['initial_amount'] ) ) {
			if ( $subscription_details['subscription_interval'] == $subscription_details['subscription_length'] && 0 == $subscription_details['trial_length'] ) {
				...
				$subscription_string = sprintf( __( '%1$s %2$s then %3$s', 'ultimatewoo-pro' ), $initial_amount_string, $subscription_details['initial_description'], $recurring_amount_string );
			}
		} else {
			...
			case 'week':
				...
				if ( 1 == $subscription_details['subscription_interval'] ) {
					if ( ! empty( $subscription_details['initial_amount'] ) ) {
						...
						$subscription_string = sprintf( __( '%1$s %2$s then %3$s every %4$s', 'ultimatewoo-pro' ), $initial_amount_string, $subscription_details['initial_description'], $recurring_amount_string, $payment_day_of_week );
					} else {
						// translators: 1$: recurring amount string, 2$: day of the week (e.g. "$10 every Wednesday")
						$subscription_string = sprintf( __( '%1$s every %2$s', 'ultimatewoo-pro' ), $recurring_amount_string, $payment_day_of_week );
					}
				} else {
					...
					if ( ! empty( $subscription_details['initial_amount'] ) ) {
						...
						$subscription_string = sprintf( __( '%1$s %2$s then %3$s every %4%s on %5$s', 'ultimatewoo-pro' ), $initial_amount_string, $subscription_details['initial_description'], $recurring_amount_string, wcs_get_subscription_period_strings( $subscription_details['subscription_interval'], $subscription_details['subscription_period'] ), $payment_day_of_week );
					} else {
						// translators: 1$: recurring amount string, 2$: period, 3$: day of the week (e.g. "$10 every 2nd week on Wednesday")
						$subscription_string = sprintf( __( '%1$s every %2$s on %3$s', 'ultimatewoo-pro' ), $recurring_amount_string, wcs_get_subscription_period_strings( $subscription_details['subscription_interval'], $subscription_details['subscription_period'] ), $payment_day_of_week );
					}
				}
				break;
			case 'month':
				if ( 1 == $subscription_details['subscription_interval'] ) {
					...
					if ( ! empty( $subscription_details['initial_amount'] ) ) {
						if ( $payment_day > 27 ) {
							...
							$subscription_string = sprintf( __( '%1$s %2$s then %3$s on the last day of each month', 'ultimatewoo-pro' ), $initial_amount_string, $subscription_details['initial_description'], $recurring_amount_string );
						} else {
							...
							$subscription_string = sprintf( __( '%1$s %2$s then %3$s on the %4$s of each month', 'ultimatewoo-pro' ), $initial_amount_string, $subscription_details['initial_description'], $recurring_amount_string, WC_Subscriptions::append_numeral_suffix( $payment_day ) );
						}
					} else {
						if ( $payment_day > 27 ) {
							// translators: placeholder is recurring amount
							$subscription_string = sprintf( __( '%s on the last day of each month', 'ultimatewoo-pro' ), $recurring_amount_string );
						} else {
							// translators: 1$: recurring amount, 2$: day of the month (e.g. "23rd") (e.g. "$5 every 23rd of each month")
							$subscription_string = sprintf( __( '%1$s on the %2$s of each month', 'ultimatewoo-pro' ), $recurring_amount_string, WC_Subscriptions::append_numeral_suffix( $payment_day ) );
						}
					}
				} else {
					...
					if ( ! empty( $subscription_details['initial_amount'] ) ) {
						if ( $payment_day > 27 ) {
							...
							$subscription_string = sprintf( __( '%1$s %2$s then %3$s on the last day of every %4$s month', 'ultimatewoo-pro' ), $initial_amount_string, $subscription_details['initial_description'], $recurring_amount_string, WC_Subscriptions::append_numeral_suffix( $subscription_details['subscription_interval'] ) );
						} else {
							...
							$subscription_string = sprintf( __( '%1$s %2$s then %3$s on the %4$s day of every %5$s month', 'ultimatewoo-pro' ), $initial_amount_string, $subscription_details['initial_description'], $recurring_amount_string, WC_Subscriptions::append_numeral_suffix( $payment_day ), WC_Subscriptions::append_numeral_suffix( $subscription_details['subscription_interval'] ) );
						}
					} else {
						if ( $payment_day > 27 ) {
							// translators: 1$: recurring amount, 2$: interval (e.g. "3rd") (e.g. "$10 on the last day of every 3rd month")
							$subscription_string = sprintf( __( '%1$s on the last day of every %2$s month', 'ultimatewoo-pro' ), $recurring_amount_string, WC_Subscriptions::append_numeral_suffix( $subscription_details['subscription_interval'] ) );
						} else {
							// translators: 1$: recurring amount, 2$: day of the month (e.g. "23rd") (e.g. "$5 every 23rd of each month")
							$subscription_string = sprintf( __( '%1$s on the %2$s day of every %3$s month', 'ultimatewoo-pro' ), $recurring_amount_string, WC_Subscriptions::append_numeral_suffix( $payment_day ), WC_Subscriptions::append_numeral_suffix( $subscription_details['subscription_interval'] ) );
						}
					}
				}
				break;
			case 'year':
				if ( 1 == $subscription_details['subscription_interval'] ) {
					...
					if ( ! empty( $subscription_details['initial_amount'] ) ) {
						...
						$subscription_string = sprintf( __( '%1$s %2$s then %3$s on %4$s %5$s each year', 'ultimatewoo-pro' ), $initial_amount_string, $subscription_details['initial_description'], $recurring_amount_string, $wp_locale->month[ $payment_day['month'] ], WC_Subscriptions::append_numeral_suffix( $payment_day['day'] ) );
					} else {
						// translators: 1$: recurring amount, 2$: month (e.g. "March"), 3$: day of the month (e.g. "23rd") (e.g. "$15 on March 15th every 3rd year")
						$subscription_string = sprintf( __( '%1$s on %2$s %3$s each year', 'ultimatewoo-pro' ), $recurring_amount_string, $wp_locale->month[ $payment_day['month'] ], WC_Subscriptions::append_numeral_suffix( $payment_day['day'] ) );
					}
				} else {
					...
					if ( ! empty( $subscription_details['initial_amount'] ) ) {
						...
						$subscription_string = sprintf( __( '%1$s %2$s then %3$s on %4$s %5$s every %6$s year', 'ultimatewoo-pro' ), $initial_amount_string, $subscription_details['initial_description'], $recurring_amount_string, $wp_locale->month[ $payment_day['month'] ], WC_Subscriptions::append_numeral_suffix( $payment_day['day'] ), WC_Subscriptions::append_numeral_suffix( $subscription_details['subscription_interval'] ) );
					} else {
						// translators: 1$: recurring amount, 2$: month (e.g. "March"), 3$: day of the month (e.g. "23rd") (e.g. "$15 on March 15th every 3rd year")
						$subscription_string = sprintf( __( '%1$s on %2$s %3$s every %4$s year', 'ultimatewoo-pro' ), $recurring_amount_string, $wp_locale->month[ $payment_day['month'] ], WC_Subscriptions::append_numeral_suffix( $payment_day['day'] ), WC_Subscriptions::append_numeral_suffix( $subscription_details['subscription_interval'] ) );
					}
				}
				break;
		}
		...
		$subscription_string = sprintf( _n( '%1$s %2$s then %3$s / %4$s', '%1$s %2$s then %3$s every %4$s', $subscription_details['subscription_interval'], 'ultimatewoo-pro' ), $initial_amount_string, $subscription_details['initial_description'], $recurring_amount_string, $subscription_period_string );
		...
		if ( true === $subscription_details['use_per_slash'] ) {
			$subscription_string = sprintf( _n( '%1$s / %2$s', '%1$s every %2$s', $subscription_details['subscription_interval'], 'ultimatewoo-pro' ), $recurring_amount_string, $subscription_period_string );
		} else {
			$subscription_string = sprintf( __( '%1$s every %2$s', 'ultimatewoo-pro' ), $recurring_amount_string, $subscription_period_string );
		}
	} else {
		$subscription_string = '';
	}
	...
	if ( $subscription_details['subscription_length'] > 0 ) {
		...
		$subscription_string = sprintf( __( '%1$s for %2$s', 'ultimatewoo-pro' ), $subscription_string, $subscription_ranges[ $subscription_details['subscription_period'] ][ $subscription_details['subscription_length'] ] );
	}
	...
	if ( $subscription_details['trial_length'] > 0 ) {
		...
		if ( ! empty( $subscription_details['initial_amount'] ) ) {
			...
			$subscription_string = sprintf( __( '%1$s after %2$s free trial', 'ultimatewoo-pro' ), $subscription_string, $trial_length );
		} else {
			// translators: 1$: trial length (e.g. "3 weeks"), 2$: subscription string (e.g. "$10 up front then $5 on March 23rd every 3rd year")
			$subscription_string = sprintf( __( '%1$s free trial then %2$s', 'ultimatewoo-pro' ), ucfirst( $trial_length ), $subscription_string );
		}
	}
...
function wcs_get_human_time_diff( $timestamp_gmt ) {
	...
	if ( $time_diff > 0 && $time_diff < WEEK_IN_SECONDS ) {
		...
		$date_to_display = sprintf( __( 'In %s', 'ultimatewoo-pro' ), human_time_diff( current_time( 'timestamp', true ), $timestamp_gmt ) );
	} elseif ( $time_diff < 0 && absint( $time_diff ) < WEEK_IN_SECONDS ) {
		// translators: placeholder is human time diff (e.g. "3 weeks")
		$date_to_display = sprintf( __( '%s ago', 'ultimatewoo-pro' ), human_time_diff( current_time( 'timestamp', true ), $timestamp_gmt ) );
	} else {
