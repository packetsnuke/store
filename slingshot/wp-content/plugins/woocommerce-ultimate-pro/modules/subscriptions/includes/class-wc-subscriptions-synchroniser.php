...
class WC_Subscriptions_Synchroniser {
	...
	public static function init() {
		...
		self::$sync_field_label      = __( 'Synchronise renewals', 'ultimatewoo-pro' );
		self::$sync_description      = __( 'Align the payment date for all customers who purchase this subscription to a specific day of the week or month.', 'ultimatewoo-pro' );
		// translators: placeholder is a year (e.g. "2016")
		self::$sync_description_year = sprintf( _x( 'Align the payment date for this subscription to a specific day of the year. If the date has already taken place this year, the first payment will be processed in %s. Set the day to 0 to disable payment syncing for this product.', 'used in subscription product edit screen', 'ultimatewoo-pro' ), gmdate( 'Y', wcs_date_to_time( '+1 year' ) ) );
	...
	public static function add_settings( $settings ) {
		...
		array_splice( $settings, $index, 0, array(
			...
			array(
				'name'          => __( 'Synchronisation', 'ultimatewoo-pro' ),
				'type'          => 'title',
				// translators: placeholders are opening and closing link tags
				'desc'          => sprintf( _x( 'Align subscription renewal to a specific day of the week, month or year. For example, the first day of the month. %sLearn more%s.', 'used in the general subscription options page', 'ultimatewoo-pro' ), '<a href="' . esc_url( 'http://docs.woocommerce.com/document/subscriptions/renewal-synchronisation/' ) . '">', '</a>' ),
				'id'            => self::$setting_id . '_title',
			),

			array(
				'name'          => self::$sync_field_label,
				'desc'          => __( 'Align Subscription Renewal Day', 'ultimatewoo-pro' ),
				'id'            => self::$setting_id,
				'default'       => 'no',
				'type'          => 'checkbox',
			),
			...
			array(
				'name'          => __( 'Prorate First Payment', 'ultimatewoo-pro' ),
				'desc'          => __( 'If a subscription is synchronised to a specific day of the week, month or year, charge a prorated amount for the subscription at the time of sign up.', 'ultimatewoo-pro' ),
	...
	public static function subscription_product_fields() {
		...
		if ( self::is_syncing_enabled() ) {
			...
			?><p class="form-field _subscription_payment_sync_date_day_field">
				...
				<span class="wrap">
					<input type="number" id="<?php echo esc_attr( self::$post_meta_key_day ); ?>" name="<?php echo esc_attr( self::$post_meta_key_day ); ?>" class="wc_input_subscription_payment_sync" value="<?php echo esc_attr( $payment_day ); ?>" placeholder="<?php echo esc_attr_x( 'Day', 'input field placeholder for day field for annual subscriptions', 'ultimatewoo-pro' ); ?>"  />

					<label for="<?php echo esc_attr( self::$post_meta_key_month ); ?>" class="wcs_hidden_label"><?php esc_html_e( 'Month for Synchronisation', 'ultimatewoo-pro' ); ?></label>
	...
	public static function get_billing_period_ranges( $billing_period = '' ) {
		...
		if ( empty( self::$billing_period_ranges ) ) {
			...
			foreach ( array( 'week', 'month', 'year' ) as $key ) {
				self::$billing_period_ranges[ $key ][0] = __( 'Do not synchronise', 'ultimatewoo-pro' );
			}
			...
			foreach ( $weekdays as $i => $weekly_billing_period ) {
				...
				self::$billing_period_ranges['week'][ $i ] = sprintf( __( '%s each week', 'ultimatewoo-pro' ), $weekly_billing_period );
			}
			...
			foreach ( range( 1, 27 ) as $i ) {
				...
				self::$billing_period_ranges['month'][ $i ] = sprintf( __( '%s day of the month', 'ultimatewoo-pro' ), WC_Subscriptions::append_numeral_suffix( $i ) );
			}
			self::$billing_period_ranges['month'][28] = __( 'Last day of the month', 'ultimatewoo-pro' );
		...
		if ( self::is_product_synced( $product ) ) {
			...
			if ( 0 != $first_payment_timestamp ) {
				...
				if ( $is_first_payment_today ) {
					$payment_date_string = __( 'Today!', 'ultimatewoo-pro' );
				} else {
				...
				if ( self::is_product_prorated( $product ) && ! $is_first_payment_today ) {
					...
					$first_payment_date = sprintf( __( 'First payment prorated. Next payment: %s', 'ultimatewoo-pro' ), $payment_date_string );
				} else {
					// translators: placeholder is a date
					$first_payment_date = sprintf( __( 'First payment: %s', 'ultimatewoo-pro' ), $payment_date_string );
				}
