...
printf( __( 'A subscription belonging to %1$s has been cancelled. Their subscription\'s details are as follows:', 'ultimatewoo-pro' ), $subscription->get_formatted_billing_full_name() );
...
if ( ! empty( $last_order_time_created ) ) {
	...
	echo sprintf( __( 'Last Order Date: %s', 'ultimatewoo-pro' ), date_i18n( wc_date_format(), $last_order_time_created ) ) . "\n";
}
...
if ( ! empty( $end_time ) ) {
	...
	echo sprintf( __( 'End of Prepaid Term: %s', 'ultimatewoo-pro' ), date_i18n( wc_date_format(), $end_time ) ) . "\n";
}

do_action( 'woocommerce_email_order_meta', $subscription, $sent_to_admin, $plain_text, $email );

echo "\n" . sprintf( _x( 'View Subscription: %s', 'in plain emails for subscription information', 'ultimatewoo-pro' ), wcs_get_edit_post_link( $subscription->get_id() ) ) . "\n";
