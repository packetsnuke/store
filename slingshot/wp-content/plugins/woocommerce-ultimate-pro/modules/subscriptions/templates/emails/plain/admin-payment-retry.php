...
echo sprintf( _x( 'The automatic recurring payment for order #%1$s from %2$s has failed. The payment will be retried %3$s.', 'In customer renewal invoice email', 'ultimatewoo-pro' ), $order->get_order_number(), $order->get_formatted_billing_full_name(), strtolower( wcs_get_human_time_diff( $retry->get_time() ) ) ) . "\n\n";
echo sprintf( __( 'The renewal order is as follows:', 'ultimatewoo-pro' ) ) . "\n\n";
