...
<tr class="shipping recurring-total <?php echo esc_attr( $recurring_cart_key ); ?>">
	...
	<td data-title="<?php echo esc_attr( $package_name ); ?>">
		<?php if ( WC_Subscriptions::is_woocommerce_pre( '2.6' ) && is_cart() ) : // WC < 2.6 did not allow string indexes for shipping methods on the cart page and there was no way to hook in ?>
			<?php echo wp_kses_post( wpautop( __( 'Recurring shipping options can be selected on checkout.', 'ultimatewoo-pro' ) ) ); ?>
		...
		<?php elseif ( ! WC()->customer->has_calculated_shipping() ) : ?>
			<?php echo wp_kses_post( wpautop( __( 'Shipping costs will be calculated once you have provided your address.', 'ultimatewoo-pro' ) ) ); ?>
		<?php else : ?>
			<?php echo wp_kses_post( apply_filters( 'woocommerce_no_shipping_available_html', wpautop( __( 'There are no shipping methods available. Please double check your address, or contact us if you need any help.', 'ultimatewoo-pro' ) ) ) ); ?>
		<?php endif; ?>
