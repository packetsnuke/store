...
<div class="woocommerce_account_subscriptions">
	...
	<h2><?php esc_html_e( 'My Subscriptions', 'ultimatewoo-pro' ); ?></h2>
	<?php endif; ?>
	...
	<thead>
		<tr>
			<th class="subscription-id order-number"><span class="nobr"><?php esc_html_e( 'Subscription', 'ultimatewoo-pro' ); ?></span></th>
			<th class="subscription-status order-status"><span class="nobr"><?php esc_html_e( 'Status', 'ultimatewoo-pro' ); ?></span></th>
			<th class="subscription-next-payment order-date"><span class="nobr"><?php echo esc_html_x( 'Next Payment', 'table heading', 'ultimatewoo-pro' ); ?></span></th>
			<th class="subscription-total order-total"><span class="nobr"><?php echo esc_html_x( 'Total', 'table heading', 'ultimatewoo-pro' ); ?></span></th>
	...
	<?php foreach ( $subscriptions as $subscription_id => $subscription ) : ?>
		<tr class="order">
			...
			<td class="subscription-status order-status" data-title="<?php esc_attr_e( 'Status', 'ultimatewoo-pro' ); ?>">
			...
			<td class="subscription-next-payment order-date" data-title="<?php echo esc_attr_x( 'Next Payment', 'table heading', 'ultimatewoo-pro' ); ?>">
				...
				<?php if ( ! $subscription->is_manual() && $subscription->has_status( 'active' ) && $subscription->get_time( 'next_payment' ) > 0 ) : ?>
					...
					$payment_method_to_display = sprintf( __( 'Via %s', 'ultimatewoo-pro' ), $subscription->get_payment_method_to_display() );
	...
	<?php else : ?>
		...
		<p class="no_subscriptions">
			...
			printf( esc_html__( 'You have no active subscriptions. Find your first subscription in the %sstore%s.', 'ultimatewoo-pro' ), '<a href="' . esc_url( apply_filters( 'woocommerce_subscriptions_message_store_url', get_permalink( wc_get_page_id( 'shop' ) ) ) ) . '">', '</a>' );
			?>
		</p>
