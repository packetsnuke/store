...
if ( empty( $subscription ) ) {
	...
	if ( ! isset( $wp->query_vars['view-subscription'] ) || 'shop_subscription' != get_post_type( absint( $wp->query_vars['view-subscription'] ) ) || ! current_user_can( 'view_order', absint( $wp->query_vars['view-subscription'] ) ) ) {
		echo '<div class="woocommerce-error">' . esc_html__( 'Invalid Subscription.', 'ultimatewoo-pro' ) . ' <a href="' . esc_url( wc_get_page_permalink( 'myaccount' ) ) . '" class="wc-forward">'. esc_html__( 'My Account', 'ultimatewoo-pro' ) .'</a>' . '</div>';
		return;
	}
...
<table class="shop_table subscription_details">
	<tr>
		<td><?php esc_html_e( 'Status', 'ultimatewoo-pro' ); ?></td>
	...
	<?php foreach ( array(
		'last_order_date_created' => _x( 'Last Order Date', 'admin subscription table header', 'ultimatewoo-pro' ),
		'next_payment'            => _x( 'Next Payment Date', 'admin subscription table header', 'ultimatewoo-pro' ),
		'end'                     => _x( 'End Date', 'table heading', 'ultimatewoo-pro' ),
		'trial_end'               => _x( 'Trial End Date', 'admin subscription table header', 'ultimatewoo-pro' ),
	...
	<?php if ( ! empty( $actions ) ) : ?>
		<tr>
			<td><?php esc_html_e( 'Actions', 'ultimatewoo-pro' ); ?></td>
			<td>
...
<?php if ( $notes = $subscription->get_customer_order_notes() ) :
	?>
	<h2><?php esc_html_e( 'Subscription Updates', 'ultimatewoo-pro' ); ?></h2>
	<ol class="commentlist notes">
		<?php foreach ( $notes as $note ) : ?>
		<li class="comment note">
			<div class="comment_container">
				<div class="comment-text">
					<p class="meta"><?php echo esc_html( date_i18n( _x( 'l jS \o\f F Y, h:ia', 'date on subscription updates list. Will be localized', 'ultimatewoo-pro' ), wcs_date_to_time( $note->comment_date ) ) ); ?></p>
...
<h2><?php esc_html_e( 'Subscription Totals', 'ultimatewoo-pro' ); ?></h2>
<table class="shop_table order_details">
	<thead>
		<tr>
			...
			<?php endif; ?>
			<th class="product-name"><?php echo esc_html_x( 'Product', 'table headings in notification email', 'ultimatewoo-pro' ); ?></th>
			<th class="product-total"><?php echo esc_html_x( 'Total', 'table heading', 'ultimatewoo-pro' ); ?></th>
		</tr>
	</thead>
	<tbody>
		<?php
						...
						<?php if ( $allow_remove_items ) : ?>
							<td class="remove_item">
								<?php if ( wcs_can_item_be_removed( $item, $subscription ) ) : ?>
									<?php $confirm_notice = apply_filters( 'woocommerce_subscriptions_order_item_remove_confirmation_text', __( 'Are you sure you want remove this item from your subscription?', 'ultimatewoo-pro' ), $item, $_product, $subscription );?>
