...
class WC_Subscriptions {
		...
		wc_register_order_type(
			...
			apply_filters( 'woocommerce_register_post_type_subscription',
				array(
					...
					'labels'              => array(
						'name'               => __( 'Subscriptions', 'ultimatewoo-pro' ),
						'singular_name'      => __( 'Subscription', 'ultimatewoo-pro' ),
						'add_new'            => _x( 'Add Subscription', 'custom post type setting', 'ultimatewoo-pro' ),
						'add_new_item'       => _x( 'Add New Subscription', 'custom post type setting', 'ultimatewoo-pro' ),
						'edit'               => _x( 'Edit', 'custom post type setting', 'ultimatewoo-pro' ),
						'edit_item'          => _x( 'Edit Subscription', 'custom post type setting', 'ultimatewoo-pro' ),
						'new_item'           => _x( 'New Subscription', 'custom post type setting', 'ultimatewoo-pro' ),
						'view'               => _x( 'View Subscription', 'custom post type setting', 'ultimatewoo-pro' ),
						'view_item'          => _x( 'View Subscription', 'custom post type setting', 'ultimatewoo-pro' ),
						'search_items'       => __( 'Search Subscriptions', 'ultimatewoo-pro' ),
						'not_found'          => self::get_not_found_text(),
						'not_found_in_trash' => _x( 'No Subscriptions found in trash', 'custom post type setting', 'ultimatewoo-pro' ),
						'parent'             => _x( 'Parent Subscriptions', 'custom post type setting', 'ultimatewoo-pro' ),
						'menu_name'          => __( 'Subscriptions', 'ultimatewoo-pro' ),
					),
					'description'         => __( 'This is where subscriptions are stored.', 'ultimatewoo-pro' ),
					'public'              => false,
					'show_ui'             => true,
	...
	private static function get_not_found_text() {
		...
		if ( true === apply_filters( 'woocommerce_subscriptions_not_empty', $subscriptions_exist ) ) {
			$not_found_text = __( 'No Subscriptions found', 'ultimatewoo-pro' );
		} else {
			$not_found_text = '<p>' . __( 'Subscriptions will appear here for you to view and manage once purchased by a customer.', 'ultimatewoo-pro' ) . '</p>';
			// translators: placeholders are opening and closing link tags
			$not_found_text .= '<p>' . sprintf( __( '%sLearn more about managing subscriptions &raquo;%s', 'ultimatewoo-pro' ), '<a href="http://docs.woocommerce.com/document/subscriptions/store-manager-guide/#section-3" target="_blank">', '</a>' ) . '</p>';
			// translators: placeholders are opening and closing link tags
			$not_found_text .= '<p>' . sprintf( __( '%sAdd a subscription product &raquo;%s', 'ultimatewoo-pro' ), '<a href="' . esc_url( WC_Subscriptions_Admin::add_subscription_url() ) . '">', '</a>' ) . '</p>';
		}
	...
	public static function register_post_status() {
		...
		$registered_statuses = apply_filters( 'woocommerce_subscriptions_registered_statuses', array(
			'wc-active'         => _nx_noop( 'Active <span class="count">(%s)</span>', 'Active <span class="count">(%s)</span>', 'post status label including post count', 'ultimatewoo-pro' ),
			'wc-switched'       => _nx_noop( 'Switched <span class="count">(%s)</span>', 'Switched <span class="count">(%s)</span>', 'post status label including post count', 'ultimatewoo-pro' ),
			'wc-expired'        => _nx_noop( 'Expired <span class="count">(%s)</span>', 'Expired <span class="count">(%s)</span>', 'post status label including post count', 'ultimatewoo-pro' ),
			'wc-pending-cancel' => _nx_noop( 'Pending Cancellation <span class="count">(%s)</span>', 'Pending Cancellation <span class="count">(%s)</span>', 'post status label including post count', 'ultimatewoo-pro' ),
		) );
	...
	public static function maybe_empty_cart( $valid, $product_id, $quantity, $variation_id = '' ) {
		...
		if ( $is_subscription && 'yes' != get_option( WC_Subscriptions_Admin::$option_prefix . '_multiple_purchase', 'no' ) ) {
			...
			wc_add_notice( __( 'A subscription renewal has been removed from your cart. Multiple subscriptions can not be purchased at the same time.', 'ultimatewoo-pro' ), 'notice' );
			...
			wc_add_notice( __( 'A subscription has been removed from your cart. Due to payment gateway restrictions, different subscription products can not be purchased at the same time.', 'ultimatewoo-pro' ), 'notice' );

		} elseif ( $cart_contains_subscription && 'yes' != get_option( WC_Subscriptions_Admin::$option_prefix . '_multiple_purchase', 'no' ) ) {

			self::remove_subscriptions_from_cart();

			wc_add_notice( __( 'A subscription has been removed from your cart. Products and subscriptions can not be purchased at the same time.', 'ultimatewoo-pro' ), 'notice' );
	...
	public static function order_button_text( $button_text ) {
		...
		if ( WC_Subscriptions_Cart::cart_contains_subscription() ) {
			$button_text = get_option( WC_Subscriptions_Admin::$option_prefix . '_order_button_text', __( 'Sign Up Now', 'ultimatewoo-pro' ) );
		}
	...
	public static function append_numeral_suffix( $number ) {
		...
		if ( strlen( $number ) > 1 && 1 == substr( $number, -2, 1 ) ) {
			...
			$number_string = sprintf( __( '%sth', 'ultimatewoo-pro' ), $number );
		} else { // Append relevant suffix
			switch ( substr( $number, -1 ) ) {
				case 1:
					// translators: placeholder is a number, numbers ending in 1
					$number_string = sprintf( __( '%sst', 'ultimatewoo-pro' ), $number );
					break;
				case 2:
					// translators: placeholder is a number, numbers ending in 2
					$number_string = sprintf( __( '%snd', 'ultimatewoo-pro' ), $number );
					break;
				case 3:
					// translators: placeholder is a number, numbers ending in 3
					$number_string = sprintf( __( '%srd', 'ultimatewoo-pro' ), $number );
					break;
				default:
...
<div id="message" class="error">
	<p><?php
		...
		printf( esc_html__( '%1$sWooCommerce Subscriptions is inactive.%2$s This version of Subscriptions requires WooCommerce 2.4 or newer. Please %3$supdate WooCommerce to version 2.4 or newer &raquo;%4$s', 'ultimatewoo-pro' ), '<strong>', '</strong>', '<a href="' . esc_url( admin_url( 'plugins.php' ) ) . '">', '</a>' ); ?>
	</p>
</div>
		<?php endif; ?>
	<?php endif;
	...
	public static function maybe_activate_woocommerce_subscriptions() {
		...
		if ( false == $is_active ) {
			...
			if ( ! get_term_by( 'slug', 'variable-subscription', 'product_type' ) ) {
				wp_insert_term( __( 'Variable Subscription', 'ultimatewoo-pro' ), 'product_type' );
			}
			...
			if ( ! empty( $_REQUEST['_wcsnonce'] ) && wp_verify_nonce( $_REQUEST['_wcsnonce'], 'wcs_duplicate_site' ) && isset( $_GET['wc_subscription_duplicate_site'] ) ) {
				...
				<div id="message" class="error">
					<p><?php
						...
						printf( esc_html__( 'It looks like this site has moved or is a duplicate site. %1$sWooCommerce Subscriptions%2$s has disabled automatic payments and subscription related emails on this site to prevent duplicate payments from a staging or test environment. %3$sLearn more &raquo;%4$s.', 'ultimatewoo-pro' ), '<strong>', '</strong>', '<a href="http://docs.woocommerce.com/document/subscriptions/faq/#section-39" target="_blank">', '</a>' ); ?></p>
					<div style="margin: 5px 0;">
						<a class="button button-primary" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'wc_subscription_duplicate_site', 'ignore' ), 'wcs_duplicate_site', '_wcsnonce' ) ); ?>"><?php esc_html_e( 'Quit nagging me (but don\'t enable automatic payments)', 'ultimatewoo-pro' ); ?></a>
						<a class="button" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'wc_subscription_duplicate_site', 'update' ), 'wcs_duplicate_site', '_wcsnonce' ) ); ?>"><?php esc_html_e( 'Enable automatic payments', 'ultimatewoo-pro' ); ?></a>
					</div>
				</div>
			<?php
			}
	...
	public static function action_links( $links ) {
		...
		$plugin_links = array(
			'<a href="' . WC_Subscriptions_Admin::settings_tab_url() . '">' . __( 'Settings', 'ultimatewoo-pro' ) . '</a>',
			'<a href="http://docs.woocommerce.com/document/subscriptions/">' . _x( 'Docs', 'short for documents', 'ultimatewoo-pro' ) . '</a>',
			'<a href="https://woocommerce.com/my-account/marketplace-ticket-form/">' . __( 'Support', 'ultimatewoo-pro' ) . '</a>',
		);
	...
	public static function update_notice( $plugin_data, $r ) {
		...
		$update_notice .= sprintf( __( 'Warning! Version 2.0 is a major update to the WooCommerce Subscriptions extension. Before updating, please create a backup, update all WooCommerce extensions and test all plugins, custom code and payment gateways with version 2.0 on a staging site. %sLearn more about the changes in version 2.0 &raquo;%s', 'ultimatewoo-pro' ), '<a href="http://docs.woocommerce.com/document/subscriptions/version-2/">', '</a>' );
		$update_notice .= '</div> ';
	...
	public static function show_downgrade_notice() {
		if ( version_compare( get_option( WC_Subscriptions_Admin::$option_prefix . '_active_version', '0' ), self::$version, '>' ) ) {
			...
			echo sprintf( esc_html__( 'Warning! You are running version %s of WooCommerce Subscriptions plugin code but your database has been upgraded to Subscriptions version 2.0. This will cause major problems on your store.', 'ultimatewoo-pro' ), esc_html( self::$version ) ) . '<br />';
			echo sprintf( esc_html__( 'Please upgrade the WooCommerce Subscriptions plugin to version 2.0 or newer immediately. If you need assistance, after upgrading to Subscriptions v2.0, please %sopen a support ticket%s.', 'ultimatewoo-pro' ), '<a href="https://woocommerce.com/my-account/marketplace-ticket-form/">', '</a>' );
			echo '</div> ';
