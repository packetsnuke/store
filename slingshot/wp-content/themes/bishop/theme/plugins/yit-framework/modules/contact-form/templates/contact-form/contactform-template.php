            ...
            <li class="submit-button col-sm-<?php echo $max_width ?> no-padding-right">
                ...
                if (function_exists('WC') && is_shop_installed() && is_product()) {
                    if (version_compare( WC()->version , '2.7', '>' ) ) {
                        $psku = $GLOBALS['product']->get_sku();
                        $pid = $GLOBALS['product']->get_id();
                    } else {
                        $psku = $GLOBALS['product']->sku;
                        $pid = $GLOBALS['product']->id;
                    }
                    ?>
                    <input type="hidden" name="yit_contact[sku]" value="<?php echo $psku ?> "/>
                    <input type="hidden" name="yit_contact[product_id]" value="<?php echo $pid ?> "/>
                    <?php
                }
                ?>
