...
function woocommerce_template_loop_product_thumbnail() {
    ...
    switch ( $yit_products_layout ) {
        ...
        case 'slideup':
            ...
            echo '<div class="quick-view'. ( ( yit_get_option( 'shop-use-quick-view' ) == 'no' ) ? ' none' : '' ) . '" data-product_id="' . esc_attr( $product->get_id() ) . '">';
            if ( yit_get_option( 'shop-use-quick-view' ) == 'yes' ) {
                ...
                echo '<a id="quick-view-trigger-' . esc_attr( $product->get_id() ) . $sc_index . '" href="#" class="trigger" data-item_id="'.$product->get_id().'">+ ' . yit_get_option( 'shop-quick-view-text' ) . '</a>';
            }
...
function yit_get_product_category() {
    global $product;
    echo '<span class="product_cat">' . ( function_exists('wc_get_product_category_list') ? wc_get_product_category_list( $product->get_id() ) : $product->get_categories() ) . '</span>';
}
...
function yit_related_posts_per_page() {
    ...
    $posts_per_page = (yit_get_option('shop-show-custom-related') =='yes') ? yit_get_option('shop-number-related') : 4;
    return array(
...
function yit_wc_3_3_removed_unused_template() {
    if ( function_exists( 'yit_remove_unused_template' ) ) {
        $option = 'yit_wc_3_3_1_template_remove';
        $files = array(
            'archive-product.php',
